    ...
    for (int i = 15; i >= 0; i--) {
        if (map->getMap() & (1 << i))
            trfprintf(pOutFile, "%s ",
                getName(machine->getARMRealRegister((TR::RealRegister::RegNum)(i + TR::RealRegister::FirstGPR))));
    }
...
static const char* opCodeToNameMap[] = {
    ...
    "strb", "strh", "stm", "stmdb", "swp", "fence", "ret", "wrtbar", "proc", "dd", "dmb_v6", "dmb", "dmb_st", "ldrex",
    "strex", "iflong", "setblong", "setbool", "setbflt", "lcmp", "flcmpl", "flcmpg", "idiv", "irem", "label", "vgdnop",
    "nop",
#if defined(__VFP_FP__) && !defined(__SOFTFP__)
