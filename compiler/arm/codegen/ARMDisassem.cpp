...
void loadStoreCoprocessor(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    case 6:
        ...
        sprintf(iBuf, "%s, [%s%s]", isDouble ? doubleRegStr(getVFPDestination(vfpInstr), instr->L)
...
void vfpExtension(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    case 12:
        ...
        sprintf(iBuf, "%s, %s", singleRegStr(getVFPDestination(instr), true), isDouble
                ? doubleRegStr(getVFPSecondOperand(instr), false)
                : singleRegStr(getVFPSecondOperand(instr), false));
        break;
    case 13:
...
void vfpSingleRegTransfer(int32_t* instrPtr, char* mBuf, char* iBuf)
{
    ...
    if (opCode == 0) {
        ...
        if (!isDouble) {
            ...
            if (instr->r) // L-bit
                sprintf(iBuf, "%s, %%s", regStr(instr->Fd, true), ((instr->Fn << 1 | instr->N) == 0)
                        ? "fpsid"
