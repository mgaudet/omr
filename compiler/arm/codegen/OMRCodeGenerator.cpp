...
void OMR::ARM::CodeGenerator::doBinaryEncoding()
{
    ...
    if (!comp->getOption(TR_DisableOOL)) {
        ...
        while (oiCursor) {
            ...
            bool needsETE = oiCursor->getFirstInstruction()->getNode()->getOpCode().hasSymbolReference() && oiCursor->getFirstInstruction()->getNode()->getSymbolReference() && oiCursor->getFirstInstruction()->getNode()->getSymbolReference()->canCauseGC();
