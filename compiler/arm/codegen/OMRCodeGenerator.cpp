...
// from f0 to gr0/gr1 following a directToJNI dispatch.
    ...
    self()->setMethodMetaDataRegister(
        self()->machine()->getARMRealRegister(_linkageProperties->getMethodMetaDataRegister()));
...
static void removeGhostRegistersFromGCMaps(TR::CodeGenerator* cg, TR::Instruction* branchOOL)
{
    ...
    // it's first use in the hot path
    // if it's also used on the cold path, since RA still needs to be done on the cold path (which is where it's future
    // use count will drop to 0), which
    // means it will be incorrectly seen as live between it's first use and the top of the hot path.
    // If there are any GC points between the top of the hot path and the first use the real reg holding the virtual
    // will be included,
            ...
            for (uint32_t regNum = TR::RealRegister::FirstGPR; regNum < TR::RealRegister::LastGPR; ++regNum) {
                ...
                if (map->getRegisterMap() & regMask) {
                    ...
                    if (regInRegMap->getState() == TR::RealRegister::Free) {
                        ...
                        // hot path was after the GC point
...
void OMR::ARM::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
    ...
    for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; i++) {
        TR::RealRegister* realReg = self()->machine()->getARMRealRegister((TR::RealRegister::RegNum)i);
...
{
    return self()->machine()->getARMRealRegister((TR::RealRegister::RegNum)self()->getGlobalRegister(i))->getState()
