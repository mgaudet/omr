...
void OMR::CodeGenerator::insertDebugCounters()
{
    ...
    for (tt = self()->comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
            ...
            if (block->getPredecessors().size() == 1) {
                ...
                if (predBlock->getEntry() && predBlock->getExit()) {
                    ...
                    if (predNode->getOpCode().isSwitch()) {
                        ...
                        TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "branchtargets/%s/(%s)/%s/%d/%d",
                                                                                  opName,
                                                                                  self()->comp()->signature(),
        ...
        if (node->getOpCode().isBranch() && !node->getOpCode().isSwitch()) {
            ...
            TR::DebugCounter::prependDebugCounter(self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "branches/%s/(%s)/%s/%d",
                                                                      opName,
                                                                      self()->comp()->signature(),
                                                                      self()->comp()->getHotnessName(self()->comp()->getMethodHotness()),
                                                                      tt->getNode()->getByteCodeIndex()),
