            ...
            if ((shftAmnt = TR::TreeEvaluator::checkPositiveOrNegativePowerOfTwo(value)) > 0) {
                ...
                } else // negative value of the multiply constant
                {
                    if (secondChild->getReferenceCount() == 1) {
                        ...
                        TR::Node* newChild = TR::Node::create(parent, TR::iconst, 0, shftAmnt);
                        TR::Node* newChild2;
                        isInteger ? newChild2 = TR::Node::create(parent, TR::ishl, 2) : newChild2
                            = TR::Node::create(parent, TR::lshl, 2);
    ...
    TR::Node* parent, TR::TreeTop* treeTop, vcount_t visitCount, TR::DataType storeType)
{
    ...
    // if the b2i is noped.
    if (opCode.isConversion() || (opCode.isAnd() && parent->getSecondChild()->getOpCode().isLoadConst()
        ...
        for (int32_t childCount = parent->getNumChildren() - 1; childCount >= 0; childCount--) {
            ...
            if (child->getVisitCount() != visitCount) {
                ...
                if ((isOneByte && (childOp == TR::i2b || childOp == TR::b2i || childOp == TR::bu2i))
                    ...
                    if (child->getReferenceCount() > 1 && (!treeTop->getNode()->getOpCode().isBooleanCompare()
                    ...
                    if (predNode->getOpCode().isSwitch()) {
                        ...
                        TR::DebugCounter::prependDebugCounter(
                            self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(),
                                                "branchtargets/%s/(%s)/%s/%d/%d", opName, self()->comp()->signature(),
