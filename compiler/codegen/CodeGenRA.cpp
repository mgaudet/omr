    ...
    TR::SymbolReference* symRef, bool checkForIMuls)
{
    ...
    while (treeTop) {
        ...
        self()->estimateRegisterPressure(node, currRegisterPressure, maxRegisterPressure,
            self()->comp()->cg()->getMaximumNumbersOfAssignableGPRs(), valuesInGlobalRegs, isCold, visitCount, symRef,
            symRefIsLive, checkForIMuls, vmThreadUsed);

        if (vmThreadUsed && (maxRegisterPressure >= self()->comp()->cg()->getMaximumNumbersOfAssignableGPRs())) {
...
TR_BackingStore* OMR::CodeGenerator::allocateVMThreadSpill()
{
    ...
    int32_t slot;
    TR::AutomaticSymbol* spillSymbol = TR::AutomaticSymbol::create(self()->trHeapMemory(), self()->IntJ(),
...
TR::Register* OMR::CodeGenerator::allocateRegister(TR_RegisterKinds rk)
{
    ...
    if (rk == TR_GPR && (!self()->getDebug() || !self()->getTraceRAOption(TR_TraceRASpillTemps)
                            || performTransformation(self()->comp(), "O^O SPILL TEMPS: Set UpperHalfIsDead on %s\n",
                                   self()->getDebug()->getName(temp)))) // allocateRegister is called for the vmthread
                                                                        // during initialization when getDebug has not
...
{
    bool enableHighWordGRA
        = self()->comp()->cg()->supportsHighWordFacility() && !self()->comp()->getOption(TR_DisableHighWordRA);
            ...
            if (block != self()->comp()->getFlowGraph()->getEnd() && !blockIsIgnorablyCold(block, self())) {
                ...
                if (blockWeight > hottestCopiedRegisterWeight) {
                    ...
                    for (TR::TreeTop* tt = block->getEntry(); tt != stopTree; tt = tt->getNextTreeTop()) {
                        ...
                        if (callNode && (self()->getLinkage()->isRecognizedBuiltin(self()->comp(), callNode)
                                ...
                                if (candidateRegister != -1) {
                                    if (!isUnpreferred) {
                                        if (remainingRegisters.isSet(candidateRegister)) {
                                            ...
                                            hottestCopiedRegisterWeight = blockWeight;
                                            if (self()->traceSimulateTreeEvaluation()) {
                                                traceMsg(self()->comp(), "            found call parameter index %i, "
                                    ...
                                    {
                                        if (self()->traceSimulateTreeEvaluation()) {
                                            traceMsg(self()->comp(), "            unprefer call parameter index %i, "
                                                                     "register %d (%s), weight %d, at %s\n",
                                                i, candidateRegister,
    ...
    TR::Node* callNode, TR_RegisterCandidate* rc, uint32_t childIndex, bool* isUnpreferred)
{
    ...
    if (paramNode->getOpCode().isLoadVarDirect()) {
        ...
        *isUnpreferred = !(paramSymRefNum == rc->getSymbolReference()->getReferenceNumber());
        TR_GlobalRegisterNumber candidateRegister
            = self()->getLinkage()->getInRegisterNumberForParameter(callNode, childIndex);
    ...
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (nodeState.hasRegister()) {
        ...
        if (isCall && (self()->getLinkage()->isRecognizedBuiltin(self()->comp(), node)
            ...
            if (isCall) {
                ...
                if ((numGPRTemporaries >= 1 || numFPRTemporaries >= 1 || numVRFTemporaries >= 1)
                    && (self()->comp()->getOption(TR_TraceRegisterPressureDetails)
                           && !performTransformation(self()->comp(), "O^O GLOBAL REGISTER ALLOCATION: Call %p has %d "
                                                                     "GPR and %d FPR and VRF % dummy registers\n",
    ...
    TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
                ...
                if (childIndex < maxChildrenInRegisters
                    ...
                    if (callNodeLinkage && self()->comp()->getOption(TR_TraceRegisterPressureDetails))
                        traceMsg(self()->comp(), " (%s arg %d in %s, %d left)", self()->getDebug()->getName(node),
                            childIndex, self()->getDebug()->getRegisterKindName(
    ...
    //   traceMsg(TR::comp(), "state->_gprPressure = %d summary->_gprPressure = %d summary->PRESSURE_LIMIT =
