            ...
            if (block != self()->comp()->getFlowGraph()->getEnd() && !blockIsIgnorablyCold(block, self())) {
                ...
                if (blockWeight > hottestCopiedRegisterWeight) {
                    ...
                    for (TR::TreeTop* tt = block->getEntry(); tt != stopTree; tt = tt->getNextTreeTop()) {
                        ...
                        if (callNode && (self()->getLinkage()->isRecognizedBuiltin(self()->comp(), callNode) || self()->getLinkage()->isAlwaysInlined(callNode))) {
                            callNode = NULL;
                        }
...
void OMR::CodeGenerator::simulateTreeEvaluation(TR::Node* node, TR_RegisterPressureState* state, TR_RegisterPressureSummary* summary)
{
    ...
    if (nodeState.hasRegister()) {
        ...
        if (isCall && (self()->getLinkage()->isRecognizedBuiltin(self()->comp(), node) || self()->getLinkage()->isAlwaysInlined(node))) {
            isCall = false;
        }
        ...
        if (state->candidateIsLiveAfterGRA()) {
            ...
            if (isCall) {
                ...
                if (!rcSymbol->isParm() || !linkage->isSpecialNonVolatileArgumentRegister(rcSymbol->getParmSymbol()->getLinkageRegisterIndex()))
