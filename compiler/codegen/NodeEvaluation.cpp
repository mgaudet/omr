...
OMR::CodeGenerator::evaluate(TR::Node* node)
{
    ...
    if (opcode != TR::BBStart && node->getRegister()) {
        ...
        if (self()->comp()->useAnchors()) {
            if (((node->getOpCode().isStoreIndirect() && (self()->comp()->useCompressedPointers() && (node->getSymbolReference()->getSymbol()->getDataType() == TR::Address))) || opcode == TR::wrtbari) && node->getReferenceCount() <= 2 && !checkRefCount)
                checkRefCount = true;
        }
