...
void OMR::CodeGenPhase::performProcessRelocationsPhase(TR::CodeGenerator* cg, TR::CodeGenPhase* phase)
{
    ...
    if (regex && TR::SimpleRegex::match(regex, comp->getCurrentMethod())) {
        if (TR::Compiler->target.is64Bit()) {
            setDllSlip(
                (char*)cg->getCodeStart(), (char*)cg->getCodeStart() + cg->getWarmCodeLength(), "SLIPDLL64", comp);
            if (cg->getColdCodeStart())
                setDllSlip((char*)cg->getColdCodeStart(), (char*)cg->getColdCodeStart() + cg->getColdCodeLength(),
                    "SLIPDLL64", comp);
        } else {
            setDllSlip(
                (char*)cg->getCodeStart(), (char*)cg->getCodeStart() + cg->getWarmCodeLength(), "SLIPDLL31", comp);
            if (cg->getColdCodeStart())
                setDllSlip((char*)cg->getColdCodeStart(), (char*)cg->getColdCodeStart() + cg->getColdCodeLength(),
                    "SLIPDLL31", comp);
        }
...
void OMR::CodeGenPhase::performEmitSnippetsPhase(TR::CodeGenerator* cg, TR::CodeGenPhase* phase)
{
    ...
    if (comp->getOption(TR_TraceCG) || comp->getOptions()->getTraceCGOption(TR_TraceCGPostBinaryEncoding)) {
        diagnostic("\nbuffer start = %8x, code start = %8x, buffer length = %d", cg->getBinaryBufferStart(),
            cg->getCodeStart(), cg->getEstimatedWarmLength());
        ...
        diagnostic("\nAmount of code memory allocated for this function        = %d"
            ...
            cg->getEstimatedMethodLength(), cg->getWarmCodeLength() + cg->getColdCodeLength(), snippetLength);
    }
}
...
void OMR::CodeGenPhase::performShrinkWrappingPhase(TR::CodeGenerator* cg, TR::CodeGenPhase* phase)
{
    TR::Compilation* comp = cg->comp();
