    ...
    , _lmmdFailed(false)
{
    ...
    _disableInternalPointers = self()->comp()->getOption(TR_MimicInterpreterFrameShape) || self()->comp()->getOptions()->realTimeGC() || self()->comp()->getOptions()->getOption(TR_DisableInternalPointers);
...
bool OMR::CodeGenerator::use64BitRegsOn32Bit()
{
    ...
    if (!(TR::Compiler->target.isZOS() || TR::Compiler->target.isLinux()))
        ...
        bool use64BitRegs = (longReg && !longRegHeur && self()->comp()->getJittedMethodSymbol()->mayHaveLongOps()) || (longReg && longRegHeur && self()->comp()->useLongRegAllocation());
        return use64BitRegs;
    }
    ...
    for (auto e = block->getSuccessors().begin(); e != block->getSuccessors().end(); ++e) {
        ...
        if (comp->getFlowGraph()->compareExceptionSuccessors(block, succ) == 0) {
            ...
            for (TR::TreeTop* tt = succ->getEntry(); tt != succ->getExit(); tt = tt->getNextTreeTop()) {
                ...
                if (aNode && (aNode->getOpCodeValue() == TR::tfinish || aNode->getOpCodeValue() == TR::monexit || ((aNode->getOpCodeValue() == TR::treetop || aNode->getOpCodeValue() == TR::NULLCHK) && aNode->getFirstChild()->getOpCodeValue() == TR::monexit))) {
                    if (traceIt)
...
void TR_SetMonitorStateOnBlockEntry::set(bool& lmmdFailed, bool traceIt)
{
    ...
    while (!_blocksToVisit.isEmpty()) {
        ...
        TR_Stack<TR::SymbolReference*>* monitorStack = (_liveMonitorStacks->Exists(block->getNumber())) ? _liveMonitorStacks->ElementAt(block->getNumber()) : NULL;
        ...
        for (TR::TreeTop* tt = block->getEntry();; tt = tt->getNextTreeTop()) {
            ...
            if ((node->getOpCode().isStore() && //only monents are represented by this store now
            ...
            } else if ((node->getOpCode().getOpCodeValue() == TR::monexitfence) && monitorStack && !monitorStack->isEmpty() && canPopMonitorStack(comp(), monitorStack, node, blockExitsMethod, traceIt)) {
                if (!block->getExceptionSuccessors().empty())
                    monitorExitFence++;
                ...
                if (monitorExitFence > 0) {
                    ...
                    for (; edge != block->getExceptionSuccessors().end(); ++edge) {
                        TR::Block* succBlock = toBlock((*edge)->getTo());
                        if (node->getByteCodeInfo().getCallerIndex() == succBlock->getEntry()->getNode()->getByteCodeInfo().getCallerIndex()) {
            ...
            if (tt == block->getExit()) {
                ...
                if (blockHasMonexit) {
                    ...
                    if ((monitorPoppedForExceptionSucc > 0) && monitorStackTop && needToPushMonitor(comp(), block, traceIt)) {
                        if (traceIt)
...
bool OMR::CodeGenerator::nodeMatches(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
    ...
    if (addr1 == addr2) {
        ...
        foundMatch = true;
    } else if (addr1->getOpCodeValue() == TR::aload && addr2->getOpCodeValue() == TR::aload && self()->getLinkage()->isAddressOfStaticSymRef(addr1->getSymbolReference()) && self()->getLinkage()->isAddressOfStaticSymRef(addr2->getSymbolReference()) && addr1->getSymbolReference() == addr2->getSymbolReference()) {
        foundMatch = true;
    } else if (addr1->getOpCodeValue() == TR::aload && addr2->getOpCodeValue() == TR::aload && self()->getLinkage()->isAddressOfPrivateStaticSymRef(addr1->getSymbolReference()) && self()->getLinkage()->isAddressOfPrivateStaticSymRef(addr2->getSymbolReference())) {
        foundMatch = true;
        ...
        if (addr1->getOpCodeValue() == TR::aload && addr2->getOpCodeValue() == TR::aload && addr1->getSymbolReference() == addr2->getSymbolReference()) {
            foundMatch = true;
        } else if (addr1->getOpCodeValue() == TR::aloadi && addr2->getOpCodeValue() == TR::aloadi && addr1->getSymbolReference() == addr2->getSymbolReference() && self()->nodeMatches(addr1->getFirstChild(), addr2->getFirstChild(), addressesUnderSameTreeTop)) {
            foundMatch = true;
        }
    }
...
TR::Instruction* OMR::CodeGenerator::getVirtualGuardForPatching(TR::Instruction* vgdnop)
{
    ...
    for (TR::Instruction* prevI = vgdnop->getPrev(); prevI; prevI = prevI->getPrev()) {
        if (prevI->isVirtualGuardNOPInstruction()) {
            ...
            if (mergeableGuard(prevI) && prevI->getNode()->getBranchDestination() == vgdnop->getNode()->getBranchDestination()) {
