...
TR::Node* generatePoisonNode(TR::Compilation* comp, TR::Block* currentBlock, TR::SymbolReference* liveAutoSymRef)
{
    ...
    if (comp->getOption(TR_TraceCG) && comp->getOption(TR_PoisonDeadSlots)) {
        if (poisoned)
            traceMsg(comp, "POISON DEAD SLOTS --- Live local %d  from parent block %d going dead .... poisoning slot "
                           "with node 0x%x .\n",
                liveAutoSymRef->getReferenceNumber(), currentBlock->getNumber(), storeNode);
        else
            traceMsg(comp, "POISON DEAD SLOTS --- Live local %d of unsupported type from parent block %d going dead "
                           ".... poisoning skipped.\n",
...
TR::CodeGenerator* OMR::CodeGenerator::self() { return static_cast<TR::CodeGenerator*>(this); }
...
{
    if (traceIt)
        traceMsg(comp(), "\tIn SMSOBE::addSuccessors for cfgNode %d, monitorStack %p dontPropagateMonitor %d "
                         "monitorType = %d callerIndex %d walkOlyExceptionSuccs %d\n",
    ...
    for (TR::CFGEdge* edge = succs.getFirst(); edge; edge = succs.getNext()) {
        ...
        if (succBlock->getEntry() && succBlock->getVisitCount() != _visitCount) {
            ...
            if (monitorType == MonitorEnter) {
                if (traceIt)
                    traceMsg(comp(),
                        "\tIn TR_SetMonitorStateOnBlockEntry::addSuccessors monitorType = MonitorEnter  block %d\n",
                        succBlock->getNumber());
            ...
            if (traceIt)
                traceMsg(comp(), "process succBlock %d propagate (t/f: %d) isCatchBlock=%d monitorType=%d "
                                 "callerIndex=%d entryCallerIndex=%d\n",
        ...
        if (callerIndex == -1) {
            if (comp->getJittedMethodSymbol()->isSynchronised()) {
                ...
                if (monitorStack->size() == 1 && !comp->getJittedMethodSymbol()->isStatic() && !blockExitsMethod) {
                    if (traceIt)
                        traceMsg(comp, "monitorStack is empty (except for special DLT sync object slot) for DLT "
        ...
        if (comp->getFlowGraph()->compareExceptionSuccessors(block, succ) == 0) {
            ...
            for (TR::TreeTop* tt = succ->getEntry(); tt != succ->getExit(); tt = tt->getNextTreeTop()) {
                ...
                if (aNode && (aNode->getOpCodeValue() == TR::tfinish || aNode->getOpCodeValue() == TR::monexit
                    ...
                    if (traceIt)
                        traceMsg(comp, "overriding identical exception decision because node %p in block %d is either "
                                       "monexit or tfinish",
...
void TR_SetMonitorStateOnBlockEntry::set(bool& lmmdFailed, bool traceIt)
{
    ...
    while (!_blocksToVisit.isEmpty()) {
        ...
        TR_Stack<TR::SymbolReference*>* monitorStack = (_liveMonitorStacks->Exists(block->getNumber()))
        ...
        for (TR::TreeTop* tt = block->getEntry();; tt = tt->getNextTreeTop()) {
            ...
            if (tt == block->getExit()) {
                ...
                if (traceIt)
                    traceMsg(comp(), "blockHasMonent=%d blockHasMonexit=%d dontPropagateMonitor=%d callerIndex=%d "
...
bool OMR::CodeGenerator::addressesMatch(TR::Node* addr1, TR::Node* addr2, bool addressesUnderSameTreeTop)
{
    ...
    if (!foundMatch && addressesUnderSameTreeTop) {
        ...
        // the block, is if both are first references)
        ...
        if (self()->isSupportedAdd(addr1) && self()->isSupportedAdd(addr2)
            ...
            if (traceInspect())
                traceMsg(comp(), "\t\tfound possibly matching additions : update addr1 to %s (%p), addr2 to %s (%p) "
...
void OMR::CodeGenerator::zeroOutAutoOnEdge(TR::SymbolReference* liveAutoSymRef, TR::Block* block, TR::Block* succBlock,
...
int32_t OMR::CodeGenerator::setEstimatedLocationsForSnippetLabels(int32_t estimatedSnippetStart, bool isWarm)
{
    // *this    swipeable for debugging purposes
    TR::Snippet* cursor;
    ...
    if (self()->hasTargetAddressSnippets()) {
        estimatedSnippetStart
            = self()->setEstimatedLocationsForTargetAddressSnippetLabels(estimatedSnippetStart, isWarm);
    }
...
TR::Instruction* OMR::CodeGenerator::saveOrRestoreRegisters(TR_BitVector* regs, TR::Instruction* cursor, bool doSaves)
{
        ...
        if (prevIdx != -1) {
            if (curIdx == prevIdx + 1) {
                ...
                if (i > 1) {
                    ...
                    //
                    traceMsg(
                        self()->comp(), "found pattern for start %d end %d at cursor %p\n", startIdx, endIdx, cursor);
                    cursor = self()->getLinkage()->composeSavesRestores(
                        cursor, startIdx, endIdx, -1 /*_mapRegsToStack[startIdx]*/, numRegs, doSaves);
                    savedRegs += i;
                } else {
                    // insert a single store/load for prevIdx
                    //
                    traceMsg(
                        self()->comp(), "pattern broken idx %d at cursor %p doSaves %d\n", prevIdx, cursor, doSaves);
                    if (doSaves)
    ...
    TR::Instruction* cursor, const char* name, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
    ...
    TR::DebugCounterAggregation* aggregatedCounters
        = self()->comp()->getPersistentInfo()->getDynamicCounters()->createAggregation(self()->comp());
...
void OMR::CodeGenerator::startRecordingRegisterUsage()
{
    self()->setReferencedRegisterList(new (self()->trHeapMemory())
            TR::list<OMR::RegisterUsage*>(getTypedAllocator<OMR::RegisterUsage*>(TR::comp()->allocator())));
...
void OMR::CodeGenerator::addAOTRelocation(
    TR_Relocation* r, char* generatingFileName, uintptr_t generatingLineNumber, TR::Node* node)
{
...
intptrj_t OMR::CodeGenerator::hiValue(intptrj_t address)
{
    if (self()->comp()->compileRelocatableCode()) // We don't want to store values using HI_VALUE at compile time,
                                                  // otherwise, we do this a 2nd time when we relocate (and new value is
                                                  // based on old one)
