...
public:
    TR_SetMonitorStateOnBlockEntry(
        TR::Compilation* c, CS2::TableOf<TR_Stack<TR::SymbolReference*>*, TR::Allocator>* liveMonitorStacks)
...
public:
    ...
    void incrementEventCounter(TR::Node* node, TR::SymbolReference* symRef, TR::CodeGenerator* cg)
    {
    ...
    bool mulDecompositionCostIsJustified(
        int numOfOperations, char bitPosition[], char operationType[], int64_t value); // no virt
    ...
    TR_GlobalRegisterNumber pickRegister(TR_RegisterCandidate*, TR::Block**, TR_BitVector& availableRegisters,
        TR_GlobalRegisterNumber& highRegisterNumber, TR_LinkHead<TR_RegisterCandidate>* candidates); // no virt
    ...
    void apply16BitLabelRelativeRelocation(
        int32_t* cursor, TR::LabelSymbol* label, int8_t d, bool isInstrOffset = false); // no virt
    void apply24BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol*); // no virt
    void apply16BitLoadLabelRelativeRelocation(
        TR::Instruction*, TR::LabelSymbol*, TR::LabelSymbol*, int32_t); // no virt
...
#include "codegen/RegisterPressureSimulatorInner.hpp"
    ...
    TR::Instruction* generateNop(
        TR::Node* node, TR::Instruction* instruction = 0, TR_NOPKind nopKind = TR_NOPStandard); // no virt, cast
