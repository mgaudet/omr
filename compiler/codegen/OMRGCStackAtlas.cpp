...
void OMR::GCStackAtlas::close(TR::CodeGenerator* cg)
{
    ...
    for (mapEntry = _mapList.getListHead(); mapEntry; mapEntry = next) {
        ...
        if (mapBytes == nextMap->getMapSizeInBytes() && map->getRegisterMap() == nextMap->getRegisterMap() && map->getHighWordRegisterMap() == nextMap->getHighWordRegisterMap() && !memcmp(map->getMapBits(), nextMap->getMapBits(), mapBytes) && (comp->getOption(TR_DisableShrinkWrapping) || (map->getRegisterSaveDescription() == nextMap->getRegisterSaveDescription())) && map->isByteCodeInfoIdenticalTo(nextMap)) {
