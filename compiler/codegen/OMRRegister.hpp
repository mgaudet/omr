...
#define TR_SSR_ASSERT()                                                                                                \
    TR_ASSERT(self()->getKind() != TR_SSR, "only non-TR_SSR registers use _totalUseCount : an aggregate or BCD type "  \
                                           "node has been evaluated as a non-aggregate or BCD node (missing o2x "      \
                                           "cast?)\n")
