...
bool OMR::TreeEvaluator::instanceOfOrCheckCastNeedSuperTest(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (castClassSymRef->isUnresolved()) {
        ...
        //
        if (castClassSym && (clazz = (TR_OpaqueClassBlock*)castClassSym->getStaticAddress()) && !TR::Compiler->cls.isClassArray(cg->comp(), clazz) && !TR::Compiler->cls.isInterfaceClass(cg->comp(), clazz) && !TR::Compiler->cls.isClassFinal(cg->comp(), clazz) && helperSym->preservesAllRegisters() && !TR::comp()->getOption(TR_OptimizeForSpace))
            return true;
    }
    return false;
}
...
bool OMR::TreeEvaluator::nodeIsIAddOverflowCheck(TR::Node* node, TR_ArithmeticOverflowCheckNodes* u)
{
    ...
    static TR_Pattern* pattern = pb.choice(pb(TR::ificmpge), pb(TR::ificmplt), pb.children(
                                                                                   pb(TR::iand,
                                                                                       pb(TR::ixor,
...
bool OMR::TreeEvaluator::nodeIsLAddOverflowCheck(TR::Node* node, TR_ArithmeticOverflowCheckNodes* u)
{
    ...
    TR_PatternBuilder pb(comp);
    static TR_Pattern* pattern = pb.choice(pb(TR::iflcmpge), pb(TR::iflcmplt), pb.children(
                                                                                   pb(TR::land,
                                                                                       pb(TR::lxor,
