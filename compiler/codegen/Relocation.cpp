...
void TR_Relocation::apply(TR::CodeGenerator* codeGen) { TR_ASSERT(0, "Should never get here"); }

void TR_Relocation::trace(TR::Compilation* comp) { TR_ASSERT(0, "Should never get here"); }

void TR_Relocation::setDebugInfo(TR_RelocationDebugInfo* info) { this->_genData = info; }
...
void TR_8BitLabelRelativeRelocation::apply(TR::CodeGenerator* codeGen)
{
    ...
    AOTcgDiag2(
        codeGen->comp(), "TR_8BitLabelRelativeRelocation::apply cursor=%x label=%x\n", getUpdateLocation(), getLabel());
...
void TR_16BitLoadLabelRelativeRelocation::apply(TR::CodeGenerator* codeGen)
{
    AOTcgDiag3(codeGen->comp(),
        "TR_16BitLoadLabelRelativeRelocation::apply lastInstruction=%x startLabel=%x endLabel=%x\n",
...
{
    AOTcgDiag3(codeGen->comp(),
        "TR_32BitLoadLabelRelativeRelocation::apply lastInstruction=%x startLabel=%x endLabel=%x\n",
        getLastInstruction(), getStartLabel(), getEndLabel());
    ...
    if (comp->getOption(TR_AOT)) {
        ...
        for (r = aot.getFirst(); r != 0; r = r->getNext()) {
            if (r->getTargetAddress2())
                AOTcgDiag6(comp, "r=%x full=%x target=%x target2=%x, kind=%x modifier=%x\n", r, r->full(),
                    r->getTargetAddress(), r->getTargetAddress2(), r->getTargetKind(), r->getModifierValue());
            else
                AOTcgDiag5(comp, "r=%x full=%x target=%x kind=%x modifier=%x\n", r, r->full(), r->getTargetAddress(),
            ...
            if (r->full() == false && r->getTargetAddress() == _targetAddress
                ...
                if (r->getTargetAddress2())
                    ...
                    AOTcgDiag5(comp, "r=%x full=%x target=%x kind=%x modifier=%x\n", r, r->full(),
                        r->getTargetAddress(), r->getTargetKind(), r->getModifierValue());
                AOTcgDiag2(comp, "#sites=%x size=%x\n", r->getNumberOfRelocationSites(), r->getSizeOfRelocationData());
                return;
            }
        }
        TR_IteratedExternalRelocation* temp = _targetAddress2
            ? new (codeGen->trHeapMemory())
                  TR_IteratedExternalRelocation(_targetAddress, _targetAddress2, _kind, modifier, codeGen)
        ...
        if (_targetAddress2)
            AOTcgDiag6(comp, "temp=%x full=%x target=%x target2=%x kind=%x modifier=%x\n", temp, temp->full(),
                temp->getTargetAddress(), temp->getTargetAddress2(), temp->getTargetKind(), temp->getModifierValue());
        else
            AOTcgDiag5(comp, "temp=%x full=%x target=%x kind=%x modifier=%x\n", temp, temp->full(),
                temp->getTargetAddress(), temp->getTargetKind(), temp->getModifierValue());
...
TR_32BitExternalOrderedPairRelocation::TR_32BitExternalOrderedPairRelocation(uint8_t* location1, uint8_t* location2,
...
char* TR_ExternalRelocation::_externalRelocationTargetKindNames[TR_NumExternalRelocationKinds] = {
    "TR_ConstantPool (0)", "TR_HelperAddress (1)", "TR_RelativeMethodAddress (2)", "TR_AbsoluteMethodAddress (3)",
    "TR_DataAddress (4)", "TR_ClassObject (5)", "TR_MethodObject (6)", "TR_InterfaceObject (7)",
    "TR_AbsoluteHelperAddress (8)", "TR_FixedSequenceAddress (9)", "TR_FixedSequenceAddress2 (10)",
    "TR_JNIVirtualTargetAddress (11)", "TR_JNIStaticTargetAddress  (12)", "TR_ArrayCopyHelper (13)",
    "TR_ArrayCopyToc (14)", "TR_BodyInfoAddress (15)", "TR_Thunks (16)", "TR_StaticRamMethodConst (17)",
    "TR_Trampolines (18)", "TR_PicTrampolines (19)", "TR_CheckMethodEnter(20)", "TR_RamMethod (21)",
    "TR_RamMethodSequence (22)", "TR_RamMethodSequenceReg (23)", "TR_VerifyClassObjectForAlloc (24)",
    "TR_ConstantPoolOrderedPair (25)", "TR_AbsoluteMethodAddressOrderedPair (36)", "TR_VerifyRefArrayForAlloc (27)",
    "TR_J2IThunks (28)", "TR_GlobalValue (29)", "TR_BodyInfoAddressLoad (30)", "TR_ValidateInstanceField (31)",
    "TR_InlinedStaticMethodWithNopGuard (32)", "TR_InlinedSpecialMethodWithNopGuard (33)",
    "TR_InlinedVirtualMethodWithNopGuard (34)", "TR_InlinedInterfaceMethodWithNopGuard (35)",
    "TR_SpecialRamMethodConst (36)", "TR_InlinedHCRMethod (37)", "TR_ValidateStaticField (38)", "TR_ValidateClass (39)",
    "TR_ClassAddress (40)", "TR_HCR (41)", "TR_ProfiledMethodGuardRelocation (42)",
    "TR_ProfiledClassGuardRelocation (43)", "TR_HierarchyGuardRelocation (44)", "TR_AbstractGuardRelocation (45)",
    "TR_ProfiledInlinedMethodRelocation (46)", "TR_MethodPointer (47)", "TR_ClassPointer (48)",
    "TR_CheckMethodExit (49)", "TR_ValidateArbitraryClass (50)", "TR_EmitClass (51)", "TR_JNISpecialTargetAddress (52)",
...
void TR_IteratedExternalRelocation::addRelocationEntry(uint32_t locationOffset)
{
    ...
    TR::Compilation* comp = TR::comp();
    AOTcgDiag1(
        comp, "TR_IteratedExternalRelocation::addRelocationEntry _relocationDataCursor=%x\n", _relocationDataCursor);
    if (!needsWideOffsets()) {
