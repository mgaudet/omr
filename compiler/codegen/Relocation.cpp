...
void TR_ExternalRelocation::addAOTRelocation(TR::CodeGenerator* codeGen)
{
    ...
    if (comp->getOption(TR_AOT)) {
        ...
        TR_IteratedExternalRelocation* temp = _targetAddress2 ? new (codeGen->trHeapMemory()) TR_IteratedExternalRelocation(_targetAddress, _targetAddress2, _kind, modifier, codeGen) : new (codeGen->trHeapMemory()) TR_IteratedExternalRelocation(_targetAddress, _kind, modifier, codeGen);
...
uint8_t TR_32BitExternalOrderedPairRelocation::collectModifier()
{
    ...
    if (TR::Compiler->target.cpu.isPower() && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethod || kind == TR_GlobalValue || kind == TR_BodyInfoAddressLoad || kind == TR_DataAddress)) {
        TR::Instruction* instr = (TR::Instruction*)getUpdateLocation();
...
void TR_32BitExternalOrderedPairRelocation::apply(TR::CodeGenerator* codeGen)
{
    ...
    if (comp->getOption(TR_AOT)) {
        ...
        TR_ExternalRelocationTargetKind kind = getRelocationRecord()->getTargetKind();
        if (TR::Compiler->target.cpu.isPower() && (kind == TR_ArrayCopyHelper || kind == TR_ArrayCopyToc || kind == TR_RamMethodSequence || kind == TR_GlobalValue || kind == TR_BodyInfoAddressLoad || kind == TR_DataAddress)) {
            TR::Instruction* instr = (TR::Instruction*)getUpdateLocation();
            TR::Instruction* instr2 = (TR::Instruction*)getLocation2();
