...
char* TR_StorageInfo::TR_StorageClassNames[TR_NumStorageClassTypes] = {
    "<unknown_class>", "<direct_mapped_auto>", "<direct_mapped_static>", "<static_base_address>",
    "<private_static_base_address>",
};

char* TR_StorageInfo::TR_StorageOverlapKindNames[TR_NumOverlapTypes] = {
    "NoOverlap", "MayOverlap", "TR_PostPosOverlap", "TR_SamePosOverlap", "TR_DestructiveOverlap", "TR_PriorPosOverlap",
};
    ...
    if (_address->getOpCode().hasSymbolReference() && _address->getSymbolReference()) {
        ...
        if (_address->getOpCodeValue() == TR::loadaddr) {
            ...
            if (comp()->cg()->getLinkage()->isAddressOfStaticSymRef(_symRef)) {
                ...
                // the base address of the private static area has been cached in _symRef and is being loaded here with
                // an aload
...
TR_StorageOverlapKind TR_StorageInfo::mayOverlapWith(TR_StorageInfo* info)
{
    ...
    // fundamental ways
    // of accessing statics) but if they are both present then they may overlap as the TR_StaticBaseAddress points to
    // the start of the
