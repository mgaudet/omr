...
uint8_t OMR::Compilation::_NOPTranslateTable[TR_NOP_TRANSLATE_TABLE_SIZE] = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
    0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19,
    ...
    0x79, 0x7a, 0x7b, 0x7c, 0x7d, 0x7e, 0x7f, 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x87, 0x88, 0x89, 0x8a, 0x8b,
...
OMR::Compilation::Compilation(int32_t id, OMR_VMThread* omrVMThread, TR_FrontEnd* fe, TR_ResolvedMethod* compilee,
    TR::IlGenRequest& ilGenRequest, TR::Options& options, const TR::Region& dispatchRegion, TR_Memory* m,
    ...
    , _tlsManager(*self())
{
    ...
    if (TR::isJ9()) {
        ...
        _ObjectClassPointer = _RunnableClassPointer = _StringClassPointer = _SystemClassPointer = _ReferenceClassPointer
            = _JITHelpersClassPointer = NULL;
    }
    ...
    if (self()->isDLT() || (((self()->getMethodHotness() < warm) || self()->compileRelocatableCode()
    ...
    if (_options->getOption(TR_EnableOSR)) {
        ...
        if (((self()->getMethodHotness() < warm) || self()->compileRelocatableCode()
            ...
            && !_options->getOption(TR_FullSpeedDebug)) // Off for two reasons : 1) not sure if we can afford the
                                                        // increase in compile time due to the extra OSR control flow at
                                                        // cold and 2) not sure at this stage in 727 whether OSR can
                                                        // work with AOT (will try to find out soon) but disabling till
                                                        // I do find out
...
bool OMR::Compilation::canAffordOSRControlFlow()
{
    if (self()->getOption(TR_DisableOSR) || !self()->getOption(TR_EnableOSR)) {
        if (self()->getOption(TR_TraceOSR)) {
            traceMsg(self(),
                "canAffordOSRControlFlow returning false due to OSR options: disableOSR %d, enableOSR %d\n",
...
bool OMR::Compilation::supportsInduceOSR()
{
    ...
    if (_osrCompilationData && _osrCompilationData->seenClassPreventingInducedOSR()) {
        if (self()->getOption(TR_TraceOSR))
            traceMsg(self(), "Cannot guarantee OSR transfer of control to the interpreter will work for calls "
        ...
        if ((self()->getMethodHotness() <= warm)) {
            ...
            if (TR::Compiler->target.cpu.isX86())
                TR::Options::getCmdLineOptions()->setOption(
                    TR_DisableLateEdgeSplitting); // TR_DisableLateEdgeSplitting only used on X86
        }
                            ...
                            if (method) {
                                ...
                                if (callerIndex != -1) {
                                    int32_t callerLen = self()->fej9()->printTruncatedSignature(callerBuf,
                                        sizeof(callerBuf) - 1, self()->fej9()->getInlinedCallSiteMethod(
                                ...
                                if (node->getSymbol()->getResolvedMethodSymbol())
                                    calleeSize = self()->fej9()->getMethodSize(node->getSymbol()
...
int32_t OMR::Compilation::performOptimizations()
{
    ...
    if (!self()->getOption(TR_EnableSpecializedEpilogues) && self()->getOption(TR_DisableShrinkWrapping)
...
void OMR::Compilation::addMonitorAuto(TR::RegisterMappedSymbol* a, int32_t callerIndex)
{
    ...
    if (!autos)
        _monitorAutos[callerIndex + 1] = autos
...
{
    TR_ASSERT(guardNode->isTheVirtualGuardForAGuardedInlinedCall() || guardNode->isHCRGuard()
...
{
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
        if ((*current)->getKind() != TR_HCRGuard && ((*current)->getCalleeIndex() == guard->getCalleeIndex())
...
bool OMR::Compilation::isVirtualGuardNOPingRequired(TR_VirtualGuard* virtualGuard)
{
    if (self()->isProfilingCompilation()) {
        if (!virtualGuard) {
            ...
            || (virtualGuard->getKind() == TR_HCRGuard) || (virtualGuard->getKind() == TR_MutableCallSiteTargetGuard)
