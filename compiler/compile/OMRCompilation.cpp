    ...
    , _tlsManager(*self())
{
    ...
    _isOptServer = (!_options->getOption(TR_NoOptServer)) && (_options->getOption(TR_Server)
#ifdef J9_PROJECT_SPECIFIC
...
bool OMR::Compilation::isPotentialOSRPoint(TR::TreeTop* tt, TR::Node* ttNode)
{
    ...
    if (self()->getHCRMode() == TR::osr) {
        if (node->getOpCodeValue() == TR::asynccheck) {
            ...
            if (callSymRef->getReferenceNumber() > self()->getSymRefTab()->getNonhelperIndex(self()->getSymRefTab()->getLastCommonNonhelperSymbol()))
...
int32_t OMR::Compilation::performOptimizations()
{
    ...
    if (!self()->getOption(TR_EnableSpecializedEpilogues) && self()->getOption(TR_DisableShrinkWrapping) && !TR::Compiler->target.cpu.isZ() && // 390 now uses UseDefs in CodeGenPrep
...
OMR::Compilation::findVirtualGuardInfo(TR::Node* guardNode)
{
    ...
    if (guardKind != TR_NoGuard) {
        ...
        for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
            ...
            if ((*current)->getByteCodeIndex() == guardNode->getByteCodeInfo().getByteCodeIndex() && (*current)->getCalleeIndex() == guardNode->getByteCodeInfo().getCallerIndex() && (*current)->getKind() != TR_SideEffectGuard && (*current)->getKind() != TR_HCRGuard && (*current)->getKind() != TR_MethodEnterExitGuard && (*current)->getKind() != TR_MutableCallSiteTargetGuard) {
                guard = *current;
                if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
...
{
    for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current) {
        if ((*current)->getKind() != TR_HCRGuard && ((*current)->getCalleeIndex() == guard->getCalleeIndex()) && ((*current)->getByteCodeIndex() == guard->getByteCodeIndex())) {
            if (self()->getOption(TR_TraceRelocatableDataDetailsCG))
        ...
        if (!virtualGuard) {
            for (auto current = _virtualGuards.begin(); current != _virtualGuards.end(); ++current)
                if (self()->isVirtualGuardNOPingRequired(*current))
                    return true;
            return false;
        } else if ((virtualGuard->getKind() == TR_SideEffectGuard) || (virtualGuard->getKind() == TR_DummyGuard) || (virtualGuard->getKind() == TR_HCRGuard) || (virtualGuard->getKind() == TR_MutableCallSiteTargetGuard) || ((virtualGuard->getKind() == TR_InterfaceGuard) && (virtualGuard->getTestType() == TR_MethodTest)))
            return true;
        else
            return false;
    } else
...
bool OMR::Compilation::notYetRunMeansCold()
{
    ...
    intptrj_t initialCount = currentMethod->hasBackwardBranches() ? self()->getOptions()->getInitialBCount() : self()->getOptions()->getInitialCount();

    if (currentMethod->convertToMethod()->isBigDecimalMethod() || currentMethod->convertToMethod()->isBigDecimalConvertersMethod())
        initialCount = 0;
