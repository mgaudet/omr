...
TR::SymbolReference* OMR::SymbolReferenceTable::findOrCreateNewStructHeaderSymbolRef(TR::ResolvedMethodSymbol*)
{
...
void OMR::SymbolReferenceTable::initRegisterSymbols(TR::ResolvedMethodSymbol* owningMethodSymbol)
{
    ...
    for (i = 0; i < nr; i++) {
        TR::SymbolReference* symRef
            = createRegisterSymbol(owningMethodSymbol, machine->getRealRegister(i)->getKind(), TR::NoType, i);
...
TR::SymbolReference* OMR::SymbolReferenceTable::createRegisterSymbol(TR::ResolvedMethodSymbol* owningMethodSymbol,
    TR_RegisterKinds regKind, TR::DataType dataType, TR_GlobalRegisterNumber grn)
{
    ...
    TR::SymbolReference* symRef
        = new (trHeapMemory()) TR::SymbolReference(self(), sym, owningMethodIndex, 0 /* slot */);
    ...
    TR::ResolvedMethodSymbol* owningMethodSymbol, int32_t cpIndex, void* classObject, bool cpIndexOfStatic)
{
    ...
    // contains
    // the same resolved address.  As a result we can end up sharing symbol references between an explicit reference to
    // the class
    // and an implicit reference used on a write barrier node.   In such a case we make sure that the shared symref uses
    // the
    ...
    if (fej9->getSupportsRecognizedMethods()) {
        TR_ASSERT(cpIndex != -1 || !comp()->compileRelocatableCode(), "we missed one of the cases where enabling "
                                                                      "recognized methods triggers a creation of a "
                                                                      "classSymbol with CPI of -1");
    }
#endif
    ...
    for (; symRef; symRef = i.getNext())
        ...
        // dual symref match, when original type is an address, and needs to be coerced
        {
            ...
            if (symRef->getSymbol()->getParmSymbol() || comp()->getOption(TR_DontJitIfSlotsSharedByRefAndNonRef)) {
                comp()->fe()->outOfMemory(
                    comp(), "stack mapping can't handle a parameter slot that is shared between refs and nonrefs 0");
            }
