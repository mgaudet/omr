    ...
    TR::Node* callNode, TR::TreeTop* destination, TR_OpaqueClassBlock* thisClass)
{
    ...
    TR_ASSERT(((callNode->getNumChildren() == 3) && callNode->getSecondChild() && callNode->getSecondChild()->getOpCodeValue() == TR::aconst), "Unexpected children in hierarchy of vm_send_without_block when creating ruby inline guard.");
