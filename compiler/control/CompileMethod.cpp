    ...
    OMR_VMThread* omrVMThread, TR::IlGeneratorMethodDetails& details, TR_Hotness hotness, int32_t& rc)
{
    ...
    if (!methodCanBeCompiled(&fe, compilee, filterInfo, &trMemory)) {
        ...
        options = new (trMemory.trHeapMemory())
            TR::Options(&trMemory, optionSetIndex, lineNumber, &compilee, 0, plan, false);
        ...
        TR_ASSERT(TR::comp() == NULL, "there seems to be a current TLS TR::Compilation object %p for this thread. At "
                                      "this point there should be no current TR::Compilation object",
            TR::comp());
        ...
        try {
            ...
            if (rc == 0) // success!
            {
                ...
                if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileEnd)
                    || TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerboseCompileTime)) {
                    const char* signature = compilee.signature(&trMemory);
                    TR_VerboseLog::writeLineLocked(TR_Vlog_COMP, "(%s @ %#p t=%lldms  %lld.%lldms) %s",
                        ...
                        TR::Compiler->vm.getUSecClock() - fe.getStartTime(), translationTime / 1000,
                        translationTime % 1000, signature);
                    trfflush(jitConfig->options.vLogFile);
                }

                if (TR::Options::getCmdLineOptions()->getOption(TR_PerfTool))
                    generatePerfToolEntry(startPC, compiler.cg()->getCodeEnd(), compiler.signature(),
