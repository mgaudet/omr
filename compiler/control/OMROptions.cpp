    ...
    TR::OptionSet* optionSet)
{
    ...
    while (1) {
        ...
        //
        if (opt->name[opt->length - 1] == '=' && strlen(option) >= opt->length && option[opt->length - 1] == '@' && strnicmp_ignore_locale(option, opt->name, opt->length - 1) == 0) {
            cmp = 0;
        }
    ...
    for (i++; i < numEntries; i++) {
        ...
        if (next->name[next->length - 1] == '=' && strlen(option) >= next->length && option[next->length - 1] == '@' && strnicmp_ignore_locale(option, next->name, next->length - 1) == 0) {
            cmp = 0;
        }
        if (cmp < 0) {
...
bool OMR::Options::requiresLogFile()
{
    ...
    //
    if (self()->getAnyOption(TR_TraceAll) || self()->getAnyOption(TR_TraceBBVA) || self()->getAnyOption(TR_TraceBVA) || self()->getAnyOption(TR_TraceUseDefs) || self()->getAnyOption(TR_TraceNodeFlags) || self()->getAnyOption(TR_TraceValueNumbers) || self()->getAnyOption(TR_TraceLiveness) || self()->getAnyOption(TR_TraceILGen))
        return true;
