...
TR::OptionTable OMR::Options::_jitOptions[] = {
    ...
    { "aggressiveRecompilationChances=", "O<nnn>\tnumber of chances per method to recompile with the "
        ...
        NOT_IN_SUBSET },
    { "allowVPRangeNarrowingBasedOnDeclaredType", "I\tallow value propagation to assume that integers declared "
    ...
    { "aot", "O\tahead-of-time compilation", SET_OPTION_BIT(TR_AOT), NULL, NOT_IN_SUBSET },
    ...
    { "blockShufflingSequence=", "D<string>\tDescription of the particular block shuffling operations to perform; see "
    ...
    { "breakOnWriteBarrierSnippet", "D\tinsert breakpoint instruction at beginning of write barrier snippet",
        SET_OPTION_BIT(TR_BreakOnWriteBarrierSnippet), "F" },
    { "checkGRA", "D\tPreserve stores that would otherwise be removed by GRA, and then verify that the stored value "
                  "matches the global register",
        SET_OPTION_BIT(TR_CheckGRA), "F" },
    { "classesWithFoldableFinalFields=", "O{regex}\tAllow hard-coding of values of final fields in the specified "
    ...
    { "debugCounters=", "D{regex}\tEnable dynamic debug counters with names matching regex (unless they fail to meet "
                        "some other criterion)",
        TR::Options::setRegex, offsetof(OMR::Options, _enabledDynamicCounterNames), 0, "F" },
    { "debugCounterWarmupSeconds=", "D<nnn>\tDebug counters will be reset to zero after nnn seconds, so only "
    ...
    {
        "disableArch11PackedToDFP", "O\tdisable arch(11) packed to DFP conversion instructions",
    ...
    { "disableDememoization", "O\talways call \"memoizing\" getters (like Integer.valueOf) rather than having Escape "
                              "Analysis turn them into noncontiguous stack allocations",
    ...
    { "disableGRACostBenefitModel", "O\tdisable GRA cost/benefit model", RESET_OPTION_BIT(TR_EnableGRACostBenefitModel),
        "F" },
    { "disableGuardedCallArgumentRemat", "O\tdon't rematerialize a guarded virtual call's arguments on the cold path; "
                                         "instead, leave the expressions on the mainline path",
    ...
    { "disableLateEdgeSplitting", "C\tconservatively add regdeps for the vmthread on any edge that might need it",
    ...
    { "disableLongDispNodes", "C\tdisable 390 long-displacement fixup using TR Instructions",
        SET_OPTION_BIT(TR_DisableLongDispNodes), "F" },
    { "disableLongDispStackSlot", "O\tdisable use of stack slot for handling long displacements on 390",
    ...
    { "disableMaxMinOptimization", "O\tdisable masking of VFT Pointers", SET_OPTION_BIT(TR_DisableMaxMinOptimization),
        "F" },
    ...
    { "disableMCSBypass", "O\tdisable allocating JNI global references to skip some levels of indirection when "
                          "accessing a MutableCallSite's target MethodHandle in jitted code",
    ...
    { "disableMethodIsCold", "O\tdo not use heuristics to determine whether whole methods are cold based on how many "
                             "times they have been interpreted",
    ...
    { "disableOutlinedPrologues", "O\tdo all prologue logic in-line", RESET_OPTION_BIT(TR_EnableOutlinedPrologues),
        "F" },
    { "disablePackedLongConversionOn32Bit", "O\t prevent pd2l/l2pd/pd2lu/lu2pd conversions from using the 64 bit IL "
    ...
    { "disableProloguePushes", "O\tuse stores instead of pushes in x86 prologues",
    ...
    { "disableRefinedAliases", "O\tdisable collecting side-effect summaries from compilations to improve aliasing info "
    ...
    { "disableShrinkWrapping", "M\tdisable shrink wrapping of callee saved registers",
    ...
    { "disableTrivialStoreSinking", "O\tdisable trivial store sinking", RESET_OPTION_BIT(TR_EnableTrivialStoreSinking),
        "F" },
    { "disableTrueRegisterModel", "C\tdisable use of true liveness model in local RA instead of Future Use Count "
                                  "(Dangerous use unless no Global Virtual Regs used)",
    ...
    { "disableVMCSProfiling", "O\tdisable VM data for virtual call sites", SET_OPTION_BIT(TR_DisableVMCSProfiling), "F",
        NOT_IN_SUBSET },
    { "disableVMThreadGRA", "O\tdisable reuse of the vmThread's real register as a global register",
    ...
    { "enableExpensiveOptsAtWarm", "O\tenable store sinking, shrink wrapping and OSR at warm and below",
    ...
    { "enableJITHelpershashCodeImpl", "O\tenalbe java version of object hashCode()",
        SET_OPTION_BIT(TR_EnableJITHelpershashCodeImpl), "F" },
    { "enableJITHelpersoptimizedClone", "O\tenalbe java version of object clone()",
        SET_OPTION_BIT(TR_EnableJITHelpersoptimizedClone), "F" },
    { "enableJProfiling=", "R\tenable generation of profiling code by the JIT", TR::Options::setJProfilingBits, 0, 0,
        "F", NOT_IN_SUBSET },
    ...
    { "enableLateEdgeSplitting", "C\trematerialize the vmthread at codegen time only on paths that need it",
    ...
    { "enableOutlinedNew", "O\tdo object allocation logic with a fast jit helper", SET_OPTION_BIT(TR_EnableOutlinedNew),
        "F" },
    { "enableOutlinedPrologues", "O\tcall a helper routine to do some prologue logic",
    ...
    { "enableRefinedAliases", "O\tenable collecting side-effect summaries from compilations to improve aliasing info "
                              "in subsequent compilations",
    ...
    { "enableSCSSRelocations", "O\tenable relocations in sequential constant store simplification sequences",
        SET_OPTION_BIT(TR_EnableSCSSRelocations), "F" },
    ...
    { "enableTieredCodeCache", "D\tseparate code cache into cold and warm", SET_OPTION_BIT(TR_EnableTieredCodeCache),
        "F", NOT_IN_SUBSET },
    ...
    { "enableZAccessRegs", "O\tenable use of access regs as spill area on 390.", SET_OPTION_BIT(TR_Enable390AccessRegs),
        "F" },
    { "enableZEpilogue", "O\tenable 64-bit 390 load-multiple breakdown.", SET_OPTION_BIT(TR_Enable39064Epilogue), "F" },
    { "enableZFreeVMThreadReg", "O\tenable use of vm thread reg as assignable reg on 390.",
    ...
    { "GCRCount=", "R<nnn>\tthe value to which the counter is set for a method containing GCR body. The default is the "
    ...
    { "GCRresetCount=", "R<nnn>\tthe value to which the counter is reset to after being tripped by guarded counting "
    ...
    { "hotFieldThreshold=", "M<nnn>\t The normalized frequency of a reference to a field to be marked as hot.   Values "
                            "are 0 to 10000.  Default is 10",
        TR::Options::setStaticNumeric, (intptrj_t)&OMR::Options::_hotFieldThreshold, 0, " %d", NOT_IN_SUBSET },
    { "hotMaxStaticPICSlots=", " <nnn>\tmaximum number of polymorphic inline cache slots pre-populated from profiling "
                               "info for hot and above.  A negative value -N means use N times the maxStaticPICSlots "
                               "setting.",
    ...
    { "inlineCntrCalleeTooDeepBucketSize=", "R<nnn>\tThe unit used to categorize uninlined calls when the recursively "
    ...
    { "inlinerArgumentHeuristicFractionUpToWarm=", "O<nnn>\tAt hotness up to and including warm, decreases estimated "
                                                   "size by 1/N when the inliner's argument heuristic kicks in",
    ...
    { "labelTargetNOPLimit=", "C<nnn>\t(labelTargetAddress&0xff) > _labelTargetNOPLimit are padded out with NOPs until "
                              "the next 256 byte boundary",
    ...
    { "listing=", "L<filename>\twrite listing to filename", TR::Options::setString,
        offsetof(OMR::Options, _listingFileName), 0, "P%s" },
    { "lnl=", "C<nnn>\t(labelTargetAddress&0xff) > _labelTargetNOPLimit are padded out with NOPs until the next 256 "
              "byte boundary",
        TR::Options::set32BitNumeric, offsetof(OMR::Options, _labelTargetNOPLimit), TR_LABEL_TARGET_NOP_LIMIT, "F%d" },
    ...
    { "numRestrictedGPRs=", "O<nnn>\tnumber of restricted GPRS (0-5). Currently 390 only", TR::Options::set32BitNumeric,
        offsetof(OMR::Options, _numRestrictedGPRs), 0, "F%d" },
    ...
    { "randomSeedSignatureHash", "R\tSet random seed value based on a hash of the method's signature, in order to get "
                                 "varying seeds while maintaining reproducibility",
    ...
    { "sampleThreshold=", "R<nnn>\tThe maximum number of global samples taken during a sample interval for which the "
    ...
    { "sinkAllStores", "O\tin trivialStoreSinking sink all stores possible by agressively creating anchors for "
                       "indirect loads and killed syms",
    ...
    { "startupMethodDontDowngradeThreshold=",
        ...
        NOT_IN_SUBSET },
    { "staticDebugCounters", "D\tEnable static versions of all enabled dynamic debug counters (unless "
    ...
    { "traceGRA", "L\ttrace tree based global register allocator", TR::Options::traceOptimization,
        tacticalGlobalRegisterAllocator, 0, "P" },
#ifdef J9_PROJECT_SPECIFIC
    { "traceHCRGuardInsertion", "L\ttrace HCR guard insertion", TR::Options::traceOptimization, hcrGuardInsertion, 0,
        "P" },
    ...
    { "traceLocalVP", "L\ttrace local value propagation", TR::Options::traceOptimization, localValuePropagation, 0,
        "P" },
    { "traceLongRegAllocationHeuristic", "L\ttrace long register allocation", TR::Options::traceOptimization,
        longRegAllocation, 0, "P" },
    ...
    { "traceRegisterITF=", "L{regex}\tlist of additional register interference graph build options: basic, listing, "
                           "results, build, details, live, colourability",
    ...
    { "traceShrinkWrapping", "L\ttrace shrink wrapping of callee saved registers ",
    ...
    { "trustAllInterfaceTypeInfo", "O\tGive Java interface type information the same level of trust afforded to class "
                                   "info.  The Java spec is much more lackadasical about interface type safety, and "
                                   "requires us to be conservative.",
    ...
    { "useIdleTime", "M\tuse cpu idle time to compile more aggressively", SET_OPTION_BIT(TR_UseIdleTime), "F",
        NOT_IN_SUBSET },
    { "useJProfilingForAllFirstTimeComps", "M\tHeuristic", SET_OPTION_BIT(TR_UseJProfilingForAllFirstTimeComps), "F",
        NOT_IN_SUBSET },
    { "useJProfilingForDLT", "M\tHeuristic. Use jProfiling for DLT methods", SET_OPTION_BIT(TR_UseJProfilingForDLT),
        "F", NOT_IN_SUBSET },
    { "useJProfilingForInterpSampledMethods", "M\tHeuristic. Use jProfiling for methods sampled by interpreter",
        SET_OPTION_BIT(TR_UseJProfilingForInterpSampledMethods), "F", NOT_IN_SUBSET },
    { "useJProfilingForLPQ", "M\tHeuristic. Use jProfiling for methods from low priority queue",
...
// enable GCR filtering
            ...
            if (TR::Options::_smallMethodBytecodeSizeThresholdForCold == -1) // not yet set
                ...
                TR::Options::_smallMethodBytecodeSizeThresholdForCold = std::max<int32_t>(
                    8, (TR::Compiler->target.numberOfProcessors() != 0 ? 32 / TR::Compiler->target.numberOfProcessors()
                                                                       : 8));
#endif
    ...
    char* startOption, TR::OptionTable* table, void* base, int32_t numEntries, TR::OptionSet* optionSet)
{
    ...
    while (1) {
        ...
        //
        if (opt->name[opt->length - 1] == '=' && strlen(option) >= opt->length && option[opt->length - 1] == '@'
    ...
    for (i++; i < numEntries; i++) {
        ...
        if (next->name[next->length - 1] == '=' && strlen(option) >= next->length && option[next->length - 1] == '@'
...
bool OMR::Options::jitPostProcess()
{
    ...
    if (self()->getOption(TR_MimicInterpreterFrameShape)
        && self()->getOption(
               TR_FSDGRA)) // If we are enabling FSDGRA, we must disable VmThreadGRA or bad things will happen.
...
OMR::Options::TR_OptionStringToBit OMR::Options::_optionStringToBitMapping[] = {
    ...
    { "lateEdgeSplitting", TR_TraceRALateEdgeSplitting }, { "preRA", TR_TraceRAPreAssignmentInstruction },
...
char* OMR::Options::_verboseOptionNames[TR_NumVerboseOptions] = {
    "options", "compileStart", "compileEnd", "compileRequest", "gc", "compileTime", "recompile", "compilePerformance",
    "filters", "sampling", "mmap", "compileExclude", "link", "classLoadPhase", "GCcycle", "compilationYieldStats",
    "heartbeat", "Extended", "SChints", "counts", "failures", "jitState", "jitMemory", "compilationThreads",
...
char* OMR::Options::_jprofilingOptionNames[TR_NumJProfilingFlags] = {
    "invokevirtual", "invokeinterface", "invokestatic", "branches", "checkcast", "instanceof",
};
...
void OMR::Options::setDefaultsForDeterministicMode()
{
    if (TR::Options::getDeterministicMode() != -1 // if deterministic mode was set
        ...
        && !self()->getOption(TR_noquickstart) && !self()->getOption(TR_AggressiveOpts)) {
