...
template <class base_allocator = CS2::malloc_allocator> class trace_allocator : private base_allocator {
public:
    void* allocate(size_t size, const char* name = NULL)
    {
...
private:
    struct AllocationRecord {
        ...
        AllocationRecord(size_t size = 0)
            : _count(1)
            , _size(size)
        {
        }

        template <class ostr> friend ostr& operator<<(ostr& o, const struct AllocationRecord& a)
        {
...
public:
    ...
    struct stat_sort {
        ...
        template <class ostr> friend ostr& operator<<(ostr& o, const struct stat_sort& a)
        {
...
template <class base_allocator = CS2::malloc_allocator> class stat_allocator : private base_allocator {
public:
    void* allocate(size_t size, const char* name = NULL)
    {
        void* ret = (void*)base_allocator::allocate(size, name);
...
template <class base_allocator = CS2::malloc_allocator> class check_allocator : private base_allocator {
...
public:
    ...
    check_allocator(const check_allocator& c)
        : base_allocator(base_allocator(c))
        , record_table(c)
    {
    }
    ...
    void deallocate(void* pointer, size_t size, const char* name = NULL)
    {
        ...
        if (memcmp((char*)pointer + size + sizeof(void*), &pointer, sizeof(void*))) {
            printf("buffer overflow: %p!=%p\n*alloc=%s[%ld]\n*dealloc=%s[%ld]\n", pointer,
                *(void**)((char*)pointer + size + 4), x.name ? x.name : "", x.size, name ? name : "", size);
            CS2Assert(false, ("Overflow: %p", pointer));
        }
