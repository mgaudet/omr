...
protected:
    ...
    public:
        ...
        DerivedElement(Initializer& element)
            : fElement(element)
        {
        }
        ~DerivedElement() {}
...
CS2_ARTEMP inline void CS2_BASEARDECL::GrowTo(size_t newSize)
{
    ...
    if (segmentMapIndex >= fMaxSegments) {
        if (fSegmentMap == NULL) {
            ...
            DerivedElement** newSegmentMap
                = (DerivedElement**)Allocator::allocate(maxSegments * sizeof(DerivedElement*));
            memcpy(newSegmentMap, fSegmentMap, fMaxSegments * sizeof(DerivedElement*));
            Allocator::deallocate(fSegmentMap, fMaxSegments * sizeof(DerivedElement*));
    ...
    CS2Assert(
        segmentMapIndex + 1 == fNumberOfSegments, ("Number of current segments %d is smaller than expected size of %d.",
