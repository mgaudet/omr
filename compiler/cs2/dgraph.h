...
public:
    ...
    {
        CS2Assert(direction == kSuccessorList || direction == kPredecessorList, ("Invalid direction: %d", direction));
    ...
    void SetFirstSuccessor(EdgeIndex edgeIndex) { SetFirstEdge(kSuccessorList, edgeIndex); }

    // Get/set the index of the first predecessor edge
    EdgeIndex FirstPredecessor() const { return FirstEdge(kPredecessorList); }

    void SetFirstPredecessor(EdgeIndex edgeIndex) { SetFirstEdge(kPredecessorList, edgeIndex); }

    // Dump routine
    template <class ostr> friend ostr& operator<<(ostr& out, const DirectedGraphNode& node)
    {
...
public:
    ...
    void SetNode(bool direction, NodeIndex inputNode) { fNodes[direction] = inputNode; }
    ...
    void SetPredecessor(NodeIndex inputNode) { SetNode(kPredecessorList, inputNode); }

    // Sets the next edge link in the given chain
    void SetNext(bool direction, EdgeIndex inputEdge) { fEdges[direction] = inputEdge; }

    // Sets the next successor edge link
    void SetNextSuccessor(EdgeIndex inputEdge) { SetNext(kSuccessorList, inputEdge); }

    // Sets the next predecessor edge link
    void SetNextPredecessor(EdgeIndex inputEdge) { SetNext(kPredecessorList, inputEdge); }

    // Dump routine
    template <class ostr> friend ostr& operator<<(ostr& out, const DirectedGraphEdge& edge)
    {
...
#define CS2_DFC_DECL CS2_DG_DECL::DepthFirstCursor
    ...
    public:
        DepthFirstCursor(
            const CS2_DG_DECL&, NodeIndex, bool traversalType = kPostOrder, bool direction = kForwardDirection);
        // ~DepthFirstCursor();
...
#define CS2_DGEC_DECL CS2_DG_DECL::EdgeCursor
    ...
    template <class ostr> friend ostr& operator<<(ostr& out, const CS2_DG_DECL& graph)
    {
        /*
          out << "Node Table" << "\n";
          out << "----------" << "\n";
...
CS2_DG_TEMP inline CS2_DG_DECL::DirectedGraph(NodeIndex ignore, EdgeIndex ignore2, const Allocator& a)
    : Allocator(a)
    , fEdgeTable(a)
    , fNodeTable(a)
{
...
CS2_DG_TEMP inline CS2_DG_DECL::DirectedGraph(const CS2_DG_DECL& inputGraph) { *this = inputGraph; }
...
CS2_DG_TEMP inline ANodeType& CS2_DG_DECL::NodeAt(NodeIndex nodeIndex) const { return fNodeTable.ElementAt(nodeIndex); }

CS2_DG_TEMP inline bool CS2_DG_DECL::NodeExists(NodeIndex nodeIndex) const { return fNodeTable.Exists(nodeIndex); }

CS2_DG_TEMP inline AEdgeType& CS2_DG_DECL::EdgeAt(EdgeIndex edgeIndex) const { return fEdgeTable.ElementAt(edgeIndex); }

CS2_DG_TEMP inline bool CS2_DG_DECL::EdgeExists(EdgeIndex edgeIndex) const { return fEdgeTable.Exists(edgeIndex); }

// DirectedGraph::HighestNodeIndex
//
// Return the highest allocated node index

CS2_DG_TEMP inline NodeIndex CS2_DG_DECL::HighestNodeIndex() const { return fNodeTable.HighestIndex(); }
CS2_DG_TEMP inline NodeIndex CS2_DG_DECL::LowestNodeIndex() const { return fNodeTable.LowestIndex(); }

// DirectedGraph::HighestEdgeIndex
//
// Return the highest allocated edge index

CS2_DG_TEMP inline EdgeIndex CS2_DG_DECL::HighestEdgeIndex() const { return fEdgeTable.HighestIndex(); }
CS2_DG_TEMP inline EdgeIndex CS2_DG_DECL::LowestEdgeIndex() const { return fEdgeTable.LowestIndex(); }
...
CS2_DG_TEMP inline bool CS2_DG_DECL::AddNodeAtPosition(NodeIndex inputPosition)
{
...
CS2_DG_TEMP inline CS2_DFC_DECL::operator NodeIndex() const { return fNodeList[fCurrentIndex]; }
...
CS2_DG_TEMP inline CS2_BFC_DECL::operator NodeIndex() const { return fNodeList[fCurrentIndex]; }
...
CS2_DG_TEMP inline void CS2_DG_DECL::VerifyDGraph() const
{
    ...
    for (nodeIndex.SetToFirst(); nodeIndex.Valid(); nodeIndex.SetToNext()) {
        ...
        for (edgeIndex = NodeAt(nodeIndex).FirstSuccessor(); edgeIndex != 0; edgeIndex = nextEdge) {
        ...
        for (edgeIndex = NodeAt(nodeIndex).FirstPredecessor(); edgeIndex != 0; edgeIndex = nextEdge) {
...
CS2_DG_TEMP inline void CS2_DG_DECL::RemoveNode(NodeIndex victimNode)
{
    ...
    for (edgeIndex = NodeAt(victimNode).FirstSuccessor(); edgeIndex != 0; edgeIndex = nextEdge) {
        nextEdge = EdgeAt(edgeIndex).NextSuccessor();
        RemoveEdge(edgeIndex);
    }
    CS2Assert(NodeAt(victimNode).FirstSuccessor() == 0, ("Failed to remove all successor edges"));

    // Remove all the incident predecessor edges
    for (edgeIndex = NodeAt(victimNode).FirstPredecessor(); edgeIndex != 0; edgeIndex = nextEdge) {
        nextEdge = EdgeAt(edgeIndex).NextPredecessor();
        RemoveEdge(edgeIndex);
    }
    CS2Assert(NodeAt(victimNode).FirstPredecessor() == 0, ("Failed to remove all predecessor edges"));
...
CS2_DG_TEMP inline EdgeIndex CS2_DG_DECL::AddEdge(NodeIndex fromNode, NodeIndex toNode, bool direction)
{
    EdgeIndex newIndex;
...
CS2_DG_TEMP inline void CS2_DG_DECL::RemoveEdge(EdgeIndex edgeIndex)
{
    ...
    if (NodeAt(succNode).FirstPredecessor() == edgeIndex)
        ...
        for (siblingEdge = NodeAt(succNode).FirstPredecessor(); siblingEdge != 0; siblingEdge = nextEdge) {
    ...
    if (NodeAt(predNode).FirstSuccessor() == edgeIndex)
        ...
        for (siblingEdge = NodeAt(predNode).FirstSuccessor(); siblingEdge != 0; siblingEdge = nextEdge) {
...
// Get rid of link from a source node to an edge.
CS2_DG_TEMP void inline CS2_DG_DECL::UndoLink(NodeIndex sourceNode, EdgeIndex edgeToBeRemoved, bool direction)
{
    EdgeIndex incidentEdge, nextEdge, lastEdge;

    lastEdge = 0;
    for (incidentEdge = NodeAt(sourceNode).FirstEdge(direction); incidentEdge != 0; incidentEdge = nextEdge) {
...
CS2_DG_TEMP uint32_t inline CS2_DG_DECL::NumberOfPredecessors(NodeIndex node) const
{
    uint32_t numOfPredecessors = 0;
    NodeIndex headIndex = node;
    for (EdgeIndex edgeIndex = NodeAt(headIndex).FirstPredecessor(); edgeIndex != 0;
         edgeIndex = EdgeAt(edgeIndex).NextPredecessor()) {
        numOfPredecessors++;
    }
    return numOfPredecessors;
}

CS2_DG_TEMP uint32_t inline CS2_DG_DECL::NumberOfSuccessors(NodeIndex node) const
{
