...
template <class Allocator = CS2::allocator> class ASparseBitVector : private Allocator {
    class SparseBitRef;
    ...
    typename ASparseBitVector<Allocator>::Segment& outputSegment)
{
    ...
    for (i = 0; i < thisSegment.PopulationCount(); i++) {
        ...
        if (inputSegment.Indices()[ii] == thisSegment.Indices()[i]) {
            CS2Assert(oi < outputSegment.AllocatedSize(), ("Writing past the end of segment %d >= %d (is=%d)", oi,
