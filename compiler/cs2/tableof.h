...
#define CS2_TBL_TEMP                                                                                                   \
    template <class AElementType, class Allocator, uint32_t segmentBits, template <class> class SupportingBitVector>
...
public:
    ...
    template <class Initializer> TableIndex AddEntryAtPosition(TableIndex, Initializer element);
...
#define CS2_TBLC_DECL CS2_TBL_DECL::Cursor
    ...
    template <class ostr> friend ostr& operator<<(ostr& out, const CS2_TBL_DECL& table)
    {
...
CS2_TBL_TEMP inline TableIndex CS2_TBL_DECL::ClearLastOneIfThereIsOne(bool& foundone)
{
    ...
    if (fLowestPossibleRemoved == fHighestPossibleRemoved) {
        ...
        if (fFreeVector.ValueAt(fHighestPossibleRemoved)) {
            ...
            CS2Assert(
                fLowestPossibleRemoved < fHighestPossibleRemoved, ("fLowestPossibleRemoved not less than highest"));
            ...
            if (foundone) {
                CS2Assert(lastOne >= fLowestPossibleRemoved && lastOne <= fHighestPossibleRemoved,
...
CS2_TBL_TEMP inline AElementType& CS2_TBL_DECL::operator[](TableIndex index) const { return ElementAt(index); }
...
CS2_TBL_TEMP inline CS2_TBL_DECL::~TableOf()
{
    ...
    for (elementIndex.SetToFirst(); elementIndex.Valid(); elementIndex.SetToNext()) {
...
CS2_TBL_TEMP
template <class Initializer> inline TableIndex CS2_TBL_DECL::AddEntry(Initializer& initializer)
{
    TableIndex newIndex;
    bool foundone;
...
CS2_TBL_TEMP inline TableIndex CS2_TBL_DECL::AddEntryAtPosition(TableIndex newIndex)
{
    if (fHighestIndex < newIndex) {
        if (fHighestIndex < newIndex - 1) {
            if (fLowestPossibleRemoved == fHighestPossibleRemoved && !fFreeVector.ValueAt(fLowestPossibleRemoved)) {
...
CS2_TBL_TEMP inline CS2_TBLCC_DECL::ConstCursor(const CS2_TBL_DECL& aTable, const SupportingBitVector<Allocator>& fv)
    : fTable(aTable)
    , fFreeCursor(fv)
    , fNextFree(0)
    , fIndex(0)
...
CS2_TBL_TEMP inline CS2_TBLCC_DECL::ConstCursor(
    const typename CS2_TBLCC_DECL& inputCursor, const SupportingBitVector<Allocator>& fv)
    : fTable(inputCursor.fTable)
    , fFreeCursor(fv)
    , fIndex(inputCursor.fIndex)
    , fNextFree(inputCursor.fNextFree)
...
CS2_TBL_TEMP inline bool CS2_TBLCC_DECL::Valid() const { return fIndex < fNextFree; }

// TableOf::Cursor::operator TableIndex
//
// Return the index of the element pointed to by the cursor.

CS2_TBL_TEMP inline CS2_TBLCC_DECL::operator TableIndex() const { return fIndex; }
