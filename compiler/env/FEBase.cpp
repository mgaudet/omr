...
void feOutOfMemory(void*, TR::Compilation* comp, const char* reason)
{
    if (comp) {
        ...
        if (debug("traceOutOfMemory") || debug("stopOnOutOfMemory")
            || comp->getOptions()->getOption(TR_StopOnFailure)) {
            diagnostic(
                "JIT: terminating compile of %s: %s\n", comp->signature(), reason ? reason : "<no reason provided>");
        ...
        if (jitConfig && jitConfig->options.verboseFlags != 0) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_COMPFAIL, "JIT: terminating compile of %s: %s", comp->signature(),
                reason ? reason : "<no reason provided>");
        }
...
static const int S390MachineTypes[] = { TR_FREEWAY, TR_Z800, TR_MIRAGE, TR_MIRAGE2, TR_TREX, TR_Z890, TR_GOLDEN_EAGLE,
    ...
    TR_Z13, TR_Z13BC, TR_ZH, TR_DATAPOWER, TR_ZH_RESERVE1, TR_ZH_RESERVE2 };
...
TR::OptionTable OMR::Options::_feOptions[] = {
    { "code=", "C<nnn>\tsize of a single code cache, in KB", TR::Options::set32BitNumeric,
...
void TR_PatchNOPedGuardSite::compensate(TR_FrontEnd* fe, bool isSMP, uint8_t* location, uint8_t* destination)
{
...
void TR_RuntimeAssumption::addToRAT(TR_PersistentMemory* persistentMemory, TR_RuntimeAssumptionKind kind,
    TR_FrontEnd* fe, TR_RuntimeAssumption** sentinel)
{
...
TR_PatchNOPedGuardSiteOnClassPreInitialize* TR_PatchNOPedGuardSiteOnClassPreInitialize::make(
    TR_FrontEnd* fe, TR_PersistentMemory*, char*, unsigned int, unsigned char*, unsigned char*, TR_RuntimeAssumption**)
{
