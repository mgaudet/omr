...
template <class Derived> void FEBase<Derived>::unreserveCodeCache(TR::CodeCache* codeCache)
{
    codeCacheManager().unreserveCodeCache(codeCache);
}

template <class Derived> TR::CodeCache* FEBase<Derived>::getDesignatedCodeCache(TR::Compilation*)
{
    ...
    TR::Compilation* comp, uint32_t warmCodeSize, uint32_t coldCodeSize, uint8_t** coldCode, bool isMethodHeaderNeeded)
{
    ...
    if (warmCode == NULL) {
        comp->setErrorCode(
            jitConfig()->isCodeCacheFull() ? COMPILATION_CODE_MEMORY_EXHAUSTED : COMPILATION_ALL_CODE_CACHES_RESERVED);
    }
    ...
    TR_ASSERT(!((warmCodeSize && !warmCode) || (coldCodeSize && !coldCode)),
        "Allocation failed but didn't call outOfMemory()");
...
template <class Derived> uint8_t* FEBase<Derived>::allocateRelocationData(TR::Compilation* comp, uint32_t size)
{
    ...
    return (uint8_t*)mmap(0, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
}

template <class Derived> void FEBase<Derived>::switchCodeCache(TR::CodeCache* newCache)
{
...
template <class Derived> intptrj_t FEBase<Derived>::indexedTrampolineLookup(int32_t helperIndex, void* callSite)
{
