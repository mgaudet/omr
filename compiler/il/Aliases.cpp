...
TR_BitVector* OMR::SymbolReference::getUseDefAliasesBV(bool isDirectCall, bool includeGCSafePoint)
{
    ...
    if (!self()->reallySharesSymbol(comp)) {
        ...
        case TR::Symbol::IsStatic: {
            if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) || _symbol->isVolatile()
                || self()->isLiteralPoolAddress() || self()->isFromLiteralPool() || _symbol->isUnsafeShadowSymbol()
    ...
    case TR::Symbol::IsResolvedMethod: {
        ...
        if (!TR::Options::getCmdLineOptions()->getOption(TR_EnableHCR)) {
            ...
            case TR::java_lang_System_arraycopy: {
                TR_BitVector* aliases = new (comp->trHeapMemory())
        ...
        if (methodInfo && (methodInfo->hasRefinedAliasSets() || comp->getMethodHotness() >= veryHot
    ...
    case TR::Symbol::IsStatic: {
        if ((self()->isUnresolved() && !_symbol->isConstObjectRef()) || self()->isLiteralPoolAddress()
            || self()->isFromLiteralPool() || _symbol->isVolatile()) {
...
bool OMR::SymbolReference::storeCanBeRemoved()
{
    ...
    return !s->isVolatile() && (((s->getDataType() != TR::Double) && (s->getDataType() != TR::Float))
...
TR_BitVector* OMR::SymbolReference::getSymRefUseDefAliasesBV(TR::Compilation* c, bool isDirectCall, bool gcSafe)
{
