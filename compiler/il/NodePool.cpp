...
TR::Node* TR::NodePool::allocate(ncount_t poolIndex)
{
    ...
    if (debug("traceNodePool")) {
        diagnostic("%sAllocating Node[%p] with Global Index %d, Pool Index %d\n", OPT_DETAILS_NODEPOOL, &newNode,
            newNode.getGlobalIndex(), poolIndex);
    }
    return &newNode;
}
...
OMR::Node::OptAttributes* TR::NodePool::allocateOptAttributes()
{
    ...
    TR_ASSERT(optAttribPoolIndex == _poolIndex,
        "Cannot eliminate variable _optAttribPoolIndex and use _poolIndex instead. Was hoping this was possible");
