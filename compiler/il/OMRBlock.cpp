    ...
    , _catchBlockExtension(NULL)
{
    ...
    if (other._catchBlockExtension) {
        _catchBlockExtension = new (other.trHeapMemory()) TR_CatchBlockExtension(*other._catchBlockExtension);
    }
 ...
 * edge from
 *        "from" to origTo.  BE CAREFUL when calling this function that there is another predecessor edge for "origTo"
 * or else when
...
void OMR::Block::changeBranchDestination(TR::TreeTop* newDestination, TR::CFG* cfg)
{
...
void OMR::Block::uncommonNodesBetweenBlocks(TR::Compilation* comp, TR::Block* newBlock)
{
    ...
    TR_HandleInjectedBasicBlock ibb(
        comp, NULL, comp->getMethodSymbol(), symbolReferenceTempsA, injectedBasicBlockTemps, symbolReferenceTempsC);
...
TR::Block* OMR::Block::split(
    TR::TreeTop* startOfNewBlock, TR::CFG* cfg, bool fixupCommoning, bool copyExceptionSuccessors)
{
    ...
    TR::Block* block2 = new (self()->trHeapMemory()) TR::Block(
        TR::TreeTop::create(comp, TR::Node::create(startNode, TR::BBStart)), self()->getExit(), self()->trMemory());
    ...
    if (structure) {
        ...
        if (thisBlockStructure) {
            TR_BlockStructure* blockStructure2
                = new (self()->trHeapMemory()) TR_BlockStructure(comp, block2->getNumber(), block2);
            TR_RegionStructure* parentStructure = thisBlockStructure->getParent()->asRegion();
            TR_StructureSubGraphNode* blockStructureNode2
                = new (self()->trHeapMemory()) TR_StructureSubGraphNode(blockStructure2);
...
#define MIN_EXCEPTION_NAME_SIZE 5
    ...
    { 9, "Exception", CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck
            | CanCatchArrayNew | CanCatchCheckCast | CanCatchMonitorExit },
    ...
    { 16, "RuntimeException", CanCatchNullCheck | CanCatchDivCheck | CanCatchBoundCheck | CanCatchArrayStoreCheck
            | CanCatchArrayNew | CanCatchCheckCast | CanCatchMonitorExit },
    ...
    TR::Block* from, TR::Block* to, TR::Compilation* c, TR::TreeTop** lastTreeTop, bool findOptimalInsertionPoint)
{
    ...
    // splitting the block at the point of the return or throw is the safest way to do this
        ...
        TR_ASSERT(opCode == TR::athrow || lastTree->getNode()->getOpCode().isReturn(),
...
bool OMR::Block::isSimpleGenControlBlock()
{
...
TR::Block* TR_BlockCloner::doBlockClone(TR_LinkHeadAndTail<BlockMapper>* bMap)
{
    ...
    // Luckily, so far no code has ever not done that.  Most code that clonesBranchesExactly clones a single block, and
    // the few cases that
