...
FirstOMROp,
    ...
    instanceof, // instanceof - symref is the class object, cp index is in the
              ...
              trtLookup, // special lookupswitch (child1 must be trt, child2 the default destination, subsequent
                         // children are case nodes)
              // The internal control flow is similar to lookup, but each CASE represents a special semantics associated
              // with a flag on it
              ...
              table, // tableswitch (child1 is the selector, child2 the default destination, subsequent children are the
                     // branch targets
              // (the last child may be a branch table address, use getCaseIndexUpperBound() when iterating over branch
              // targets)
              exceptionRangeFence, // (J9) SymbolReference is the aliasing effect, initialiser is where the code address
                                   // gets put when binary is generated
              ...
              ResolveAndNULLCHK, // Resolve check a static, field or method and Null check the underlying pointer.
                                 // child 1 is reference to be resolved. Symbolref indicates failure action/destination
              ...
              OverflowCHK, // Overflow check. child 1 is the operation node(add, mul, sub). Child 2 and child 3 are the
                           // operands. Symbolref indicates failure action/destination
