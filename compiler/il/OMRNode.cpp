...
void* OMR::Node::operator new(size_t s, TR::NodePool& nodes, ncount_t poolIndex)
{
...
TR::Node* OMR::Node::recreateWithSymRefAndCopyValidProperties(
 ...
 * from
 * the originalNode that are valid for the new node only will be copied. All other properties will be reset to their
 * default values.
 */
...
TR::Node* OMR::Node::recreateWithSymRef(
    TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...)
{
    va_list args;
    va_start(args, numChildArgs);
...
{
    TR_ASSERT(TR::Node::isLegalCallToCreate(op), "assertion failure");
    return TR::Node::recreateWithSymRef(
    ...
    TR::Node* first, TR::Node* second, TR::Node* third, TR::Node* fourth, TR::Node* fifth)
{
    ...
    if (symbolRefOrBranchTarget != NULL || node->hasSymbolReference() || node->hasBranchDestinationNode()) {
        ...
        if (node->hasSymbolReference())
            ...
            TR_ASSERT(false, "attempting to access _branchDestinationNode or _symbolReference field for node %s %p "
...
TR::Node* OMR::Node::duplicateTreeWithCommoningImpl(CS2::HashTable<TR::Node*, TR::Node*, CS2::allocator>& nodeMapping)
{
...
bool OMR::Node::containsAnyNode(List<TR::Node*>& nodeList, vcount_t visitCount)
{
 ...
 */
bool OMR::Node::containsAnyNode(TR::SparseBitVector& nodeSet, vcount_t visitCount, TR::Compilation* comp)
{
...
{
    TR::Compilation* comp = TR::comp();
    TR_ASSERT(self()->getDataType() == TR::Address, "addressPointsAtObject should only be called for nodes whose data "
...
bool OMR::Node::isRematerializable(TR::Node* parent, bool onlyConsiderOpCode)
{
    ...
    if (opCode.isArrayLength() || opCode.isConversion()) {
        if (onlyConsiderOpCode || (((self()->getFirstChild()->getFutureUseCount() & 0x7fff) > 0)
...
TR::Node* OMR::Node::getReturnCode(bool isReason)
{
    ...
    // just return code
    else if (self()->getOpCodeValue() == TR::ireturn && self()->getNumChildren() == 1)
        codeChild = self()->getChild(0);

    // return code and return reason
    else if ((self()->getOpCodeValue() == TR::Return)
        && (self()->getNumChildren() == 2
                 || (self()->getNumChildren() == 3 && self()->getChild(2)->getOpCodeValue() == TR::GlRegDeps))) {
        codeChild = self()->getChild(0);
...
#define SignExtensionFlagsIndex(x)                                                                                     \
    reinterpret_cast<long>(self()->hasNodeExtension() ? _unionBase._extension.getExtensionPtr()->getElem<TR::Node*>(1) \
...
bool OMR::Node::couldIgnoreExtend()
{
    ...
    // traceMsg(c, "\t\tChecking couldIgnore on %d+%d == %c on %p\n", SignExtensionFlagsIndex(0),
    // (int)couldSkipExtension, flags && SupportedType(getOpCode()) &&
    // flags->isSet(SignExtensionFlagsIndex(couldSkipExtension))?'1':'0', this);
    return flags && SupportedType(self()) && flags->isSet(SignExtensionFlagsIndex(couldSkipExtension));
}
...
void OMR::Node::setCouldIgnoreExtend(bool b)
{
    ...
    // (int)couldSkipExtension, b?'1':'0', this);
    if (b)
...
const char* OMR::Node::printCouldIgnoreExtend() { return self()->couldIgnoreExtend() ? "couldSkipExt " : ""; }

bool OMR::Node::force64BitLoad()
{
    TR::Compilation* c = TR::comp();
    TR_BitVector* flags = c->cg()->signExtensionFlags();
    // traceMsg(c, "\t\tChecking force64BitReg on %d+%d == %c on %p\n", SignExtensionFlagsIndex(0), (int)force64BitReg,
    // flags && SupportedType(getOpCode()) && flags->isSet(SignExtensionFlagsIndex(force64BitReg))?'1':'0', this);
...
const char* OMR::Node::printForce64BitLoad() { return self()->force64BitLoad() ? "force64Reg " : ""; }

bool OMR::Node::isUnsignedLoad()
{
    TR::Compilation* c = TR::comp();
    TR_BitVector* flags = c->cg()->signExtensionFlags();
    // traceMsg(c, "\t\tChecking unsignedLoad on %d+%d == %c on %p\n", SignExtensionFlagsIndex(0), (int)unsignedLoad,
    // flags && SupportedType(getOpCode()) && flags->isSet(SignExtensionFlagsIndex(unsignedLoad))?'1':'0', this);
...
const char* OMR::Node::printIsUnsignedLoad() { return self()->isUnsignedLoad() ? "isUnsignedLoad " : ""; }
...
void OMR::Node::setByteCodeInfo(const TR_ByteCodeInfo& bcInfo) { _byteCodeInfo = bcInfo; }

void OMR::Node::copyByteCodeInfo(TR::Node* from) { _byteCodeInfo = from->_byteCodeInfo; }
...
int64_t OMR::Node::get64bitIntegralValue()
{
    ...
    if (type.isInt8())
        ...
        TR_ASSERT(
            false, "Must be an integral or address but it is type %d on node %p\n", self()->getDataType(), self());
        return 0;
    }
...
TR::DataType OMR::Node::getArrayCopyElementType()
{
    TR::Compilation* comp = TR::comp();
    TR_ASSERT(
        self()->getOpCodeValue() == TR::arraycopy || self()->getOpCodeValue() == TR::arrayset, "assertion failure");
...
TR::ILOpCodes OMR::Node::setOverflowCHKInfo(TR::ILOpCodes op)
{
...
TR::ILOpCodes OMR::Node::getOverflowCHKInfo()
{
...
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(c->getOption(TR_EnableOSR), "Currently this OMR::Node flag means different things on different front "
                                          "ends. If we wanted to reuse this query across front ends other than Java "
                                          "(which uses these kinds of stores for OSR) then we probably need to map it "
                                          "to a different bit in the OMR::Node flags");
    TR_ASSERT((self()->getOpCode().isStore() && self()->getSymbolReference()->getSymbol()->isAutoOrParm()),
        "Opcode must be store to a stack slot");
...
void OMR::Node::setIsMethodEnterExitGuard(bool b)
{
    TR::Compilation* c = TR::comp();
    TR_ASSERT(self()->getOpCode().isIf(), "assertion failure");
    if (b && performNodeTransformation2(
