...
bool OMR::Node::isTernaryHigh()
{
    if (((self()->getOpCodeValue() == TR::luaddc) || (self()->getOpCodeValue() == TR::lusubb))
        ...
        if ((ccOpValue == TR::computeCC) && (((self()->getOpCodeValue() == TR::luaddc) && (pairOpValue == TR::luadd))
...
refCanBeKilled(TR::Node* node, TR::Compilation* comp)
{
    ...
    if (node->getOpCodeValue() == TR::aload && (comp->cg()->getLinkage()->isAddressOfStaticSymRef(node->getSymbolReference()) || comp->cg()->getLinkage()->isAddressOfPrivateStaticSymRef(node->getSymbolReference()))) {
        return false;
    }
...
bool OMR::Node::isNotCollected()
{
    ...
    if (self()->getOpCode().hasSymbolReference() && self()->getSymbolReference()->getSymbol()->isNotCollected())
        return true;
    else if (comp->generateArraylets() && (self()->getOpCode().isArrayRef() && (self()->getFirstChild()->isNotCollected() || !self()->getFirstChild()->isInternalPointer())))
        return true;
    return false;
}
...
OMR::Node::getReturnCode(bool isReason)
{
    ...
    else if ((self()->getOpCodeValue() == TR::Return) && (self()->getNumChildren() == 2 || (self()->getNumChildren() == 3 && self()->getChild(2)->getOpCodeValue() == TR::GlRegDeps))) {
        codeChild = self()->getChild(0);
...
OMR::Node::createStoresForVar(TR::SymbolReference*& nodeRef, TR::TreeTop* insertBefore, bool simpleRef)
{
    ...
    if (self()->getOpCode().isArrayRef() || (self()->getOpCode().isLoadVarDirect() && self()->getSymbolReference()->getSymbol()->isAuto() && self()->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer()))
...
#define SupportedType(node) (node->getOpCode().isLoadVar() && TR_LoadExtensions::supportedType(node) || (node->getOpCode().isLoadConst() && TR_LoadExtensions::supportedConstLoad(node, c)))
...
bool OMR::Node::isDebug()
{
    return _flags.testAny(nodeIsDebug);
}

void OMR::Node::setIsDebug(bool v)
{
