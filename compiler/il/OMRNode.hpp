...
protected:
    Node(
        TR::Node* originatingByteCodeNode, TR::ILOpCodes op, uint16_t numChildren, OptAttributes* optAttributes = NULL);
...
public:
    ...
    static TR::Node* recreateWithSymRefAndCopyValidProperties(
...
private:
    ...
    static TR::Node* recreateWithSymRef(
        TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren, uint16_t numChildArgs, ...);
...
private:
    ...
    template <class... ChildrenAndSymRefType>
    static TR::Node* recreateWithSymRef(TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren,
...
protected:
    ...
    struct OptAttributes {
        ...
        ncount_t _poolIndex; ///< index to retrieve node from nodepool, this can be shared amongst nodes with
                             ///non-intersecting lifetimes
    };
...
protected:
    enum {
        ...
        arraysetLengthMultipleOfPointerSize = 0x00020000, ///< flag used by TR::arrayset to guarantee that checking for
                                                          ///lengths smaller than pointer size is not needed (leading to
                                                          ///more efficient code being generated, e.g. REP STOSQ on X86
        ...
        ParentSupportsLazyClobber = 0x00002000, ///< Tactical x86 codegen flag.  Only when refcount <= 1.  Indicates
                                                ///that parent will consult the register's node count before clobbering
                                                ///it (not just the node's refcount).
        ...
        referenceIsNonNull = 0x00008000, ///< Sometimes we can't mark the child as non-null (because it's not provably
                                         ///non-null everywhere) but we know it's non-null by the time we run this
                                         ///parent
