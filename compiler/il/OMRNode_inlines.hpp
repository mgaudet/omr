...
template <class... ChildrenAndSymRefType>
TR::Node* OMR::Node::recreateWithSymRef(TR::Node* originalNode, TR::ILOpCodes op, uint16_t numChildren,
...
{
    return recreateWithSymRef(originalNode, op, numChildren, numChildArgs, first, children...);
}
...
{
    return recreateWithSymRef(0, op, numChildren, numChildArgs, first, childrenAndSymRef...);
}
template <class... Children>
...
vcount_t OMR::Node::getVisitCount() { return self()->getOptAttributes()->_visitCount; }

vcount_t OMR::Node::setVisitCount(vcount_t vc) { return (self()->getOptAttributes()->_visitCount = vc); }

vcount_t OMR::Node::incVisitCount()
{
    ++self()->getOptAttributes()->_visitCount;
    TR_ASSERT(self()->getOptAttributes()->_visitCount > 0, "Assertion failure : %s (0x%p)",
        self()->getOpCode().getName(), this);
    return self()->getOptAttributes()->_visitCount;
}

rcount_t OMR::Node::getReferenceCount() { return self()->getOptAttributes()->_referenceCount; }

rcount_t OMR::Node::setReferenceCount(rcount_t rc) { return (self()->getOptAttributes()->_referenceCount = rc); }

rcount_t OMR::Node::incReferenceCount()
{
    ++(self()->getOptAttributes()->_referenceCount);
    TR_ASSERT(self()->getOptAttributes()->_referenceCount > 0, "Assertion failure : %s (0x%p)",
        self()->getOpCode().getName(), this);
...
{
    TR_ASSERT(self()->getOptAttributes()->_referenceCount > 0 || self()->getOpCode().isTreeTop(),
        "Assertion failure : %s (0x%p)", self()->getOpCode().getName(), this);
    return --(self()->getOptAttributes()->_referenceCount);
}

scount_t OMR::Node::getLocalIndex() { return self()->getOptAttributes()->_localIndex; }

scount_t OMR::Node::setLocalIndex(scount_t li) { return (self()->getOptAttributes()->_localIndex = li); }
...
scount_t OMR::Node::getFutureUseCount() { return self()->getOptAttributes()->_localIndex; }

scount_t OMR::Node::setFutureUseCount(scount_t li) { return (self()->getOptAttributes()->_localIndex = (scount_t)li); }
...
scount_t OMR::Node::decFutureUseCount() { return --self()->getOptAttributes()->_localIndex; }

uint16_t OMR::Node::getUseDefIndex() { return self()->getOptAttributes()->_unionA._useDefIndex; }

uint16_t OMR::Node::setUseDefIndex(uint16_t udi) { return (self()->getOptAttributes()->_unionA._useDefIndex = udi); }

ncount_t OMR::Node::getNodePoolIndex() { return self()->getOptAttributes()->_poolIndex; }

void OMR::Node::setNodePoolIndex(ncount_t i) { self()->getOptAttributes()->_poolIndex = i; }
