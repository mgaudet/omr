...
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCallNode(
    TR::TreeTop* insertionPoint, int32_t numChildren, bool copyChildren, bool shouldSplitBlock)
{
    ...
    TR::SymbolReference* induceOSRSymRef
        = symRefTab->findOrCreateRuntimeHelper(TR_induceOSRAtCurrentPC, true, true, true);
    ...
    if ((refNode->getNumChildren() > 0) && refNode->getFirstChild()->getOpCode().isCall()) {
...
bool OMR::ResolvedMethodSymbol::induceOSRAfter(
    TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, TR::TreeTop* branch, bool extendRemainder)
{
...
{
    if (self()->supportsInduceOSR(insertionPoint->getNode()->getByteCodeInfo(), insertionPoint->getNode(),
            insertionPoint->getEnclosingBlock(), NULL, self()->comp()))
        return self()->genInduceOSRCallAndCleanUpFollowingTreesImmediately(
            insertionPoint, insertionPoint->getNode()->getByteCodeInfo(), false, false, self()->comp());
    if (self()->comp()->getOption(TR_TraceOSR))
        traceMsg(self()->comp(),
...
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(TR::TreeTop* insertionPoint,
    TR_ByteCodeInfo induceBCI, bool copyChildren, bool shouldSplitBlock, TR::Compilation* comp)
{
    ...
    TR::TreeTop* induceOSRCallTree = self()->genInduceOSRCall(
        insertionPoint, induceBCI.getCallerIndex(), numChildrenOfInduceCall, copyChildren, shouldSplitBlock);
...
TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCallForGuardedCallee(TR::TreeTop* insertionPoint,
    TR::ResolvedMethodSymbol* calleeSymbol, int32_t numChildren, bool copyChildren, bool shouldSplitBlock)
{
    if (self()->comp()->getOption(TR_TraceOSR)) {
    ...
    TR_OSRMethodData* osrMethodData = self()->comp()->getOSRCompilationData()->findCallerOSRMethodData(
        self()->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
            self()->comp()->getCurrentInlinedSiteIndex(), calleeSymbol));
    return self()->genInduceOSRCall(insertionPoint, self()->comp()->getCurrentInlinedSiteIndex(), osrMethodData,
        calleeSymbol, numChildren, copyChildren, shouldSplitBlock);
}
    ...
    int32_t numChildren, bool copyChildren, bool shouldSplitBlock)
{
    ...
    return self()->genInduceOSRCall(
        insertionPoint, inlinedSiteIndex, osrMethodData, NULL, numChildren, copyChildren, shouldSplitBlock);
}

TR::TreeTop* OMR::ResolvedMethodSymbol::genInduceOSRCall(TR::TreeTop* insertionPoint, int32_t inlinedSiteIndex,
    TR_OSRMethodData* osrMethodData, TR::ResolvedMethodSymbol* callSymbolForDeadSlots, int32_t numChildren,
    bool copyChildren, bool shouldSplitBlock)
{
    ...
    TR::TreeTop* induceOSRCallTree
        = self()->genInduceOSRCallNode(insertionPoint, numChildren, copyChildren, shouldSplitBlock);
    ...
    enclosingBlock->append(
        TR::TreeTop::create(self()->comp(), TR::Node::createWithSymRef(TR::athrow, 1, 1, loadExcpSymbol,
    ...
    self()->insertStoresForDeadStackSlotsBeforeInducingOSR(
        self()->comp(), inlinedSiteIndex, induceOSRCallTree, callSymbolForDeadSlots);
    traceMsg(
    ...
    for (TR::TreeTop* tt = self()->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (self()->comp()->isPotentialOSRPoint(NULL, tt->getNode())) {
            ...
            TR_OSRPoint* osrPoint
                = new (self()->comp()->trHeapMemory()) TR_OSRPoint(ttnode, osrMethodData, self()->comp()->trMemory());
...
bool OMR::ResolvedMethodSymbol::supportsInduceOSR(TR_ByteCodeInfo bci, TR::Node* nodeToOSRAt, TR::Block* blockToOSRAt,
...
void OMR::ResolvedMethodSymbol::setShouldNotAttemptOSR(int32_t n) { _shouldNotAttemptOSR->set(n); }

bool OMR::ResolvedMethodSymbol::cannotAttemptOSR(TR_ByteCodeInfo bci, TR::Node* nodeToOSRAt, TR::Block* blockToOSRAt,
    TR::ResolvedMethodSymbol* calleeSymbolIfCallNode, TR::Compilation* comp)
{
    if (comp->getOption(TR_TraceOSR))
    ...
    /*if (nodeToOSRAt->getByteCodeInfo().doNotProfile())
      ...
      return true; // if this is a call node created by the optimizer (string peepholes) it may not exist in the
      bytecodes
      }*/
    ...
    if (!osrMethodData) {
        if (comp->getOption(TR_TraceOSR))
            traceMsg(comp, "Top level OSR method data is NULL - bci: %d.%d symbol: %s\n", callSite, byteCodeIndex,
                self()->getResolvedMethod()->signature(comp->trMemory()));
        return true;
    }
    ...
    while (osrMethodData) {
        if (comp->getOption(TR_TraceOSR))
            traceMsg(comp,
                "Checking if OSR can be attempted at caller index %d and bytecode index %d for induceOSR call %p\n",
...
bool OMR::ResolvedMethodSymbol::cleanupUnreachableOSRBlocks(int32_t inlinedSiteIndex, TR::Compilation* comp)
{
...
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(TR::Compilation* comp,
    int32_t inlinedSiteIndex, TR::TreeTop* induceOSRTree, TR::ResolvedMethodSymbol* callSymbolForDeadSlots)
{
    if (!comp->osrStateIsReliable()) {
        traceMsg(comp, "OSR state may not be reliable enough to trust liveness info computed at IL gen time; so "
    ...
    if (callSymbolForDeadSlots)
        osrMethodData = comp->getOSRCompilationData()->findCallerOSRMethodData(
            comp->getOSRCompilationData()->findOrCreateOSRMethodData(inlinedSiteIndex, callSymbolForDeadSlots));
    else
    ...
    while (osrMethodData) {
        if (comp->getOption(TR_TraceOSR))
            traceMsg(comp, "Inserting stores for dead stack slots in method at caller index %d and bytecode index %d "
        ...
        if (deadSymRefs) {
            ...
            while (bvi.hasMoreElements()) {
                ...
                if (performTransformation(comp, "OSR LIVE RANGE ANALYSIS : Local %d is reset before induceOSR call "
                                                "tree %p (caller index %d bytecode index %d)\n",
                    ...
                    TR::Node* storeNode = TR::Node::createWithSymRef(
                        ...
                        TR::Node::createConstDead(induceOSRTree->getNode(), nextSymRef->getSymbol()->getDataType()),
                        nextSymRef);
...
bool OMR::ResolvedMethodSymbol::doJSR292PerfTweaks() { return self()->hasMethodHandleInvokes(); }
