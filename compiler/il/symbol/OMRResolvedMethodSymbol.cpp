...
bool OMR::ResolvedMethodSymbol::induceOSRAfter(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, TR::TreeTop* branch, bool extendRemainder)
{
...
OMR::ResolvedMethodSymbol::genInduceOSRCallAndCleanUpFollowingTreesImmediately(TR::TreeTop* insertionPoint, TR_ByteCodeInfo induceBCI, bool copyChildren, bool shouldSplitBlock, TR::Compilation* comp)
{
    ...
    if ((insertionPoint->getNode()->getNumChildren() > 0) && (insertionPoint->getNode()->getFirstChild()->getOpCode().isCall())) {
...
bool OMR::ResolvedMethodSymbol::genIL(TR_FrontEnd* fe, TR::Compilation* comp, TR::SymbolReferenceTable* symRefTab, TR::IlGenRequest& customRequest)
{
    ...
    if (!_firstTreeTop || !comp->isPeekingMethod()) {
        ...
        if (_methodFlags.testAny(IlGenSuccess)) {
            ...
            // add OSR points.
            //
            bool doOSR = comp->getOption(TR_EnableOSR) && !comp->isPeekingMethod() && (comp->getOption(TR_EnableNextGenHCR) || !comp->getOption(TR_EnableHCR));
...
bool OMR::ResolvedMethodSymbol::cannotAttemptOSR(TR_ByteCodeInfo bci,
    TR::Node* nodeToOSRAt,
    TR::Block* blockToOSRAt,
    TR::ResolvedMethodSymbol* calleeSymbolIfCallNode,
    TR::Compilation* comp)
...
void OMR::ResolvedMethodSymbol::insertStoresForDeadStackSlotsBeforeInducingOSR(TR::Compilation* comp, int32_t inlinedSiteIndex, TR::TreeTop* induceOSRTree, TR::ResolvedMethodSymbol* callSymbolForDeadSlots)
{
...
OMR::ResolvedMethodSymbol::findOSRPoint(TR::Node* node)
{
    for (intptrj_t i = 0; i < _osrPoints.size(); ++i) {
        ...
        if (bcinfo.getByteCodeIndex() == node->getByteCodeIndex() && bcinfo.getCallerIndex() == node->getInlinedSiteIndex())
            return _osrPoints[i];
    }
