...
public:
    ...
    enum {
        ...
        // Available                 = 0x10000000,
        IsVMThreadLive
        = 0x08000000, // reg assigner has determined that vmthread must be in the proper register at this label
...
{
    TR_ASSERT(
        isRegisterMappedSymbol(), "OMR::Symbol::castToRegisterMappedSymbol, symbol is not a register mapped symbol");
...
{
    TR_ASSERT(isInternalPointerAuto(),
...
{
    TR_ASSERT(
        isLocalObject(), "OMR::Symbol::castToLocalObjectSymbol, symbol is not an internal pointer automatic symbol");
    return (TR::AutomaticSymbol*)this;
}
...
{
    TR_ASSERT(isMethod(), "OMR::Symbol::castToMethodSymbol, symbol[%p] is not a method symbol", this);
...
{
    TR_ASSERT(
        isCallSiteTableEntry(), "OMR::Symbol::castToCallSiteTableEntrySymbol expected a call site table entry symbol");
    return (TR::StaticSymbol*)this;
}
...
{
    TR_ASSERT(
        isVariableSizeSymbol(), "OMR::Symbol::castToVariableSizeSymbol, symbol is not a VariableSizeSymbol symbol");
    return (TR::AutomaticSymbol*)this;
}
