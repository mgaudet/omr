...
TR::IlValue* IlBuilder::newValue(TR::IlType* dt) { return newValue(dt->getPrimitiveType()); }
...
void IlBuilder::pullInBuilderTrees(
...
TR::Node* IlBuilder::zero(TR::IlType* dt) { return zero(dt->getPrimitiveType()); }

TR::Node* IlBuilder::zeroNodeForValue(TR::IlValue* v) { return zero(v->getSymbol()->getDataType()); }
...
void IlBuilder::AppendBuilder(TR::IlBuilder* builder)
{
    ...
    // this block we're about to create
...
TR::Node* IlBuilder::loadValue(TR::IlValue* v) { return TR::Node::createLoad(v); }
...
TR::IlValue* IlBuilder::indirectLoadNode(TR::IlType* dt, TR::Node* addr, bool isVectorLoad)
{
...
void IlBuilder::StoreAt(TR::IlType* dt, TR::IlValue* address, TR::IlValue* value)
{
    ILB_REPLAY(
        "%s->StoreAt(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_TYPE(dt), REPLAY_VALUE(address), REPLAY_VALUE(value));
...
void IlBuilder::VectorStoreAt(TR::IlType* dt, TR::IlValue* address, TR::IlValue* value)
{
    ILB_REPLAY("%s->VectorStoreAt(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_TYPE(dt), REPLAY_VALUE(address),
        REPLAY_VALUE(value));
...
TR::IlValue* IlBuilder::CreateLocalArray(int32_t numElements, TR::IlType* elementType)
{
    ...
    ILB_REPLAY("%s = %s->CreateLocalArray(%d, %s);", REPLAY_VALUE(arrayAddressValue), REPLAY_BUILDER(this), numElements,
        REPLAY_TYPE(elementType));

    TraceIL("IlBuilder[ %p ]::CreateLocalArray array allocated %d bytes, address in %d\n", this, size,
        arrayAddressValue->getCPIndex());
...
TR::IlValue* IlBuilder::CreateLocalStruct(TR::IlType* structType)
{
    ...
    ILB_REPLAY("%s = %s->CreateLocalStruct(%s);", REPLAY_VALUE(structAddressValue), REPLAY_BUILDER(this),
        REPLAY_TYPE(newStructType));

    TraceIL("IlBuilder[ %p ]::CreateLocalStruct struct allocated %d bytes, address in %d\n", this, size,
        structAddressValue->getCPIndex());
    return structAddressValue;
}

void IlBuilder::StoreIndirect(const char* type, const char* field, TR::IlValue* object, TR::IlValue* value)
{
    ILB_REPLAY("%s->StoreIndirect(\"%s\", \"%s\", %s, %s);", REPLAY_BUILDER(this), type, field, REPLAY_VALUE(object),
        REPLAY_VALUE(value));
...
TR::IlValue* IlBuilder::LoadIndirect(const char* type, const char* field, TR::IlValue* object)
{
    TR::SymbolReference* symRef = (TR::SymbolReference*)_types->FieldReference(type, field);
    TR::DataType fieldType = symRef->getSymbol()->getDataType();
    TR::IlValue* returnValue = newValue(fieldType);
    TraceIL("IlBuilder[ %p ]::%d is LoadIndirect %s.%s from (%d)\n", this, returnValue->getCPIndex(), type, field,
        object->getCPIndex());
    storeNode(returnValue,
        TR::Node::createWithSymRef(comp()->il.opCodeForIndirectLoad(fieldType), 1, loadValue(object), 0, symRef));
    ILB_REPLAY("%s = %s->LoadIndirect(%s, %s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), type, field,
        REPLAY_VALUE(object));
    return returnValue;
}
...
TR::IlValue* IlBuilder::LoadAt(TR::IlType* dt, TR::IlValue* address)
{
    ...
    ILB_REPLAY("%s = %s->LoadAt(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_TYPE(dt),
        REPLAY_VALUE(address));
    return returnValue;
}
...
TR::IlValue* IlBuilder::IndexAt(TR::IlType* dt, TR::IlValue* base, TR::IlValue* index)
{
    ...
    ILB_REPLAY("%s = %s->IndexAt(%s, %s, %s);", REPLAY_VALUE(address), REPLAY_BUILDER(this), REPLAY_TYPE(dt),
        REPLAY_VALUE(base), REPLAY_VALUE(index));
...
TR::IlValue* IlBuilder::ConvertTo(TR::IlType* t, TR::IlValue* v)
{
    ...
    ILB_REPLAY("%s = %s->ConvertTo(%s, %s);", REPLAY_VALUE(convertedValue), REPLAY_BUILDER(this), REPLAY_TYPE(t),
        REPLAY_VALUE(value));
...
TR::Node* IlBuilder::binaryOpNodeFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
...
TR::IlValue* IlBuilder::binaryOpFromNodes(TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode)
{
...
TR::IlValue* IlBuilder::binaryOpFromOpMap(OpCodeMapper mapOp, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
...
TR::IlValue* IlBuilder::binaryOpFromOpCode(TR::ILOpCodes op, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
...
TR::IlValue* IlBuilder::compareOp(TR_ComparisonTypes ct, bool needUnsigned, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    TR::Node* rightNode = loadValue(right);
...
TR::IlValue* IlBuilder::NotEqualTo(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is NotEqualTo %d != %d?\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->NotEqualTo(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
void IlBuilder::Return(TR::IlValue* value)
{
    ...
    appendBlock();
    TR::Node* returnNode
        = TR::Node::create(TR::ILOpCode::returnOpCode(value->getSymbol()->getDataType()), 1, loadValue(value));
    genTreeTop(returnNode);
    cfg()->addEdge(_currentBlock, cfg()->getEnd());
    setDoesNotComeBack();
}
...
TR::IlValue* IlBuilder::Sub(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Sub %d - %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->Sub(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::Add(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Add %d + %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->Add(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
 ...
 */
void IlBuilder::appendExceptionHandler(TR::Block* blockThrowsException, TR::IlBuilder** handler, uint32_t catchType)
{
...
{
    TR::Node* overflowChkNode = TR::Node::createWithRoomForOneMore(TR::OverflowCHK, 3,
...
TR::IlValue* IlBuilder::operationWithOverflow(
    TR::ILOpCodes op, TR::Node* leftNode, TR::Node* rightNode, TR::IlBuilder** handler)
{
    /*
     * BB1:
     *    overflowCHK
...
TR::IlValue* IlBuilder::AddWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
    ...
    if (left->getSymbol()->getDataType() == TR::Address) {
        if (right->getSymbol()->getDataType() == TR::Int32)
            ...
            TR_ASSERT(0, "the right child type must be either TR::Int32 or TR::Int64 when the left child of Add is "
                         "TR::Address\n");
    } else
        op = addOpCode(leftNode->getDataType());

    TR::IlValue* addValue = operationWithOverflow(op, leftNode, rightNode, handler);
    TraceIL("IlBuilder[ %p ]::%d is AddWithOverflow %d + %d\n", this, addValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    // ILB_REPLAY("%s = %s->AddWithOverflow(%s, %s);", REPLAY_VALUE(addValue), REPLAY_BUILDER(this),
    // REPLAY_BUILDER(*handler), REPLAY_VALUE(left), REPLAY_VALUE(right));
    return addValue;
}

TR::IlValue* IlBuilder::SubWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
    ...
    TR::Node* rightNode = loadValue(right);
    TR::IlValue* subValue
        = operationWithOverflow(TR::ILOpCode::subtractOpCode(leftNode->getDataType()), leftNode, rightNode, handler);
    TraceIL("IlBuilder[ %p ]::%d is SubWithOverflow %d + %d\n", this, subValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    return subValue;
}

TR::IlValue* IlBuilder::MulWithOverflow(TR::IlBuilder** handler, TR::IlValue* left, TR::IlValue* right)
{
    appendBlock();
    ...
    TR::Node* rightNode = loadValue(right);
    TR::IlValue* mulValue
        = operationWithOverflow(TR::ILOpCode::multiplyOpCode(leftNode->getDataType()), leftNode, rightNode, handler);
    TraceIL("IlBuilder[ %p ]::%d is MulWithOverflow %d + %d\n", this, mulValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    return mulValue;
}
...
TR::IlValue* IlBuilder::Mul(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Mul %d * %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->Mul(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::Div(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Div %d / %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->Div(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::And(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is And %d & %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->And(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::Xor(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is Xor %d ^ %d\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->Xor(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::ShiftL(TR::IlValue* v, TR::IlValue* amount)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this, returnValue->getCPIndex(), v->getCPIndex(),
        amount->getCPIndex());
    ILB_REPLAY("%s = %s->ShiftL(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(v),
        REPLAY_VALUE(amount));
    return returnValue;
}
...
TR::IlValue* IlBuilder::ShiftR(TR::IlValue* v, TR::IlValue* amount)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this, returnValue->getCPIndex(), v->getCPIndex(),
        amount->getCPIndex());
    ILB_REPLAY("%s = %s->ShiftR(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(v),
        REPLAY_VALUE(amount));
    return returnValue;
}
...
TR::IlValue* IlBuilder::UnsignedShiftR(TR::IlValue* v, TR::IlValue* amount)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is shr %d << %d\n", this, returnValue->getCPIndex(), v->getCPIndex(),
        amount->getCPIndex());
    ILB_REPLAY("%s = %s->UnsignedShiftR(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(v),
        REPLAY_VALUE(amount));
    return returnValue;
}
...
TR::IlValue* IlBuilder::EqualTo(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is EqualTo %d == %d?\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->EqualTo(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::LessThan(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is LessThan %d < %d?\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->LessThan(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::GreaterThan(TR::IlValue* left, TR::IlValue* right)
{
    ...
    TraceIL("IlBuilder[ %p ]::%d is GreaterThan %d < %d?\n", this, returnValue->getCPIndex(), left->getCPIndex(),
        right->getCPIndex());
    ILB_REPLAY("%s = %s->GreaterThan(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    return returnValue;
}
...
TR::IlValue* IlBuilder::Call(const char* functionName, int32_t numArgs, TR::IlValue** argValues)
{
    //   ILB_REPLAY("%s = %s->GreaterThan(%s, %s);", REPLAY_VALUE(returnValue), REPLAY_BUILDER(this),
    //   REPLAY_VALUE(left), REPLAY_VALUE(right));
    ...
    TR::SymbolReference* methodSymRef
        = symRefTab()->findOrCreateMethodSymbol(JITTED_METHOD_INDEX, -1, resolvedMethod, TR::MethodSymbol::Static);
...
void IlBuilder::IfCmpNotEqualZero(TR::IlBuilder** target, TR::IlValue* condition)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY(
        "%s->IfCmpNotEqualZero(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(condition));
    TraceIL("IlBuilder[ %p ]::IfCmpNotEqualZero %d? -> B%d\n", this, condition->getCPIndex(),
        (*target)->getEntry()->getNumber());
    ifCmpNotEqualZero(condition, (*target)->getEntry());
}

void IlBuilder::IfCmpNotEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY("%s->IfCmpNotEqual(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
    TraceIL("IlBuilder[ %p ]::IfCmpNotEqual %d == %d? -> B%d\n", this, left->getCPIndex(), right->getCPIndex(),
        (*target)->getEntry()->getNumber());
...
void IlBuilder::IfCmpEqualZero(TR::IlBuilder** target, TR::IlValue* condition)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY(
        "%s->IfCmpEqualZero(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(condition));
    TraceIL("IlBuilder[ %p ]::IfCmpEqualZero %d == 0? -> B%d\n", this, condition->getCPIndex(),
        (*target)->getEntry()->getNumber());
    ifCmpEqualZero(condition, (*target)->getEntry());
}

void IlBuilder::IfCmpEqual(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY("%s->IfCmpEqual(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
...
void IlBuilder::IfCmpLessThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY("%s->IfCmpLessThan(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
...
void IlBuilder::IfCmpGreaterThan(TR::IlBuilder** target, TR::IlValue* left, TR::IlValue* right)
{
    *target = createBuilderIfNeeded(*target);
    ILB_REPLAY("%s->IfCmpGreaterThan(%s, %s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(left),
        REPLAY_VALUE(right));
...
void IlBuilder::ifCmpNotEqual(TR::IlValue* left, TR::IlValue* right, TR::Block* target)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    ifjump(comp()->il.opCodeForIfCompareNotEquals(leftNode->getDataType()), leftNode, loadValue(right), target);
    appendBlock();
}
...
void IlBuilder::ifCmpEqual(TR::IlValue* left, TR::IlValue* right, TR::Block* target)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    ifjump(comp()->il.opCodeForIfCompareEquals(leftNode->getDataType()), leftNode, loadValue(right), target);
    appendBlock();
}

void IlBuilder::ifCmpLessThan(TR::IlValue* left, TR::IlValue* right, TR::Block* target)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    ifjump(comp()->il.opCodeForIfCompareLessThan(leftNode->getDataType()), leftNode, loadValue(right), target);
    appendBlock();
}

void IlBuilder::ifCmpGreaterThan(TR::IlValue* left, TR::IlValue* right, TR::Block* target)
{
    appendBlock();
    TR::Node* leftNode = loadValue(left);
    ifjump(comp()->il.opCodeForIfCompareGreaterThan(leftNode->getDataType()), leftNode, loadValue(right), target);
    appendBlock();
}
 ...
 */
void IlBuilder::IfThenElse(TR::IlBuilder** thenPath, TR::IlBuilder** elsePath, TR::IlValue* condition)
{
    // ILB_REPLAY_BEGIN();
    ...
    // ILB_REPLAY_END();
    // ILB_REPLAY("IfThenElse(%s, %s, %s);", REPLAY_PTRTOBUILDER(thenPath), REPLAY_PTRTOBUILDER(elsePath),
    // REPLAY_VALUE(condition));
}

void IlBuilder::Switch(const char* selectionVar, TR::IlBuilder** defaultBuilder, uint32_t numCases, int32_t* caseValues,
    TR::IlBuilder** caseBuilders, bool* caseFallsThrough)
{
    // ILB_REPLAY_BEGIN();
    TR::IlValue* selectorValue = Load(selectionVar);
    TR_ASSERT(
        selectorValue->getSymbol()->getDataType() == TR::Int32, "Switch only supports selector having type Int32");
    *defaultBuilder = createBuilderIfNeeded(*defaultBuilder);
    ...
    // each case handler is a sequence of two builder objects: first the one passed in via caseBuilder (or will be
    // passed
...
void IlBuilder::Switch(const char* selectionVar, TR::IlBuilder** defaultBuilder, uint32_t numCases, ...)
{
    // TODO figure out switch replay
    // ILB_REPLAY("%s->IfCmpGreaterThan(%s, %s);", REPLAY_BUILDER(this), REPLAY_BUILDER(target), REPLAY_VALUE(left),
    // REPLAY_VALUE(right));
    ...
    TR::IlBuilder** caseBuilders
        = (TR::IlBuilder**)_comp->trMemory()->allocateHeapMemory(numCases * sizeof(TR::IlBuilder*));
...
void IlBuilder::ForLoop(bool countsUp, const char* indVar, TR::IlBuilder** loopCode, TR::IlBuilder** breakBuilder,
    TR::IlBuilder** continueBuilder, TR::IlValue* initial, TR::IlValue* end, TR::IlValue* increment)
{
    ...
    *loopCode = createBuilderIfNeeded(*loopCode);
    ...
    TraceIL("IlBuilder[ %p ]::ForLoop ind %s initial %d end %d increment %d loopCode %p countsUp %d\n", this, indVar,
        initial->getCPIndex(), end->getCPIndex(), increment->getCPIndex(), *loopCode, countsUp);
