...
TR::MethodBuilder* MethodBuilder::asMethodBuilder() { return static_cast<TR::MethodBuilder*>(this); }
...
TR::IlValue* MethodBuilder::lookupSymbol(const char* name)
{
    ...
    if (_parameterSlot->locate(name, slotID)) {
        symRef = symRefTab()->findOrCreateAutoSymbol(
            _methodSymbol, (int32_t)(uintptr_t)_parameterSlot->getData(slotID), type, true, false, true);
    } else {
...
TR::BytecodeBuilder* MethodBuilder::OrphanBytecodeBuilder(int32_t bcIndex, char* name)
{
...
void MethodBuilder::DefineMemory(const char* name, TR::IlType* dt, void* location)
{
    MB_REPLAY(
        "DefineMemory(\"%s\", %s, " REPLAY_POINTER_FMT ");", name, REPLAY_TYPE(dt), REPLAY_POINTER(location, name));
    TR_HashId typesID;
    _symbolTypes->add(name, typesID, (void*)dt);
...
void MethodBuilder::DefineArrayParameter(const char* name, TR::IlType* elementType)
{
...
void MethodBuilder::DefineFunction(
    char* name, char* fileName, char* lineNumber, void* entryPoint, TR::IlType* returnType, int32_t numParms, ...)
{
...
void MethodBuilder::DefineFunction(char* name, char* fileName, char* lineNumber, void* entryPoint,
    TR::IlType* returnType, int32_t numParms, TR::IlType** parmTypes)
{
    ...
    TR::ResolvedMethod* method = new (PERSISTENT_NEW)
        TR::ResolvedMethod(fileName, lineNumber, name, numParms, parmTypes, returnType, entryPoint, 0);
