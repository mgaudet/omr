...
#define MB_REPLAY(...) \
    {                  \
        REPLAY({sprintf(_rpLine, ##__VA_ARGS__); (*_rpCpp) << "\t" << _rpLine << std::endl; })    \
    }
#define MB_REPLAY_NONL(...) \
    {                       \
        REPLAY({sprintf(_rpLine, ##__VA_ARGS__); (*_rpCpp) << "\t" << _rpLine; })         \
    }
    ...
    , _allBytecodeBuilders(0)
{
    REPLAY({
        ...
        (*_rpCpp) << "#include \"ilgen/TypeDictionary.hpp\"" << std::endl
                  << std::endl;
        (*_rpCpp) << "#include \"ReplayMethod.hpp\"" << std::endl
                  << std::endl;
        ...
        (*_rpILCpp) << "#include \"ilgen/TypeDictionary.hpp\"" << std::endl
                    << std::endl;
        (*_rpILCpp) << "#include \"ReplayMethod.hpp\"" << std::endl
                    << std::endl;
...
void MethodBuilder::DefineFunction(char* name,
    char* fileName,
    char* lineNumber,
    void* entryPoint,
