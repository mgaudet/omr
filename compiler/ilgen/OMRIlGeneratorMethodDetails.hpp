 ...
 *
 * IlGeneratorMethodDetailsBase defines the IlGenRequest API that common code can count on, although it's more
 * documentation
 ...
 *
 * Note all functions are non-virtual in this class, so declarations should never use the IlGeneratorMethodDetailsBase
 * type
