...
size_t IlType::getStructSize()
{
    TR_ASSERT(0, "The input type is not a StructType\n");
    return 0;
}

const uint8_t primitiveTypeAlignment[TR::NumOMRTypes] = { 1, 1, 2, 4, 8,
...
size_t StructType::getStructSize()
{
    size_t size = 0;
...
void TypeDictionary::DefineField(const char* structName, const char* fieldName, TR::IlType* type)
{
    TR_HashId structID = 0;
    if (_structsByName->locate(structName, structID)) {
...
TR::IlType* TypeDictionary::GetFieldType(const char* structName, const char* fieldName)
{
    TR_HashId structID = 0;
    _structsByName->locate(structName, structID);
...
TR::IlType* TypeDictionary::PointerTo(TR::IlType* baseType) { return new (PERSISTENT_NEW) PointerType(baseType); }

TR::IlReference* TypeDictionary::FieldReference(const char* structName, const char* fieldName)
{
    TR_HashId structID = 0;
    _structsByName->locate(structName, structID);
    StructType* theStruct = (StructType*)_structsByName->getData(structID);
