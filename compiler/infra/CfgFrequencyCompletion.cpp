...
void TR_BlockFrequenciesCompletion::propogateToDominator(
...
void TR_BlockFrequenciesCompletion::gatherFreqFromSingleEdgePredecessors(
    TR::Block* block, TR_Queue<TR::Block>* blockQueue)
{
...
void TR_BlockFrequenciesCompletion::gatherFreqFromSingleEdgeSuccessors(
    TR::Block* block, TR_Queue<TR::Block>* blockQueue)
{
...
bool TR_EdgeFrequenciesCompletion::canCompletingEdge(
    TR::CFGEdge* edge, EdgeDirection direction, int32_t iterationNumber, int32_t* maxAllowedFrequency)
{
    ...
    if (direction == INCOMING)
        neighbors.insert(
            neighbors.end(), block->getExceptionPredecessors().begin(), block->getExceptionPredecessors().end());
    else
        neighbors.insert(
            neighbors.end(), block->getExceptionSuccessors().begin(), block->getExceptionSuccessors().end());
