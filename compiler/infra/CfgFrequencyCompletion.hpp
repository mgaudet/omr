...
private:
    ...
    TR::CFGEdge* getIfSingleEdge(TR::Block* block, bool isPredecessor)
    {
        ...
        if (isPredecessor)
            edgesList.insert(
                edgesList.end(), block->getExceptionPredecessors().begin(), block->getExceptionPredecessors().end());
        else
            edgesList.insert(
                edgesList.end(), block->getExceptionSuccessors().begin(), block->getExceptionSuccessors().end());
...
public:
    TR_BlockFrequenciesCompletion(TR::Compilation* comp, TR::ResolvedMethodSymbol* methodSymbol, TR::CFG* cfg = 0)
        : _comp(comp)
        , _cfg(cfg ? cfg : _comp->getFlowGraph())
    {
        if (methodSymbol) {
    ...
    void propogateToDominator(
        TR::Block* block, TR_BitVector* regionBlocks, TR_Queue<TR::Block>* blockQueue, bool usePostDominators);
...
public:
    TR_EdgeFrequenciesCompletion(TR::Compilation* comp, TR::CFG* cfg = 0)
        : _comp(comp)
        , _cfg(cfg ? cfg : comp->getFlowGraph())
        , _edgeList(_comp->trMemory())
        , _blockWithUnknownFreqList(_comp->trMemory())
    {
    }
    void dumpEdge(TR::Compilation* comp, const char* title, TR::CFGEdge* edge);
    bool canCompletingEdge(
        TR::CFGEdge* edge, EdgeDirection direction, int32_t iterationNumber, int32_t* maxAllowedFrequency);
