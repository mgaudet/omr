...
TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
    TR::Block* start, TR::Optimization* opt, const char* name)
...
TR::ReversePostorderSnapshotBlockIterator::ReversePostorderSnapshotBlockIterator(
    TR::CFG* cfg, TR::Optimization* opt, const char* name)
    : BlockIterator(opt, name)
...
void TR::ILValidator::checkSoundness(TR::TreeTop* start, TR::TreeTop* stop)
{
    ...
    for (TR::TreeTop* currentTree = start; currentTree != stop; currentTree = currentTree->getNextTreeTop()) {
        ...
        if (next) {
            soundnessRule(currentTree, next->getNode() != NULL, "Tree after n%dn must have a node",
                currentTree->getNode()->getGlobalIndex());
            soundnessRule(currentTree, next->getPrevTreeTop() == currentTree,
            ...
            soundnessRule(currentTree, stop == NULL,
                ...
                currentTree->getNode()->getGlobalIndex(), stop ? stop->getNode()->getGlobalIndex() : NULL);
            checkNodeSoundness(currentTree, currentTree->getNode(), ancestorNodes, visitedNodes);
        }
    }
}

void TR::ILValidator::checkNodeSoundness(
    TR::TreeTop* location, TR::Node* node, NodeChecklist& ancestorNodes, NodeChecklist& visitedNodes)
{
...
void TR::ILValidator::updateNodeState(Location& newLocation)
{
    ...
    if (node->getReferenceCount() == state._futureReferenceCount) {
        ...
        if (node->getReferenceCount() == 0) {
            validityRule(
                newLocation, node->getOpCode().isTreeTop(), "Only nodes with isTreeTop opcodes can have refcount == 0");
        } else {
            _liveNodes.add(node);
        }
        ...
        for (auto childIter = node->childIterator(); TR::Node* child = *childIter; ++childIter) {
            ...
            if (_liveNodes.contains(child)) {
                validityRule(newLocation, childState._futureReferenceCount >= 1,
                    "Child %s n%dn already has reference count 0", child->getOpCode().getName(),
                    child->getGlobalIndex());
                if (--childState._futureReferenceCount == 0) {
                    _liveNodes.remove(child);
                }
            } else {
                validityRule(newLocation, node->getOpCode().isTreeTop(), "Child %s n%dn has already gone dead",
...
bool TR::ILValidator::treesAreValid(TR::TreeTop* start, TR::TreeTop* stop)
{
    ...
    for (PostorderNodeOccurrenceIterator iter(start, _opt, "VALIDATOR"); iter != stop; ++iter) {
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            ...
            if (nextTree) {
                validityRule(
                    iter, nextTree->getNode()->getOpCodeValue() == TR::BBStart, "Expected BBStart after BBEnd");
...
void TR::ILValidator::validateEndOfExtendedBlock(Location& location)
{
    ...
    for (LiveNodeWindow::Iterator lnwi(_liveNodes); lnwi.currentNode(); ++lnwi)
        validityRule(
            location, false, "Node cannot live across block boundary at n%dn", lnwi.currentNode()->getGlobalIndex());
