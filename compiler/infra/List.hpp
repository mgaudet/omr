...
public:
    ...
    {
        TR_ASSERT(this->_allocKind != persistentAlloc, "Should not be using heap allocation to add to a persistent "
    ...
    ListElement<T>* addAfter(T* elem, ListElement<T>* previous)
    {
        ...
        if (previous == NULL)
            ...
            TR_ASSERT(this->_allocKind != persistentAlloc, "Should not be using heap allocation to add to a persistent "
                                                           "list. The 'addAfter' routine in TR_PersistentList should "
                                                           "be called\n");
