...
public:
    ...
    {
        if (_pTail == NULL) {
            List<T>::_pHead = _pTail = _useFreeList ? new (_freeList) ListElement<T>(elem, List<T>::_pHead) : new (List<T>::trStackMemory()) ListElement<T>(elem, List<T>::_pHead);
        } else {
            ListElement<T>* p = _useFreeList ? new (_freeList) ListElement<T>(elem, NULL) : new (List<T>::trStackMemory()) ListElement<T>(elem, NULL);
