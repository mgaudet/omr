...
class LoopInfo {
public:
    LoopInfo(TR::Allocator allocator)
        : isInLoop(allocator)
    {
    }
    int32_t top;
                        ...
                        if (loops[j].top >= index) {
                            if (trace)
                                dumpOptDetails(comp(), "Block %d in a loop based at block %d\n", number,
                                    stack[loops[j].bottom]->getNumber());
...
void OMR::CFG::setBlockAndEdgeFrequenciesBasedOnStructure()
{
    ...
    if (setBlockFrequencies) {
        if (_compilation->getOption(TR_TraceBFGeneration))
            dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating block and edge "
                                   "frequencies within regions...\n");
...

        {
            ...
            if (_compilation->getOption(TR_TraceBFGeneration))
                dumpOptDetails(comp(), "\nsetBlockAndEdgeFrequenciesBasedOnStructure: Propagating weight factors based "
                                       "on CFG structure...\n");
...
void OMR::CFG::findReachableBlocks(TR_BitVector* result)
{
        ...
        if (!result->get(node->getNumber())) {
...

            {
                ...
                for (TR::CFGEdge* edge = edges.getFirst(); edge; edge = edges.getNext()) {
                    ...
                    // remove the start block predecessor of AlwaysKeepBlocks if they can be reached from somewhere else
                    // (excluding multiple entry points, and external_defined labels)
                    if (edge->getTo()->asBlock()->isAlwaysKeepBlock() && edge->getFrom() != getStart()
                        && edge->getTo()->hasPredecessor(getStart())) {
                    }
                }
            }
        }
    ...
    // make the alwayskeepblocks unreachable
