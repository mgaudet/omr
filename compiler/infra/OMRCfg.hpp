...
public:
     ...
     */
    TR::CFGEdge* getCurrent() { return (currentIterator == combinedList.end()) ? NULL : *currentIterator; }

    /**
     * Get the next element, return NULL if we've hit the end of the list
     */
    TR::CFGEdge* getNext() { return (++currentIterator == combinedList.end()) ? NULL : *currentIterator; }

private:
    TR::list<TR::CFGEdge*> combinedList;

    std::list<TR::CFGEdge*, TR::typed_allocator<TR::CFGEdge*, TR::Allocator> >::iterator currentIterator;
};
