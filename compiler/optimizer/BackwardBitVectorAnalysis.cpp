...
bool TR_BackwardDFSetAnalysis<Container*>::canGenAndKillForStructure(TR_Structure* nodeStructure)
{
    ...
    if (naturalLoop && naturalLoop->isNaturalLoop()) {
        ...
        predList.insert(
            predList.end(), entryNode->getExceptionPredecessors().begin(), entryNode->getExceptionPredecessors().end());
        for (auto pred = predList.begin(); pred != predList.end(); ++pred) {
            ...
            succList.insert(
                succList.end(), predNode->getExceptionSuccessors().begin(), predNode->getExceptionSuccessors().end());
        ...
        for (TR::CFGEdge* edge = ei.getCurrent(); edge != NULL; edge = ei.getNext()) {
            ...
            predList.insert(
                predList.end(), toNode->getExceptionPredecessors().begin(), toNode->getExceptionPredecessors().end());
            for (auto pred = predList.begin(); pred != predList.end(); ++pred) {
...
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(
    TR_RegionStructure* regionStructure, TR::BitVector& pendingList, TR::BitVector& exitNodes, bool lastIteration)
{
    while (this->_analysisQueue.getListHead()
        ...
        TR::BitVector seenNodes(this->comp()->allocator());
        {
            ...
            succList.insert(succList.end(), nodeStructure->getExceptionSuccessors().begin(),
            ...
            for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
                if (!regionStructure->isExitEdge(*succ)) {
                    ...
                    if ((regionStructure->getEntry() == succNode) || exitNodes.ValueAt(succNode->getNumber()))
                        continue;
        ...
        predList.insert(predList.end(), nodeStructure->getExceptionPredecessors().begin(),
...
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfoForRegion(TR_RegionStructure* region)
{
    ...
    // TR_BitVector *originalPendingList = new (this->trStackMemory()) TR_BitVector(this->_numberOfNodes,
    // this->trMemory(), stackAlloc);
...
bool TR_BackwardDFSetAnalysis<Container*>::analyzeNodeIfSuccessorsAnalyzed(
    TR_RegionStructure* regionStructure, TR::BitVector& pendingList, TR::BitVector& exitNodes)
{
