...
template <class Container>
void TR_BackwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(TR_RegionStructure* regionStructure, TR::BitVector& pendingList, TR::BitVector& exitNodes, bool lastIteration)
{
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        if (!pendingList.ValueAt(nodeStructure->getStructure()->getNumber()) && (*(this->_changedSetsQueue.getListHead()->getData()) == 0)) {
...
template <class Container>
bool TR_BackwardDFSetAnalysis<Container*>::analyzeNodeIfSuccessorsAnalyzed(TR_RegionStructure* regionStructure, TR::BitVector& pendingList, TR::BitVector& exitNodes)
{
    ...
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        for (auto pred = predList.begin(); pred != predList.end(); ++pred) {
            TR_StructureSubGraphNode* predNode = (TR_StructureSubGraphNode*)(*pred)->getFrom();
            TR_Structure* predStructure = predNode->getStructure();
            if ((needToIterate || (node != regionStructure->getEntry())) && (pendingList.ValueAt(predStructure->getNumber()) || inSetChanged)) {
                this->_nodesInCycle->Clear();
