...
template <class Container> TR::BitVector* TR_BasicDFSetAnalysis<Container*>::allocateBitVector()
{
    ...
    TR_RegionStructure* regionStructure, TR::BitVector& pendingList)
{
    while (this->_analysisQueue.getListHead()
        ...
        if (node != regionStructure->getEntry()) {
            ...
            predList.insert(
                predList.end(), node->getExceptionPredecessors().begin(), node->getExceptionPredecessors().end());
            bool firstPred = true;
    ...
    TR_RegionStructure* regionStructure, TR::BitVector& pendingList)
{
    ...
    while (this->_analysisQueue.getListHead()
        ...
        if (this->supportsGenAndKillSets() && canGenAndKillForStructure(nodeStructure->getStructure())) {
            TR::list<TR::CFGEdge*> succList(node->getSuccessors());
            succList.insert(
                succList.end(), node->getExceptionSuccessors().begin(), node->getExceptionSuccessors().end());
            int count = 0;
            for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
                bool normalSucc = (++count <= node->getSuccessors().size());
