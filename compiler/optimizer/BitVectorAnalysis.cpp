...
template <class Container>
void TR_ForwardDFSetAnalysis<Container*>::initializeGenAndKillSetInfo(TR_RegionStructure* regionStructure, TR::BitVector& pendingList)
{
        ...
        *this->_temp2 = *_currentRegularKillSetInfo;
        ...
        for (auto succ = succList.begin(); succ != succList.end(); ++succ) {
            ...
            *_currentRegularKillSetInfo = *this->_temp2;
            ...
            if ((!regionStructure->isExitEdge(*succ)) && (pendingList.ValueAt(succNode->getNumber()))) {
                this->_nodesInCycle->Clear();
...
template <class Container>
bool TR_ForwardDFSetAnalysis<Container*>::analyzeNodeIfPredecessorsAnalyzed(TR_RegionStructure* regionStructure, TR::BitVector& pendingList)
{
    ...
    while (this->_analysisQueue.getListHead() && (this->_analysisQueue.getListHead()->getData()->getStructure() != regionStructure)) {
        ...
        for (auto succ = node->getSuccessors().begin(); succ != node->getSuccessors().end(); ++succ) {
            TR::CFGNode* succNode = (*succ)->getTo();
            if ((!regionStructure->isExitEdge(*succ)) && (needToIterate || (succNode != regionStructure->getEntry())) && (pendingList.ValueAt(succNode->getNumber()) || outSetChanged)) {
                this->_nodesInCycle->Clear();
