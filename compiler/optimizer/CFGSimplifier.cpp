    ...
    TR::Block* joinBlock, TR::Node* branchNode, TR::Node* store1Node, TR::Node* store2Node)
{
    ...
    // Everything matches
    if (!performTransformation(comp(), "%sReplace (branch on condition code [%p] -> boolean stores -> "
                                       "branch-and-compare using stored boolean) with single branch on condition "
                                       "code\n",
