...
private:
    void replaceTargetNodeBySourceTree()
    {
        ...
        TR_ASSERT(_parentOfTargetNode != 0, "could not find _targetNode " POINTER_PRINTF_FORMAT
    ...
    void fixupBCDPrecisionIfRequired()
    {
        ...
        if (oldNode->getType().isBCD() && newNode->getType().isBCD()) {
            ...
            if (newNode->getNumChildren() == 2) {
                ...
                // checks
                // below pass then can reduce the newPrecision to 30 so a pdModPrec correction is not needed (as the
                // propagated symbol will not have
                ...
                if (newNode->getType().isAnyPacked()) {
                        ...
                        oldSize == newSymSize) // 16 == 16
                    {
                        if (trace() || comp()->cg()->traceBCDCodeGen()) {
                            traceMsg(comp(), "reduce newPrecision %d->%d for odd to even truncation (origNode %s (%p) "
                        ...
                        newPrecision = oldPrecision; // 31->30 -- no correction needed for same byte odd to even
                                                     // truncation if the origNode was only storing out the even $ of
                                                     // digits
                    }
                }
            }
    ...
    void propagateThroughToCommoningPoints()
    {
        ...
        if (oldNode->getReferenceCount() > 0) {
            ...
            // commoned the old node.
            // We want to replace those commonings with a commoning to the new RHS of the node because the value is the
            // same.
...
int32_t TR_CopyPropagation::perform()
{
...

    {
        ...
        _storeTreeTopsAsArray
            = (TR::TreeTop**)trMemory()->allocateStackMemory(_numStoreTreeTops * sizeof(TR::TreeTop*));
        ...
        int32_t* usesToBeFixed
            = (int32_t*)trMemory()->allocateStackMemory(useDefInfo->getTotalNodes() * sizeof(int32_t));
                    ...
                    if (prevDefNode->getOpCode().isStoreDirect()
                        ...
                        if (rhsOfStoreDefNode == rhsOfPrevStoreDefNode) {
                            ...
                            if (prevDefNode->getType().isBCD()) {
                                TR_ASSERT(defNode->getType().isBCD(), "expecting defNode to be a BCD type (dt=%d) when "
                                                                      "prevDefNode is a BCD type (dt=%d)\n",
...
#endif
            {
                ...
                if (origNode->getOpCode().isLoadIndirect()) {
                    ...
                    if (trace() || comp()->cg()->traceBCDCodeGen())
                        traceMsg(comp(),
                            ...
                            origNode->getOpCode().hasSymbolReference()
                                ...
                                : -1,
                            origNode->getOpCode().getName(), origNode, origNode->getOpCode().hasSymbolReference()
                                ? origNode->getSymbolReference()->getReferenceNumber()
                                : -1,
                    ...
                    if (origNode->getNumChildren() == 2) {
                        ...
                        // sizes match and the other checks
                        // below pass then can reduce the newPrecision to 30 so a pdModPrec correction is not needed (as
                        // the propagated symbol will not have
                        // the top nibble 30->31 set).
                        ...
                        if (node->getType().isAnyPacked()) {
                            ...
                            {
                                if (trace() || comp()->cg()->traceBCDCodeGen())
                                    traceMsg(comp(), "reduce newPrecision %d->%d for odd to even truncation (origNode "
                                                     "%s (%p) prec=%d, node %s (%p) prec=%d\n",
                    ...
                    if (needsClean || needsPrecisionCorrection) {
                        ...
                        // modifyPrecision node must be prepended in case
                        // the new parent requires explicit widening (such as if the parent is a call).
                        // Also if the defNode was going to clean must also insert a pdclean on top of the node with the
                        // propagated symRef
                        ...
                        dumpOptDetails(comp(), "node %p precision %d != propagated symRef #%d precision %d and/or "
                                               "needsClean (%s) so create nodeCopy %p (isTwoChildrenCase=%s)\n",
                        ...
                        if (needsClean) {
                            ...
                            TR_ASSERT(cleanOp != TR::BadILOp,
                                "no bcd modify precision cleanOp found for node %p with type %d\n", nodeCopy,
                                nodeCopy->getDataType());
                        ...
                        dumpOptDetails(comp(), "attach new first %s (%p) and second %s (%p) to node %s (%p) in the "
...
bool TR_CopyPropagation::isLoadNodeSuitableForPropagation(TR::Node* useNode, TR::Node* storeNode, TR::Node* loadNode)
{
    ...
    if (useNode->getType().isBCD() && loadNode->getType().isBCD()) {
        dumpOptDetails(comp(), "isLoadNodeSuitableForPropagation : useNode %p (prec %d), loadNode %p (prec %d) -- "
    ...
    const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
    ...
    while (currentTree != NULL) {
        ...
        if (visitPredecessors) {
            ...
            predList.insert(
                predList.end(), block->getExceptionPredecessors().begin(), block->getExceptionPredecessors().end());
    ...
    TR::Node* useNode, TR::Node* storeNode, const TR_UseDefInfo::BitVector& defs, TR_UseDefInfo* useDefInfo)
{
    ...
    if (useNode->getType().isBCD() && storeNode->getType().isBCD()) {
        dumpOptDetails(comp(), "isCorrectToReplace : useNode %p (prec %d), storeNode %p (prec %d) -- isCorrect=%s "
                               "(only correct when useNodePrec <= storeNodePrec)\n",
...
TR::Node* TR_CopyPropagation::isCheapRematerializationCandidate(TR::Node* defNode, TR::Node* node)
{
    ...
    //      rc=1 vc=9438 vn=- sti=- udi=- nc=0 addr=4
    //      iconst 1212 (X!=0 X>=0 )                                                      [0x2B016B70] bci=[-1,37,329]
    //      rc=1 vc=9438 vn=- sti=- udi=- nc=0 flg=0x104
    //
    if (node->getOpCode().isLoadIndirect()) {
