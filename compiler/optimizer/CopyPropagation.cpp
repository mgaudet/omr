...

    {
                        ...
                        if (rhsOfStoreDefNode == rhsOfPrevStoreDefNode) {
                            ...
                            if (prevDefNode->getType().isBCD()) {
                                ...
                                if (defNode->getType().isBCD() && defNode->getDataType() == prevDefNode->getDataType() && prevDefNode->getDecimalPrecision() == defNode->getDecimalPrecision() && (defNode->getDataType() != TR::PackedDecimal || prevDefNode->mustCleanSignInPDStoreEvaluator() == defNode->mustCleanSignInPDStoreEvaluator())) {
                                    ...
                                    if ((!thisSymRefIsPreferred && (prevSymRefIsPreferred || (equivalentDefs[j - firstRealDefIndex] != j)))) {
                                        if (trace())
        ...
        for (i = useDefInfo->getFirstUseIndex(); i <= lastUseIndex; i++) {
            ...
            if (defNode->getOpCode().isStoreDirect() && defNode->getSymbolReference()->getSymbol()->isAutoOrParm()) {
                ...
                if (copySymbolReference->getSymbol()->isAutoOrParm() && performTransformation(comp(), "%s   Copy1 Propagation replacing Copy symRef #%d %p %p\n", OPT_DETAILS, copySymbolReference->getReferenceNumber(), defNode, equivalentDefNode)) {
