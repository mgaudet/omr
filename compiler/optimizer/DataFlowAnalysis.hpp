...
#include "optimizer/DataFlowAnalysis.enum"
   ...
   virtual TR_ReachingDefinitions           *asReachingDefinitions();
    ...
    bool treeHasChecks(TR::TreeTop* tt, TR::Node* node = NULL)
    {
        ...
        return (tt->getNode()->exceptionsRaised() != 0 || (comp()->isPotentialOSRPoint(tt)));
    }
...
private:
    void initializeGenAndKillSetInfoForNode(
        TR::Node* node, TR_UseDefInfo::BitVector& defsKilled, bool seenException, int32_t blockNum, TR::Node* parent);
...
public:
    ...
    void findUseOfLocal(TR::Node* node, int32_t blockNum, TR_BitVector** genSetInfo, TR_BitVector** killSetInfo,
        TR_BitVector* commonedLoads, bool movingForwardThroughTrees, vcount_t visitCount);
...
public:
    TR_RegisterAnticipatability(
        TR::Compilation* comp, TR::Optimizer* optimizer, TR_Structure*, TR_BitVector**, bool trace = false);
