...
// static bool fixUpTree(TR::Node *node, TR::TreeTop *treeTop, TR::SparseBitVector &seenNodes, bool &highGlobalIndex,
// TR::Compilation *comp, vcount_t oldCompVisitCount)
static bool fixUpTree(
    TR::Node* node, TR::TreeTop* treeTop, TR::SparseBitVector& seenNodes, bool& highGlobalIndex, TR::Optimization* opt)
{
    ...
    if ((node->getReferenceCount() > 1) && !seenNodes.ValueAt(node->getGlobalIndex())
            ...
            if (callWithNoSideEffects) {
                ...
                if (child->getReferenceCount() == 1) {
                    ...
                    safeToReplaceNode = isSafeToReplaceNode(child, iter.currentTree(), &seenConditionalBranch,
                        visitCount, comp(), &_targetTrees, _cannotBeEliminated);
