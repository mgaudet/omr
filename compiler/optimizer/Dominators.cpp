...
TR::Block* TR_Dominators::getDominator(TR::Block* block) { return _dominators.ValueAt(block->getNumber()); }
...
int TR_Dominators::dominates(TR::Block* block, TR::Block* other)
{
    ...
    if (other == block)
        return 1;
    for (TR::Block* d = other;
         d != NULL && _dfNumbers.ValueAt(d->getNumber()) >= _dfNumbers.ValueAt(block->getNumber());
         d = getDominator(d)) {
        if (d == block)
            return 1;
    }
