    ...
    TR_RegionStructure* region, List<TR::Block>* candidateBlocksList)
{
    ...
    for (TR::Block* block = blocks.getFirst(); block; block = blocks.getNext()) {
        ...
        if (!(cfgNode->getExceptionSuccessors().empty()) || blockHasCalls(block, comp())) {
            if (trace())
                traceMsg(comp(), "An exception can be thrown from block_%d. Removing all the blocks, since we cannot "
...
void TR_ExpressionsSimplification::transformNode(TR::Node* srcNode, TR::Block* dstBlock)
{
    ...
    if (lastTree->getNode()->getOpCode().isBranch() || (lastTree->getNode()->getOpCode().isJumpWithMultipleTargets()
