    ...
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock && ((loopInvariantBlock->getBlock()->getPredecessors().size() == 1)
...
void TR_FieldPrivatizer::privatizeElementCandidates()
{
    ...
    for (auto cursor = _epCandidates.begin(); cursor != _epCandidates.end(); ++cursor) {
        ...
        if (tempMap.Locate(candidate.valueNum, index)) {
            ...
            tempSymRef = comp()->getSymRefTab()->createCoDependententTemporary(comp()->getMethodSymbol(),
