    ...
    TR_StructureSubGraphNode* branchNode)
{
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
        ...
        TR::ILOpCodes addOpCode = getIndexType().isAddress() ? (getTestChildType().isInt64() ? TR::aladd : TR::aiadd) : (getTestChildType().isInt64() ? TR::ladd : TR::iadd);
        tempValue = TR::Node::create(addOpCode, 2, originalLimitClone, constNode);
        ...
        storeOpCode = getIndexType().isAddress() ? TR::astore : (getTestChildType().isInt64() ? TR::lstore : TR::istore);
        //         }
        ...
        TR::ILOpCodes opCode;
        if (getIndexType().isAddress()) {
            opCode = getTestChildType().isInt32() ? (isIncreasingLoop() ? TR::ifiucmplt : TR::ifiucmpgt) : (isIncreasingLoop() ? TR::iflucmplt : TR::iflucmpgt);
        } else {
            if (getTestIsUnsigned()) {
                if (getTestChildType().isInt32())
    ...
    if (_spillLoopRequired) {
        ...
        if (1) {
            ...
            if (1) {
                ...
                TR_StructureSubGraphNode* gotoSubNode = new (trHeapMemory()) TR_StructureSubGraphNode(new (trHeapMemory()) TR_BlockStructure(comp(), gotoBlock->getNumber(), gotoBlock));
...
TR_Structure* TR_LoopUnroller::cloneBlockStructure(TR_BlockStructure* blockStructure)
{
    ...
    TR::Block* clonedBlock = GET_CLONE_BLOCK(block);
    TR_BlockStructure* clonedBlockStructure = new (trHeapMemory()) TR_BlockStructure(comp(), clonedBlock->getNumber(), clonedBlock);
