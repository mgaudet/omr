            ...
            } else if (succ == loop->getEntry()) // Type C
            {
                ...
                if (_iteration != 1) // for all iterations except the first iteration
                {
                    if (shouldConnectToNextIteration(toStructureSubGraphNode(subNode), loop)) {
                        ...
                        if (trace())
                            traceMsg(comp(), "Redirecting cold edge from block_%d to unrolled loop header rather than "
    ...
    TR_RegionStructure* loop, TR_StructureSubGraphNode* loopNode, TR_StructureSubGraphNode* branchNode)
{
    ...
    if (pBlock->getEntry() == 0) {
        ...
        TR_BlockStructure* newBlockStr
            = new (trHeapMemory()) TR_BlockStructure(comp(), newBlock->getNumber(), newBlock);
    ...
    if (originalLimitNode->getOpCode().isLoadConst() && !const_overflow) {
        ...
        TR::ILOpCodes addOpCode = getIndexType().isAddress() ? (getTestChildType().isInt64() ? TR::aladd : TR::aiadd)
        ...
        storeOpCode
            = getIndexType().isAddress() ? TR::astore : (getTestChildType().isInt64() ? TR::lstore : TR::istore);
        //         }
        ...
        TR::ILOpCodes opCode;
        if (getIndexType().isAddress()) {
            opCode = getTestChildType().isInt32() ? (isIncreasingLoop() ? TR::ifiucmplt : TR::ifiucmpgt)
    ...
    if (_spillLoopRequired) {
        ...
        if (1) {
            ...
            if (1) {
                ...
                newIfBlockStructure
                    = new (trHeapMemory()) TR_BlockStructure(comp(), newIfBlock->getNumber(), newIfBlock);
...
void TR_GeneralLoopUnroller::profileNonCountedLoops(List<TR_RegionStructure>& innerLoops)
{
    ...
    for (TR_RegionStructure* loop = it.getFirst(); loop; loop = it.getNext()) {
        ...
        entryBlock->prepend(TR::TreeTop::create(
            comp(), TR::Node::create(TR::lstore, 1,
        ...
        postBlock->prepend(
            TR::TreeTop::create(comp(), TR::Node::createWithSymRef(TR::lstore, 1, 1,
                                            TR::Node::createLoad(firstNode, counterSymRef), counterSymRef)));
