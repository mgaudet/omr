...
int32_t TR_GlobalRegisterAllocator::perform()
{
    ...
    candidates->_candidateForSymRefs = (TR_RegisterCandidate**)trMemory()->allocateStackMemory(
    ...
    if (cg()->getSupportsGlRegDeps() && !debug("disableGRA") && cg()->prepareForGRA()) {
        ...
        _registerCandidates
            = (TR_RegisterCandidate**)trMemory()->allocateStackMemory(_origSymRefCount * sizeof(TR_RegisterCandidate*));
...
void TR_GlobalRegisterAllocator::transformBlock(TR::TreeTop* tt)
{
    ...
    } while ((tt = tt->getNextTreeTop()) && (node = tt->getNode(), node->getOpCodeValue() != TR::BBStart
            ...
            if (!value) {
                ...
                if (!value) {
                    if (parent->getOpCode().isStore() && (parent->getSymbolReference() == node->getSymbolReference())) {
                        ...
                        dumpOptDetails(comp(), "%s change load var [%p] of %s symRef#%d to load reg\n", OPT_DETAILS,
                            node, node->getSymbolReference()->getSymbol()->isMethodMetaData()
                                ? node->getSymbolReference()->getSymbol()->castToMethodMetaDataSymbol()->getName()
                                : "",
                            node->getSymbolReference()->getReferenceNumber());
                    }
    ...
    TR::Block* block, TR_GlobalRegister* extgr, TR::Block*& successorBlock, int32_t i)
{
    ...
    if (!predecessors.empty() && !(predecessors.size() == 1)
        ...
        if (liveOnAllPredExits && allPredFreq >= successorBlock->getFrequency()) {
            if (trace())
                traceMsg(comp(), "Extended live range of #%d into successor since candidate is available in register "
                                 "on all predecessor's exits: block=%d succ=%d allPredFreq=%d succFreq=%d\n",
            ...
            if (newBlockCanBeReused) {
                ...
                if ((nextNewBlock->getSuccessors().size() == 1)
                    ...
                    if (numRegistersLiveOnNewSuccessor + (needs2Regs ? 2 : 1)
                        ...
                        if (trace())
                            traceMsg(comp(), "numRegistersLiveOnNewSuccessor %d on nextNewBlock %d > "
    ...
    if (i != -1) {
        if (comp->nodeNeeds2Regs(store))
            ...
            dumpOptDetails(comp, "%s create store [%p] from Register %d for %s #%d\n", OPT_DETAILS, store,
                rc->getGlobalRegisterNumber(), rc->getSymbolReference()->getSymbol()->isMethodMetaData()
                    ? rc->getSymbolReference()->getSymbol()->castToMethodMetaDataSymbol()->getName()
                    : "",
                rc->getSymbolReference()->getReferenceNumber());
    }
        ...
        if ((paramCursor->isReferencedParameter() && isTypeAvailable(symRef)) && !onlySelectedCandidates) {
            ...
            if (new2Offer) {
                ...
                for (node = cfg->getFirstNode(); node; node = node->getNext()) {
                    ...
                    if (rc->_blocks.find(block->getNumber()) || (block == startBlock || block == endBlock)
                ...
                if ((sym->isAuto() && methodSymbol->getAutomaticList().find(sym->castToAutoSymbol())
                    ...
                    if (new2Offer) {
                        for (i = 0; i < numInterested; i++) {
                            ...
                            if ((rc
                                    && rc->_blocks.find(
                                           blockNumber))) // handle reg(*) only in block with abnormal blocks
                                continue;
                            if (!referencedAutoSymRefsInBlock[blockNumber]->get(symRefNumber))
                                weight = 0;
                            // If there is a reference in some block then actually create a candidate
...
void TR_GlobalRegisterAllocator::findLoopAutoRegisterCandidates()
{
    ...
    TR_RegisterCandidate** registerCandidates = (TR_RegisterCandidate**)trMemory()->allocateStackMemory(
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(
    TR_StructureSubGraphNode* structureNode, vcount_t visitCount, TR_RegisterCandidate** registerCandidates)
{
...
void TR_GlobalRegisterAllocator::markAutosUsedIn(TR::Node* node, TR::Node* parent, TR::Node* grandParent,
    ...
    int32_t executionFrequency, TR_RegisterCandidate** registerCandidates, TR_BitVector* assignedAutosInCurrentLoop,
    TR_BitVector* symsThatShouldNotBeAssignedInCurrentLoop, bool hasCatchBlock)
{
    ...
    if (enableNewSignExtGRA) {
        ...
        if (!canSkip && ((node->getOpCodeValue() == TR::iadd) || (node->getOpCodeValue() == TR::isub)
...
void TR_LiveRangeSplitter::splitLiveRanges()
{
    ...
    if (!cg()->getLiveLocals()) {
        ...
        if (numLocals > 0 && (!trace() || performTransformation(comp(),
...
void TR_LiveRangeSplitter::replaceAutosUsedIn(TR::TreeTop* currentTree, TR::Node* node, TR::Node* parent,
    ...
    TR_BitVector* autosThatCannotBeReplacedInCurrentLoop, TR_StructureSubGraphNode* loop, TR::Block* loopInvariantBlock)
{
...
void TR_LiveRangeSplitter::fixExitsAfterSplit(TR::SymbolReference* symRef,
    ...
    TR_RegisterCandidate** registerCandidates, TR_StructureSubGraphNode* loop, TR_BitVector* replacedAutosInCurrentLoop,
            ...
            if (!(exitBlock->getPredecessors().size() == 1)) {
                ...
                for (nextEdge = ei.getFirst(); nextEdge; nextEdge = ei.getNext()) {
                    ...
                    if (pred->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && !_splitBlocks.isEmpty()) {
                        ...
                        for (nextSplitBlock = splitBlocksIt.getFirst(); nextSplitBlock != NULL;
                            ...
                            if (exitBlockIsASplitBlock
                                ...
                                dumpOptDetails(comp(), " place store-back of auto #%d using auto #%d in (existing) "
                    ...
                    if (!splitBlock) {
                        ...
                        dumpOptDetails(comp(), " place store-back of auto #%d using auto #%d in (new) loop exit "
