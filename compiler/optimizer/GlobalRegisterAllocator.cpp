    ...
    TR::Node* node, TR::Node* parent, int32_t childIndex, TR::TreeTop* tt, TR::Block*& block, TR_Array<TR_GlobalRegister>& registers, TR_NodeMappings* extBlockNodeMapping)
{
    ...
    if (node->getOpCode().isJumpWithMultipleTargets() && !node->getOpCode().isSwitch() && node->getOpCodeValue() != TR::tstart)
        ...
        if (changeNode) {
            ...
            if (!value) {
                value = gr->getValue();
                if (comp()->cg()->needToAvoidCommoningInGRA() && value && (!value->getOpCode().isLoadReg() && !value->getOpCode().isLoadVar()) && (symbol->getDataType() == TR::Float
        ...
        if ((paramCursor->isReferencedParameter() && isTypeAvailable(symRef)) && !onlySelectedCandidates) {
            ...
            if (new2Offer) {
                ...
                for (node = cfg->getFirstNode(); node; node = node->getNext()) {
                    ...
                    if (rc->_blocks.find(block->getNumber()) || (block == startBlock || block == endBlock) || !block->getExceptionPredecessors().empty())
                        continue;
                ...
                if ((sym->isAuto() && methodSymbol->getAutomaticList().find(sym->castToAutoSymbol()) && !onlySelectedCandidates)) {
                    ...
                    if (new2Offer) {
                        for (i = 0; i < numInterested; i++) {
                            int32_t weight = 1;
                            int32_t blockNumber = interestedCFGNodes[i];
                            if ((rc && rc->_blocks.find(blockNumber))) // handle reg(*) only in block with abnormal blocks
                                continue;
                            if (!referencedAutoSymRefsInBlock[blockNumber]->get(symRefNumber))
                                weight = 0;
                            // If there is a reference in some block then actually create a candidate
...
void TR_GlobalRegisterAllocator::findLoopsAndCorrespondingAutos(TR_StructureSubGraphNode* structureNode, vcount_t visitCount, TR_RegisterCandidate** registerCandidates)
{
    ...
    if (structure->asRegion()) {
        ...
        if (!regionStructure->isAcyclic() && structureNode) {
            ...
            bool excludeInvariantsEnabled = comp()->cg()->excludeInvariantsFromGRAEnabled() && (!comp()->getOptions()->getOption(TR_DisableRXusage));
