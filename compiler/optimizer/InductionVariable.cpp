...
static int32_t countInternalPointerOrPinningArrayStores(TR::Compilation* comp, TR::Block* block)
{
    ...
    for (TR::TreeTop* tt = block->getEntry(); tt != block->getExit(); tt = tt->getNextTreeTop()) {
        ...
        if (node->getOpCode().isStoreDirect()
            && ((node->getSymbol()->isAuto() && (node->getSymbol()->castToAutoSymbol()->isInternalPointer()
    ...
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock) {
        ...
        _storeTreesList
            = (List<TR_StoreTreeInfo>**)trMemory()->allocateStackMemory(symRefCount * sizeof(List<TR_StoreTreeInfo>*));
        ...
        _reassociatedAutos
            = (TR::SymbolReference**)trMemory()->allocateStackMemory(symRefCount * sizeof(TR::SymbolReference*));
            ...
            if (symRefPair) {
                ...
                while (symRefPair) {
                    ...
                    dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted "
                                           "invariant initialization tree : %p for new symRef #%d\n",
                    ...
                    *newSymbolReference = NULL;
                    ...
                    if (canCreateNewSymRef) {
                        ...
                        *newSymbolReference = comp()->getSymRefTab()->createTemporary(
                        ...
                        if (isInternalPointer && !pinningArrayPointer && comp()->isPinningNeeded()) {
    ...
    TR::Node* constNode, bool isAddition)
{
    ...
    dumpOptDetails(comp(), "\nO^O INDUCTION VARIABLE ANALYSIS: Induction variable analysis inserted loop incremental "
                           "step tree : %p for new symRef #%d\n",
        newStoreTreeTop->getNode(), newSymbolReference->getReferenceNumber());
    return newStore;
}
    ...
    TR_Structure* loopStructure, TR::SymbolReference* newSymbolReference, TR::NodeChecklist& exLoads)
{
    TR::DebugCounter::incStaticDebugCounter(
        comp(), TR::DebugCounter::debugCounterName(comp(), "loopStrider.widen/(%s)/%s/loop=%d/iv=%d",
    ...
    replaceLoadsInStructure(loopStructure, _loopDrivingInductionVar, newSymbolReference, exLoads, visited);
 ...
 * \see detectLoopsForIndVarConversion(TR_Structure*, TR::NodeChecklist&)
 */
void TR_LoopStrider::replaceLoadsInStructure(TR_Structure* structure, int32_t iv, TR::SymbolReference* newSR,
...
void TR_LoopStrider::replaceLoadsInSubtree(
    TR::Node* node, int32_t iv, TR::SymbolReference* newSR, TR::NodeChecklist& exLoads, TR::NodeChecklist& visited)
{
    if (visited.contains(node))
        return;
...
void TR_LoopStrider::eliminateSignExtensionsInSubtree(
    TR::Node* parent, TR::NodeChecklist& exLoads, TR::NodeChecklist& visited, NodeMap& signExtMemo)
{
 ...
 * \see transmuteDescendantsIntoTruncations(TR::Node*, TR::Node*)
 */
TR::Node* TR_LoopStrider::signExtend(TR::Node* node, TR::NodeChecklist& exLoads, NodeMap& memo)
{
    ...
    if (ext != NULL) {
        ...
        if (trace())
            traceMsg(comp(), "[Sign-Extn] sign-extended n%un %s into n%un %s\n", node->getGlobalIndex(),
                node->getOpCode().getName(), ext->getGlobalIndex(), ext->getOpCode().getName());
    }
...
TR::Node* TR_LoopStrider::signExtendBinOp(TR::ILOpCodes op64, TR::Node* node, TR::NodeChecklist& exLoads, NodeMap& memo)
{
    ...
    TR_RegionStructure* loop, TR::Block* loopTestBlock, TR::Node* candidateIV)
{
    ...
    TR::list<TR::CFGEdge*> workList(getTypedAllocator<TR::CFGEdge*>(comp()->allocator()));
    ...
    if (valueNode->getOpCode().isLoadVarDirect() && valueNode->getReferenceCount() > 1) {
        ...
        for (TR::TreeTop* tt = controllingBranch->getPrevTreeTop();
            ...
            if (curNode->getOpCode().isStoreDirect()
                ...
                if (curNode->containsNode(valueNode, comp()->getVisitCount())) // valueNode is used in the store subtree
                    ...
                    if (trace())
                        traceMsg(comp(), "\tRejected - tested value commoned across a store %p\n", tt->getNode());
                    return false;
                }
            }
        ...
        if (usesUnchangedValueInLoopTest) {
            if (!isIVUnchangedInLoop(loop, controllingBranch->getEnclosingBlock(), valueNode)) {
                if (trace())
                    traceMsg(comp(), "\tReject - IV node has been changed in one of the blocks that is not part of the "
...
void TR_IVTypeTransformer::changeIVTypeFromAddrToInt(TR_RegionStructure* natLoop)
{
    ...
    } while (!astoreNode && (tt->getNode()->getOpCodeValue() != TR::BBStart
    ...
    auto intIdxStoreTT
        = TR::TreeTop::create(cm, TR::Node::createStore(_intIdxSymRef,
    ...
    auto intIncrementTT = TR::TreeTop::create(
        cm, TR::Node::createStore(_intIdxSymRef, TR::Node::create(TR::Compiler->target.is64Bit() ? TR::ladd : TR::iadd,
    ...
    if (astoreNode->getSymbolReference() == firstChildSymRef) // Match if children to original backedgeIf children
    {
        newIf
            = TR::Node::createif(getIntegralIfOpCode(backEdgeIfNode->getOpCodeValue(), TR::Compiler->target.is64Bit()),
                intNodeForBackEdgeTest, TR::Node::create(TR::Compiler->target.is64Bit() ? TR::lsub : TR::isub, 2,
