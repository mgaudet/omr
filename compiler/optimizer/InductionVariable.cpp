...
TR::Node* TR_LoopStrider::updateLoadUsedInLoopIncrement(TR::Node* node, int32_t inductionVariable)
{
    ...
    if (_indirectInductionVariable && opCode.isLoadVar() && (node->getSymbolReference()->getReferenceNumber() < _numSymRefs) && ((!_storeTreesList && _writtenExactlyOnce.ValueAt(node->getSymbolReference()->getReferenceNumber())) || (_storeTreesList && _storeTreesList[node->getSymbolReference()->getReferenceNumber()]->isSingleton()))) {
    ...
    TR_Array<TR_BasicInductionVariable*>& basicIVs)
{
    ...
    if (valueNode->getOpCode().isLoadVarDirect() && valueNode->getReferenceCount() > 1) {
        ...
        for (TR::TreeTop* tt = controllingBranch->getPrevTreeTop();
             tt && (tt->getNode()->getOpCodeValue() != TR::BBStart || tt->getNode()->getBlock()->isExtensionOfPreviousBlock());
             tt = tt->getPrevTreeTop()) {
            TR::Node* curNode = tt->getNode();
            if (curNode->getOpCode().isStoreDirect() && curNode->getSymbolReference() == valueNode->getSymbolReference()) {
                //storeSeen = true;
                if (trace())
