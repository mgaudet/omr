...
bool OMR_InlinerUtil::validateInterfaceImplementation(TR_ResolvedMethod* interfaceMethod)
{
    return true;
}
    ...
    TR_CallTarget* calltarget)
{
    ...
    if (guard->_kind != TR_InnerGuard)
        virtualGuard = block1->append(TR::TreeTop::create(comp(), createVirtualGuard(callNode, calleeSymbol, block4->getEntry(),
            ...
            if (!parmMap->_parmIsModified && (!arg->getOpCode().isFloatingPoint() || comp()->cg()->getSupportsJavaFloatSemantics())) {
                if (parmMap->_addressTaken) {
                    ...
                    if (disableParmTempOpt) {
                        ...
                        if ((arg->getOpCode().isLoadVarDirect() && arg->getSymbol()->isAutoOrParm()) && ((arg->getReferenceCount() == 1)
                                                                                                            || (_callNode->getOpCode().isCallIndirect() && (argIndex == _callNode->getFirstArgumentIndex()) && (_callNode->getFirstChild()->getNumChildren() > 0) && (arg == _callNode->getFirstChild()->getFirstChild()) && arg->getSymbol()->isAuto() && (arg->getReferenceCount() == 2)))) {
...
void TR_TransformInlinedFunction::transform()
{
    ...
    //
    if ((firstBlock->getPredecessors().size() > 1) || firstBlock->hasExceptionSuccessors() || comp()->fe()->isMethodEnterTracingEnabled(calleeResolvedMethod->getPersistentIdentifier()) || TR::Compiler->vm.canMethodEnterEventBeHooked(comp())) {
    ...
    bool isIndirect, int32_t offset)
{
    ...
    if (value->isInternalPointer() && value->getPinningArrayPointer()) {
        ...
        if (
            (value->getOpCode().isArrayRef() || (value->getOpCode().isLoadVarDirect() && value->getSymbolReference()->getSymbol()->isAuto() && value->getSymbolReference()->getSymbol()->castToAutoSymbol()->isInternalPointer())))
...
{
    return _ecsPrexArgInfo && _ecsPrexArgInfo->get(0) && _ecsPrexArgInfo->get(0)->classIsPreexistent() && _ecsPrexArgInfo->get(0)->getFixedClass();
}

TR_OpaqueClassBlock* TR_IndirectCallSite::getClassFromArgInfo()
{
    TR_ASSERT(_ecsPrexArgInfo && _ecsPrexArgInfo->get(0) && _ecsPrexArgInfo->get(0)->getFixedClass(), "getClassFromArgInfo is NOT guarded by hasFixedTypeArgInfo,hasResolvedTypeArgInfo");
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack* callStack, TR_CallSite* callsite)
{
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if (!forceInline(calltarget) && exceedsSizeThreshold(callsite, bytecodeSize,
                                            (callsite->_callerBlock != NULL) ? callsite->_callerBlock : (callsite->_callNodeTreeTop ? callsite->_callNodeTreeTop->getEnclosingBlock() : 0),
...
void TR_InlinerBase::inlineFromGraph(TR_CallStack* prevCallStack, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo* innerPrexInfo)
{
    ...
    for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount && tt->getNode()->getChild(0)->getInlinedSiteIndex() == thisSiteIndex &&
 ...
 */
void OMR_InlinerUtil::calleeTreeTopPreMergeActions(TR::ResolvedMethodSymbol* calleeResolvedMethodSymbol, TR_CallSite* callSite)
{
}
