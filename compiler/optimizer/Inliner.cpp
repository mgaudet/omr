...
void TR_InlinerBase::setInlineThresholds(TR::ResolvedMethodSymbol* callerSymbol)
{
    ...
    heuristicTrace(tracer(), "Inlining Limits:\n\tCaller Side Weight Limit (_callerWeightLimit) = %d \n\tCall Graph "
                             "Size Threshold (_maxRecursiveCallByteCodeSizeEstimate) = %d "
                             "\n\tMethod size threshold (_methodByteCodeSizeThreshold) = %d \n\tMethod size threshold "
                             "for warm (and above) compiles (_methodInWarmBlockByteCodeSizeThreshold) = %d"
                             "\n\tsize threshold for cold Calls (_methodInColdBlockByteCodeSizeThreshold) = %d\n\tNode "
                             "Count Threshold (_nodeCountThreshold) = %d "
...
// 1) If b corresponds to the top caller (root of the call tree), it doesn't do anything because we have already
// implanted, during
// ilgen, a TR::igoto at the end of b which transfers control back to the VM
// 2) Otherwise, replace the last treetop of b (which must be a goto or a return) to a goto to the OSR code block of the
// caller, and
...
bool OMR_InlinerUtil::validateInterfaceImplementation(TR_ResolvedMethod* interfaceMethod) { return true; }
 ...
 */
static TR_YesNoMaybe gatherNodesToCheck(TR::Compilation* comp, TR_InlinerTracer* tracer, TR::Node* privArg,
    TR::Node* currentNode, TR::SparseBitVector& scanTargets, TR::SparseBitVector& symRefsToCheck,
    TR::SparseBitVector& visitedNodes)
{
    ...
    if (opCode.hasSymbolReference() && !opCode.isLoad()) {
        debugTrace(tracer, "  priv arg remat: Can't fully remat [%p] due to [%p] - non-load with a symref", privArg,
            currentNode);
        return TR_no;
        ...
        if (comp->requiresSpineChecks() && currentNode->getSymbol()->isArrayShadowSymbol()) {
            debugTrace(tracer, "  priv arg remat: Can't fully remat [%p] due to [%p] - array access needs spine check",
                privArg, currentNode);
        ...
        if (gatherNodesToCheck(
                comp, tracer, privArg, currentNode->getFirstChild(), scanTargets, symRefsToCheck, visitedNodes)
            != TR_no) {
            scanTargets[currentNode->getGlobalIndex()] = true;
            symRefsToCheck[currentNode->getSymbolReference()->getReferenceNumber()] = true;
            return TR_yes;
        ...
        for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
            ...
            TR_YesNoMaybe childResult = gatherNodesToCheck(
                comp, tracer, privArg, currentNode->getChild(i), childScanTargets, childSymRefsToCheck, visitedNodes);
            if (childResult == TR_no) {
                debugTrace(tracer, "  priv arg remat: Can't fully remat [%p] due to [%p] - unsafe arithmetic child %d",
        ...
        heuristicTrace(
            tracer, "  guarded call remat: Can't fully remat [%p] due to [%p] - unhandled case", privArg, currentNode);
        return TR_no;
    }
}
    ...
    TR::Node* currentNode, TR::SparseBitVector& scanTargets, TR::SparseBitVector& symRefsToCheck)
{
    ...
    return gatherNodesToCheck(comp, tracer, privArg, currentNode, scanTargets, symRefsToCheck, visitedNodes);
}

static void walkNodesCalculatingRematSafety(TR::Compilation* comp, TR_InlinerTracer* tracer, TR::Node* currentNode,
    ...
    TR::SparseBitVector& visitedNodes)
{
    for (uint16_t i = 0; i < currentNode->getNumChildren(); ++i) {
        ...
        walkNodesCalculatingRematSafety(
            comp, tracer, currentNode->getChild(i), scanTargets, enabledSymRefs, unsafeSymRefs, visitedNodes);
    }
 ...
 */
static void walkTreesCalculatingRematSafety(TR::Compilation* comp, TR_InlinerTracer* tracer, TR::TreeTop* start,
    TR::TreeTop* end, TR::SparseBitVector& scanTargets, TR::SparseBitVector& unsafeSymRefs)
{
    ...
    while (start != end) {
        if (start->getNode()) {
            walkNodesCalculatingRematSafety(
                comp, tracer, start->getNode(), scanTargets, enabledSymRefs, unsafeSymRefs, visitedNodes);
        }
...
public:
    ...
    void dumpInfo(TR::Compilation* comp, TR_InlinerTracer* tracer)
    {
        for (uint32_t i = 0; i < dependentSymRefs.size(); ++i) {
            debugTrace(tracer, "  Arg Remat Safety Info for priv arg store node %d",
                argumentTreeTops[i]->getNode()->getGlobalIndex());
            if (rematTreeTops[i]) {
                if (rematTreeTops[i] != argumentTreeTops[i])
                    debugTrace(
                        tracer, "     partial remat candidate node %d", rematTreeTops[i]->getNode()->getGlobalIndex());
                else
 ...
 */
static void walkTreeTopsCalculatingRematFailureAlternatives(TR::Compilation* comp, TR_InlinerTracer* tracer,
    TR::TreeTop* start, TR::TreeTop* end, TR::list<TR::TreeTop*>& failedArgs, TR::SparseBitVector& scanTargets,
    RematSafetyInformation& rematInfo)
{
    ...
    while (start != end) {
        debugTrace(tracer, "  priv arg remat: visiting [%p]: isStore %d privArg %d failedTargetMatch %d",
        ...
        if (start->getNode() && start->getNode()->getOpCode().isStoreDirect() && start->getNode()->getSymbol()->isAuto()
            ...
            for (auto iter = failedArgs.begin(); iter != failedArgs.end(); ++iter) {
                if ((*iter) && (*iter)->getNode()->getOpCode().hasSymbolReference()
                    ...
                    debugTrace(tracer, "  priv arg remat: attempting partial remat with node [%p] for [%p]",
...
static void rematerializeCallArguments(TR::Compilation* comp, TR_InlinerTracer* tracer,
    TR_TransformInlinedFunction& tif, TR_VirtualGuardSelection* guard, TR::Node* callNode, TR::Block* block1,
    ...
    TR_VirtualGuardSelection* guard, TR::TreeTop** induceOSRCallTree, TR_CallTarget* calltarget)
{
    ...
    if (guard->_kind != TR_InnerGuard)
        virtualGuard = block1->append(
            TR::TreeTop::create(comp(), createVirtualGuard(callNode, calleeSymbol, block4->getEntry(),
    ...
    // recognition to work. Can we do something better?
    ...
    if (!disableHCRGuards && comp()->getHCRMode() == TR::traditional && guard->_kind != TR_HCRGuard
    ...
    rematerializeCallArguments(
        comp(), tracer(), tif, guard, callNode, block1, block4->getFirstRealTreeTop()->getPrevTreeTop());
    ...
    if (callerSymbol->supportsInduceOSR(callNode->getByteCodeInfo(), callNode, block1, calleeSymbol, comp())) {
        ...
        if (shouldUseOSR && (osrAll || comp()->getOption(TR_EnableOSROnGuardFailure)
                                || (createdHCRAndVirtualGuard && shouldAttemptOSR && osrForHCRGuardsOfIndirect)
                                || createdHCRGuard || (osrForNonHCRGuards && shouldAttemptOSR))) {
            TR::TreeTop* induceTree = callerSymbol->genInduceOSRCallForGuardedCallee(guardedCallNodeTreeTop,
                calleeSymbol, (callNode->getNumChildren() - callNode->getFirstArgumentIndex()), false, false);
    ...
    TR::ResolvedMethodSymbol* callerSymbol, bool isIndirectCall)
{
    ...
    while (osrMethodData) {
        ...
        if (comp()->getOption(TR_TraceOSR))
            traceMsg(comp(), "OSR caller at inlined site index %d has %d bytecodes and %d stack slots, total callers "
                             "bytecodes %d total callers stack slots %d total callers live stack slots %d total "
                             "pending push slots %d\n",
...
void TR_ParameterToArgumentMapper::printMapping()
{
    ...
    for (TR_ParameterMapping* pm = _mappings.getFirst(); pm; pm = pm->getNext()) {
        debugTrace(tracer(), "Mapping at addr %p:\n\tparmSymbol = %p (offset %d) \treplacementSymRef = "
                             "%d\t_parameterNode = %p\treplacementSymRef2 = %d\treplacementSymRef3 = %d\n"
            ...
            if (!parmMap->_parmIsModified
                ...
                if (parmMap->_addressTaken) {
                    ...
                    debugTrace(tracer(), "Setting parameterNode to n%in %s, argOffset=%d, argIndex=%d, _callNode n%in",
                        arg->getNodePoolIndex(), arg->getOpCode().getName(), argOffset, argIndex,
                        _callNode->getNodePoolIndex());
...
void TR_TransformInlinedFunction::transform()
{
    ...
    //
    if ((firstBlock->getPredecessors().size() > 1) || firstBlock->hasExceptionSuccessors()
...
void TR_HandleInjectedBasicBlock::createTemps(bool replaceAllReferences)
{
    ...
    for (MultiplyReferencedNode* ref = _multiplyReferencedNodes.getFirst(); ref; ref = ref->getNext()) {
        ...
        if (rematConstant || opcode.getOpCodeValue() == TR::loadaddr) {
            ...
            && comp()->cg()->getLinkage()->isAddressOfStaticSymRef(ref->_node->getSymbolReference())) {
            ref->_isConst = true;
        } else if (opcode.getOpCodeValue() == TR::aload
            && comp()->cg()->getLinkage()->isAddressOfPrivateStaticSymRef(ref->_node->getSymbolReference())) {
            ref->_isConst = true;
        } else {
            ...
            // automatically add it to a list of injected basic block temps
            // When I enabled the ParmTempOpt, this is what happened.  An auto symbol got added to the
...
/*
 * handles the temp sharing between caller and inlined callee by storing a value from the inlined method to a temp of
 * the caller
 */
...
{
    TR_ASSERT(_ecsPrexArgInfo && _ecsPrexArgInfo->get(0) && _ecsPrexArgInfo->get(0)->getFixedClass(),
...
bool TR_DirectCallSite::findCallSiteTarget(TR_CallStack* callStack, TR_InlinerBase* inliner)
{
    ...
    if (!disableHCRGuards2 && comp()->getHCRMode() == TR::traditional && !comp()->compileRelocatableCode()
...
void TR_InlinerBase::applyPolicyToTargets(TR_CallStack* callStack, TR_CallSite* callsite)
{
    for (int32_t i = 0; i < callsite->numTargets(); i++) {
        ...
        if (!forceInline(calltarget)
            && exceedsSizeThreshold(callsite, bytecodeSize, (callsite->_callerBlock != NULL)
        ...
        int32_t selfInliningLimit
            = selfInliningLimitStr ? atoi(selfInliningLimitStr) : comp()->getMethodSymbol()->doJSR292PerfTweaks()
    ...
    TR_CallStack* prevCallStack, TR_CallTarget* calltarget, TR_InnerPreexistenceInfo* innerPrexInfo)
{
    ...
    for (TR::TreeTop* tt = callerSymbol->getFirstTreeTop(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (isCall && tt->getNode()->getChild(0)->getVisitCount() != _visitCount
            ...
            if (!isCold) {
                debugTrace(tracer(), "inlineFromGraph:: about to call findAndUpdateCallSiteInGraph on call target %p "
                                     "for call at node %p",
                    calltarget, node);
                TR_CallSite* site
                ...
                if (calleeSymbol->getMethod())
                    heuristicTrace(tracer(), "Block containing call node %p (callee name: %s) but either caller block "
                                             "or function is cold. Skipping call.",
    ...
    TR_CallStack* callStack, int32_t& numLivePendingPushSlots, int32_t& nestingDepth)
{
    if (optimizer->comp()->getOption(TR_EnableOSR)) {
        ...
        TR_OSRMethodData* osrMethodData = optimizer->comp()->getOSRCompilationData()->findOrCreateOSRMethodData(
            optimizer->comp()->getCurrentInlinedSiteIndex(), callStack->_methodSymbol);
    ...
    TR_CallStack* callStack, TR_CallTarget* calltarget, TR::TreeTop** cursorTreeTop, bool inlinefromgraph, int32_t)
{
    ...
    debugTrace(tracer(), "Starting Physical Inlining for call target %p at callsite %p with visit count %d and "
                         "numtargets = %d \n Available temps: ",
    ...
    */
    ...
    if (induceOSRCallTree) {
        ...
        induceOSRCallNode->setNumChildren(numOfChildren - firstArgChild); // tricky "swelling" of the node but we
                                                                          // allocated it of the right size when we
    ...
    if (calltarget->_myCallSite->numTargets() > 1
        ...
        if (lastNodeInCallBlock->getOpCode().isReturn() || lastNodeInCallBlock->getOpCode().isGoto()) {
            ...
            nextBBEndInCaller->getNode()->getBlock()->getPrevBlock()->getExit()->insertBefore(TR::TreeTop::create(
                comp(), TR::Node::create(virtualGuard->getNode(), TR::Goto, 0,
    ...
    TR::DebugCounter::prependDebugCounter(
        comp(), "inliner.callSites/succeeded:#bytecodeSize", callNodeTreeTop, bytecodeSize);
    TR::DebugCounter::prependDebugCounter(
        comp(), "inliner.callSites/succeeded:#localsInCallee", callNodeTreeTop, numberOfLocalsInCallee);
...
void TR_CallSite::removecalltarget(int32_t index, TR_InlinerTracer* tracer, TR_InlinerFailureReason reason)
{
    ...
    if (_comp->cg()->traceBCDCodeGen() && _callNode != NULL) {
        ...
        traceMsg(_comp, "q^q : failing to inline %s into %s (callNode %p on line_no=%d)\n",
            signature(_comp->trMemory()), _comp->fe()->sampleSignature(_callerResolvedMethod->getPersistentIdentifier(),
                                              callerName, 1024, _comp->trMemory()),
            _callNode, _comp->getLineNumber(_callNode));
    }
    ...
    TR_ResolvedMethod* implementer, TR_OpaqueClassBlock* receiverClass, TR_AllocationKind allocKind, float ratio)
{
    ...
    if (inliner->tracer()->heuristicLevel()) {
        ...
        heuristicTrace(inliner->tracer(), "Creating a call target %p for callsite %p using a %s and %s .  Signature %s",
            _mytargets[_numtargets - 1], this, inliner->tracer()->getGuardKindString(guard),
...

    {
        ...
        if (arg && arg->getFixedClass()) {
            ...
            traceMsg(comp(),
                ...
                arg->classIsFixed(), arg->classIsPreexistent(), arg->getFixedClass(), className);
        } else {
...
void OMR_InlinerUtil::calleeTreeTopPreMergeActions(
    TR::ResolvedMethodSymbol* calleeResolvedMethodSymbol, TR_CallSite* callSite)
{
}
