...
int32_t TR_IsolatedStoreElimination::performWithUseDefInfo()
{
    ...
    // "ShouldNotBeRemoved") should be set.
    // The 4th bitvector is inTransit vector.
    // So "NotToBeExamine should set the bits in "Visited" and also on "ShouldNotBeRemoved".
    // After finding the group set the right bits in "ToBeRemoved" or "ShouldNotBeRemoved and update Visited vector.
    // Also, emtpy InTransit vector
    //*******************************************************************************************
...
void TR_IsolatedStoreElimination::performDeadStructureRemoval(TR_UseDefInfo* info)
{
    ...
    TR_BitVector* nodesInStructure
        = new (trStackMemory()) TR_BitVector(info->getTotalNodes(), trMemory(), stackAlloc, growable);
...
bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(TR_UseDefInfo* info, TR_Structure* structure,
    vcount_t visitCount, TR_BitVector* nodesInStructure, bool* propagateRemovalToParent)
{
    ...
    if (canRemoveStructure) {
        ...
        TR_BitVector* defNodesInStructure
            = new (trStackMemory()) TR_BitVector(info->getNumDefNodes(), trMemory(), stackAlloc, growable);
        int32_t i;
...
void TR_IsolatedStoreElimination::analyzeSingleBlockLoop(TR_RegionStructure* regionStructure, TR::Block* block)
{
    ...
    if (((lastTreeInLoopHeader->getOpCodeValue() == TR::ificmple)
        ...
        && inductionSymRef && (secondChildOfLastTree->getOpCode().isLoadConst()
