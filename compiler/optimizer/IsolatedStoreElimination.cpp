...
bool TR_IsolatedStoreElimination::findStructuresAndNodesUsedIn(TR_UseDefInfo* info, TR_Structure* structure, vcount_t visitCount, TR_BitVector* nodesInStructure, bool* propagateRemovalToParent)
{
    ...
    if (structure->asRegion()) {
        ...
        if (regionStructure->isNaturalLoop() && (regionStructure->getParent() && regionStructure->getParent()->asRegion()->isCanonicalizedLoop())) {
...
bool TR_IsolatedStoreElimination::markNodesAndLocateSideEffectIn(TR::Node* node, vcount_t visitCount, TR_BitVector* nodesSeen)
{
    if (node->getVisitCount() == visitCount)
        return false;
