    ...
    , _useDefInfo(NULL)
{
    ...
    _signExtensionFlags = new (trHeapMemory())
        TR_BitVector(nodeCount * TR::Node::numSignExtensionFlags, trMemory(), heapAlloc, growable);
...
int32_t TR_LoadExtensions::perform()
{
    ...
    if (
        ...
        0 &&
#endif
        (comp()->getOptLevel() > noOpt) && _seenLoads & 0x2 && optimizer() && !optimizer()->cantBuildGlobalsUseDefInfo()
#ifdef REUSE_USEDEFS
        && (NULL == (_useDefInfo = optimizer()->getUseDefInfo()) || !_useDefInfo->hasGlobalsUseDefs()
    ...
    if (opcode.isConversion() && supportedType(parent)) {
        ...
        if ((load->getOpCode().isLoadVar() || load->getOpCode().isLoadReg() || load->getOpCode().isLoadConst())
            ...
            if (load->getOpCode().isLoadReg()
                || (load->getOpCode().isLoadVar() && load->getSymbol()->isRegisterSymbol())) {
                if (optimizer() && _useDefInfo && _useDefInfo->infoIsValid() && load->getUseDefIndex()
                    ...
                    && (parent->getSize() < 8)) // On Java disable i2l, because of interferance with HRA
                {
                    ...
                    if (!info.IsZero()) {
                        if (trace())
                            traceMsg(comp(), "\t\tPeeking through RegLoad %p for Conversion %p (%s)\n", load, parent,
                        ...
                        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                            ...
                            if ((defNode->getOpCode().isStoreReg()
                                ...
                                && ((realLoad->getOpCode().isLoadVar() && !realLoad->getSymbol()->isRegisterSymbol())
                                       || realLoad->getOpCode().isLoadConst() && supportedConstLoad(realLoad, comp()))
                                && supportedType(realLoad)) {
                                if (trace())
                                    traceMsg(comp(),
                                        "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), Counting.\n",
                                        load, load->getOpCode().getName(), defNode, defNode->getOpCode().getName(),
                                        realLoad, realLoad->getOpCode().getName());
                                countLoad(realLoad, parent);
                            } else {
                                if (trace())
                                    traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), "
                                                     "setting Ignore.\n",
                                        load, load->getOpCode().getName(), defNode, defNode->getOpCode().getName(),
    ...
    for (int32_t i = 0; i < parent->getNumChildren(); i++) {
        ...
        if (child->getOpCode().isConversion()) {
            if (opcode.isStoreReg()
                || (opcode.isStore() && parent->getSymbol() && parent->getSymbol()->isRegisterSymbol())) {
                ...
                setOverrideOpt(child, narrowOverride);
            // We counted all the loads feeding into conversions (before the loop), now count the other references to
            // loads
            else if (!opcode.isConversion() && child->getOpCode().isLoadVar() && supportedType(child))
                countLoad(child, parent);
        }

        // See how a load is being used. If a load feeds only to narrowing conversions, we may set couldIgnoreExtend()
        // flag on the load
        // WARNING: "Clever idea" -- set the LSB, to mean that does _not_ feed into a narrowing conversion, use other
        // bits as int
        if (child->getOpCode().isLoadVar()
            && (!opcode.isConversion() || (opcode.isConversion() && parent->getSize() < child->getSize()))) {
...
bool TR_LoadExtensions::detectReverseNeededConversionPattern(TR::Node* parent, TR::Node* conversion)
{
    ...
    // Many programmers zero out top bits with AND. Calculate the AND mask that will result in conversion being
    // unneeeded
    // i.e.
        ...
        && (conversion->getSize() - conversion->getFirstChild()->getSize()) > 0) // Must be widening conversion
    {
        ...
        if ((NULL != (constant = parent->getFirstChild()) && constant->getOpCode().isLoadConst()
                || NULL != (constant = parent->getSecondChild()) && constant->getOpCode().isLoadConst()
            ...
            return false;
                                            ...
                                            && !(parent->getSymbol() && parent->getSymbol()->isRegisterSymbol())))
        return false;
...
bool TR_LoadExtensions::detectUnneededConversionPattern(TR::Node* conversion, TR::Node* child, bool& mustForce64)
{
    ...
    if (supportedType(child)) {
        ...
        if (((childOpcode.isLoadVar() && !(child->getSymbol()->isRegisterSymbol()))
            ...
            && (opcode.getOpCodeValue() != TR::a2l
                   || child->getReferenceCount() == 1
                       && TR::Compiler->target.is32Bit())) // 4 byte a2l really means 31->64! careful!
        {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion and load nodes %p\n", conversion);
        ...
        // l2i can be also ignored: will either grab the reg->getLowOrder() or keep on passing GPR64
            ...
            && (!isOverriden(conversion, narrowOverride)
                   ...
                   childOpcode.isConversion() && NULL != (grandChild = child->getFirstChild())
                       && supportedType(grandChild) && grandChild->getSize() == conversion->getSize())) {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension pattern on narrowing conversion node %p\n", conversion);
            return true;
        } else if (childOpcode.isRightShift() && child->getSecondChild()->getOpCode().isLoadConst()
            && child->getSecondChild()->getInt() == (child->getSize() - conversion->getSize())) //(16|24|32...?)
        {
            if (trace())
                traceMsg(
                    comp(), "\t\tDetected Sign Extension pattern on conversion and right shift nodes %p\n", conversion);
            return true;
            ...
            && conversion->getSize() != 8) // Needs a conversion from GPR32 to GPR64 or 2xGPR32
        {
            if (trace())
                traceMsg(
                    comp(), "\t\tDetected Sign Extension pattern on conversion and compare nodes %p\n", conversion);
            return true;
        }
        // Many programmers zero out top bits with AND. Calculate the AND mask that will result in conversion being
        // unneeeded
        ...
        //     iconst 0x7FFF          iconst 0xFFFF
            ...
            if (mask == 0)
                ...
                if (trace())
                    traceMsg(
                        comp(), "\t\tDetected Sign Extension pattern on conversion and AND nodes %p\n", conversion);
                return true;
        ...
        // Only for  PLX
        else if (isConvWide && conversion->getSize() == 4 && cg()->isUsing32BitEvaluator(child)
            && child->cannotOverflow()
            && ((opcode.isSignExtension() && !childOpcode.isLoadReg()) || child->isNonNegative())) {
            if (trace())
                traceMsg(comp(),
                    "\t\tDetected Sign Extension s2i/b2i pattern on ssub/sadd, bsub/badd and/or sRegLoad/bRegLoad %p\n",
                    conversion);
            return true;
        }
#endif
    ...
    if (supportedType(node)) {
        if (opcode.isLoadVar() || (opcode.isLoadConst() && supportedConstLoad(node, comp()))) {
            ...
            if (child->getSize() == node->getSize()) {
                ...
                {
                    if (child->getSize() >= 2
                        && (node->getOpCodeValue() == TR::bu2i || node->getOpCodeValue() == TR::b2i)) {
                    } else if (child->getSize() >= 4
                        && (node->getOpCodeValue() == TR::su2i || node->getOpCodeValue() == TR::s2i)) {
                    } else if (child->getSize() == 8
                        && (node->getOpCodeValue() == TR::iu2l || node->getOpCodeValue() == TR::i2l)) {
                    } else {
                        node->setUnneededConversion(true);
                    }
                }
            } else if ((childOpcode.isLoadReg() || (childOpcode.isLoadVar() && child->getSymbol()->isRegisterSymbol()))
                && !(node->getSize() > 4 && TR::Compiler->target.is32Bit()) && !isOverriden(node, regLoadOverride)
                && supportedType(child)) {
                TR_ASSERT(optimizer() && _useDefInfo && _useDefInfo->infoIsValid() && child->getUseDefIndex()
                ...
                for (cursor.SetToFirstOne(); isUnneeded && cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (trace())
                        traceMsg(comp(),
                            "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), evaluate %s%s.\n", child,
                            child->getOpCode().getName(), defNode, defNode->getOpCode().getName(), realLoad,
                            realLoad->getOpCode().getName(), isUnneeded ? "unneed" : "need",
                            mustForce64 ? " mustForce64" : "");
                }

                if (isUnneeded && performTransformation(
                    ...
                    if (node->getType().isInt64() && node->getSize() > child->getSize()) {
                        if (trace())
                            traceMsg(comp(), "\t\t\tset num %d (%s) in getExtendedToInt64GlobalRegisters for child %s "
                        ...
                        // getExtendedToInt64GlobalRegisters is used by the evaluators to force a larger virtual
                        // register to be used when
                        // evaluating the regload so any instructions generated by local RA are the correct size to
...
ncount_t TR_LoadExtensions::indexNodesForCodegen(TR::Node* parent, ncount_t nodeCount, vcount_t visitCount)
{
    ...
    TR::ILOpCode& opcode = parent->getOpCode();
    if (opcode.isLoadVar() && supportedType(parent) || opcode.isLoadConst() && supportedConstLoad(parent, comp())
        //|| opcode.isConversion() // can't index conversion nodes, the second child is already reused for other
        //purposes
        ) {
        if (trace())
    ...
    // The following line should not be needed, especially in case somebody really wants to mark conversion nodes as
    // unneeded earlier
    // but I am tired of finding bugs with flags incorrectly set (i.e. changing Node opcode and inheriting
    // -incorrect/overloaded- flags)
    // As far as I know, nobody else is using these flags in other optimizations yet.
    // Another option is to move the unneededConversion flag to its own bitvector, as with load flags. That is also more
    // work because
    // second-child in conversion nodes may already be sometimes used by <code>bool
    // TR_DynamicLiteralPool::addNewAloadChild(TR::Node *node)</code>
