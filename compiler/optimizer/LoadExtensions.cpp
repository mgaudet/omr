...
#ifdef REUSE_USEDEFS
        && (NULL == (_useDefInfo = optimizer()->getUseDefInfo()) || !_useDefInfo->hasGlobalsUseDefs() || !_useDefInfo->_useDefForRegs)
#endif
            ) {
...
void TR_LoadExtensions::countLoadExtensions(TR::Node* parent, vcount_t visitCount)
{
    ...
    if (opcode.isConversion() && supportedType(parent)) {
        ...
        if ((load->getOpCode().isLoadVar() || load->getOpCode().isLoadReg() || load->getOpCode().isLoadConst()) && supportedType(load) && parent->getSize() != load->getSize()) {
            if (load->getOpCode().isLoadReg() || (load->getOpCode().isLoadVar() && load->getSymbol()->isRegisterSymbol())) {
                if (optimizer() && _useDefInfo && _useDefInfo->infoIsValid() && load->getUseDefIndex() && _useDefInfo->isUseIndex(load->getUseDefIndex()) && (parent->getSize() < 8)) // On Java disable i2l, because of interferance with HRA
                {
                    ...
                    if (!info.IsZero()) {
                        ...
                        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                            ...
                            if ((defNode->getOpCode().isStoreReg() || (defNode->getOpCode().isStore() && defNode->getSymbol() && defNode->getSymbol()->isRegisterSymbol())) && NULL != (realLoad = defNode->getFirstChild()) && ((realLoad->getOpCode().isLoadVar() && !realLoad->getSymbol()->isRegisterSymbol()) || realLoad->getOpCode().isLoadConst() && supportedConstLoad(realLoad, comp())) && supportedType(realLoad)) {
                                if (trace())
                                    traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), Counting.\n",
                                ...
                                if (trace())
                                    traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), setting Ignore.\n",
                                        load, load->getOpCode().getName(),
                                        defNode, defNode->getOpCode().getName(),
    ...
    for (int32_t i = 0; i < parent->getNumChildren(); i++) {
        ...
        if (child->getOpCode().isLoadVar() && (!opcode.isConversion() || (opcode.isConversion() && parent->getSize() < child->getSize()))) {
...
bool TR_LoadExtensions::detectReverseNeededConversionPattern(TR::Node* parent, TR::Node* conversion)
{
    ...
    if (opcode.isAnd() && (conversion->getSize() - conversion->getFirstChild()->getSize()) > 0) // Must be widening conversion
    {
        TR::Node* constant;
        int64_t andMask = 8 * conversion->getFirstChild()->getSize();
        andMask = (1ll << andMask) - 1;
        int64_t andMask2 = andMask >> 1ll;
        if ((NULL != (constant = parent->getFirstChild()) && constant->getOpCode().isLoadConst() || NULL != (constant = parent->getSecondChild()) && constant->getOpCode().isLoadConst() && !constant->getType().isAggregate()) && (constant->get64bitIntegralValue() == andMask2 || conversion->getOpCode().isZeroExtension() && constant->get64bitIntegralValue() == andMask))
            return false;
...
bool TR_LoadExtensions::detectUnneededConversionPattern(TR::Node* conversion, TR::Node* child, bool& mustForce64)
{
    ...
    if (supportedType(child)) {
       ...
       */
        ...
        if (((childOpcode.isLoadVar() && !(child->getSymbol()->isRegisterSymbol())) || childOpcode.isLoadConst() && supportedConstLoad(child, comp())) && isConvWide && (conversion->getSize() != 8 || (TR::Compiler->target.is64Bit() || comp()->cg()->use64BitRegsOn32Bit())) && (loadIsSigned && loadIsSigned == opcode.isSignExtension() || !loadIsSigned && (!loadIsSigned) == conversion->isZeroExtension()) && (opcode.getOpCodeValue() != TR::a2l || child->getReferenceCount() == 1 && TR::Compiler->target.is32Bit())) // 4 byte a2l really means 31->64! careful!
        {
            if (trace())
        ...
        // l2i can be also ignored: will either grab the reg->getLowOrder() or keep on passing GPR64
                                    ...
                                    childOpcode.isConversion() && NULL != (grandChild = child->getFirstChild()) && supportedType(grandChild) && grandChild->getSize() == conversion->getSize())) {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension pattern on narrowing conversion node %p\n", conversion);
            return true;
        } else if (childOpcode.isRightShift() && child->getSecondChild()->getOpCode().isLoadConst() && child->getSecondChild()->getInt() == (child->getSize() - conversion->getSize())) //(16|24|32...?)
        {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion and right shift nodes %p\n", conversion);
            return true;
        } else if (childOpcode.isBooleanCompare() && conversion->getSize() != 8) // Needs a conversion from GPR32 to GPR64 or 2xGPR32
        {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion and compare nodes %p\n", conversion);
            return true;
        }
        // Many programmers zero out top bits with AND. Calculate the AND mask that will result in conversion being unneeeded
        // i.e.
        ...
        //     iconst 0x7FFF          iconst 0xFFFF
            ...
            if (mask == 0)
                ...
                if (trace())
                    traceMsg(comp(), "\t\tDetected Sign Extension pattern on conversion and AND nodes %p\n", conversion);
                return true;
        ...
        // Only for  PLX
        else if (isConvWide && conversion->getSize() == 4 && cg()->isUsing32BitEvaluator(child) && child->cannotOverflow() && ((opcode.isSignExtension() && !childOpcode.isLoadReg()) || child->isNonNegative())) {
            if (trace())
                traceMsg(comp(), "\t\tDetected Sign Extension s2i/b2i pattern on ssub/sadd, bsub/badd and/or sRegLoad/bRegLoad %p\n", conversion);
            return true;
        }
#endif
        else {
    ...
    if (supportedType(node)) {
        if (opcode.isLoadVar() || (opcode.isLoadConst() && supportedConstLoad(node, comp()))) {
            ...
            if (child->getSize() == node->getSize()) {
                ...
                {
                    if (child->getSize() >= 2 && (node->getOpCodeValue() == TR::bu2i || node->getOpCodeValue() == TR::b2i)) {
                    } else if (child->getSize() >= 4 && (node->getOpCodeValue() == TR::su2i || node->getOpCodeValue() == TR::s2i)) {
                    } else if (child->getSize() == 8 && (node->getOpCodeValue() == TR::iu2l || node->getOpCodeValue() == TR::i2l)) {
                    } else {
                ...
                for (cursor.SetToFirstOne(); isUnneeded && cursor.Valid(); cursor.SetToNextOne()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "\t\tPeeked through %p (%s) and found %p (%s) with child %p (%s), evaluate %s%s.\n",
