...
public:
    ...
    static bool supportedType(TR::Node* node)
    {
        ...
        if (!TR::comp()->cg()->getAccessStaticsIndirectly() && node->getOpCode().hasSymbolReference() && node->getSymbol()->isStatic() && node->getOpCode().isLoadDirect() && !(node->getOpCode().isInt() || node->getOpCode().isLong()))
            return false;
