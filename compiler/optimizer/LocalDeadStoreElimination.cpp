...
void TR::LocalDeadStoreElimination::eliminateDeadObjectInitializations()
{
    ...
    for (i = symRefTab->getIndexOfFirstSymRef(); i < symRefCount; i++) {
        ...
        if (symRef) {
            TR::Symbol* sym = symRef->getSymbol();
            if (sym) {
                if (sym->isLocalObject() && sym->getLocalObjectSymbol()->getKind() == TR::New)
    ...
    for (tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop()) {
        ...
        if (storeNode && storeNode->getOpCode().isStoreIndirect() && !storeNode->getSymbolReference()->isUnresolved()) {
            if (storeNode->getFirstChild()->getOpCode().hasSymbolReference() && storeNode->getFirstChild()->getSymbolReference()->getSymbol()->isLocalObject() && (storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalObjectSymbol()->getKind() == TR::New) && !usedLocalObjectSymbols.ValueAt(storeNode->getFirstChild()->getSymbolReference()->getSymbol()->getLocalIndex()))
...
{
    if (node->getOpCode().hasSymbolReference() && (node->getSymbolReference()->getSymbol()->isLocalObject() && node->getSymbolReference()->getSymbol()->getLocalObjectSymbol()->getKind() == TR::New) && !(parent->getOpCode().isStoreIndirect() && childNum == 0 && ((uint32_t)parent->getSymbolReference()->getOffset() < fe()->getObjectHeaderSizeInBytes())))
