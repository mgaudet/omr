    ...
    vcount_t initialVisitCount, bool& compactionDone)
{
    ...
    if (cursorNode->getOpCodeValue() == TR::NULLCHK) {
        ...
        if (isEquivalent) {
            ...
            if (TR::comp()->getOptions()->getOption(TR_NoResumableTrapHandler) || TR::Compiler->om.offsetOfObjectVftField() >= comp()->cg()->getNumberBytesReadInaccessible())
                canBeRemoved = false;
...
void TR_EliminateRedundantGotos::placeAsyncCheck(TR::Block* block)
{
    ...
    TR::TreeTop* actt = TR::TreeTop::create(comp(), TR::Node::createWithSymRef(tt->getNode(), TR::asynccheck, 0,
    ...
    for (TR::TreeTop *treeTop = startTree, *exitTreeTop;
        ...
        if (rootStructure && blockRegion && blockRegion->getNumber() == block->getNumber()) {
            ...
            for (auto inEdge = block->getPredecessors().begin(); inEdge != block->getPredecessors().end();) {
                ...
                if (prevBlock->getLastRealTreeTop()->getNode()->getOpCode().isBranch() && prevBlock->getLastRealTreeTop()->getNode()->getBranchDestination() == block->getEntry()) {
...
int32_t TR_CleanseTrees::process(TR::TreeTop* startTree, TR::TreeTop* endTreeTop)
{
    ...
    for (treeTop = startTree; (treeTop != endTreeTop); treeTop = exitTreeTop->getNextTreeTop()) {
        ...
        if ((block->getNextBlock() && block->getNextBlock()->isGenControlBlock()) || (node->getBranchDestination() && node->getBranchDestination()->getNode()->getBlock()->isGenControlBlock()))
            continue;

        if ((block->getNextBlock() && block->getNextBlock()->isAlwaysKeepBlock()) || (node->getBranchDestination() && node->getBranchDestination()->getNode()->getBlock()->isAlwaysKeepBlock()))
            continue;
...
void TR_Rematerialization::rematerializeSSAddress(TR::Node* parent, int32_t addrChildIndex)
{
    ...
    if (addressNode->getReferenceCount() > 1 && ((addressNode->getOpCodeValue() == TR::loadaddr && addressNode->getSymbolReference()->getSymbol()->isAutoOrParm())
                                                    || (addressNode->getOpCode().isArrayRef() && addressNode->getSecondChild()->getOpCode().isLoadConst()) && cg()->getSupportsConstantOffsetInAddressing(addressNode->getSecondChild()->get64bitIntegralValue())))
...
int32_t TR_Rematerialization::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (treeTop = startTree; (treeTop != endTree); treeTop = treeTop->getNextRealTreeTop()) {
        ...
        if (!shouldOnlyRunLongRegHeuristic())
            ...
            if (!(node->getOpCodeValue() == TR::BBStart || node->getOpCodeValue() == TR::BBEnd || node->getOpCodeValue() == TR::asynccheck)) {
    ...
    TR_RematAdjustments& adjustments)
{
   ...
   */
    ...
    if (!shouldOnlyRunLongRegHeuristic()) {
        ...
        if ((!considerRegPressure || ((state->_currentlyCommonedNodes.getSize() + childAdjustment + adjustments.adjustmentFromParent + restrictedGPRNum) > (cg()->getMaximumNumbersOfAssignableGPRs() /* -1 */))) && (!state->_currentlyCommonedCandidates.isEmpty() || !state->_currentlyCommonedLoads.isEmpty())) {
            //_counter++;
...
void TR_LongRegAllocation::makeLongRegDecision()
{
   ...
   else*/ if (totalRatio > LONGREG_TOTAL_HIGH_WATERMARK && spillRatio < LONGREG_SPILL_THRESHOLD && argRatio < LONGREG_ARG_THRESHOLD) {
        if (trace())
...
{
    if (comp()->getOptions()->getOption(TR_Disable64BitRegsOn32Bit) || !cg()->supportsLongRegAllocation() || !comp()->getJittedMethodSymbol()->mayHaveLongOps()) {
    ...
    if (!comp()->getOptions()->getOption(TR_Disable64BitRegsOn32Bit) && comp()->getOptions()->getOption(TR_Disable64BitRegsOn32BitHeuristic)) {
        comp()->setUseLongRegAllocation(true);
        setLongRegDecision(true);
...
int32_t TR_BlockSplitter::perform()
{
    ...
    while (children->size() > 0) {
        ...
        if (
            (toBlock(children->top())->hasExceptionPredecessors() == true) || (children->top()->getPredecessors().size() == 1) || children->top()->getPredecessors().empty()) {
            children->pop();
            continue;
        }
      ...
      if ( exitTree->getNextTreeTop() && exitTree->getNextTreeTop()->getNode()->getBlock()->isGenAsmBlock())
        ...
        if (currentBlock->isGenAsmFlowBlock()) {
            ...
            for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
                ...
                // we can alwasy insert a long jump if currentBlock is fall through of predBlock
                if (currentBlock->getExit() && currentBlock->getExit()->getNextTreeTop() && currentBlock->getExit()->getNextTreeTop()->getNode()->getBlock() == succBlock && succBlock->getEntry()->getNode()->getLabel() == NULL)
                    continue;
