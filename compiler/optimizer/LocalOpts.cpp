...
bool TR_BlockManipulator::isBestChoiceForFallThrough(TR::Block* block1, TR::Block* block2)
{
    ...
    if (block1->isSuperCold() || ((block1->getSuccessors().size() == 1)
        ...
        if ((node1->getOpCode().isReturn() ||
            ...
            for (auto nextEdge = block->getPredecessors().begin(); nextEdge != block->getPredecessors().end();) {
                ...
                if (isLoopHdr) {
                    ...
                    for (auto nextEdge = block->getSuccessors().begin(); nextEdge != block->getSuccessors().end();
                         ++nextEdge) {
    ...
    if (currentNode->getOpCodeValue() == TR::arraycopy) {
        ...
        if ((arraycopyDestAddressNode == addressChild)
            ...
            *scratch &= *readAndWrittenSymbols;
            ...
            if (scratch->isEmpty()
                && performTransformation(comp(), "%sEliminating some stores done by arrayset node %p because stores "
        ...
        if ((lastRealNode->getOpCode().isBranch() && (lastRealNode->getOpCodeValue() != TR::Goto)
            ...
            while (seenAndNode) {
                ...
                if ((andNode->getFirstChild() == lastRealNode->getFirstChild())
                    ...
                    && lastRealNode->getOpCode().isBooleanCompare()) // this code was allowing unsigned types in -- I
                                                                     // made this test stronger // NOTE that isCompare
                                                                     // is only true for signed compares -- this is very
                                                                     // ugly though
                {
            ...
            if (!seenAndBefore) {
                ...
                if (cg()->branchesAreExpensive() && (lastRealNode->getOpCodeValue() == TR::ificmpge
                        ...
                        if ((firstTree == lastTree) && (nextBlock->getPredecessors().size() == 1)) {
                            ...
                            if (firstNode->getOpCode().isStoreDirect()
                                ...
                                if (areTreesInRequiredForm
                                    ...
                                    TR::Node* newIShrNode
                                        = TR::Node::create((lastFirstChild->getType().isInt32()) ? TR::ishr : TR::lshr,
                                            2, lastFirstChild, TR::Node::create(lastRealNode, TR::iconst, 0,
...
int32_t TR_EliminateRedundantGotos::process(TR::TreeTop* startTree, TR::TreeTop* endTree)
{
    ...
    for (TR::TreeTop *treeTop = startTree, *exitTreeTop; (treeTop != endTree);
        ...
        while (inEdge != block->getPredecessors().end()) {
            ...
            if (!emptyBlock && (!tt->getNode()->getOpCode().isBranch()
...
int32_t TR_CleanseTrees::process(TR::TreeTop* startTree, TR::TreeTop* endTreeTop)
{
    ...
    for (treeTop = startTree; (treeTop != endTreeTop); treeTop = exitTreeTop->getNextTreeTop()) {
        ...
        if ((block->getNextBlock() && block->getNextBlock()->isGenControlBlock())
            || (node->getBranchDestination()
                   && node->getBranchDestination()->getNode()->getBlock()->isGenControlBlock()))
            continue;

        if ((block->getNextBlock() && block->getNextBlock()->isAlwaysKeepBlock())
            || (node->getBranchDestination()
        ...
        if (node->getOpCodeValue() == TR::BBEnd) {
            ...
            case TR::anewarray: {
                ...
                TR_ValueInfo* numElementsInfo
                    = (TR_ValueInfo*)TR_ValueProfiler::getProfiledValueInfo(numElementsNode, comp());
                ...
                if (numElementsInfo) {
                    ...
                    if (totalFrequency > 0) {
                        ...
                        for (TR_ExtraAbstractInfo* profiledInfo = i.getFirst(); profiledInfo != NULL;
                    ...
                    if (_curLongOps > 0) {
                        ...
                        if (isInLoop) {
                            TR_RegionStructure* region
                                = treeTop->getEnclosingBlock()->getParentStructureIfExists(comp()->getFlowGraph());
                            if (trace())
    ...
    List<TR::Node>* loadsAlreadyVisitedThatCannotBeRematerialized, const TR::SparseBitVector& aliases)
{
    ...
    while (commonedElement) {
        ...
        if ((commonedElement->getData() == node)
            || (checkSymRefs && ((commonedElement->getData()->getSymbolReference() == node->getSymbolReference())
                                    || aliases.ValueAt(
    ...
    TR_RematState* state, TR_RematAdjustments& adjustments)
{
    ...
    */
    ...
    if (node->getOpCode().isCall()
        && (node->getOpCode().isTreeTop() || (parent && ((parent->getOpCodeValue() == TR::treetop)
    ...
    if (!shouldOnlyRunLongRegHeuristic()) {
        ...
        if (trace() && !shouldOnlyRunLongRegHeuristic()) {
            traceMsg(comp(), "At node %p parent %p GPR pressure is %d (child adjust %d parent adjust %d) limit is %d\n",
                node, parent, numRegisters + childAdjustment + adjustments.adjustmentFromParent + restrictedGPRNum,
                childAdjustment, adjustments.adjustmentFromParent, (cg()->getMaximumNumbersOfAssignableGPRs() - 1));
        ...
        if ((!considerRegPressure || ((state->_currentlyCommonedNodes.getSize() + childAdjustment
    ...
    if (!shouldOnlyRunLongRegHeuristic()) {
        if ((!considerRegPressure || ((state->_currentlyCommonedFPNodes.getSize() + fpChildAdjustment
    ...
    if (!shouldOnlyRunLongRegHeuristic()) {
        if ((!considerRegPressure || ((state->_currentlyCommonedVector128Nodes.getSize() + vector128ChildAdjustment
                                          + adjustments.vector128AdjustmentFromParent)
...
void TR_LongRegAllocation::makeLongRegDecision()
{
    ...
    /*if (totalRatio < LONGREG_TOTAL_LOW_WATERMARK)
      ...
      return ;
        ...
        && spillRatio < LONGREG_SPILL_THRESHOLD && argRatio < LONGREG_ARG_THRESHOLD) {
        if (trace())
...
{
    if (comp()->getOptions()->getOption(TR_Disable64BitRegsOn32Bit) || !cg()->supportsLongRegAllocation()
...
int32_t TR_BlockSplitter::perform()
{
    ...
    if (disableSynergy() && comp()->isOptServer()) {
        if (trace())
            traceMsg(comp(), "Large app detected, making BlockSplitter very conservative to avoid cloning code "
    ...
    while (children->size() > 0) {
        ...
        if ((toBlock(children->top())->hasExceptionPredecessors() == true)
            || (children->top()->getPredecessors().size() == 1) || children->top()->getPredecessors().empty()) {
            children->pop();
            continue;
        }
...
int32_t TR_InvariantArgumentPreexistence::perform()
{
...

    {
        ...
        for (TR::TreeTopIterator iter(methodSymbol->getFirstTreeTop(), this); iter != NULL && numInvariantArgs > 0;
             ++iter) {
        ...
        if (!isInterface && !resolvedMethod->virtualMethodIsOverridden() && !resolvedMethod->isAbstract()) {
            ...
            if (methSymbol->isInterface() || methodSymbol) {
                ...
                if (method && !method->virtualMethodIsOverridden()) {
                    ...
                    if (addAssumptions
                        && performTransformation(comp(), "%sspecialize and devirtualize invoke [%p] based on only a "
        ...
        case TR::ifacmpeq:
            ...
            if (checkcastTree) {
                if (node->getFirstChild() != checkcastTree->getNode()->getFirstChild()
                    ...
                    traceMsg(comp(), "Found suitable ifacmpeq [%p] after checkcast [%p], safe to continue to next "
                                     "block in search of a profiled guard\n",
    ...
    for (TR::AllBlockIterator iter(optimizer()->getMethodSymbol()->getFlowGraph(), this); iter.currentBlock(); ++iter) {
        ...
        if (block->isCold())
            ...
            if ((coldness <= MAX_COLD_BLOCK_COUNT)
                && ((!controlled) || performTransformation(comp(), "%s%s marked block_%d cold\n", optDetailString(),
                                         name(), block->getNumber()))) {
                block->setIsCold();
                block->setFrequency(coldness);
                foundColdBlocks = true;
            } else if (_enableFreqCBO && block->getFrequency() == 0
                && ((!controlled) || performTransformation(comp(), "%s%s marked block_%d rare\n", optDetailString(),
                                         name(), block->getNumber()))) {
                foundColdBlocks = true;
            }
        }
...
#ifdef J9_PROJECT_SPECIFIC
                !firstChild->getOpCode().isPackedExponentiation()
    ...
    if (child->getReferenceCount() > 1) {
        if (!commonedTreeTopList.isEmpty()) {
            ...
            while (listTT) {
                ...
                //
                if (trace)
                    traceMsg(comp, "\tcomparing listTT %p with firstChild %s (%p) to commoned child %s (%p, refCount "
                                   "%d) (listTT == _currentTreeTop -- %s)\n",
