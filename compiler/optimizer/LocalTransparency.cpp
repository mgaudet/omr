                ...
                if (!_supportedNodes->get(child->getLocalIndex()))
                    ...
                    for (i = 0; i < comp()->getMaxAliasIndex(); i++) {
                        if (!_transparencyInfo[i]->get(child->getLocalIndex())
                            ...
                            if (trace())
                                traceMsg(comp(), "Expression %d killed by symRef #%d because child %d is already "
                ...
                if (!_transparencyInfo[i]->get(child->getLocalIndex())) {
                    ...
                    if (trace()) {
                        if (TR::Compiler->target.is64Bit())
                            traceMsg(comp(), "Expression %d killed by symRef #%d because grandchild (child of aladd) "
                                             "%d is already killed by the symRef\n",
                                node->getLocalIndex(), i, child->getLocalIndex());
                        else
                            traceMsg(comp(), "Expression %d killed by symRef #%d because grandchild (child of aiadd) "
                                             "%d is already killed by the symRef\n",
                                node->getLocalIndex(), i, child->getLocalIndex());
                    }
                }
