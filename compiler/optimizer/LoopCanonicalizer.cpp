    ...
    bool innerFirst)
{
    while (!_analysisStack.isEmpty() && _analysisStack.top()->getStructure() != region) {
        ...
        if (!_nodesInCycle->get(rootNode->getNumber())) {
            ...
            predList.insert(predList.end(), rootNode->getExceptionPredecessors().begin(),
        ...
        succList.insert(
            succList.end(), rootNode->getExceptionSuccessors().begin(), rootNode->getExceptionSuccessors().end());
 ...
 region->getEntry()->getStructure(), region->getEntry(), region, visitCount, pendingList, innerFirst);
    ...
    */
    ...
    if (asLoopCanonicalizer()) {
            ...
            if ((hdrBlock->hasExceptionPredecessors() == false) && (hdrBlock->hasExceptionSuccessors() == false)) {
                ...
                if (entryNode->getSuccessors().size() == 2) {
                    ...
                    if (succ1Internal ^ succ2Internal) {
                        ...
                        while ((firstTree && (firstTree->getNode()->getOpCodeValue() == TR::asynccheck
                                        ...
                                        if (exitBlock != hdrBlock) {
                                            if (exitBlock->getSuccessors().size() == 2) {
                                                ...
                                                TR::CFGEdge* exitSucc2
                                                    = exitBlock->getSuccessors().back(); // Get the 2nd Successor
        ...
        // region->getParent());
        // bool pred2Internal = region->contains(toStructureSubGraphNode(pred2->getFrom())->getStructure(),
        // region->getParent());
...
void TR_LoopCanonicalizer::canonicalizeNaturalLoop(TR_RegionStructure* whileLoop)
{
    ...
    */
    ...
    TR_BlockStructure* clonedHdrBlockStructure
        = new (trHeapMemory()) TR_BlockStructure(comp(), clonedHeader->getNumber(), clonedHeader);
    if (needToSetFlag) {
    ...
    TR_StructureSubGraphNode* node = new (trHeapMemory())
        TR_StructureSubGraphNode(new (trHeapMemory()) TR_BlockStructure(comp(), splitter2->getNumber(), splitter2));
    ...
    node = new (trHeapMemory())
        TR_StructureSubGraphNode(new (trHeapMemory()) TR_BlockStructure(comp(), splitter1->getNumber(), splitter1));
    properRegion->addSubNode(node);
    TR::CFGEdge::createEdge(hdrNode, node, trMemory());
    ...
    if (reversedBranch) {
        ...
        TR_BlockStructure* extraGotoStruct
            = new (trHeapMemory()) TR_BlockStructure(comp(), extraGoto->getNumber(), extraGoto);
            ...
            if (toNum == joinNumber) {
                if (fromNode->getStructure() == clonedHdrBlockStructure) {
                    edge->setTo(TR_StructureSubGraphNode::create(node->getNumber(),
                        whileLoop)); // new (trHeapMemory()) TR_StructureSubGraphNode(node->getNumber()));
                } else
...
void TR_LoopCanonicalizer::canonicalizeDoWhileLoop(TR_RegionStructure* doWhileLoop)
{
    ...
    TR_BlockStructure* invariantBlockStructure
        = new (trHeapMemory()) TR_BlockStructure(comp(), loopInvariantBlock->getNumber(), loopInvariantBlock);
    ...
    if (isEntry) {
        ...
        dummyEntryBlockStructure
            = new (trHeapMemory()) TR_BlockStructure(comp(), dummyEntryBlock->getNumber(), dummyEntryBlock);
    ...
    if (primaryInductionVariable && bivIterator.getFirst()) {
        ...
        for (nextIndVar = bivIterator.getFirst(); nextIndVar != NULL;) {
            ...
            if ((nextIndVar->getSymRef()->getSymbol()->getDataType()
                ...
                if (incrementsInLockStep && (!derivedInductionVarIncrementBlocks.isEmpty()
    ...
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!nextInductionSymRef->getUseonlyAliases().isZero(comp()) || !_storeTrees[nextInductionVariableNumber]
            || !_loopTestTree || ((_loopTestTree->getNode()->getOpCodeValue() != TR::ificmplt)
        ...
        if (isInvertibleLoop(nextInductionVariableNumber, loopStructure)) {
            ...
            if (v) {
                ...
                if (entryVal && entryVal->asIntConst()
                    ...
                    // For signed case, we may allow abs(incr)>1, but exit value calc is not just RhsOfStoreNode if it
