...
bool TR_LRAddressTree::checkAiadd(TR::Node* aiaddNode, int32_t elementSize)
{
    ...
    if (_multiplyNode.isNull() && ((elementSize != _increment) && (elementSize != -_increment))) {
        dumpOptDetails(comp(), "checkAiadd: sub-tree does not have induction variable change consistent with increment "
                               "of multiplier (%d %d)\n",
...
bool TR_LoopReducer::generateArraycopy(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
    ...
    if (arraycopyLoop.hasWriteBarrier() && needWriteBarrier && !comp()->cg()->getSupportsArrayCopy()) {
        dumpOptDetails(comp(), "arraycopy arraystore tree has write barrier as root and write barriers are enabled but "
                               "no support for this platform- no arraycopy reduction\n");
        return false;
    }
    ...
    if (storeNode->getType().isBCD() && !(storeSize == 1 || storeSize == 2 || storeSize == 4 || storeSize == 8)) {
        dumpOptDetails(comp(), "arraycopy storeNode %p is a BCD type (%d) and the storeSize (%d) is not 1,2,4 or 8 so "
                               "do not reduce arraycopy\n",
...
bool TR_LoopReducer::generateArrayset(TR_InductionVariable* indVar, TR::Block* loopHeader)
{
    ...
    if (!performTransformation(comp(), "%sReducing arrayset %d from storeNode [" POINTER_PRINTF_FORMAT
    ...
    TR_RegionStructure* whileLoop, TR_InductionVariable* indVar, TR::Block* branchBlock, TR::Block* incrementBlock)
{
    ...
    //   updated)
    //   The number of elements to compare (induction variable initial value - comparison value) / increment /
    //   multiplier
    //
    // Tree-Top
    // ArrayCmp call
    ...
    TR_InductionVariable* byteIndVar, TR_InductionVariable* charIndVar, TR::Block* loopHeader)
{
    ...
    {
        dumpOptDetails(comp(), "Loop does not have an increment of +/-1 and +/-2, but instead %d and %d - no byte to "
                               "char arraycopy reduction\n",
            incrA, incrB);
        return false;
    }
...
void TR_LoopReducer::removeSelfEdge(TR::list<TR::CFGEdge*> succList, int32_t selfNumber)
{
...
void TR_LoopReducer::removeEdge(TR::list<TR::CFGEdge*> succList, int32_t selfNumber, int32_t destNumber)
{
    for (auto edge = succList.begin(); edge != succList.end();) {
        TR::Block* dest = toBlock((*edge)->getTo());
        TR::Block* src = toBlock((*edge)->getFrom());
...
int32_t TR_LoopReducer::perform()
{
    ...
    if (!comp()->cg()->getSupportsArraySet() && !comp()->cg()->getSupportsArrayCopy()
