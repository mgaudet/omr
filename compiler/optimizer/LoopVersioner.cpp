...
int32_t TR_LoopVersioner::performWithoutDominators()
{
    ...
    for (nextWhileLoop = whileLoopsIt.getFirst(); nextWhileLoop != NULL; nextWhileLoop = whileLoopsIt.getNext()) {
        ...
        bool nullChecksMayBeEliminated = detectChecksToBeEliminated(naturalLoop, &nullCheckedReferences,
            ...
            &divCheckTrees, &iwrtbarTrees, &checkCastTrees, &arrayStoreCheckTrees, &specializedInvariantNodes,
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases())
            checkCastTreesWillBeEliminated
                = detectInvariantTrees(&checkCastTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
        else
        ...
        if (!shouldOnlySpecializeLoops() && !refineAliases()) {
            if (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                ...
                || boundChecksWillBeEliminated || divChecksWillBeEliminated || iwrtBarsWillBeEliminated
                ...
                conditionalsWillBeEliminated
                    = detectInvariantTrees(&conditionalTrees, true, &containsNonInlineGuard, reverseBranchInLoops);
            } else {
                conditionalsWillBeEliminated
                    = detectInvariantTrees(&conditionalTrees, false, &containsNonInlineGuard, reverseBranchInLoops);
        ...
        if ((versionToRefineAliases || !refineAliases())
            && (((debug("nullCheckVersion") || comp()->cg()->performsChecksExplicitly()
                   ...
                   || iwrtBarsWillBeEliminated || checkCastTreesWillBeEliminated || arrayStoreCheckTreesWillBeEliminated
            ...
            versionNaturalLoop(naturalLoop, &nullCheckedReferences, &nullCheckTrees, &boundCheckTrees, &spineCheckTrees,
                &conditionalTrees, &divCheckTrees, &iwrtbarTrees, &checkCastTrees, &arrayStoreCheckTrees,
        ...
        if (distinctTargets[i] && distinctTargets[i]->_count != 0) {
            ...
            if (!loopTempSymRef) {
                ...
                clonedHeader->prepend(TR::TreeTop::create(
                    comp(), TR::Node::createWithSymRef(TR::istore, 1, 1,
...
bool TR_LoopVersioner::detectInvariantTrees(List<TR::TreeTop>* trees, bool onlyDetectHighlyBiasedBranches,
...
TR::Node* TR_LoopVersioner::isDependentOnInductionVariable(TR::Node* useNode, bool noArithmeticAllowed,
    bool& isIndexChildMultiplied, TR::Node*& mulNode, TR::Node*& strideNode)
{
    TR_UseDefInfo* useDefInfo = optimizer()->getUseDefInfo();
    if (!useDefInfo)
        return NULL;
                ...
                while (indexSymRef && changedIndexSymRef) {
                    ...
                    if (!isInductionVariable) {
                        ...
                        indexChild = isDependentOnInductionVariable(
                            indexChild, changedIndexSymRefAtSomePoint, isIndexChildMultiplied, mulNode, strideNode);
    ...
    if (comp->getMethodHotness() >= hot && callSymRef) {
        if (callSymRef->getSymbol() && callSymRef->getSymbol()->castToMethodSymbol()
            ...
            if (disableLoopCodeRatioCheck || loopCodeRatio < 0.25) {
                traceMsg(comp, "Skipping versioning of profiled guard %p because we found more than 2 JIT'd "
                               "implementors at warm or above and the loop code ratio is too low\n",
                    guardNode);
                risky = true;
                TR::DebugCounter::incStaticDebugCounter(
                    comp, TR::DebugCounter::debugCounterName(comp,
                              "profiledVersioning/unsuitableForVersioning/interfaceGuard/(%s)/bci=%d.%d",
    ...
    if (node->isProfiledGuard()) {
        ...
        if (isGetFieldCacheCounts || disableProfiledGuardVersioning) {
            ...
            TR_AddressInfo* valueInfo = (TR_AddressInfo*)profileManager->getValueInfo(
                ics._byteCodeInfo, comp, TR_ValueProfileInfoManager::allProfileInfoKinds, NotBigDecimalOrString);
...
bool TR_LoopVersioner::detectChecksToBeEliminated(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences,
    ...
    List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* iwrtbarTrees, List<TR::TreeTop>* checkCastTrees,
        ...
        if (nextBlock->isCold()) {
            ...
            if (blockFrequency >= 0 // frequency must be valid
                ...
                && performTransformation(comp(),
                       "%sDisregard unimportant block_%d frequency %d < loop %d frequency %d\n",
                       OPT_DETAILS_LOOP_VERSIONER, nextBlock->getNumber(), blockFrequency, whileLoop->getNumber(),
                       loopFrequency)) {
                isUnimportant = true;
        ...
        while (!(currentTree == exitTree)) {
            ...
            if (!isUnimportant && !_containsCall && !refineAliases()) {
                ...
                if (comp()->getOptions()->getGcMode() == TR_WrtbarCardMarkAndOldCheck
                    ...
                    if ((currentOpCode.getOpCodeValue() != TR::wrtbari) && (possibleIwrtbarNode->getNumChildren() > 0))
                        possibleIwrtbarNode = possibleIwrtbarNode->getFirstChild();

                    if ((possibleIwrtbarNode->getOpCodeValue() == TR::wrtbari) && !possibleIwrtbarNode->skipWrtBar()
    ...
    TR::Block* block, int32_t warmBranchCount)
{
    ...
    if (TR_LocalAnalysis::isSupportedOpCode(node->getOpCode(), comp())) {
        if (node->getType().isInt32() || (node->getType().isInt64() && node->getOpCode().isLoadVar()
            ...
            if (/* comp()->getRecompilationInfo() && */ collectProfiledExprs
                ...
                if (valueInfo) {
                    if ((valueInfo->getTopProbability() > MIN_PROFILED_FREQUENCY) && (valueInfo->_totalFrequency > 0)
                        ...
                        if ((!node->getType().isInt64()) || (profileLongParms && (valueInfo->getTopValue() == 0))) {
                            ...
                            if (comp()->requiresSpineChecks() && node->getOpCodeValue() == TR::contigarraylength
                                ...
                                dumpOptDetails(comp(), "From value profiling, NOT USING inconclusive contigarraylength "
                                                       "node %p with value 0 freq %d total freq %d\n",
                                    node,
                                    ((int32_t)(valueInfo->getTopProbability() * ((float)valueInfo->_totalFrequency))),
...
void TR_LoopVersioner::versionNaturalLoop(TR_RegionStructure* whileLoop, List<TR::Node>* nullCheckedReferences,
    ...
    List<TR::TreeTop>* conditionalTrees, List<TR::TreeTop>* divCheckTrees, List<TR::TreeTop>* iwrtbarTrees,
    ...
    SharedSparseBitVector& reverseBranchInLoops)
{
            ...
            if (clonedSucc) {
                ...
                if (!fallsThrough)
                    ...
                    TR_BlockStructure* newGotoBlockStructure
                        = new (trHeapMemory()) TR_BlockStructure(comp(), newGotoBlock->getNumber(), newGotoBlock);
    ...
    if (_requiresAdditionalCheckForIncrement
        && performTransformation(comp(), "%s Creating test outside loop for checking if loop driving induction "
        ...
        if (_constNode->getType().isInt32()) {
            ...
            TR_ASSERT(0, "Unable to create test outside of loop for checking if iv update increment is greater than 0 "
                         "due to unknown type\n");
        }
    }
        ...
        if (loopLimit->getType().isInt32() && !loopLimit->getByteCodeInfo().doNotProfile()) {
            ...
            if (valueInfo) {
                if (valueInfo->getTotalFrequency()) {
                    dumpOptDetails(comp(), "From value profiling, loop test node %p has value %d freq %d total freq "
                                           "%d, and max value %d\n",
                    ...
                    // if (shouldOnlySpecializeLoops())
                    {
                        ...
                        for (TR_ExtraAbstractInfo* profiledInfo = sortedValuesIt.getFirst(); profiledInfo != NULL;
                        ...
                        if (valueInfo->getMaxValue() > profiledLoopLimit && valuedFreq / totalFreq < 0.95f) {
                            dumpOptDetails(comp(), "Maximum profiled value of %d exceeds profiled loop limit of %d, "
                                                   "and value profiled iterations below limit account for only %5.3f "
                                                   "of iterations.  Delaying asyncCheck removal for node %p\n",
        ...
        if (!refineAliases() && _canPredictIters && !comp()->isProfilingCompilation()
            ...
            if (isIncreasing)
                duplicateLoopLimit = TR::Node::create(
                    TR::isub, 2, duplicateLoopLimit, _loopTestTree->getNode()->getFirstChild()->duplicateTree());
            else {
                // printf("Eliminating an async on decreasing guard in %s\n", comp()->signature());
                duplicateLoopLimit = TR::Node::create(
                    TR::isub, 2, _loopTestTree->getNode()->getFirstChild()->duplicateTree(), duplicateLoopLimit);
            }
            collectAllExpressionsToBeChecked(nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
                duplicateLoopLimit, &comparisonTrees, clonedLoopInvariantBlock, comp()->incVisitCount());
            TR::Node* nextComparisonNode = TR::Node::createif(comparisonOpCode, duplicateLoopLimit,
                TR::Node::create(duplicateLoopLimit, TR::iconst, 0, profiledLoopLimit),
    ...
    // to be null checked.
    //
    if (comp()->cg()->performsChecksExplicitly() || (comp()->getMethodHotness() >= veryHot)) {
    ...
    if (!iwrtbarTrees->isEmpty() && !shouldOnlySpecializeLoops()) {
        buildIwrtbarComparisonsTree(iwrtbarTrees, nullCheckTrees, divCheckTrees, checkCastTrees, arrayStoreCheckTrees,
            &comparisonTrees, clonedLoopInvariantBlock);
    }
    ...
    if (comparisonTrees.isEmpty()) {
        ...
        TR::Node* nextComparisonNode
            = TR::Node::createif(TR::ificmpne, constNode, constNode, clonedLoopInvariantBlock->getEntry());
        comparisonTrees.add(nextComparisonNode);
        dumpOptDetails(comp(), "The comparison tree is empty.  The node %p has been created because a test is required "
                               "when there are two versions of the loop.\n",
            nextComparisonNode);
    }
    ...
    TR_StructureSubGraphNode* clonedInvariantNode
        = new (trHeapMemory()) TR_StructureSubGraphNode(clonedInvariantBlockStructure);
    ...
    while (currComparisonBlock) {
        ...
        TR_BlockStructure* comparisonBlockStructure
            = new (trHeapMemory()) TR_BlockStructure(comp(), actualComparisonBlock->getNumber(), actualComparisonBlock);
        ...
        if (currComparisonBlock) {
            TR_StructureSubGraphNode* criticalEdgeNode
                = new (trHeapMemory()) TR_StructureSubGraphNode(currCriticalEdgeBlock->getData()->getStructureOf());
...
void TR_LoopVersioner::buildIwrtbarComparisonsTree(List<TR::TreeTop>* iwrtbarTrees, List<TR::TreeTop>* nullCheckTrees,
        ...
        if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is iwrtbar is required\n",
            ...
            if (!isX86 && (isVariableHeapBase || isVariableHeapSize)) {
                if (TR::Compiler->target.is64Bit())
                    ifNode1
                        = TR::Node::create(TR::lucmpge, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
                            TR::Node::createWithSymRef(TR::lload, 0,
                                               comp()->getSymRefTab()->findOrCreateThreadLowTenureAddressSymbolRef()));
                else
                    ifNode1
                        = TR::Node::create(TR::iucmpge, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
                            TR::Node::createWithSymRef(TR::iload, 0,
                                               comp()->getSymRefTab()->findOrCreateThreadLowTenureAddressSymbolRef()));
            } else {
                if (TR::Compiler->target.is64Bit()) {
                    ifNode1 = TR::Node::create(TR::lucmpge, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
                        TR::Node::create(duplicateBase, TR::lconst, 0, 0));
                    ifNode1->getSecondChild()->setLongInt(fej9->getLowTenureAddress());
                } else
                    ifNode1 = TR::Node::create(TR::iucmpge, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
                        TR::Node::create(duplicateBase, TR::iconst, 0, fej9->getLowTenureAddress()));
            }
            ...
            if (!isX86 && (isVariableHeapBase || isVariableHeapSize)) {
                if (TR::Compiler->target.is64Bit())
                    ifNode2
                        = TR::Node::create(TR::lucmplt, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
                            TR::Node::createWithSymRef(TR::lload, 0,
                                               comp()->getSymRefTab()->findOrCreateThreadHighTenureAddressSymbolRef()));
                else
                    ifNode2
                        = TR::Node::create(TR::iucmplt, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
                            TR::Node::createWithSymRef(TR::iload, 0,
                                               comp()->getSymRefTab()->findOrCreateThreadHighTenureAddressSymbolRef()));
            } else {
                if (TR::Compiler->target.is64Bit()) {
                    ifNode2 = TR::Node::create(TR::lucmplt, 2, TR::Node::create(TR::a2l, 1, duplicateBase),
                        TR::Node::create(duplicateBase, TR::lconst, 0, 0));
                    ifNode2->getSecondChild()->setLongInt(fej9->getHighTenureAddress());
                } else
                    ifNode2 = TR::Node::create(TR::iucmplt, 2, TR::Node::create(TR::a2i, 1, duplicateBase),
                        TR::Node::create(duplicateBase, TR::iconst, 0, fej9->getHighTenureAddress()));
            }
    ...
    TR::Block* exitGotoBlock, TR::Block* loopInvariantBlock)
{
    ...
    while (nextInvariantNode) {
        ...
        if (nextInvariantNode->getData()->_node->getReferenceCount() < 1
            ...
            if (trace())
                traceMsg(comp(), "skipping node %p or its parent %p which were removed as a part of a versioned check "
                                 "(e.g. count < 1)\n",
    ...
    while (nextInvariantNode) {
        if (nextInvariantNode->getData()->_symRef) {
            ...
            if (nextInvariantNode->getData()->_node->getReferenceCount() > 0
                ...
                if (trace())
                    traceMsg(comp(), "skipping node %p or its parent %p which were removed as a part of a versioned "
                                     "check (e.g. count < 1)\n",
                        nextInvariantNode->getData()->_node, nextInvariantNode->getData()->_parent);
            }
        }
    ...
    while (nextTree) {
        ...
        // if (includeComparisonTree)
        {
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is conditional\n",
                ...
                if (trace())
                    traceMsg(comp(), "Frequency Test for conditional node [%p], destination Frequency %d, fallThrough "
                                     "frequency %d\n",
                ...
                if (reverseBranchInLoops[conditionalNode->getGlobalIndex()]) {
                    if (trace())
                        traceMsg(comp(), "Branch reversed for conditional node [%p], destination Frequency %d, "
                ...
                if (conditionalNode->isVersionableIfWithMaxExpr() || conditionalNode->isVersionableIfWithMinExpr()) {
                    ...
                    if (isInductionVar && (conditionalNode->isVersionableIfWithMaxExpr() && isAddition
      ...
      if (boundCheckNode->getOpCodeValue() == TR::BNDCHKwithSpineCHK)
        ...
        if (isIndexInvariant) {
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                ...
                nextComparisonNode = TR::Node::createif(TR::ificmplt, indexNode->duplicateTree(),
            ...
            if (performTransformation(comp(), "%s Creating test outside loop for checking if %p exceeds bounds\n",
                ...
                if (boundCheckNode->getOpCodeValue() == TR::BNDCHK
                    ...
                    nextComparisonNode = TR::Node::createif(TR::ificmpge, indexNode->duplicateTree(),
                        arrayLengthNode->duplicateTree(), exitGotoBlock->getEntry());
                else
                    nextComparisonNode = TR::Node::createif(TR::ificmpgt, indexNode->duplicateTree(),
                        arrayLengthNode->duplicateTree(), exitGotoBlock->getEntry());
            ...
            if (performFirstBoundCheck) {
                if (isSpecialInductionVariable) {
                    ...
                    nextComparisonNode
                        = TR::Node::createif(TR::ificmplt, boundCheckNode->getChild(indexChildIndex)->duplicateTree(),
                            TR::Node::create(boundCheckNode, TR::iconst, 0, 0), exitGotoBlock->getEntry());
                    if (trace())
    ...
    TR::Node* node, List<TR::Node>* comparisonTrees, TR::Block* exitGotoBlock, vcount_t visitCount)
{
    ...
    //
    if (node->isInternalPointer() || (((node->getOpCode().isIndirect() && node->getOpCode().hasSymbolReference()
        ...
        if (node->isInternalPointer()
            ...
            if (node->isInternalPointer() || node->getOpCode().isArrayLength()) {
                ...
                dumpOptDetails(comp(), "For node %p len %d other len %d sig %p other sig %p\n", node, len, otherLen,
                    sig, otherSig);
                if (otherSig)
                    otherClassObject = otherSymRef->getOwningMethod(comp())->getDeclaringClassFromFieldOrStatic(
                        comp(), otherSymRef->getCPIndex());

                // the call to findOrCreateClassSymbol is safe even though we pass CPI of -1 since the call above
                // getClassFromSignature is not vetted for AOT
                if (cl && otherClassObject && (fe()->isInstanceOf(cl, otherClassObject, true) == TR_yes))
                    ...
                    duplicateClassPtr = TR::Node::createWithSymRef(node, TR::loadaddr, 0,
                        comp()->getSymRefTab()->findOrCreateClassSymbol(
                            otherSymRef->getOwningMethodSymbol(comp()), -1, otherClassObject, false));
            }
        }
        ...
        if (instanceOfReqd) {
            if (otherClassObject) {
                if (performTransformation(comp(),
                        "%s Creating test outside loop for checking if %p is of the correct type\n",
                        OPT_DETAILS_LOOP_VERSIONER, node->getFirstChild())) {
                ...
                if (performTransformation(comp(), "%s Creating test outside loop for checking if %p is of array type\n",
                    ...
                    TR::Node* romClassLoad = TR::Node::createWithSymRef(
                        TR::aloadi, 1, 1, vftLoad, comp()->getSymRefTab()->findOrCreateClassRomPtrSymbolRef());
                    ...
                    TR::Node* andConstNode = TR::Node::create(
                        isArrayField, TR::iconst, 0, TR::Compiler->cls.flagValueForArrayCheck(comp()));
                    ...
                    dumpOptDetails(
                        comp(), "The node %p has been created for testing if object is of array type\n", cmp);
        ...
        if (!duplicateIndex) {
            if (!childInRequiredForm) {
                ...
                duplicateIndex
                    = TR::Node::create(type.isInt32() ? TR::isub : TR::lsub, 2, offset->duplicateTree(), constNode);
                if (type.isInt64())
    ...
    TR_Structure* loopStructure, TR_BitVector** optSetInfo, int32_t bitVectorSize)
{
    ...
    if (loopInvariantBlock) {
        /////if (isInvariantLoop)
        {
            ...
            if ((isPredictableLoop > 0) && ((_loopTestTree->getNode()->getOpCodeValue() == TR::ificmplt)
                                               || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpgt)
                                               || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmpge)
                                               || (_loopTestTree->getNode()->getOpCodeValue() == TR::ificmple))) {
                if (trace()) {
