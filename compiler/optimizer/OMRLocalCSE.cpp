...
void OMR::LocalCSE::transformBlock(TR::TreeTop* entryTree, TR::TreeTop* exitTree)
{
    ...
    _hashTable._buckets
        = (HashTableEntry**)trMemory()->allocateStackMemory(_hashTable._numBuckets * sizeof(HashTableEntry*));
    ...
    _hashTableWithSyms._buckets
        = (HashTableEntry**)trMemory()->allocateStackMemory(_hashTableWithSyms._numBuckets * sizeof(HashTableEntry*));
    ...
    _hashTableWithCalls._buckets
        = (HashTableEntry**)trMemory()->allocateStackMemory(_hashTableWithCalls._numBuckets * sizeof(HashTableEntry*));
    ...
    _hashTableWithConsts._buckets
        = (HashTableEntry**)trMemory()->allocateStackMemory(_hashTableWithConsts._numBuckets * sizeof(HashTableEntry*));
...
void OMR::LocalCSE::examineNode(TR::Node* node, SharedSparseBitVector& seenAvailableLoadedSymbolReferences,
    TR::Node* parent, int32_t childNum, int32_t* nextLoadIndex, bool* parentCanBeAvailable, int32_t depth)
{
    if (depth > MAX_DEPTH) {
    ...
    if (symRef && node->getOpCode().isLoadVar()) {
        ...
        if (i && _storeNodes.Exists(i)
            ...
            doCopyPropagationIfPossible(
                node, parent, childNum, _storeNodes[i], symRef, visitCount, doneCopyPropagation);
            ...
            for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
                if (_storeNodes[nc]->getOpCode().isStoreIndirect()
                    ...
                    if (doCopyPropagationIfPossible(
                            node, parent, childNum, _storeNodes[nc], symRef, visitCount, doneCopyPropagation))
                        break;
                }
            }
        }
    ...
    TR::SymbolReference* symRef, vcount_t visitCount, bool& doneCopyPropagation)
{
    ...
    if ((!comp()->getOption(TR_MimicInterpreterFrameShape) || !comp()->areSlotsSharedByRefAndNonRef()
        ...
        && performTransformation(comp(), "%s   Local Common Subexpression Elimination propagating local #%d in node : "
...
void OMR::LocalCSE::makeNodeAvailableForCommoning(
    TR::Node* parent, TR::Node* node, SharedSparseBitVector& seenAvailableLoadedSymbolReferences, bool* canBeAvailable)
{
                ...
                if (rhsOfStoreDefNode->getType().isBCD()) {
                    if (defNode && defNode->getDataType() == TR::PackedDecimal && defNode->getOpCode().isStore()
                        ...
                        TR_ASSERT(rhsOfStoreDefNode->getDataType() == TR::PackedDecimal,
                            "rhsOfStoreDefNode %p (type %d) must be pd type if defNode %p is pd\n", rhsOfStoreDefNode,
                            rhsOfStoreDefNode->getDataType(), defNode);
                    ...
                    if (rhsOfStoreDefNode->getDecimalPrecision() != nodePrecision) {
                        ...
                        // changing pdshl must be prepended in case
...
//
bool OMR::LocalCSE::killExpressionsIfVolatileLoad(TR::Node* node,
    SharedSparseBitVector& seenAvailableLoadedSymbolReferences, TR_NodeKillAliasSetInterface& UseDefAliases)
{
...
void OMR::LocalCSE::killAvailableExpressionsAtGCSafePoints(
    TR::Node* node, TR::Node* parent, SharedSparseBitVector& seenAvailableLoadedSymbolReferences)
{
        ...
        if (node->getOpCode().hasSymbolReference() && ((node->getOpCodeValue() != TR::loadaddr) || _loadaddrAsLoad)) {
            if (node->getOpCode().isCall()) {
                ...
                TR_ASSERT(hashValue >= 0 && hashValue < _hashTableWithCalls._numBuckets,
                    "LocalCSE hash problem - hashValue %d on node %p %s outside of _hashTableWithCalls size %d\n",
                ...
                TR_ASSERT(hashValue >= 0 && hashValue < _hashTableWithSyms._numBuckets,
                    "LocalCSE hash problem - hashValue %d on node %p %s outside of _hashTableWithSyms size %d\n",
            ...
            TR_ASSERT(hashValue >= 0 && hashValue < _hashTableWithConsts._numBuckets,
                "LocalCSE hash problem - hashValue %d on node %p %s outside of _hashTableWithConsts size %d\n",
            ...
            TR_ASSERT(hashValue >= 0 && hashValue < _hashTable._numBuckets,
                "LocalCSE hash problem - hashValue %d on node %p %s outside of _hashTable size %d\n", hashValue, node,
                node->getOpCode().getName(), _hashTable._numBuckets);
        }
    }
