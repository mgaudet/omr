...
void OMR::LocalCSE::examineNode(TR::Node* node, SharedSparseBitVector& seenAvailableLoadedSymbolReferences, TR::Node* parent, int32_t childNum, int32_t* nextLoadIndex, bool* parentCanBeAvailable, int32_t depth)
{
    ...
    if (symRef && node->getOpCode().isLoadVar()) {
        ...
        if (i && _storeNodes.Exists(i) && _storeNodes[i]->getSymbolReference()->getReferenceNumber() == symRef->getReferenceNumber()) {
            ...
            for (nc.SetToFirst(); nc.Valid(); nc.SetToNext()) {
                if (_storeNodes[nc]->getOpCode().isStoreIndirect() && _storeNodes[nc]->getSymbolReference()->getSymbol()->getSize() == symRef->getSymbol()->getSize() && _storeNodes[nc]->getSymbolReference()->getSymbol()->getDataType() == symRef->getSymbol()->getDataType() && allowNodeTypes(_storeNodes[nc], node) && _storeNodes[nc]->getSymbolReference()->getOffset() == symRef->getOffset()
#ifdef J9_PROJECT_SPECIFIC
                    && (!_storeNodes[nc]->getType().isBCD() || _storeNodes[nc]->getDecimalPrecision() == node->getDecimalPrecision())
#endif
                        ) {
    ...
    // to be available.
    //
    if (!doneCopyPropagation && (((node->getOpCodeValue() == TR::NULLCHK) && (isAvailableNullCheck(node, seenAvailableLoadedSymbolReferences))) || (canBeAvailable(parent, node, seenAvailableLoadedSymbolReferences, nodeCanBeAvailable))
    ...
    if (node->getOpCode().isStore() || hasAliases) {
        ...
        //
        if (symRef && symRef->getSymbol()->isAuto() && _availablePinningArrayExprs.ValueAt(symRef->getReferenceNumber())) {
...
bool OMR::LocalCSE::canBeAvailable(TR::Node* parent, TR::Node* node, SharedSparseBitVector& seenAvailableLoadedSymbolReferences, bool canBeAvailable)
{
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        //
        if ((!seenAvailableLoadedSymbolReferences.ValueAt(node->getSymbolReference()->getReferenceNumber())) || ((_volatileState == VOLATILE_ONLY) && !node->getSymbol()->isAutoOrParm() && !node->getSymbol()->isVolatile() && !node->getSymbol()->isFinal()))
            return false;
...
TR::Node* OMR::LocalCSE::getAvailableExpression(TR::Node* parent, TR::Node* node)
{
    ...
    if ((node->getOpCode().isArrayRef()) && cg()->supportsInternalPointers() && (node->getFirstChild()->getOpCodeValue() == TR::aload) && (node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto()) && !_killedPinningArrayExprs[node->getFirstChild()->getSymbolReference()->getReferenceNumber()]) {
...
void OMR::LocalCSE::addToHashTable(TR::Node* node, int32_t hashValue)
{
    ...
    if (node->getOpCode().hasSymbolReference() && !_relevantNodes.ValueAt(node->getSymbolReference()->getReferenceNumber()))
        return;
