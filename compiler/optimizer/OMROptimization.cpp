...
const char* OMR::Optimization::optDetailString() { return self()->manager()->optDetailString(); }
    ...
    TR::Node* node, TR::Node* child, TR::TreeTop* anchorTree, TR::Block* block, bool correctBCDPrecision)
{
    ...
    if (correctBCDPrecision && node->getType().isBCD() && child->getType().isBCD()
        ...
        if (node->getReferenceCount() > 1) {
            ...
            dumpOptDetails(self()->comp(), "%sPrecision mismatch when replacing parent %s [" POINTER_PRINTF_FORMAT
