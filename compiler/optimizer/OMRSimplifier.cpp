    ...
    for (; treeTop; treeTop = treeTop->getNextTreeTop()) {
        ...
        if (_performLowerTreeSimplifier) {
            _performLowerTreeNode = postWalkLowerTreeSimplifier(
                _performLowerTreeSimplifier, _performLowerTreeNode, block, (TR::Simplifier*)this);
...
TR::Node* OMR::Simplifier::simplify(TR::Node* node, TR::Block* block)
{
    ...
    if ((node != newNode) || (newNode && ((newNode->getOpCodeValue() != node->getOpCodeValue())
    ...
    TR::Node* node, TR::Node* firstChild, TR::TreeTop* anchorTree, TR::ILOpCodes opcode, bool anchorChildren)
{
    ...
    if (firstChild->getOpCodeValue() == opcode && node->getType().isBCD() && firstChild->getType().isBCD()
        ...
        //   l2dd
        //     lX
        // Folding could give an incorrect result because the max precision of a dd is 16 and the max precision of an l
        // is 19
    ...
    if (firstChild->getOpCodeValue() == opcode && (node->getType().isAggregate() || firstChild->getType().isAggregate())
        ...
        // to be zero
        // (i.e. there really isn't an intermediate truncation of 4->3 even though it appears that way from looking at
        // the sizes alone)
        // o2i
        //   i2o size=3
        //     iushr
        ...
        if (node->getType().isIntegral() && nodeSize == grandChild->getSize() && nodeSize > firstChild->getSize()) {
            ...
            if (grandChild->getOpCode().isRightShift() && grandChild->getOpCode().isShiftLogical()
                ...
                disallow = false;
                if (trace())
                    traceMsg(comp(), "do allow unaryCancel of node %s (%p) and firstChild %s (%p) as grandChild %s "
                                     "(%p) zeros the %d truncated bytes\n",
        ...
        if (disallow) {
            if (trace())
                traceMsg(comp(), "disallow unaryCancel of node %s (%p) and firstChild %s (%p) due to unequal sizes "
                                 "(nodeSize %d, firstChildSize %d, firstChild->childSize %d)\n",
        ...
        if (node->getType().isBCD() && grandChild->getType().isBCD()
            ...
            if (node->getDecimalPrecision() != grandChild->getDecimalPrecision()) {
                ...
                grandChild = TR::Node::create(
                    TR::ILOpCode::modifyPrecisionOpCode(grandChild->getDataType()), 1, origGrandChild);
            ...
            // if the top level was always setting a particular sign code (e.g. ud2pd) then must maintain this
            // side-effect here when cancelling
            ...
            grandChild->getFirstChild()
                ->decReferenceCount(); // inc'd an extra time when creating modPrecision node above
            grandChild->incReferenceCount();
            dumpOptDetails(comp(), "%sCreate %s [" POINTER_PRINTF_FORMAT
                                   "] 0x%llx to account for %d truncated bytes between node %s [" POINTER_PRINTF_FORMAT
                                   "] grandChild %s [" POINTER_PRINTF_FORMAT "]\n",
                optDetailString(), grandChild->getOpCode().getName(), grandChild, (uint64_t)constantForAnd,
                bytesLeftAfterTruncation, node->getOpCode().getName(), node,
                grandChild->getFirstChild()->getOpCode().getName(), grandChild->getFirstChild());
        }
