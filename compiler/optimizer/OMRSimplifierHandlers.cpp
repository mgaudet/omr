    ...
    if (s->reassociate() && (node->getOpCode().isAdd() || node->getOpCode().isSub()) && node->getFirstChild()->getReferenceCount() > 1 && node->getSecondChild()->getOpCode().isLoadConst() && s->comp()->cg()->isMaterialized(node->getSecondChild())) {
        ...
        if (!s->_hashTable.locate(node->getFirstChild()->getGlobalIndex(), index)) {
            ...
            if (base_node != node && base_node->getReferenceCount() >= 1 && (base_node->getOpCodeValue() == node->getOpCodeValue()) && base_node->getFirstChild() == node->getFirstChild() &&
                ...
                if (!s->comp()->cg()->isMaterialized(diff) && performTransformation(s->comp(), "%sReusing big constant from node 0x%p in node 0x%p\n", s->optDetailString(), base_node, node)) {
...
static TR::Node* foldDemotionConversion(TR::Node* node, TR::ILOpCodes opcode, TR::ILOpCodes foldedOpCode, TR::Simplifier* s)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (node->getOpCode().isConversionWithFraction() && firstChild->getOpCode().isConversionWithFraction() && node->getDecimalFraction() != firstChild->getDecimalFraction()) {
    ...
    if (node->getOpCode().isConversionWithFraction() && !firstChild->getOpCode().isConversionWithFraction() && node->getDecimalFraction() != 0) {
        // illegal to fold a pattern like:
        // pd2f  frac=5
        //    i2pd
        //       i
...
static void partialRedundantCompareElimination(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (comp->cg()->getGRACompleted() || comp->cg()->getLimitedGRACompleted() || comp->cg()->getGRAForShortLoopsCompleted())
        return;
...
{
#ifdef J9_PROJECT_SPECIFIC
    if (child->getOpCodeValue() == TR::imul && child->getSecondChild()->getOpCodeValue() == TR::iconst && child->getSecondChild()->getInt() == mulConst && child->getFirstChild()->getOpCodeValue() == b2iOpCode && child->getFirstChild()->getReferenceCount() == 1 && child->getFirstChild()->getFirstChild()->getOpCodeValue() == TR::bloadi && child->getFirstChild()->getFirstChild()->getReferenceCount() == 1 && child->getFirstChild()->getFirstChild()->getSymbolReference() == s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int8)) {
...
{
#ifdef J9_PROJECT_SPECIFIC
    if (child->getOpCodeValue() == TR::bu2i && child->getReferenceCount() == 1 && child->getFirstChild()->getOpCodeValue() == TR::bloadi && child->getFirstChild()->getReferenceCount() == 1 && child->getFirstChild()->getSymbolReference() == s->getSymRefTab()->findOrCreateUnsafeSymbolRef(TR::Int8)) {
...
{
    if (node->getOpCodeValue() == TR::isub && node->getReferenceCount() == 1 && node->getSecondChild()->getOpCodeValue() == TR::iconst && node->getSecondChild()->getInt() == isubConst) {
...
static TR::Node* isOrOfTwoConsecutiveBytes(TR::Node* ior, TR::Simplifier* s)
{
    ...
    if ((byte1 = isUnsafeIorByteChild(ior->getFirstChild(), TR::b2i, 256, s)) && (byte2 = isLastUnsafeIorByteChild(ior->getSecondChild(), s))) {
    ...
    TR::Node* bbStartNode, TR::list<TR::CFGEdge*>& inEdge, TR::Simplifier* s, bool& blockIsEmpty)
{
    ...
    if ((block->isGenControlBlock() && !nextBlock->isGenControlBlock()) || (block->isGenControlBlock() && !nextBlock->isSimpleGenControlBlock()) || (!block->isGenControlBlock() && nextBlock->isGenControlBlock()) || (block->isGenControlBlock() && block->getPredecessors().empty() && block->getExceptionPredecessors().empty()))
        return false;
    ...
    if (cfg &&
        /* cfg->getStructure() && */
        !s->comp()->cg()->getGRACompleted() && ((block->getStructureOf() && block->getStructureOf()->isLoopInvariantBlock()) || (nextBlock->getStructureOf() && nextBlock->getStructureOf()->isLoopInvariantBlock())))
        return false;
...
{
#ifdef J9_PROJECT_SPECIFIC
    return (symbol && (symbol->getRecognizedMethod() == TR::java_lang_Math_pow || symbol->getRecognizedMethod() == TR::java_lang_StrictMath_pow));
#else
    return false;
#endif
}
...
{
#ifdef J9_PROJECT_SPECIFIC
    return (symbol && ((symbol->getRecognizedMethod() == TR::java_lang_Math_abs_D) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_F) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_I)));
#else
    return false;
#endif
}
...
static TR::Node* foldAbs(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (childNode && (childNode->isNonNegative() || (node->getReferenceCount() == 1)) && performTransformation(s->comp(), "%sFolded abs for postive argument on node [%p]\n", s->optDetailString(), node)) {
...
static bool convertToSinglePrecisionSQRT(TR::Simplifier* s, TR::Node* sqrtCall)
{
    ...
    if (!methodSymbol || (methodSymbol->getRecognizedMethod() != TR::java_lang_Math_sqrt && methodSymbol->getRecognizedMethod() != TR::java_lang_StrictMath_sqrt)) {
        return false;
    }
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (nodeOp == TR::aloadi || nodeOp == TR::iloadi || nodeOp == TR::lloadi) {
        ...
        if (nodeSymref == s->getSymRefTab()->findClassAndDepthFlagsSymbolRef()
            ...
            TR::Node* konstNode = nodeOp == TR::iloadi ? TR::Node::iconst(node, konst) : nodeOp == TR::lloadi ? TR::Node::lconst(node, konst) : NULL;
    ...
    if (
        firstChild->getOpCodeValue() == TR::loadaddr && !s->comp()->cg()->getLinkage()->isArgumentListSymbol(firstChild->getSymbolReference()->getSymbol(), s->comp())) // argList symbol can grow later on
    {
        bool newOType = false;
        ...
        if (isSame && (node->getSymbol()->getSize() == firstChild->getSymbol()->getSize()) && (firstChild->getSymbolReference()->getSymbol()->isAutoOrParm() || localStatic) && node->getSymbolReference()->getOffset() == 0 && (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile()) && indirectToDirectConvBeneficial(node->getSymbolReference(), firstChild->getSymbolReference(), s) && performTransformation(s->comp(), "%sReplace indirect load %s [" POINTER_PRINTF_FORMAT "] with ",
                                                                                                                                                                                                                                                                                                                                                                                                                     s->optDetailString(), node->getOpCode().getName(), node)) {
...
TR::Node* indirectStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
        ...
        firstChild->getOpCodeValue() == TR::loadaddr && !s->comp()->cg()->getLinkage()->isArgumentListSymbol(firstChild->getSymbolReference()->getSymbol(), s->comp())) // argList symbol can grow later on
    {
        ...
        if ((storeDataType == addrDataType && isSameType) && (firstChild->getSymbolReference()->getSymbol()->isAutoOrParm() || localStatic)
            && node->getSymbolReference()->getOffset() == 0 && (node->getSymbol()->isVolatile() == firstChild->getSymbol()->isVolatile()) && indirectToDirectConvBeneficial(node->getSymbolReference(), firstChild->getSymbolReference(), s) && performTransformation(s->comp(), "%sReplace indirect store %s [" POINTER_PRINTF_FORMAT "] with ",
                                                                                                                                                                                                                                                    s->optDetailString(), node->getOpCode().getName(), node)) {
    ...
    if ((node->getOpCodeValue() == TR::wrtbari) && 0 && !node->getSymbolReference()->getSymbol()->isArrayShadowSymbol()) {
...
TR::Node* ifdCallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (callSymbolIsRecognizedAbsMethod(symbol)) {
        node = foldAbs(node, s);
    } else if (node->getSymbol() && callSymbolIsRecognizedPowMethod(node->getSymbol()->getResolvedMethodSymbol())) {
        ...
        if (callSymbolIsRecognizedPowMethod(node->getSymbol()->getResolvedMethodSymbol())) {
            if (valueNode->getOpCodeValue() == TR::dconst && expNode->getOpCodeValue() == TR::dconst && (valueNode->getDouble() == 10.0 && expNode->getDouble() == 4.0)) {
...
TR::Node* lcallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (s->comp()->cg()->getSupportsCurrentTimeMaxPrecision()) {
        ...
        if (methodSymbol) {
            if ((methodSymbol->getRecognizedMethod() == TR::java_lang_System_currentTimeMillis) && (methodSymbol->isJNI() || methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative()) && performTransformation(s->comp(), "%sConvert currentTimeMillis to currentTimeMaxPrecision with divide of" INT64_PRINTF_FORMAT " on node [%p]\n", s->optDetailString(), divisor, node)) {
                ...
                s->_alteredBlock = true;
            } else if (methodSymbol->getRecognizedMethod() == TR::java_lang_System_nanoTime && (methodSymbol->isJNI() || methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative()) && performTransformation(s->comp(), "%sConvert nanoTime to currentTimeMaxPrecision with multiply of " INT64_PRINTF_FORMAT " on node [%p]\n", s->optDetailString(), multiplier, node)) {
                TR::Node* lcallNode = TR::Node::createWithSymRef(node, TR::lcall, 0, s->comp()->getSymRefTab()->findOrCreateCurrentTimeMaxPrecisionSymbol());
                TR::Node* tempResult = TR::Node::create(TR::treetop, 1, lcallNode);
                TR::TreeTop* callTreeTop = findTreeTop(node, block);
                TR_ASSERT(callTreeTop != NULL, "should have been able to find this tree top");
        ...
        if (methodSymbol && (methodSymbol->getRecognizedMethod() == TR::java_lang_System_currentTimeMillis) && (methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative())) {
            ...
            for (tt = block->getEntry(); tt != exitTree; tt = tt->getNextRealTreeTop()) {
                if (((tt->getNode()->getOpCodeValue() == TR::treetop) || (tt->getNode()->getOpCodeValue() == TR::ResolveCHK)) && tt->getNode()->getFirstChild() == node)
                    break;
            }
            ...
            if (tt != exitTree) // found the call tree
            {
                ...
                if ((opCode == TR::lstorei || opCode == TR::lstore) && nextNode->getOpCode().hasSymbolReference() && !nextNode->mightHaveVolatileSymbolReference()) {
                    TR::Node* valueChild;
                    ...
                    if (node == valueChild && performTransformation(s->comp(), "%sFolded long store of currentTimeMillis to use address of destination as argument on node [%p]\n", s->optDetailString(), node)) {
                        node->setNumChildren(1);
...
#ifdef J9_PROJECT_SPECIFIC
    if ((node->getOpCode().isCallDirect()) && !node->getSymbolReference()->isUnresolved() && (node->getSymbol()->isResolvedMethod()) && ((node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigDecimal_valueOf) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigDecimal_add) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigDecimal_subtract) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigDecimal_multiply) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigInteger_add) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigInteger_subtract) || (node->getSymbol()->getResolvedMethodSymbol()->getRecognizedMethod() == TR::java_math_BigInteger_multiply)) && (node->getReferenceCount() == 1) && performTransformation(s->comp(), "%sRemoved dead BigDecimal/BigInteger call node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
        TR::Node* firstChild = node->getFirstChild();
        s->anchorChildren(node, s->_curTree);
...
TR::Node* iabsSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (child->isNonNegative() && performTransformation(s->comp(), "%sSimplify iabs of non-negative child at [" POINTER_PRINTF_FORMAT "]\n",
                                      s->optDetailString(), node)) {
...
TR::Node* labsSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    simplifyChildren(node, block, s);
    auto child = node->getChild(0);
    if (child->isNonNegative() && performTransformation(s->comp(), "%sSimplify labs of non-negative child at [" POINTER_PRINTF_FORMAT "]\n",
                                      s->optDetailString(), node)) {
        return s->replaceNodeWithChild(node, child, s->_curTree, block);
    }
    return node;
...
TR::Node* iorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getReferenceCount() == 1) {
        ...
        // look for reassociation to fold constants
            ...
            if (lrChild->getOpCodeValue() == TR::iconst) {
                ...
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::ior && firstChild->getReferenceCount() == 1 && (byte1 = isUnsafeIorByteChild(firstChild->getSecondChild()->getFirstChild(), TR::bu2i, 16777216, s)) && (byte2 = isUnsafeIorByteChild(firstChild->getSecondChild()->getSecondChild(), TR::bu2i, 65536, s)) && (byte3 = isUnsafeIorByteChild(firstChild->getFirstChild(), TR::bu2i, 256, s)) && (byte4 = isLastUnsafeIorByteChild(node->getSecondChild(), s))) {
                    if (TR::Compiler->target.cpu.isLittleEndian()) {
                        temp = byte1, byte1 = byte4, byte4 = temp;
                        temp = byte2, byte2 = byte3, byte3 = temp;
                    }

                    if ((addr = getUnsafeBaseAddr(byte2, -1)) && addr == byte1 && (addr = getUnsafeBaseAddr(byte3, -2)) && addr == byte1 && (addr = getUnsafeBaseAddr(byte4, -3)) && addr == byte1 && performTransformation(s->comp(), "%sconvert ior to iiload node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
    ...
    firstChild = node->getFirstChild();
    secondChild = node->getSecondChild();
    if (firstChild->getOpCodeValue() == TR::l2i && secondChild->getOpCodeValue() == TR::l2i && firstChild->getFirstChild()->getOpCodeValue() == TR::lshr && secondChild->getFirstChild()->getOpCodeValue() == TR::lushr) {
        TR::Node* firstGrandChild = firstChild->getFirstChild();
        TR::Node* secondGrandChild = secondChild->getFirstChild();
        if (secondGrandChild->getFirstChild()->getOpCodeValue() == TR::lneg && firstGrandChild->getSecondChild()->getOpCodeValue() == TR::iconst && firstGrandChild->getSecondChild()->getInt() == 63 && secondGrandChild->getSecondChild()->getOpCodeValue() == TR::iconst && secondGrandChild->getSecondChild()->getInt() == 63) {
            TR::Node* loadVal = firstGrandChild->getFirstChild();
            if (secondGrandChild->getFirstChild()->getFirstChild() == loadVal && (loadVal->getOpCode().isLoadVar() || loadVal->getOpCode().isLoadReg()) && performTransformation(s->comp(), "%sTransform ior to lcmp [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
#ifdef J9_PROJECT_SPECIFIC
    TR::Node* address;
    if (firstChild->getOpCodeValue() == TR::ior && firstChild->getReferenceCount() == 1 && (address = isOrOfTwoConsecutiveBytes(firstChild, s)) && performTransformation(s->comp(), "%sconvert ior to isload node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* ificmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isBooleanCompare() && !firstChild->getOpCode().isLongCompare() && (secondChild->getOpCode().isLoadConst()) && ((secondChild->getInt() == 0) || (secondChild->getInt() == 1)) && (firstChild->getOpCode().convertCmpToIfCmp() != TR::BadILOp) && (s->comp()->cg()->getSupportsJavaFloatSemantics() || !(firstChild->getNumChildren() > 1 && firstChild->getFirstChild()->getOpCode().isFloatingPoint())) && performTransformation(s->comp(), "%sChanging if opcode %p because first child %p is a comparison opcode\n", s->optDetailString(), node, firstChild)) {
...
static bool boundGELength(TR::Node* boundChild, TR::Node* lengthChild, TR::Simplifier* s)
{
    ...
    if (boundOp == TR::iadd) {
        ...
        if (first->getOpCodeValue() == TR::aloadi && lengthChild->getOpCodeValue() == TR::iloadi && first->getFirstChild() == lengthChild->getFirstChild()) {
            TR::SymbolReference* boundSymRef = first->getSymbolReference();
            TR::SymbolReference* lengthSymRef = lengthChild->getSymbolReference();
            if ((boundSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_String_value && lengthSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_String_count) || (boundSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_StringBuffer_value && lengthSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_StringBuffer_count) || (boundSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_StringBuilder_value && lengthSymRef->getSymbol()->getRecognizedField() == TR::Symbol::Java_lang_StringBuilder_count)) {
                return true;
            }
        }
    }
    ...
    if (rhsChild->getOpCodeValue() == TR::iadd) {
        ...
        if (lengthChild == boundChild || boundGELength(boundChild, lengthChild, s)) {
            ...
            if (indexChild->getOpCodeValue() == TR::isub) {
                ...
                } else if (indexChild->getFirstChild()->getOpCodeValue() == TR::iadd && indexChild->getSecondChild()->getOpCodeValue() == TR::iconst && indexChild->getSecondChild()->getInt() == -1) {
                    TR::ILOpCodes boundOp = boundChild->getOpCodeValue();
                    TR::Node* boundReference = NULL;
                    if (boundChild->getOpCode().isArrayLength() && boundChild->getFirstChild()->getOpCodeValue() == TR::aloadi) {
...
TR::Node* bndchkwithspinechkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (boundChild->getOpCode().isLoadConst() && !s->comp()->generateArraylets()) {
        ...
        if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(boundChild->getInt(), elementSize) && performTransformation(s->comp(),
...
TR::Node* arrayLengthSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::contigarraylength) {
        if (firstChildOp == TR::newarray || firstChildOp == TR::anewarray) {
            ...
            if (arraySize->getOpCode().isLoadConst() && elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(arraySize->getInt(), elementSize) && performTransformation(s->comp(),
