...
template <typename T> static void foldConstant(TR::Node* node, T value, TR::Simplifier* s, bool anchorChildrenP)
{
...
static bool reduceLongOp(TR::Node* node, TR::Block* block, TR::Simplifier* s, TR::ILOpCodes newConversionOp)
{
    ...
    if (newConversionOp == TR::BadILOp) {
        ...
        node->setAndIncChild(1, convertSecondChild ? TR::Node::create(TR::l2i, 1, firstChild->getSecondChild())
        ...
        if ((firstOp == TR::i2l) && performTransformation(s->comp(), "%sLong compare narrower for node [%p]\n",
            ...
            if (secondOp == TR::iconst || secondOp == TR::i2l
                ...
                if (secondOp == TR::i2l) {
                    ...
                    if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 i2l and child 2 lconst in "
                                                  "iconst range in method %s\n",
                            s->comp()->signature());
                    }
            ...
            if (firstOp == TR::su2l) {
                if (secondOp == TR::cconst || secondOp == TR::su2l
                    ...
                    if (secondOp == TR::su2l) {
                        ...
                        if (reportCompareDemotions) {
                            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 c2l and child 2 lconst in "
                                                      "cconst range in method %s\n",
                                s->comp()->signature());
                        }
                    } else if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(),
                ...
                if (secondOp == TR::sconst || secondOp == TR::s2l
                    ...
                    if (secondOp == TR::s2l) {
                        ...
                        if (reportCompareDemotions) {
                            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 s2l and child 2 lconst in "
                                                      "sconst range in method %s\n",
                                s->comp()->signature());
                        }
                    } else if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(),
                ...
                if (secondOp == TR::bconst || secondOp == TR::b2l
                    ...
                    if (secondOp == TR::b2l) {
                        ...
                        if (reportCompareDemotions) {
                            dumpOptDetails(s->comp(), "Long Compare Narrower: found child 1 b2l and child 2 lconst in "
                                                      "bconst range in method %s\n",
                                s->comp()->signature());
                        }
                    } else if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(),
...
static bool indirectToDirectConvBeneficial(
    TR::SymbolReference* indirectSymRef, TR::SymbolReference* directSymRef, TR::Simplifier* s)
{
    return true;
}
...
TR::Node* getQuotientUsingMagicNumberMultiply(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->getOpCodeValue() == TR::idiv || node->getOpCodeValue() == TR::irem) {
        ...
        TR_ASSERT(0, "getQuotientUsingMagicNumberMultiply called with incorrect opcode in node %p, need "
    ...
    TR::Node* bbStartNode, TR::list<TR::CFGEdge*>& inEdge, TR::Simplifier* s, bool& blockIsEmpty)
{
    ...
    if ((block->isGenControlBlock() && !nextBlock->isGenControlBlock())
        ...
        || (block->isGenControlBlock() && block->getPredecessors().empty()
...
static void changeBranchDestinationsForMergeBlocks(TR::Block* block, TR::Block* nextBlock, TR::Node* bbStartNode,
    ...
    std::list<TR::CFGEdge*, TR::typed_allocator<TR::CFGEdge*, TR::Allocator> >::iterator& inEdgeIter, TR::Simplifier* s)
{
...
{
#ifdef J9_PROJECT_SPECIFIC
    return (symbol && (symbol->getRecognizedMethod() == TR::java_lang_Math_pow
...
{
#ifdef J9_PROJECT_SPECIFIC
    return (symbol && ((symbol->getRecognizedMethod() == TR::java_lang_Math_abs_D)
...
static TR::Node* foldAbs(TR::Node* node, TR::Simplifier* s)
{
    ...
    if (childNode && (childNode->isNonNegative() || (node->getReferenceCount() == 1))
        && performTransformation(
               s->comp(), "%sFolded abs for postive argument on node [%p]\n", s->optDetailString(), node)) {
        TR::TreeTop::create(s->comp(), s->_curTree->getPrevTreeTop(), TR::Node::create(TR::treetop, 1, childNode));
        ...
        if (firstOp == TR::su2i && firstChild->getReferenceCount() == 1) {
            if (secondOp == TR::su2i
                ...
                if (secondOp == TR::su2i) {
                    ...
                    if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 c2i and child 2 iconst in "
                                                  "cconst range in method %s\n",
                            s->comp()->signature());
                    }
                } else if (reportCompareDemotions) {
            ...
            if (secondOp == TR::s2i
                ...
                if (secondOp == TR::s2i) {
                    ...
                    if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 s2i and child 2 iconst in "
                                                  "sconst range in method %s\n",
                            s->comp()->signature());
                    }
                } else if (reportCompareDemotions) {
            ...
            if (secondOp == TR::b2i || (secondOp == TR::iconst && secondChild->getInt() >= SCHAR_MIN
                ...
                if (secondOp == TR::b2i) {
                    ...
                    if (reportCompareDemotions) {
                        dumpOptDetails(s->comp(), "Integer Compare Narrower: found child 1 b2i and child 2 iconst in "
                                                  "bconst range in method %s\n",
                            s->comp()->signature());
                    }
                } else if (reportCompareDemotions) {
...
template <typename T> static bool checkAndReplaceRotation(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    TR_ASSERT(node->getOpCode().isOr() || node->getOpCode().isXor(),
        "Node %p being checked for rotation pattern is not an (x)or!\n", node); // I believe this is general enough to
                                                                                // allow other opcodes, like add's,
                                                                                // xor's. But just want to be safe for
                                                                                // now.
...
static bool convertToSinglePrecisionSQRT(TR::Simplifier* s, TR::Node* sqrtCall)
{
    ...
    if (!methodSymbol || (methodSymbol->getRecognizedMethod() != TR::java_lang_Math_sqrt
    ...
    TR_ASSERT(callTreeTop != NULL, "should have been able to find this tree top (call %p) in block %d", sqrtCall,
        s->_curTree->getEnclosingBlock()->getNumber());
...
TR::Node* indirectLoadSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
        ...
        if (childOp == TR::aloadi || childOp == TR::iloadi || childOp == TR::lloadi) {
...
#ifdef J9_PROJECT_SPECIFIC
            if (symRefPairMatches(nodeSymref, childSymref, s->getSymRefTab()->findClassFromJavaLangClassSymbolRef(),
            ...
            if (fieldsAreComplementary
                && performTransformation(s->comp(), "%sFolded complementary field load [%p]->%s->%s\n",
                       s->optDetailString(), grandchild, nodeSymref->getName(s->getDebug()),
...
#ifdef J9_PROJECT_SPECIFIC
        if (nodeSymref == s->getSymRefTab()->findClassAndDepthFlagsSymbolRef() && somePackedOp == TR::loadaddr
            ...
            TR_OpaqueClassBlock* clazz
                = (TR_OpaqueClassBlock*)childSymref->getSymbol()->getStaticSymbol()->getStaticAddress();
    ...
    if (firstChild->getOpCodeValue() == TR::loadaddr
        && !s->comp()->cg()->getLinkage()->isArgumentListSymbol(
               firstChild->getSymbolReference()->getSymbol(), s->comp())) // argList symbol can grow later on
    {
        bool newOType = false;
    ...
    if ((node->getOpCodeValue() == TR::wrtbari) && 0
        ...
        if ((valueChild->getOpCodeValue() == TR::New) || (valueChild->getOpCodeValue() == TR::newarray)
            ...
            if (removeWrtBar && !s->comp()->getOptions()->realTimeGC()
                && performTransformation(s->comp(), "%sFolded indirect write barrier to iastore because GC could not "
...
TR::Node* directStoreSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // NOTE: this likely won't improve register pressure; it's purely a codegen quality improvement
        ...
        if (firstLoadsSymRef && secondIsLoadConst || firstIsLoadConst && secondLoadsSymRef) {
            ...
            // of block
            // remember tree where last use of node was on this walk: insert our update tree following that last use
            // tree
            ...
            if (loadTree->getNode()->getOpCodeValue() == TR::treetop) {
                ...
                if (loadNode->getOpCode().isLoadVarDirect() && loadNode->getSymbolReference() == symRef) {
                    ...
                    if (insertionTree != NULL
                        ...
                        // subsequently try to undo it
                        // by moving the load below the update tree (bad pass!).  We'll allow it to do that (because we
                        // can come along and re-swing
                        // the update below the load) so long as it doesn't move the load under a branch tree.  If the
                        // load gets moved into a branch
                        // tree, then there's no place we could put the update tree that's below the load but still in
                        // the block
                        // (executive summary: all our careful work above would be for naught).  So mark the load here
                        // so that
...
TR::Node* ifdCallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (callSymbolIsRecognizedAbsMethod(symbol)) {
        ...
        // In strictmath.pow(), if both arguments are integers, then the result is exactly equal to the mathematical
        // result
        // of raising the first argument to the power of the second argument if that result can in fact be represented
        // exactly as a double value.
        //(In the foregoing descriptions, a floating-point value is considered to be an integer if and only if it is
        //finite and a fixed point of
        // the method ceil or, equivalently, a fixed point of the method floor. A value is a fixed point of a
        // one-argument method if and only if
...
TR::Node* lcallSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (s->comp()->cg()->getSupportsCurrentTimeMaxPrecision()) {
        ...
        if (methodSymbol) {
            if ((methodSymbol->getRecognizedMethod() == TR::java_lang_System_currentTimeMillis)
                ...
                TR::Node* lcallNode = TR::Node::createWithSymRef(
                    node, TR::lcall, 0, s->comp()->getSymRefTab()->findOrCreateCurrentTimeMaxPrecisionSymbol());
                ...
                && (methodSymbol->isJNI() || methodSymbol->isVMInternalNative() || methodSymbol->isJITInternalNative())
                && performTransformation(s->comp(),
                           "%sConvert nanoTime to currentTimeMaxPrecision with multiply of " INT64_PRINTF_FORMAT
                           " on node [%p]\n",
        ...
        if (methodSymbol && (methodSymbol->getRecognizedMethod() == TR::java_lang_System_currentTimeMillis)
            ...
            if (tt != exitTree) // found the call tree
            {
                ...
                if ((opCode == TR::lstorei || opCode == TR::lstore) && nextNode->getOpCode().hasSymbolReference()
                    ...
                    if (node == valueChild
                        && performTransformation(s->comp(), "%sFolded long store of currentTimeMillis to use address "
                                                            "of destination as argument on node [%p]\n",
                               s->optDetailString(), node)) {
                        node->setNumChildren(1);
                        ...
                        if (addressChild) {
                            if (TR::Compiler->target.is64Bit()) {
                                ...
                                storeAddressNode
                                    = TR::Node::create(TR::aiadd, 2, addressChild,
...
TR::Node* iaddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // reduce imul operations by factoring
        ...
        TR::Node* lrChild = firstChild->getSecondChild();
        if ((lrChild->getOpCodeValue() == TR::iconst)
            && ((firstChild->getFutureUseCount() == 0) || (!s->comp()->cg()->areAssignableGPRsScarce()
        ...
        // [0x2BB38260] loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=2 flg=0x80
        // n1.3081n   (  0)        a2i (in *GPR_6512) (profilingCode unneededConv )
        // [0x2BB381C0] loc=[-1,2847,1153] rc=0 vc=631 vn=- sti=- udi=14723 nc=1 addr=4 flg=0x8080
        // n1.26322n  (  0)          aiadd (in *GPR_6512) (profilingCode X>=0 internalPtr )
        // [0x374931DC] loc=[-1,2844,1153] rc=0 vc=1859 vn=- sti=- udi=14723 nc=2 addr=4 flg=0x8180
        ...
        // [0x2BB38210] loc=[-1,2848,1153] rc=0 vc=631 vn=- sti=- udi=- nc=0 flg=0x284
        //
...
TR::Node* laddSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // reduce imul operations by factoring
        ...
        if (secondChildOp == TR::lconst) {
            ...
            {
                if ((firstChild->getReferenceCount() == 1)
                    && performTransformation(s->comp(), "%sFound ladd of non-lconst with ladd or lsub of x and lconst "
...
TR::Node* lsubSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // reduce imul operations by factoring
        ...
        if (secondChildOp == TR::lconst) {
            ...
            {
                if ((firstChild->getReferenceCount() == 1)
                    && performTransformation(s->comp(), "%sFound lsub of non-lconst with ladd or lsub of x and lconst "
                                                        "in node [" POINTER_PRINTF_FORMAT "]\n",
                           s->optDetailString(), node)) {
                    node->setChild(1, lrChild);
                    firstChild->setChild(1, secondChild);
    ...
    // 184251
        ...
        if (shftAmnt > 0 && (firstChild->getOpCodeValue() == TR::iadd || firstChild->getOpCodeValue() == TR::isub)
            ...
            if ((iValue << shftAmnt) == (int32_t)(lValue << shftAmnt)
                && performTransformation(s->comp(), "%sFound lsub/lmul of +ve powOf2 lconst with i2l of iadd or isub "
            ...
            if (secondChildOp == TR::lconst) {
                ...
                if (lrChild->getOpCodeValue() == TR::iconst) {
                    if (node->isNonNegative() && i2lNode->isNonNegative()) {
                        ...
                        // transformation
                        // we can loosen this up in the future if we have more information on children nodes but not the
                        // parents
                        int64_t value = (int64_t)lrChild->getInt();
                        value = -secondChild->getLongInt();
                        if (performTransformation(s->comp(), "%sFound lsub of lconst with i2l of iadd or isub of x and "
...
TR::Node* inegSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (opCode == TR::ineg) {
        ...
        if (shiftBy == 63 && performTransformation(s->comp(),
...
TR::Node* lshlSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        //
        performTransformation(s->comp(), "%sCanonicalize long left shift by constant in node [" POINTER_PRINTF_FORMAT
        ...
        if (constGrandChild->getOpCodeValue() == TR::iconst && constGrandChild->getInt() == (1 << rightShiftValue)) {
            ...
            if (subTree->getReferenceCount() == 1) {
                if (opCode == TR::s2i && rightShiftValue == 16) {
                    if (performTransformation(s->comp(), "%sReduced left shift followed by iushr equivalent to zero "
        ...
        if (constGrandChild->getOpCodeValue() == TR::lconst
            ...
            if (subTree->getReferenceCount() == 1) {
                if (opCode == TR::i2l && rightShiftValue == 32) {
                    if (performTransformation(s->comp(), "%sReduced left shift followed by lushr equivalent to zero "
...
TR::Node* removeRedundantIntegralOrPattern2(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    dumpOptDetails(s->comp(), "%sCreate new zero extension conversion %s [" POINTER_PRINTF_FORMAT
        ...
        // look for reassociation to fold constants
            ...
            if (lrChild->getOpCodeValue() == TR::iconst) {
                ...
                if (firstChild->getSecondChild()->getOpCodeValue() == TR::ior && firstChild->getReferenceCount() == 1
                    && (byte1
                           = isUnsafeIorByteChild(firstChild->getSecondChild()->getFirstChild(), TR::bu2i, 16777216, s))
                    && (byte2
                           = isUnsafeIorByteChild(firstChild->getSecondChild()->getSecondChild(), TR::bu2i, 65536, s))
...
TR::Node* lorSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // regs on a 32 bit platform by default
...
TR::Node* i2bSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    // 90762 zLinux perlbench perf degradation when we enable cancelling out of i2b and bu2i. todo: investigate why perf
    // degrades when we enable this
...
TR::Node* i2sSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::ior && firstChild->getReferenceCount() == 1
        ...
        && performTransformation(
               s->comp(), "%sconvert ior to isload node [" POINTER_PRINTF_FORMAT "]\n", s->optDetailString(), node)) {
...
TR::Node* bu2iSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCodeValue() == TR::l2b && firstChild->getFirstChild()->getOpCodeValue() == TR::lushr
        ...
        if (shiftBy >= 56 && performTransformation(s->comp(),
...
TR::Node* ificmpeqSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && conditionalBranchFold((firstChild->getInt() == secondChild->getInt()),
    ...
    if (firstChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCode().getOpCodeValue() == TR::lookup && NULL != shiftNode && shiftNode->getOpCode().isShift()) {
            ...
            if (NULL != shiftAmtNode && shiftAmtNode->getOpCode().isLoadConst()) {
                ...
                // the
                // case constants must be 0. For the constants, we can check the bits to be shifted out at compile time.
                // For
                // the shift node, which is non-constant, the only way we know those bits are 0 is if they are
                // explicitly
                // zeroed with an AND node.
                //
                // Example: if the selector is 3 (binary 11) and we shift it right 1, and have case values 0 and 1,
                // we'll match 1 (since 11b >> 1 is 1b)
...
TR::Node* checkcastSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if ((node->getFirstChild()->getReferenceCount() > 1) && (node->getSecondChild()->getReferenceCount() > 1)) {
        ...
        while (prevTree && ((prevTree->getNode()->getOpCodeValue() == TR::BBStart)
        ...
        if (prevTree) {
            ...
            if ((prevNode->getOpCodeValue() == TR::ificmpeq) || (prevNode->getOpCodeValue() == TR::ificmpne)) {
                ...
                if (firstChild->getOpCodeValue() == TR:: instanceof && secondChild->getOpCodeValue() == TR::iconst
                        && ((secondChild->getInt() == 0 && equalityTest) || (secondChild->getInt() == 1
                                                                                && !equalityTest))
...
TR::Node* imulhSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (firstChild->getOpCode().isLoadConst() && secondChild->getOpCode().isLoadConst()) {
        ...
        int64_t src2 = secondChild->getInt();
        if (src2 == 0 && performTransformation(s->comp(),
...
TR::Node* bndchkSimplifier(TR::Node* node, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (removableBoundCheck || (indexOp == TR::irem && indexChild->getFirstChild()->isNonNegative()
        ...
        if (lengthChild == boundChild || boundGELength(boundChild, lengthChild, s)) {
            ...
            if (indexChild->getOpCodeValue() == TR::isub) {
                ...
                if (boundChild->getOpCodeValue() == TR::iadd
                    ...
                    if (performTransformation(s->comp(), "%sRemoved arraycopy bndchk with arrayLength index in node "
