...
// If the exp operation itself is unsigned (for example from pduexp) then the exponent value is interpreted as an
// unsigned number.
    ...
    TR::Node* parent, TR::Node* exponent, int64_t maxNegativeExponent, int64_t maxPositiveExponent, TR::Simplifier* s)
{
    TR_ASSERT(exponent->getType().isIntegral(),
        "isIntegralExponentInRange only valid for integral exponents and not type %d\n", exponent->getDataType());
    TR_ASSERT(
...
#endif
    // There are two algorithms here -- they are equivalent in the number of multiply operations however the second is
    // better
...
bool propagateSignState(TR::Node* node, TR::Node* child, int32_t shiftAmount, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (!node->hasKnownOrAssumedCleanSign() && child->hasKnownOrAssumedCleanSign()
        ...
        && performTransformation(s->comp(), "%sSet Has%sCleanSign=true on %s [" POINTER_PRINTF_FORMAT
...
void convertStringToPacked(
    char* result, int32_t resultLen, bool resultIsEvenPrecision, char* source, int32_t sourceLen, uint32_t signCode)
{
    TR_ASSERT(signCode >= TR::DataType::getFirstValidSignCode() && signCode <= TR::DataType::getLastValidSignCode(),
...
void convertStringToZoned(
    char* result, int32_t resultLen, char* source, int32_t sourceLen, uint32_t signCode, bool signLeading)
{
    TR_ASSERT(signCode >= TR::DataType::getFirstValidSignCode() && signCode <= TR::DataType::getLastValidSignCode(),
        "invalid signCode 0x%x\n", signCode);
    TR_ASSERT(TR::DataType::isBCDSignChar(source[0]), "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
...
void convertStringToZonedSeparate(
    char* result, int32_t resultLen, char* source, int32_t sourceLen, uint32_t signCode, bool signLeading)
{
    TR_ASSERT(signCode == TR::DataType::getZonedSeparatePlus() || signCode == TR::DataType::getZonedSeparateMinus(),
        "invalid signCode 0x%x\n", signCode);
    TR_ASSERT(TR::DataType::isBCDSignChar(source[0]), "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
...
void convertStringToUnicodeSeparate(
    char* result, int32_t resultLen, char* source, int32_t sourceLen, uint32_t signCode, bool signLeading)
{
    TR_ASSERT(signCode == TR::DataType::getUnicodePlusCode() || signCode == TR::DataType::getUnicodeMinusCode(),
        "invalid signCode 0x%x\n", signCode);
    TR_ASSERT(TR::DataType::isBCDSignChar(source[0]), "expecting a minus/plus/unsigned and not 0x%x\n", source[0]);
    TR_ASSERT(isEven(resultLen), "Can't create a unicode constant with odd length\n");
...
TR::Node* removeOperandWidening(TR::Node* node, TR::Node* parent, TR::Block* block, TR::Simplifier* s)
{
    ...
    // This cannot be done globally because other node types (such a packed node under a call) must be explicitly
    // widened.
