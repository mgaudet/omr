...
bool propagateSignState(TR::Node* node, TR::Node* child, int32_t shiftAmount, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (!node->hasKnownOrAssumedSignCode() && child->hasKnownOrAssumedSignCode() && TR::Node::typeSupportedForSignCodeTracking(node->getDataType()) && performTransformation(s->comp(), "%sTransfer %sSignCode 0x%x from %s [" POINTER_PRINTF_FORMAT "] to %s [" POINTER_PRINTF_FORMAT "]\n",
                                                                                                                                                           s->optDetailString(),
    ...
    if (!node->hasKnownOrAssumedCleanSign() && child->hasKnownOrAssumedCleanSign() && ((node->getDecimalPrecision() >= child->getDecimalPrecision() + shiftAmount) || child->isNonNegative()) && performTransformation(s->comp(), "%sSet Has%sCleanSign=true on %s [" POINTER_PRINTF_FORMAT "] due to %s already clean %schild %s [" POINTER_PRINTF_FORMAT "]\n",
                                                                                                                                                                                                     s->optDetailString(),
...
{
    bool validateOp = node->getType().isBCD() && ((node->getOpCode().isConversion() && node->getNumChildren() == 1)
#ifdef J9_PROJECT_SPECIFIC
...
TR::Node* removeOperandWidening(TR::Node* node, TR::Node* parent, TR::Block* block, TR::Simplifier* s)
{
    ...
    if (node->isSimpleWidening()) {
    ...
    } else if ((node->getOpCodeValue() == TR::i2pd || node->getOpCodeValue() == TR::l2pd) && node->hasSourcePrecision() && node->getReferenceCount() == 1 && // the removal of widening may not be valid in the other contexts
        node->getDecimalPrecision() > node->getSourcePrecision() && performTransformation(s->comp(), "%sReducing %s [" POINTER_PRINTF_FORMAT "] precision %d to its child integer precision of %d\n",
    ...
    } else if (node->getOpCode().isShift() && node->getReferenceCount() == 1 && node->getSecondChild()->getOpCode().isLoadConst()) {
        ...
        if ((maxShiftedPrecision > 0) && (node->getDecimalPrecision() > maxShiftedPrecision) && performTransformation(s->comp(), "%sReducing %s [" POINTER_PRINTF_FORMAT "] precision %d to the max shifted result precision of %d\n",
    ...
    else if ((node->getOpCodeValue() == TR::pdclean || node->getOpCodeValue() == TR::zd2zdsle || node->getOpCodeValue() == TR::zdsle2zd || node->getOpCodeValue() == TR::zd2pd || node->getOpCodeValue() == TR::pd2zd || node->getOpCodeValue() == TR::pdSetSign || node->getOpCodeValue() == TR::pdclear || node->getOpCodeValue() == TR::pdclearSetSign) && node->getReferenceCount() == 1 && node->getDecimalPrecision() > node->getFirstChild()->getDecimalPrecision() && performTransformation(s->comp(), "%sReducing %s [" POINTER_PRINTF_FORMAT "] precision %d to its child precision of %d\n",
