    ...
    bool castToIntegral)
{
    ...
    if ((comp->getOptLevel() == noOpt) || !comp->getOptions()->getOption(TR_ScalarizeSSOps) || node->getOpCodeValue() != TR::arraycopy || node->getNumChildren() != 3 || comp->requiresSpineChecks() || !node->getChild(2)->getOpCode().isLoadConst() || cg->getOptimizationPhaseIsComplete())
        return node;
