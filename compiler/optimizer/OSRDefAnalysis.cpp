...
void TR_OSRDefInfo::buildOSRDefs(
    TR::Node* node, void* vanalysisInfo, TR_OSRPoint* osrPoint, TR::Node* parent, AuxiliaryData& aux)
{
    ...
    if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
        ...
        if (!defsForSymbol.IsZero() && isExpandedDefIndex(expandedNodeIndex) && !sym->isRegularShadow()
            && !sym->isMethod()) {
            /*
                        if (trace())
                           {
...
int32_t TR_OSRLiveRangeAnalysis::perform()
{
    ...
    if (!canAffordAnalysis()) {
        if (comp()->getOption(TR_TraceOSR)) {
            traceMsg(comp(),
                "%s OSR live range analysis is not affordable for this method because of the expected cost\n",
                optimizer()->getMethodSymbol()->signature(comp()->trMemory()));
            traceMsg(comp(), "Returning...\n");
        }
...

    {
        ...
        while (block) {
            ...
            for (tt = block->getExit(); tt != firstTT; tt = tt->getPrevTreeTop()) {
                ...
                if (osrPoint)
                    buildOSRLiveRangeInfo(tt->getNode(), _liveVars, osrPoint, liveLocalIndexToSymRefNumberMap,
                            ...
                            if (node->getFirstChild()->getOpCode().hasSymbolReference()
                                ...
                                if (rhsLocal && !rhsLocal->isLiveLocalIndexUninitialized()) {
                                    ...
                                    rhsLocal->setLocalIndex(
                                        rhsLocal->getLocalIndex() + node->getFirstChild()->getReferenceCount());
                                }
                            }
                        }
...
void TR_OSRLiveRangeAnalysis::buildOSRLiveRangeInfo(TR::Node* node, TR_BitVector* liveVars, TR_OSRPoint* osrPoint,
    int32_t* liveLocalIndexToSymRefNumberMap, int32_t maxSymRefNumber, int32_t numBits, TR_OSRMethodData* osrMethodData)
{
            ...
            if (catchBlock->isOSRCatchBlock()) {
                if (!blockCanOSR
                    ...
                    TR::DebugCounter::incStaticDebugCounter(comp(),
                        TR::DebugCounter::debugCounterName(comp(), "exceptionEdgeRemoval/(%s)", comp()->signature()));
                }
            }
        }
    }
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
{
    ...
    if (comp()->getOption(TR_FullSpeedDebug)) {
        if (comp()->getOption(TR_TraceOSR))
            traceMsg(comp(), "FSD enabled -- returning from OSR exception edge removal analysis since we implement FSD "
                             "with exception edges.\n");
        return 0;
    }
