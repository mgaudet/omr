...
void TR_OSRDefInfo::buildOSRDefs(TR::Node* node, void* vanalysisInfo, TR_OSRPoint* osrPoint, TR::Node* parent, AuxiliaryData& aux)
{
    ...
    if (expandedNodeIndex != NULL_USEDEF_SYMBOL_INDEX && expandedNodeIndex != 0) {
        ...
        if (!defsForSymbol.IsZero() && isExpandedDefIndex(expandedNodeIndex) && !sym->isRegularShadow() && !sym->isMethod()) {
            /*
            if (trace())
               {
...

    {
        ...
        for (i = 0; i < numSlots; i++) {
            ...
            for (nextSymRef = symRefsIt.getCurrent(); nextSymRef; nextSymRef = symRefsIt.getNext()) {
                if (nextSymRef->getSymbol()->isAuto() && !nextSymRef->getSymbol()->castToAutoSymbol()->isLiveLocalIndexUninitialized() && !nextSymRef->getSymbol()->holdsMonitoredObject()) {
    ...
    TR::Block* block)
{
    ...
    if (node->getOpCode().isStoreDirect()) {
                ...
                if (_pendingPushVars->get(localIndex)) {
                    if (_pendingSlotValueParents[localIndex]) {
                        ...
                        if (node->getFirstChild()->getVisitCount() != visitCount) {
                            ...
                            if (node->getFirstChild()->getOpCode().hasSymbolReference() && node->getFirstChild()->getSymbolReference()->getSymbol()->isAuto()) {
        ...
        if (local && !local->isLiveLocalIndexUninitialized()) {
            ...
            if ((!disallowOSRPPS3 || !_pendingPushVars->get(localIndex)) && ((node->getLocalIndex() == 1 || node->getOpCodeValue() == TR::loadaddr) && !liveVars->isSet(localIndex))) {
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
        ...
        for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();) {
            ...
            if (catchBlock->isOSRCatchBlock()) {
                if (!blockCanOSR && performTransformation(comp(), "%s: Remove redundant exception edge from block_%d at [%p] to OSR catch block_%d at [%p]\n", OPT_DETAILS, block->getNumber(), block, catchBlock->getNumber(), catchBlock)) {
...
int32_t TR_OSRExceptionEdgeRemoval::perform()
{
    ...
    for (cfgNode = cfg->getFirstNode(); cfgNode; cfgNode = cfgNode->getNext()) {
        ...
        TR::TreeTop* treeTop;
        for (treeTop = block->getEntry(); treeTop != block->getExit(); treeTop = treeTop->getNextTreeTop()) {
            if ((treeTop->getNode()->getNumChildren() > 0) && treeTop->getNode()->getFirstChild()->getOpCode().hasSymbolReference() && (comp()->getSymRefTab()->element(TR_induceOSRAtCurrentPC) == treeTop->getNode()->getFirstChild()->getSymbolReference())) {
        ...
        for (auto edge = block->getExceptionSuccessors().begin(); edge != block->getExceptionSuccessors().end();) {
            TR::Block* catchBlock = toBlock((*edge++)->getTo());
            if (catchBlock->isOSRCatchBlock()) {
                if (!seenInduceOSRCall && performTransformation(comp(), "%s: Remove redundant exception edge from block_%d at [%p] to OSR catch block_%d at [%p]\n", OPT_DETAILS, block->getNumber(), block, catchBlock->getNumber(), catchBlock))
                    cfg->removeEdge(block, catchBlock);
            }
        }
    }
