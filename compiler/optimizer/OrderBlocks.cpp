...
bool TR_OrderBlocks::needBetterChoice(TR::CFG* cfg, TR::CFGNode* block, TR::CFGNode* bestSucc)
{
    ...
    if (bestSucc != comp()->getStartBlock() && bestSucc->asBlock()->isSuperCold() && !block->asBlock()->isSuperCold()
        && safeToMoveAway(bestSucc->asBlock(), block->asBlock())) {
        if (trace())
...
bool TR_OrderBlocks::safeToMoveAway(TR::Block* block, TR::Block* prevBlock)
{
    ...
    if (_changeBlockOrderBasedOnHWProfile)
        return true;
#if 1
    if (hasOtherHotAsmFlowPredeccessors(block, prevBlock) || asmFlowHasNonFallThroughHotSuccessors(block)) {
        if (trace())
            traceMsg(comp(), "\t\tit's not safe to move block_%d away  \n", block->getNumber());
        return false;
    } else {
        if (trace())
            traceMsg(comp(), "\t\tit's safe to move block_%d away  \n", block->getNumber());
        return true;
    }
#endif
}
...
bool TR_OrderBlocks::hasOtherHotAsmFlowPredeccessors(TR::Block* block, TR::Block* prevBlock)
{
    ...
    for (auto predEdge = predecessors.begin(); predEdge != predecessors.end(); ++predEdge) {
        ...
        if (prevBlock != NULL && predBlock == prevBlock && block->getEntry() && block->getEntry()->getNode()
            && block->getEntry()->getNode()->getLabel() == NULL)
            continue;
        ...
        if (predBlock->isGenAsmFlowBlock() && !predBlock->isSuperCold()) {
            ...
            if (trace())
                traceMsg(comp(),
                    "\thasOtherHotAsmFlowPredeccessors: one predeccessor %d of block_%d is genAsmFlow block \n",
...
bool TR_OrderBlocks::asmFlowHasNonFallThroughHotSuccessors(TR::Block* block)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (!succBlock->isSuperCold()) {
            ...
            if (trace())
                traceMsg(comp(), "\tasmFlowHasNonFallThroughHotSuccessors: one successor %d of "
                                 "block_%d(genAsmFlowBlock) is not cold \n",
        ...
        if (block->asBlock()->isSuperCold() != bestSucc->asBlock()->isSuperCold() && block->asBlock()->getExit()
            ...
            if (trace())
                traceMsg(comp(), "\t\tEnd path because coldness changed from block_%d to block_%d "
                                 "block->asBlock()->getExit()=%p block->asBlock()->getExit()->getNextTreeTop()=%p\n",
...
bool TR_OrderBlocks::candidateIsBetterSuccessorThanBest(TR::CFGEdge* candidateEdge, TR::CFGEdge* currentBestEdge)
{
    ...
    if (candidate->hasSuccessor(currentBest)) {
        if (candidate->getFrequency() > (prevBlock->getFrequency() - candidate->getFrequency())) {
            if (trace())
                traceMsg(comp(), "\t\thas has current best succ as a succ, detecting an if-then structure and making "
                                 "the if block my best choice\n");
            return true;
        }
    }
        ...
        if (!_changeBlockOrderBasedOnHWProfile) {
            ...
            if (candBlock->isSuperCold() && safeToMoveAway(candBlock, block->asBlock())) {
                if (!_hotPathList.isEmpty()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "\tcandidate %d is super cold, discard because there are still choices which "
                                         "are not so cold \n",
                            theCandidate->getNumber());
                    return NULL;
                }
...
void TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode* block, TR::CFGNode* excludeBlock)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (block->getNumber() != 0 && succBlock->getNumber() == 1 && block->asBlock()->isAlwaysKeepBlock()
...
bool TR_OrderBlocks::peepHoleBranchAroundSimpleGenControlBlocks(
    TR::CFG* cfg, TR::Block* block, TR::Block* followingBlock, char* title)
{
    ...
    // nextBlock->isSimpleGenControlBlock());
    ...
    if (prevBlock != block && branchTarget == nextBlock) {
        if (performTransformation(comp(), "%s branch/goto in block_%d branch/goto around simple gen control bloks to "
                                          "block_%d, removing the branch/goto node\n",
            ...
            // oldNext->getNumber(), prevBlock->getNumber(), nextBlock->getNumber(),branchTarget->getNumber());
        ...
        if (!doNotRemove)
            ...
            if (takenBlock == blockAfterFallThrough
                ...
                if (fallThroughSuccessors.front()->getTo()->asBlock() != blockAfterFallThrough) {
                    ...
                    if (performTransformation(comp(), "%s in block_%d, branch taken dest (%d) is a block after a "
    ...
    while (madeAChange) {
        ...
        if (prevBlock != NULL) {
            ...
            if (madeAChange && prevBlock->isEmptyBlock() && (prevBlock->hasExceptionPredecessors() == false)
                && !comp()->isProfilingCompilation() && prevBlock->isTargetOfJumpWhoseTargetCanBeChanged(comp())) {
    ...
    while (!newBlockOrder.isEmpty()) {
        ...
        if (fallThroughTT != block->getEntry()) {
            ...
            if (prevBlock->getEntry() != NULL && fallThroughTT != NULL
                ...
                //     clean-up later)
                //     4) block is not a successor of prevBlock, but it begins the path we (arbitrarily?) chose to
                //     follow prevBlock
                //     5) block is a successor of prevBlock, but it's not the original fall through (due to
                //     endPathAtBlock)
