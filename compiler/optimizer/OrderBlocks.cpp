...
bool TR_OrderBlocks::needBetterChoice(TR::CFG* cfg, TR::CFGNode* block, TR::CFGNode* bestSucc)
{
    ...
    if (bestSucc != comp()->getStartBlock() && bestSucc->asBlock()->isSuperCold() && !block->asBlock()->isSuperCold() && safeToMoveAway(bestSucc->asBlock(), block->asBlock())) {
        if (trace())
...
bool TR_OrderBlocks::asmFlowHasNonFallThroughHotSuccessors(TR::Block* block)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (block->getExit() && block->getExit()->getNextTreeTop() && block->getExit()->getNextTreeTop()->getNode()->getBlock() == succBlock && succBlock->getEntry()->getNode()->getLabel() == NULL)
            continue;
...
void TR_OrderBlocks::addRemainingSuccessorsToList(TR::CFGNode* block, TR::CFGNode* excludeBlock)
{
    ...
    for (auto succEdge = successors.begin(); succEdge != successors.end(); ++succEdge) {
        ...
        if (block->getNumber() != 0 && succBlock->getNumber() == 1 && block->asBlock()->isAlwaysKeepBlock() && block->asBlock()->getExit()->getNextTreeTop() != NULL)
            continue;
