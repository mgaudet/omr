    ...
    if (treeHasChecks(tt) || node->getOpCode().isStore()) {
        ...
        if (node->getOpCode().isCheck()) {
            ...
            if (opCodeValue == TR::NULLCHK) {
                ...
                !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _nullCheckKilled->get(blockNum) || nodeKilledByChild) && !checkIfNodeCanSurvive(node->getNullCheckReference(), _indirectAccessesThatSurvive)) {
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                     !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex())*/ || _resolveCheckKilled->get(blockNum) || nodeKilledByChild)) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                    _divCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                     !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _boundCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _nullCheckKilled->set(blockNum);
                    _resolveCheckKilled->set(blockNum);
                    _divCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                     !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */ || _divCheckKilled->get(blockNum) || nodeKilledByChild) && !checkIfNodeCanSurvive(node->getFirstChild()->getSecondChild(), _dividesThatSurvive)) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _nullCheckKilled->set(blockNum);
                    _resolveCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                    !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _arrayStoreCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                     !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _arrayCheckKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
            ...
            } else if (node->getOpCode().isCheckCast()) //(opCodeValue == TR::checkcast)
            {
                ...
                if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                     !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _checkCastKilled->get(blockNum) || nodeKilledByChild) {
                    _killedGenExprs[blockNum]->set(node->getLocalIndex());
                    _resolveCheckKilled->set(blockNum);
                    _nullCheckKilled->set(blockNum);
                    _boundCheckKilled->set(blockNum);
                ...
                if (node->getOpCodeValue() == TR::checkcastAndNULLCHK) {
                    if ((!_optimisticRednSetInfo[blockNum]->get(node->getLocalIndex()) /* &&
                                                                                        !_optimisticOptSetInfo[blockNum]->get(node->getLocalIndex()) */) || _nullCheckKilled->get(blockNum) || nodeKilledByChild) {
                        if (!alreadyKilled) {
                            _killedGenExprs[blockNum]->set(node->getLocalIndex());
                            _resolveCheckKilled->set(blockNum);
                            _boundCheckKilled->set(blockNum);
