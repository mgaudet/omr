        ...
        if (anticipatabilityInfo->get(nextOptimalComputation)) {
            ...
            while (placeToInsertOptimalComputations != exitTree) {
                if (nextOptimalNode->getOpCode().isCheck()) {
                    ...
                    if (optimalNode && ((optimalNode->getLocalIndex() != nextOptimalComputation) || rhsOfStore
                                           || !lastInsertionMade)) {
                        //
                ...
                if (isSupportedOpCode(nullCheckReferenceNode, NULL) && !nullCheckReferenceOpCode.isLoadVarDirect()) {
                    if (((nullCheckReferenceNode->getLocalIndex() != MAX_SCOUNT)
                        ...
                        if ((_newSymbolsMap[nullCheckReferenceNode->getLocalIndex()] > -1)
                            ...
                            if (trace())
                                traceMsg(comp(), "Duplicate null check had its new null check reference %p replaced by "
                                                 "%p with symRef #%d\n",
            ...
            if ((firstOpCodeInTree.isStore()
                ...
                if (isExpressionRedundant(currentTree->getNode(), redundantComputations, anticipatabilityInfo)) {
                    ...
                    if (!currentTree->getNode()->getOpCode().isStore()) {
                        if (0 && performTransformation(comp(),
                ...
                if (newSymbolReference
                    ...
                    if (node->getDataType() == TR::NoType) {
                        ...
                        if (parent && parent->getOpCode().isConversion()
                            && (parent->getOpCodeValue() == TR::ILOpCode::getProperConversion(parent->getDataType(),
    ...
    vcount_t visitCount)
{
    ...
    if (isSupportedOpCode(node, parent)
        ...
        if (((node->getLocalIndex() != MAX_SCOUNT) && (node->getLocalIndex() != 0))
            ...
            // a result
            // of earlier optimal node placement not having been done (because of opt transformation index) that
            // interfere with last opt transformation index based search.
            // This is done because optimal sub node replacement is rarely the cause of bugs on it's own and this issue
            // is just making it harder to
            // debug problems in other PRE transformations.
            // We created another option TR_TracePREForOptimalSubNodeReplacement to continue to allow optimal sub node
            // replacement to be done regardless
            // but in order to make this more useful we turn off limiting optimal placements when this new option is in
            // effect (so that it becomes safe to narrow down to a
            ...
            if (shouldAllowOptimalSubNodeReplacement(comp())
                ...
                if (trace())
                    traceMsg(comp(), "Note : Duplicate parent %p wanted to replace its child %p by possibly already "
                                     "available symRef but FAILED to do so\n",
    ...
    if (!sourceList->isEmpty() && !targetList->isEmpty()) {
        ...
        for (newNode = sourceList->getListHead(); newNode != NULL;) {
            ...
            if ((oldNode == NULL) || (newNode == NULL)
                ...
                while (oldNode && ((oldNode->getData()->getOpCode().isIndirect()
                ...
                if (newNode)
                    newNode = newNode->getNextElement();
                while (newNode && ((newNode->getData()->getOpCode().isIndirect()
