...
void TR_RegisterCandidate::addBlock(TR::Block* block, int32_t numberOfLoadsAndStores, TR_Memory* m,
    bool ifNotFound)
{
    if (find(block)) {
        if (!ifNotFound)
    ...
    TR_BitVector& isExtensionOfPreviousBlock)
{
    ...
    while (bc.hasMoreElements()) {
        ...
        if (((firstBlock && _allBlocks && cg->getSupportsGlRegDepOnFirstBlock()) || (!firstBlock /* && (symbolIsLive(b) || hasLoopExitBlock(b)) */)) || !disableGRAChange) {
            ...
            //if (b == extendedBlock)
            if (disableGRAChange || (firstBlock && _allBlocks && cg->getSupportsGlRegDepOnFirstBlock()) || (!firstBlock && (symbolIsLive(b) || (hasLoopExitBlock(b) && (_blocks.getNumberOfLoadsAndStores(blockNumber) == 0)))))
    ...
    TR_Array<TR::Block*>& startOfExtendedBBForBB, bool callToRemoveUnusedLoops)
{
    ...
    if ((maxFrequency > 1) && removeUnusedLoops) {
        ...
        while (lookForBlocksToRemove) {
            ...
            if ((numUnreferencedBlocksAtThisFrequency > 0) && (comp->getOptions()->getOption(TR_EnableRangeSplittingGRA) || // move this check above ?
                ...
                while (bvi.hasMoreElements()) {
                    ...
                    if (blockWeight == frequency) {
                        ...
                        if (!referencedInNearestLoop && (((*_loadsAndStores)[blockNumber] == 0) || (dontAssignInColdBlocks(comp) && block->isCold()) /*||
                                                                            block->isRare(comp) */)) {
                            referencedInNearestLoop = false;
                            TR_RegionStructure* parent = blockStructure->getParent()->asRegion();
                            while (parent && !parent->isNaturalLoop()) {
                                        ...
                                        for (nextBlock = blocksIt.getCurrent(); nextBlock; nextBlock = blocksIt.getNext()) {
                                            if (nextBlock->getEntry()) {
                                                ...
                                                if (_liveOnEntry.get(liveBlock->getNumber())) {
                                                    if (((*_loadsAndStores)[liveBlock->getNumber()] != 0) && (!dontAssignInColdBlocks(comp) || !nextBlock->isCold())) {
    ...
    while (bvi.hasMoreElements()) {
        ...
        if ((!dontAssignInColdBlocks(comp) || !block->isCold()) && (blockWeight >= maxFrequency * freqRatio || useProfilingFrequencies)) {
            ++numberOfBlocks;
        }
        ...
        for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
            ...
            if (((predWeight > blockWeight) && !liveOnExitFromPred && !_liveOnEntry.get(pred->getNumber())) && (*_loadsAndStores)[startOfExtendedBBForBB[pred->getNumber()]->getNumber()] == 0 && (*_loadsAndStores)[pred->getNumber()] == 0) {
                bool reset = true;
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (!_liveOnEntry.get(extBlockNumber) && (block != comp->getFlowGraph()->getStart()) && ((*_loadsAndStores)[extBlockNumber] == 0)) {
            //do {
            ...
            for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
                ...
                do {
                    ...
                    if (!_liveOnEntry.get(currBlock->getNumber()) && // not set already
                        _liveOnExit.get(currBlock->getNumber()) && availableInPrevBlock && !getAvailableOnExit()->get(currBlock->getNumber())) {
                        if (trace)
   ...
   for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next)
      {
        ...
        if (!comp()->getOption(TR_DisableRegisterPressureSimulation) && comp()->cg()->supportsHighWordFacility() && !comp()->getOption(TR_DisableHighWordRA)) {
        ...
        if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
            ...
            //
                ...
                for (i = firstRegister; i <= lastRegister; ++i) {
                    ...
                    if ((!useRegisterPressureInfo && availableRegisters.get(i)) || ((i == comp()->cg()->getVMThreadGlobalRegisterNumber()) && rc->dontAssignVMThreadRegister())) {
...
void TR_RegisterCandidates::computeAvailableRegisters(TR_RegisterCandidate* rc, int32_t firstRegister, int32_t lastRegister, TR::Block** blocks, TR_BitVector* availableRegisters)
{
    ...
    for (i = firstRegister; i <= lastRegister; ++i) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (rc1 && (!overlaps.Locate(rc1->getSymbolReference()->getReferenceNumber(), hi1) || !overlaps.Locate(rc->getSymbolReference()->getReferenceNumber(), hi2) || overlaps[hi1].last < overlaps[hi2].first)) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (rc2 && (!overlaps.Locate(rc->getSymbolReference()->getReferenceNumber(), hi1) || !overlaps.Locate(rc2->getSymbolReference()->getReferenceNumber(), hi2) || overlaps[hi1].last < overlaps[hi2].first)) {
        ...
        if (_liveOnEntryConflicts[i].isEmpty() && _liveOnExitConflicts[i].isEmpty() && _exitEntryConflicts[i].isEmpty() && _entryExitConflicts[i].isEmpty() && comp()->cg()->isGlobalRegisterAvailable(i, rc->getDataType()) && ((i != comp()->cg()->getVMThreadGlobalRegisterNumber()) || !rc->dontAssignVMThreadRegister())) {
