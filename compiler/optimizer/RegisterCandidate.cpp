...
TR_RegisterCandidate* TR_RegisterCandidates::findOrCreate(TR::SymbolReference* symRef)
{
    ...
    TR_ASSERT(comp()->cg()->considerTypeForGRA(symRef),
        "should create a registerCandidate for symRef #%d with datatype %d\n", symRef->getReferenceNumber(),
        rc->getDataType());
...
TR_RegisterCandidate* TR_RegisterCandidates::find(TR::SymbolReference* symRef)
{
    if (_candidateForSymRefsSize
        && symRef->getReferenceNumber() > _candidateForSymRefsSize) // some spots in GRA add more candidates after
                                                                    // _candidateForSymRefs was created in certain cases
                                                                    // (ex if there are unused parameters, offerAllAutos
                                                                    // may create a new symref)
    {
...
void TR_RegisterCandidate::addBlock(TR::Block* block, int32_t numberOfLoadsAndStores, TR_Memory* m, bool ifNotFound)
{
    if (find(block)) {
        if (!ifNotFound)
    ...
    TR_BitVector& isExtensionOfPreviousBlock)
{
    ...
    _loadsAndStores
        = new (comp->trStackMemory()) TR_Array<uint32_t>(comp->trMemory(), numberOfBlocks, true, stackAlloc);
    ...
    while (bc.hasMoreElements()) {
        ...
        bool hasLoadNearStart
            = !isExtensionOfPreviousBlock.isSet(blockNumber) && findLoadNearStartOfBlock(b, getSymbolReference());
        ...
        if (((firstBlock && _allBlocks && cg->getSupportsGlRegDepOnFirstBlock())
            ...
            // getSymbolReference()->getReferenceNumber(), blockWeight, b->getNumber(), extendedBlock->getNumber(),
            // symbolIsLive(b));

            // if (b == extendedBlock)
            if (disableGRAChange || (firstBlock && _allBlocks && cg->getSupportsGlRegDepOnFirstBlock())
                || (!firstBlock && (symbolIsLive(b) || (hasLoopExitBlock(b)
    ...
    bool callToRemoveUnusedLoops)
{
    ...
    while (trace && bvi.hasMoreElements()) {
        ...
        if ((!dontAssignInColdBlocks(comp) || !block->isCold()) && (blockWeight >= maxFrequency * freqRatio))
            ++origNumberOfBlocks;
    ...
    while (bvi.hasMoreElements()) {
        int32_t blockNumber = bvi.getNextElement();
        if (!_liveOnEntry.get(blockNumber) && ((*_loadsAndStores)[blockNumber] > 0)) {
    ...
    while (bvi.hasMoreElements()) {
        ...
        do {
            ...
            if (((*_loadsAndStores)[blockNumber] == 0) && ((*_loadsAndStores)[blockNumber] == 0)) {
                    ...
                    if (((*succ)->getTo() != block->getNextBlock())
                        ...
                        if (!_liveOnEntry.get((*succ)->getTo()->getNumber())) {
                            ...
                            if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ (!block->isCold()
                                ...
                                if (trace)
                                    traceMsg(comp, "Reduced weight of candidate %d by %d (new weight %d) because of "
                                                   "end in live range at block_%d succ %d\n",
    ...
    while (bvi.hasMoreElements()) {
        ...
        if (!_liveOnEntry.get(extBlockNumber) && (block != comp->getFlowGraph()->getStart())
            ...
            // do {
            if (((*_loadsAndStores)[extBlockNumber] == 0) && ((*_loadsAndStores)[blockNumber] == 0)) {
                bool foundEndOfRange = false;
                for (auto succ = block->getSuccessors().begin(); succ != block->getSuccessors().end(); ++succ) {
                    if ((*succ)->getTo() != block->getNextBlock()) {
                        if (_liveOnEntry.get((*succ)->getTo()->getNumber())) {
                            ...
                            if (!exclude && blockStructure && (/* !dontAssignInColdBlocks(comp) || */ !block->isCold())
                                ...
                                if (trace)
                                    traceMsg(comp, "Reduced weight of candidate %d by %d (new weight %d) because of "
                                                   "start of live range at block_%d succ %d\n",
                                        getSymbolReference()->getReferenceNumber(), blockWeight, loadsAndStores,
                                        blockNumber, (*succ)->getTo()->getNumber());
                                foundEndOfRange = true;
                                break;
    ...
    bool isPPS = false; // (comp->getOption(TR_EnableOSR) && getSymbol()->isAuto() &&
                        // (getSymbolReference()->getCPIndex() < 0)); // This hurts in WAS performance; so it is disable
                        // until I can figure out the issue
            ...
            for (auto e = block->getPredecessors().begin(); e != block->getPredecessors().end(); ++e) {
                ...
                do {
                    ...
                    TR::GlobalSet::Set* autosInBlock
                        = comp->getGlobalRegisterCandidates()->getReferencedAutoSymRefsInBlock(currBlock->getNumber());
                    if (autosInBlock && autosInBlock->get(getSymbolReference()->getReferenceNumber()))
    ...
    TR::Block** cfgBlocks, int32_t numberOfBlocks, int32_t& lowestNumber, int32_t& highestNumber)
{
    ...
    TR_BitVector* availableOnExit
        = new (comp()->trStackMemory()) TR_BitVector(comp()->getFlowGraph()->getNextNodeNumber(), comp()->trMemory());
    ...
    for (; rc; rc = next) {
        ...
        dumpOptDetails(comp(), "Weight of candidate (symRef #%d %s) is %d\n",
            rc->getSymbolReference()->getReferenceNumber(), rc->getSymbolReference()->getSymbol()->isMethodMetaData()
   ...
   for (rc = first; rc; assign_candidate_loop_trace_increment(comp(), rc, iterationCount), rc = next)
      {
            ...
            while (bvi.hasMoreElements()) {
                ...
                if (isFloat) {
                    if (totalFPRCount[blockNum] >= (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
                        ...
                        for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
                            ...
                            if (hasMultiWayBranch || (totalFPRCountOnEntry[b->getNumber()]
                                                         >= (maxFPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                                if (trace)
                                    dumpOptDetails(comp(), "FPR count exceeded : resetting live on entry block_%d "
                                                           "(because of extended block_%d)\n",
                ...
                if (isVector) {
                    if (totalVRFCount[blockNum] >= maxVRFsLiveOnExit[blockNum]) {
                        ...
                        for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
                            ...
                            if (hasMultiWayBranch
                                ...
                                if (trace)
                                    dumpOptDetails(comp(), "VRF count exceeded : resetting live on entry block_%d "
                                                           "(because of extended block_%d)\n",
                                        b->getNumber(), blockNum);
                                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                    ...
                    if (totalGPRCount[blockNum] >= (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0))) {
                        ...
                        for (TR::CFGEdge* e = ebsi.getFirst(); e; e = ebsi.getNext()) {
                            TR::Block* b = e->getTo()->asBlock();
                            if (hasMultiWayBranch || (totalGPRCountOnEntry[b->getNumber()]
                                                         >= (maxGPRsLiveOnExit[blockNum] - (needs2Regs ? 1 : 0)))) {
                                if (trace)
                                    dumpOptDetails(comp(), "GPR count exceeded : resetting live on entry block_%d "
                                                           "(because of extended block_%d)\n",
                                        b->getNumber(), blockNum);
                                rc->getBlocksLiveOnEntry().reset(b->getNumber());
                                TR::Block* cursorBlock = problematicBlock;
        ...
        if (!comp()->getOption(TR_DisableRegisterPressureSimulation) && comp()->cg()->supportsHighWordFacility()
            ...
            // For symmetry, the converse is done : if the GPR is used for 64-bit value, we cannot assign an HPR to its
            // highword.
            {
                for (int8_t i = firstRegister; i <= lastRegister; ++i) {
                    ...
                    if (!availableRegisters.isSet(i) && cg->isGlobalGPR(i) && !cg->isGlobalHPR(i)) {
                        ...
                        if (trace) {
                            ...
                            traceMsg(comp(), "removing HPR %s from available list since its use will clobber the "
                                             "corresponding GPR\n",
        ...
        if ((registerNumber == -1) || (needs2Regs && (highRegisterNumber == -1))) {
            ...
            //
                ...
                const bool useRegisterPressureInfo = !comp()->getOption(TR_DisableRegisterPressureSimulation)
                    ...
                    && (!trace || performTransformation(comp(),
                ...
                if (conflictingRegister != -1) {
                    ...
                    if (trace) {
                        dumpOptDetails(comp(), "\nDue to conflicts, removing the following blocks from live on entry "
                                               "ranges of candidate #%d conflicting register %d\n",
    ...
    int32_t lastRegister, TR::Block** blocks, TR_BitVector* availableRegisters)
{
    ...
    for (i = firstRegister; i <= lastRegister; ++i) {
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (rc1 && (!overlaps.Locate(rc1->getSymbolReference()->getReferenceNumber(), hi1)
        ...
        while (bvi.hasMoreElements()) {
            ...
            if (rc2 && (!overlaps.Locate(rc->getSymbolReference()->getReferenceNumber(), hi1)
        ...
        if (_liveOnEntryConflicts[i].isEmpty() && _liveOnExitConflicts[i].isEmpty() && _exitEntryConflicts[i].isEmpty()
            && _entryExitConflicts[i].isEmpty() && comp()->cg()->isGlobalRegisterAvailable(i, rc->getDataType())
