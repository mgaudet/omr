...
void TR_ShrinkWrap::prePerformOnBlocks()
{
    ...
    _linkageUsesPushes = comp()->cg()->getLinkage()->mapPreservedRegistersToStackOffsets(
...
TR::Instruction* TR_ShrinkWrap::findJumpInstructionsInBlock(int32_t blockNum, TR::list<TR::Instruction*>* jmpInstrs)
{
    ...
    // As we do not handle multi-target jumps in Shrink Wrapping, this is a jump to label marking the successor of this
    // block
    TR::Instruction* current = _swBlockInfo[blockNum]._endInstr;

    while (current && current != _swBlockInfo[blockNum]._startInstr
        && (!comp()->cg()->isBranchInstruction(current) || current->getLabelSymbol()->isStartOfColdInstructionStream()))
        current = current->getPrev();

    TR_ASSERT(current != _swBlockInfo[blockNum]._startInstr, "Current should not have been able to make it all the way "
                                                             "back to the start instruction without encountering a "
                                                             "jump\n");
...
void TR_ShrinkWrap::findJumpInstructionsInCodeRegion(
    TR::Instruction* firstInstr, TR::Instruction* lastInstr, CS2::HashTable<TR::Instruction*, bool>& setOfJumps)
{
...
void TR_ShrinkWrap::analyzeInstructions()
{
    ...
    for (int32_t blockNum = 0; blockNum < _numberOfNodes; blockNum++) {
        if ((blockNum == _cfg->getEnd()->getNumber()) || (blockNum == _cfg->getStart()->getNumber()))
            continue;
   ...
   for (TR::TreeTop *tt = comp()->getStartTree(); tt; tt = tt->getNextTreeTop())
    ...
    for (TR::CFGNode* node = _cfg->getFirstNode(); node; node = node->getNext()) {
        ...
        if (!block->getExceptionPredecessors().empty()) {
            for (auto e = block->getExceptionPredecessors().begin(); e != block->getExceptionPredecessors().end();
                 ++e) {
...
void TR_ShrinkWrap::computeSaveRestoreSets(
    TR_RegisterAnticipatability& registerAnticipatability, TR_RegisterAvailability& registerAvailability)
{
    ...
    TR_RegisterAnticipatability& registerAnticipatability, TR_RegisterAvailability& registerAvailability)
{
        ...
        *temp |= *_saveInfo[firstBlockNum];
        ...
        for (PreservedRegisterInfo* p = _preservedRegs.getFirst(); p; p = p->getNext()) {
            ...
            if (1) {
                ...
                while (resIt.hasMoreElements()) {
                    ...
                    if (curBlock->getStructureOf()->getContainingLoop()) {
                        traceMsg(comp(), "         O^O Register %d is going to be restored in a block_%d inside a "
                                         "loop, no shrink wrap\n",
                    ...
                    //
                        ...
                        for (auto e = curBlock->getExceptionSuccessors().begin();
                            ...
                            if (r && !registerAvailability._inSetInfo[catchBlockNum]->get(regIndex)
                                ...
                                {
                                    traceMsg(comp(), "         O^O Register %d is going to be restored on a catch "
                                                     "block_%d that contains a return and needs to be split\n",
                                ...
                                //
                                traceMsg(comp(), "         O^O Register %d is going to be restored in a block_%d that "
                                                 "has exception edges\n",
                                    regIndex, blockNum);
                                found = true;
                                break;
                        ...
                        if (!found) {
                            ...
                            if ((isReturnBlock == TR_maybe) && hasExceptionSuccs) {
                                traceMsg(comp(), "         O^O Register %d is going to be restored in a block_%d that "
                                                 "contains a throw and has exception successors\n",
                                    regIndex, blockNum);
                                found = true;
                                break;
                            }
                    ...
                    if (!curBlock->getExceptionPredecessors().empty()) {
                        ...
                        for (auto e = curBlock->getExceptionPredecessors().begin();
                            ...
                            if (registerAnticipatability._outSetInfo[predBlockNum]->get(regIndex)
                                ...
                                if (_traceSW)
                                    traceMsg(comp(), "         O^O Edge between predecessor block_%d of catch block_%d "
                                                     "needs to be split\n",
                        ...
                        if (anyPredsNeedSplit) {
                            traceMsg(comp(), "         O^O Register %d is going to be saved in a catch block_%d that "
                                             "needs to be split\n",
                                regIndex, blockNum);
                            found = true;
                            break;
            ...
            bool doSave = performTransformation(comp(), "%s register %s [%d] at stack offset [%d]\n", OPT_DETAILS,
                comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, _mapRegsToStack[regIndex]);
            if (!doSave) {
                comp()->cg()->getPreservedRegsInPrologue()->set(regIndex);
                debugMode = true;
            } else
    ...
    for (PreservedRegisterInfo* p = _preservedRegs.getFirst(); p; p = p->getNext()) {
        ...
        if (p->_saveBlocks->isEmpty() || comp()->cg()->getPreservedRegsInPrologue()->get(regIndex)) {
            // nothing to do
            //
            continue;
        }
            ...
            if (!curBlock->getExceptionPredecessors().empty()) {
                ...
                for (auto e = curBlock->getExceptionPredecessors().begin();
                    ...
                    if (!registerAnticipatability._outSetInfo[predBlock->getNumber()]->get(regIndex)
                        ...
                        && !doneSave) {
                        traceMsg(comp(), "         O^O Saving register %s [%d] at instr [%p] in catch block [%d] with "
                                         "stack offset [%d]\n",
                            comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, cursor, blockNum,
                    ...
                    if (!doneSave) {
                        if (_traceSW)
                            traceMsg(comp(), "      register %s [%d] is used in some exception predecessor of catch "
                                             "block [%d], not saved\n",
                ...
                if (!registerAnticipatability._outSetInfo[predBlock->getNumber()]->get(regIndex)) {
                    traceMsg(comp(),
                        "         O^O Saving register %s [%d] at instr [%p] block [%d] with stack offset [%d]\n",
                        comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, cursor, blockNum,
                        _mapRegsToStack[regIndex]);
                    int32_t offset = _linkageUsesPushes ? -1 : _mapRegsToStack[regIndex];
                    comp()->cg()->getLinkage()->savePreservedRegister(cursor, regIndex, offset);
                    if (!_swBlockInfo[blockNum]._savedRegs) {
                        _swBlockInfo[blockNum]._savedRegs
                ...
                if (1) {
                    for (auto e = curBlock->getPredecessors().begin(); e != curBlock->getPredecessors().end(); ++e) {
                        ...
                        if (isFallThrough) {
                            ...
                            traceMsg(comp(),
                                "         O^O Found fall-through block_%d needsJump %d anyPredsNeedSplit %d\n",
                        ...
                        if (!registerAvailability._blockAnalysisInfo[predBlockNum]->get(regIndex)) {
                                ...
                                for (auto pred = curBlock->getPredecessors().begin();
                                    ...
                                    if (pei) {
                                        ...
                                        *temp4 &= *_preservedRegsInLinkage;
                                        traceMsg(comp(), "         O^O temp2 %d _restoreInfo[predBlockNum] %d "
                                                         "_restoreInfo[pNum] %d isFallThrough %d\n",
                                            temp2->isEmpty(), _restoreInfo[predBlockNum]->isEmpty(),
                                        ...
                                        if (temp2->isEmpty() && (*temp3 == *temp4)
                                            && _restoreInfo[predBlockNum]->isEmpty() && _restoreInfo[pNum]->isEmpty()
                                            && !isFallThrough) {
                                            traceMsg(comp(),
                                                "         O^O Can share split for saves between %d and %d\n",
                                                predBlockNum, pNum);
                            ...
                            if (!ei->_restoreSplitLocation) //! ei->_saveSplitLocation)
                            {
                                ...
                                cursor = comp()->cg()->splitEdge(
                                    cursor, isFallThrough, needsJump, newSplitLocation, &jmpInstrs, true);
                            ...
                            //
                            if (doSaves) {
                                traceMsg(comp(), "         O^O Saving register %s [%d] for block [%d] with instr [%p] "
                                                 "at predecessor block (split) [%d] with stack offset [%d]\n",
                                    comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, blockNum, cursor,
                                    predBlockNum, _mapRegsToStack[regIndex]);
                                int32_t offset = _linkageUsesPushes ? -1 : _mapRegsToStack[regIndex];
                                comp()->cg()->getLinkage()->savePreservedRegister(cursor, regIndex, offset);
                                ////doneSaves = true;
                            ...
                            if (saveLocation) {
                                ...
                                cursor = comp()->cg()->splitEdge(
                                    cursor, isFallThrough, false, newSaveLocation, &jmpInstrs);
                            }
                        } else
                            traceMsg(comp(), "         O^O Not saving register %s [%d] for block [%d] since its "
                                             "available in pred [%d]\n",
                                comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, blockNum,
                                predBlockNum);
                    }
        ...
        while (resIt.hasMoreElements()) {
            ...
            if (curBlock->getSuccessors().size() == 1) {
                ...
                if (doRestore) {
                    ...
                    traceMsg(comp(), "         O^O Restoring register %s [%d] at instr [%p] at end of block [%d] with "
                                     "stack offset [%d]\n",
                        comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, cursor, blockNum,
                        _mapRegsToStack[regIndex]);
                    ...
                    if (!_swBlockInfo[blockNum]._restoredRegs) {
                        _swBlockInfo[blockNum]._restoredRegs
                            = new (trStackMemory()) TR_BitVector(numRegs, trMemory(), stackAlloc);
                    ...
                    if (!registerAvailability._inSetInfo[succBlockNum]->get(regIndex)
                        ...
                        if (!(succBlock->getPredecessors().size() == 1)) {
                                ...
                                for (auto pred = succBlock->getPredecessors().begin();
                                    ...
                                    if (pei) {
                                        ...
                                        *temp2 &= *_preservedRegsInLinkage;
                                        ...
                                        if ((*temp1 == *temp2) && pei->_savedRegs->isEmpty() && !isFallThrough) {
                                            if (_traceSW)
                            ...
                            if (!ei->_restoreSplitLocation) {
                                ...
                                TR_ASSERT(isFallThrough || comp()->cg()->isBranchInstruction(cursor),
                                    "cursor [%p] is not branch before split edge\n", cursor);
                                cursor
                                    = comp()->cg()->splitEdge(cursor, isFallThrough, needsJump, NULL, &jmpInstrs, true);
                                ei->_restoreSplitLocation = cursor;
                            ...
                            if (doRestore) {
                                traceMsg(comp(), "         O^O Restoring register %s [%d] for block [%d] with instr "
                                                 "[%p] at successor block (split) [%d] with stack offset [%d]\n",
                                    comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, blockNum, cursor,
                                    succBlockNum, _mapRegsToStack[regIndex]);
                                comp()->cg()->getLinkage()->restorePreservedRegister(
                                    cursor, regIndex, _mapRegsToStack[regIndex]);
                            ...
                            /// else
                            {
                                traceMsg(comp(), "         O^O Not restoring register again %s [%d] for block [%d] at "
                                                 "successor block (split) [%d]\n",
                                    comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, blockNum,
                                    succBlockNum);
                                ...
                                if (!isFallThrough) {
                                    ...
                                    cursor = findJumpInstructionsInBlock(blockNum, &jmpInstrs);
                                    cursor = comp()->cg()->splitEdge(
                                        cursor, isFallThrough, false, newSplitLocation, &jmpInstrs);
                                }
                            }
                        } else // this is not possible according to the data flow solutions
                        {
                            cursor = _swBlockInfo[succBlockNum]._startInstr;
                            traceMsg(comp(), "         O^O Restoring register %s [%d] for block [%d] with instr [%p] "
                                             "at successor block [%d] with stack offset [%d]\n",
                                comp()->getDebug()->getRealRegisterName(regIndex - 1), regIndex, blockNum, cursor,
                                succBlockNum, _mapRegsToStack[regIndex]);
                            comp()->cg()->getLinkage()->restorePreservedRegister(
                                cursor, regIndex, _mapRegsToStack[regIndex]);
                ...
                if (curIdx == prevIdx + 1) {
                    ...
                    if (i > 1) {
                        ...
                        lmgInstrs = comp()->cg()->getLinkage()->composeSavesRestores(
                            lmgInstrs, startIdx, endIdx, _mapRegsToStack[startIdx], numRegs, doSaves);
                        if (forwardWalk) {
