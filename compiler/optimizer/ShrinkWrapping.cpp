                ...
                while (resIt.hasMoreElements()) {
                    ...
                    //
                        ...
                        for (auto e = curBlock->getExceptionSuccessors().begin(); e != curBlock->getExceptionSuccessors().end(); ++e) {
                            ...
                            if (r && !registerAvailability._inSetInfo[catchBlockNum]->get(regIndex) && !registerAnticipatability._blockAnalysisInfo[catchBlockNum]->get(regIndex)) {
                ...
                while (savIt.hasMoreElements()) {
                    ...
                    if (!curBlock->getExceptionPredecessors().empty()) {
                        ...
                        for (auto e = curBlock->getExceptionPredecessors().begin(); e != curBlock->getExceptionPredecessors().end(); ++e) {
                            ...
                            if (registerAnticipatability._outSetInfo[predBlockNum]->get(regIndex) || registerAvailability._blockAnalysisInfo[predBlockNum]->get(regIndex)) {
    ...
    for (int32_t i = 0; i < _cfg->getForwardTraversalLength(); i++) {
        ...
        for (TR::CFGEdge* e = predIt.getFirst(); e; e = predIt.getNext()) {
            ...
            if (!isBackEdge) {
                ...
                while (resIt.hasMoreElements()) {
                    int32_t regIndex = resIt.getNextElement();
                    if (!registerAvailability._inSetInfo[blockNum]->get(regIndex) && !registerAnticipatability._blockAnalysisInfo[blockNum]->get(regIndex))
...
//
         ...
         if (isInLoop &&
            ...
            if (!curBlock->getExceptionPredecessors().empty()) {
                ...
                for (auto e = curBlock->getExceptionPredecessors().begin(); e != curBlock->getExceptionPredecessors().end(); ++e) {
                    TR::Block* predBlock = toBlock((*e)->getFrom());
                    if (!registerAnticipatability._outSetInfo[predBlock->getNumber()]->get(regIndex) && !registerAvailability._blockAnalysisInfo[predBlock->getNumber()]->get(regIndex) && !doneSave) {
                                ...
                                for (auto pred = curBlock->getPredecessors().begin(); pred != curBlock->getPredecessors().end(); ++pred) {
                                    ...
                                    if (pei) {
                                        ...
                                        *temp4 &= *_preservedRegsInLinkage;
                                        ...
                                        //
                                        if (temp2->isEmpty() && (*temp3 == *temp4) && _restoreInfo[predBlockNum]->isEmpty() && _restoreInfo[pNum]->isEmpty() && !isFallThrough) {
    ...
    for (PreservedRegisterInfo* p = _preservedRegs.getFirst(); p; p = p->getNext()) {
        ...
        while (resIt.hasMoreElements()) {
            ...
            if (curBlock->getSuccessors().size() == 1) {
                ...
                //
                TR::Block* succBlock = toBlock(e->getTo());
                if (registerAvailability._inSetInfo[succBlock->getNumber()]->get(regIndex) && (succBlock != _cfg->getEnd())) {
                    if (_traceSW)
                ...
                for (auto e = curBlock->getSuccessors().begin(); e != curBlock->getSuccessors().end(); ++e) {
                    ...
                    if (!registerAvailability._inSetInfo[succBlockNum]->get(regIndex) && !registerAnticipatability._blockAnalysisInfo[succBlockNum]->get(regIndex)) {
      ...
      int32_t allocSize = numPreservedRegsInMethod - numRegsRestored;
        ...
        for (ReturnBlockInfo* r = _returnBlocks.getFirst(); r; r = r->getNext()) {
            ...
            if (r->_restoresRegisters) {
                ...
                while (resIt.hasMoreElements()) {
                    int32_t regIndex = resIt.getNextElement();
                    if (findPreservedRegisterInfo(regIndex) && !comp()->cg()->getPreservedRegsInPrologue()->get(regIndex)) {
                        if (_traceSW)
