...
// it contains sym1.
// This is a problem because then detecting when a particular sym1 reference is killed by another store or when the
// first reference to a particular
// symbol is impossible using the bitvectors alone.
// While the bitvectors are useful as a hint to the commoned symbols a more precise mechanism is required to track
// exactly what commoned references
// a particular store contains.
// Each movable store will contain a list of commoned loads beneath it. The loads will be wrapped in a class to mark the
// load as satisfied and/or killed and
// the symIdx that the load references.
// A commoned load is satisifed when the first reference to this load has been seen (regardless of whether the store has
// been killed first).
...
// The savedLiveCommoned bitvector can be more precisely refined to removed symbols when no active movable store
    ...
    , _isLoadStatic(false)
{
    ...
    if (_s->enablePreciseSymbolTracking() && _commonedLoadsUnderTree && !_commonedLoadsUnderTree->isEmpty()) {
        ...
        // NOTE: _commonedLoadsCount < _commonedLoadsUnderTree->elementCount() when a node is commoned within the tree
...
bool TR_MovableStore::satisfyCommonedLoad(TR::Node* node)
{
    ...
    for (ListElement<TR_CommonedLoad>* le = _commonedLoadsList->getListHead(); le; le = le->getNextElement()) {
        ...
        if ((node == commonedLoad->getNode()) && !commonedLoad->isSatisfied()) {
            if (_s->trace())
                traceMsg(comp(), "      satisfyCommonedLoad (store %p) symIdx %d setting commonedLoad %p with node %p "
                                 "satisfied (isKilled = %d, isSatisfied = %d)\n",
...
bool TR_MovableStore::killCommonedLoadFromSymbol(int32_t symIdx)
{
    ...
    for (ListElement<TR_CommonedLoad>* le = _commonedLoadsList->getListHead(); le; le = le->getNextElement()) {
        ...
        if (!commonedLoad->isSatisfied() && (commonedLoad->getSymIdx() == symIdx)) {
            if (_s->trace())
                traceMsg(comp(), "      killCommonedLoadFromSymbol (store %p) symIdx %d setting commonedLoad %p with "
                                 "node %p killed\n",
    ...
    if (_placementsForEdgesToBlock[toBlockNumber] != NULL) {
        ...
        while (ptr != NULL) {
            ...
            if (placementEdgeInfo != NULL) {
                ...
                // then when we ultimately place the stores on this edge, they will be stored in the correct order (i.e.
                // a store X that
                // originally appeared before another store Y in the code will be on _placement->_stores list in the
                // order X, Y
...
int32_t TR_SinkStores::performStoreSinking()
{
...

    {
        ...
        // anyway (after the block loop is done).  Having whole block info like this is probably not needed because we
        // are
            ...
            do {
                ...
                if (local != NULL) {
                    ...
                    if (symIdx != INVALID_LIVENESS_INDEX) {
                        TR_ASSERT(symIdx < numLocals, "Found store to local with larger index than my liveness "
                ...
                while (bvi.hasMoreElements()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "      symbol stores to a live commoned reference, searching stores below "
                                         "that load this symbol via commoned reference ...\n");
                    _killMarkWalks++;
                    ...
                    while (storeElement != NULL) {
                        ...
                        if (store->_movable && store->_commonedLoadsUnderTree
                            ...
                            if (trace()) {
                                if (!store->_movable)
                                    traceMsg(comp(), "         marking store [" POINTER_PRINTF_FORMAT
                ...
                // The anchored value of sym1 must be from just above the iload reference in b1 because an incorrect
                // anchor in b3 would cause
                    ...
                    if (!firstRefsToCommonedSymbols->isEmpty()) {
                            ...
                            if (store->_movable && store->_commonedLoadsUnderTree
                                ...
                                while (bvi.hasMoreElements()) {
                                    ...
                                    if (!store->containsUnsatisfedLoadFromSymbol(firstRefSymIdx)) {
                                        if (trace())
                                            traceMsg(comp(), "            symIdx %d now satisfied increment "
                                        ...
                                        traceMsg(comp(), "            symIdx %d is not yet satisfied do increment "
                                ...
                                if ((*firstRefsOfCommonedSymbolsUnderThisStore) != (*firstRefsToCommonedSymbols)) {
                                    ...
                                    while (bvi.hasMoreElements()) {
                                        int32_t firstRefSymIdx = bvi.getNextElement();
                                        if (trace())
                                            traceMsg(comp(), "            symIdx %d not under this store increment "
                                                             "satisfiedCommonedSymCount %d -> %d\n",
                                                firstRefSymIdx, satisfiedCommonedSymCount[firstRefSymIdx],
                                                satisfiedCommonedSymCount[firstRefSymIdx] + 1);
                                        satisfiedCommonedSymCount[firstRefSymIdx]++;
                                ...
                                while (bvi.hasMoreElements()) {
                                    int32_t firstRefSymIdx = bvi.getNextElement();
                                    if (trace())
                                        traceMsg(comp(), "            symIdx %d not at all under this store increment "
                                                         "satisfiedCommonedSymCount %d -> %d\n",
                                            firstRefSymIdx, satisfiedCommonedSymCount[firstRefSymIdx],
                                            satisfiedCommonedSymCount[firstRefSymIdx] + 1);
                                    satisfiedCommonedSymCount[firstRefSymIdx]++;
                        ...
                        while (firstRefsItAfter.hasMoreElements()) {
                            ...
                            if (satisfiedCommonedSymCount[firstRefSymIdx] == storeCount) {
                                if (trace())
                                    traceMsg(comp(), "            satisfiedCommonedSymCount[%d] = storeCount = %d set "
                                                     "in satisfiedLiveCommonedLoads\n",
                                        firstRefSymIdx, storeCount);
                                satisfiedLiveCommonedLoads->set(firstRefSymIdx);
                            } else if (trace()) {
                                traceMsg(comp(), "            satisfiedCommonedSymCount[%d] = %d and storeCount = %d "
                                                 "do not set in satisfiedLiveCommonedLoads\n",
                ...
                if (!enablePreciseSymbolTracking()) {
                    ...
                    // With this information an anchor or temp store can be created when/if it is actually needed in
                    // during
                    // sinkStorePlacement()
                        ...
                        while (storeElement != NULL) {
                            ...
                            if (store->_movable && store->_needTempForCommonedLoads) {
                                if (store->_needTempForCommonedLoads->intersects(*firstRefsToKilledSymbols)) {
                                    ...
                                    if (trace()) {
                                        traceMsg(comp(), "            found store [" POINTER_PRINTF_FORMAT
                ...
                if (local != NULL && node->getOpCode().isStoreDirect()) {
                    ...
                    if (canMoveStore && usedSymbols->intersects(*killedLiveCommonedLoads)) {
                        ...
                        if (treeDepth >= MIN_TREE_DEPTH_TO_MOVE) {
                            ...
                            canMoveStore = false;
                            if (trace())
                                traceMsg(comp(), "         store will not be moved because its tree depth of %d is too "
                                                 "small (MIN_DEPTH = %d)\n",
                ...
                if (!usedSymbols->isEmpty() || !killedSymbols->isEmpty()
                    ...
                    if (trace())
                        traceMsg(comp(), "      creating use or kill info on %s node [" POINTER_PRINTF_FORMAT
                ...
                if (canMoveStore) {
                    ...
                    if (enablePreciseSymbolTracking() && myCommonedLoads) {
                        ...
                        if (myCommonedLoads->isSet(symIdx)) {
                            ...
                            if (trace())
                                traceMsg(comp(), "      store kills its own commoned symbol (%d), mark the store as "
                                                 "needing a temp load\n",
                                    symIdx);
            ...
            while (!useOrKillInfoList.isEmpty()) {
                ...
                if (isMovableStore) {
                    ...
                    if (performTransformation(comp(),
                        ...
                        TR_ASSERT(_killedSymbolsToMove->isSet(symIdx), "_killedSymbolsToMove is inconsistent with "
                ...
                if (movedStore) {
                    ...
                    // if it is commoned within the Ebb.
                    // For now, just set the sym to be if it is commoned below so that stores from earlier node can push
                    // thru this block.
                    ...
                    if (store->_commonedLoadsAfter) {
                        ...
                        if (trace()) {
                            traceMsg(comp(), "        symbolsUsed in block_%d is updated due to commoned loads.  "
                                             "Symbols USED BEF is ",
                                blockNumber);
                            _symbolsUsedInBlock[blockNumber]->print(comp());
                            traceMsg(comp(), "\n");
...
{
    return (_lastSinkOptTransformationIndex == -1) || (_firstSinkOptTransformationIndex <= _numTransformations
    ...
    if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
        ...
        if (symIdx != INVALID_LIVENESS_INDEX && firstRefsOfCommonedSymbolsForThisStore->isSet(symIdx)) {
            ...
            if (trace() && commonedLoad)
                traceMsg(comp(), "      movableStore %p containsCommonedLoad (node %p, symIdx %d, isSatisfied = %d, "
                                 "isKilled = %d)\n",
            ...
            if ((node->getFutureUseCount() == 0) && movableStore->satisfyCommonedLoad(node)) {
                ...
                if (!findFirstUseOfLoad(node)) {
                    ...
                    if (trace())
                        traceMsg(comp(), "      searchAndMarkFirstUses creating and adding firstUse %p with node %p "
                                         "and anchor treetop %p to hash\n",
...
void TR_SinkStores::coalesceSimilarEdgePlacements()
{
    ...
    while (firstEdgePtr != NULL) {
        ...
        // redirected)
        // not really expecting there to be a large number of edges on this list, so just do the stupidest N^2 thing for
        // now
...
void TR_SinkStores::placeStoresAlongEdges(List<TR_StoreInformation>& stores, List<TR_EdgeInformation>& edges)
{
    ...
    if (!to->getExceptionPredecessors().empty()) {
        ...
        // to->getOwningMethod()); works on trychk2
        ...
        // placementBlock->setHandlerInfo(to->getCatchType(), to->getInlineDepth(), 0, to->getOwningMethod()); fail
        // modena 113380
    ...
    if (numChildren == 0) {
        ...
        if (node->getOpCode().isStore()
            && (node->dontEliminateStores()
                   || (node->getSymbolReference()->getSymbol()->isAuto() && (/*node->dontEliminateStores(comp()) ||*/
    ...
    vcount_t& highVisitCount)
{
    ...
    // enabled then
    // the call below must pass in comp()->incVisitCount() instead of comp()->getVisitCount() and treeVisitCount and
    // highVisitCount
    // must be incremented
    if (enablePreciseSymbolTracking()) {
        treeVisitCount++;
    ...
    int symIdx, TR::CFGNode* block, TR::CFGNode* succBlock, TR_BitVector* allEdgeInfoUsedOrKilledSymbols)
{
    ...
    if (_placementsForEdgesToBlock[succBlockNumber] != NULL) {
        ...
        for (TR_EdgeStorePlacement* placement = edgeIt.getFirst(); placement != NULL; placement = edgeIt.getNext()) {
            ...
            if (isSafe && from == block && (edgeInfo->_symbolsUsedOrKilled->intersects(*_usedSymbolsToMove)
    ...
    List<TR_MovableStore>& potentiallyMovableStores)
{
    if (node->getOpCode().isLoadVarDirect() && node->getOpCode().hasSymbolReference()) {
        ...
        // FIXME: does isCorrectCommonedLoad need to be called for all symbols or just TR_MethodMetaData symbols
        {
            ...
            if (performTransformation(comp(), "%s Create new temp store node for commoned loads sym %d and place above "
                                              "store [" POINTER_PRINTF_FORMAT "]\n",
...
// different reasons
// insertAnchoredNodes will take care of all possible node replacements so only a single recursive walk of a store is
// required
    ...
    List<TR_IndirectLoadAnchor>* indirectLoadAnchorsForThisStore, vcount_t visitCount)
{
    ...
    if (indirectLoadAnchorsForThisStore && nodeOrig->getOpCode().isLoadIndirect()) {
        ...
        if (nodeParentCopy) {
            ...
            // in the fall through case (where the node is not being replaced) then the children of this indirect load
            // must be
    ...
    */
        ...
        // nodeCopy == NULL)
        // In the case where the current store is the one being moved to the lowest fall through path we are not
        // duplicating and therefore no uncommoning
        ...
        if (nodeCopy && needTempForCommonedLoads && needTempForCommonedLoads->isSet(symIdx)
            ...
            if (trace())
                traceMsg(comp(), "      insertAnchoredNodes needTemp for symIdx %d found firstUse %p with node %p and "
                                 "anchor treetop %p\n",
            ...
            if (trace())
                traceMsg(comp(), "      insertAnchoredNodes commonedBlocked for symIdx %d found firstUse %p with node "
                                 "%p and anchor treetop %p\n",
                    symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
            ...
            if (!firstUseOfLoad) {
                ...
                if (trace())
                    traceMsg(comp(), "      insertAnchoredNodes usedBlocked create/add for symIdx %d  firstUse %p with "
                                     "node %p and anchor treetop\n",
                        symIdx, firstUseOfLoad, nodeOrig, store->_useOrKillInfo->_tt->getNode());
            } else {
                if (trace())
                    traceMsg(comp(), "      insertAnchoredNodes usedBlocked found for symIdx %d firstUse %p with node "
                                     "%p and anchor treetop %p\n",
                        symIdx, firstUseOfLoad, nodeOrig, firstUseOfLoad->getAnchorLocation());
            }
        }
...
TR::TreeTop* TR_TrivialSinkStores::genSideExitTree(TR::TreeTop* storeTree, TR::Block* exitBlock, bool isFirstGen)
{
    ...
    if (isFirstGen) {
        ...
        // will uncommon
        // the computeCC node anyway as cc computations require that the opNode operation is done right before the cc
        // sequence is
        ...
        //       sideExitTree)
        //       child2   <- anchor this node (except for const nodes, these are uncommoned -- duplicated in the
        //       sideExitTree)
    ...
    TR_MovableStore* store, bool storeChildIsAnchored, bool nextStoreWasMoved)
{
    ...
    // both an indirectLoad and
    // a used sym that is killed below (sym1) two anchors are required.
    // Instead we might as well just anchor the two children of the opNode to better deal with the case where they are
    // more complex expressions.
    // Blindly duplicating the entire tree and just inserting the minimally required anchors will cause lots of
    // uncommoning to occur as a result
    // of the duplication.
...

{
        ...
        if (storeCanMoveThroughBlock(
            ...
            if (_symbolsKilledInBlock[currentBlockNumber]->intersects(*usedSymbols)) {
                ...
                if (comp()->getOption(TR_SinkAllStores) || 1) {
                    ...
                    if (trace())
                        traceMsg(comp(), "         not sinking store %p any further as it is not a cc and there are "
                                         "blocking used symbols\n",
                            storeNode);
                    break;
                }
            }
            ...
            if (!(currentBlock->getSuccessors().size() == 1)) {
                ...
                if (lastTree->getNode()->getOpCode().isBranch()) {
                    ...
                    if (passesAnchoringTest(store, storeChildIsAnchored, nextStoreWasMoved)) {
                        ...
                        if (trace())
                            traceMsg(comp(), "                  store anchor branch first - create new store %p with "
                        ...
                        canMoveToAllPaths = true;
                        if (trace())
                            traceMsg(comp(), "               store child %p is not anchored under the branch or a "
                                             "subsequent store\n",
                        ...
                        if (anchoredByBranch) {
                            ...
                            copyStore = false;
                            TR_ASSERT(anchoredByBranchNode, "anchoredByBranchNode should have been created already "
                            ...
                            if (trace())
                                traceMsg(comp(), "                  branch anchor next - create new store %p with "
                            ...
                            if (trace())
                                traceMsg(comp(), "                  store anchor branch next - create new store %p "
                                                 "with commoned store->firstChild %p on sideExit block_%d\n",
                                    sideExitStoreTree->getNode(), storeNode->getFirstChild(), exitBlock->getNumber());
                        } else {
                            sideExitStoreTree = storeTree;
                            if (trace()) {
                                traceMsg(comp(), "                  cc child is not already anchored and should not be "
                                                 "anchored here\n");
                ...
                if (currentBlock->getExceptionSuccessors().size() == 1) {
                    ...
                    // exception
                    // then we will have to split the block as the store may not be live on all exception paths out of
                    // the block
                    // For now just require that an exception raising node must end a block. This is the excepted
                    // behaviour in zEmulator
                    // control flow graphs.
                    ...
                    if (exceptingNodes == 1) {
                        ...
                        if (passesAnchoringTest(store, storeChildIsAnchored, nextStoreWasMoved)) {
                            ...
                            if (trace())
                                traceMsg(comp(), "                  store anchor exception first - create new store %p "
                                                 "with commoned store->firstGrandChild %p on sideExit block_%d\n",
                                    sideExitStoreTree->getNode(), storeNode->getFirstChild()->getFirstChild(),
                                    exitBlock->getNumber());
                        } else if (!storeChildIsAnchored && indirectLoadCount) {
                            sideExitStoreTree = storeTree;
                            ...
                            if (anchoredByBranch) {
                                ...
                                sideExitStoreTree = TR::TreeTop::create(
                                    ...
                                    TR::Node::createWithSymRef(storeNode->getOpCodeValue(), 1, 1,
                                        storeNode->getFirstChild(), storeNode->getSymbolReference()));
                                if (trace())
                                    traceMsg(comp(), "                  create new store %p with commoned "
                                ...
                                sideExitStoreTree = genSideExitTree(storeTree, exitBlock, false);
                                if (trace())
                                    traceMsg(comp(), "                  store anchor exception next - create new store "
                                                     "%p with commoned store->firstChild %p on sideExit block_%d\n",
                                        sideExitStoreTree->getNode(), storeNode->getFirstChild(),
                                        exitBlock->getNumber());
                            } else {
                                sideExitStoreTree = storeTree;
                                if (trace()) {
                                    traceMsg(comp(), "                  cc child is not already anchored and should "
                                                     "not be anchored here\n");
                                    traceMsg(comp(), "                  create copy of storeTree %p on side exit\n",
                                        storeTree->getNode());
                                }
                            }
                        ...
                        if (trace() || comp()->getOption(TR_TraceOptDetails))
                            traceMsg(comp(), "            missed a store sinking opportunity as a current block_%d "
                                             "with a single exception succ does not have exactly 1 excepting node (%d "
                                             "excepting nodes)\n",
            ...
            if (canMoveToAllPaths) {
                ...
                if (sideExitStoreTree) {
                    ...
                    if (copyStore && store->_depth > MAX_TREE_DEPTH_TO_DUP) {
                        if (trace())
                            traceMsg(comp(), "no anchor found and depth %d is > than max duplication depth of %d so do "
                                             "not sink this store\n",
                                store->_depth, MAX_TREE_DEPTH_TO_DUP);
                        break;
                    }
                    if (copyStore && (indirectLoadCount || killedCommonedSymbols || blockingUsedSymbols
                                         || blockingCommonedSymbols)) {
                        storeInfo->_storeTemp = duplicateTreeForSideExit(sideExitStoreTree);
                        storeInfo->_needsDuplication = false;
                        if (trace())
                            traceMsg(comp(), "                  call insertAnchoredNodes dupTree is %p, copyStore = "
                                             "%d, indirectLoadCount = %d, killedCommonedSymbols = %p, "
                                             "blockingUsedSymbols = %p, blockingCommonedSymbols = %p\n",
                        ...
                        if (trace())
                            traceMsg(comp(), "                  do not call insertAnchoredNodes dupTree is %p, "
                                             "copyStore = %d, indirectLoadCount = %d, killedCommonedSymbols = %p, "
                                             "blockingUsedSymbols = %p, blockingCommonedSymbols = %p\n",
                                storeInfo->_storeTemp->getNode(), copyStore, indirectLoadCount, killedCommonedSymbols,
                                blockingUsedSymbols, blockingCommonedSymbols);
                    }
    ...
    if ((trace() || comp()->getOption(TR_TraceOptDetails)) && fallThroughBlock
        ...
        traceMsg(comp(), "            fallThroughBlock %d is not part of same superblock as currentBlock %d so will "
                         "not try to sink further\n",
    ...
    if (sourceBlock != currentBlock) {
        ...
        // down
        // then the indirect load may get moved passed a store to the same memory location (or loads will get reordered
        // which
        // is also not allowed). This happens, among other places, in the IL for TS where the iiload under the bustore
        // cc gets
        // swung below the iistore to the same memory location.
        if (anchorIndirectLoads || blockingUsedSymbols || blockingCommonedSymbols) {
            if (trace())
                traceMsg(comp(), "                  call insertAnchoredNodes !storeChildisAnchored = %d, "
            ...
            if (trace())
                traceMsg(comp(), "                  do not call insertAnchoredNodes !storeChildisAnchored = %d, "
                                 "indirectLoadCount = %d, killedCommonedSymbols = %p, blockingUsedSymbols = %p, "
                                 "blockingCommonedSymbols = %p\n",
                    !storeChildIsAnchored, indirectLoadCount, killedCommonedSymbols, blockingUsedSymbols,
                    blockingCommonedSymbols);
    ...
    // E_2b
    // E_3b
    // The stores are visited in the order S_b, S_a but for each store the exit blocks are visited in the order
    // E_1,E_2,E_3.
    // So the futureUseCounts will work correctly so the indirect load is anchored before its first use we want the
    // master list of
    // _indirectLoadAnchors to be in reverse order (the first loadAnchor in program order is the last one dealt with in
    // doSinking)
    //
    ...
    // futureUseCount will
    // be 6. When doSinking processes the list it will decrement the futureUseCount each time it encounters a regStore
    // and then
    // finally anchor it when the futureUseCount has reached zero.  Because L_1a shows up last in the list doSinking
    // will correctly
...
bool TR_GeneralSinkStores::sinkStorePlacement(TR_MovableStore* movableStore, bool nextStoreWasMoved)
{
    ...
    succList.insert(
        succList.end(), sourceBlock->getExceptionSuccessors().begin(), sourceBlock->getExceptionSuccessors().end());
    ...
    while (!worklist.isEmpty()) {
        ...
        if (!checkLiveMergingPaths(blockEntry, symIdx)) {
            ...
            // block
            // the liveness of the symbols used by the store can be determined from the liveness of the symbol(s) killed
            // by the store
            ...
            //        exception
            //        points within the block but not others, which would force us to split the block to safely push the
            //        store to
            ...
            if (_liveOnNotAllPaths->_inSetInfo[blockNumber]->get(symIdx)
                ...
                // if we can't push it all the way to the beginning of the successor block, then place def along the
                // edge
                if (trace())
                ...
                succList.insert(
                    succList.end(), block->getExceptionSuccessors().begin(), block->getExceptionSuccessors().end());
                for (auto succEdge = succList.begin(); succEdge != succList.end(); ++succEdge) {
                    TR::CFGNode* succBlock = (*succEdge)->getTo();
                    int32_t succBlockNumber = succBlock->getNumber();
                    ...
                    if (_liveOnSomePaths->_blockAnalysisInfo[succBlockNumber]->get(symIdx)
                        ...
                        if (!canSinkOutOfEBB && !isSuccBlockInEBB) {
                            if (trace())
                                traceMsg(comp(), "              attempt to sink out of Ebb but commoned syms are "
                                                 "killed in the Ebb => don't sink\n");
                            placeDefInBlock = true;
                            break;
                        }
                        if (!_sinkThruException && succBlock->asBlock()->isCatchBlock()
                ...
                if (placeDefInBlock) {
                    if (numOfEdgePlacements) {
                        if (trace())
                            traceMsg(comp(), "              all previously added edge store placements from this block "
                                             "will be removed\n");
