...
void TR_RegionAnalysis::simpleIterator(TR_Stack<int32_t>& workStack, SparseBitVector& vector,
    WorkBitVector& regionNodes, WorkBitVector& nodesInPath, bool& cyclesFound, TR::Block* hdrBlock, bool doThisCheck)
{
    ...
    for (cursor.SetToLastOne(); cursor.Valid(); cursor.SetToPreviousOne()) {
        ...
        //
        if (doThisCheck && next._succ.IsZero() && next._originalBlock == _cfg->getEnd()) {
        ...
        if (regionNodes.ValueAt(cursor)) {
            if (!cyclesFound && nodesInPath.ValueAt(cursor) && _dominators.dominates(hdrBlock, next._originalBlock)) {
                cyclesFound = true;
                if (trace()) {
    ...
    StructInfo& node, WorkBitVector& regionNodes, WorkBitVector& nodesInPath)
{
    ...
    TR_RegionStructure* region
        = new (trHeapMemory()) TR_RegionStructure(_compilation, node._structure->getNumber() /* node._nodeIndex */);
    if (cyclesFound) {
        if (trace())
 ...
 * flow graph.
 */
void TR_RegionAnalysis::buildRegionSubGraph(
    TR_RegionStructure* region, StructInfo& entryNode, WorkBitVector& regionNodes, SubGraphNodes& cfgNodes)
{
    StructInfo* toNode;
        ...
        for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
            ...
            if (cfgNodes[toIndex] == NULL) {
                if (regionNodes.ValueAt(toIndex))
                    ...
                    cfgNodes[toIndex]
                        = new (trHeapMemory()) TR_StructureSubGraphNode(toNode._structure->getNumber(), trMemory());
            }
            to = cfgNodes[toIndex];
