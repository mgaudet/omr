...
void TR_RegionStructure::cleanupAfterEdgeRemoval(TR::CFGNode* node)
{
    ...
    if (blockSt != NULL && blockSt->getBlock()->isAlwaysKeepBlock())
        return;
...
static bool findCycle(TR_StructureSubGraphNode* node, TR::SparseBitVector& regionNodes, TR_BitVector& nodesSeenOnPath,
    ...
    List<TR_Structure>* correspondingWhileLoops)
{
    ...
    TR_BlockStructure* clonedBlockStructure
        = new (trHeapMemory()) TR_BlockStructure(comp(), correspondingBlock->getNumber(), correspondingBlock);
    ...
    List<TR_Structure>* correspondingWhileLoops)
{
    TR_RegionStructure* clonedRegionStructure
        = new (trHeapMemory()) TR_RegionStructure(comp(), correspondingBlocks[getNumber()]->getNumber());
...
void TR_RegionStructure::checkStructure(TR_BitVector* _blockNumbers)
{
    ...
    for (node = si.getCurrent(); node != NULL; node = si.getNext()) {
        ...
        TR_ASSERT(node->getNumber() == node->getStructure()->getNumber(),
    ...
    for (edge = exitEdges.getFirst(); edge; edge = exitEdges.getNext()) {
        ...
        for (seenNode = exitNodesIt.getFirst(); seenNode; seenNode = exitNodesIt.getNext()) {
            if ((seenNode->getNumber() == toNode->getNumber()) && (seenNode != toNode))
                TR_ASSERT(0, "Exit edges to the same node number %d in region %p must have a unique structure subgraph "
                             "node (node1 %p node2 %p)\n",
                    ...
                    while (bvi.hasMoreElements()) {
                        ...
                        if (!(optSetInfo[outerLoopInvariantBlock->getNumber()])->get(nextOptimalComputation)) {
                            if (outerLoop->isExpressionTransparentIn(nextOptimalComputation, transparencyInfo)) {
                                ...
                                dumpOptDetails(comp(), "\nO^O PARTIAL REDUNDANCY ELIMINATION: Hoisting loop invariant "
                                                       "computation %d over nested loops (lifted from inner block_%d "
                                                       "to outer block_%d)\n",
