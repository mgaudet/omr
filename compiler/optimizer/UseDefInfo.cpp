...
void TR_UseDefInfo::findTrivialSymbolsToExclude(TR::Node* node, TR::TreeTop* treeTop, AuxiliaryData& aux)
{
    ...
    if (node->getOpCode().hasSymbolReference() && aux._neverReferencedSymbols.ValueAt(node->getSymbolReference()->getReferenceNumber())) {
...
bool TR_UseDefInfo::isValidAutoOrParm(TR::SymbolReference* symRef)
{
    ...
    return (useDefAliases.PopulationCount() == 1 && useOnlyAliases.PopulationCount() == 1 && !comp()->cg()->getLinkage()->isAddressOfStaticSymRef(symRef));
}
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData& aux)
{
    ...
    for (refIter.SetToFirstOne(); refIter.Valid(); refIter.SetToNextOne()) {
        ...
        if (symRef) {
            ...
            if (sym) {
                ...
                if (!isTrivialUseDefSymRef(symRef, aux) && ((sym->isParm() && !aux._neverWrittenSymbols.ValueAt(symRef->getReferenceNumber())) || (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull())))
    ...
    for (refIter.SetToFirstOne(); refIter.Valid(); refIter.SetToNextOne()) {
        ...
        if (symRef && (!isTrivialUseDefSymRef(symRef, aux) && ((symRef->getSymbol()->isParm() && !aux._neverWrittenSymbols.ValueAt(symRef->getReferenceNumber())) || (aux._onceReadSymbolsIndices[symRef->getReferenceNumber()].IsNull() && aux._onceWrittenSymbolsIndices[symRef->getReferenceNumber()].IsNull())))) {
            sym = symRef->getSymbol();
            if (trace()) {
