...
TR_UseDefInfo::TR_UseDefInfo(TR::Compilation* comp, TR::CFG* cfg, TR::Optimizer* optimizer, bool requiresGlobals,
    ...
    , _valueNumbersToMemorySymbolsMap(
          comp->allocator(allocatorName), TR_UseDefInfo::MemorySymbolList(comp->allocator(allocatorName)))
...
void TR_UseDefInfo::setVolatileSybolsIndexAndRecurse(TR::BitVector& volatileSymbols, int32_t symRefNum)
{
    ...
    for (aliasesCursor.SetToFirstOne(); aliasesCursor.Valid(); aliasesCursor.SetToNextOne()) {
        ...
        // I believe this to be a possible optimization.  However, it seems like some pali aliases end up making us
        // still overly conservative
        // I think more alias refinement should be done -- ie if a symbol does not appear in the method, we should
        // remove it from all alias sets.
    ...
    if (node->getOpCode().isStoreDirect()) {
        ...
        if (symRef->getSymbol()->isAutoOrParm()) {
            ...
            if (aux._neverWrittenSymbols.ValueAt(symRefNum)) {
                ...
                // following code fails assert when aux._onceWrittenSymbolsIndices[symRefNum].IsZero() - needs to be
                // fixed
            ...
            != node->getFirstChild()) // value number is shared with some other node
    {
        ...
        if (trace())
            traceMsg(comp(), "Node %p has memory symbol index %d (%d:%d:%d)\n", node,
                _valueNumbersToMemorySymbolsMap[valueNumber].Head()->_localIndex, valueNumber, size, offset);
    }
}
...
bool TR_UseDefInfo::indexSymbolsAndNodes(AuxiliaryData& aux)
{
    ...
    for (refIter.SetToFirstOne(); refIter.Valid(); refIter.SetToNextOne()) {
        ...
        if (symRef && (!isTrivialUseDefSymRef(symRef, aux)
    ...
    // a cleared _symRefToLocalIndexMap is required for each call to indexSymbolsAndNodes - would be better to create as
    // a local variable here,
    // and pass down to findUseDefNodes, but findUseDefNodes is recursive, and don't want to increase stack size, so
    // keep in aux
    ...
    AuxiliaryData& aux, TR::SparseBitVector& expandedLIndexes, bool considerImplicitStores)
{
    ...
    if (trace()) {
        if (!definesMultipleSymbols)
            traceMsg(comp(), "Node : %p   opCode = %s useDefIndex = %d localIndex = %d definesMultipleSymbols=%d "
            ...
            traceMsg(comp(), "Node : %p   opCode = %s useDefIndex = %d localIndex = %d-%d definesMultipleSymbols=%d "
                             "isTrivialUseDefNode=%d adjustArray=%d \n",
...
void TR_UseDefInfo::buildUseDefs(
    TR::Node* node, void* vanalysisInfo, TR::BitVector& nodesToBeDereferenced, TR::Node* parent, AuxiliaryData& aux)
{
    // *this    swipeable for debugging purposes
    vcount_t visitCount = comp()->getVisitCount();
    ...
    if (isExpandedUseIndex(expandedNodeIndex)
        || ((expandedNodeIndex == 0) && (!node->getOpCode().isStore() || !node->getSymbol()->isAutoOrParm()
    ...
    if (symIndex == NULL_USEDEF_SYMBOL_INDEX || node->getOpCode().isCall() || node->getOpCode().isFence()
        ...
        //
        numDefNodes = 1;
        if (!aux._defsForSymbol[symIndex].IsZero() && (!sym || (!sym->isShadow() && !sym->isMethod()))) {
            {
...
const TR_UseDefInfo::BitVector& TR_UseDefInfo::getUseDef_ref_body(
    int32_t useIndex, TR_UseDefInfo::BitVector& visitedDefs, TR_UseDefInfo::BitVector* defs)
{
...
void TR_UseDefInfo::buildDefUseInfo(bool loadAsDef)
{
    ...
    if ((_defUseInfo.NumberOfElements() > 0) && ((_loadDefUseInfo.NumberOfElements() > 0) || !loadAsDef))
        return;
...
{
    TR_ASSERT(
        udIndex < _sideTableToSymRefNumMap.NumberOfElements(), "index into _sideTableToSymRefNumMap out of bounds");
