...
public:
    TR_VPShortConstraint(int16_t v)
        : TR_VPConstraint(ShortPriority)
    {
        _low = v;
        _overflow = TR_no;
    }
...
public:
    TR_VPShortConst(int16_t v)
        : TR_VPShortConstraint(v)
    {
    }
...
public:
    TR_VPShortRange(int16_t low, int16_t high)
        : TR_VPShortConstraint(low)
        , _high(high)
    {
    }
...
public:
    TR_VPIntConstraint(int32_t v)
        : TR_VPConstraint(IntPriority)
    {
        _low = v;
        _overflow = TR_no;
    }
...
public:
    TR_VPIntConst(int32_t v)
        : TR_VPIntConstraint(v)
    {
    }
...
public:
    TR_VPIntRange(int32_t low, int32_t high)
        : TR_VPIntConstraint(low)
        , _high(high)
    {
    }
...
public:
    TR_VPLongConstraint(int64_t v)
        : TR_VPConstraint(LongPriority)
    {
        _low = v;
        _overflow = TR_no;
    }
...
public:
    TR_VPLongConst(int64_t v)
        : TR_VPLongConstraint(v)
    {
    }
...
public:
    TR_VPLongRange(int64_t low, int64_t high)
        : TR_VPLongConstraint(low)
        , _high(high)
    {
    }
...
public:
    TR_VPFixedClass(TR_OpaqueClassBlock* klass, TR::Compilation* comp, int32_t p = FixedClassPriority)
        : TR_VPResolvedClass(klass, comp, p)
    {
    }
...
protected:
    TR_VPConstString(TR_OpaqueClassBlock* klass, TR::Compilation* comp, TR::SymbolReference* symRef)
        : TR_VPFixedClass(klass, comp, ConstStringPriority)
        , _symRef(symRef)
    {
    }
...
public:
    TR_VPUnresolvedClass(const char* sig, int32_t len, TR_ResolvedMethod* method)
        : TR_VPClassType(UnresolvedClassPriority)
        , _method(method)
    {
        _sig = sig;
        _len = len;
...
public:
    TR_VPClassPresence()
        : TR_VPConstraint(ClassPresencePriority)
    {
    }
...
public:
    TR_VPPreexistentObject(TR_OpaqueClassBlock* c = NULL)
        : TR_VPConstraint(ClassPreexistencePriority)
    {
        _assumptionClass = c;
    }
...
public:
    TR_VPArrayInfo(int32_t lowBound, int32_t highBound, int32_t elementSize)
        : TR_VPConstraint(ArrayInfoPriority)
        , _lowBound(lowBound)
...
private:
    TR_VPObjectLocation(TR_VPObjectLocationKind kind)
        : TR_VPConstraint(ObjectLocationPriority)
        , _kind(kind)
    {
    }
...
protected:
    TR_VPKnownObject(TR_OpaqueClassBlock* klass, TR::Compilation* comp, TR::KnownObjectTable::Index index, bool isJavaLangClass)
        : TR_VPFixedClass(klass, comp, KnownObjectPriority)
        , _index(index)
