        ...
        if (!symRef->getSymbol()->isArrayShadowSymbol()) {
            ...
            if (haveClassLookaheadInfo) {
                ...
                if (sym->isStatic() && sym->isFinal()) {
                    ...
                    TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(classOfStatic, vp->comp());
                    if (classInfo && classInfo->isInitialized())
                        isClassInitialized = true;

#ifdef J9_PROJECT_SPECIFIC
                    if (classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized && !vp->comp()->getOption(TR_AOT) && (type == TR::Address)) {
                ...
                if (!foundInfo) {
                    TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(symRef->getOwningMethod(vp->comp())->classOfStatic(symRef->getCPIndex()), vp->comp());
    ...
    if (node->getOpCode().hasSymbolReference()) {
        ...
        if (base && node->getOpCode().hasSymbolReference() && (node->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
...

                {
                    ...
                    if (constrainIaloadCriticalSection.hasVMAccess()) {
                        ...
                        if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
                            ...
                            if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), clazz) && (TR::Compiler->cls.isClassFinal(vp->comp(), clazz)
            ...
            if (symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
                ...
                // but clearly others make invalid assumptions on seeing this fixed class constraint so be conservative
                // and don't let the world know if we aren't sure the elements match this type
                if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass) && (TR::Compiler->cls.isClassFinal(vp->comp(), componentClass)
                ...
                if (clazz) {
                    ...
                    if (jlClass) {
                        if (clazz != jlClass) {
                            int32_t fieldNameLen = -1;
                            char* fieldName = NULL;
                            if (underlyingArray && underlyingArray->getOpCode().hasSymbolReference() && (underlyingArray->getSymbolReference()->getSymbol()->isStatic() || underlyingArray->getSymbolReference()->getSymbol()->isShadow())) {
...
TR::Node* constrainWrtBar(TR_ValuePropagation* vp, TR::Node* node)
{
    ...
    if (doOpt && ((gcMode == TR_WrtbarCardMarkAndOldCheck) || (gcMode == TR_WrtbarOldCheck)) && (node->getOpCodeValue() == TR::wrtbari) && !node->skipWrtBar()) {
    ...
    if ((node->getOpCodeValue() == TR::wrtbari) && !node->getSymbolReference()->getSymbol()->isArrayShadowSymbol() && !vp->comp()->getOption(TR_DisableWriteBarriersRangeCheck)) {
    ...
    if (rhs && rhs->asIntConst()) {
        ...
        if (mask == 0) {
            ...
            if (rhs && rhs->asIntConst()) {
                ...
                if ((firstChild->getOpCodeValue() == TR::iloadi) && (firstChild->getSymbolReference() == vp->comp()->getSymRefTab()->findClassIsArraySymbolRef()) && (rhs->getLowInt() == TR::Compiler->cls.flagValueForArrayCheck(vp->comp()))) {
                    TR::Node* firstGrandChild = firstChild->getFirstChild();
                    if ((firstGrandChild->getOpCodeValue() == TR::aloadi) && (firstGrandChild->getSymbolReference() == vp->comp()->getSymRefTab()->findClassRomPtrSymbolRef())) {
                        TR::Node* vftLoad = firstGrandChild->getFirstChild();
                        if ((vftLoad->getOpCodeValue() == TR::aloadi) && (vftLoad->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef())) {
...
static TR::Node* constrainNarrowIntValue(TR_ValuePropagation* vp, TR::Node* node, int32_t low, int32_t high)
{
    ...
    if (low <= high) {
        ...
        case TR::Int16:
            constraint = TR_VPShortRange::create(vp, low, high);
    ...
    TR_VPConstraint* classConstraint, bool& applyType, bool isInstanceOf)
{
    ...
    if (objectRefConstraint) {
        ...
        if (instanceClass) {
            ...
            else if (isInstanceOf && !objectRefConstraint->getClassType() && !castIsClassObject && !castIsInterfaceImplementedByClass && objectRefConstraint->isNonNullObject() && (objectRefConstraint->isClassObject() == TR_yes)) {
...
#ifdef J9_PROJECT_SPECIFIC
    if (!vp->comp()->compileRelocatableCode() && vp->comp()->isOptServer() && vp->lastTimeThrough() && vp->comp()->performVirtualGuardNOPing() && !vp->_curBlock->isCold() && !vp->_curBlock->getNextBlock()->isExtensionOfPreviousBlock() && (node->getNumChildren() == 2) && ((node->getOpCodeValue() == TR::ifacmpeq) || (node->getOpCodeValue() == TR::ifacmpne))) {
            ...
            if (objectRefConstraint) {
                ...
                if (typeConstraint) {
                    ...
                    if (resolvedTypeConstraint) {
                        ...
                        TR_PersistentClassInfo* classInfo = vp->comp()->getPersistentInfo()->getPersistentCHTable()->findClassInfoAfterLocking(clazz, vp->comp());
        ...
        if (instanceofDetected) {
            ...
            if (objectRefConstraint) {
                ...
                if (instanceClass) {
                    ...
                    if (newTypeConstraint && instanceofDetectedAndFixedType && newTypeConstraint->asResolvedClass() && !newTypeConstraint->asFixedClass()) {
                    ...
                    if (!newConstraint && applyType)
                        removeBranch = true;
                    else if (newConstraint && !isVirtualGuardNopable && !vp->addEdgeConstraint(instanceofObjectRef, newConstraint, edgeConstraints))
                        removeBranch = true;
                ...
                if (applyType && !isVirtualGuardNopable && !vp->addEdgeConstraint(instanceofObjectRef, newConstraint, edgeConstraints)) {
                    if (!vp->intersectionFailed())
                        cannotBranch = true;
                    else
                        vp->setIntersectionFailed(false);
        ...
        if (instanceofDetected) {
            ...
            if (objectRefConstraint) {
                ...
                if (instanceClass) {
                    ...
                    if (!newConstraint && applyType)
                        removeBranch = true;
                    else if (newConstraint && !isVirtualGuardNopable && !vp->addBlockConstraint(instanceofObjectRef, newConstraint, NULL, false))
                        removeBranch = true;
                    if (removeBranch) {
                        if (!vp->intersectionFailed())
                            cannotFallThrough = true;
                ...
                if (applyType && !isVirtualGuardNopable && !vp->addBlockConstraint(instanceofObjectRef, newConstraint, NULL, false)) {
                    if (!vp->intersectionFailed())
                        cannotFallThrough = true;
                    else
                        vp->setIntersectionFailed(false);
        ...
        //
            ...
            if (sig && sig[0] == '[') {
                ...
                //
                    ...
                    else if (!vp->comp()->compileRelocatableCode() && !(TR::Options::getCmdLineOptions()->getOption(TR_DisableArrayStoreCheckOpts)) && arrayComponentClass && objectClass && vp->fe()->isInstanceOf(objectClass, arrayComponentClass, true, true)) {
                        if (vp->trace())
