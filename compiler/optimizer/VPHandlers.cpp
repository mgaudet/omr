...
static void checkForNonNegativeAndOverflowProperties(
    TR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* constraint = NULL)
{
    if (!constraint) {
        bool isGlobal;
                    ...
                    if (node->getOpCode().isLoadIndirect() && !node->getSymbolReference()->hasKnownObjectIndex()) {
                        ...
                        TR::SymbolReference* improvedSymRef
                            = vp->comp()->getSymRefTab()->findOrCreateSymRefWithKnownObject(
                                node->getSymbolReference(), knot->getPointerLocation(knownObject->getIndex()));
...
bool reduceLongOpToIntegerOp(TR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* nodeConstraint)
{
    ...
    // to integers and insert conversion trees.
    // Conversion trees are not free.
    // LoadExtensions (and other codegen peepholes) should in theory catch uneeded conversions (like widening) as
    ...
    if (dataType == TR::Int16 && node->getOpCode().isIndirect() && node->getSymbol()->isArrayShadowSymbol()
        ...
        if (index->getOpCode().isLoadConst() && array->getOpCode().isIndirect()) {
            ...
            if (baseVPConstraint && baseVPConstraint->isConstString()) {
                ...
                uintptrj_t string
                    = *(uintptrj_t*)constString->getSymRef()->getSymbol()->castToStaticSymbol()->getStaticAddress();
    ...
    if (objectReferenceLocation) {
        ...
        if (isString && symRef->getSymbol()->isStatic()) {
            ...
            vp->addGlobalConstraint(
                node, TR::VPClass::create(vp, TR::VPConstString::create(vp, symRef), TR::VPNonNullObject::create(vp),
            ...
            if (haveClassLookaheadInfo) {
                ...
                if (sym->isStatic() && sym->isFinal()) {
                    ...
                    if (classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
                        ...
                        if ((*(void**)p != 0) && constrainAloadCriticalSection.hasVMAccess()) {
                            int32_t arrLength = *((int32_t*)(((uintptrj_t) * (void***)p)
                                + (uintptrj_t)vp->fe()->getOffsetOfContiguousArraySizeField()));

                            if (arrLength == 0 && TR::Compiler->om.useHybridArraylets()) {
                                arrLength = *((int32_t*)(((uintptrj_t) * (void***)p)
                                    + (uintptrj_t)vp->fe()->getOffsetOfDiscontiguousArraySizeField()));
                            }
                            ...
                            if (arrayFieldInfo && arrayFieldInfo->isDimensionInfoValid()) {
                                ...
                                if (firstDimension >= 0) {
                                    ...
                                    if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                                        ...
                                        if (elementSize != 0) {
                                            if (vp->trace())
                                                traceMsg(vp->comp(), "Using class lookahead info to find out non null, "
                                                                     "array dimension, and object location\n");
                                            vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp, firstDimension,
                            ...
                            if (*sig == '[' || sig[0] == 'L') {
                                ...
                                if (elementSize != 0) {
                                    constraint = TR::VPClass::create(vp, (TR::VPClassType*)constraint, NULL, NULL,
                                        TR::VPArrayInfo::create(vp, 0, elementSize == 0 ? TR::getMaxSigned<TR::Int32>()
...
static TR::Node* findArrayIndexNode(TR::ValuePropagation* vp, TR::Node* node, int32_t stride)
{
    ...
    if (usingAladd) {
        ...
        if ((offset->getOpCode().isAdd() && (offset->getSecondChild()->getOpCode().isLoadConst()
        ...
        if (base && node->getOpCode().hasSymbolReference()
            ...
            if (!base->isFixedClass()) {
                vp->addBlockOrGlobalConstraint(
                    node, TR::VPClass::create(vp, NULL, nonnull, NULL, NULL,
                ...
                vp->addBlockOrGlobalConstraint(
                    ...
                    TR::VPClass::create(vp,
                        ...
                        nonnull, NULL, NULL, TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                    isGlobal);
            } else {
                vp->addBlockOrGlobalConstraint(
                    node, TR::VPClass::create(vp, TR::VPFixedClass::create(vp, base->getClass()), nonnull, NULL, NULL,
                              TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                    isGlobal);
            }
            node->setIsNonNull(true);
...

                {
                    ...
                    if (constrainIaloadCriticalSection.hasVMAccess()) {
                        ...
                        if (TR::Compiler->cls.isClassInitialized(vp->comp(), clazz)) {
                            ...
                            // precisely
                            // but clearly others make invalid assumptions on seeing this fixed class constraint so be
                            // conservative
                            ...
                            if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), clazz)
                                ...
                                vp->addBlockOrGlobalConstraint(
                                    node, TR::VPClass::create(vp, TR::VPFixedClass::create(vp, clazz),
                                              TR::VPNonNullObject::create(vp), NULL, NULL,
                                              TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                                    isGlobal);
                            } else {
                                vp->addBlockOrGlobalConstraint(
                                    node, TR::VPClass::create(vp, TR::VPResolvedClass::create(vp, clazz),
                                              TR::VPNonNullObject::create(vp), NULL, NULL,
                                              TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                                    isGlobal);
                            }
            ...
            if (symRef == vp->comp()->getSymRefTab()->findJavaLangClassFromClassSymbolRef()) {
                ...
                vp->addBlockOrGlobalConstraint(
                    node, TR::VPClass::create(vp, TR::VPKnownObject::createForJavaLangClass(vp, knownObjectIndex),
                              TR::VPNonNullObject::create(vp), NULL, NULL,
                              TR::VPObjectLocation::create(vp, TR::VPObjectLocation::JavaLangClassObject)),
                    isGlobal);
                node->setIsNonNull(true);
                ...
                if (!TR::Compiler->cls.isInterfaceClass(vp->comp(), componentClass)
                    ...
                    vp->addBlockOrGlobalConstraint(
                        node, TR::VPClass::create(vp, TR::VPFixedClass::create(vp, componentClass),
                                  TR::VPNonNullObject::create(vp), NULL, NULL,
                                  TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                        isGlobal);
                } else {
                    vp->addBlockOrGlobalConstraint(
                        node, TR::VPClass::create(vp, TR::VPResolvedClass::create(vp, componentClass),
                                  TR::VPNonNullObject::create(vp), NULL, NULL,
                                  TR::VPObjectLocation::create(vp, TR::VPObjectLocation::J9ClassObject)),
                        isGlobal);
                }
                ...
                if (sym->isStatic() && sym->isFinal()) {
                    ...
                    if ((classOfStatic != vp->comp()->getSystemClassPointer() && isClassInitialized
                        ...
                        TR::VMAccessCriticalSection constrainIaloadCriticalSection(
                            vp->comp(), TR::VMAccessCriticalSection::tryToAcquireVMAccess);
                        void* p = symbol->getStaticAddress();
                        if ((*(void**)p != 0) && constrainIaloadCriticalSection.hasVMAccess()) {
                            arrLength = *((int32_t*)(((uintptrj_t) * (void***)p)
                                + (uintptrj_t)vp->fe()->getOffsetOfContiguousArraySizeField()));
                            ...
                            if (sig && (len > 0) && (sig[0] == '[' || sig[0] == 'L')) {
                                ...
                                if (elementSize != 0) {
                                    // vp->addGlobalConstraint(node, TR::VPNonNullObject::create(vp));
                                    // vp->addGlobalConstraint(node, TR::VPArrayInfo::create(vp, arrLength, arrLength,
                                    // elementSize));
                                    foundInfo = true;
                                    isFixed = true;
                                }
            ...
            if (jlClass) {
                if (classBlock != jlClass) {
                    ...
                    if (*sig == '[' || sig[0] == 'L') {
                        ...
                        if (elementSize != 0) {
                            constraint = TR::VPClass::create(vp, (TR::VPClassType*)constraint, NULL, NULL,
                                TR::VPArrayInfo::create(vp, 0, elementSize == 0
                                                                     ? TR::getMaxSigned<TR::Int32>()
...
TR::Node* constrainWrtBar(TR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if ((node->getOpCodeValue() == TR::wrtbari) && !node->getSymbolReference()->getSymbol()->isArrayShadowSymbol()
...
TR::Node* constrainCheckcast(TR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (result == 1 && ((node->getOpCodeValue() != TR::checkcastAndNULLCHK)
...
TR::Node* constrainCheckcastNullChk(TR::ValuePropagation* vp, TR::Node* node) { return constrainCheckcast(vp, node); }
...
static TR::VPConstraint* createIntConstConstraint(TR::ValuePropagation* vp, int32_t n)
{
    return TR::VPIntConst::create(vp, n);
}
static TR::VPConstraint* createIntRangeConstraint(TR::ValuePropagation* vp, int32_t l, int32_t h)
{
    return TR::VPIntRange::create(vp, l, h);
}
static TR::VPConstraint* createLongConstConstraint(TR::ValuePropagation* vp, int64_t n)
{
    return TR::VPLongConst::create(vp, n);
}
static TR::VPConstraint* createLongRangeConstraint(TR::ValuePropagation* vp, int64_t l, int64_t h)
{
...
void addValidRangeBlockOrGlobalConstraint(
    TR::ValuePropagation* vp, TR::Node* node, FUNC createRange, FUNC2 processValue, T low, T high, bool childGlobal)
{
...
static TR::Node* constrainHighestOneBitAndLeadingZerosHelper(TR::ValuePropagation* vp, TR::Node* node, A getConst,
...
template <typename A, typename B, typename C, typename D, typename E, typename F, typename G, typename T>
static TR::Node* constrainLowestOneBitAndTrailingZerosHelper(TR::ValuePropagation* vp, TR::Node* node, A getConst,
    B getRange, C getValue, D getValues, E createConst, F createRange, G processValue, T MIN_VALUE, T MAX_VALUE)
{
...
static void refineSymbolReferenceForProfiledToOverridden(
    TR::ValuePropagation* vp, TR::Node* callNode, TR::SymbolReference* newSymRef)
{
...
static TR::MethodSymbol* refineMethodSymbolInCall(TR::ValuePropagation* vp, TR::Node* node, TR::SymbolReference* symRef,
            ...
            if (constraint && constraint->getKnownObject() && knot) {
                ...
                if (resolvedMethod) {
                    ...
                    if (vp->optimizer()->getOptimization(OMR::methodHandleInvokeInliningGroup)->requested()) {
                        if (vp->trace())
                            traceMsg(vp->comp(), "Not inlining call [%p] because the MethodHandle.invoke inlining "
                                                 "group will do a better job\n",
                                node);
                        return;
                    }
                } else {
...

    {
        ...
        if (operand
            && ((operand->isClassObject() == TR_yes)
                   || (operand->getClassType() && ((operand->getClassType()->isArray() == TR_no)
            ...
            */
            ...
            if (constraint && constraint->getKnownObject() && constraint->isClassObject() == TR_yes
                ...
                if (j9class && TR::Compiler->cls.isClassInitialized(vp->comp(), j9class)
                    && performTransformation(vp->comp(), "%s Remove TR::sun_misc_Unsafe_ensureClassInitialized call, "
...
#ifdef J9_PROJECT_SPECIFIC
void getHelperSymRefs(TR::ValuePropagation* vp, TR::Node* curCallNode, TR::SymbolReference*& getHelpersSymRef,
...
TR::Node* transformCloneCallSetup(
    TR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* constraint, bool isDirectCall)
{
                ...
                if (constraint) {
                    if (constraint->isFixedClass()) {
                        ...
                        if (!vp->comp()->compileRelocatableCode()) {
                            if (constraint->getClassType() && constraint->getClassType()->isArray() == TR_no
                                ...
                                vp->_objectCloneTypes.add(new (vp->trStackMemory())
                                        TR::ValuePropagation::ObjCloneInfo(constraint->getClass(), true));
                        ...
                        if (enableDynamicObjectClone && constraint->getClassType()->isArray() == TR_no
                            ...
                            vp->_objectCloneCalls.add(vp->_curTree);
                            vp->_objectCloneTypes.add(new (vp->trStackMemory())
                                    TR::ValuePropagation::ObjCloneInfo(constraint->getClass(), false));
                        }
                    }
                }
    ...
    int64_t rangeMax, int64_t& resultLow, int64_t& resultHigh, bool allowZeroInDivisorRange)
{
    ...
    // integer division by 0 is
    // meaningless and should be handled by an exception when executed. In other words, this method determines the range
    // of the result
        ...
        if (lhs->asIntConst() && rhs->asIntConst()) {
            ...
            if (lhsInt && rhsInt) {
                ...
                if (isUnsigned) {
                    ...
                    range_min = 0;
                    range_max = TR::getMaxSigned<TR::Int32>(); // we don't support unsigned range constraint yet. Only
                                                               // support upto TR::getMaxSigned<TR::Int32>() for
                                                               // unsigned
                } else {
...
TR::Node* removeRedundantREM(TR::ValuePropagation* vp, TR::Node* node, TR::VPConstraint* nodeConstraint,
    TR::VPConstraint* firstChildConstraint, TR::VPConstraint* secondChildConstraint)
{
    ...
    if (powerOfTen) {
        ...
        if (!isUnsigned && fcPrecision <= refinedPrecision
            ...
            // prec=1 dividend that is negative
            // when interpreted as unsigned will actually have 10 digits of precision (-1 would be 0xFFffFFff =
            // 4,294,967,295)
            //
...
static TR::Node* distributeShift(
    TR::ValuePropagation* vp, TR::Node* node, int32_t shiftAmount, TR::VPConstraint*& constraint)
{
    ...
    if (rhs && rhs->asIntConst()) {
        ...
        if (constraint) {
            ...
            if (longShr) {
                ...
                // we must reduce this shift value. Integer shifts are
                // masked by 0x1f in java, and if the first child is integer constrained, a shift by an amount in this
                        ...
                        if ((vftLoad->getOpCodeValue() == TR::aloadi)
                            ...
                            if (objConstraint && objConstraint->getClassType()
                                ...
                                if (objConstraint->getClassType()->isArray() == TR_yes)
                                    constraint
                                        = TR::VPIntConst::create(vp, rhs->asIntConst()->getLowInt() /*, isUnsigned*/);
                                else
                                    constraint = TR::VPIntConst::create(vp, 0 /*, isUnsigned*/);
                            } else {
...
static TR::Node* constrainWidenToLong(
    TR::ValuePropagation* vp, TR::Node* node, int64_t low, int64_t high, bool isUnsigned)
{
    if (findConstant(vp, node))
        return node;
    constrainChildren(vp, node);
...
void replaceWithSmallerType(TR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (performTransformation(comp, "%sReplace [" POINTER_PRINTF_FORMAT "] and [" POINTER_PRINTF_FORMAT
...
//
static TR::VPConstraint* genTypeResult(TR::ValuePropagation* vp, TR::VPConstraint* objectRefConstraint,
    TR::VPConstraint* classConstraint, bool& applyType, bool isInstanceOf)
{
    ...
    if (objectRefConstraint) {
        ...
        if (instanceClass) {
            ...
            // one of the types could be null
            ...
            else if (isInstanceOf && !objectRefConstraint->getClassType() && !castIsClassObject
                && !castIsInterfaceImplementedByClass && objectRefConstraint->isNonNullObject()
                ...
                if ((objectRefConstraint && (objectRefConstraint->isClassObject() == TR_yes)) || castIsClassObject)
        ...
        if (applyType) {
            ...
            newConstraint = TR::VPClass::create(vp, newType, classConstraint->getClassPresence(), NULL, NULL, loc);
...
static void addDelayedConvertedGuard(TR::Node* node, TR::Node* callNode, TR::ResolvedMethodSymbol* methodSymbol,
    TR_VirtualGuard* oldVirtualGuard, TR::ValuePropagation* vp, TR_VirtualGuardKind guardKind,
    TR_OpaqueClassBlock* objectClass)
{
    ...
    vp->_convertedGuards.add(new (vp->trStackMemory())
            TR::ValuePropagation::VirtualGuardInfo(vp, oldVirtualGuard, newGuard, newGuardNode, callNode));
}
...
static TR::Node* constrainIfcmpeqne(TR::ValuePropagation* vp, TR::Node* node, bool branchOnEqual)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (!vp->comp()->compileRelocatableCode() && vp->comp()->isOptServer() && vp->lastTimeThrough()
                ...
                if (typeConstraint) {
                    ...
                    if (clazzToBeInitialized
                        && (((clazzNameLen == 35)
                               ...
                               || ((clazzNameLen == 41)
                                      && !strncmp(clazzToBeInitialized, "Lcom/ibm/websphere/ras/TraceEnabledToken;",
                                             clazzNameLen)))
                        && performTransformation(vp->comp(), "%sUsing side-effect guard to fold away condition on a "
                                                             "load from an uninitializec class  %s [%p]\n",
                        ...
                        vp->_classesToCheckInit.add(new (vp->trStackMemory())
                                TR::ValuePropagation::ClassInitInfo(vp, clazzToBeInitializedCopy, clazzNameLen));
                    }
                }
            }
        }
            ...
            case TR_MethodTest: {
                ...
                /*(vtableEntryNode->getSymbolReference() == vp->comp()->getSymRefTab()->findVftSymbolRef()))*/
                {
                    ...
                    if (ignoreVirtualGuard && classConstraint && classConstraint->isFixedClass()) {
                        ...
                        bool testForEquality = (node->getOpCodeValue() == TR::ifacmpeq);
                        traceMsg(vp->comp(), "TR_MethodTest: node=%p, vtableEntryNode=%p, clazz=%p, vftOffset=%d, "
                                             "vftEntry=%p, childrenAreEqual=%d, testForEquality=%d\n",
        ...
        if (instanceofDetected) {
            ...
            if (objectRefConstraint) {
                ...
                if (instanceClass) {
                    ...
                    if (newTypeConstraint && instanceofDetectedAndFixedType && newTypeConstraint->asResolvedClass()
    ...
    if (cannotBranch && performTransformation(vp->comp(), "%sRemoving conditional branch [%p] %s\n", OPT_DETAILS, node,
        ...
        // [0x7fffd5110a40] bci=[0,0,314] rc=0 vc=56 vn=49 sti=- udi=- nc=2 flg=0x1020
        // n233n       iaload <vtable-entry-symbol> [#552 Shadow +768] [flags 0x10607 0x0 ]
        // [0x7fffd51109b0] bci=[-1,0,2302] rc=1 vc=56 vn=7 sti=- udi=- nc=1
        // n232n         iaload <vft-symbol> [#449 Shadow] [flags 0x18607 0x0 ] (X>=0 )
        // [0x7fffd5110968] bci=[-1,0,2302] rc=1 vc=56 vn=6 sti=- udi=- nc=1 flg=0x100
        // n3n             ==>aload
        // n234n       aconst 0x7fd070 (methodPointerConstant )
        // [0x7fffd51109f8] bci=[0,0,314] rc=1 vc=56 vn=5 sti=- udi=- nc=0 flg=0x2000

        bool isGlobal;
        TR::VPConstraint* intersected = vp->getConstraint(lhsChild->getFirstChild()->getFirstChild(), isGlobal);

        intersected = (intersected) ? intersected->intersect(constraintFromMethodPointer, vp) : intersected
        ...
        if (!disableProfiled2Overridden && callNode && vp->lastTimeThrough()) {
                    ...
                    == TR_yes) /*the object class may be less specific than callClass*/
            {
                ...
                if (methodSymbol
                    ...
                    // classes in the given hierarchy
                    // most likely rvm will also be NULL in this case, but it is better to not rely on this implicit
                    ...
                    /*
                      ...
                      invariantargumentpreexisence downstream
...
TR::Node* constrainIfcmpeq(TR::ValuePropagation* vp, TR::Node* node) { return constrainIfcmpeqne(vp, node, true); }

TR::Node* constrainIfcmpne(TR::ValuePropagation* vp, TR::Node* node) { return constrainIfcmpeqne(vp, node, false); }
...
static TR::Node* simplifyInequality(TR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild,
...
static TR::Node* constrainIfcmplessthan(
    TR::ValuePropagation* vp, TR::Node* node, TR::Node* lhsChild, TR::Node* rhsChild, bool orEqual)
{
...
TR::Node* constrainCmpeq(TR::ValuePropagation* vp, TR::Node* node) { return constrainCmpeqne(vp, node, true); }

TR::Node* constrainCmpne(TR::ValuePropagation* vp, TR::Node* node) { return constrainCmpeqne(vp, node, false); }
...
TR::Node* constrainLoadaddr(TR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (symbol->isAddressOfClassObject()) {
        ...
        // add a ClassObject property
        TR::VPConstraint* constraint = TR::VPClass::create(vp, TR::VPClassType::create(vp, symRef, true), NULL, NULL,
...
TR::Node* constrainBndChkWithSpineChk(TR::ValuePropagation* vp, TR::Node* node)
{
    ...
    if (sizeConstraint && !vp->comp()->generateArraylets()) {
        ...
        if (elementSize > 0 && !TR::Compiler->om.isDiscontiguousArray(sizeConstraint->getHighInt(), elementSize)
            ...
            && performTransformation(vp->comp(), "%sRemoving unnecessary spine check from composite node [%p] : min "
    ...
    if (node->getNumChildren() == 3) {
        ...
        if (!alreadyAdded)
            vp->_scalarizedArrayCopies.add(
                new (vp->comp()->trStackMemory()) TR::ValuePropagation::TR_TreeTopNodePair(vp->_curTree, node));
    } else {
        vp->createExceptionEdgeConstraints(TR::Block::CanCatchArrayStoreCheck, NULL, node);
    }
