...
bool TR_ValueNumberInfo::congruentNodes(TR::Node* node, TR::Node* entryNode)
{
    ...
    if (node->getType().isBCD()) {
        if (!node->isDecimalSizeAndShapeEquivalent(entryNode)) {
            if (trace())
                traceMsg(comp(), "BCD node %s (%p) and BCD entryNode %s (%p) have size/shape mismatch -- do not "
        ...
        if (!node->isSignStateEquivalent(entryNode)) {
            if (trace() || comp()->cg()->traceBCDCodeGen())
                traceMsg(comp(), "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- do not "
                                 "consider as matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            return false;
        }
        ...
        if (trace())
            traceMsg(comp(), "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch -- do not "
                             "consider as matching\n",
                node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
        return false;
    } else if (node->chkOpsCastedToBCD() && node->castedToBCD() != entryNode->castedToBCD()) {
        if (trace())
            traceMsg(comp(), "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD %d "
...
void TR_ValueNumberInfo::initializeNode(TR::Node* node, int32_t& negativeValueNumber)
{
    ...
    for (entry = _hashTable._buckets[hashValue]; entry; entry = entry->_next) {
        ...
        if (node->getOpCode().isSetSignOnNode() && node->getSetSign() != entryNode->getSetSign()) {
            if (trace())
                traceMsg(comp(), "Nodes %s (%p) and entryNode %s (%p) with setSignOnNode have sign mismatch -- do not "
                                 "consider as matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            continue;
        }
        ...
        if (node->getType().isBCD()) {
            ...
            if (!node->isSignStateEquivalent(entryNode)) {
                if (trace() || comp()->cg()->traceBCDCodeGen())
                    traceMsg(comp(), "x^x : BCD node %s (%p) and BCD entryNode %s (%p) have sign state mismatch -- do "
            ...
            && node->getDecimalFraction() != entryNode->getDecimalFraction()) {
            if (trace())
                traceMsg(comp(), "fracConv node %s (%p) and fracConv entryNode %s (%p) have fraction mismatch -- do "
                                 "not consider as matching\n",
                    node->getOpCode().getName(), node, entryNode->getOpCode().getName(), entryNode);
            continue;
        } else if (node->chkOpsCastedToBCD() && node->castedToBCD() != entryNode->castedToBCD()) {
            if (trace())
                traceMsg(comp(), "castedToBCD mismatch : node %s (%p) castedToBCD %d and entryNode %s (%p) castedToBCD "
                        ...
                        if (!otherUseDefiningLoads.IsZero() && (otherUseDefiningLoads == definingLoads)) {
                            ...
                            if (baseVN == -1
                                ...
                                if (value >= 0) {
                                    ...
                                    if (trace()) {
                                        traceMsg(comp(), "  Change value number for load %d at [%p] to value number %d "
                                                         "of load %d at [%p] reached by same dominating loads (defs)\n",
