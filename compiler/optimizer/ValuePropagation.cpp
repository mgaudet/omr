...
TR::ValuePropagation::Relationship* TR::ValuePropagation::createRelationship(
...
void TR::ValuePropagation::freeRelationship(Relationship* rel) { _relationshipCache.add(rel); }
...
TR::ValuePropagation::StoreRelationship* TR::ValuePropagation::createStoreRelationship(
...
//
TR::VPConstraint* TR::ValuePropagation::addConstraintToList(TR::Node* node, int32_t valueNumber, int32_t relative,
    ...
    TR::Node* node, int32_t valueNumber, TR::VPConstraint* constraint, int32_t relative)
{
    ...
    // assumes (as on WCode or binopt).
    // This condition should rarely show up in WCode or binopt (as Simplifier transformations will homogenize the types)
    // but can happen, for example, when an aggregate symbol is
...
TR::VPConstraint* TR::ValuePropagation::addBlockConstraint(
...
//
TR::ValuePropagation::GlobalConstraint* TR::ValuePropagation::GlobalConstraint::create(
...
//
inline uint32_t TR::ValuePropagation::hashGlobalConstraint(int32_t valueNumber)
{
...
//
TR::ValuePropagation::EdgeConstraints* TR::ValuePropagation::EdgeConstraints::create(
...
TR::ValuePropagation::EdgeConstraints* TR::ValuePropagation::createEdgeConstraints(
...
TR::ValuePropagation::StoreRelationship* TR::ValuePropagation::findStoreRelationship(
...
TR::ValuePropagation::Relationship* TR::ValuePropagation::findConstraintInList(
...
TR::ValuePropagation::StoreRelationship* TR::ValuePropagation::findStoreValueConstraint(
...
TR::ValuePropagation::StoreRelationship* TR::ValuePropagation::findStoreConstraint(
    int32_t valueNumber, TR::Symbol* symbol)
{
    return findStoreValueConstraint(valueNumber, symbol, _curConstraints);
}

TR::ValuePropagation::StoreRelationship* TR::ValuePropagation::findStoreEdgeConstraint(
...
void TR::ValuePropagation::checkTypeRelationship(
...
TR::VPConstraint* TR::ValuePropagation::mergeDefConstraints(
    ...
    for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
        ...
        if (!hasBeenStored(defValueNumber, sym, _curConstraints)) {
            ...
            for (loopInfo = _loopInfo; loopInfo && loopInfo != container; loopInfo = loopInfo->_parent) {
                if (trace())
                    traceMsg(comp(), "   %s [%p] looking at back edges in loop %d for def %s [%p]: ",
                        node->getOpCode().getName(), node, loopInfo->_loop->getNumber(), defNode->getOpCode().getName(),
                        defNode);
        ...
        if (defs.IsZero())
            ...
            for (cursor.SetToFirstOne(); cursor.Valid(); cursor.SetToNextOne()) {
                ...
                if ((defVN != incrementVN) && !notInLoop) {
                    ...
                    isInductionVariable = false;
                    if (trace())
                        traceMsg(comp(), "      ivInfo is invalid for sym [%p], def [%d] node %p does not have the "
                                         "same value number as another increment\n",
                            sym, defVN, defNode);
                    break;
                }
    ...
    if (comp()->getRecompilationInfo()) {
        if (TR::Compiler->target.is64Bit()) {
            TR_LongValueInfo* valueInfo
                = (TR_LongValueInfo*)TR_ValueProfiler::getProfiledValueInfo(arrayCopyNode, comp());
...
TR::TreeTop* TR::ArraycopyTransformation::tryToSpecializeForLength(TR::TreeTop* tt, TR::Node* arraycopyNode)
{
    ...
    if (specificLength >= 0 && performTransformation(comp(), "%sSpecializing arraycopy %s for length of %d bytes\n",
    ...
    TR::SymbolReference* srcObjRef, TR::SymbolReference* dstObjRef)
{
    ...
    TR::TreeTop* specificIfCheck
        = TR::TreeTop::create(comp(), TR::Node::createif(lengthNode->getType().isInt32() ? TR::ificmpne : TR::iflcmpne,
...
int32_t TR::ArraycopyTransformation::perform()
{
    ...
    bool mustSpecializeForDirection
        = comp()->cg()->getSupportsPostProcessArrayCopy() && comp()->cg()->getSupportsPrimitiveArrayCopy();
...
void TR::ValuePropagation::createNewBucketForArrayIndex(ArrayLengthToVersion* array,
...
TR::TreeTop* TR::ValuePropagation::createReferenceArrayNodeWithoutFlags(TR::TreeTop* tree, TR::TreeTop* newTree,
...
void TR::ValuePropagation::transformReferenceArrayCopyWithoutCreatingStoreTrees(TR_TreeTopWrtBarFlag* arrayTree,
    TR::SymbolReference* srcObjRef, TR::SymbolReference* dstObjRef, TR::SymbolReference* srcRef,
    TR::SymbolReference* dstRef, TR::SymbolReference* lenRef)
{
...
void TR::ValuePropagation::transformStringCtors(VPTreeTopPair* treeTopPair)
{
    ...
    /* Before									After
      ...
      treetop2								treetop2
          ...
          new										treetop
          loadaddr								acall
      Method[java/lang/String.cachedConstantString]
                                                                                  aload
                                                                                  aload
                                                                                  iconst
    */
...
void TR::ValuePropagation::replacePackedArrayLoad(
    TR::Node* loadNode, TR::Node* packedNode, TR::Node* curNode, vcount_t visitCount)
{
    ...
    for (uint16_t i = 0; i < curNode->getNumChildren(); i++) {
        ...
        if (childNode == loadNode) {
            ...
            if (trace())
                traceMsg(comp(),
                    "Replacing child %d of node [" POINTER_PRINTF_FORMAT "] from load node [" POINTER_PRINTF_FORMAT
...
void TR::ValuePropagation::doDelayedTransformations()
{
                        ...
                        if (guard && (guard->getSuccessors().size() == 2) && guard->getExit()) {
                            ...
                            if (guardNode) {
                                ...
                                canBeRemoved = virtualGuard->canBeRemoved()
                                    && callNode->getOpCode().isCallDirect(); // do not remove guards for indirect calls.
                                                                             // If the ifacmpne handler
                            ...
                            if (guardNode && guardNode->isProfiledGuard()) {
                                ...
                                if (typeCompatibleStatus == TR_no) {
                                    // ifacmpne
                                    if ((guardType == 1)
                                        && performTransformation(comp(), "%sChanging branch (guard) %p in block_%d to "
    ...
    for (predictedThrow = throwsIt.getFirst(); predictedThrow; predictedThrow = throwsIt.getNext()) {
        ...
        TR::Node* firstReferenceOfCatchObject
            = predictedCatchBlock->findFirstReference(excpSymRef->getSymbol(), comp()->incVisitCount());
        ...
        if (firstNode->getOpCodeValue() == TR::astore && firstNode->getFirstChild() == firstReferenceOfCatchObject) {
            ...
            if (treeTop->getNode()->getOpCodeValue() == TR::NULLCHK) {
                ...
                treeTop
                    = TR::TreeTop::create(comp(), treeTop, TR::Node::createWithSymRef(TR::astore, 1, 1,
                                                               node->getFirstChild(), firstNode->getSymbolReference()));
            } else {
    ...
    TR_OpaqueClassBlock* optClnClass
        = comp()->fe()->getClassFromSignature("packagePlaceholder/OptCloneClass", 32, comp()->getCurrentMethod());
    ...
    if (clnCallNode && optClnClass) {
        ...
        for (method = clnit.getCurrent(); method; method = clnit.getNext()) {
            ...
            if (!strncmp(sig, "packagePlaceholder/OptCloneClass.OptimizedClone(Ljava/lang/Object;Ljava/lang/"
                              "Object;)Ljava/lang/Object;",
                    len)) {
                optClnClassSymRef = comp()->getSymRefTab()->findOrCreateMethodSymbol(
                    clnCallNode->getSymbolReference()->getOwningMethodIndex(), -1, method, TR::MethodSymbol::Special);
                break;
            }
        }
        ...
        for (; clnCallNode && optClnClassSymRef; clnCallNode = clnNodeIt.getNext()) {
            ...
            if (srcClass) {
                ...
                if (!performTransformation(comp(),
                        "%sCreating new object tree [%p] to populate second parameter of optimized clone call [%p] \n",
                    ...
                    //
                    if (performTransformation(comp(),
                            "%sTransforming Object.Clone call node [%p] to J9VMIntenrals.optimizedClone call \n",
                            OPT_DETAILS, clnCallNode)) {
                        TR::Node* newNode = TR::Node::createWithSymRef(clnCallNode, TR::New, 1,
                            comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(
                                clnCallNode->getFirstChild()->getSymbolReference()->getOwningMethodSymbol(comp())));
                        TR::Node* classPtr = TR::Node::createWithSymRef(clnCallNode, TR::loadaddr, 0,
                            comp()->getSymRefTab()->findOrCreateClassSymbol(
                                clnCallNode->getFirstChild()->getSymbolReference()->getOwningMethodSymbol(comp()), 0,
                                srcClass));
    ...
    for (ClassInitInfo* cii = _classesToCheckInit.getFirst(); cii; cii = cii->getNext()) {
        ...
        if ((origFirst->getOpCodeValue() == TR::aload) && origFirst->getSymbol()->isStatic()) {
