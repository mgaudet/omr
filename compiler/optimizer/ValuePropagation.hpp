...
//
    ...
    public:
        ...
        virtual ValueConstraint* allocate(int32_t key) { return _vp->createValueConstraint(key, NULL, NULL); }
    ...
    public:
        ...
        ValueConstraint* allocate(int32_t key) { return _vp->createValueConstraint(key, NULL, NULL); }
    ...
    struct TR_PackedArrayLoad {
        ...
        TR_PackedArrayLoad(TR::TreeTop* loadTT, TR::Node* loadNode, TR::Node* clazzPointer, TR::Node* packedDS)
            : _loadTT(loadTT)
            , _loadNode(loadNode)
    ...
    struct InductionVariable : public TR_Link<InductionVariable> {
        InductionVariable(
            TR::Symbol* sym, TR::Node* entryDef, int32_t incrVN, TR::VPConstraint* incr, TR::ValuePropagation* vp)
            : _symbol(sym)
            , _entryDef(entryDef)
            , _entryConstraint(0)
            , _incrementVN(incrVN)
    ...
    struct VirtualGuardInfo : public TR_Link<VirtualGuardInfo> {
        VirtualGuardInfo(
            TR::ValuePropagation* vp, TR_VirtualGuard* vgOld, TR_VirtualGuard* vgNew, TR::Node* newGNode, TR::Node* cn)
