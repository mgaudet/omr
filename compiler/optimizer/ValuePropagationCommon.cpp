...
TR::ValuePropagation::ValueConstraint* TR::ValuePropagation::createValueConstraint(
...
void TR::ValuePropagation::transformArrayCopyCall(TR::Node* node)
{
    ...
    //
    if ((primitiveTransform || fullTransform) && !comp()->getOption(TR_DisableArrayCopyOpts)
    ...
    if (transformReferenceArray && !comp()->generateArraylets()) {
        ...
        //
        TR_ResolvedMethod* newMethod = findResolvedClassMethod(
            comp(), "Ljava/lang/System;", "arraycopy", "([Ljava/lang/Object;I[Ljava/lang/Object;II)V");
    }
#else
...
TR::TreeTop* TR::ValuePropagation::createPrimitiveArrayNodeWithoutFlags(TR::TreeTop* tree, TR::TreeTop* newTree,
...
#ifdef J9_PROJECT_SPECIFIC
void TR::ValuePropagation::generateArrayTranslateNode(TR::TreeTop* callTree, TR::TreeTop* arrayTranslateTree,
...
TR::TreeTop* TR::ValuePropagation::createConverterCallNodeAfterStores(TR::TreeTop* tree, TR::TreeTop* origTree,
    TR::SymbolReference* srcRef, TR::SymbolReference* dstRef, TR::SymbolReference* lenRef,
...
void TR::ValuePropagation::generateRTArrayNodeWithoutFlags(TR_RealTimeArrayCopy* rtArrayCopy,
...
TR::TreeTop* TR::ValuePropagation::createArrayCopyVCallNodeAfterStores(TR::TreeTop* tree, TR::SymbolReference* srcRef,
...
//
TR::TreeTop* TR::ValuePropagation::createAndInsertStoresForArrayCopySpineCheck(TR_ArrayCopySpineCheck* checkInfo)
{
...
TR::TreeTop* TR::ValuePropagation::createArrayCopyCallForSpineCheck(TR_ArrayCopySpineCheck* checkInfo)
{
...
void TR::ValuePropagation::transformRealTimeArrayCopy(TR_RealTimeArrayCopy* rtArrayCopyTree)
{
    ...
    if (!comp()->getOptions()->realTimeGC() && ((rtArrayCopyTree->_flag & NEED_WRITE_BARRIER) != 0)) {
        ...
        if (comp()->useCompressedPointers()) {
            ...
            prevTree = TR::TreeTop::create(
                comp(), prevTree, TR::Node::createCompressedRefsAnchor(
    ...
    TR::TreeTop* callTree, TR::ValuePropagation::ObjCloneInfo* cloneInfo)
{
    ...
    if (callTree->getNode()->getOpCode().isCheck()) {
        TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK, "it only makes sense to do this object clone "
                                                                        "transform for checks with a NULLCHK - how did "
                                                                        "we get here otherwise??");
        TR::Node* passthrough = TR::Node::create(callNode, TR::PassThrough, 1, callNode->getChild(0));
        callTree->insertBefore(
            TR::TreeTop::create(comp(), TR::Node::createWithSymRef(callNode, callTree->getNode()->getOpCodeValue(), 1,
    ...
    if (!cloneInfo->_isFixed) {
        ...
        TR::TreeTop* compareTree = TR::TreeTop::create(
            comp(),
            TR::Node::createif(TR::ifacmpne, TR::Node::createWithSymRef(callNode, TR::aloadi, 1, objNode,
    ...
    TR::Node* loadaddr = TR::Node::createWithSymRef(
        callNode, TR::loadaddr, 0, comp()->getSymRefTab()->findOrCreateClassSymbol(
                                       callNode->getSymbolReference()->getOwningMethodSymbol(comp()), 0, j9class));
    TR::Node::recreateWithoutProperties(
        callNode, TR::New, 1, comp()->getSymRefTab()->findOrCreateNewObjectSymbolRef(
...
void TR::ValuePropagation::transformArrayCloneCall(TR::TreeTop* callTree, TR_OpaqueClassBlock* j9arrayClass)
{
    ...
    if (callTree->getNode()->getOpCode().isCheck()) {
        TR_ASSERT(callTree->getNode()->getOpCodeValue() == TR::NULLCHK, "it only makes sense to do this array clone "
                                                                        "transform for checks with a NULLCHK - how did "
                                                                        "we get here otherwise??");
        TR::Node* passthrough = TR::Node::create(callNode, TR::PassThrough, 1, callNode->getChild(0));
        callTree->insertBefore(
...
void TR::ValuePropagation::transformConverterCall(TR::TreeTop* callTree)
{
    ...
    TR::TreeTop* actt
        = TR::TreeTop::create(comp(), TR::Node::createWithSymRef(callNode, TR::asynccheck, 0,
    ...
    if (TR::Compiler->target.cpu.isZ()) {
        ...
        if (threshold > 0) {
            TR::TreeTop* thresholdTestTree
                = TR::TreeTop::create(comp(), TR::Node::createif(TR::ificmplt, TR::Node::createLoad(callNode, lenRef),
