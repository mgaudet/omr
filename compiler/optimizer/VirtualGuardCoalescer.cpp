...
int32_t TR_VirtualGuardTailSplitter::perform()
{
    ...
    if (globalSplit && !comp()->isProfilingCompilation() && _numGuards >= 15 && numBlocks / 5 < _numGuards) {
        ...
        //
        splitGlobal();
        printf("---$$$--- GlobalSplit %d,%d %s\n", nodeCount, comp()->getNodeCount(), comp()->signature());
        return 10;
    }
...
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block* first, TR::Block* last)
{
    ...
    while (next != last) {
        ...
        if (_cfg->getStructure()) {
            next->getStructureOf()->getParent()->asRegion()->addSubNode(new (trHeapMemory()) TR_StructureSubGraphNode(
    ...
    TR::Block* block, VGInfo* parent)
{
    ...
    if (call && guardNode->isProfiledGuard()) {
        ...
        TR_AddressInfo* valueInfo = (TR_AddressInfo*)profileManager->getValueInfo(
            node->getByteCodeInfo(), comp(), TR_ValueProfileInfoManager::allProfileInfoKinds, NotBigDecimalOrString);
                ...
                if (thisChild->getOpCode().isLoadVarDirect()
                    ...
                    while (guardBlock) {
                        while (prevTree && prevTree != startOfExtendedBlock->getEntry()) {
                            ...
                            // prevNode, startOfExtendedBlock->getEntry(), startOfExtendedBlock->getNumber());
        ...
        if (thisExpr) {
            ...
            for (treeTop = storeBlock->getEntry(); treeTop != tt; treeTop = treeTop->getNextTreeTop()) {
                ...
                if (cursorNode->getOpCode().isResolveCheck()) {
                    ...
                    if (cursorNode->getOpCodeValue() == TR::treetop || cursorNode->getOpCode().isNullCheck()
                    ...
                    if (!cursorNode->getOpCode().isLoad()) {
                        ...
                        if (!cursorNode->getOpCode().isCall() && ((cursorNode->getOpCodeValue() != TR::monent)
...
/// dumpOptDetails(comp(), "Success: Can rematerialize for guard %p\n", node);
            ...
            TR::Node* newThis = TR::Node::createWithSymRef(
                thisExpr, thisExpr->getOpCodeValue(), thisExpr->getNumChildren(), thisExpr->getSymbolReference());
...
bool TR_VirtualGuardTailSplitter::isLegalToClone(TR::Node* node, vcount_t visitCount)
{
    ...
    // dumpOptDetails(comp(), "Reached 1 canonicalized for node %p\n", node);
    bool canonicalized = false;
    if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray
