...
int32_t TR_VirtualGuardTailSplitter::perform()
{
    ...
    if (globalSplit && !comp()->isProfilingCompilation() && _numGuards >= 15 && numBlocks / 5 < _numGuards) {
...
void TR_VirtualGuardTailSplitter::eliminateColdVirtualGuards(TR::TreeTop* treetop)
{
    ...
    while (treetop) {
        ...
        if (virtualGuard && block->isCold() && performTransformation(comp(), "%s remove guard from cold block_%d\n", OPT_DETAILS, block->getNumber())) {
...
void TR_VirtualGuardTailSplitter::transformLinear(TR::Block* first, TR::Block* last)
{
    ...
    while (next != last) {
        ...
        if (_cfg->getStructure()) {
            next->getStructureOf()->getParent()->asRegion()->addSubNode(new (trHeapMemory()) TR_StructureSubGraphNode(new (trHeapMemory()) TR_BlockStructure(comp(), clone->getNumber(), clone)));
        }
    ...
    for (; block; block = block->getNextBlock()) {
        ...
        if (node->isNopableInlineGuard()) {
            ...
            if (callNode && callNode->getOpCode().isCallIndirect()) {
                ...
                if (thisChild->getOpCode().isLoadVarDirect() && thisChild->getSymbolReference()->getSymbol()->isAuto()) {
                    while (guardBlock) {
                        while (prevTree && prevTree != startOfExtendedBlock->getEntry()) {
                            ...
                            if (prevNode->getOpCode().isStoreDirect() && prevNode->getSymbolReference() == thisChild->getSymbolReference()) {
                    ...
                    if (symRef && storeBlock) {
                        ...
                        if (thisExpr->getOpCode().isLoadVarDirect() && (thisExpr->getSymbolReference() == symRef)) {
                            TR::Block* cursorBlock = storeBlock;
                            TR::TreeTop* cursorPrev = prevTree->getPrevTreeTop();
                            while (cursorPrev && (cursorPrev != startOfExtendedBlock->getEntry())) {
                                TR::Node* cursorPrevNode = cursorPrev->getNode();
                                if (cursorPrevNode->getOpCode().isStoreDirect() && (cursorPrevNode->getSymbolReference() == symRef)) {
        ...
        if (thisExpr) {
            ...
            for (treeTop = storeBlock->getEntry(); treeTop != tt; treeTop = treeTop->getNextTreeTop()) {
                ...
                if (cursorNode->getOpCode().isResolveCheck()) {
                    ...
                    if (cursorNode->getOpCode().isIndirect() && cursorNode->getOpCode().isLoadVarOrStore())
                        continue;
                } else {
                    if (cursorNode->getOpCodeValue() == TR::treetop || cursorNode->getOpCode().isNullCheck() || cursorNode->getOpCodeValue() == TR::ArrayStoreCHK) {
                        cursorNode = cursorNode->getFirstChild();
                    }
                    ...
                    if (!cursorNode->getOpCode().isLoad()) {
                        ...
                        if (!cursorNode->getOpCode().isCall() && ((cursorNode->getOpCodeValue() != TR::monent) && (cursorNode->getOpCodeValue() != TR::monexit)))
                            continue;
                    } else {
...
bool TR_VirtualGuardTailSplitter::isLegalToClone(TR::Node* node, vcount_t visitCount)
{
    ...
    //dumpOptDetails(comp(), "Reached 1 canonicalized for node %p\n", node);
    bool canonicalized = false;
    if (node->getOpCode().isCall() || node->getOpCodeValue() == TR::New || node->getOpCodeValue() == TR::newarray || node->getOpCodeValue() == TR::anewarray || node->getOpCodeValue() == TR::multianewarray || node->getOpCodeValue() == TR::MergeNew)
        return false;
