    ...
    if (stopTheWorld) {
        ...
        for (TR::TreeTop* tt = start; tt && tt != guardCandidate; tt = tt->getNextTreeTop()) {
            if (tt->getNode()->getOpCodeValue() != TR::BBStart && tt->getNode()->getOpCodeValue() != TR::BBEnd
                && !tt->getNode()->chkIsPrivatizedInlinerArg() && !tt->getNode()->isNopableInlineGuard())
                return false;
        ...
        if (isMergeableGuard(guard1)) {
            ...
            for (TR::Block* nextBlock = block->getNextBlock(); nextBlock; nextBlock = nextBlock->getNextBlock()) {
                ...
                if (guard1->getInlinedSiteIndex() == guard2->getInlinedSiteIndex()) {
                    ...
                    if (trace())
                        traceMsg(comp(), "  Guard1 cold destination block_%d has guard2 cold destination block_%d as "
                                         "its only successor - proceeding with guard merging\n",
                            cold1->getNumber(), cold2->getNumber());
                } else {
                    if (trace())
                        traceMsg(comp(), "  Cold1 block_%d and cold2 block_%d of guard2 [%p] in unknown relationship - "
                ...
                if (!performTransformation(comp(),
                        "%sRedirecting guard [%p] in block_%d to parent guard cold block_%d\n", OPT_DETAILS, guard2,
                        guard2Block->getNumber(), cold1->getNumber()))
                    continue;
                ...
                if (guard2Tree != guard2Block->getFirstRealTreeTop()) {
                    ...
                    // TODO We can safely leave code ahead of an HCR guard in place if no runtime guards are going to be
                    // moved or if
                    //     there is no data dependence between the code moved from ahead of the runtime guards and the
                    //     code ahead
                    //     of the HCR guard. This opt will increase the chances of a patch point not requiring nops
                    //     (perf opportunity).
                ...
                if (insertPoint != guard2Block->getPrevBlock()) {
                    TR::DebugCounter::incStaticDebugCounter(
                        comp(), TR::DebugCounter::debugCounterName(comp(), "headMerger/%s_%s/(%s)",
                                    guard1->isHCRGuard() ? "hcr" : "runtime", guard2->isHCRGuard() ? "hcr" : "runtime",
                                    comp()->signature()));
                    cfg->setStructure(NULL);
