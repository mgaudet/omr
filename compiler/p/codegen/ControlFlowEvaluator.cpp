...
{
    return compareLongsForOrder(branchOp, reversedBranchOp, node->getBranchDestination()->getNode()->getLabel(), node,
    ...
    TR::LabelSymbol* dstLabel, TR::Node* node, TR::CodeGenerator* cg, bool isHint, bool likeliness)
{
    ...
    if (firstChild->getOpCodeValue() == TR::b2i && firstChild->getOpCodeValue() == secondChild->getOpCodeValue() &&
        ...
        // it instead
        // If a child is in a register, regardless of whether or not we evaluated it, we don't have to decrement the
        // grandchild
...
static void lookupScheme3(TR::Node* node, bool unbalanced, TR::CodeGenerator* cg)
{
    ...
    else // if 32bit mode
    {
        ...
        cg->getAheadOfTimeCompile()->getRelocationList().push_front(new (cg->trHeapMemory())
                TR_PPCPairedRelocation(rel1, rel2, (uint8_t*)recordInfo, TR_AbsoluteMethodAddressOrderedPair, node));
