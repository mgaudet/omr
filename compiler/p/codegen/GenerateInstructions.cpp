    ...
    } else // otherwise, do generic load/store sequences for moving from fpr2gpr or gpr2fpr
    {
        ...
        if (mode == fpr2gprHost64)
            ...
            cursor = generateTrg1MemInstruction(cg, TR::InstOpCode::lfd, node, reg0,
                new (cg->trHeapMemory()) TR::MemoryReference(node, location->getSymbolReference(), 8, cg), cursor);
        else if (mode == gprSp2fpr)
    ...
    if (!disableFlipCompare && TR::Compiler->target.cpu.id() == TR_PPCp6 && TR::InstOpCode(op).isCompare()
        ...
        if (s1reg->containsInternalPointer()
            || (!TR::InstOpCode(op).isFloat()
                   && registerRecentlyWritten(s1reg, preced ? preced : comp->getAppendInstruction()))) {
