    ...
    , _globalLabelsEncountered(getTypedAllocator<TR::LabelSymbol*>(self()->comp()->allocator()))
{
    ...
    _specializedEpilogues
        = self()->comp()->getOption(TR_EnableSpecializedEpilogues) && self()->comp()->getOptLevel() >= hot;
    ...
    if (_specializedEpilogues) {
        _blocksThatModifyRegister = (TR_BitVector**)self()->trMemory()->allocateHeapMemory(
            TR::RealRegister::NumRegisters * sizeof(TR_BitVector*), TR_Memory::CodeGenerator);
    ...
    self()->setStackPointerRegister(
        self()->machine()->getPPCRealRegister(_linkageProperties->getNormalStackPointerRegister()));
    self()->setMethodMetaDataRegister(
        self()->machine()->getPPCRealRegister(_linkageProperties->getMethodMetaDataRegister()));
    ...
    if (self()->getSupportsTM() && !disableTMDCAS && TR::Compiler->target.is64Bit()
...
static bool isWAWOrmrPeepholeCandidateInstr(TR::Instruction* instr)
{
    if (instr != NULL && !instr->willBePatched()
        && (!instr->isLabel() || (instr->getNode()->getOpCodeValue() == TR::BBStart
...
bool OMR::Power::CodeGenerator::processInstruction(
    TR::Instruction* instr, TR_BitVector** registerUsageInfo, int32_t& blockNum, int32_t& blockMarker, bool traceIt)
{
    TR::Instruction* ppcInstr = instr;
    ...
    case OMR::Instruction::IsTrg1Mem: {
        int32_t tgtRegNum
            = ((TR::RealRegister*)((TR::PPCTrg1Instruction*)ppcInstr)->getTargetRegister())->getRegisterNumber();
        if (traceIt)
    ...
    case OMR::Instruction::IsControlFlow: {
        ...
        for (int32_t i = 0; i < numTargets; i++) {
            int32_t tgtRegNum = ((TR::RealRegister*)((TR::PPCControlFlowInstruction*)ppcInstr)->getTargetRegister(i))
                                    ->getRegisterNumber();
            if (traceIt)
...
uint32_t OMR::Power::CodeGenerator::isPreservedRegister(int32_t regIndex)
{
    ...
    //
    for (int32_t pindex = TR::RealRegister::gr15; pindex <= TR::RealRegister::LastGPR; pindex++) {
...
bool OMR::Power::CodeGenerator::isLabelInstruction(TR::Instruction* instr) { return (instr->isLabel()); }
...
bool OMR::Power::CodeGenerator::isAlignmentInstruction(TR::Instruction* instr) { return false; }

TR::Instruction* OMR::Power::CodeGenerator::splitEdge(TR::Instruction* instr, bool isFallThrough, bool needsJump,
    TR::Instruction* newSplitLabel, TR::list<TR::Instruction*>* jmpInstrs, bool firstJump)
{
    ...
    if (!isFallThrough && needsJump) {
        ...
        TR::Instruction* i
            = generateLabelInstruction(self(), TR::InstOpCode::b, jmpLocation->getNode(), targetLabel, jmpLocation);
        traceMsg(self()->comp(), "split edge jmp instr at [%p]\n", i);
    }
    return cursor;
}

TR::Instruction* OMR::Power::CodeGenerator::splitBlockEntry(TR::Instruction* instr)
{
...
int32_t OMR::Power::CodeGenerator::computeRegisterSaveDescription(TR_BitVector* regs, bool populateInfo)
{
    uint32_t rsd = 0;
...
void OMR::Power::CodeGenerator::processIncomingParameterUsage(TR_BitVector** registerUsageInfo, int32_t blockNum)
{
...
void OMR::Power::CodeGenerator::updateSnippetMapWithRSD(TR::Instruction* instr, int32_t rsd)
{
    ...
    if (data.warmEstimate) {
        ...
        if (self()->getGlobalLabelsEncountered().empty()) {
            ...
            self()->setEstimatedColdLength(
                self()->getEstimatedColdLength() + (i * sizeof(uint32_t) * 6)); // 2 instructions + traceback table
...
static int32_t identifyFarConditionalBranches(
    int32_t estimate, TR::CodeGenerator* cg, int32_t* incWarmSize, int32_t warmSize)
{
...
void OMR::Power::CodeGenerator::buildRegisterMapForInstruction(TR_GCStackMap* map)
{
    ...
    for (int i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; i++) {
        TR::RealRegister* realReg = self()->machine()->getPPCRealRegister((TR::RealRegister::RegNum)i);
...
int32_t OMR::Power::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart, bool isWarm)
{
    if (isWarm
        && !self()->comp()->getOption(
               TR_EnableHCR)) // PPC currently should not have any constant data snippets as warm.
...
int32_t OMR::Power::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    if (node->getOpCode().isSwitch()) {
        ...
        return (TR::Compiler->target.is64Bit() ? _numGPR : _numGPR
    ...
    TR::Instruction* liInstruction, TR::LabelSymbol* startLabel, TR::LabelSymbol* endLabel, int32_t deltaToStartLabel)
{
    ...
    // If the register deps are not done properly then the register assigner may insert instructions after the call but
    // before
...
{
    return self()->machine()->getPPCRealRegister((TR::RealRegister::RegNum)self()->getGlobalRegister(i))->getState()
            ...
            if (typeAddress == -1) {
                ...
                if (typeAddress == TR_DataAddress) {
                    if (doAOTRelocation) {
                        ...
                        // printf("TreeEvaluator.cpp:137, inlinedSiteIndex: %d, tempSR->getCPIndex(): %x,
    ...
    TR::Instruction* secondInstruction, int16_t typeAddress, int32_t value)
{
    ...
    if (typeAddress == TR_DataAddress) {
        ...
        self()->addAOTRelocation(new (self()->trHeapMemory()) TR_32BitExternalOrderedPairRelocation(
    ...
    TR::Node* node, TR::LabelSymbol* label, TR::Instruction* firstInstruction, TR::Instruction* secondInstruction)
{
    ...
    self()->addRelocation(new (self()->trHeapMemory())
            TR_PPCPairedLabelAbsoluteRelocation(firstInstruction, secondInstruction, NULL, NULL, label));
}
    ...
    TR::Node* node, TR::LabelSymbol* label, TR::Register* tempReg, TR::Instruction** q)
{
    ...
    if (!self()->comp()->compileRelocatableCode()) {
        self()->addRelocation(
            new (self()->trHeapMemory()) TR_PPCPairedLabelAbsoluteRelocation(q[0], q[1], q[2], q[3], label));
    }

    self()->addAOTRelocation(
        new (self()->trHeapMemory()) TR_BeforeBinaryEncodingExternalRelocation(q[0], (uint8_t*)(label),
...
bool OMR::Power::CodeGenerator::is64BitProcessor() { return TR::Compiler->target.cpu.getPPCis64bit(); }
