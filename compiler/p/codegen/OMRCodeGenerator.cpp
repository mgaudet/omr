    ...
    , _globalLabelsEncountered(getTypedAllocator<TR::LabelSymbol*>(self()->comp()->allocator()))
{
    ...
    if (self()->getSupportsTM() && !disableTMDCAS && TR::Compiler->target.is64Bit() && !TR::Options::useCompressedPointers()) {
...
bool OMR::Power::CodeGenerator::isTargetSnippetOrOutOfLine(TR::Instruction* instr, TR::Instruction** start, TR::Instruction** end)
{
    TR::PPCLabelInstruction* labelInstr = (TR::PPCLabelInstruction*)instr;
    TR::LabelSymbol* targetLabel = labelInstr->getLabelSymbol();
    TR_PPCOutOfLineCodeSection* oiCursor = self()->findOutLinedInstructionsFromLabel(targetLabel);
...
{
    if ((TR::Compiler->target.cpu.getPPCSupportsAES() || (TR::Compiler->target.cpu.getPPCSupportsVMX() && TR::Compiler->target.cpu.getPPCSupportsVSX())) && !self()->comp()->getOptions()->getOption(TR_DisableAESInHardware))
        return true;
    else
        return false;
}
...
{
    return TR::Compiler->target.cpu.getPPCSupportsVSX() && !self()->comp()->getOptions()->getOption(TR_DisableSIMDUTF16LEEncoder);
}

bool OMR::Power::CodeGenerator::getSupportsEncodeUtf16BigWithSurrogateTest()
{
    return TR::Compiler->target.cpu.getPPCSupportsVSX() && !self()->comp()->getOptions()->getOption(TR_DisableSIMDUTF16BEEncoder);
}
    ...
    TR::Instruction* secondInstruction)
{
    ...
    self()->getAheadOfTimeCompile()->getRelocationList().push_front(new (self()->trHeapMemory()) TR_PPCPairedRelocation(firstInstruction, secondInstruction, (uint8_t*)recordInfo,
        TR_AbsoluteMethodAddressOrderedPair, node));

    self()->addRelocation(new (self()->trHeapMemory()) TR_PPCPairedLabelAbsoluteRelocation(firstInstruction, secondInstruction, NULL, NULL, label));
}
