...
namespace Power {
    ...
    public:
        ...
        int32_t maxPositiveDisplacement()
        {
            return UPPER_IMMED;
        }
        ...
        bool restoreRegister(TR::RealRegister::RegNum reg, int32_t blockNumber)
        {
            if (_specializedEpilogues)
                return _reachingBlocks->_blockAnalysisInfo[blockNumber]->intersects(*_blocksThatModifyRegister[reg]) || _blocksThatModifyRegister[reg]->get(blockNumber);
            else
                return true;
        }
