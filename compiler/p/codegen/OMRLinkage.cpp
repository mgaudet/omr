...
TR::MemoryReference* OMR::Power::Linkage::getOutgoingArgumentMemRef(int32_t argSize, TR::Register* argReg,
    TR::InstOpCode::Mnemonic opCode, TR_PPCMemoryArgument& memArg, uint32_t length)
{
    ...
    TR::MemoryReference* result = new (self()->trHeapMemory())
        TR::MemoryReference(machine->getPPCRealRegister(properties.getNormalStackPointerRegister()),
...
#define REGNUM(ri) ((TR::RealRegister::RegNum)(ri))
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        // TR_ASSERT(!fsd || (fsd && lri >=0 && (ai<0 || hasToBeOnStack(paramCursor))), "Registers not saved on Full
        // Speed Debug");
        if (lri >= 0) {
            ...
            if (((ai < 0 || self()->hasToBeOnStack(paramCursor)) && !fsd) || (fsd && saveOnly)) {
                ...
                case TR::Address:
                    if (!all_saved)
                        cursor
                            = generateMemSrc1Instruction(self()->cg(), TR::InstOpCode::Op_st, firstNode,
            ...
            case TR::Address:
                if (freeScratchable.isSet(ai)) {
                    cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::Op_load, firstNode,
                        REAL_REGISTER(REGNUM(ai)), new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset,
                                                            TR::Compiler->om.sizeofReferenceAddress(), self()->cg()),
                        cursor);
                    freeScratchable.reset(ai);
                } else {
            ...
            if (hasToLoadFromStack && numIntArgs < properties.getNumIntArgRegs()) {
                ...
                if (TR::Compiler->target.is64Bit())
                    ...
                    if (numIntArgs < properties.getNumIntArgRegs() - 1) {
                        argRegister
                            = machine->getPPCRealRegister(properties.getIntegerArgumentRegister(numIntArgs + 1));
                        cursor = generateTrg1MemInstruction(self()->cg(), TR::InstOpCode::lwz, firstNode, argRegister,
                            new (self()->trHeapMemory()) TR::MemoryReference(stackPtr, offset + 4, 4, self()->cg()),
                            cursor);
                    }
