    ...
    , _staticRelocation(NULL)
{
    ...
    if (symbol->isParm()) {
        //        setOffset(getOffset() + cg->getLinkage()->calculateActualParameterOffset(0,
    ...
    TR::Instruction* currentInstruction, uint8_t* cursor, TR::CodeGenerator* cg)
{
    ...
    base = (self()->getBaseRegister() == NULL) ? cg->machine()->getPPCRealRegister(TR::RealRegister::gr0)
    ...
    if (TR::Compiler->target.is64Bit() && self()->isTOCAccess()) {
        ...
        if (self()->getUnresolvedSnippet() != NULL) {
            ...
            cg->addRelocation(new (cg->trHeapMemory())
                    TR_24BitLabelRelativeRelocation(cursor, getUnresolvedSnippet()->getSnippetLabel()));
    ...
    TR::Node* node, TR::SymbolReference* ref, bool isStore, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        if (ref->isUnresolved() || comp->compileRelocatableCode()) {
            ...
            specialCase = symbol->isStatic() && (symbol->getType().isInt64()) && comp->getOption(TR_AOT);
            ...
            if (comp->getOption(TR_AOT)) {
                ...
                staticRelocation = new (cg->trHeapMemory())
                    TR_PPCPairedRelocation(rel1, rel2, (uint8_t*)recordInfo, relocationKind, node);
