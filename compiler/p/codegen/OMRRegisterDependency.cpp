...
public:
    ...
    void addDependency(TR::RegisterDependency& dep, uint32_t index)
    {
        ...
        if (rr != TR::RealRegister::NoReg && rr != TR::RealRegister::SpilledReg) {
            ...
            TR_ASSERT(targetTable[rr] == MAP_NIL
                    || // TODO: Figure out where the same dep is being added more than once!
        ...
        if (assignedReg) {
            ...
            TR_ASSERT(rr >= 0 && rr < TR::RealRegister::NumRegisters,
                "Register number %d used as index but out of range", rr);
            TR_ASSERT(assignedTable[rr] == MAP_NIL
                    || // TODO: Figure out where the same dep is being added more than once!
    ...
    TR::RegisterDependency* getDependencyWithTarget(TR::RealRegister::RegNum rr)
    {
        ...
        TR_ASSERT(
            rr != TR::RealRegister::SpilledReg, "Multiple dependencies can map to 'SpilledReg', can't return just one");
...
void OMR::Power::RegisterDependencyConditions::bookKeepingRegisterUses(TR::Instruction* instr, TR::CodeGenerator* cg)
{
    ...
    // exists for instruction selection
    // so we don't want these associations to clobber the associations that were set in main line code, which are more
    // important
...
static TR::RegisterDependency* findDependencyChainHead(TR::RegisterDependency* dep, TR_PPCRegisterDependencyMap& map)
{
    // *this    swipeable for debugging purposes
...
static void assignFreeRegisters(TR::Instruction* currentInstruction, TR::RegisterDependency* dep,
    TR_PPCRegisterDependencyMap& map, TR::CodeGenerator* cg)
{
    // *this    swipeable for debugging purposes
    TR::Machine* machine = cg->machine();
    ...
    while (dep) {
        TR_ASSERT(machine->getPPCRealRegister(dep->getRealRegister())->getState() == TR::RealRegister::Free,
...
static void assignContendedRegisters(TR::Instruction* currentInstruction, TR::RegisterDependency* dep,
    TR_PPCRegisterDependencyMap& map, bool depsBlocked, TR::CodeGenerator* cg)
{
    // *this    swipeable for debugging purposes
    TR::Machine* machine = cg->machine();
    ...
    TR_RegisterKinds kindToBeAssigned, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
{
    ...
    TR_ASSERT(num_gprs <= (TR::RealRegister::LastGPR - TR::RealRegister::FirstGPR + 1
                              - machine->getNumberOfLockedRegisters(TR_GPR)),
        "Too many GPR dependencies, unable to assign");
    TR_ASSERT(num_fprs <= (TR::RealRegister::LastFPR - TR::RealRegister::FirstFPR + 1
                              - machine->getNumberOfLockedRegisters(TR_FPR)),
        "Too many FPR dependencies, unable to assign");
    TR_ASSERT(num_vrfs <= (TR::RealRegister::LastVRF - TR::RealRegister::FirstVRF + 1
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (comp->getOption(TR_DisableOOL) || (!(cg->isOutOfLineColdPath()) && !(cg->isOutOfLineHotPath()))) {
            TR_ASSERT(dependentRegister->getAssignedRegister(), "assignedRegister can not  be NULL");
        }
