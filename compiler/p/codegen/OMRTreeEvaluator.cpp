...
TR::Instruction* loadConstant(
    TR::CodeGenerator* cg, TR::Node* node, int64_t value, TR::Register* trgReg, TR::Instruction* cursor, bool isPicSite)
{
...
{
    bool result
        = (checkNode->getOpCode().isCall() || (checkNode != node && (checkNode->getOpCodeValue() == TR::aloadi
...
TR::Register* OMR::Power::TreeEvaluator::vstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    case TR::VectorDouble:
        ...
        break;
    default:
        TR_ASSERT(false, "unknown vector store TRIL: unrecognized vector type %i\n", node->getDataType());
        return NULL;
    }
...
static TR::Register* inlineArrayCmp(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateConditionalBranchInstruction(
        cg, TR::InstOpCode::blt, node, /* residueLoopStartLabel */ mid2Label, condRegToBeUsed);
...
// call the "C" helper, handle the exception case
    ...
    if (verboseArrayCopy)
        fprintf(stderr, "arraycopy [0x%p] isReferenceArrayCopy:[%d] isForwardArrayCopy:[%d] "
    ...
    if (node->getOpCodeValue() == TR::aloadi
        ...
        if (!disableHotFieldPrefetch && prefetchOffset >= 0) // Prefetch based on hot field information
        {
            ...
            if (!canSkipNullChk) {
                ...
                TR::Register* tempReg = cg->allocateRegister();
                TR::RegisterDependencyConditions* deps
                    = new (cg->trHeapMemory()) TR::RegisterDependencyConditions(1, 2, cg->trMemory());
                ...
                if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
                    TR::MemoryReference* tempMR
                        = new (cg->trHeapMemory()) TR::MemoryReference(targetRegister, prefetchOffset, 8, cg);
                    generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tempReg, tempMR);
                } else {
                    TR::MemoryReference* tempMR
                        = new (cg->trHeapMemory()) TR::MemoryReference(targetRegister, prefetchOffset, 4, cg);
                    generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, tempReg, tempMR);
                }

                TR::MemoryReference* targetMR
                    = new (cg->trHeapMemory()) TR::MemoryReference(tempReg, (int32_t)0, 4, cg);
                ...
                {
                    TR_PrefetchInfo* pf = new (cg->trHeapMemory()) TR_PrefetchInfo(
                        cg->getCurrentEvaluationTreeTop(), useTree, node, useNode, prefetchOffset, false);
                        ...
                        if (!fromRegLoad) {
                            ...
                            if (indexObject->getOpCode().isLoadConst()) {
                                ...
                                if (len >= LOWER_IMMED && len <= UPPER_IMMED)
                                    generateTrg1Src1ImmInstruction(
                                        cg, TR::InstOpCode::addi, node, pointerReg, baseReg, len);
                                else {
                                    indexReg = cg->allocateRegister();
                                    loadConstant(cg, node, len, indexReg);
                                    generateTrg1Src2Instruction(
                                        cg, TR::InstOpCode::add, node, pointerReg, baseReg, indexReg);
                                    cg->stopUsingRegister(indexReg);
                                }
                            } else
                                generateTrg1Src2Instruction(
            ...
            if (!optDisabled) {
                ...
                if (pointerReg && (heapTop > comp->getOptions()->getHeapBase())) // Check for gencon
                {
                    ...
                    if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, temp3Reg,
                                new (cg->trHeapMemory()) TR::MemoryReference(pointerReg,
                                                           (int32_t)TR::Compiler->om.sizeofReferenceField(), 8, cg));
                        else
                            generateTrg1MemInstruction(
                                cg, TR::InstOpCode::ld, node, temp3Reg,
                                new (cg->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 8, cg));
                    } else {
                        if (!prefetch2Ahead)
                            generateTrg1MemInstruction(cg, TR::InstOpCode::lwz, node, temp3Reg,
                                new (cg->trHeapMemory()) TR::MemoryReference(pointerReg,
                            ...
                            generateTrg1MemInstruction(
                                ...
                                new (cg->trHeapMemory()) TR::MemoryReference(
                                    pointerReg, (int32_t)(TR::Compiler->om.sizeofReferenceField() * 2), 4, cg));
                    }
                    ...
                    TR::MemoryReference* targetMR
                        = new (cg->trHeapMemory()) TR::MemoryReference(temp3Reg, (int32_t)0, 4, cg);
                    targetMR->forceIndexedForm(node, cg);
                    generateMemInstruction(
                        cg, TR::InstOpCode::dcbt, node, targetMR); // use dcbt for prefetch next element
        ...
        if (!disableStringObjPrefetch && 0 && node->getSymbolReference() && !node->getSymbolReference()->isUnresolved()
            ...
            if (fieldName && strstr(fieldName, "Ljava/lang/String;")) {
                TR::MemoryReference* targetMR
                    = new (cg->trHeapMemory()) TR::MemoryReference(targetRegister, (int32_t)0, 4, cg);
                targetMR->forceIndexedForm(node, cg);
                generateMemInstruction(cg, TR::InstOpCode::dcbt, node, targetMR);
            }
        }
    ...
    if (node->getOpCodeValue() == TR::aloadi
        ...
        if (!disableTreeMapPrefetch) {
            ...
            if (TR::Compiler->target.is32Bit()) {
                if (!(firstChild && firstChild->getOpCodeValue() == TR::aiadd && firstChild->isInternalPointer() &&
                        ...
                        strstr(comp->fe()->sampleSignature(node->getOwningMethod(), 0, 0, cg->trMemory()),
        ...
        if (!optDisabled) {
            ...
            if (baseReg && indexReg && loopSize > 0) {
                TR::Register* tempReg = cg->allocateRegister();
                generateTrg1Src1ImmInstruction(cg, TR::InstOpCode::addi, node, tempReg, indexReg,
                    (int32_t)(prefetchElementStride * TR::Compiler->om.sizeofReferenceField()));
                if (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) {
                    TR::MemoryReference* targetMR
                        = new (cg->trHeapMemory()) TR::MemoryReference(baseReg, tempReg, 8, cg);
                    generateTrg1MemInstruction(cg, TR::InstOpCode::ld, node, tempReg, targetMR);
                } else {
                    TR::MemoryReference* targetMR
                        = new (cg->trHeapMemory()) TR::MemoryReference(baseReg, tempReg, 4, cg);
        ...
        if (prefetchOffset >= 0) {
            TR::MemoryReference* targetMR = new (cg->trHeapMemory())
                TR::MemoryReference(targetRegister, prefetchOffset, TR::Compiler->om.sizeofReferenceAddress(), cg);
            targetMR->forceIndexedForm(node, cg);
            generateMemInstruction(cg, TR::InstOpCode::dcbt, node, targetMR);
        }
    }
