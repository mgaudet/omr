...
void addPrefetch(TR::CodeGenerator* cg, TR::Node* node, TR::Register* targetRegister)
{
    ...
    if (node->getOpCodeValue() == TR::aloadi || (TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && node->getOpCodeValue() == TR::l2a)) {
        ...
        if (!disableHotFieldPrefetch && prefetchOffset >= 0) // Prefetch based on hot field information
        {
            ...
            while (!disableDelayPrefetch && tt && (tt->getNode()->getOpCodeValue() != TR::BBEnd)) {
                checkNode = tt->getNode();
                if (checkNode->getOpCodeValue() == TR::ifacmpeq && checkNode->getFirstChild() == node && checkNode->getSecondChild()->getDataType() == TR::Address && checkNode->getSecondChild()->isZero()) {
                    canSkipNullChk = true;
                }
                tt = tt->getNextTreeTop();
            }
            ...
            if (!disableHotFieldNextElementPrefetch) {
                // 32bit
                if (TR::Compiler->target.is32Bit()) {
                    if (!(firstChild->getOpCodeValue() == TR::aiadd && firstChild->getFirstChild() && firstChild->isInternalPointer()) && !(firstChild->getOpCodeValue() == TR::aRegLoad && firstChild->getSymbolReference()->getSymbol()->isInternalPointer())) {
                        optDisabled = true;
                    } else {
                ...
                // 64bit CR
                    ...
                    if (!(firstChild->getOpCodeValue() == TR::iu2l && firstChild->getFirstChild() && firstChild->getFirstChild()->getOpCodeValue() == TR::iloadi && firstChild->getFirstChild()->getNumChildren() == 1)) {
                        optDisabled = true;
                    } else {
                        fromRegLoad = true;
        ...
        if (!disableStringObjPrefetch && 0 && node->getSymbolReference() && !node->getSymbolReference()->isUnresolved() && (node->getSymbolReference()->getSymbol()->getKind() == TR::Symbol::IsShadow) && (node->getSymbolReference()->getCPIndex() >= 0)) {
            int32_t len;
    ...
    if (node->getOpCodeValue() == TR::aloadi || (TR::Compiler->target.is64Bit() && comp->useCompressedPointers() && node->getOpCodeValue() == TR::iloadi)) {
        TR::Node* firstChild = node->getFirstChild();
        optDisabled = false;
