...
uint32_t mappedOffsetToFirstLocal(TR::CodeGenerator* cg, const TR_PPCLinkageProperties& linkage)
{
...
const TR_PPCLinkageProperties& TR_PPCSystemLinkage::getProperties() { return _properties; }
...
void TR_PPCSystemLinkage::initPPCRealRegisterLinkage()
{
    ...
    machine->getPPCRealRegister((TR::RealRegister::RegNum)icount)
        ->setAssignedRegister(machine->getPPCRealRegister((TR::RealRegister::RegNum)icount));
...
uintptr_t TR_PPCSystemLinkage::calculateActualParameterOffset(uintptr_t o, TR::ParameterSymbol& p)
{
...
void TR_PPCSystemLinkage::mapParameters(TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
...
void TR_PPCSystemLinkage::mapSingleAutomatic(TR::AutomaticSymbol* p, uint32_t& stackIndex)
{
    // *this    swipeable for debugging purposes
    size_t align = 1;
...
void TR_PPCSystemLinkage::createPrologue(TR::Instruction* cursor, List<TR::ParameterSymbol>& parmList)
{
    // *this    swipeable for debugging purposes
    TR::Machine* machine = cg()->machine();
    ...
    for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
        ...
        cursor = generateMemSrc1Instruction(cg(), TR::InstOpCode::stfd, firstNode,
            new (trHeapMemory()) TR::MemoryReference(sp, argSize, 8, cg()), machine->getPPCRealRegister(regIndex),
            cursor);
    }
    ...
    if (bodySymbol->isEHAware() || machine->getPPCRealRegister(TR::RealRegister::cr2)->getHasBeenAssignedInMethod()
    ...
    for (regIndex = TR::RealRegister::LastFPR; regIndex >= savedFirst;
        ...
        if (cg()->restoreRegister(regIndex, blockNumber))
            cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::lfd, currentNode,
                machine->getPPCRealRegister(regIndex), new (trHeapMemory()) TR::MemoryReference(sp, saveSize, 8, cg()),
                cursor);
    }
    ...
    if (savedFirst <= TR::RealRegister::LastGPR) {
        if (TR::Compiler->target.cpu.id() == TR_PPCgp || TR::Compiler->target.is64Bit()
            ...
            for (regIndex = TR::RealRegister::LastGPR; regIndex >= savedFirst;
                ...
                if (cg()->restoreRegister(regIndex, blockNumber))
                    cursor = generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, currentNode,
                        machine->getPPCRealRegister(regIndex), new (trHeapMemory()) TR::MemoryReference(sp, saveSize,
                                                                   TR::Compiler->om.sizeofReferenceAddress(), cg()),
                        cursor);
            }
        else {
            ...
            cursor = generateTrg1MemInstruction(cg(),
                ...
                machine->getPPCRealRegister(savedFirst), new (trHeapMemory()) TR::MemoryReference(sp, saveSize,
                                                             4 * (TR::RealRegister::LastGPR - savedFirst + 1), cg()),
                cursor);
        }
    }
...
int32_t TR_PPCSystemLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* dependencies)
    ...
    for (i = firstArgumentChild; i < callNode->getNumChildren(); i++) {
        ...
        case TR::Aggregate: {
            ...
            if (numIntegerArgs >= properties.getNumIntArgRegs())
                ...
                memArgs += (properties.getNumIntArgRegs() - numIntegerArgs) > slots ? 0 : slots
                        - (properties.getNumIntArgRegs() - numIntegerArgs);
...
void TR_PPCSystemLinkage::buildDirectCall(TR::Node* callNode, TR::SymbolReference* callSymRef,
    TR::RegisterDependencyConditions* dependencies, const TR_PPCLinkageProperties& pp, int32_t argSize)
{
...
// Here we use aix_style_linkage only.
    ...
    TR_ASSERT(
        (grTOCReg != NULL), "TR_PPCSystemLinkage::buildVirtualDispatch no dependence set on Library TOC register gr2.");
    TR_ASSERT((grSysStackReg != NULL),
        "TR_PPCSystemLinkage::buildVirtualDispatch no dependence set on Native C Stack register gr1.");
    ...
    if (TR::Compiler->target.cpu.isBigEndian())
        generateTrg1MemInstruction(cg(), TR::InstOpCode::Op_load, callNode, grTOCReg,
            new (trHeapMemory())
                TR::MemoryReference(callNode->getChild(0)->getRegister(), TR::Compiler->om.sizeofReferenceAddress(),
                                       TR::Compiler->om.sizeofReferenceAddress(), cg()));
