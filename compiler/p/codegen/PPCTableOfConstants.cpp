...
void TR_PPCTableOfConstants::reinitializeMemory()
{
    ...
    memset(tocManagement->getTOCPtr(), 0, (tocManagement->getTOCSize() >> 1)
    ...
    int32_t val, struct TR_tocHashEntry* tmplate, int32_t* offsetInSlot, TR::CodeGenerator* cg)
{
    ...
    *offsetInSlot = 0;
            ...
            if (hash[idx]._keyTag == tmplate->_keyTag && hash[idx]._cpKey == tmplate->_cpKey
                ...
                if (debug("traceTOCHash"))
                    ; // diagnostic(comp, "   Hash Hit static2class:%p  cpIndex:%d  return:%d\n", (void
                      // *)hash[idx]._cpKey, hash[idx]._staticCPIndex, rval);
                tocManagement->getTOCMonitor()->exit();
                return rval;
            }
            break;
    ...
    case TR_FLAG_tocDoubleKey:
        ...
        if (debug("traceTOCHash"))
            ; // diagnostic(comp, "    New hash double:%p set:%p  return:%d\n", (void *)hash[idx]._dKey, (void
              // *)getTOCSlot(rval*sizeof(intptrj_t)), rval);
        break;
    case TR_FLAG_tocFloatKey:
