...
#define TRACE_VMX1(m, a1)  \
    do {                   \
        if (traceVMX(cg))  \
            printf(m, a1); \
    } while (0)
#define TRACE_VMX2(m, a1, a2)  \
    do {                       \
        if (traceVMX(cg))      \
            printf(m, a1, a2); \
    } while (0)
#define TRACE_VMX3(m, a1, a2, a3)  \
    do {                           \
        if (traceVMX(cg))          \
            printf(m, a1, a2, a3); \
    } while (0)
#define TRACE_VMX4(m, a1, a2, a3, a4)  \
    do {                               \
        if (traceVMX(cg))              \
            printf(m, a1, a2, a3, a4); \
    } while (0)
#define TRACE_VMX5(m, a1, a2, a3, a4, a5)  \
    do {                                   \
        if (traceVMX(cg))                  \
            printf(m, a1, a2, a3, a4, a5); \
    } while (0)
#define TRACE_VMX6(m, a1, a2, a3, a4, a5, a6)  \
    do {                                       \
        if (traceVMX(cg))                      \
            printf(m, a1, a2, a3, a4, a5, a6); \
    } while (0)
#define TRACE_VMX7(m, a1, a2, a3, a4, a5, a6, a7)  \
    do {                                           \
        if (traceVMX(cg))                          \
            printf(m, a1, a2, a3, a4, a5, a6, a7); \
    } while (0)
#define TRACE_VMX8(m, a1, a2, a3, a4, a5, a6, a7, a8)  \
    do {                                               \
        if (traceVMX(cg))                              \
            printf(m, a1, a2, a3, a4, a5, a6, a7, a8); \
    } while (0)
...
static void arraysetConstElemAlign8ConstLen16N(TR::Node* node, TR::Node* addrNode, TR::Node* elemNode, TR::Node* lenNode, int byteLen, TR::CodeGenerator* cg)
{
    ...
    TRACE_VMX1("@arraysetConstElemAlign8ConstLen16N: Using %s instruction\n",
        insnType == ARRAYSET_INSN_VMX ? "VMX" : insnType == ARRAYSET_INSN_STFDP ? "stfdp" : "stfd");
...
static void arraysetConstLen(TR::Node* node, TR::Node* addrNode, TR::Node* elemNode, TR::Node* lenNode, int byteLen, TR::CodeGenerator* cg)
{
    ...
    if ((data_type == TR_Bool || data_type == TR_SInt8 || data_type == TR_UInt8 || data_type == TR_SInt16 || data_type == TR_UInt16) &&
        /*elemIsConst != 0 &&*/
...
static TR::Register* arraytranslateTROTSimple256ScalarOpt(TR::Node* node, TR::CodeGenerator* cg, int max)
{
    ...
    /*l2*/ STH(ch1, 2, output);
    LBZ(ch5, 5, input1);
    ADDI(output1, output, 0);
    /*l3*/ STH(ch2, 4, output);
    LBZ(ch6, 6, input1);
    /*l4*/ STH(ch3, 6, output);
    LBZ(ch7, 7, input1);
    /*l5*/ STH(ch4, 8, output1);
    LBZ(ch0, 0, input);
    ADDI(input1, input, 0);
    /*l6*/ STH(ch5, 10, output1);
    LBZ(ch1, 1, input);
    ADDI(output, output1, 16);
    /*l7*/ STH(ch6, 12, output1);
    LBZ(ch2, 2, input);
    /*l8*/ STH(ch7, 14, output1);
    LBZ(ch3, 3, input);
    BDNZ(cnd, Loop);
    // Loop end
...
static TR::Register* arraytranslateTROTSimpleVMX(TR::Node* node, TR::CodeGenerator* cg, int max)
{
    ...
    TRACE_VMX1("@arraytranslateTROTSimpleVMX: delimNode=%s\n",
        delimNode->getOpCodeValue() == TR::iconst ? "iconst" : delimNode->getOpCodeValue() == TR::lconst ? "lconst" : "expr");
    TR::Node* lenNode = node->getChild(4);
    TRACE_VMX2("@arraytranslateTROTSimpleVMX: lenNode=%s (0x%x)\n",
        lenNode->getOpCodeValue() == TR::iconst ? "iconst" : lenNode->getOpCodeValue() == TR::lconst ? "lconst" : "expr",
...
static TR::Register* arraytranslateTRTOGenericScalarNoBreak(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    /*l4*/ STB(ch3, 3, output);
    LHZ(ch7, 14, input1);
    /*l5*/ STB(ch4, 4, output1);
    LHZ(ch0, 0, input);
    ADDI(input1, input, 0);
    /*l6*/ STB(ch5, 5, output1);
    LHZ(ch1, 2, input);
    ADDI(output, output1, 8);
    /*l7*/ STB(ch6, 6, output1);
    LHZ(ch2, 4, input);
    /*l8*/ STB(ch7, 7, output1);
    LHZ(ch3, 6, input);
    BDNZ(cnd, Loop);
    // Loop end
...
static TR::Register* arraytranslateTRTOGenericScalarWithBreak(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    /*a2*/ LHZ(ch2, 4, input);
    /*a3*/ LHZ(ch3, 6, input);
    /*b0*/ LBZX(xch0, table, ch0);
    LHZ(ch4, 8, input1);
    ADDI(input, input1, 16);
    /*b1*/ LBZX(xch1, table, ch1);
    LHZ(ch5, 10, input1);
    /*b2*/ LBZX(xch2, table, ch2);
    LHZ(ch6, 12, input1);
    /*b3*/ LBZX(xch3, table, ch3);
    LHZ(ch7, 14, input1);
    /*c0*/ CMP4(cnd, xch0, delim);
    LBZX(xch4, table, ch4);
    LHZ(ch0, 0, input);
    ADDI(input1, input, 0);
    /*c1*/ CMP4(cnd1, xch1, delim);
    LBZX(xch5, table, ch5);
    LHZ(ch1, 2, input);
    /*c2*/ CMP4(cnd2, xch2, delim);
    LBZX(xch6, table, ch6);
    LHZ(ch2, 4, input);
    /*c3*/ CMP4(cnd3, xch3, delim);
    LBZX(xch7, table, ch7);
    LHZ(ch3, 6, input);
    LABEL(Loop);
    /*l0*/ BEQ(cnd, Out0);
    /*l1*/ STB(xch0, 0, output);
    CMP4(cnd, xch4, delim);
    LBZX(xch0, table, ch0);
    LHZ(ch4, 8, input1);
    ADDI(input, input1, 16);
    BEQ(cnd1, Out1);
    /*l2*/ STB(xch1, 1, output);
    CMP4(cnd1, xch5, delim);
    LBZX(xch1, table, ch1);
    LHZ(ch5, 10, input1);
    ADDI(output1, output, 0);
    BEQ(cnd2, Out2);
    /*l3*/ STB(xch2, 2, output);
    CMP4(cnd2, xch6, delim);
    LBZX(xch2, table, ch2);
    LHZ(ch6, 12, input1);
    BEQ(cnd3, Out3);
    /*l4*/ STB(xch3, 3, output);
    CMP4(cnd3, xch7, delim);
    LBZX(xch3, table, ch3);
    LHZ(ch7, 14, input1);
    BEQ(cnd, Out4);
    /*l5*/ STB(xch4, 4, output1);
    CMP4(cnd, xch0, delim);
    LBZX(xch4, table, ch4);
    LHZ(ch0, 0, input);
    ADDI(input1, input, 0);
    BEQ(cnd1, Out5);
    /*l6*/ STB(xch5, 5, output1);
    CMP4(cnd1, xch1, delim);
    LBZX(xch5, table, ch5);
    LHZ(ch1, 2, input);
    ADDI(output, output1, 8);
    BEQ(cnd2, Out6);
    /*l7*/ STB(xch6, 6, output1);
    CMP4(cnd2, xch2, delim);
    LBZX(xch6, table, ch6);
    LHZ(ch2, 4, input);
    BEQ(cnd3, Out7);
    /*l8*/ STB(xch7, 7, output1);
    CMP4(cnd3, xch3, delim);
    LBZX(xch7, table, ch7);
    LHZ(ch3, 6, input);
    ADDI(index, index, 8);
    BDNZ(cnd, Loop);
...
static TR::Register* arraytranslateTRTOSimpleScalar(TR::Node* node, TR::CodeGenerator* cg, int max)
{
    ...
    LHZ(ch4, 8, input1);
    ADDI(input, input1, 16);
    /*p5*/ CMP4(cnd1, ch1, max);
    LHZ(ch5, 10, input1);
    /*p6*/ CMP4(cnd2, ch2, max);
    LHZ(ch6, 12, input1);
    /*p7*/ CMP4(cnd3, ch3, max);
    LHZ(ch7, 14, input1);
...
#endif
    TR_ASSERT((termCharNodeIsHint == true && sourceCellIsTermChar == false) || (termChar >= 0 && termChar <= 0xffff), "termChar out of range");

    uint8_t* trto_tab = (uint8_t*)(tableNode->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress());

#define IS_TRTO_DELIM(tab, idx, _termChar, _termCharNodeIsHint, _sourceCellIsTermChar) \
    (tab[idx] == termChar && (_termCharNodeIsHint == false || _termCharNodeIsHint == true && _sourceCellIsTermChar == true && idx != _termChar))
...
static int analyzeTROTTable(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    uint16_t* trot_tab = (uint16_t*)(tableNode->getSymbolReference()->getSymbol()->getStaticSymbol()->getStaticAddress());
...
static TR::Register* arraytranslateAndTestGenVMX(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    loadAddressConstant(cg, node, (intptrj_t)vconstTable, constTableReg);
    for (i = 0; i < 16; i++) {
        ((uint8_t*)vconstTable)[i] = delmNode->getInt();
    }
    for (i = 4; i < 8; i++)
        vconstTable[i] = 0x80200802;
...
static TR::Register* findBytesGenSRSTVMX(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    for (int i = 0; i < 4; i++)
        vconstDelim[i] = (delimNode->getInt() << 24) | (delimNode->getInt() << 16) | (delimNode->getInt() << 8) | delimNode->getInt();
    for (int i = 4; i < 8; i++)
        vconstDelim[i] = 0x80200802;
    vconstDelim[8] = 0x03070b0f;
    for (int i = 9; i < 12; i++)
        vconstDelim[i] = 0;
...
static int getOffset(TR::Node* addrNode, TR::CodeGenerator* cg)
{
    ...
    if (addrNode->getOpCodeValue() == TR::aiadd && (child0 = addrNode->getFirstChild()) != NULL && child0->getOpCodeValue() == TR::aloadi && (child1 = addrNode->getSecondChild()) != NULL && child1->getOpCodeValue() == TR::iconst)
