...
void TR_CFGChecker::performCorrectnessCheck()
{
    ...
    if (_cfg->getStart()->getSuccessors().size() != 1) {
        ...
        if (_cfg->getStart()->getSuccessors().front()->getTo() != _blocksInProgramOrder[0]) {
            if (_outFile)
                trfprintf(_outFile, "The successor block for the (dummy) start block in the CFG is NOT the start block "
    ...
    */
    ...
    if (!(opCode.isBranch() || opCode.isSwitch() || opCode.isReturn() || opCode.isJumpWithMultipleTargets()
        ...
        if (!(block->getSuccessors().size() == 1)) {
            ...
            if (_outFile)
                trfprintf(_outFile, "Last non-fence opcode in block [%d] is not a branch, switch, or a return and it "
        ...
        if (block->getSuccessors().front()->getTo() != _blocksInProgramOrder[i + 1]) {
            if (_outFile)
                trfprintf(_outFile, "Successor block [%d] of block [%d] (with no branch, switch, or return at the end) "
                                    "is not the fall through block\n",
    ...
    if (opCode.isBranch()) {
        ...
        if (opCode.getOpCodeValue() == TR::Goto) {
            ...
            if (!(numUniqueChildren == block->getSuccessors().size())) {
                if (_outFile)
                    trfprintf(_outFile, "Number of successors of block [%d] having an if at the exit is not equal to "
                                        "the number of unique targets of the if\n",
                        block->getNumber());
                return false;
            }
        }
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (!((next == fallThroughBlock) || (next == branchBlock))) {
                if (_outFile)
                    trfprintf(_outFile, "Successor block [%d] of block [%d] containing a branch does not match the "
        ...
        if (!(getNumUniqueCases(lastNode) == block->getSuccessors().size())) {
            if (_outFile)
                trfprintf(_outFile, "Number of successors of block [%d] having a switch at the exit is not equal to "
                                    "the number of destinations in the IL switch instruction\n",
                    block->getNumber());
            return false;
        }
        ...
        for (auto edge = block->getSuccessors().begin(); edge != block->getSuccessors().end(); ++edge) {
            ...
            if (!equalsAnyChildOf(next->getEntry(), lastNode)) {
                if (_outFile)
                    trfprintf(_outFile, "Successor block [%d] of block [%d] containing a switch does not match any of "
                                        "the destinations specified in the IL switch instruction\n",
                        next->getNumber(), block->getNumber());
                return false;
            }
        }
...
bool TR_CFGChecker::isConsistent(TR::Block* block)
{
    ...
    for (auto nextEdge = block->getExceptionPredecessors().begin(); nextEdge != block->getExceptionPredecessors().end();
        ...
        if (matchEdge == nextBlock->getExceptionSuccessors().end()) {
            if (_outFile)
                trfprintf(_outFile, "Exception Predecessor block [%d] of block [%d] does not contain block [%d] in its "
