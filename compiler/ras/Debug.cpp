...
void TR_Debug::printAsmDumpHeader()
{
    ...
    trfprintf(_file, "\n  |%*s +----------------------------------------- binary representation", addressFieldWidth - 3,
        " ", addressFieldWidth - 1, " ");
    trfprintf(_file, "\n  |%*s | %*s+----------------------------- opcode and operands", addressFieldWidth - 3, " ",
        codeByteColumnWidth + 4, " ");
    trfprintf(_file, "\n  |%*s | %*s|\t\t\t\t    +------- additional information", addressFieldWidth - 3, " ",
        codeByteColumnWidth + 4, " ");
...
static const char* commonNonhelperSymbolNames[]
    = { "<contiguousArraySize>", "<discontiguousArraySize>", "<arrayClassRomPtr>", "<classRomPtr>",
        ...
        "<componentClass>", "<componentClassAsPrimitive>", "<isArray>", "<isClassAndDepthFlags>", "<isClassFlags>",
        "<vft>", "<bcdCheckSymbol>", "<currentThread>", "<recompilationCounter>", "<excp>", "<indexableSize>",
        "<resolveCheck>", "<arrayTranslate>", "<arrayTranslateAndTest>", "<long2String>", "<bitOpMem>", "<reverseLoad>",
        "<reverseStore>", "<currentTimeMaxPrecision>", "<headerFlags>", "<singlePrecisionSQRT>", "<threadPrivateFlags>",
        ...
        "<heapBase>", "<heapTop>", "<j9methodExtraField>", "<startPCLinkageInfo>", "<instanceShapeFromROMClass>",
        "<pythonFrame_CodeObject>", "<pythonFrame_FastLocals>", "<pythonFrame_Globals>", "<pythonFrame_Builtins>",
        "<pythonCode_Constants>", "<pythonCode_NumLocals>", "<pythonCode_Names>", "<pythonObject_Type>",
        "<pythonType_IteratorMethod>", "<pythonObject_ReferenceCount>" };
...
int32_t TR_Debug::dumpLiveRegisters(TR::FILE* pOutFile, TR_RegisterKinds rk)
{
    ...
    for (TR_LiveRegisterInfo* p = liveReg->getFirstLiveRegister(); p; p = p->getNext()) {
        ...
        ++n;
        if (regPair)
            trfprintf(pOutFile, "\t[" POINTER_PRINTF_FORMAT "] %d:  " POINTER_PRINTF_FORMAT
...
void TR_Debug::dumpMethodInstrs(TR::FILE* pOutFile, const char* title, bool dumpTrees, bool header)
{
    ...
    while (instr) {
        ...
        if (printLinenos) {
            ...
            if ((crtLineNo != -1) && ((crtLineNo != lastLineNoPrinted) || (inlinedIndex != lastInlinedIndex)
...
void TR_Debug::dumpMixedModeDisassembly()
{
    ...
    for (; inst; inst = inst->getNext()) {
        if (inst->getNode() != NULL
            && (!n || ((inst->getNode()->getByteCodeInfo().getCallerIndex() != n->getByteCodeInfo().getCallerIndex()
...
void TR_Debug::printCommonDataMiningAnnotations(TR::FILE* pOutFile, TR::Instruction* inst, bool needsStartComment)
{
    if (inst != NULL && inst->getNode()) {
        ...
        TR::SimpleRegex* regex = _comp->getOptions()->getTraceForCodeMining();
        if (regex && (TR::SimpleRegex::match(regex, "ALL") || TR::SimpleRegex::match(regex, IL_KEY)
...
void TR_Debug::printFullRegInfo(TR::FILE* pOutFile, TR::Register* reg)
{
    ...
    if (reg->getRealRegister()) {
        ...
        if (ignoreFreeAndLockedRegs && (reg->getRealRegister()->getState() == TR::RealRegister::Locked
...
void TR_Debug::dump(TR::FILE* pOutFile, TR_CHTable* chTable)
{
    ...
    if (!inDebugExtension() && !vguards.empty()) {
        ...
        for (auto info = vguards.begin(); info != vguards.end(); ++info, ++i) {
            ...
            sprintf(guardKindName, "%s %s", getVirtualGuardKindName((*info)->getKind()),
            ...
            for (TR_VirtualGuardSite* site = siteIt.getFirst(); site; site = siteIt.getNext()) {
                ...
                trfprintf(pOutFile, "\tSite: location=" POINTER_PRINTF_FORMAT
...
const char* TR_Debug::getRuntimeHelperName(int32_t index)
{
    ...
    if (index < TR_FSRH) {
        ...
        if (index < TR_LXRH) {
            ...
            case TR_IA32referenceArrayCopy:
                return "__referenceArrayCopy";
            case TR_IA32generalArrayCopy:
            ...
            case TR_AMD64forwardHalfWordArrayCopy:
                return "__forwardHalfWordArrayCopy";
            case TR_AMD64referenceArrayCopy:
                return "__referenceArrayCopy";
            case TR_AMD64generalArrayCopy:
                return "__generalArrayCopy";
...
const char* TR_Debug::getWriteBarrierKindName(int32_t kind)
{
    static const char* names[TR_NumberOfWrtBars] = {
        "None", "Always", "OldCheck", "CardMark", "CardMarkAndOldCheck", "CardMarkIncremental", "RealTime",
    };
    return names[kind];
}
