...
void TR::DebugCounter::getInsertionCounterNames(TR::Compilation* comp, TR::Node* node, const char* (&counterNames)[3])
{
    ...
    if (insertByteCode) {
        counterNames[0] = TR::DebugCounter::debugCounterName(comp,
            comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byByteCode.(%s)=%d"), method,
    ...
    if (insertMethod) {
        counterNames[2] = TR::DebugCounter::debugCounterName(
            comp, comp->getOptions()->debugCounterInsertedFormat(comp->trMemory(), getName(), "byMethod.(%s)"), method);
    }
}
...
void TR::DebugCounterAggregation::aggregateDebugCounterInsertions(TR::Compilation* comp, TR::Node* node,
...
void TR::DebugCounterAggregation::aggregateDebugCounterHistogram(TR::Compilation* comp, TR::Node* node,
    TR::DebugCounter* counter, int32_t delta, int8_t fidelity, int32_t staticDelta)
{
...
{
    TR::DebugCounterAggregation* aggregatedCounters
        = new (comp->trPersistentMemory()) TR::DebugCounterAggregation(comp->trMemory());
