...
class DebugCounterAggregation : public DebugCounterBase {
    ...
    void aggregateDebugCounterInsertions(TR::Compilation* comp, TR::Node* node, DebugCounter* counter, int32_t delta,
        int8_t fidelity, int32_t staticDelta);
    void aggregateDebugCounterHistogram(TR::Compilation* comp, TR::Node* node, DebugCounter* counter, int32_t delta,
        int8_t fidelity, int32_t staticDelta);

protected:
    DebugCounterAggregation(TR_Memory* mem)
        : _mem(mem)
        , _counterDeltas(new (mem->trPersistentMemory()) TR_PersistentList<CounterDelta>())
        , _symRef(NULL)
        , _bumpCount(0)
        , _lastBumpCount(0)
    {
