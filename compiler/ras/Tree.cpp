...
// TR::Compilationl::_prevSymRefTabSize
...
void TR_Debug::printSymRefTable(TR::FILE* pOutFile, bool printFullTable)
{
    ...
    if (currSymRefTabSize > 0 && _comp->getPrevSymRefTabSize() < currSymRefTabSize) // entry(ies) have been added to
                                                                                    // symbol refetence table since last
...
void TR_Debug::printNodesInEdgeListIterator(TR::FILE* pOutFile, TR::list<TR::CFGEdge*>& li, bool fromNode)
{
    TR::CFGEdge* edge;
    TR::Block* b;
    int num = 0;
...
void TR_Debug::printNodeInfo(TR::Node* node, TR_PrettyPrinterString& output, bool prettyPrint)
{
    ...
    if (node->getOpCode().hasSymbolReference() && node->getSymbolReference()) {
        ...
        if ((nextBlock = block->getNextBlock()) && !nextBlock->isExtensionOfPreviousBlock()) // end of a block that is
                                                                                             // not the last block and
                                                                                             // the next block is not an
                                                                                             // extension of it
            output.append(" =====");
...
void TR_Debug::printBCDNodeInfo(TR::Node* node, TR_PrettyPrinterString& output)
{
    if (node->getType().isBCD()) {
        if (node->getOpCode().isPackedArithmeticSelect()) {
            output.append(" <prec=%d (len=%d) dividendPrec=%d divisorPrec=%d> ", node->getDecimalPrecision(),
                node->getSize(), node->getSelectDividendPrecision(), node->getSelectDivisorPrecision());
        } else if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst()
...
void TR_Debug::printVCG(TR::FILE* pOutFile, TR::CFG* cfg, const char* sig)
{
    ...
    if (debug("nestedVCG")) {
        ...
        trfprintf(pOutFile, "node: {title: \"Top1\" label: \"%s\" vertical_order: 0 horizontal_order: 0 textcolor: "
                            "blue borderwidth: 1}\n",
            sig);
