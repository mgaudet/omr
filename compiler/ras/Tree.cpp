...
void TR_Debug::printBCDNodeInfo(TR::Node* node, TR_PrettyPrinterString& output)
{
    if (node->getType().isBCD()) {
        ...
        } else if (node->getOpCode().isStore() || node->getOpCode().isCall() || node->getOpCode().isLoadConst() || (node->getOpCode().isConversion() && !node->getOpCode().isConversionWithFraction())) {
...
void TR_Debug::print(TR::FILE* pOutFile, TR_VPConstraint* info)
{
    if (pOutFile == NULL)
        return;
