...
//
void OMR::CodeCache::adjustTrampolineReservation(TR_OpaqueMethodBlock* method, void* cp, int32_t cpIndex)
{
...
void OMR::CodeCache::resolveHashEntry(CodeCacheHashEntry* entry, TR_OpaqueMethodBlock* method)
{
...
void OMR::CodeCache::onClassRedefinition(TR_OpaqueMethodBlock* oldMethod, TR_OpaqueMethodBlock* newMethod)
{
...
void OMR::CodeCache::patchCallPoint(TR_OpaqueMethodBlock* method, void* callSite, void* newStartPC, void* extraArg)
{
    ...
    // scope to patch trampoline
    {
        ...
        if (TR::Options::getCmdLineOptions()->getVerboseOption(TR_VerbosePatching)) {
            TR_VerboseLog::writeLineLocked(TR_Vlog_PATCH, "Patching callsite=0x%p using "
                                                          "j9method=0x%p,resolvedTramp=0x%p,methodRunAddress=0x%p,"
                                                          "newStartPC=0x%p,extraArg=0x%p",
...
//
bool OMR::CodeCache::addUnresolvedMethod(void* constPool, int32_t constPoolIndex)
{
...
bool OMR::CodeCache::addFreeBlock2WithCallSite(uint8_t* start, uint8_t* end, char* file, uint32_t lineNumber)
{
    ...
    if (end <= (start + sizeof(CodeCacheFreeCacheBlock))) {
        if (config.verboseReclamation()) {
            TR_FrontEnd* fe = _manager->fe();
            TR_VerboseLog::writeLineLocked(TR_Vlog_FAILURE, "addFreeBlock2[%s.%d]: failed to add free block. start = "
                                                            "0x%016x end = 0x%016x alignment = 0x%04x "
        ...
        // scope for cache walk
        {
            ...
            for (CodeCacheFreeCacheBlock* currLink = _freeBlockList; currLink; currLink = currLink->_next) {
                ...
                if (currLink->_size > cacheSize) {
                    fprintf(stderr, "checkForErrors cache %p: Error: Size of the free block %u is bigger than the size "
                                    "of the cache %u\n",
                ...
                if (currLink->_next) {
                    if ((uint8_t*)currLink->_next == endBlock) {
                        ...
                        if (!((uint8_t*)currLink < _warmCodeAlloc && endBlock >= _coldCodeAlloc)) {
                            fprintf(stderr, "checkForErrors cache %p: Error: missed freed block coalescing "
                                            "opportunity. Next block (%p) is adjacent to current one %p-%p\n",
                                this, currLink->_next, currLink, endBlock);
                            doCrash = true;
                        }
                    } else {
                        if ((uint8_t*)currLink->_next <= endBlock) {
                            fprintf(stderr, "checkForErrors cache %p: Error: next block (%p) should come after end of "
                                            "current one %p-%p\n",
                                this, currLink->_next, currLink, endBlock);
                            doCrash = true;
                            ;
                        ...
                        if (endBlock != _warmCodeAlloc) {
                            ...
                            if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
                                fprintf(stderr, "checkForErrors cache %p: Error: block coming after this free one "
                                                "(%p-%p) does not have the eye catcher but %u\n",
            ...
            while (start < this->_trampolineBase) {
                ...
                } else // used seg
                {
                    ...
                    if (*eyeCatcherPosition != *(_manager->codeCacheConfig().warmEyeCatcher())) {
                        ...
                        fprintf(stderr, "checkForErrors cache %p: block %p is not in the free list and does not have "
                                        "eye-catcher; prevBlock=%p\n",
...
void OMR::CodeCache::setupSegment(
    TR::CodeCacheMemorySegment* codeCacheSegment, size_t codeCacheSizeAllocated, CodeCacheHashEntrySlab* hashEntrySlab)
{
    // heapSize can be calculated as (codeCache->helperTop - codeCacheSegment->heapBase), which is equal to segmentSize
    // If codeCachePadKB is set, this will make the system believe that we allocated segmentSize bytes,
    // instead of _jitConfig->codeCachePadKB * 1024 bytes
...
OMR::CodeCacheHashEntry* OMR::CodeCache::findUnresolvedMethod(void* constPool, int32_t constPoolIndex)
{
    ...
    TR::CodeCacheManager* manager, size_t segmentSize, int32_t reservingCompThreadID)
{
    ...
    if (codeCacheSegment) {
        ...
        CodeCacheHashEntrySlab* hashEntrySlab
            = CodeCacheHashEntrySlab::allocate(manager, config.codeCacheHashEntryAllocatorSlabSize());
        if (hashEntrySlab) {
            uint8_t* start = NULL;
            TR::CodeCache* codeCache
                = manager->allocateCodeCacheObject(codeCacheSegment, codeCacheSizeAllocated, hashEntrySlab);
