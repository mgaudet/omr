    ...
    if (_elfHeader) {
        ...
        if (numCharsWritten > 0 && numCharsWritten < maxElfFilenameSize) {
            ...
            fwrite(repositorySegment->segmentBase(), sizeof(uint8_t),
                repositorySegment->segmentTop() - repositorySegment->segmentBase(), elfFile);
...
//
void OMR::CodeCacheManager::onClassRedefinition(TR_OpaqueMethodBlock* oldMethod, TR_OpaqueMethodBlock* newMethod)
{
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    if (!config.needsMethodTrampolines())
        return;
...
void OMR::CodeCacheManager::addFaintCacheBlock(
    FaintCacheBlock*& faintBlockList, FaintCacheBlock* block, uint8_t bytesToSaveAtStart)
{
...
// Caller should expect that block may sometimes not be added.
void OMR::CodeCacheManager::freeFaintCacheBlock(FaintCacheBlock* block, uint8_t* startPC)
{
    TR::CodeCache* owningCodeCache = self()->findCodeCacheFromPC(startPC);
...
//        allocationRetries, numCachesVisited, numCachesAlreadyReserved);
    ...
    TR::CodeCacheConfig& config = self()->codeCacheConfig();
    size_t allocateSize = 2 * (warmCodeSize + coldCodeSize + config.numRuntimeHelpers() * config.trampolineCodeSize()
...
#ifdef MCT_DEBUG
    fprintf(stderr, "cache %p reset reservation in allocateCodeMemory after new code cache allocation. New cache=%p "
...
{
    _elfTrailerSize = sizeof(ELFCodeCacheTrailer) + _numELFSymbols * sizeof(ELFSymbol)
        + // NOTE: ELFCodeCacheTrailer includes 1 ELFSymbol: UNDEF
    ...
    trlr->dynsymSection.sh_addr
        = 0; //(ELFAddress) &((uint8_t *)_elfHeader + symbolStartOffset); // fake address because not continuous
    ...
    trlr->shstrtabSection.sh_size = sizeof(trlr->zeroSectionName) + sizeof(trlr->shstrtabSectionName)
        + sizeof(trlr->textSectionName) + sizeof(trlr->dynsymSectionName) + sizeof(trlr->dynstrSectionName);
...
void OMR::CodeCacheManager::initializeELFHeaderForPlatform(ELFCodeCacheHeader* hdr)
{
