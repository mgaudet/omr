...
#define REACHEABLE_RANGE_KB (2048 * 1024)
    ...
    config._needsMethodTrampolines = !(config.trampolineCodeSize() == 0
        ...
        || (!TR::Options::getCmdLineOptions()->getOption(TR_EnableMethodTrampolineReservation) && _codeCacheRepositorySegment && config.codeCacheTotalKB() <= REACHEABLE_RANGE_KB)
#endif
            );
...
//
void OMR::CodeCacheManager::onClassRedefinition(TR_OpaqueMethodBlock* oldMethod,
...
{
    _elfTrailerSize = sizeof(ELFCodeCacheTrailer) + _numELFSymbols * sizeof(ELFSymbol) + // NOTE: ELFCodeCacheTrailer includes 1 ELFSymbol: UNDEF
    ...
    trlr->shstrtabSection.sh_size = sizeof(trlr->zeroSectionName) + sizeof(trlr->shstrtabSectionName) + sizeof(trlr->textSectionName) + sizeof(trlr->dynsymSectionName) + sizeof(trlr->dynstrSectionName);
