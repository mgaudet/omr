...
uint8_t* TR_AMD64FPConversionSnippet::genFPConversion(uint8_t* buffer)
{
    ...
    if (targetReg != TR::RealRegister::eax) {
        ...
        //
        *buffer++
            = TR::RealRegister::REX | TR::RealRegister::REX_W | targetRegister->rexBits(TR::RealRegister::REX_R, false);
        *buffer++ = 0x8b;
        *buffer = 0xc0;
    ...
    if (targetReg != TR::RealRegister::eax) {
        ...
        //
        *buffer++
            = TR::RealRegister::REX | TR::RealRegister::REX_W | targetRegister->rexBits(TR::RealRegister::REX_B, false);
        *buffer = 0x90;
...
void TR_Debug::print(TR::FILE* pOutFile, TR_AMD64FPConversionSnippet* snippet)
{
    ...
    if (treg != TR::RealRegister::eax) {
        ...
        trfprintf(pOutFile, ", ");
        print(pOutFile, machine->getX86RealRegister(TR::RealRegister::eax), size);
        trfprintf(pOutFile, "\t%s restore result reg & put result in target reg", commentString());
        bufferPos += instrSize;
    }
