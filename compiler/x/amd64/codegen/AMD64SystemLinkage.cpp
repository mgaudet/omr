    ...
    : TR_AMD64SystemLinkage(cg)
{
    ...
    //
    _properties._properties = EightBytePointers | EightByteParmSlots | CallerCleanup | IntegersInRegisters
        | LongsInRegisters | FloatsInRegisters | ReservesOutgoingArgsInPrologue | AlwaysDedicateFramePointerRegister;
...
//
int32_t TR_AMD64SystemLinkage::buildArgs(TR::Node* callNode, TR::RegisterDependencyConditions* deps)
{
    ...
    for (i = first; i != last; i += direction) {
        ...
        if (rregIndex != noReg) {
            ...
            generateMemRegInstruction(movOpcodes[MemReg][fullRegisterMovType(vreg)], child,
...
TR::Register* TR_AMD64SystemLinkage::buildDirectDispatch(TR::Node* callNode, bool spillFPRegs)
{
    TR::SymbolReference* methodSymRef = callNode->getSymbolReference();
    TR::MethodSymbol* methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
...
void TR_AMD64ABILinkage::mapIncomingParms(TR::ResolvedMethodSymbol* method, uint32_t& stackIndex)
{
    ...
    for (parmCursor = parameterIterator.getFirst(); parmCursor; parmCursor = parameterIterator.getNext()) {
        if ((parmCursor->getLinkageRegisterIndex() >= 0)
            ...
            if (comp()->getOption(TR_TraceCG))
                traceMsg(comp(), "mapIncomingParms setParameterOffset %d for param symbol (reg param without home "
...
bool TR_AMD64SystemLinkage::layoutTypeInRegs(
    TR::DataType type, uint16_t& intArgs, uint16_t& floatArgs, parmLayoutResult& layoutResult)
{
...
int32_t TR_AMD64SystemLinkage::layoutParm(
    TR::Node* parmNode, int32_t& dataCursor, uint16_t& intReg, uint16_t& floatReg, parmLayoutResult& layoutResult)
{
...
int32_t TR_AMD64SystemLinkage::layoutParm(TR::ParameterSymbol* parmSymbol, int32_t& dataCursor, uint16_t& intReg,
    uint16_t& floatReg, parmLayoutResult& layoutResult)
{
    // AMD64 SysV ABI:  if the size of an object is larger than four eightbytes, or it contains unaligned fields, it has
    // class MEMORY.
