...
void OMR::X86::AMD64::MemoryReference::addMetaDataForCodeAddressWithLoad(
    uint8_t* displacementLocation, TR::Instruction* containingInstruction, TR::CodeGenerator* cg)
{
    TR::SymbolReference& sr = self()->getSymbolReference();
    intptrj_t displacement = self()->getDisplacement();
        ...
        if (self()->getUnresolvedDataSnippet()) {
            ...
            if (sr.getSymbol()->isStatic()) {
                if (cg->needClassAndMethodPointerRelocations())
                    cg->addAOTRelocation(
                        new (cg->trHeapMemory())
                            TR_ExternalRelocation(displacementLocation, (uint8_t*)&_symbolReference,
                                (uint8_t*)containingInstruction->getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg),
            ...
            if (cg->needRelocationsForStatics())
                cg->addAOTRelocation(new (cg->trHeapMemory())
                                         TR_ExternalRelocation(displacementLocation, 0, TR_BodyInfoAddress, cg),
                    __FILE__, __LINE__, containingInstruction->getNode());
        } else if (sr.getSymbol()->isGCRPatchPoint()) {
            if (cg->needRelocationsForStatics()) {
                TR_ExternalRelocation* r = new (cg->trHeapMemory())
                    TR_ExternalRelocation(displacementLocation, 0, TR_AbsoluteMethodAddress, cg);
                cg->addAOTRelocation(r, __FILE__, __LINE__, containingInstruction->getNode());
            }
        } else if (sr.getSymbol()->isCompiledMethod()) {
            if (cg->needRelocationsForStatics())
                cg->addAOTRelocation(new (cg->trHeapMemory())
                                         TR_ExternalRelocation(displacementLocation, 0, TR_RamMethod, cg),
                    __FILE__, __LINE__, containingInstruction->getNode());
    ...
    if (self()->getDataSnippet() || self()->getLabel()) {
        ...
        if (_symbolReference.getSymbol()) {
            ...
            TR::SymbolReference* symRef
                = new (cg->trHeapMemory()) TR::SymbolReference(cg->symRefTab(), _symbolReference, 0);
