...
bool OMR::X86::TreeEvaluator::analyseSubForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1) {
        ...
        if (firstOp.isAdd() && constNodeValueIs32BitSigned(secondChild, &dummyConstValue, cg)) {
            // sub
            //    add         (firstChild)
            //       ?        (llChild)
            //       ?        (lrchild)
...
bool OMR::X86::TreeEvaluator::analyseAddForLEA(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (indexNode) {
        ...
        if (constNode) {
            ...
            if (constNode->getOpCodeValue() == TR::lconst)
                ...
                leaMR = generateX86MemoryReference(NULL,
                    ...
                    constNode->getInt(), cg);
            tempNode = NULL;
        } else if (baseNode->getRegister() == NULL && baseNode->getReferenceCount() == 1 && baseNode->getOpCode().isAdd() && baseNode->getSecondChild()->getOpCode().isLoadConst() && constNodeValueIs32BitSigned(baseNode->getSecondChild(), &dummyConstValue, cg)) {
            // add            (** children may be reversed **)
            //    add            (baseNode)
            //       ?
            //       const
        ...
        node->setRegister(targetRegister);
        return true;
    } else if (constNode && constNodeValueIs32BitSigned(constNode, &dummyConstValue, cg)) {
        // add
        //    firstChild
        //    const
        //
        if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
            ...
            if (stride1 | stride2) {
                ...
                if (indexNode->getOpCode().isLoadConst()) {
                    ...
                    leaMR = generateX86MemoryReference(cg->evaluate(baseNode),
                        integerConstNodeValue(constNode, cg) + integerConstNodeValue(indexNode, cg), cg);
                } else {
                    leaMR = generateX86MemoryReference(cg->evaluate(baseNode),
                        cg->evaluate(indexNode),
                        0,
        ...
        node->setRegister(targetRegister);
        return true;
    } else if ((firstChild->getOpCode().isSub() || firstChild->getOpCode().isAdd()) && (firstChild->getOpCode().getSize() == node->getOpCode().getSize()) && (firstChild->getRegister() == NULL) && (firstChild->getSecondChild()->getOpCode().isLoadConst()) && constNodeValueIs32BitSigned(firstChild->getSecondChild(), &dummyConstValue, cg) && (firstChild->getReferenceCount() == 1)) {
        // add
        //    sub/add    (firstChild)
        //        child 0
        //        loadconstant
...
TR::Register* OMR::X86::TreeEvaluator::integerAddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!computesCarry && analyseAddForLEA(node, cg)) {
        ...
        if (!targetRegister && secondChild->getOpCode().isLoadConst() && (secondChild->getRegister() == NULL) && ((constNodeValueIs32BitSigned(secondChild, &constValue, cg)) || (comp->useCompressedPointers() && (constValue == TR::Compiler->vm.heapBaseAddress()) && firstChild->getReferenceCount() > 1 && !isMemOp)) && performTransformation(comp, "O^O analyseAddForLEA: checking that second node is a memory reference %x\n", constValue)) {
            if (!isMemOp)
...
TR::Register* OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    intptrj_t constValue;
    TR::Register* testRegister = secondChild->getRegister();
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL && constNodeValueIs32BitSigned(secondChild, &constValue, cg) && performTransformation(comp, "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 bit constant. Register value: %x\n", testRegister)) {
...
TR::Register* OMR::X86::TreeEvaluator::integerMulEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (value = integerConstNodeValue(secondChild, cg))) {
...
TR::Register* OMR::X86::TreeEvaluator::integerDivOrRemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && integerConstNodeValue(secondChild, cg) != 0) {
...
TR::Register* OMR::X86::TreeEvaluator::integerShlEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if (shiftAmountNode->getOpCode().isLoadConst() && (shiftAmount = integerConstNodeValue(shiftAmountNode, cg) & shiftMask(nodeIs64Bit),
