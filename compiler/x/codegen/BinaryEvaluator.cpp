        ...
        if (firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && firstOp.isAdd()) {
            ...
            if (stride1 | stride2) {
                ...
                if (indexNode->getOpCode().isLoadConst()) {
                    ...
                    leaMR = generateX86MemoryReference(
                        cg->evaluate(baseNode), cg->evaluate(indexNode), 0, integerConstNodeValue(constNode, cg), cg);
                }
            }
            targetRegister = cg->allocateRegister();
            generateRegMemInstruction(LEARegMem(nodeIs64Bit), node, targetRegister, leaMR, cg);
...
TR::Register* OMR::X86::TreeEvaluator::integerSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && testRegister == NULL
        && constNodeValueIs32BitSigned(secondChild, &constValue, cg)
        && performTransformation(comp, "O^O IntegerSubEvaluator: register is not NULL, or second operand is not a 32 "
                                       "bit constant. Register value: %x\n",
               testRegister)) {
    ...
    if (isPowerOf2(dvalue)) {
        ...
        if (rootOpCode.isRem()) {
            ...
            if (isMinInt) {
                ...
                if (1 <= dvalue && dvalue <= 0x40000000) {
                    ...
                    TR_ASSERT(dvalue > CONSTANT64(0x80000000), "dvalue should not be negative at this point; "
        ...
        if ((dividend->isNonNegative() == false) && (dvalue > 0 || isMinInt)) {
            ...
            if (dvalue == 2) // special case when working with 2
            {
                ...
                TR_ASSERT(mask >= 0, "AMD64: mask must have sign bit clear so that, when sign-extended, it clears the "
                                     "upper 32 bits of tempRegister");
    ...
    if (secondChild->getOpCode().isLoadConst() && integerConstNodeValue(secondChild, cg) != 0) {
        ...
        if (needsExplicitOverflowCheck) {
            ...
            cg->addSnippet(new (cg->trHeapMemory())
                    TR_X86DivideCheckSnippet(restartLabel, overflowSnippetLabel, divisionLabel, node->getOpCode(),
                        node->getType(), divideInstr->getIA32RegInstruction()->getIA32RegRegInstruction(), cg));
        }
        ...
        if (shiftAmount == 0) {
            ...
            if (SHIFT_MAY_HAVE_ADDRESS_CHILD && node->getOpCode().isLeftShift()
                ...
                targetRegister = intOrLongClobberEvaluate(
                    firstChild, (SHIFT_MAY_HAVE_ADDRESS_CHILD) ? getNodeIs64Bit(firstChild, cg) : nodeIs64Bit, cg);
            }
...
TR::Register* OMR::X86::TreeEvaluator::integerUshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->isDirectMemoryUpdate()) {
        TR::Instruction* shiftInstruction
            = generateMemoryShift(node, SHRMemImm1(nodeIs64Bit), SHRMemCL(nodeIs64Bit), cg);
        if (shiftInstruction)
