...
void TR_Debug::print(TR::FILE* pOutFile, TR_IA32ConstantDataSnippet* snippet)
{
    ...
    trfprintf(pOutFile, "%s \t%s", snippet->getConstantSize() == 8 ? dqString() : (snippet->getConstantSize() == 4 ? ddString() : dwString()),
        hexPrefixString());
