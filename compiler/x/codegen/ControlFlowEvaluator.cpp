...
void OMR::X86::TreeEvaluator::compareIntegersForEquality(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && ((secondChild->getSize() <= 2) && (!secondChild->isUnsigned()) || constNodeValueIs32BitSigned(secondChild, &constValue, cg) && !cg->constantAddressesCanChangeSize(secondChild))) {
    ...
    TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && constNodeValueIs32BitSigned(secondChild, &constValue, cg)) {
...
static bool canBeHandledByIfInstanceOfHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL && !cg->comp()->getOption(TR_DisableInlineIfInstanceOf)) {
        intptrj_t constValue = integerConstNodeValue(secondChild, cg);
        return (firstChild->getOpCodeValue() == TR:: instanceof &&firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && (constValue == 0 || constValue == 1));
    } else {
        return false;
    }
}
...
static bool canBeHandledByIfArrayCmpHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (cg->getX86ProcessorInfo().supportsSSE2())) {
        intptrj_t constValue = integerConstNodeValue(secondChild, cg);
        return (firstChild->getOpCodeValue() == TR::arraycmp && !firstChild->isArrayCmpLen() && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && constValue == 0);
    } else {
        return false;
    }
}
