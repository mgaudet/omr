...
bool isConditionCodeSetForCompareToZero(TR::Node* node, bool justTestZeroFlag)
{
    ...
    for (prevInstr = comp->getAppendInstruction(); prevInstr; prevInstr = prevInstr->getPrev()) {
...
TR::Register* OMR::X86::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* tempMR = generateX86MemoryReference((TR::Register*)NULL, selectorReg,
    ...
    if (cg->getLinkage()->getProperties().getMethodMetaDataRegister() != TR::RealRegister::NoReg) {
        ...
        if (secondChild->getNumChildren() > 0) {
            deps
                = generateRegisterDependencyConditions(secondChild->getFirstChild(), cg, needVMThreadDep ? 1 : 0, NULL);
...
static bool canBeHandledByIfInstanceOfHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL
        ...
        return (firstChild->getOpCodeValue() == TR:: instanceof &&firstChild->getRegister() == NULL
                    && firstChild->getReferenceCount() == 1 && (constValue == 0 || constValue == 1));
    } else {
        return false;
    }
}
...
static bool canBeHandledByIfArrayCmpHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && (cg->getX86ProcessorInfo().supportsSSE2())) {
        intptrj_t constValue = integerConstNodeValue(secondChild, cg);
        return (firstChild->getOpCodeValue() == TR::arraycmp && !firstChild->isArrayCmpLen()
            && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && constValue == 0);
    } else {
        return false;
    }
}
...
static inline void generateMergedHCRGuardCodeIfNeeded(
...
TR::Register* OMR::X86::TreeEvaluator::ifbcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        if ((firstChild->getReferenceCount() == 1) && (firstChild->getRegister() == NULL)
            ...
            && (value == 0 || (value == (int32_t)firstChild->getSecondChild()->get64bitIntegralValue()
    ...
    uint8_t* target, TR::Node* caseNode, TR::X86MemTableInstruction* jmpTableInstruction)
{
    ...
    self()->addAOTRelocation(new (self()->trHeapMemory())
