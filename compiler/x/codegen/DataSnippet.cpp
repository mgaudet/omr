...
void TR_IA32DataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR_IA32DataSnippet* snippet)
{
    ...
    trfprintf(pOutFile, "%s \t%s", snippet->getDataSize() == 8 ? dqString() : (snippet->getDataSize() == 4 ? ddString() : dwString()),
        hexPrefixString());
