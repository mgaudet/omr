...
TR::Register* OMR::X86::TreeEvaluator::dnegEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (sourceRegister->getKind() == TR_FPR) {
        ...
        targetRegister = cg->allocateRegister(TR_FPR);
        generateRegMemInstruction(
            cg->getXMMDoubleLoadOpCode(), node, targetRegister, generateX86MemoryReference(cds, cg), cg);
    ...
    TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
    ...
    //
    cg->addSnippet(
        new (cg->trHeapMemory()) TR_X86FPConvertToIntSnippet(reStartLabel, snippetLabel, helperSymRef, loadInstr, cg));
    ...
    TR::Node* node, TR::SymbolReference* helperSymRef, TR::CodeGenerator* cg)
{
    ...
    if (child->getOpCode().isDouble() && cg->useSSEForDoublePrecision()) {
        ...
        if (!optLevel) {
            int16_t fpcw = comp->getJittedMethodSymbol()->usesSinglePrecisionMode() ? SINGLE_PRECISION_ROUND_TO_NEAREST
                                                                                    : DOUBLE_PRECISION_ROUND_TO_NEAREST;
        ...
        if (optLevel) {
            ...
            if (optLevel > 1 && cg->getX86ProcessorInfo().supportsFCOMIInstructions()) {
                ...
                generateFPRegMemInstruction(
                    FLLDRegMem, node, tempFPR3, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
                ...
                generateFPRegMemInstruction(
                    FLLDRegMem, node, tempFPR4, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
                ...
                generateRegMemInstruction(
                    L4RegMem, node, accReg, generateX86MemoryReference(*convertedLongMR, 0, cg), cg);
        ...
        // Create the conversion snippet.
        //
        cg->addSnippet(new (cg->trHeapMemory()) TR_X86FPConvertToLongSnippet(
...
TR::Register* OMR::X86::TreeEvaluator::f2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    if (cg->useSSEForSinglePrecision()) {
        ...
        if (sourceRegister->getKind() == TR_X87 && child->getReferenceCount() == 1) {
            ...
            instr = generateRegRegInstruction(cvttOpCode, node, targetRegister, sourceRegister, cg);
        }

        if (TR::Compiler->target.is64Bit()) {
            cg->addSnippet(new (cg->trHeapMemory())
                    TR_AMD64FPConversionSnippet(reStartLabel, snippetLabel, helperSymRef, instr, cg));
        } else {
            cg->addSnippet(new (cg->trHeapMemory())
                    TR_X86FPConvertToIntSnippet(reStartLabel, snippetLabel, helperSymRef, instr, cg));
        }
        ...
        return fpConvertToInt(
            node, cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32floatToInt, false, false, false), cg);
    }
}
...
TR::Register* OMR::X86::TreeEvaluator::d2iEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::SymbolReference* helperSymRef
        = cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToInt, false, false, false);
    ...
    if (cg->useSSEForDoublePrecision()) {
        ...
        if (sourceRegister->getKind() == TR_X87 && child->getReferenceCount() == 1) {
            ...
            instr = generateRegMemInstruction(
                CVTTSD2SIReg4Mem, node, targetRegister, generateX86MemoryReference(*tempMR, 0, cg), cg);
        } else {
            instr = generateRegRegInstruction(CVTTSD2SIReg4Reg, node, targetRegister, sourceRegister, cg);
        }

        cg->addSnippet(
            new (cg->trHeapMemory()) TR_X86FPConvertToIntSnippet(reStartLabel, snippetLabel, helperSymRef, instr, cg));
        ...
        return fpConvertToInt(
            node, cg->symRefTab()->findOrCreateRuntimeHelper(TR_IA32doubleToInt, false, false, false), cg);
    }
}
    ...
    if (node->normalizeNanValues()) {
        ...
        if (debug("newfbits2i")) {
            ...
            TR::X86RegImmInstruction* instr
                = generateRegImmInstruction(ROL4RegImm1, node, target, 23, cg); // restore bits to proper order
            TR::RegisterDependencyConditions* deps = generateRegisterDependencyConditions((uint8_t)0, (uint8_t)1, cg);
            deps->addPostCondition(target, TR::RealRegister::NoReg, cg);
            TR_X86fbits2iSnippet* snippet
                = new (cg->trHeapMemory()) TR_X86fbits2iSnippet(restartLabel, snippetLabel, instr, cg);
