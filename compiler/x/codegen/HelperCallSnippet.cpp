...
TR_X86HelperCallSnippet::TR_X86HelperCallSnippet(TR::CodeGenerator* cg, TR::Node* node, TR::LabelSymbol* restartlab,
...
TR_X86HelperCallSnippet::TR_X86HelperCallSnippet(TR::CodeGenerator* cg, TR::LabelSymbol* restartlab,
...
void TR_X86HelperCallSnippet::addMetaDataForLoadAddrArg(uint8_t* buffer, TR::Node* child)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR_X86HelperCallSnippet* snippet)
{
...
void TR_Debug::printBody(TR::FILE* pOutFile, TR_X86HelperCallSnippet* snippet, uint8_t* bufferPos)
{
    // *this    swipeable for debugging purposes
    ...
    if (snippet->getCallNode()) {
        ...
        for (; i < snippet->getCallNode()->getNumChildren(); i++) {
            ...
            if (child->getOpCodeValue() == TR::loadaddr && !child->getRegister()) {
                ...
                TR_ASSERT(deps, "null dependencies on restart label of helper call snippet with register args");
                print(pOutFile,
                    _cg->machine()->getX86RealRegister(
                        deps->getPostConditions()->getRegisterDependency(registerArgs++)->getRealRegister()),
                    TR_WordReg);
                bufferPos++;
            }
