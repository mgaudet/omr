...
void OMR::X86::CodeGenerator::initialize(TR::Compilation* comp)
{
    ...
    // geometry.
    //
    if (((!comp->getOption(TR_DisableTLHPrefetch) && (comp->cg()->getX86ProcessorInfo().isIntelCore2()
...
void OMR::X86::CodeGenerator::beginInstructionSelection()
{
    ...
    if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
        TR_IA32ConstantDataSnippet* cds
...
void OMR::X86::CodeGenerator::endInstructionSelection()
{
    ...
    if (self()->enableSinglePrecisionMethods() && comp->getJittedMethodSymbol()->usesSinglePrecisionMode()) {
        ...
        TR_IA32ConstantDataSnippet* cds = self()->findOrCreate2ByteConstant(
 ...
 */
static bool willNotInlineCompareAndSwapNative(TR::Node* node, int8_t size, bool isObject, TR::Compilation* comp)
{
#ifdef J9_PROJECT_SPECIFIC
...
// some recognized methods might be transformed into very simple hardcoded assembly sequence which doesn't need register
// spills as a real call does
bool OMR::X86::CodeGenerator::willBeEvaluatedAsCallByCodeGen(TR::Node* node, TR::Compilation* comp)
{
    ...
    case TR::sun_misc_Unsafe_compareAndSwapObject_jlObjectJjlObjectjlObject_Z:
        return willNotInlineCompareAndSwapNative(
            node, (TR::Compiler->target.is64Bit() && !comp->useCompressedPointers()) ? 8 : 4, true, comp);
#endif
    default:
        return true;
    }
...
TR::SymbolReference* OMR::X86::CodeGenerator::findOrCreateCurrentTimeMillisTempSymRef()
{
...
TR::Instruction* OMR::X86::CodeGenerator::findBetterSpillPlacement(TR::Register* virtReg, int32_t realRegNum)
{
    ...
    if (info && (info->_freeRealRegs & TR::RealRegister::getRealRegisterMask(
    ...
    TR_RegisterKinds kindsToAssign, TR::Instruction* instructionCursor, TR::Instruction* appendInstruction)
{
    ...
    while (instructionCursor && instructionCursor != appendInstruction) {
        ...
        if (comp->cg()->getSupportsVMThreadGRA() && inst->getKind() == TR::Instruction::IsLabel
            ...
            if (node && node->getOpCodeValue() == TR::BBEnd) {
                ...
                if (block
                    && (!block->getNextBlock()
                           || !block->getNextBlock()->isExtensionOfPreviousBlock())) { // Reset vmThread register state.
...
void OMR::X86::CodeGenerator::doBinaryEncoding()
{
    ...
    if (vmThreadSpillCursor && self()->getVMThreadRegister()->getBackingStorage()) {
        if (vmThreadSpillCursor == (TR::Instruction*)0xffffffff || self()->comp()->mayHaveLoops()) {
            vmThreadSpillCursor = estimateCursor;
        }

        new (self()->trHeapMemory()) TR::X86MemRegInstruction(vmThreadSpillCursor, SMemReg(),
            generateX86MemoryReference(self()->getVMThreadRegister()->getBackingStorage()->getSymbolReference(),
                                                                  self()),
...
#define OVER_ESTIMATATION 4
    if (warmEstimate) {
        ...
        self()->setEstimatedColdLength(
            estimate - warmEstimate - MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE + OVER_ESTIMATATION);
    } else {
    ...
    if (TR::Compiler->target.is64Bit() && self()->comp()->getCodeCacheSwitched() && self()->getPicSlotCount() != 0) {
        ...
        TR_ASSERT(numTrampolinesToReserve >= 0, "Discrepancy with number of IPIC trampolines to reserve "
    ...
    while (oiIterator != self()->getOutlinedInstructionsList().end()) {
        ...
        bool needsETE
            = (*oiIterator)->getCallNode() && (*oiIterator)->getCallNode()->getSymbolReference()->canCauseGC();
    ...
    if (self()->comp()->getOption(TR_EnableGCRPatching) && self()->comp()->getRecompilationInfo()
        ...
        if (!addrToPatch) {
            ...
            self()->comp()->fe()->outOfMemory(
                self()->comp(), "Must have updated gcrPatchPointSymbol with the correct address by now");
        }
    }
#endif
...
TR_OutlinedInstructions* OMR::X86::CodeGenerator::findOutlinedInstructionsFromLabelForShrinkWrapping(
    TR::LabelSymbol* label)
{
    auto oiIterator = self()->getOutlinedInstructionsList().begin();
    while (oiIterator != self()->getOutlinedInstructionsList().end()) {
...
TR_IA32ConstantDataSnippet* OMR::X86::CodeGenerator::findOrCreateConstant(
    TR::Node* n, void* c, uint8_t size, bool isWarm)
{
    // *this    swipeable for debugging purposes
    ...
    TR::Instruction* instr, TR_BitVector** registerUsageInfo, int32_t& blockNum, int32_t& isFence, bool traceIt)
{
    ...
    if (x86Instr->getOpCode().isCallOp()) {
        ...
        if (callNode->getOpCode().hasSymbolReference()) // getSymbolReference())
        {
            ...
            if (m && ((m->isHelper() && !m->preservesAllRegisters()) || m->isVMInternalNative()
...
TR::Instruction* OMR::X86::CodeGenerator::splitEdge(TR::Instruction* instr, bool isFallThrough, bool needsJump,
...
bool OMR::X86::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    ...
    if ((method == TR::java_util_concurrent_atomic_AtomicBoolean_getAndSet)
        ...
        || (method == TR::java_util_concurrent_atomic_AtomicReference_getAndSet)
        || (method == TR::java_lang_Object_clone) || (method == TR::java_lang_Integer_rotateLeft)) {
        return true;
    } else
#endif
    {
...
uint8_t OMR::X86::CodeGenerator::k8PaddingEncoding[PADDING_TABLE_MAX_ENCODING_LENGTH][PADDING_TABLE_MAX_ENCODING_LENGTH]
    = {
        { 0x90 }, { 0x66, 0x90 }, { 0x66, 0x66, 0x90 }, { 0x66, 0x66, 0x66, 0x90 }, { 0x66, 0x66, 0x90, 0x66, 0x90 },
        { 0x66, 0x66, 0x90, 0x66, 0x66, 0x90 }, { 0x66, 0x66, 0x66, 0x90, 0x66, 0x66, 0x90 },
        { 0x66, 0x66, 0x66, 0x90, 0x66, 0x66, 0x66, 0x90 }, { 0x66, 0x66, 0x90, 0x66, 0x66, 0x90, 0x66, 0x66, 0x90 },
      };
        ...
        if (neighborhood) {
            ...
            if (neighborhood->getKind() == TR::Instruction::IsVirtualGuardNOP && guardNode
                ...
                TR::DebugCounter::incStaticDebugCounter(
                    self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "nopCount/%d/%s/%s",
                                        blockFrequency, neighborhood->description(), guardKind));
                if (length > 0) {
                    TR::DebugCounter::incStaticDebugCounter(
                        self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s/%s",
                                            blockFrequency, neighborhood->description(), guardKind));
                    ...
                    for (TR::Instruction* ninst = neighborhood->getNext(); ninst; ninst = ninst->getNext()) {
                        ...
                        if (ninst->isPatchBarrier()) {
                            if (ninst->getOpCodeValue() != LABEL)
                                TR::DebugCounter::incStaticDebugCounter(
                                    self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(),
                ...
                if (length > 0)
                    TR::DebugCounter::incStaticDebugCounter(
                        self()->comp(), TR::DebugCounter::debugCounterName(self()->comp(), "nopInst/%d/%s",
    ...
    for (auto li = _deferredSplits.begin(); li != _deferredSplits.end(); ++li) {
        ...
        if (self()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
            traceMsg(self()->comp(), "LATE EDGE SPLITTING: Pointed branch %s at vmThread-restoring label %s\n",
                self()->getDebug()->getName(*li), self()->getDebug()->getName(newLabelSymbol));
    ...
    TR::LabelSymbol* targetLabel, TR::X86LabelInstruction* instructionToDefer)
{
    ...
    if (!targetLabel->getVMThreadRestoringLabel()) {
        ...
        newLabel->setInstruction(
            generateLabelInstruction(targetLabel->getInstruction()->getPrev(), LABEL, newLabel, self()));
        self()->generateDebugCounter(
            targetLabel->getInstruction(), "cg.lateSplitEdges", 1, TR::DebugCounter::Exorbitant);
        if (self()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
            traceMsg(self()->comp(), "LATE EDGE SPLITTING: Inserted vmThread-restoring label %s before %s\n",
                self()->getDebug()->getName(newLabel), self()->getDebug()->getName(targetLabel));
    }
    ...
    if (instructionCursor->totalReferencedGPRegisters(self()) > 0) {
        ...
        if (debug("dumpGPRegStatus")) {
            self()->machine()->printGPRegisterStatus(
                self()->fe(), self()->machine()->getRegisterFile(), self()->comp()->getOutFile());
        }
    }
}
