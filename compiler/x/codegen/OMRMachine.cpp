...
OMR::X86::Machine::Machine(uint8_t numIntRegs, uint8_t numFPRegs, TR::CodeGenerator* cg,
    TR::RealRegister** registerFile, TR::Register** registerAssociations, uint8_t numGlobalGPRs,
    uint8_t numGlobal8BitGPRs, uint8_t numGlobalFPRs, TR::Register** xmmGlobalRegisters,
    ...
    TR_RegisterSizes requestedRegSize, bool considerUnlatched)
{
    ...
    if (!debug("disableForceVMThreadToEBP") && virtReg == self()->cg()->getVMThreadRegister()) {
        ...
        //
        if (_registerFile[TR::RealRegister::ebp]->getState() == TR::RealRegister::Free
            || _registerFile[TR::RealRegister::ebp]->getState() == TR::RealRegister::Unlatched) {
    ...
    TR_RegisterSizes requestedRegSize, TR::RealRegister::RegNum targetRegister, bool considerVirtAsSpillCandidate)
{
    ...
    if (!debug("disableForceVMThreadToEBP") && (virtReg == self()->cg()->getVMThreadRegister())) {
        ...
        TR_ASSERT(_registerFile[TR::RealRegister::ebp]->getState() == TR::RealRegister::Assigned,
        ...
        if (false)
            TR_ASSERT(!self()->cg()->insideInternalControlFlow(), "The instruction %p from %s, should not be inside an "
                                                                  "Internal Control Flow with a depth of %d with a "
                                                                  "safe depth of %d\n",
                currentInstruction, currentInstruction->getNode()->getOpCode().getName(),
                self()->cg()->internalControlFlowNestingDepth(), self()->cg()->internalControlFlowSafeNestingDepth());
        ...
        for (i = first; i <= last; i++) {
            ...
            if (i == TR::RealRegister::ebp && (interference & TR::RealRegister::ebpMask)
                && realReg->getAssignedRegister() == self()->cg()->getVMThreadRegister()) {
        ...
        if (self()->getDebug()) {
            self()->cg()->traceRegisterAssignment("VMThread spill instruction is now %s\n",
                (self()->cg()->getVMThreadSpillInstruction() == (TR::Instruction*)0xffffffff)
                    ? "sentinel"
                    : self()->getDebug()->getName(self()->cg()->getVMThreadSpillInstruction()));
        }
    }
...
void OMR::X86::Machine::initialiseRegisterFile(const struct TR_X86LinkageProperties& properties)
{
    // *this    swipeable for debugging purposes
    int reg;
...
uint32_t* OMR::X86::Machine::getGlobalRegisterTable(const struct TR_X86LinkageProperties& property)
{
...
void OMR::X86::Machine::disassociateUnspilledBackingStorage()
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= endReg;
        ...
        if (_registerFile[i]->getState() == TR::RealRegister::Assigned) {
            ...
            if (location) {
                int32_t size = (virtReg->getKind() == TR_FPR) ? (virtReg->isSinglePrecision() ? 4 : 8)
