...
TR_IA32DataSnippet* OMR::X86::MemoryReference::getDataSnippet()
{
    return (self()->hasUnresolvedDataSnippet() || self()->hasUnresolvedVirtualCallSnippet())
        ? NULL
        : (TR_IA32DataSnippet*)_dataSnippet;
}
...
uint32_t OMR::X86::MemoryReference::getBinaryLengthLowerBound(TR::CodeGenerator* cg)
{
    ...
    if (self()->getBaseRegister()) {
        ...
        if (registerNumber == TR::RealRegister::vfp) {
            ...
            registerNumber
                = toRealRegister(cg->machine()->getX86RealRegister(registerNumber)->getAssignedRealRegister())
                      ->getRegisterNumber();
        }
    }
    ...
    case 2: {
        if (self()->needsCodeAbsoluteExternalRelocation()) {
            cg->addAOTRelocation(new (cg->trHeapMemory())
                                     TR_ExternalRelocation(cursor, 0, TR_AbsoluteMethodAddress, cg),
            ...
            && cg->wantToPatchClassPointer(NULL, cursor)) // might not point to beginning of class
        {
            cg->jitAdd32BitPicToPatchOnClassRedefinition(
                (void*)*(int32_t*)cursor, (void*)cursor, self()->getUnresolvedDataSnippet() != NULL);
        }
                    ...
                    if (symbol->isConst()) {
                        ...
                        if (cg->needClassAndMethodPointerRelocations()) {
                            *(int32_t*)cursor = (int32_t)(TR::Compiler->cls.persistentClassPointerFromClassPointer(
                                cg->comp(), (TR_OpaqueClassBlock*)(self()->getSymbolReference().getOffset()
                                                + (intptrj_t)staticSym->getStaticAddress())));
                            cg->addAOTRelocation(
                                new (cg->trHeapMemory())
                                    TR_ExternalRelocation(cursor, (uint8_t*)&self()->getSymbolReference(),
                        ...
                        if (staticSym->isCountForRecompile()) {
                            ...
                            cg->addAOTRelocation(
                                new (cg->trHeapMemory()) TR_ExternalRelocation(cursor,
                                    ...
                                    node ? (uint8_t*)node->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress, cg),
                                __FILE__, __LINE__, node);
                        }
                    }
                } else {
    ...
    uint8_t* modRM, TR::Instruction* containingInstruction, TR::CodeGenerator* cg)
{
    ...
    case 1: {
        ...
        if (baseRegisterNumber == TR::RealRegister::vfp) {
            ...
            base = toRealRegister(cg->machine()->getX86RealRegister(baseRegisterNumber)->getAssignedRealRegister());
            baseRegisterNumber = base->getRegisterNumber();
            ...
            if (label != NULL) {
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    *(int32_t*)cursor = -(int32_t)(intptrj_t)(
                        cursor + 4 + TR_X86OpCode::getImmediateSize(containingInstruction->getOpCodeValue()));
                } else {
...
TR::MemoryReference* generateX86MemoryReference(TR_IA32DataSnippet* cds, TR::CodeGenerator* cg)
{
