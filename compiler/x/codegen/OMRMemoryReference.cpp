    ...
    TR::CodeGenerator* cg, TR::Node* parent)
{
    ...
    if (evalSubTree && subTree->getReferenceCount() > 1 || subTree->getRegister() != NULL || self()->inUpcastingMode() && !subTree->cannotOverflow()) {
        if (_baseRegister != NULL) {
            if (_indexRegister != NULL) {
...
OMR::X86::MemoryReference::getBinaryLengthLowerBound(TR::CodeGenerator* cg)
{
    ...
    if (self()->getBaseRegister()) {
        ...
        if (registerNumber == TR::RealRegister::vfp) {
            ...
            registerNumber = toRealRegister(cg->machine()->getX86RealRegister(registerNumber)->getAssignedRealRegister())->getRegisterNumber();
        }
    }
