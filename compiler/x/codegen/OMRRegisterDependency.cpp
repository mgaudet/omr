    ...
    TR_RegisterKinds kindsToBeAssigned, TR_X86RegisterDependencyIndex numberOfRegisters, TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (cg->getSupportsVMThreadGRA() && dependentRegNum == TR::RealRegister::ebp && virtReg
            && virtReg != vmThreadReg && vmThreadReg->getAssignedRealRegister() == vmThreadRealReg) {
            ...
            TR::X86RegMemInstruction* inst
                = generateRegMemInstruction(currentInstruction, LRegMem(), vmThreadRealReg, tempMR, cg);
