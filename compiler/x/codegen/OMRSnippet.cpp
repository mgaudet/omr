...
int32_t TR_Debug::printRestartJump(TR::FILE* pOutFile, TR_X86RestartSnippet* snippet, uint8_t* bufferPos)
{
    int32_t size
...
#ifdef J9_PROJECT_SPECIFIC
int32_t TR_Debug::printRestartJump(TR::FILE* pOutFile, TR_AMD64WriteBarrierSnippet* snippet, uint8_t* bufferPos)
{
    int32_t size
        = snippet->estimateRestartJumpLength(JMP4, bufferPos - (uint8_t*)snippet->cg()->getBinaryBufferStart());
    printPrefix(pOutFile, NULL, bufferPos, size);
...
int32_t TR_Debug::printRestartJump(
    TR::FILE* pOutFile, TR_X86RestartSnippet* snippet, uint8_t* bufferPos, int32_t branchOp, const char* branchOpName)
{
