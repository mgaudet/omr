...
static inline bool isByteLengthSmallEnough(uintptrj_t byteLength) { return true; }
        ...
        if ((*iterator)->getRematerializationInfo()->isRematerializableFromMemory()
            ...
            if (debug("dumpRemat")) {
                diagnostic("\n---> Deleting static discardable candidate %s at %s instruction " POINTER_PRINTF_FORMAT,
                    regCursor->getRegisterName(comp), comp->getAppendInstruction()->getOpCode().getOpCodeName(cg),
...
TR::Register* OMR::X86::TreeEvaluator::arraycmpEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    generateRegMemInstruction(
        (byteLen == 4) ? L4RegMem : L8RegMem, node, src1Reg, generateX86MemoryReference(src1AddrReg, 0, cg), cg);

    generateRegMemInstruction(
        CMPRegMem(), node, src1Reg, generateX86MemoryReference(src1AddrReg, src2AddrReg, 0, 0, cg), cg);
...
TR::Register* OMR::X86::TreeEvaluator::SSE2IfArraycmpEvaluator(
    TR::Node* node, TR::LabelSymbol* targetLabel, bool branchonEqual, TR::CodeGenerator* cg)
{
...
void constLengthArrayCopy(TR::Node* node, TR::CodeGenerator* cg, TR::Register* byteSrcReg, TR::Register* byteDstReg,
    TR::Node* byteLenNode, bool preserveSrcPointer, bool preserveDstPointer)
{
    ...
    if (node->isForwardArrayCopy() && byteLength >= 64 && byteLength <= 160) {
        struct TR_FancyConstLengthStrategy {
            ...
            {
                return (p.isIntelCore2() || p.isIntelNehalem() || p.isIntelWestmere() || p.isIntelSandyBridge()
                           || p.isAMD15h())
                    ? _core2Latency
        ...
        static TR_FancyConstLengthStrategy fancyConstLengthStrategies[] = {
            { "none" }, { "4-byte mov", L4RegMem, S4MemReg, 2, TR_GPR, 3, 3, 3, 4 },
            { "8-byte mov", L8RegMem, S8MemReg, 3, TR_GPR, 3, 3, 3, 4 }, // Only valid when is64BitTarget
            { "movq", MOVQRegMem, MOVQMemReg, 3, TR_FPR, 4, 4, 4,
                4 }, // Not the latencies from the proc manuals... beyond 4 regs it doesn't seem to make much difference
        ...
        if ((stratNum > 0) && ((byteLength & strideMask) == 0)
            ...
            int32_t numIterations = numCopies / numRegs; // An "iteration" is a sequence of loads and stores using all
                                                         // the registers.  Has nothing to do with loops (though it is
                                                         // true that a loop, if generated, does exactly one iteration).
            int32_t residue = numCopies - numIterations * numRegs;

            if (comp->getOption(TR_TraceCG)) {
                traceMsg(comp, "   strategy:%d (%s) stride:%d bytes:%d architecture:%x\n", stratNum, strat->_name,
                    strat->_stride, 1 << strat->_stride, p.getX86Architecture());
                traceMsg(comp, "   copies:%d regs:%d ramp:%d iterations:%d residue:%d\n", numCopies, numRegs, ramp,
                    numIterations, residue);
            }
            ...
            for (copyNum = 0; copyNum < ramp; copyNum++) {
                ...
                generateRegMemInstruction(strat->_loadOpCode, node, regs[regIndex],
                    generateX86MemoryReference(byteSrcReg, copyNum << strat->_stride, cg), cg);
            }
            ...
            for (copyNum = ramp; copyNum < numCopies; copyNum++) {
                ...
                generateMemRegInstruction(strat->_storeOpCode, node,
                    ...
                    regs[regIndex], cg);
                generateRegMemInstruction(strat->_loadOpCode, node, regs[regIndex],
                    generateX86MemoryReference(byteSrcReg, inductionReg, 0, copyNum << strat->_stride, cg), cg);
            }
            ...
            if (ramp < numRegs) {
                ...
                for (copyNum = 0; copyNum < ramp; copyNum++) {
                    regIndex = copyNum % numRegs;
                    generateMemRegInstruction(strat->_storeOpCode, node,
                        generateX86MemoryReference(byteDstReg, copyNum << strat->_stride, cg), regs[regIndex], cg);
                }
            } else {
                ...
                for (copyNum = numCopies; copyNum < numCopies + ramp; copyNum++) {
                    regIndex = copyNum % numRegs;
                    generateMemRegInstruction(strat->_storeOpCode, node,
                        generateX86MemoryReference(byteDstReg, inductionReg, 0, (copyNum - numRegs) << strat->_stride,
                                                  cg),
                        regs[regIndex], cg);
                }
            }
    ...
    if (wordCount <= 3) {
        ...
        while (wordCount--) {
            TR::MemoryReference* tempSrcMR = generateX86MemoryReference(
                byteSrcReg, constWordSize * (originalWordCount - wordCount) - constWordSize, cg);
            generateRegMemInstruction(LRegMem(), node, lenReg, tempSrcMR, cg);
            TR::MemoryReference* tempDstMR = generateX86MemoryReference(
                byteDstReg, constWordSize * (originalWordCount - wordCount) - constWordSize, cg);
        ...
        if (reportConstArrayCopy)
            diagnostic(
                "constArrayCopy: found one inline with bytelength = %d in method %s\n", byteLength, comp->signature());
    } else if (!disableConstArrayCopyLoop && wordCount < 64
        && (byteSrcReg != byteDstReg)) // word count must be > 1 or pipelined loop below is bad
    {
        ...
        if (srcIsOffset && preserveSrcPointer) {
            generateRegMemInstruction(LEARegMem(), node, byteSrcReg,
                generateX86MemoryReference(byteDstReg, byteSrcReg, 0, -numBytesMoved, cg), cg);
            srcPtrChanged = false;
        } else if (srcIsOffset) {
        ...
        if (reportConstArrayCopy)
            diagnostic("constArrayCopy: found one repmovs%s with bytelength = %d in method %s\n",
                TR::Compiler->target.is64Bit() ? "q" : "d", byteLength, comp->signature());
    }
...
TR::Register* OMR::X86::TreeEvaluator::constLengthArrayCopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR::Register* OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isArrayStoreCheckUnnecessary && (alwaysInlineArrayCopy(cg) || node->isForwardArrayCopy())
        && byteLenNode->getOpCode().isLoadConst() && isByteLengthSmallEnough(constantByteLength)
...
#ifdef J9_PROJECT_SPECIFIC
    if (m && (m->getRecognizedMethod() == TR::java_util_Arrays_copyOf_byte
...
#ifdef J9_PROJECT_SPECIFIC
    if (isArrayStoreCheckUnnecessary && !useSSECopy && comp->getRecompilationInfo()
        ...
        if (valueInfo) {
            if ((valueInfo->_totalFrequency > 0) && (valueInfo->_frequency1 > (valueInfo->_totalFrequency / 2))
    ...
    if (isArrayStoreCheckUnnecessary && !shortCopy && node->isForwardArrayCopy()
        && (node->isWordElementArrayCopy() || node->isHalfWordElementArrayCopy()) && !useSSECopy) {
        intptrj_t size = 0;
        if (byteLenNode && byteLenNode->getRegister() == NULL && byteLenNode->getOpCode().isMul()
    ...
    if ((shortCopy && node->isForwardArrayCopy() && !node->isWordElementArrayCopy()
    ...
    if (node->isForwardArrayCopy()) {
        ...
        if (useSSECopy) {
            ...
            if (!shortCopy) {
                ...
                helper = (p.isAMDOpteron() || p.isAMD15h()) ? TR_IA32SSEforwardArrayCopyAMDOpteron
        ...
        if (shortCopy && !generateInlineJumpTable && !useSSECopy) {
            ...
            TR_X86HelperCallSnippet* snippet
                = new (cg->trHeapMemory()) TR_X86HelperCallSnippet(cg, node, restartLabel, snippetLabel, helperSymRef);
            ...
            generateRegMemInstruction(
                LEARegMem(), node, byteDstReg, generateX86MemoryReference(byteDstReg, byteLenReg, 0, cg), cg);

            generateRegMemInstruction(
                LEARegMem(), node, byteSrcReg, generateX86MemoryReference(byteSrcReg, byteLenReg, 0, cg), cg);
            ...
            TR::MemoryReference* mr
                = generateX86MemoryReference(NULL, byteLenReg, scale, (intptrj_t)&fwdHalfWordCopyTable, cg);
        ...
        if (!shortCopy && !(elementSize == 8 && TR::Compiler->target.is32Bit())) {
            ...
            generateRegMemInstruction(
                LEARegMem(), node, byteDstReg, generateX86MemoryReference(byteDstReg, byteSrcReg, 0, cg), cg);
        ...
        if (elementSize == 8) {
            ...
            helper = TR::Compiler->target.is64Bit()
                ? (useSSECopy) ? TR_AMD64halfWordArrayCopyAggressive : TR_AMD64halfWordArrayCopy
            ...
            helper = TR::Compiler->target.is64Bit()
                ? (false ? TR_AMD64BCarrayCopy : (useSSECopy) ? TR_AMD64arrayCopyAggressive : TR_AMD64arrayCopy)
...
TR::Register* OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
        ...
        if (comp->getOption(TR_DisableLateEdgeSplitting)) {
            ...
            if (node->getNumChildren() > 0)
                inst = generateLabelInstruction(
                    LABEL, node, label, node->getFirstChild(), &popRegisters, needVMThreadDep, cg);
            else
    ...
    if (comp->getOption(TR_BreakBBStart)) {
        ...
        generateRegImmInstruction(
            TEST4RegImm4, node, machine->getX86RealRegister(TR::RealRegister::esp), block->getNumber(), cg);
...
TR::Register* OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
        ...
        bool needVMThreadDep = comp->getOption(TR_DisableLateEdgeSplitting)
            || !performTransformation(comp, "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n",
                                   node->getOpCode().getName(), cg->getDebug()->getName(node));
        ...
        if (node->getNumChildren() > 0)
            labelInst = generateLabelInstruction(
                LABEL, node, generateLabelSymbol(cg), node->getFirstChild(), NULL, needVMThreadDep, cg);
        else
...
TR::Register* OMR::X86::TreeEvaluator::VMarrayStoreCheckArrayCopyEvaluator(TR::Node*, TR::CodeGenerator*) { return 0; }

TR::Register* OMR::X86::TreeEvaluator::VMAESHelperEvaluator(TR::Node* node, TR::CodeGenerator* cg) { return 0; }

bool OMR::X86::TreeEvaluator::VMinlineCallEvaluator(TR::Node*, bool, TR::CodeGenerator*) { return false; }

TR::Register* OMR::X86::TreeEvaluator::VMifInstanceOfEvaluator(TR::Node*, TR::CodeGenerator*) { return 0; }

TR::Register* OMR::X86::TreeEvaluator::VMifArrayCmpEvaluator(TR::Node*, TR::CodeGenerator*) { return 0; }
