    ...
    TR::Instruction* currentInstruction)
{
    ...
    if (currentInstruction) {
        ...
        if (target && node && node->getOpCodeValue() == TR::aconst && node->isClassPointerConstant() && (cg->fe()->isUnloadAssumptionRequired((TR_OpaqueClassBlock*)node->getAddress(),
    ...
    bool preserveDstPointer)
{
    ...
    if (node->isForwardArrayCopy() && byteLength >= 64 && byteLength <= 160) {
        struct TR_FancyConstLengthStrategy {
            ...
            {
                return (p.isIntelCore2() || p.isIntelNehalem() || p.isIntelWestmere() || p.isIntelSandyBridge() || p.isAMD15h()) ? _core2Latency : p.isAMDOpteron() ? _opteronLatency : _pentium4Latency;
            }
        };
    ...
    if (useSSECopy && byteLength > 48) {
        ...
        TR::X86ImmSymInstruction* callInstr = generateHelperCallInstruction(node, TR_IA32forwardSSEArrayCopyNoAlignCheck, sseDeps, cg);
    ...
    } else if (!disableConstArrayCopyLoop && wordCount < 64 && (byteSrcReg != byteDstReg)) // word count must be > 1 or pipelined loop below is bad
    {
        ...
        TR_RematerializableTypes type = byteLenNode->getOpCode().isLong() ? TR_RematerializableLong : TR_RematerializableInt;
...
TR::Register* OMR::X86::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isArrayStoreCheckUnnecessary && (alwaysInlineArrayCopy(cg) || node->isForwardArrayCopy()) && byteLenNode->getOpCode().isLoadConst() && isByteLengthSmallEnough(constantByteLength) && !disableConstantLengthArrayCopy) {
...
#ifdef J9_PROJECT_SPECIFIC
    if (m && (m->getRecognizedMethod() == TR::java_util_Arrays_copyOf_byte || m->getRecognizedMethod() == TR::java_io_ByteArrayOutputStream_write || m->getRecognizedMethod() == TR::java_io_ObjectInputStream_BlockDataInputStream_read)) {
        useSSECopy = true;
    }
#endif
...
#ifdef J9_PROJECT_SPECIFIC
    if (isArrayStoreCheckUnnecessary && !useSSECopy && comp->getRecompilationInfo() && (node->isHalfWordElementArrayCopy() || node->isWordElementArrayCopy())) {
        TR_ValueInfo* valueInfo = (TR_ValueInfo*)TR_ValueProfiler::getProfiledValueInfo(node, comp);
        if (valueInfo) {
            if ((valueInfo->_totalFrequency > 0) && (valueInfo->_frequency1 > (valueInfo->_totalFrequency / 2)) && (valueInfo->_value1 > 24)) {
                shortCopy = false;
            }
        }
    }
    ...
    if (isArrayStoreCheckUnnecessary && !shortCopy && node->isForwardArrayCopy() && (node->isWordElementArrayCopy() || node->isHalfWordElementArrayCopy()) && !useSSECopy) {
        intptrj_t size = 0;
        if (byteLenNode && byteLenNode->getRegister() == NULL && byteLenNode->getOpCode().isMul() && byteLenNode->getSecondChild()->getOpCode().isLoadConst()) {
    ...
    if ((shortCopy && node->isForwardArrayCopy() && !node->isWordElementArrayCopy() && cg->useSSEForDoublePrecision())) {
    ...
    if (node->isForwardArrayCopy()) {
        ...
        if (elementSize == 8) {
            ...
            helper = TR::Compiler->target.is64Bit() ? (useSSECopy) ? TR_AMD64wordArrayCopyAggressive : TR_AMD64wordArrayCopy
            ...
            helper = TR::Compiler->target.is64Bit() ? (false ? TR_AMD64BCarrayCopy : (useSSECopy) ? TR_AMD64arrayCopyAggressive : TR_AMD64arrayCopy)
...
TR::Register* OMR::X86::TreeEvaluator::directCallEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
...
#ifdef J9_PROJECT_SPECIFIC
    if (cg->enableAESInHardwareTransformations() && (symbol->getRecognizedMethod() == TR::com_ibm_jit_crypto_JITAESCryptInHardware_doAESInHardware || symbol->getRecognizedMethod() == TR::com_ibm_jit_crypto_JITAESCryptInHardware_expandAESKeyInHardware)) {
        return TR::TreeEvaluator::VMAESHelperEvaluator(node, cg);
    }

    if (symbol->getMandatoryRecognizedMethod() == TR::com_ibm_jit_JITHelpers_transformedEncodeUTF16Big || symbol->getMandatoryRecognizedMethod() == TR::com_ibm_jit_JITHelpers_transformedEncodeUTF16Little) {
    ...
    // actually be generated inline.
    //
    else if (symbol->isVMInternalNative() || symbol->isJITInternalNative() || (symbol->getRecognizedMethod() == TR::java_lang_Integer_rotateLeft) || (symbol->getRecognizedMethod() == TR::java_lang_Math_sqrt) || (symbol->getRecognizedMethod() == TR::java_lang_StrictMath_sqrt) || (symbol->getRecognizedMethod() == TR::java_lang_Math_max_I) || (symbol->getRecognizedMethod() == TR::java_lang_Math_min_I) || (symbol->getRecognizedMethod() == TR::java_lang_Math_max_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_min_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_L) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_D) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_F) || (symbol->getRecognizedMethod() == TR::java_lang_Math_abs_I) || (symbol->getRecognizedMethod() == TR::java_lang_Long_reverseBytes) || (symbol->getRecognizedMethod() == TR::java_lang_Integer_reverseBytes) || (symbol->getRecognizedMethod() == TR::java_lang_Short_reverseBytes) || (symbol->getRecognizedMethod() == TR::java_lang_Class_isAssignableFrom) || (symbol->getRecognizedMethod() == TR::java_lang_System_nanoTime) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordCAS) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordCAS) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordCASSupported) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordCASSupported) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicMarkableReference_doubleWordSetSupported) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicStampedReference_doubleWordSetSupported) ||

        (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_Fences_orderAccesses) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_Fences_orderReads) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_Fences_orderWrites) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_Fences_reachabilityFence) ||

        (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicBoolean_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_getAndAdd) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_getAndIncrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_getAndDecrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_addAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_decrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicInteger_incrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicReference_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndAdd) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndIncrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndDecrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_addAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_decrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerArray_incrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicReferenceArray_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndAdd) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndIncrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndDecrement) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndSet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_addAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_decrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_incrementAndGet) || (symbol->getRecognizedMethod() == TR::java_util_concurrent_atomic_AtomicReference_getAndSet) || (symbol->getRecognizedMethod() == TR::sun_nio_ch_NativeThread_current) || (symbol->getRecognizedMethod() == TR::sun_misc_Unsafe_copyMemory) || (symbol->getRecognizedMethod() == TR::java_lang_String_hashCodeImplCompressed) || (symbol->getRecognizedMethod() == TR::java_lang_String_hashCodeImplDecompressed)) {
...
TR::Register* OMR::X86::TreeEvaluator::BBStartEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!block->isExtensionOfPreviousBlock()) {
        ...
        if (comp->getOption(TR_DisableLateEdgeSplitting)) {
            ...
            if (cg->getSupportsVMThreadGRA() && edge && edge->mustRestoreVMThreadRegister()) {
            ...
            bool needVMThreadDep = !performTransformation(comp, "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n",
...
TR::Register* OMR::X86::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
        ...
        if (cg->enableRegisterAssociations() && comp->getAppendInstruction()->getOpCodeValue() != ASSOCREGS) {
            machine->createRegisterAssociationDirective(comp->getAppendInstruction());
        }

        bool needVMThreadDep = comp->getOption(TR_DisableLateEdgeSplitting) || !performTransformation(comp, "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n", node->getOpCode().getName(), cg->getDebug()->getName(node));
