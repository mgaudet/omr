...
TR::X86LabelInstruction* TR::X86LabelInstruction::getIA32LabelInstruction() { return this; }
    ...
    if (kindsToBeAssigned & TR_GPR_Mask) {
        ...
        if (!comp->getOption(TR_DisableLateEdgeSplitting) && cg()->getProperties().getMethodMetaDataRegister()) {
            ...
            if (getOpCodeValue() == LABEL && ebpIsVMThreadRegister) {
                ...
                && !getNode()->getBlock()->isExtensionOfPreviousBlock() && cg()->hasDeferredSplits()) {
                ...
                if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
                    traceMsg(comp,
                        "O^O LATE EDGE SPLITTING: BBStart label %s needs vmthread in ebp because of deferred splits\n",
                ...
                if (getNode()->getOpCodeValue() == TR::BBEnd) {
                    ...
                    if (_symbol->getInstruction() && !cg()->findOutlinedInstructionsFromLabel(_symbol)) {
                        ...
                        if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting) && newLabelSymbol != _symbol)
                            traceMsg(comp,
                                "O^O LATE EDGE SPLITTING: Pointed branch %s at vmThread-restoring label %s\n",
                                cg()->getDebug()->getName(this), cg()->getDebug()->getName(newLabelSymbol));
...
void TR::X86RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
        ...
        if (feGetEnv("TR_UseOutOfDatedRegRevMoveList")) {
            if (opCode == MOVAPSRegReg || opCode == MOV8RegReg || opCode == MOV4RegReg || opCode == MOV2RegReg
                || opCode == MOV1RegReg) {
                regRegCopy = true;
            } else {
                regRegCopy = false;
...
{
    // *this    swipeable for debugging purposes
    if (reg == getTargetRegister() || reg == getSourceRegister() || reg == getSourceRightRegister()) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->refsRegister(reg);
    }
        ...
        if (!ebpIsVMThreadRegister) {
            ...
            for (ncount_t i = 0; i < getNumRelocations(); i++) {
                ...
                if (targetLabel->getInstruction()) {
                    ...
                    if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
                        traceMsg(cg()->comp(), "O^O LATE EDGE SPLITTING: Pointed jump table entry %d of %s at "
                                               "vmThread-restoring label %s\n",
                            i, cg()->getDebug()->getName(this), cg()->getDebug()->getName(relocation->getLabel()));
                }
            }
        }
    }
        ...
        if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
            ...
            if (blockedEbp) {
                ...
                case TR::RealRegister::Free:
                    cg()->machine()->getX86RealRegister(TR::RealRegister::ebp)->resetState(TR::RealRegister::Free);
                    break;
                case TR::RealRegister::Unlatched:
                    cg()->machine()->getX86RealRegister(TR::RealRegister::ebp)->resetState(TR::RealRegister::Unlatched);
                    break;
                case TR::RealRegister::Assigned:
                    cg()->machine()->getX86RealRegister(TR::RealRegister::ebp)->resetState(TR::RealRegister::Assigned);
                    break;
                case TR::RealRegister::Blocked:
                    cg()->machine()->getX86RealRegister(TR::RealRegister::ebp)->resetState(TR::RealRegister::Blocked);
                    break;
                case TR::RealRegister::Locked:
                    cg()->machine()->getX86RealRegister(TR::RealRegister::ebp)->resetState(TR::RealRegister::Locked);
                    break;
                }
            }
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(TR_X86OpCodes op, TR::Node* node, TR::MemoryReference* mr,
    TR::Register* slreg, TR::Register* srreg, TR::CodeGenerator* cg)
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(TR::Instruction* precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference* mr, TR::Register* slreg, TR::Register* srreg, TR::CodeGenerator* cg)
...
TR::X86MemRegRegInstruction::X86MemRegRegInstruction(TR::Instruction* precedingInstruction, TR_X86OpCodes op,
    TR::MemoryReference* mr, TR::Register* slreg, TR::Register* srreg, TR::RegisterDependencyConditions* cond,
    TR::CodeGenerator* cg)
...
{
    // *this    swipeable for debugging purposes
    if (getMemoryReference()->refsRegister(reg) || reg == getSourceRegister() || reg == getSourceRightRegister()) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->refsRegister(reg);
    }
    ...
    if (kindsToBeAssigned & TR_X87_Mask) {
        ...
        if (result & (kSourceCanBePopped | kTargetCanBePopped)) {
            ...
            if ((result & (kSourceCanBePopped | kTargetCanBePopped)) == (kSourceCanBePopped | kTargetCanBePopped)) {
                ...
                //
                TR_ASSERT(0, "TR::X86FPCompareRegRegInstruction::assignRegisters() ==> source operand should not be "
                             "dead after compare!");
            }
...
bool TR::X86FPCompareRegRegInstruction::swapOperands()
{
    ...
    if (cursor != NULL) {
        ...
        TR_ASSERT(0, "\nTR::X86FPCompareRegRegInstruction::swapOperands() ==> branch or set instruction not found for "
                     "FP comparison!\n");
    }
...
void TR::X86FPCompareEvalInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & TR_GPR_Mask) {
        ...
        case TR::dcmpgtu:
        case TR::fcmpgtu:
            TR_ASSERT(0, "TR::X86FPCompareEvalRegInstruction::assignRegisters() ==> shouldn't be able to generate this "
                         "condition!\n");
            break;
    ...
    TR_X86OpCodes op, TR::Node* node, TR::LabelSymbol* sym, bool needsVMThreadRegister, TR::CodeGenerator* cg)
{
    ...
    if (needsVMThreadRegister
        && cg->getLinkage()->getProperties().getMethodMetaDataRegister() != TR::RealRegister::NoReg) {
...
TR::X86LabelInstruction* generateLabelInstruction(
    TR::Instruction* pred, TR_X86OpCodes op, TR::LabelSymbol* sym, bool needsVMThreadRegister, TR::CodeGenerator* cg)
{
    TR::X86LabelInstruction* instr;
    if (needsVMThreadRegister
        && cg->getLinkage()->getProperties().getMethodMetaDataRegister() != TR::RealRegister::NoReg) {
...
TR::X86LabelInstruction* generateLabelInstruction(TR_X86OpCodes opCode, TR::Node* node, TR::LabelSymbol* label,
    TR::Node* glRegDep, List<TR::Register>* popRegisters, bool needsVMThreadRegister, bool evaluateGlRegDeps,
    TR::CodeGenerator* cg)
{
    TR::X86LabelInstruction* instr;
...
TR::X86RestoreVMThreadInstruction* generateRestoreVMThreadInstruction(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR_X86OpCodes opCode, TR::Node* jumpNode, TR::CodeGenerator* cg, bool needsVMThreadRegister, bool evaluateGlRegDeps)
{
    ...
    (jumpNode->getNumChildren() > 0)
        ? inst = generateLabelInstruction(opCode, jumpNode, destinationLabel, jumpNode->getFirstChild(), 0,
              needsVMThreadRegister, evaluateGlRegDeps, cg)
        : inst = generateLabelInstruction(opCode, jumpNode, destinationLabel, needsVMThreadRegister, cg);
    ...
    TR_X86OpCodes opCode, TR::Node* ifNode, TR::CodeGenerator* cg, bool needsVMThreadRegister)
{
    ...
    if (!comp->getOption(TR_DisableLateEdgeSplitting)) {
        needsVMThreadRegister = needsVMThreadRegister
    ...
    if (ifNode->getNumChildren() == 3) {
        ...
        inst = generateLabelInstruction(
            opCode, ifNode, destinationLabel, glRegDep, &popRegisters, needsVMThreadRegister, cg);
...
TR::AMD64Imm64SymInstruction* generateImm64SymInstruction(
    TR_X86OpCodes op, TR::Node* node, uint64_t imm, TR::SymbolReference* sr, TR::CodeGenerator* cg, uint8_t* thunk)
{
