...
void TR::X86RegRegInstruction::assignRegisters(TR_RegisterKinds kindsToBeAssigned)
{
    ...
    if (kindsToBeAssigned & getTargetRegister()->getKindAsMask()) {
        ...
        if (feGetEnv("TR_UseOutOfDatedRegRevMoveList")) {
            if (opCode == MOVAPSRegReg || opCode == MOV8RegReg || opCode == MOV4RegReg || opCode == MOV2RegReg || opCode == MOV1RegReg) {
                regRegCopy = true;
            } else {
                regRegCopy = false;
            }
...
{
    // *this    swipeable for debugging purposes
    if ((reg == getTargetRegister() && getOpCode().usesTarget()) || reg == getSourceRegister() || reg == getSourceRightRegister()) {
        return true;
    } else if (getDependencyConditions()) {
        return getDependencyConditions()->usesRegister(reg);
    }
    ...
    if ((cg()->getAssignmentDirection() == cg()->Backward)) {
        ...
        if (kindsToBeAssigned & getSourceRegister()->getKindAsMask()) {
            ...
            if (assignedSourceRegister != NULL) {
                ...
                if (getSourceRegister()->decFutureUseCount() == 0 && assignedSourceRegister->getState() != TR::RealRegister::Locked) {
                    cg()->traceRegFreed(getSourceRegister(), assignedSourceRegister);
                    getSourceRegister()->setAssignedRegister(NULL);
                    assignedSourceRegister->setState(TR::RealRegister::Unlatched);
                }

                if (_sourceRightRegister->decFutureUseCount() == 0 && assignedSourceRightRegister->getState() != TR::RealRegister::Locked) {
    ...
    bool evaluateGlRegDeps)
{
    ...
    (jumpNode->getNumChildren() > 0) ? inst = generateLabelInstruction(opCode, jumpNode, destinationLabel, jumpNode->getFirstChild(), 0, needsVMThreadRegister, evaluateGlRegDeps, cg) : inst = generateLabelInstruction(opCode,
                                                                                                                                                                                             jumpNode,
                                                                                                                                                                                             destinationLabel,
    ...
    bool needsVMThreadRegister)
{
    ...
    if (!comp->getOption(TR_DisableLateEdgeSplitting)) {
        needsVMThreadRegister = needsVMThreadRegister && !performTransformation(comp, "O^O LATE EDGE SPLITTING: Omit ebp dependency for %s node %s\n", ifNode->getOpCode().getName(), cg->getDebug()->getName(ifNode));
    }
