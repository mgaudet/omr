...
{
    _entryLabel->setStartOfColdInstructionStream();
    _block = callNode->getOpCode().hasSymbolReference() && callNode->getSymbolReference()->canCauseGC()
...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::Node* callNode, TR::ILOpCodes callOp, TR::Register* targetReg,
    TR::LabelSymbol* entryLabel, TR::LabelSymbol* restartLabel, bool rematerializeVMThread, TR::CodeGenerator* cg)
    : _restartLabel(restartLabel)
    , _firstInstruction(NULL)
    , _appendInstruction(NULL)
    , _targetReg(targetReg)
...
TR_OutlinedInstructions::TR_OutlinedInstructions(TR::Node* callNode, TR::ILOpCodes callOp, TR::Register* targetReg,
    TR::LabelSymbol* entryLabel, TR::LabelSymbol* restartLabel, TR_X86OpCodes targetRegMovOpcode, TR::CodeGenerator* cg)
    : _restartLabel(restartLabel)
    , _firstInstruction(NULL)
    , _appendInstruction(NULL)
    , _targetReg(targetReg)
...
void TR_OutlinedInstructions::generateOutlinedInstructionsDispatch()
{
    ...
    if (_rematerializeVMThread) {
        ...
        TR::MemoryReference* vmThreadMR
            = generateX86MemoryReference(vmThreadReg, (TR::Compiler->target.is64Bit()) ? 16 : 8, _cg);
    ...
    if (_targetReg) {
        ...
        if (targetRegPair) {
            ...
            generateRegRegInstruction(
                _targetRegMovOpcode, _callNode, targetRegPair->getLowOrder(), resultRegPair->getLowOrder(), _cg);
            generateRegRegInstruction(
                _targetRegMovOpcode, _callNode, targetRegPair->getHighOrder(), resultRegPair->getHighOrder(), _cg);
        } else {
        ...
        if (child->getRegister() != NULL) {
            ...
            if ((child->getOpCodeValue() == TR::loadaddr) &&
                /*(callNode->getOpCodeValue() == TR::instanceof || callNode->getOpCodeValue() == TR::checkcast ||
                   callNode->getOpCodeValue() == TR::checkcastAndNULLCHK || callNode->getOpCodeValue() == TR::New ||
                   callNode->getOpCodeValue() == TR::anewarray)    &&*/
                ...
                // will catch
                // the case of an unevaluated argument not persisting outside of the outlined region even though one of
                // its subtrees will.
                //
