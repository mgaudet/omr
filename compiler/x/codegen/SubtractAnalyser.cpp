...
void TR_X86SubtractAnalyser::longSubtractAnalyser(TR::Node* root)
{
    ...
    if (getCopyReg1()) {
        ...
        } else // assert getSubReg3Mem2() == true
        {
             ...
             * bytecode that shifts
             * by 32 bits. This is the only case where the useSecondHighOrder bit is set.
             * When the first child of the lushr is in a register, code above handles the shift. When the first child of
             * the lushr is in
             * memory, the below ensures that the upper part of the first child of the lushr is used as lowMR.
             */
            if (useSecondHighOrder) {
                TR_ASSERT(secondHighZero, "useSecondHighOrder should be consistent with secondHighZero. "
    ...
    } else // assert getSubReg1Mem2() == true
    {
         ...
         * that shifts
         * by 32 bits. This is the only case where the useSecondHighOrder bit is set.
         * When the first child of the lushr is in a register, code above handles the shift. When the first child of the
         * lushr is in
         * memory, the below ensures that the upper part of the first child of the lushr is used as lowMR.
         */
        if (useSecondHighOrder)
