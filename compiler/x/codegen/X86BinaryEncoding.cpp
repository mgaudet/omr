...
{
    // *this    swipeable for debugging purposes
    self()->setEstimatedBinaryLength(self()->getOpCode().getOpCodeLength() + self()->rexPrefixLength() + (self()->getOpCode().needs16BitOperandPrefix() ? 1 : 0) + (self()->getOpCode().needsRepPrefix() ? 1 : 0));
...
void TR::AMD64RegImm64Instruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (comp->fej9()->helpersNeedRelocation()) {
        if (getNode()->getOpCode().hasSymbolReference() && methodSymRef && (methodSymRef->getReferenceNumber() == TR_referenceArrayCopy ||
