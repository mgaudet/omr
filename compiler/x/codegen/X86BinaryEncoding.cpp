...
int32_t OMR::X86::Instruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    self()->setEstimatedBinaryLength(self()->getOpCode().getOpCodeLength() + self()->rexPrefixLength()
        + (self()->getOpCode().needs16BitOperandPrefix() ? 1 : 0) + (self()->getOpCode().needsRepPrefix() ? 1 : 0));
    ...
    if (getOpCode().hasRelativeBranchDisplacement()) {
        ...
        if (label == NULL) {
            ...
            *(uint32_t*)cursor = 0;
            ...
            TR_ASSERT(getOpCodeValue() != XBEGIN4 || !_permitShortening, "XBEGIN4 cannot be shortened and can only be "
                                                                         "used with a label instruction that cannot "
...
int32_t TR::X86LabelInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    if (getOpCode().isBranchOp()) {
            ...
            if (getLabelSymbol() && getLabelSymbol()->getEstimatedCodeLocation()) {
                ...
                if (distance >= -128 && distance < 0 && _permitShortening) {
                    immediateLength = 0; // really 1, but for conditional branches (all excep JMP4) the opcode entry
                                         // will be 1 too big for short branch
        ...
        if (!comp->getOption(TR_DisableLateEdgeSplitting) && _symbol->getVMThreadRestoringLabel()
            && _symbol->isVMThreadLive()) {
            ...
            TR::RealRegister* vmThreadRealReg
                = cg()->machine()->getX86RealRegister(cg()->getProperties().getMethodMetaDataRegister());
            if (vmThreadRealReg->getHasBeenAssignedInMethod()
                || !performTransformation(comp,
                       "O^O LATE EDGE SPLITTING: Skip vmThread reload at %p because ebp was never assigned\n",
                       this)) // TODO: could we use getModified instead?
            {
                ...
                TR::Instruction* ebpLoad = generateRegMemInstruction(getPrev(), LRegMem(), vmThreadRealReg,
                    generateX86MemoryReference(vmThreadVirtualReg->getBackingStorage()->getSymbolReference(), cg()),
                    cg());
                ...
                if (cg()->getTraceRAOption(TR_TraceRALateEdgeSplitting))
                    traceMsg(comp, "LATE EDGE SPLITTING: Added vmThread rematerialization %s before label %s\n",
                        cg()->getDebug()->getName(ebpLoad), cg()->getDebug()->getName(_symbol));

                TR::Instruction* bump
                    = cg()->generateDebugCounter(ebpLoad, "cg.lateSplitEdges:reloaded", 1, TR::DebugCounter::Expensive);
                if (bump != ebpLoad)
    ...
    } else // assume absolute code reference
    {
        setEstimatedBinaryLength(
            getOpCode().getOpCodeLength() + rexPrefixLength() + 4); // full offset for absolute address reference
    }
    return currentEstimate + getEstimatedBinaryLength();
}
...
uint8_t* TR::X86VirtualGuardNOPInstruction::generateBinaryEncoding()
{
    ...
    if (guardForPatching != this) {
        ...
        if (label->getCodeLocation() == NULL) {
            cg()->addRelocation(
                new (cg()->trHeapMemory()) TR_LabelAbsoluteRelocation((uint8_t*)(&_site->getDestination()), label));
        } else {
    ...
    if (instToBePatched && _nopSize > 0 && !cg()->nopsAlsoProcessedByRelocations()) {
        ...
        if (enlargement.getPatchGrowth() + next >= _nopSize) {
            ...
            TR_ASSERT(enlargement.getPatchGrowth() == 0, "Instructions should only partially enlarge when dealing with "
...
int32_t TR::X86ImmInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    setEstimatedBinaryLength(getOpCode().getOpCodeLength() + prefixLength + rexPrefixLength() + immediateLength);
...
void TR::X86ImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (getOpCode().isCallImmOp() || getOpCode().isBranchOp()) {
            ...
            cg()->addAOTRelocation(
                new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)(uintptrj_t)getSourceImmediate(),
            ...
            if (symbol->isConst()) {
                ...
                if (cg()->needClassAndMethodPointerRelocations()) {
                    cg()->addAOTRelocation(
                        new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                            getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ClassAddress,
                            cg()),
                        __FILE__, __LINE__, getNode());
                }
                ...
                if (staticSym && staticSym->isCompiledMethod())
                    ...
                    cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                               TR_ExternalRelocation(cursor, (uint8_t*)(staticSym->getStaticAddress()),
                                                   TR_AbsoluteMethodAddress, cg()),
                        __FILE__, __LINE__, getNode());
                else
                    cg()->addAOTRelocation(
                        new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                            getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress,
                            cg()),
                        __FILE__, __LINE__, getNode());
            }
        }
...
uint8_t TR::X86RegInstruction::getBinaryLengthLowerBound()
{
    ...
    uint8_t opCodeLength
        = (opCode.isAMD64DeprecatedInc() || opCode.isAMD64DeprecatedDec()) ? 2 : opCode.getOpCodeLength();
...
int32_t TR::X86RegInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    // *this    swipeable for debugging purposes
    TR_X86OpCode& opCode = getOpCode();
    uint8_t prefixLength = (opCode.needsSSE42OpcodePrefix()) ? 2 : opCode.needs16BitOperandPrefix()
            ? 1
    ...
    int32_t requestedEnlargementSize, int32_t maxEnlargementSize, bool allowPartialEnlargement)
{
    ...
    if (TR::Compiler->target.is64Bit() && !getOpCode().needs16BitOperandPrefix()
        && !getOpCode().needsSSE42OpcodePrefix() && !getOpCode().needsScalarPrefix()
...
void TR::X86RegImmInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getOpCode().hasIntImmediate()) {
        ...
        if (staticMethodPIC) {
            void* classPointer = (void*)cg()
                                     ...
                                     ->createResolvedMethod(cg()->trMemory(),
                                         (TR_OpaqueMethodBlock*)getSourceImmediateAsAddress(), comp->getCurrentMethod())
                                     ->classOfMethod();
        ...
        case TR_HEAP_BASE:
            cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                       TR_ExternalRelocation(cursor, (uint8_t*)TR_HeapBase, TR_GlobalValue, cg()),
                __FILE__, __LINE__, getNode());
            break;

        case TR_HEAP_TOP:
            cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                       TR_ExternalRelocation(cursor, (uint8_t*)TR_HeapTop, TR_GlobalValue, cg()),
                __FILE__, __LINE__, getNode());
            break;
    ...
    case TR_ClassObject: {
        if (cg()->needClassAndMethodPointerRelocations()) {
            ...
            *(int32_t*)cursor = (int32_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(
                cg()->comp(), (TR_OpaqueClassBlock*)getSourceImmediate());
            cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                       TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                                           getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1,
                                           (TR_ExternalRelocationTargetKind)getReloKind(), cg()),
                __FILE__, __LINE__, getNode());
        }
...
void TR::X86MemImmSymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (symbol->isConst()) {
        cg()->addAOTRelocation(
            new (cg()->trHeapMemory())
                TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference()->getOwningMethod(comp)->constantPool(),
                    getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ConstantPool, cg()),
            __FILE__, __LINE__, getNode());
        ...
        if (cg()->needClassAndMethodPointerRelocations()) {
            *(int32_t*)cursor = (int32_t)TR::Compiler->cls.persistentClassPointerFromClassPointer(
            ...
            cg()->addAOTRelocation(
                new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                    getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_ClassAddress, cg()),
                __FILE__, __LINE__, getNode());
        }
    } else if (symbol->isMethod()) {
        cg()->addAOTRelocation(
        ...
        cg()->addAOTRelocation(
            new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getSymbolReference(),
                getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_DataAddress, cg()),
            __FILE__, __LINE__, getNode());
    }
}
    ...
    if (comp->fej9()->helpersNeedRelocation()) {
        if (getNode()->getOpCode().hasSymbolReference() && methodSymRef
            ...
            cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(
                ...
                __FILE__, __LINE__, getNode());
        } else if (((getNode()->getOpCodeValue() == TR::aconst) && getNode()->isMethodPointerConstant()
                       && needsAOTRelocation())
            || staticHCRPIC) {
            cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, NULL, TR_RamMethod, cg()),
                __FILE__, __LINE__, getNode());
        } else {
            switch (_reloKind) {
            case TR_HEAP_BASE_FOR_BARRIER_RANGE:
            ...
            case TR_ClassAddress:
                ...
                cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)methodSymRef,
                                           (uint8_t*)getNode()->getInlinedSiteIndex(), TR_ClassAddress, cg()),
                    __FILE__, __LINE__, getNode());
                break;
            ...
            // intentional fall-through
            case TR_ClassPointer:
                cg()->addAOTRelocation(new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getNode(),
            ...
            case TR_SpecialRamMethodConst:
                ...
                cg()->addAOTRelocation(
                    new (cg()->trHeapMemory()) TR_ExternalRelocation(cursor, (uint8_t*)getNode()->getSymbolReference(),
                        getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1,
                        (TR_ExternalRelocationTargetKind)_reloKind, cg()),
                    __FILE__, __LINE__, getNode());
                break;
...
void TR::AMD64RegImm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    if (getSymbolReference()->getSymbol()->isLabel()) {
        ...
        cg()->addRelocation(new (cg()->trHeapMemory())
                TR_LabelAbsoluteRelocation(cursor, getSymbolReference()->getSymbol()->castToLabelSymbol()));
...
void TR::AMD64Imm64SymInstruction::addMetaDataForCodeAddress(uint8_t* cursor)
{
    if (getOpCodeValue() == DQImm64) {
        cg()->addProjectSpecializedRelocation(cursor, (uint8_t*)(uint64_t)getSourceImmediate(),
            getNode() ? (uint8_t*)getNode()->getInlinedSiteIndex() : (uint8_t*)-1, TR_Thunks, __FILE__, __LINE__,
            getNode());
    }
}
