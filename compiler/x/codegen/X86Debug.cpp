...
void TR_Debug::print(TR::FILE* pOutFile, TR::X86RestoreVMThreadInstruction* instr)
{
    // *this    swipeable for debugging purposes
    if (pOutFile == NULL)
        return;
    ...
    printPrefix(pOutFile, instr);
    if (instr->getBinaryEncoding()) {
        trfprintf(pOutFile, "mov ebp, dword ptr fs:[0]\t\t;%sRestoreVMThread ", commentString());
    }
...
void TR_Debug::printReferencedRegisterInfo(TR::FILE* pOutFile, TR::X86MemRegRegInstruction* instr)
{
    if (pOutFile == NULL)
        return;
...
int32_t TR_Debug::printFPConstant(TR::FILE* pOutFile, void* value, int8_t numBits, bool printAsBytes) { return 0; }
...
#ifdef J9_PROJECT_SPECIFIC
void TR_Debug::print(TR::FILE* pOutFile, TR_X86CallSnippet* snippet)
{
...
#ifdef J9_PROJECT_SPECIFIC
uint8_t* TR_Debug::printArgs(
    TR::FILE* pOutFile, TR_AMD64WriteBarrierSnippet* snippet, bool restoreRegs, uint8_t* bufferPos)
{
...
static uint8_t* estimateJumpSize(TR_AMD64WriteBarrierSnippet* snippet, uint8_t* bufferPos)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR_AMD64WriteBarrierSnippet* snippet)
{
    if (pOutFile == NULL)
        return;
...
static const char* opCodeToNameMap[IA32NumOpCodes] = { "BADIA32Op", "ADC1AccImm1", "ADC2AccImm2", "ADC4AccImm4",
    "ADC8AccImm4", // (AMD64)
    "ADC1RegImm1", "ADC2RegImm2", "ADC2RegImms", "ADC4RegImm4",
    "ADC8RegImm4", // (AMD64)
    "ADC4RegImms",
    "ADC8RegImms", // (AMD64)
    "ADC1MemImm1", "ADC2MemImm2", "ADC2MemImms", "ADC4MemImm4",
    "ADC8MemImm4", // (AMD64)
    "ADC4MemImms",
    "ADC8MemImms", // (AMD64)
    "ADC1RegReg", "ADC2RegReg", "ADC4RegReg",
    "ADC8RegReg", // (AMD64)
    "ADC1RegMem", "ADC2RegMem", "ADC4RegMem",
    "ADC8RegMem", // (AMD64)
    "ADC1MemReg", "ADC2MemReg", "ADC4MemReg",
    "ADC8MemReg", // (AMD64)
    "ADD1AccImm1", "ADD2AccImm2", "ADD4AccImm4",
    "ADD8AccImm4", // (AMD64)
    "ADD1RegImm1", "ADD2RegImm2", "ADD2RegImms", "ADD4RegImm4",
    "ADD8RegImm4", // (AMD64)
    "ADD4RegImms",
    "ADD8RegImms", // (AMD64)
    "ADD1MemImm1", "ADD2MemImm2", "ADD2MemImms", "ADD4MemImm4",
    "ADD8MemImm4", // (AMD64)
    "ADD4MemImms",
    "ADD8MemImms", // (AMD64)
    "ADD1RegReg", "ADD2RegReg", "ADD4RegReg",
    "ADD8RegReg", // (AMD64)
    "ADD1RegMem", "ADD2RegMem", "ADD4RegMem",
    "ADD8RegMem", // (AMD64)
    "ADD1MemReg", "ADD2MemReg", "ADD4MemReg",
    "ADD8MemReg", // (AMD64)
    "ADDSSRegReg", "ADDSSRegMem", "ADDPSRegReg", "ADDPSRegMem", "ADDSDRegReg", "ADDSDRegMem", "ADDPDRegReg",
    "ADDPDRegMem", "LXADD1MemReg", "LXADD2MemReg", "LXADD4MemReg",
    "LXADD8MemReg", // (AMD64)
    "AND1AccImm1", "AND2AccImm2", "AND4AccImm4",
    "AND8AccImm4", // (AMD64)
    "AND1RegImm1", "AND2RegImm2", "AND2RegImms", "AND4RegImm4",
    "AND8RegImm4", // (AMD64)
    "AND4RegImms",
    "AND8RegImms", // (AMD64)
    "AND1MemImm1", "AND2MemImm2", "AND2MemImms", "AND4MemImm4",
    "AND8MemImm4", // (AMD64)
    "AND4MemImms",
    "AND8MemImms", // (AMD64)
    "AND1RegReg", "AND2RegReg", "AND4RegReg",
    "AND8RegReg", // (AMD64)
    "AND1RegMem", "AND2RegMem", "AND4RegMem",
    "AND8RegMem", // (AMD64)
    "AND1MemReg", "AND2MemReg", "AND4MemReg",
    "AND8MemReg", // (AMD64)
    "BSF2RegReg", "BSF4RegReg", "BSF8RegReg", "BSR4RegReg", "BSR8RegReg", "BSWAP4Reg",
    "BSWAP8Reg", // (AMD64)
    "BTS4RegReg", "BTS4MemReg", "CALLImm4",
    "CALLREXImm4", // (AMD64)
    "CALLReg",
    "CALLREXReg", // (AMD64)
    "CALLMem",
    "CALLREXMem", // (AMD64)
    "CBWAcc", "CBWEAcc", "CMOVA4RegMem", "CMOVB4RegMem", "CMOVE4RegMem",
    "CMOVE8RegMem", // (64-bit)
    "CMOVG4RegMem", "CMOVL4RegMem", "CMOVNE4RegMem",
    "CMOVNE8RegMem", // (64-bit)
    "CMOVNO4RegMem", "CMOVNS4RegMem", "CMOVO4RegMem", "CMOVS4RegMem", "CMP1AccImm1", "CMP2AccImm2", "CMP4AccImm4",
    "CMP8AccImm4", // (AMD64)
    "CMP1RegImm1", "CMP2RegImm2", "CMP2RegImms", "CMP4RegImm4",
    "CMP8RegImm4", // (AMD64)
    "CMP4RegImms",
    "CMP8RegImms", // (AMD64)
    "CMP1MemImm1", "CMP2MemImm2", "CMP2MemImms", "CMP4MemImm4",
    "CMP8MemImm4", // (AMD64)
    "CMP4MemImms",
    "CMP8MemImms", // (AMD64)
    "CMP1RegReg", "CMP2RegReg", "CMP4RegReg",
    "CMP8RegReg", // (AMD64)
    "CMP1RegMem", "CMP2RegMem", "CMP4RegMem",
    "CMP8RegMem", // (AMD64)
    "CMP1MemReg", "CMP2MemReg", "CMP4MemReg",
    "CMP8MemReg", // (AMD64)
    "CMPXCHG1MemReg", "CMPXCHG2MemReg", "CMPXCHG4MemReg",
    "CMPXCHG8MemReg", // (AMD64)
    "CMPXCHG8BMem", // (IA32 only)
    "CMPXCHG16BMem", // (AMD64)
    "LCMPXCHG1MemReg", "LCMPXCHG2MemReg", "LCMPXCHG4MemReg",
    "LCMPXCHG8MemReg", // (AMD64)
    "LCMPXCHG8BMem", // (IA32 only)
    "LCMPXCHG16BMem", // (AMD64)
    "XALCMPXCHG8MemReg", "XACMPXCHG8MemReg", "XALCMPXCHG4MemReg", "XACMPXCHG4MemReg", "CVTSI2SSRegReg4",
    ...
    "CVTSS2SDRegReg", "CVTSS2SDRegMem", "CVTSD2SSRegReg", "CVTSD2SSRegMem", "CWDAcc", "CDQAcc",
    "CQOAcc", // (AMD64)
    "DEC1Reg", "DEC2Reg", "DEC4Reg",
    "DEC8Reg", // (AMD64)
    "DEC1Mem", "DEC2Mem", "DEC4Mem",
    "DEC8Mem", // (AMD64)
    "FABSReg", "DABSReg", "FSQRTReg", "DSQRTReg", "FADDRegReg", "DADDRegReg", "FADDPReg", "FADDRegMem", "DADDRegMem",
    "FIADDRegMem", "DIADDRegMem", "FSADDRegMem", "DSADDRegMem", "FCHSReg", "DCHSReg", "FDIVRegReg", "DDIVRegReg",
    "FDIVRegMem", "DDIVRegMem", "FDIVPReg", "FIDIVRegMem", "DIDIVRegMem", "FSDIVRegMem", "DSDIVRegMem", "FDIVRRegReg",
    "DDIVRRegReg", "FDIVRRegMem", "DDIVRRegMem", "FDIVRPReg", "FIDIVRRegMem", "DIDIVRRegMem", "FSDIVRRegMem",
    "DSDIVRRegMem", "FILDRegMem", "DILDRegMem", "FLLDRegMem", "DLLDRegMem", "FSLDRegMem", "DSLDRegMem", "FISTMemReg",
    "DISTMemReg", "FISTPMem", "DISTPMem", "FLSTPMem", "DLSTPMem", "FSSTMemReg", "DSSTMemReg", "FSSTPMem", "DSSTPMem",
    "FLDLN2", "FLDRegReg", "DLDRegReg", "FLDRegMem", "DLDRegMem", "FLD0Reg", "DLD0Reg", "FLD1Reg", "DLD1Reg", "FLDMem",
    "DLDMem", "LDCWMem", "FMULRegReg", "DMULRegReg", "FMULPReg", "FMULRegMem", "DMULRegMem", "FIMULRegMem",
    "DIMULRegMem", "FSMULRegMem", "DSMULRegMem", "FNCLEX", "FPREMRegReg", "FSCALERegReg", "FSTMemReg", "DSTMemReg",
    "FSTRegReg", "DSTRegReg", "FSTPMemReg", "DSTPMemReg", "FSTPReg", "DSTPReg", "STCWMem", "STSWMem", "STSWAcc",
    "FSUBRegReg", "DSUBRegReg", "FSUBRegMem", "DSUBRegMem", "FSUBPReg", "FISUBRegMem", "DISUBRegMem", "FSSUBRegMem",
    "DSSUBRegMem", "FSUBRRegReg", "DSUBRRegReg", "FSUBRRegMem", "DSUBRRegMem", "FSUBRPReg", "FISUBRRegMem",
    "DISUBRRegMem", "FSSUBRRegMem", "DSSUBRRegMem", "FTSTReg", "FCOMRegReg", "DCOMRegReg", "FCOMRegMem", "DCOMRegMem",
    "FCOMPReg", "FCOMPMem", "DCOMPMem", "FCOMPP", "FCOMIRegReg", "DCOMIRegReg", "FCOMIPReg", "FYL2X", "UCOMISSRegReg",
    "UCOMISSRegMem", "UCOMISDRegReg", "UCOMISDRegMem", "FXCHReg", "IDIV1AccReg", "IDIV2AccReg", "IDIV4AccReg",
    "IDIV8AccReg", // (AMD64)
    "DIV4AccReg",
    "DIV8AccReg", // (AMD64)
    "IDIV1AccMem", "IDIV2AccMem", "IDIV4AccMem",
    "IDIV8AccMem", // (AMD64)
    "DIV4AccMem",
    "DIV8AccMem", // (AMD64)
    "DIVSSRegReg", "DIVSSRegMem", "DIVSDRegReg", "DIVSDRegMem", "IMUL1AccReg", "IMUL2AccReg", "IMUL4AccReg",
    "IMUL8AccReg", // (AMD64)
    "IMUL1AccMem", "IMUL2AccMem", "IMUL4AccMem",
    "IMUL8AccMem", // (AMD64)
    "IMUL2RegReg", "IMUL4RegReg",
    "IMUL8RegReg", // (AMD64)
    "IMUL2RegMem", "IMUL4RegMem",
    "IMUL8RegMem", // (AMD64)
    "IMUL2RegRegImm2", "IMUL2RegRegImms", "IMUL4RegRegImm4",
    "IMUL8RegRegImm4", // (AMD64)
    "IMUL4RegRegImms",
    "IMUL8RegRegImms", // (AMD64)
    "IMUL2RegMemImm2", "IMUL2RegMemImms", "IMUL4RegMemImm4",
    "IMUL8RegMemImm4", // (AMD64)
    "IMUL4RegMemImms",
    "IMUL8RegMemImms", // (AMD64)
    "MUL1AccReg", "MUL2AccReg", "MUL4AccReg",
    "MUL8AccReg", // (AMD64)
    "MUL1AccMem", "MUL2AccMem", "MUL4AccMem",
    "MUL8AccMem", // (AMD64)
    "MULSSRegReg", "MULSSRegMem", "MULPSRegReg", "MULPSRegMem", "MULSDRegReg", "MULSDRegMem", "MULPDRegReg",
    "MULPDRegMem", "INC1Reg", "INC2Reg", "INC4Reg",
    "INC8Reg", // (AMD64)
    "INC1Mem", "INC2Mem", "INC4Mem",
    "INC8Mem", // (AMD64)
    "JA1", "JAE1", "JB1", "JBE1", "JE1", "JNE1", "JG1", "JGE1", "JL1", "JLE1", "JO1", "JNO1", "JS1", "JNS1", "JPO1",
    "JPE1", "JMP1", "JA4", "JAE4", "JB4", "JBE4", "JE4", "JNE4", "JG4", "JGE4", "JL4", "JLE4", "JO4", "JNO4", "JS4",
    "JNS4", "JPO4", "JPE4", "JMP4", "JMPReg", "JMPMem", "JRCXZ1", "LOOP1", "LAHF", "LDDQU", "LEA2RegMem", "LEA4RegMem",
    "LEA8RegMem", // (AMD64)
    "S1MemReg", "S2MemReg", "S4MemReg",
    "S8MemReg", // (AMD64)
    "S1MemImm1", "S2MemImm2", "S4MemImm4", "XRS4MemImm4",
    "S8MemImm4", // (AMD64)
    "XRS8MemImm4", // (AMD64)
    "L1RegMem", "L2RegMem", "L4RegMem",
    "L8RegMem", // (AMD64)
    "MOVAPSRegReg", "MOVAPSRegMem", "MOVAPSMemReg", "MOVAPDRegReg", "MOVAPDRegMem", "MOVAPDMemReg", "MOVUPSRegReg",
    "MOVUPSRegMem", "MOVUPSMemReg", "MOVUPDRegReg", "MOVUPDRegMem", "MOVUPDMemReg", "MOVSSRegReg", "MOVSSRegMem",
    "MOVSSMemReg", "MOVSDRegReg", "MOVSDRegMem", "MOVSDMemReg", "SQRTSFRegReg", "SQRTSDRegReg", "MOVDRegReg4",
    "MOVQRegReg8", // (AMD64)
    "MOVDReg4Reg",
    "MOVQReg8Reg", // (AMD64)
    "MOVDQURegReg", "MOVDQURegMem", "MOVDQUMemReg", "MOV1RegReg", "MOV2RegReg", "MOV4RegReg",
    "MOV8RegReg", // (AMD64)
    "CMOVG4RegReg",
    "CMOVG8RegReg", // (AMD64)
    "CMOVL4RegReg",
    ...
    "CMOVNE4RegReg",
    "CMOVNE8RegReg", // (AMD64)
    "MOV1RegImm1", "MOV2RegImm2", "MOV4RegImm4",
    "MOV8RegImm4", // (AMD64)
    "MOV8RegImm64", // (AMD64)
    "MOVLPDRegMem", "MOVLPDMemReg", "MOVQRegMem", "MOVQMemReg", "MOVSB", "MOVSW", "MOVSD",
    "MOVSQ", // (AMD64)
    "MOVSXReg2Reg1", "MOVSXReg4Reg1",
    "MOVSXReg8Reg1", // (AMD64)
    "MOVSXReg4Reg2",
    "MOVSXReg8Reg2", // (AMD64)
    "MOVSXReg8Reg4", // (AMD64)
    "MOVSXReg2Mem1", "MOVSXReg4Mem1",
    "MOVSXReg8Mem1", // (AMD64)
    "MOVSXReg4Mem2",
    "MOVSXReg8Mem2", // (AMD64)
    "MOVSXReg8Mem4", // (AMD64)
    "MOVZXReg2Reg1", "MOVZXReg4Reg1",
    "MOVZXReg8Reg1", // (AMD64)
    "MOVZXReg4Reg2",
    "MOVZXReg8Reg2", // (AMD64)
    "MOVZXReg8Reg4", // (AMD64)
    "MOVZXReg2Mem1", "MOVZXReg4Mem1",
    "MOVZXReg8Mem1", // (AMD64)
    "MOVZXReg4Mem2",
    "MOVZXReg8Mem2", // (AMD64)
    "NEG1Reg", "NEG2Reg", "NEG4Reg",
    "NEG8Reg", // (AMD64)
    "NEG1Mem", "NEG2Mem", "NEG4Mem",
    "NEG8Mem", // (AMD64)
    "NOT1Reg", "NOT2Reg", "NOT4Reg",
    "NOT8Reg", // (AMD64)
    "NOT1Mem", "NOT2Mem", "NOT4Mem",
    "NOT8Mem", // (AMD64)
    "OR1AccImm1", "OR2AccImm2", "OR4AccImm4",
    "OR8AccImm4", // (AMD64)
    "OR1RegImm1", "OR2RegImm2", "OR2RegImms", "OR4RegImm4",
    "OR8RegImm4", // (AMD64)
    "OR4RegImms",
    "OR8RegImms", // (AMD64)
    "OR1MemImm1", "OR2MemImm2", "OR2MemImms", "OR4MemImm4",
    "OR8MemImm4", // (AMD64)
    "OR4MemImms", "LOR4MemImms", "LOR4MemReg",
    "LOR8MemReg", // (AMD64)
    "OR8MemImms", // (AMD64)
    "OR1RegReg", "OR2RegReg", "OR4RegReg",
    "OR8RegReg", // (AMD64)
    "OR1RegMem", "OR2RegMem", "OR4RegMem",
    "OR8RegMem", // (AMD64)
    "OR1MemReg", "OR2MemReg", "OR4MemReg",
    "OR8MemReg", // (AMD64)
    "PAUSE", "PCMPEQBRegReg", "PMOVMSKB4RegReg", "PMOVZXWD", "PMULLD", "PADDD", "PSHUFDRegRegImm1", "PSHUFDRegMemImm1",
    "PSRLDQRegImm1", "PMOVZXxmm18Reg", "POPCNT4RegReg",
    "POPCNT8RegReg", // (AMD64)
    "POPReg", "POPMem", "PUSHImms", "PUSHImm4", "PUSHReg", "PUSHRegLong", "PUSHMem", "RCL1RegImm1", "RCL4RegImm1",
    "RCR1RegImm1", "RCR4RegImm1", "REPMOVSB", "REPMOVSW", "REPMOVSD", "REPMOVSQ", "REPSTOSB", "REPSTOSW", "REPSTOSD",
    "REPSTOSQ", // (AMD64)
    "RET", "RETImm2", "ROL1RegImm1", "ROL2RegImm1", "ROL4RegImm1",
    "ROL8RegImm1", // (AMD64)
    "ROL4RegCL",
    "ROL8RegCL", // (AMD64)
    "ROR1RegImm1", "ROR2RegImm1", "ROR4RegImm1",
    "ROR8RegImm1", // (AMD64)
    "SAHF", "SHL1RegImm1", "SHL1RegCL", "SHL2RegImm1", "SHL2RegCL", "SHL4RegImm1",
    "SHL8RegImm1", // (AMD64)
    "SHL4RegCL",
    "SHL8RegCL", // (AMD64)
    "SHL1MemImm1", "SHL1MemCL", "SHL2MemImm1", "SHL2MemCL", "SHL4MemImm1",
    "SHL8MemImm1", // (AMD64)
    "SHL4MemCL",
    "SHL8MemCL", // (AMD64)
    "SHR1RegImm1", "SHR1RegCL", "SHR2RegImm1", "SHR2RegCL", "SHR4RegImm1",
    "SHR8RegImm1", // (AMD64)
    "SHR4RegCL",
    "SHR8RegCL", // (AMD64)
    "SHR1MemImm1", "SHR1MemCL", "SHR2MemImm1", "SHR2MemCL", "SHR4MemImm1",
    "SHR8MemImm1", // (AMD64)
    "SHR4MemCL",
    "SHR8MemCL", // (AMD64)
    "SAR1RegImm1", "SAR1RegCL", "SAR2RegImm1", "SAR2RegCL", "SAR4RegImm1",
    "SAR8RegImm1", // (AMD64)
    "SAR4RegCL",
    "SAR8RegCL", // (AMD64)
    "SAR1MemImm1", "SAR1MemCL", "SAR2MemImm1", "SAR2MemCL", "SAR4MemImm1",
    "SAR8MemImm1", // (AMD64)
    "SAR4MemCL",
    "SAR8MemCL", // (AMD64)
    "SBB1AccImm1", "SBB2AccImm2", "SBB4AccImm4",
    "SBB8AccImm4", // (AMD64)
    "SBB1RegImm1", "SBB2RegImm2", "SBB2RegImms", "SBB4RegImm4",
    "SBB8RegImm4", // (AMD64)
    "SBB4RegImms",
    "SBB8RegImms", // (AMD64)
    "SBB1MemImm1", "SBB2MemImm2", "SBB2MemImms", "SBB4MemImm4",
    "SBB8MemImm4", // (AMD64)
    "SBB4MemImms",
    "SBB8MemImms", // (AMD64)
    "SBB1RegReg", "SBB2RegReg", "SBB4RegReg",
    "SBB8RegReg", // (AMD64)
    "SBB1RegMem", "SBB2RegMem", "SBB4RegMem",
    "SBB8RegMem", // (AMD64)
    "SBB1MemReg", "SBB2MemReg", "SBB4MemReg",
    "SBB8MemReg", // (AMD64)
    "SETA1Reg", "SETAE1Reg", "SETB1Reg", "SETBE1Reg", "SETE1Reg", "SETNE1Reg", "SETG1Reg", "SETGE1Reg", "SETL1Reg",
    "SETLE1Reg", "SETS1Reg", "SETNS1Reg", "SETPO1Reg", "SETPE1Reg", "SETA1Mem", "SETAE1Mem", "SETB1Mem", "SETBE1Mem",
    "SETE1Mem", "SETNE1Mem", "SETG1Mem", "SETGE1Mem", "SETL1Mem", "SETLE1Mem", "SETS1Mem", "SETNS1Mem",
    "SHLD4RegRegImm1", "SHLD4RegRegCL", "SHLD4MemRegImm1", "SHLD4MemRegCL", "SHRD4RegRegImm1", "SHRD4RegRegCL",
    "SHRD4MemRegImm1", "SHRD4MemRegCL", "STOSB", "STOSW", "STOSD",
    "STOSQ", // (AMD64)
    "SUB1AccImm1", "SUB2AccImm2", "SUB4AccImm4",
    "SUB8AccImm4", // (AMD64)
    "SUB1RegImm1", "SUB2RegImm2", "SUB2RegImms", "SUB4RegImm4",
    "SUB8RegImm4", // (AMD64)
    "SUB4RegImms",
    "SUB8RegImms", // (AMD64)
    "SUB1MemImm1", "SUB2MemImm2", "SUB2MemImms", "SUB4MemImm4",
    "SUB8MemImm4", // (AMD64)
    "SUB4MemImms",
    "SUB8MemImms", // (AMD64)
    "SUB1RegReg", "SUB2RegReg", "SUB4RegReg",
    "SUB8RegReg", // (AMD64)
    "SUB1RegMem", "SUB2RegMem", "SUB4RegMem",
    "SUB8RegMem", // (AMD64)
    "SUB1MemReg", "SUB2MemReg", "SUB4MemReg",
    "SUB8MemReg", // (AMD64)
    "SUBSSRegReg", "SUBSSRegMem", "SUBSDRegReg", "SUBSDRegMem", "TEST1AccImm1", "TEST1AccHImm1", "TEST2AccImm2",
    "TEST4AccImm4",
    "TEST8AccImm4", // (AMD64)
    "TEST1RegImm1", "TEST2RegImm2", "TEST4RegImm4",
    "TEST8RegImm4", // (AMD64)
    "TEST1MemImm1", "TEST2MemImm2", "TEST4MemImm4",
    "TEST8MemImm4", // (AMD64)
    "TEST1RegReg", "TEST2RegReg", "TEST4RegReg",
    "TEST8RegReg", // (AMD64)
    "TEST1MemReg", "TEST2MemReg", "TEST4MemReg",
    "TEST8MemReg", // (AMD64)
    "XABORT", "XBEGIN2", "XBEGIN4", "XCHG2AccReg", "XCHG4AccReg",
    "XCHG8AccReg", // (AMD64)
    "XCHG1RegReg", "XCHG2RegReg", "XCHG4RegReg",
    "XCHG8RegReg", // (AMD64)
    "XCHG1RegMem", "XCHG2RegMem", "XCHG4RegMem",
    "XCHG8RegMem", // (AMD64)
    "XCHG1MemReg", "XCHG2MemReg", "XCHG4MemReg",
    "XCHG8MemReg", // (AMD64)
    "XEND", "XOR1AccImm1", "XOR2AccImm2", "XOR4AccImm4",
    "XOR8AccImm4", // (AMD64)
    "XOR1RegImm1", "XOR2RegImm2", "XOR2RegImms", "XOR4RegImm4",
    "XOR8RegImm4", // (AMD64)
    "XOR4RegImms",
    "XOR8RegImms", // (AMD64)
    "XOR1MemImm1", "XOR2MemImm2", "XOR2MemImms", "XOR4MemImm4",
    "XOR8MemImm4", // (AMD64)
    "XOR4MemImms",
    "XOR8MemImms", // (AMD64)
    "XOR1RegReg", "XOR2RegReg", "XOR4RegReg",
    "XOR8RegReg", // (AMD64)
    "XOR1RegMem", "XOR2RegMem", "XOR4RegMem",
    "XOR8RegMem", // (AMD64)
    "XOR1MemReg", "XOR2MemReg", "XOR4MemReg",
    "XOR8MemReg", // (AMD64)
    "XORPSRegReg", "XORPSRegMem", "XORPDRegReg", "MFENCE", "LFENCE", "SFENCE", "PCMPESTRI", "PREFETCHNTA", "PREFETCHT0",
    "PREFETCHT1", "PREFETCHT2", "ANDNPSRegReg", "ANDNPDRegReg", "PSLLQRegImm1", "PSRLQRegImm1", "FENCE", "VGFENCE",
    "PROCENTRY",
    "DQImm64", // (AMD64)
    "DDImm4", "DWImm2", "DBImm1", "WRTBAR", "ASSOCREGS", "FPREGSPILL", "VirtualGuardNOP", "LABEL", "FCMPEVAL",
    "RestoreVMThread", "PPS_OPCount", "PPS_OPField", "AdjustFramePtr", "ReturnMarker" };
