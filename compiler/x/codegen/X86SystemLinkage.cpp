...
const TR_X86LinkageProperties& TR_X86SystemLinkage::getProperties() { return _properties; }
...
int32_t TR_X86SystemLinkage::computeMemoryArgSize(TR::Node* callNode, int32_t first, int32_t last, int8_t direction)
{
...
//
TR::Instruction* TR_X86SystemLinkage::copyParametersToHomeLocation(TR::Instruction* cursor)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (lri == NOT_LINKAGE) // It's on the stack
        {
            ...
            } else // Method body expects it to be in the ai register
            {
                ...
                // ai := stack
                loadCursor = generateRegMemInstruction(loadCursor, movOpcodes[RegMem][movDataType],
                    machine->getX86RealRegister(ai), generateX86MemoryReference(framePointer, offset, cg()), cg());
            }
        } else // It's in a linkage register
        {
            ...
            if (ai == NOT_ASSIGNED || hasToBeOnStack(paramCursor)) {
                if (comp()->getOption(TR_TraceCG))
                    traceMsg(comp(), "copyToHomeLocation param %p, linkage reg index %d, allocated index %d, parameter "
                ...
                // stack := lri
                cursor = generateMemRegInstruction(cursor, movOpcodes[MemReg][movDataType],
                    generateX86MemoryReference(framePointer, offset, cg()), machine->getX86RealRegister(sourceIndex),
                    cg());
            }
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (movStatus[paramReg].targetReg == 0) {
            ...
            while (movStatus[regCursor].sourceReg != noReg) {
                ...
                cursor = generateRegRegInstruction(cursor, movOpcodes[RegReg][movStatus[source].outgoingDataType],
                    machine->getX86RealRegister(regCursor), machine->getX86RealRegister(source), cg());
...
TR::Instruction* TR_X86SystemLinkage::savePreservedRegisters(TR::Instruction* cursor)
{
    ...
    if (_properties.getUsesPushesForPreservedRegs()) {
        ...
        for (int32_t pindex = getProperties().getMaxRegistersPreservedInPrologue() - 1; pindex >= 0; pindex--) {
            TR::RealRegister::RegNum idx = _properties.getPreservedRegister((uint32_t)pindex);
            TR::RealRegister* reg
                = machine()->getX86RealRegister(getProperties().getPreservedRegister((uint32_t)pindex));
            if (reg->getHasBeenAssignedInMethod() && reg->getState() != TR::RealRegister::Locked) {
                if (!p || p->get(idx)) {
                    cursor = generateMemRegInstruction(cursor, movOpcodes[MemReg][fullRegisterMovType(reg)],
                        generateX86MemoryReference(machine()->getX86RealRegister(TR::RealRegister::vfp), offsetCursor,
                                                           cg()),
                        reg, cg());
                }
...
void TR_X86SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (properties.getAlwaysDedicateFramePointerRegister()) {
        cursor = new (trHeapMemory()) TR::X86RegInstruction(
            cursor, PUSHReg, machine()->getX86RealRegister(properties.getFramePointerRegister()), cg());
        ...
        cursor = new (trHeapMemory()) TR::X86RegRegInstruction(cursor, MOVRegReg(),
            machine()->getX86RealRegister(properties.getFramePointerRegister()), stackPointerReg, cg());
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        debugFrameSlotInfo
            = new (trHeapMemory()) TR_DebugFrameSegmentInfo(comp(), paramCursor->getOffset(), paramCursor->getSize(),
                "Parameter", debugFrameSlotInfo, (ai == NOT_ASSIGNED) ? NULL : machine()->getX86RealRegister(ai));
    }
...
TR::Instruction* TR_X86SystemLinkage::restorePreservedRegisters(TR::Instruction* cursor)
{
    TR::ResolvedMethodSymbol* bodySymbol = comp()->getJittedMethodSymbol();
        ...
        for (int32_t pindex = _properties.getMaxRegistersPreservedInPrologue() - 1; pindex >= 0; pindex--) {
            ...
            if (comp()->getOption(TR_TraceCG)) {
                traceMsg(comp(), "reg %d, getHasBeenAssignedInMethod %d, p %x, p->get(idx) %d.\n", idx,
                    reg->getHasBeenAssignedInMethod(), p, p ? p->get(idx) : -1);
            }

            if (reg->getHasBeenAssignedInMethod()) {
                if (!p || p->get(idx)) {
                    cursor = generateRegMemInstruction(cursor, movOpcodes[RegMem][fullRegisterMovType(reg)], reg,
                        generateX86MemoryReference(machine()->getX86RealRegister(TR::RealRegister::vfp), offsetCursor,
                                                           cg()),
                        cg());
                }
...
void TR_X86SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (_properties.getAlwaysDedicateFramePointerRegister()) {
        ...
        cursor = new (trHeapMemory()) TR::X86RegRegInstruction(
            cursor, MOVRegReg(), espReal, machine()->getX86RealRegister(_properties.getFramePointerRegister()), cg());
        cursor = new (trHeapMemory()) TR::X86RegInstruction(
            cursor, POPReg, machine()->getX86RealRegister(_properties.getFramePointerRegister()), cg());
    } else if (allocSize == 0) {
        // No need to do anything
    } else {
...
void TR_X86SystemLinkage::copyGlRegDepsToParameterSymbols(TR::Node* bbStart, TR::CodeGenerator* cg)
{
...
int32_t TR_X86SystemLinkage::getParameterStartingPos(int32_t& dataCursor, uint32_t align)
{
...
int32_t TR_X86SystemLinkage::layoutTypeOnStack(TR::DataType type, int32_t& dataCursor, parmLayoutResult& layoutResult)
{
