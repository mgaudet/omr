    ...
    TR::Node* node, TR_X86OpCodes highSetOpCode, TR_X86OpCodes lowSetOpCode, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst && testRegister == NULL
        && performTransformation(comp, "O^O compareLongAndSetOrderedBoolean: checking that the second child node does "
...
TR::Register* OMR::X86::i386::TreeEvaluator::integerPairShrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        TR::IA32LinkageUtils::pushIntegerWordArg(secondChild, cg);
        TR::X86ImmSymInstruction* instr
            = generateHelperCallInstruction(node, TR_IA32longShiftRightArithmetic, dependencies, cg);
        if (!linkage->getProperties().getCallerCleanup()) {
            instr->setAdjustsFramePointerBy(-12); // 1 long, 1 integer arg
        }
        targetRegister = cg->allocateRegisterPair(lowRegister, highRegister);
...
TR::Register* OMR::X86::i386::TreeEvaluator::integerPairUshrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::iconst) {
        ...
        TR::IA32LinkageUtils::pushIntegerWordArg(secondChild, cg);
        TR::X86ImmSymInstruction* instr
            = generateHelperCallInstruction(node, TR_IA32longShiftRightLogical, dependencies, cg);
        if (!linkage->getProperties().getCallerCleanup()) {
            instr->setAdjustsFramePointerBy(-12); // 1 long, 1 integer arg
        }
