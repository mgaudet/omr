    ...
    if (getOpReg1Reg2()) {
        {
            if (targetLabel != NULL) {
                ...
                if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                    ...
                    && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: "
                                                   "reuse CC from ccInst [%p].",
                           debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister),
                    ...
                    && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: "
                                                   "reuse CC from ccInst [%p].",
                               debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister),
                               debugObj->getName(secondRegister), ccInst)) {
        ...
        if (targetLabel != NULL) {
            ...
            if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo)
                ...
                && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister)
                && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t %s, %s]: reuse "
                                               "CC from ccInst [%p].",
                           debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister),
                           debugObj->getName(secondRegister), ccInst)) {
                nodeReg = secondRegister;
    ...
    TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi
        && secondChild->getRegister() == NULL && secondChild->getFirstChild()->getRegister() == NULL
        && secondChild->getReferenceCount() == 1 && secondChild->getFirstChild()->getReferenceCount() == 1) {
        setMem2();
