    ...
    if (getOpReg1Reg2()) {
        {
            if (targetLabel != NULL) {
                ...
                if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 3 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                    nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
                } else if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 4 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
        ...
        if (targetLabel != NULL) {
            ...
            TR::Instruction* ccInst = cg()->ccInstruction();
            TR::InstOpCode opcTmp = TR::InstOpCode(regToRegOpCode);
            if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, firstRegister, secondRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 5 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                nodeReg = firstRegister; // CCInfo already exists; don't generate compare op any more;
            } else if (opcTmp.setsCompareFlag() && comp->getOption(TR_EnableEBBCCInfo) && cg()->isActiveCompareCC(regToRegOpCode, secondRegister, firstRegister) && performTransformation(comp, "O^O BinaryCommunicativeAnalyser case 6 RR Compare [%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&ccInst->getOpCode()), debugObj->getName(firstRegister), debugObj->getName(secondRegister), ccInst)) {
                nodeReg = secondRegister;
                notReversedOperands(); // CCInfo already exists; don't generate compare op any more;
            } else {
...
void TR_S390BinaryCommutativeAnalyser::integerAddAnalyser(TR::Node* root, TR::InstOpCode::Mnemonic regToRegOpCode, TR::InstOpCode::Mnemonic memToRegOpCode, TR::InstOpCode::Mnemonic copyOpCode)
{
    ...
    */
    if (secondChild->getOpCodeValue() == TR::s2i && secondChild->getFirstChild()->getOpCodeValue() == TR::sloadi && secondChild->getRegister() == NULL && secondChild->getFirstChild()->getRegister() == NULL && secondChild->getReferenceCount() == 1 && secondChild->getFirstChild()->getReferenceCount() == 1) {
        setMem2();
