...
generic32BitSubEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (firstChild->getRegister() && !cg->canClobberNodesRegister(firstChild) && ((-value) >= MIN_IMMEDIATE_VAL && (-value) <= MAX_IMMEDIATE_VAL)) {
...
iDivRemGenericEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR)
{
    ...
    } else if (secondChild->getRegister() == NULL && secondChild->getReferenceCount() == 1 && secondChild->getOpCode().isMemoryReference() && !secondChild->getSymbolReference()->getSymbol()->isRegisterSymbol() && !needCheck) {
...
genericLongShiftSingle(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lshl && firstChild->getOpCodeValue() == TR::i2l && (firstChild->getFirstChild()->isNonNegative() || firstChild->isNonNegative()) && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
...
OMR::Z::TreeEvaluator::landEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) && ((targetRegister = genericRotateAndInsertHelper(node, cg))
