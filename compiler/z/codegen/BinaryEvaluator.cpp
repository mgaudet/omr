...
TR::Register* laddHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isCompressionSequence && ((oldSequence && !olderSequence) || !firstChild->isNonZero())) {
        ...
        if (isCompressionSequence) {
            if (skipAdd) {
                if (targetRegister && (targetRegister == firstChild->getRegister())) {
                    ...
                    generateRRInstruction(
                        cg, TR::InstOpCode::LGR, node, firstRegisterCopied, firstChild->getRegister());
                }
                ...
                if (!cg->canClobberNodesRegister(secondChild)) {
                    ...
                    generateRRInstruction(
                        cg, TR::InstOpCode::LGR, node, secondRegisterCopied, secondChild->getRegister());
                }
...
inline TR::Register* lDivRemGenericEvaluator(TR::Node* node, TR::CodeGenerator* cg, bool isDivision)
{
        ...
        // case A/(+-)2^x - division is done using shifts
            ...
            if (isDivision) {
                if (!firstChild->isNonNegative()) {
                    ...
                    cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::C, node,
                        dividendPair->getHighOrder(), (signed char)0, TR::InstOpCode::COND_BNLRC, skipSet);
                    cursor->setStartInternalControlFlow();
            ...
            } else // remainder
            {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    generateRILInstruction(cg, TR::InstOpCode::NILF, node, dividendPair->getLowOrder(),
                        (int32_t)((int64_t)CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenomintor + 1));
                    if (absValueOfDenomintor != (int32_t)absValueOfDenomintor)
                        generateRILInstruction(cg, TR::InstOpCode::NILF, node, dividendPair->getHighOrder(),
                            (int32_t)(((int64_t)CONSTANT64(0xFFFFFFFFFFFFFFFF) - absValueOfDenomintor + 1) >> 32));
                    generateRRInstruction(
    ...
    if (performTransformation(comp, "O^O Using 64bit ldiv on 32bit driver.\n")) {
        ...
        cursor = generateRRInstruction(
            cg, TR::InstOpCode::LR, node, dividendPair->getHighOrder(), dividendPair->getLowOrder());

        cursor = generateRRInstruction(
            cg, TR::InstOpCode::LGR, node, dividendPair->getLowOrder(), dividendPair->getHighOrder());
        ...
        cursor = generateRRInstruction(
            cg, TR::InstOpCode::LR, node, divisorPair->getHighOrder(), divisorPair->getLowOrder());
        ...
        if (!node->getOpCode().isUnsigned()) // Signed Divide Code
        {
            if (!node->isSimpleDivCheck() && !firstChild->isNonNegative() && !secondChild->isNonNegative()
                ...
                cursor
                    = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, node, gotoDiv);
                cursor->setStartInternalControlFlow();
                ...
                if (!isDivision) {
                    cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node,
                        divisorPair->getHighOrder(), (signed char)-1, TR::InstOpCode::COND_BNERC, gotoDiv);
    ...
    if (handleConstDenominator) {
        ...
        if (denominator == -1) {
            ...
            if (!isDivision) // remainder
            {
                ...
                cursor
                    = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, endDiv);
                cursor->setStartInternalControlFlow();
        ...
        // case A/(+-)2^x - division is done using shifts
            ...
            if (isDivision) {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    cursor = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, firstRegister,
                    ...
                    deps->addPostCondition(firstRegister, TR::RealRegister::AssignAny);
                    generateS390ImmOp(
                        cg, TR::InstOpCode::AG, node, firstRegister, firstRegister, absValueOfDenomintor - 1, deps);
                    skipSet->setEndInternalControlFlow();
            ...
            } else // remainder
            {
                ...
                if (!firstChild->isNonNegative()) {
                    ...
                    generateRILInstruction(
                        cg, TR::InstOpCode::NIHF, node, tempRegister2, (int32_t)((absValueOfDenomintor - 1) >> 32));
                    generateRILInstruction(
                        cg, TR::InstOpCode::NILF, node, tempRegister2, (int32_t)(absValueOfDenomintor - 1));
    ...
    TR::Node* node, TR::CodeGenerator* cg, bool isDivision, TR::MemoryReference* divchkDivisorMR)
{
    ...
    if (divchkDivisorMR && !needCheck) {
        ...
        && !secondChild->getSymbolReference()->getSymbol()->isRegisterSymbol() && !needCheck) {
    ...
    if (needCheck) {
        ...
        generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::C, node, sourceRegister, (signed char)-1, TR::InstOpCode::COND_BNERC, doDiv);
...
inline TR::Register* genericLongShiftSingle(TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic shiftOp)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (node->getOpCodeValue() == TR::lshl && firstChild->getOpCodeValue() == TR::i2l
            && (firstChild->getFirstChild()->isNonNegative() || firstChild->isNonNegative())
            && firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL) {
            srcReg = cg->evaluate(firstChild->getFirstChild());
            TR::InstOpCode::Mnemonic opCode = cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)
                ? TR::InstOpCode::RISBGN
                : TR::InstOpCode::RISBG;
            generateRIEInstruction(
                cg, opCode, node, trgReg, srcReg, (int8_t)(32 - value), (int8_t)((63 - value) | 0x80), (int8_t)value);
        ...
        if (popCnt == (64 - lZeros - tZeros)) {
            ...
            if (msBit > lsBit) {
                ...
                generateRIEInstruction(cg,
                    cg->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12) ? TR::InstOpCode::RISBGN
                                                                                             : TR::InstOpCode::RISBG,
                    node, targetReg, sourceReg, msBit, 0x80 + lsBit, shiftAmnt);
            else {
                if (!cg->canClobberNodesRegister(firstChild->getFirstChild()))
...
TR::Register* genericLongAndAsRotateHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    TR::Node* firstChild = node->getFirstChild();
    TR::Node* secondChild = node->getSecondChild();
    TR::Node* skipedConversion = NULL;
    ...
    if ((node->getOpCodeValue() == TR::land) && secondChild->getOpCode().isLoadConst()) {
        ...
        if (popCnt == (64 - lZeros - tZeros)) {
            ...
            lsBit = 63 - tZeros;
            if (firstChild->getReferenceCount() > 1
                || skipedConversion != NULL // Better of with non-destructive RISBG as AND?
...
TR::Register* OMR::Z::TreeEvaluator::lmulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCodeValue() == TR::lconst) {
        ...
        cursor = generateS390CompareAndBranchInstruction(
            cg, TR::InstOpCode::CG, node, sourceRegister, (signed char)0, TR::InstOpCode::COND_BNLRC, posMulh);
        cursor->setStartInternalControlFlow();
...
TR::Register* OMR::Z::TreeEvaluator::mulhEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        cursor = generateS390ImmOp(
            cg, TR::InstOpCode::M, node, targetRegisterPair->getLowOrder(), targetRegisterPair, value);
    } else {
        TR::Register* secondRegister = cg->evaluate(secondChild);
    ...
    if (secondChild->getOpCode().isLoadConst()) {
        ...
        if (value == -1 && !node->getOpCode().isUnsigned()) {
            ...
            if (!firstChild->isNonNegative() && !node->getOpCode().isUnsigned()) {
                ...
                cursor
                    = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, doShift);
                cursor->setStartInternalControlFlow();
