...
uint8_t* TR_S390CallSnippet::storeArgumentItem(
...
uint8_t* TR_S390CallSnippet::loadArgumentItem(
    TR::InstOpCode::Mnemonic op, uint8_t* buffer, TR::RealRegister* reg, int32_t offset)
{
    TR::RealRegister* stackPtr = cg()->getStackPointerRealRegister();
    TR::InstOpCode opCode(op);
    opCode.copyBinaryToBuffer(buffer);
...
uint8_t* TR_S390CallSnippet::S390flushArgumentsToStack(
            ...
            if (intArgNum < linkage->getNumIntegerArgumentRegisters()) {
                if (TR::Compiler->target.is64Bit()) {
                    ...
                    if (intArgNum < linkage->getNumIntegerArgumentRegisters() - 1) {
                        buffer = storeArgumentItem(TR::InstOpCode::ST, buffer,
                            machine->getS390RealRegister(linkage->getIntegerArgumentRegister(intArgNum + 1)),
                            offset + 4, cg);
                    }
                }
            }
 ...
 */
int32_t TR_S390CallSnippet::instructionCountForArguments(TR::Node* callNode, TR::CodeGenerator* cg)
{
...
uint8_t* TR_S390CallSnippet::getCallRA()
{
...
TR_RuntimeHelper TR_S390CallSnippet::getHelper(TR::MethodSymbol* methodSymbol, TR::DataType type, TR::CodeGenerator* cg)
{
...
TR_RuntimeHelper TR_S390CallSnippet::getInterpretedDispatchHelper(TR::SymbolReference* methodSymRef, TR::DataType type)
{
    TR::Compilation* comp = cg()->comp();
    TR::MethodSymbol* methodSymbol = methodSymRef->getSymbol()->castToMethodSymbol();
    bool isJitInduceOSRCall = false;
    if (methodSymbol->isHelper() && methodSymRef == cg()->symRefTab()->element(TR_induceOSRAtCurrentPC)) {
...
uint8_t* TR_S390CallSnippet::emitSnippetBody()
{
    TR_ASSERT(0, "TR_S390CallSnippet::emitSnippetBody not implemented");
    return NULL;
}

uint32_t TR_S390CallSnippet::getLength(int32_t estimatedSnippetStart)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR_S390CallSnippet* snippet)
{
