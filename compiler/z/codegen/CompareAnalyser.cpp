    ...
    bool& internalControlFlowStarted)
{
    ...
    if (_cg->whichChildToEvaluate(root) == 0) {
        ...
        if (getEvalChild2()) {
            if (secondChild->getReferenceCount() == 1 && secondChild->getRegister() == NULL && !(secondChild->getOpCode().hasSymbolReference() && secondChild->getSymbolReference()->getSymbol()->isRegisterSymbol()) && (secondChild->getOpCodeValue() == TR::lload || (secondChild->getOpCodeValue() == TR::iload && secondIU2L))) {
                lowSecondMR = generateS390MemoryReference(secondChild, _cg);
                delayedSecond = _cg->allocateRegister();
                if (!secondIU2L) {
                    highSecondMR = generateS390MemoryReference(*lowSecondMR, 4, _cg);
        ...
        if (getEvalChild1()) {
            if (firstChild->getReferenceCount() == 1 && firstChild->getRegister() == NULL && !(firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference()->getSymbol()->isRegisterSymbol()) && (firstChild->getOpCodeValue() == TR::lload || (firstChild->getOpCodeValue() == TR::iload && firstIU2L))) {
                lowFirstMR = generateS390MemoryReference(firstChild, _cg);
                delayedFirst = _cg->allocateRegister();
                if (!firstIU2L) {
                    highFirstMR = generateS390MemoryReference(*lowFirstMR, 4, _cg);
