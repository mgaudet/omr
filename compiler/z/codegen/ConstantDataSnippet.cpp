...
void TR_S390ConstantDataSnippet::addMetaDataForCodeAddress(uint8_t* cursor)
{
    ...
    case TR_JNIVirtualTargetAddress: {
        ...
        cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                   TR_ExternalRelocation(cursor, (uint8_t*)getNode()->getSymbolReference(),
                                       getNode() ? (uint8_t*)(intptr_t)getNode()->getInlinedSiteIndex() : (uint8_t*)-1,
                                       (TR_ExternalRelocationTargetKind)reloType, cg()),
            __FILE__, __LINE__, getNode());
    } break;
    ...
    case TR_BodyInfoAddress:
        ...
        if (TR::Compiler->target.is64Bit()) {
            ...
            cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                       TR_ExternalRelocation(cursor, (uint8_t*)(intptr_t) * ((uint32_t*)cursor),
                                           (TR_ExternalRelocationTargetKind)reloType, cg()),
                __FILE__, __LINE__, getNode());
        }
        break;
    ...
    case TR_GlobalValue:
        ...
        cg()->addAOTRelocation(new (cg()->trHeapMemory())
                                   TR_ExternalRelocation(cursor, (uint8_t*)TR_CountForRecompile, TR_GlobalValue, cg()),
            __FILE__, __LINE__, getNode());
        break;
    ...
    case TR_ClassPointer: {
        ...
        // node to get the target address
        // so we need to pass it to relocation in targetaddress2 for now
        // two instances where use this relotype in such way are: profile checkcast and arraystore check object check
        // optimiztaions
        ...
        relo = new (cg()->trHeapMemory()) TR_ExternalRelocation(
...
uint8_t* TR_S390ConstantDataSnippet::emitSnippetBody()
{
    // *this   swipeable for debugging purposes
...
uint32_t TR_S390ConstantDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    // *this   swipeable for debugging purposes
    return _length;
}

TR_S390ConstantInstructionSnippet::TR_S390ConstantInstructionSnippet(
...
uint8_t* TR_S390ConstantInstructionSnippet::emitSnippetBody()
{
...
int64_t TR_S390ConstantInstructionSnippet::getDataAs8Bytes()
{
    emitSnippetBody();
...
uint8_t* TR_S390EyeCatcherDataSnippet::emitSnippetBody()
{
    uint8_t* cursor = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(cursor);
 ...
 */
TR_S390WarmEyeCatcherDataSnippet::TR_S390WarmEyeCatcherDataSnippet(
    TR::CodeGenerator* cg, TR::Node* n, TR::LabelSymbol* fullEyeCatcherSnippetLabel)
    : TR_S390ConstantDataSnippet(cg, n, NULL, 0)
{
    _fullEyeCatcherSnippet = fullEyeCatcherSnippetLabel;

    setWarmSnippet(); // Set as Warm Snippet - this is always warm... If TCC is disabled, only the full eyecatcher will
                      // be emitted then.
...
uint8_t* TR_S390WarmEyeCatcherDataSnippet::emitSnippetBody()
{
    uint8_t* cursor = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(cursor);
...
//////////////////////////////////////////////////////////////////////////////////////
TR_S390TargetAddressSnippet::TR_S390TargetAddressSnippet(
...
uint8_t* TR_S390TargetAddressSnippet::emitSnippetBody()
{
    // *this   swipeable for debugging purposes
    uint8_t* cursor = cg()->getBinaryBufferCursor();
    ...
    if (_targetsnippet != NULL) {
        *(intptrj_t*)cursor = 0;
        ...
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_LabelAbsoluteRelocation(cursor, _targetsnippet->getSnippetLabel()));
...
uint32_t TR_S390TargetAddressSnippet::getLength(int32_t estimatedSnippetStart)
{
    // *this   swipeable for debugging purposes
...
uint8_t* TR_S390LookupSwitchSnippet::emitSnippetBody()
{
    ...
    for (int32_t i = 1; i <= numChildren; i++) {
        ...
        intptrj_t caseKeyTarget
            = (intptrj_t)caseKeyNode->getBranchDestination()->getNode()->getLabel()->getCodeLocation();
...
uint32_t TR_S390LookupSwitchSnippet::getLength(int32_t estimatedSnippetStart)
{
    TR::Compilation* comp = cg()->comp();
...
///////////////////////////////////////////////////////////////////////////////
TR_S390InterfaceCallDataSnippet::TR_S390InterfaceCallDataSnippet(
...
TR_S390InterfaceCallDataSnippet::TR_S390InterfaceCallDataSnippet(
    TR::CodeGenerator* cg, TR::Node* node, uint8_t n, uint8_t* thunkPtr, bool useCLFIandBRCL)
    : TR_S390ConstantDataSnippet(cg, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), 0)
    , _numInterfaceCallCacheSlots(n)
    , _codeRA(NULL)
...
uint8_t* TR_S390InterfaceCallDataSnippet::emitSnippetBody()
{
    ...
    int32_t padBytes
        = ((intptrj_t)cursor + 5 * sizeof(intptrj_t) + 7) / 8 * 8 - ((intptrj_t)cursor + 5 * sizeof(intptrj_t));
    cursor += padBytes;
    ...
    *(uintptrj_t*)cursor = (uintptrj_t)_codeRA;
    AOTcgDiag1(comp, "add TR_AbsoluteMethodAddress cursor=%x\n", cursor);
    cg()->addProjectSpecializedRelocation(cursor, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, callNode);
    cursor += TR::Compiler->om.sizeofReferenceAddress();
    ...
    *(uintptrj_t*)cursor = (uintptrj_t)callNode->getSymbolReference()->getOwningMethod(comp)->constantPool();
    ...
    cg()->addProjectSpecializedRelocation(cursor, *(uint8_t**)cursor,
        callNode ? (uint8_t*)(intptr_t)callNode->getInlinedSiteIndex() : (uint8_t*)-1, TR_Thunks, __FILE__, __LINE__,
        callNode);
    cursor += TR::Compiler->om.sizeofReferenceAddress();
    ...
    *(intptrj_t*)cursor = 0;
    ...
    if (!comp->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot)) {
        ...
        *(intptrj_t*)(cursor) = snippetStart + getFirstSlotOffset() - (2 * TR::Compiler->om.sizeofReferenceAddress());
        AOTcgDiag1(comp, "add TR_AbsoluteMethodAddress cursor=%x\n", cursor);
        cg()->addProjectSpecializedRelocation(
            cursor, NULL, NULL, TR_AbsoluteMethodAddress, __FILE__, __LINE__, callNode);
        cursor += TR::Compiler->om.sizeofReferenceAddress();
    ...
    if (updateField) {
        *(intptrj_t*)(cursorlastCachedSlot)
            = snippetStart + getFirstSlotOffset() + ((getNumInterfaceCallCacheSlots() - numInterfaceCallCacheSlots - 1)
...
uint32_t TR_S390InterfaceCallDataSnippet::getCallReturnAddressOffset() { return 0; }

uint32_t TR_S390InterfaceCallDataSnippet::getSingleDynamicSlotOffset()
{
    return getCallReturnAddressOffset() + 5 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390InterfaceCallDataSnippet::getLastCachedSlotFieldOffset()
{
    return getCallReturnAddressOffset() + 6 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390InterfaceCallDataSnippet::getFirstSlotFieldOffset()
{
    return getLastCachedSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390InterfaceCallDataSnippet::getLastSlotFieldOffset()
{
    return getFirstSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390InterfaceCallDataSnippet::getFirstSlotOffset()
{
    return getLastSlotFieldOffset() + TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390InterfaceCallDataSnippet::getLastSlotOffset()
{
...
uint32_t TR_S390InterfaceCallDataSnippet::getLength(int32_t)
{
    TR::Compilation* comp = cg()->comp();
    ...
    if (comp->getOption(TR_enableInterfaceCallCachingSingleDynamicSlot))
        return 2 * TR::Compiler->om.sizeofReferenceAddress() + getSingleDynamicSlotOffset()
...
uint32_t TR_S390JNICallDataSnippet::getJNICallOutFrameFlagsOffset()
{
    return TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390JNICallDataSnippet::getReturnFromJNICallOffset()
{
    return 2 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390JNICallDataSnippet::getSavedPCOffset() { return 3 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR_S390JNICallDataSnippet::getTagBitsOffset() { return 4 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR_S390JNICallDataSnippet::getPCOffset() { return 5 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR_S390JNICallDataSnippet::getLiteralsOffset() { return 6 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR_S390JNICallDataSnippet::getJitStackFrameFlagsOffset()
{
    return 7 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390JNICallDataSnippet::getConstReleaseVMAccessMaskOffset()
{
    return 8 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390JNICallDataSnippet::getConstReleaseVMAccessOutOfLineMaskOffset()
{
    return 9 * TR::Compiler->om.sizeofReferenceAddress();
}

uint32_t TR_S390JNICallDataSnippet::getTargetAddressOffset() { return 10 * TR::Compiler->om.sizeofReferenceAddress(); }

uint32_t TR_S390JNICallDataSnippet::getLength(int32_t estimatedSnippetStart)
{
    return 12 * TR::Compiler->om.sizeofReferenceAddress(); /*one ptr more for possible padding */
}

TR_S390JNICallDataSnippet::TR_S390JNICallDataSnippet(TR::CodeGenerator* cg, TR::Node* node)
    : TR_S390ConstantDataSnippet(cg, node, TR::LabelSymbol::create(cg->trHeapMemory(), cg), 0)
    , _baseRegister(0)
    , _ramMethod(0)
...
uint8_t* TR_S390JNICallDataSnippet::emitSnippetBody()
{
    uint8_t* cursor = cg()->getBinaryBufferCursor();
    ...
    *(intptrj_t*)cursor = (intptrj_t)_ramMethod;
    ...
    AOTcgDiag2(comp, "add relocation (%d) cursor=%x\n", reloType, cursor);
    cg()->addAOTRelocation(new (cg()->trHeapMemory())
                               TR_ExternalRelocation(cursor, (uint8_t*)callNode->getSymbolReference(),
    ...
    *(intptrj_t*)cursor = (intptrj_t)(_returnFromJNICallLabel->getCodeLocation());
    ...
    cg()->addAOTRelocation(new (cg()->trHeapMemory())
                               TR_ExternalRelocation(cursor, NULL, TR_AbsoluteMethodAddress, cg()),
        __FILE__, __LINE__, getNode());
...
void TR_S390JNICallDataSnippet::print(TR::FILE* pOutFile, TR_Debug* debug)
{
    /*
           ramMethod
           JNICallOutFrameFlags
...
void TR_Debug::print(TR::FILE* pOutFile, TR_S390TargetAddressSnippet* snippet)
{
    // *this   swipeable for debugger
    if (pOutFile == NULL) {
        return;
...
void TR_Debug::print(TR::FILE* pOutFile, TR_S390LookupSwitchSnippet* snippet)
{
...
void TR_Debug::print(TR::FILE* pOutFile, TR_S390ConstantDataSnippet* snippet)
{
    // *this   swipeable for debugger
    if (pOutFile == NULL) {
        return;
...
void TR_Debug::print(TR::FILE* pOutFile, TR_S390InterfaceCallDataSnippet* snippet)
{
    uint8_t* bufferPos = snippet->getSnippetLabel()->getCodeLocation();
    ...
    if (snippet->getNumInterfaceCallCacheSlots() == 0) {
        ...
        for (i = 0; i < snippet->getNumInterfaceCallCacheSlots(); i++) {
            if (snippet->isUseCLFIandBRCL()) {
                printPrefix(pOutFile, NULL, bufferPos, sizeof(intptrj_t));
                trfprintf(pOutFile, "DC   \t%p \t\t# Addr of CLFI immed field for cached class %d",
...
TR_S390LabelTableSnippet::TR_S390LabelTableSnippet(TR::CodeGenerator* cg, TR::Node* node, uint32_t size)
    : TR_S390ConstantDataSnippet(cg, node, NULL, 0)
    , _size(size)
{
...
uint8_t* TR_S390LabelTableSnippet::emitSnippetBody()
{
...
uint32_t TR_S390LabelTableSnippet::getLength(int32_t estimatedSnippetStart) { return _size * 4; }

TR_S390DeclTrampSnippet::TR_S390DeclTrampSnippet(TR::CodeGenerator* cg, TR::LabelSymbol* label)
    : TR_S390ConstantDataSnippet(cg, NULL, NULL, 0)
    , _label(label)
{
...
uint8_t* TR_S390DeclTrampSnippet::emitSnippetBody()
{
    uint8_t* snip = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(snip);
    uint8_t* cursor = snip;
...
uint8_t* TR_S390SortJumpTrampSnippet::emitSnippetBody()
{
    uint8_t* snip = cg()->getBinaryBufferCursor();
    getSnippetLabel()->setCodeLocation(snip);
    uint8_t* cursor = snip;
