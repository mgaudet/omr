...
extern TR::Instruction* generateS390AggregateCompareAndBranchOps(TR::Node* node, TR::CodeGenerator* cg,
    TR::InstOpCode::S390BranchCondition fBranchOpCond, TR::InstOpCode::S390BranchCondition rBranchOpCond,
    TR::InstOpCode::S390BranchCondition& retBranchOpCond, TR::LabelSymbol* branchTarget = NULL);
...
static bool virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((!node->isNopableInlineGuard() && !node->isHCRGuard()) || !cg->getSupportsVirtualGuardNOPing()) {
        return false;
    }

    TR_VirtualGuard* virtualGuard = comp->findVirtualGuardInfo(node);
    if (!node->isHCRGuard() && !(comp->performVirtualGuardNOPing() && comp->isVirtualGuardNOPingRequired(virtualGuard))
    ...
    TR::InstOpCode::S390BranchCondition brCond, bool isBoolean)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getRegister() == NULL) {
        ...
        instr = generateS390CompareAndBranchInstruction(cg, isUnsigned ? TR::InstOpCode::CLGR : TR::InstOpCode::CGR,
...
TR::Register* OMR::Z::TreeEvaluator::fcmplEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::Instruction* cursor
        = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, doneCmp);
...
TR::Register* lcmpHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (secondChild->getOpCode().isLoadConst() && secondChild->getLongInt() == 0) {
        ...
        // If LT we are done
        TR::Instruction* cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, labelLT);
        cursor->setStartInternalControlFlow();
...
{
    return node->getOpCode().isIf() && isSingleRefUnevalAndCompareOrBu2iOverCompare(node->getFirstChild())
    ...
    if (node->isTheVirtualGuardForAGuardedInlinedCall() && cg->getSupportsVirtualGuardNOPing()) {
        ...
        if (virtualGuard && virtualGuard->mergedWithHCRGuard()) {
...

            {
                ...
                TR::Instruction* vgnopInstr = generateVirtualGuardNOPInstruction(
                    cg, node, site, mergedHCRDeps, label, instr ? instr->getPrev() : NULL);
                vgnopInstr->setNext(instr);
                comp->setAppendInstruction(instr);
            }
...
TR::Register* OMR::Z::TreeEvaluator::ificmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq
        ...
        reg = generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, TR::InstOpCode::COND_BERC);
    } else {
        reg = generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC);
    }
    generateMergedHCRGuardCodeIfNeeded(node, cg);
    return reg;
}
...
TR::Register* OMR::Z::TreeEvaluator::ificmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BHRC);
}
...
TR::Register* OMR::Z::TreeEvaluator::ificmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, TR::InstOpCode::COND_BNHRC);
}
...
TR::Register* OMR::Z::TreeEvaluator::ificmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BLRC);
}
...
TR::Register* OMR::Z::TreeEvaluator::ificmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, TR::InstOpCode::COND_BNLRC);
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_NOP,
            TR::InstOpCode::COND_BERC, CMP4CONTROLFLOW);
    }
    generateMergedHCRGuardCodeIfNeeded(node, cg);
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        generateS390CompareBranch(
            node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC);
    } else {
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, TR::InstOpCode::COND_BNERC,
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BLRC,
            TR::InstOpCode::COND_BNHRC, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BLRC,
            TR::InstOpCode::COND_BLRC, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BHRC,
            TR::InstOpCode::COND_BNLRC, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::iflcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BHRC,
            TR::InstOpCode::COND_BHRC, CMP4CONTROLFLOW);
    }
    return NULL;
}
...
TR::Register* OMR::Z::TreeEvaluator::lcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC,
...
TR::Register* OMR::Z::TreeEvaluator::lcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC,
...
TR::Register* OMR::Z::TreeEvaluator::lcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
        ...
        return generateS390lcmpEvaluator(node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC,
...
TR::Register* OMR::Z::TreeEvaluator::tableEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (upperBound < node->getNumChildren() && (node->getChild(upperBound)->getOpCodeValue() == TR::loadaddr
     ...
     * branch to it, and an address table, where we load an address out of the table and branch to it
    ...
    case AddressTable32bit: {
        if (!node->chkCannotOverflow())
            generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CL, node, selectorReg, numBranchTableEntries,
                TR::InstOpCode::COND_BNLRC, node->getSecondChild()->getBranchDestination()->getNode()->getLabel(),
                false, false); // make sure the case selector is within range of our case constants
            ...
            if (type.isInt64()) {
                if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())
                    ...
                    generateS390BranchInstruction(
                        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, node, skipLoCompareLabel);
        ...
        if (cg->getHasResumableTrapHandler()) {
            ...
            if (!firstChild->getOpCode().isCall() && reference->getOpCode().isLoadVar()
                ...
                appendTo = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, snippetLabel, appendTo);
...
TR::Register* OMR::Z::TreeEvaluator::ZEROCHKEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (valueToCheck->getOpCode().getOpCodeValue() == TR::acmpeq) {
        TR::InstOpCode::S390BranchCondition opBranchCond
            = generateS390CompareOps(valueToCheck, cg, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC);
...
TR::Register* OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (comp->getOption(TR_TraceCG))
        traceMsg(comp, "Starting evaluation of ternary node %p condition %p (in reg %p) trueVal %p (in reg %p) "
                       "falseVal %p (in reg %p)\n",
    ...
    if (treeContainsAllOtherUsesForNode(condition, trueVal)
        ...
        if (comp->getOption(TR_TraceCG))
            traceMsg(comp, "Calling evaluate (instead of clobber evaluate for node %p because all other uses are in "
                           "the compare tree)\n",
                trueVal);
    ...
    if (condition->getOpCode().isBooleanCompare()
        ...
        if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
            ...
            generateRRFInstruction(cg, is64BitRegister ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR, node,
                trueReg->getHighOrder(), falseReg->getHighOrder(),
                getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true);
            generateRRFInstruction(cg, is64BitRegister ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR, node,
                trueReg->getLowOrder(), falseReg->getLowOrder(),
                getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4, true);
        } else
            generateRRFInstruction(cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR,
                node, trueReg, falseReg, getMaskForBranchCondition(mapBranchConditionToLOCRCondition(bc)) >> 4, true);
    } else {
        if (comp->getOption(TR_TraceCG))
        ...
        TR::Instruction* compareInst = generateRILInstruction(cg,
            ...
            condition->getRegister(), (uintptrj_t)0);
        ...
        if (trueReg->getRegisterPair() || falseReg->getRegisterPair()) {
            ...
            generateRRFInstruction(cg, trueVal->getOpCode().is8Byte() ? TR::InstOpCode::LOCGR : TR::InstOpCode::LOCR,
                node, trueReg, falseReg->getRegister(), getMaskForBranchCondition(TR::InstOpCode::COND_BERC) >> 4,
                true);
    }
...
TR::Register* OMR::Z::TreeEvaluator::dternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (conditionNode->getOpCode().isCompareDouble()) {
        ...
        // correct "true value" for cmpne operations
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::RegisterDependencyConditions** retDeps)
{
    ...
    cursor = new (cg->trHeapMemory()) TR::S390RRFInstruction(TR::InstOpCode::CLRT, node, zeroReg, zeroReg,
