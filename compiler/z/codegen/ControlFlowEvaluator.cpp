...
virtualGuardHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isHCRGuard() && !(comp->performVirtualGuardNOPing() && comp->isVirtualGuardNOPingRequired(virtualGuard)) && virtualGuard->canBeRemoved()) {
        return false;
    }
...
{
    return node->getOpCode().isIf() && isSingleRefUnevalAndCompareOrBu2iOverCompare(node->getFirstChild()) && node->getSecondChild()->getOpCode().isLoadConst() && node->getSecondChild()->getType().isInt32();
}
...
OMR::Z::TreeEvaluator::ternaryEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (treeContainsAllOtherUsesForNode(condition, trueVal) && (!trueVal->isUnneededConversion() || treeContainsAllOtherUsesForNode(condition, trueVal->getFirstChild()))) {
        if (comp->getOption(TR_TraceCG))
