...
inline TR::Register* convertToFixed(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else // Signed Target Path
    {
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node, doneLabel); // NaN results in CC3
...
TR::Register* commonLong2FloatEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::LabelSymbol* doneLabel
        = deps ? TR::LabelSymbol::create(cg->trHeapMemory(), cg) : NULL; // attach all deps to this label at end
    if (isUnsigned) {
        TR::Register* litBase = NULL;
        bool stopUsingMemRefRegs = false;
        if (litBaseNode) {
        ...
        TR::Instruction* cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, node, doneLabel);
        if (is128) {
...
TR::Register* OMR::Z::TreeEvaluator::floatRemHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getDataType() == TR::Float) {
        ...
        cursor = generateRRFInstruction(
            cg, TR::InstOpCode::DIEBR, node, targetRegister, secondRegister, tempRegister, 0x5);
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BCRC, node, labelNotExact); // remainder not exact
        cursor->setStartInternalControlFlow();
        generateRXInstruction(cg, TR::InstOpCode::TCEB, node, targetRegister, 0x800); // c is +0 ?
        generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, labelOK); // it is not +0
    ...
    if (node->getDataType() == TR::Float)
        remHelper = (void*)cg->symRefTab()
                        ->findOrCreateRuntimeHelper(TR_S390floatRemainder, false, false, false)
                        ->getMethodAddress();
    else
        remHelper = (void*)cg->symRefTab()
                        ->findOrCreateRuntimeHelper(TR_S390doubleRemainder, false, false, false)
                        ->getMethodAddress();
...
TR::Register* OMR::Z::TreeEvaluator::lbits2dEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            ...
            TR_ASSERT(false, "must ensure sourceReg->getHighOrder() is a 64BitReg in SLLG so local RA will use 64 bit "
                             "instructions\n");
            TR::Register* targetReg = cg->allocateRegister(TR_FPR);
...
TR::Register* OMR::Z::TreeEvaluator::dbits2lEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* positiveInfinity
        = generateS390MemoryReference((int64_t)CONSTANT64(0x7ff0000000000000), TR::Int64, cg, litBase);
    TR::MemoryReference* negativeInfinity
        = generateS390MemoryReference((int64_t)CONSTANT64(0xfff0000000000000), TR::Int64, cg, litBase);
    TR::MemoryReference* NaN
        = generateS390MemoryReference((int64_t)CONSTANT64(0x7ff8000000000000), TR::Int64, cg, litBase);
    if ((TR::Compiler->target.cpu.getS390SupportsFPE()) && (!disabled)) {
        if (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit()) {
            TR::RegisterDependencyConditions* deps
            ...
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node, positiveInfinityNumber);
            generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, negativeInfinityNumber);
        ...
        generateRXInstruction(cg, TR::InstOpCode::TCDB, node, sourceReg, (uint32_t)0x03f);
        cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node, cleansedNumber);
        cursor->setStartInternalControlFlow();
        if (litBase) {
            deps->addPostCondition(litBase, TR::RealRegister::AssignAny);
        }
...
TR::Register* d2lHelper64(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (checkNaN) {
        ...
        TR::Instruction* cursor
            = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNZRC, node, label1);
        cursor->setStartInternalControlFlow();
...
TR::Register* OMR::Z::TreeEvaluator::iffcmpeqEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, TR::InstOpCode::COND_BERC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpneEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpltEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BHRC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpgeEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, TR::InstOpCode::COND_BNHRC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpgtEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BLRC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpleEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, TR::InstOpCode::COND_BNLRC, false);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpequEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, TR::InstOpCode::COND_BERC, true);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpneuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, TR::InstOpCode::COND_BNERC, true);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpltuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, TR::InstOpCode::COND_BHRC, true);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpgeuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNLRC, TR::InstOpCode::COND_BNHRC, true);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpgtuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BHRC, TR::InstOpCode::COND_BLRC, true);
}

TR::Register* OMR::Z::TreeEvaluator::iffcmpleuEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    return generateS390CompareBranch(
        node, cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, TR::InstOpCode::COND_BNLRC, true);
}
