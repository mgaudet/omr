...
{
    return (_mnemonic == DIRECTIVE || _mnemonic == RET || _mnemonic == ASSOCREGS || _mnemonic == DEPEND
        || _mnemonic == FENCE || _mnemonic == SCHEDFENCE || _mnemonic == PROC || _mnemonic == DC || _mnemonic == DC2
        || _mnemonic == LOCK || _mnemonic == UNLOCK || _mnemonic == ASM || _mnemonic == DS);
}
...
{
    uint64_t mask
        = ((i == 1) ? S390OpProp_TargetLW : 0) | ((i == 2) ? S390OpProp_SrcLW : 0) | ((i == 3) ? S390OpProp_Src2LW : 0);
    return properties[_mnemonic] & mask;
}
...
/* Using a table that maps values of bit 0-1 to instruction length from POP chapter 5,  topic: Instruction formats (page
 * 5.5)*/
...
#define GET_OPCODE_FROM_NODE(opKind, opcode64, opcode32)                                                               \
    TR::InstOpCode::Mnemonic OMR::Z::InstOpCode::get##opKind##OpCodeFromNode(TR::Node* node)                           \
    {                                                                                                                  \
...
{
    return TR::Compiler->target.is64Bit() ? TR::InstOpCode::CGFRT : TR::InstOpCode::CRT;
}
...
{
    return TR::Compiler->target.is64Bit() ? TR::InstOpCode::CLGFRT : TR::InstOpCode::CLRT;
}
