    ...
    , _outgoingArgLevelDuringTreeEvaluation(0)
{
    ...
    if (enableBranchPreload || (!disableBranchPreload && comp->isOptServer()
    ...
    if ((enableBranchPreload && bpp) || (bpp && !disableBranchPreload && comp->isOptServer()
                                            && _processorInfo.supportsArch(TR_S390ProcessorInfo::TR_zEC12)))
...
void OMR::Z::CodeGenerator::setupSpecializedEpilogues()
{
    ...
    if (self()->specializedEpilogues()) {
        ...
        _blocksThatModifyRegister = (TR_BitVector**)self()->trMemory()->allocateHeapMemory(
            numRegs * sizeof(TR_BitVector*), TR_Memory::CodeGenerator);

        int32_t reg;
        TR_ASSERT(
            self()->comp()->getFlowGraph(), "TR::CodeGenerator - %s needs a flow graph\n", self()->comp()->signature());
        for (reg = 0; reg < numRegs; reg++)
            _blocksThatModifyRegister[reg] = new (self()->trHeapMemory()) TR_BitVector(
                self()->comp()->getFlowGraph()->getNextNodeNumber(), self()->trMemory(), heapAlloc, growable);
    }
}
    ...
    if (!_globalRegisterTable) {
        ...
        if (enableHighWordGRA) {
            _globalGPRsPreservedAcrossCalls.init(
                NUM_S390_GPR + NUM_S390_FPR + NUM_S390_AR + NUM_S390_HPR, self()->trMemory());
        ...
        if (!self()->comp()->getOption(TR_DisableRegisterPressureSimulation)) {
            ...
            for (grn = 0; grn < self()->getNumberOfGlobalRegisters(); grn++) {
                ...
                TR_ASSERT(reg != -1, "Register pressure simulator doesn't support gaps in the global register table; "
                                     "reg %d must be removed",
                    grn);
 ...
 */
bool OMR::Z::CodeGenerator::mayImmedInstructionCauseOverFlow(TR::Node* node) { return false; }
...
void OMR::Z::CodeGenerator::insertPad(TR::Node* theNode, TR::Instruction* insertionPoint, int32_t padSize, bool before)
{
    if (0 == padSize) {
        return;
    }
    ...
    switch (padSize) {
    case 4:
        (void)new (self()->trHeapMemory())
            TR::S390NOPInstruction(TR::InstOpCode::NOP, 4, theNode, insertionPoint, self());
        break;
    ...
    case 6:
        (void)new (self()->trHeapMemory())
            TR::S390NOPInstruction(TR::InstOpCode::NOP, 4, theNode, insertionPoint, self());
        (void)new (self()->trHeapMemory())
            TR::S390NOPInstruction(TR::InstOpCode::NOP, 2, theNode, insertionPoint, self());
        break;
    ...
    if (self()->supportsBranchPreload()) {
        if (_hottestReturn._frequency > 6) {
            ...
            TR::RealRegister* spReg
                = self()->getS390Linkage()->getS390RealRegister(self()->getS390Linkage()->getStackPointerRegister());
    ...
    if (!self()->isOutOfLineColdPath() && _extentOfLitPool < 0) {
        ...
        if (self()->comp()->getOption(TR_Randomize)) {
            if (randomizer.randomBoolean(300)
                && performTransformation(self()->comp(), "O^O Random Codegen - Added 6000 dummy slots to  Literal Pool "
            ...
            if (li->getLabelSymbol() != NULL) {
                if (li->getLabelSymbol()->isStartInternalControlFlow()) {
                    TR_ASSERT(!li->isStartInternalControlFlow(),
                        "An instruction should not be both the start of internal control flow and have a lebel that is "
                        "pegged as the start of internal control flow, because we should only count it once");
                    _internalControlFlowNestingDepth--;
                    if (_internalControlFlowNestingDepth == 0)
                        self()->endInternalControlFlow(instructionCursor); // Walking backwards so start is end
                }
                if (li->getLabelSymbol()->isEndInternalControlFlow()) {
                    TR_ASSERT(!li->isEndInternalControlFlow(),
                        "An instruction should not be both the end of internal control flow and have a lebel that is "
                        "pegged as the end of internal control flow, because we should only count it once");
                    _internalControlFlowNestingDepth++;
                    self()->startInternalControlFlow(instructionCursor);
                }
            }
                            ...
                            if (instructionCursor->getNode()->getBranchDestination()->getNode() != NULL) {
                                if ((instructionCursor->getNode()->getBranchDestination()->getNode()->getBlock()
                                    ...
                                    if (instructionCursor->getNode()
                                        ...
                                        TR_ASSERT(1, "ASSERTION - Branch found outside Internal Control Flow that "
                                                     "should be located inside Internal Control Flow\n");
                                    }
                                }
                            }
                        }
...
void OMR::Z::CodeGenerator::printStats(int32_t instCount)
{
    ...
    fprintf(stderr, "REGISTER MOTION Compiled as [%10.10s] Factor [%4.4f] Contains hot blocks with "
                    "[spills:%4.4d,xfers:%4.4d,moves:%4.4d] cold blocks with [spills:%4.4d,xfers:%4.4d,moves:%4.4d] "
                    "for %s\n",
...
bool TR_S390Peephole::AGIReduction()
{
    ...
    if (_cg->getCurrentlyRestrictedRegisters()
        && _cg->getCurrentlyRestrictedRegisters()->isSet(toRealRegister(lgrTargetReg)->getRegisterNumber() - 1))
        return performed;
...
bool TR_S390Peephole::LAReduction()
{
    ...
    if (s390Instr->getKind() == TR::Instruction::IsRX) {
        ...
        if (mr && mr->isBucketBaseRegMemRef() && mr->getOffset() == 0 && laBaseReg && laTargetReg
            && laBaseReg == laTargetReg && laIndexReg == NULL && (symRef == NULL || symRef->getOffset() == 0)
...
bool TR_S390Peephole::clearsHighBitOfAddressInReg(TR::Instruction* inst, TR::Register* targetReg)
{
 ...
 * in a memref
 */
bool TR_S390Peephole::seekRegInFutureMemRef(int32_t maxWindowSize, TR::Register* targetReg)
{
    TR::Instruction* current = _cursor->getNext();
    int32_t windowSize = 0;
    ...
    if (lgrTargetReg == lgrSourceReg
        ...
        if ((prev->getOpCodeValue() == TR::InstOpCode::LR && lgrOpCode.getOpCodeValue() == TR::InstOpCode::LTR)
            ...
            if ((lgrTargetReg == prevTargetReg || lgrTargetReg == prevSourceReg)
                && performTransformation(comp(), "\nO^O S390 PEEPHOLE: Transforming load register into load and test "
        ...
        // ie. Add/Subtract Logical
        // this is also done by isActiveLogicalCC, and the end of generateS390CompareAndBranchOpsHelper when the virtual
        // registers match
        // but those cannot handle the case when the virtual registers are not the same but we do have the same
        // restricted register
        // which is why we are handling it here when all the register assignments are done, and the redundant LR's from
        // the
        ...
        if (prev->getOpCode().setsCC() && prev->getOpCode().setsCarryFlag()
            ...
            if ((branchCond == TR::InstOpCode::COND_BERC || branchCond == TR::InstOpCode::COND_BNERC)
                && performTransformation(comp(), "\nO^O S390 PEEPHOLE: Removing redundant Load and Test instruction at "
                                                 "%p, because CC can be reused from logical instruction %p\n",
                       _cursor, prev)) {
                _cg->deleteInst(_cursor);
 ...
 *    ...    ...
 */
bool TR_S390Peephole::ConditionalBranchReduction(TR::InstOpCode::Mnemonic branchOPReplacement)
{
    ...
    TR::InstOpCode::S390BranchCondition cond
        = getBranchConditionForMask(0xF - ((getMaskForBranchCondition(branchInst->getBranchCondition()) >> 4) & 0xF));
    ...
    if (performTransformation(
        ...
        while (currInst = realInstructionWithLabelsAndRET(nextInst, true)) {
            ...
            _cg->replaceInst(
                RIInst, _cursor = generateRIEInstruction(_cg,
...
bool TR_S390Peephole::LoadAndMaskReduction(TR::InstOpCode::Mnemonic LZOpCode)
{
    ...
    if (_cursor->getNext()->getOpCodeValue() == TR::InstOpCode::NILL) {
        ...
        if (performTransformation(
            ...
            _cg->replaceInst(
                loadInst, _cursor = generateRXYInstruction(_cg, LZOpCode, comp()->getStartTree()->getNode(),
...
bool TR_S390Peephole::DAAHandleMemoryReferenceSpill(bool hasPadding)
{
    ...
    if (_cursor->throwsImplicitException()) {
        ...
        if (nextInst->getOpCodeValue() != TR::InstOpCode::BRC && nextInst->getOpCodeValue() != TR::InstOpCode::BRCL) {
            if (performTransformation(comp(), "O^O S390 PEEPHOLE: Eliminating DAA intrinsic due to a memory reference "
 ...
 * runtime performance.
 */
bool TR_S390Peephole::inlineEXtargetHelper(TR::Instruction* inst, TR::Instruction* instrB4EX)
{
...
void TR_S390Peephole::markBlockThatModifiesRegister(TR::Instruction* cursor, TR::Register* targetReg, int32_t blockNum)
{
...
void TR_S390Peephole::reloadLiteralPoolRegisterForCatchBlock()
{
    ...
    if (TR::Compiler->target.isZOS() && !isZ10 && !_cg->isLiteralPoolOnDemandOn()) {
        ...
        if ((_cg->getLinkage())->setupLiteralPoolRegister(firstSnippet) > 0) {
            ...
            // phase
            TR::S390RILInstruction* inst = (TR::S390RILInstruction*)generateRILInstruction(
                _cg, TR::InstOpCode::LARL, _cursor->getNode(), _cg->getLitPoolRealRegister(), 0xBABE, _cursor);
...
void TR_S390Peephole::setupSpecializedEpilogues()
{
    _cg->setSpecializedEpilogues(
        comp()->getOption(TR_EnableSpecializedEpilogues) && comp()->getMethodHotness() > noOpt);
...
void TR_S390Peephole::perform()
{
    ...
    while (_cursor != NULL) {
        ...
        if (!deferMarkingRegsUntilAfterTailCall && _cursor->getOpCodeValue() != TR::InstOpCode::FENCE
        ...
        case TR::InstOpCode::LHI: {
            ...
            // this transformation
            // is functionally valid or not. The issue resides in the various runtime patching sequences using the LHI
            // instruction as a
            // runtime patch point for an offset. One concrete example can be found in the virtual dispatch sequence for
            // unresolved calls
            ...
            if (_cursor->getNext()->getOpCodeValue() == _cursor->getOpCodeValue()) {
                ...
                if ((na->getKind() == TR::Instruction::IsRS) && (nb->getKind() == TR::Instruction::IsRS)) {
                    ...
                    bool instrMatch = (cast_na->getFirstRegister() == cast_nb->getFirstRegister())
                        && (!cast_na->hasMask()) && (!cast_nb->hasMask()) && (cast_na->getMemoryReference() == NULL)
...
void OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(
    TR_S390ConstantDataSnippet* ifcSnippet, TR::list<TR_OpaqueClassBlock*>* PICSlist)
{
...
TR::list<TR_OpaqueClassBlock*>* OMR::Z::CodeGenerator::getPICsListForInterfaceSnippet(
    TR_S390ConstantDataSnippet* ifcSnippet)
{
    if (_interfaceSnippetToPICsListHashTab == NULL)
        return NULL;
...
void OMR::Z::CodeGenerator::markBlockThatModifiesRegister(TR::RealRegister::RegNum reg, int32_t blockNum)
{
...
////////////////////////////////////////////////////////////////////////////////
bool OMR::Z::CodeGenerator::restoreRegister(TR::RealRegister::RegNum reg, int32_t blockNumber)
{
...
{
    _reachingBlocks = new (self()->comp()->allocator())
        TR_ReachingBlocks(self()->comp(), (TR::Optimizer*)self()->comp()->getOptimizer());
...
void OMR::Z::CodeGenerator::doBinaryEncoding()
{
    ...
    while (data.cursorInstruction) {
        ...
        //
        if (data.cursorInstruction->isLastWarmInstruction() && !self()->comp()->getOption(TR_AOT)
    ...
    if (!self()->allowSplitWarmAndColdBlocks()) {
        ...
        TR_ASSERT(self()->getEstimatedWarmLength() == 0,
    ...
    if (doBinaryEncoding) {
        ...
        TR_HashTab* labelHashTable
            = new (self()->trStackMemory()) TR_HashTab(self()->comp()->trMemory(), stackAlloc, 60, true);
        TR_HashTab* notPrintLabelHashTable
            = new (self()->trStackMemory()) TR_HashTab(self()->comp()->trMemory(), stackAlloc, 60, true);
        ...
        while (data.cursorInstruction) {
            ...
            if (self()->comp()->cg()->isBranchInstruction(data.cursorInstruction)) {
                TR::LabelSymbol* branchLabelSymbol
                    = ((TR::S390BranchInstruction*)data.cursorInstruction)->getLabelSymbol();
            ...
            TR_ASSERT(data.cursorInstruction->getEstimatedBinaryLength()
                ...
                self()->getDebug() ? self()->getDebug()->getOpCodeName(&data.cursorInstruction->getOpCode())
                                   : "(unknown)",
            ...
            //
            if (data.cursorInstruction->isLastWarmInstruction() && !self()->comp()->getOption(TR_AOT)
                && self()->allowSplitWarmAndColdBlocks() && self()->comp()->getOption(TR_EnableTieredCodeCache)) {
                ...
                self()->addAccumulatedInstructionLengthError(
                    self()->getWarmCodeEnd() - coldCode + MIN_DISTANCE_BETWEEN_WARM_AND_COLD_CODE);
            }
        ...
        for (TR::LabelSymbol* labelSymbol = (TR::LabelSymbol*)lit.getFirst(); labelSymbol;
             labelSymbol = (TR::LabelSymbol*)lit.getNext()) {
            TR_HashId hashIndex = 0;
    ...
    } else { // sets values for warmCodeEnd and coldCodeStart to avoid failing later assertions
...
TR::RegisterPair* OMR::Z::CodeGenerator::allocateArGprPair(TR::Register* lowRegister, TR::Register* highRegister)
{
    TR::RegisterPair* regPair = NULL;
    TR_ASSERT(
        (highRegister->getKind() == TR_GPR64 || highRegister->getKind() == TR_GPR) && lowRegister->getKind() == TR_AR,
    ...
    if (node) {
        ...
        if (node->getOpaquePseudoRegister()) {
            ...
            if (self()->traceBCDCodeGen())
                traceMsg(self()->comp(), "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg %s - "
        ...
        if (node->safeToDoRecursiveDecrement()) {
            ...
            if (node->getOpaquePseudoRegister() && !alreadyProcessedUnusedStorageRef) {
                ...
                TR_StorageReference* ref = reg->getStorageReference();
                if (self()->traceBCDCodeGen())
                    traceMsg(self()->comp(), "\tprocessUnusedNodeDuringEvaluation : bcd/aggr const/ixload %s (%p) reg "
    ...
    int8_t linkageRegisterIndex, TR::DataType type)
{
    ...
    if (self()->getS390Linkage()->isCAASpecialArgumentRegister(linkageRegisterIndex)) {
        ...
        //      traceMsg(TR::comp(), "getS390Linkage()->getIntegerArgumentRegister(linkageRegisterIndex) =
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfAssignableGPRs()
{
    ...
    maxGPRs = 11 + (self()->isLiteralPoolOnDemandOn() ? 1 : 0) // => Litpool ptr is available
        + ((self()->getExtCodeBaseRegisterIsFree() && !self()->comp()->getOption(TR_DisableLongDispStackSlot))
                      ? 1
...
int32_t OMR::Z::CodeGenerator::getMaximumNumberOfGPRsAllowedAcrossEdge(TR::Node* node)
{
    ...
    if (node->getOpCode().isSwitch()) {
        ...
        if (node->getSecondChild()->getOpCode().isLoadConst()) {
            ...
            if (self()->comp()->cg()->getS390ProcessorInfo()->supportsArch(TR_S390ProcessorInfo::TR_zEC12)
...
TR::Register* OMR::Z::CodeGenerator::gprClobberEvaluate(TR::Node* node, bool force_copy, bool ignoreRefCount)
{
    ...
    if (!self()->canClobberNodesRegister(node, 1, &data, ignoreRefCount) || force_copy) {
        ...
        if ((node->getType().isInt64() || TR::Compiler->target.is64Bit()) && TR::Compiler->target.is32Bit()) {
            ...
            cursor
                = generateRRInstruction(self(), loadRegOpCode, node, targetRegister->getGPRofArGprPair(), srcRegister);
    ...
    TR::Node* node, TR::MemoryReference* sourceMR) // sourceMR can be NULL
{
    ...
    if (needsClobberDueToRefCount || needsClobberDueToRegCount || needsClobberDueToReuse) {
        ...
        if (self()->traceBCDCodeGen())
            traceMsg(self()->comp(), "ssrClobberEvaluate: node %s (%p) with srcReg %s (srcRegSize %d, "
                                     "srcReg->validSymSize %d, srcReg->liveSymSize %d) and srcStorageReference #%d "
                                     "with %s (symSize %d)\n",
        ...
        if (srcStorageReference->isReadOnlyTemporary() && srcRegister->getRightAlignedIgnoredBytes() > 0) {
            ...
            // bytes are not included as these have been invalidated/clobbered).
            // The subsequent uses of this copied field (via copyStorageReference) will adjust based on the owning
        ...
        if (self()->traceBCDCodeGen() && alreadyClearedDigits) {
            traceMsg(self()->comp(), "\tsrcRegister %s had leftAlignedZeroDigits %d on original ref #%d\n",
                ...
                srcStorageReference->getReferenceNumber());
            traceMsg(self()->comp(), "\t\t* setting surviving leftAlignedZeroDigits %d on srcRegister %s with new copy "
                                     "ref #%d (digitsInSrc - digitsToClear = %d - %d = %d)\n",
        ...
        if (srcStorageReference->getNodesToUpdateOnClobber()) {
            ...
            //       2 (pdaddA nodeRefCount)
            //       nop         mark t1 as readOnly (redundant), add pdcleanB (w/regB) to list (pdcleanB), increment t1
            //       refCount by 3 (pdcleanA nodeRefCount)
            ...
            //       and update regA with t4 (on clobber eval for pdaddB)
            //       AP    t1,tx
            //    ...
            ...
            for (TR::Node* listNode = listIt.getFirst(); listNode; listNode = listIt.getNext()) {
                ...
                if (listReg != srcRegister) {
                    if (listReg->getStorageReference() != srcStorageReference) {
                        ...
                        // skipCopyOnStore/changeCommonedChildAddress in pdstoreEvaluator
                        // so skip this update (the reference counts of the address subtree will be wrong if the
                        // refCount is update here.
                        ...
                        //
                        if (self()->comp()->cg()->traceBCDCodeGen())
                            traceMsg(self()->comp(), "\ty^y : skip update on reg %s node %s (%p), storageRef has "
                        ...
                        if (self()->comp()->cg()->traceBCDCodeGen())
                            traceMsg(self()->comp(), "\ty^y : update reg %s with ref #%d (%s) on node %s (%p) refCount "
                                                     "%d from _nodesToUpdateOnClobber with copy ref #%d (%s) (skip if "
                                                     "refCount == 0)\n",
...
bool OMR::Z::CodeGenerator::isMemcpyWithPadIfFoldable(TR::Node* node, TR_MemCpyPadTypes type)
{
    ...
    {
        return true;
    } else if (self()->inlineNDmemcpyWithPad(node) && equalNode->getOpCode().isLoadConst()
...
void OMR::Z::CodeGenerator::apply12BitLabelRelativeRelocation(int32_t* cursor, TR::LabelSymbol* label, bool isCheckDisp)
{
    ...
    if (s != NULL && (s->getKind() == TR::Snippet::IsConstantData || s->getKind() == TR::Snippet::IsWritableData
 ...
 * supports instructions with 16-bit relocations,
...
// OMR::Z::CodeGenerator::Constant Data List Functions
////////////////////////////////////////////////////////////////////////////////
TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreateConstant(
    TR::Node* node, void* c, uint16_t size, bool isWarm)
{
    // *this    swipeable for debugging purposes
    CS2::HashIndex hi;
...
TR_S390ConstantInstructionSnippet* OMR::Z::CodeGenerator::createConstantInstruction(
    TR::CodeGenerator* cg, TR::Node* node, TR::Instruction* instr)
{
    TR_S390ConstantInstructionSnippet* cis
        = new (cg->trHeapMemory()) TR_S390ConstantInstructionSnippet(cg, node, instr);
    _constantList.push_front(cis);
    return cis;
}

TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::CreateConstant(TR::Node* node, void* c, uint16_t size, bool writable)
{
    // *this    swipeable for debugging purposes
...
TR_S390LabelTableSnippet* OMR::Z::CodeGenerator::createLabelTable(TR::Node* node, int32_t size)
{
    TR_S390LabelTableSnippet* labelTableSnippet
        = new (self()->trHeapMemory()) TR_S390LabelTableSnippet(self(), node, size);
    _snippetDataList.push_front(labelTableSnippet);
    return labelTableSnippet;
}

void OMR::Z::CodeGenerator::addDataConstantSnippet(TR_S390ConstantDataSnippet* snippet)
{
 ...
 */
int32_t OMR::Z::CodeGenerator::setEstimatedOffsetForConstantDataSnippets(int32_t targetAddressSnippetSize, bool isWarm)
{
    // *this    swipeable for debugging purposes
    TR_S390ConstantDataSnippet* cursor;
    bool first;
...
int32_t OMR::Z::CodeGenerator::setEstimatedLocationsForDataSnippetLabels(int32_t estimatedSnippetStart, bool isWarm)
{
    // *this    swipeable for debugging purposes
    TR_S390ConstantDataSnippet* cursor;
    bool first;
...
TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::create64BitLiteralPoolSnippet(TR::DataType dt, int64_t value)
{
...
TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::createLiteralPoolSnippet(TR::Node* node)
{
...
TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate2ByteConstant(TR::Node* node, int16_t c, bool isWarm)
{
    return self()->findOrCreateConstant(node, &c, 2, isWarm);
}

TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate4ByteConstant(TR::Node* node, int32_t c, bool isWarm)
{
    return self()->findOrCreateConstant(node, &c, 4, isWarm);
}

TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::findOrCreate8ByteConstant(TR::Node* node, int64_t c, bool isWarm)
{
    return self()->findOrCreateConstant(node, &c, 8, isWarm);
}

TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::Create4ByteConstant(TR::Node* node, int32_t c, bool writable)
{
    return self()->CreateConstant(node, &c, 4, writable);
}

TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::Create8ByteConstant(TR::Node* node, int64_t c, bool writable)
{
    return self()->CreateConstant(node, &c, 8, writable);
}

TR_S390WritableDataSnippet* OMR::Z::CodeGenerator::CreateWritableConstant(TR::Node* node)
{
    if (TR::Compiler->target.is64Bit()) {
...
TR_S390ConstantDataSnippet* OMR::Z::CodeGenerator::getFirstConstantData()
{
    ...
    for (exp = self()->constantDataSnippetExponent(); exp > 0; exp--) {
        ...
        for (auto iterator = _constantList.begin(); iterator != _constantList.end(); ++iterator) {
            if (HANDLE_CONSTANT_SNIPPET(
                    (*iterator), false, size)) // false matches case where !(*iterator)->isWarmSnippet()
            {
                return *iterator;
            }
        }
    ...
    for (exp = self()->constantDataSnippetExponent(); exp > 0; exp--) {
        ...
        for (auto writeableiterator = _writableList.begin(); writeableiterator != _writableList.end();
            ...
            if (HANDLE_CONSTANT_SNIPPET((*writeableiterator), false,
                    size)) // false matches case where !(*writeableiterator)->isWarmSnippet()
            {
...
////////////////////////////////////////////////////////////////////////////////
int32_t OMR::Z::CodeGenerator::setEstimatedLocationsForTargetAddressSnippetLabels(
...
TR_S390LookupSwitchSnippet* OMR::Z::CodeGenerator::CreateLookupSwitchSnippet(TR::Node* node, TR::Snippet* s)
{
    _targetList.push_front((TR_S390LookupSwitchSnippet*)s);
    return (TR_S390LookupSwitchSnippet*)s;
}

TR_S390TargetAddressSnippet* OMR::Z::CodeGenerator::CreateTargetAddressSnippet(TR::Node* node, TR::Snippet* s)
{
...
TR_S390TargetAddressSnippet* OMR::Z::CodeGenerator::CreateTargetAddressSnippet(TR::Node* node, TR::LabelSymbol* s)
{
    TR_S390TargetAddressSnippet* targetsnippet;
    targetsnippet = new (self()->trHeapMemory()) TR_S390TargetAddressSnippet(self(), node, s);
    _targetList.push_front(targetsnippet);
...
TR_S390TargetAddressSnippet* OMR::Z::CodeGenerator::CreateTargetAddressSnippet(TR::Node* node, TR::Symbol* s)
{
...
TR_S390TargetAddressSnippet* OMR::Z::CodeGenerator::findOrCreateTargetAddressSnippet(TR::Node* node, uintptrj_t addr)
{
    TR_ASSERT(self()->supportsOnDemandLiteralPool() == false, "May not be here with Literal Pool On Demand enabled\n");
...
TR_S390TargetAddressSnippet* OMR::Z::CodeGenerator::getFirstTargetAddress()
{
    if (_targetList.empty())
        return NULL;
    else
...
//
bool OMR::Z::CodeGenerator::processInstruction(
    TR::Instruction* instr, TR_BitVector** registerUsageInfo, int32_t& blockNum, int32_t& isFence, bool traceIt)
{
        ...
        if (callNode->getSymbolReference()) {
         ...
         if (symRef->getSymbol() && symRef->getSymbol()->isMethod())
            ...
            if (symRef->getSymbol()->isMethod()) {
                ...
                if ((m->isHelper() && !m->preservesAllRegisters()) || m->isVMInternalNative()
                    || m->isJITInternalNative() || m->isJNI() || m->isSystemLinkageDispatch()
    ...
    case TR::Instruction::IsReg: {
        ...
        if (regPair) {
            ...
            if (traceIt)
                traceMsg(
                    self()->comp(), "reg instr [%p] USES register pair [%d, %d]\n", s390Instr, tgtLowNum, tgtHighNum);
    ...
    case TR::Instruction::IsRR: {
        if (((TR::S390RRInstruction*)s390Instr)->getFirstConstant() >= 0)
            ...
            if (regPair) {
                ...
                if (traceIt)
                    traceMsg(
                        self()->comp(), "RR instr [%p] USES register pair [%d %d]\n", s390Instr, tgtLowNum, tgtHighNum);
                registerUsageInfo[blockNum]->set(tgtHighNum);
                registerUsageInfo[blockNum]->set(tgtLowNum);
            } else {
                int32_t tgtRegNum = toRealRegister(targetRegister)->getRegisterNumber();
    ...
    case TR::Instruction::IsRRE: {
        ...
        if (regPair) {
            ...
            if (traceIt)
                traceMsg(
                    self()->comp(), "RRE instr [%p] USES register pair [%d %d]\n", s390Instr, tgtLowNum, tgtHighNum);
            registerUsageInfo[blockNum]->set(tgtHighNum);
            registerUsageInfo[blockNum]->set(tgtLowNum);
        } else {
            int32_t tgtRegNum = toRealRegister(targetRegister)->getRegisterNumber();
    ...
    case TR::Instruction::IsRXY: {
        if (s390Instr->getRegisterOperand(1)->getRegisterPair()) {
            ...
            if (traceIt)
                traceMsg(
                    self()->comp(), "RS instr [%p] USES register pair [%d %d]\n", s390Instr, tgtLowNum, tgtHighNum);
        } else {
            int32_t tgtRegNum = toRealRegister(s390Instr->getRegisterOperand(1))->getRegisterNumber();
            registerUsageInfo[blockNum]->set(tgtRegNum);
            if (traceIt)
...
uint32_t OMR::Z::CodeGenerator::isPreservedRegister(int32_t regIndex)
{
    ...
    //
    for (int32_t pindex = TR::RealRegister::GPR6; pindex <= TR::RealRegister::GPR12; pindex++) {
...
bool OMR::Z::CodeGenerator::isBranchInstruction(TR::Instruction* instr) { return instr->isBranchOp(); }

bool OMR::Z::CodeGenerator::isLabelInstruction(TR::Instruction* instr) { return instr->isLabel(); }
...
bool OMR::Z::CodeGenerator::isAlignmentInstruction(TR::Instruction* instr) { return false; }

TR::Instruction* OMR::Z::CodeGenerator::splitEdge(TR::Instruction* instr, bool isFallThrough, bool needsJump,
    TR::Instruction* newSplitLabel, TR::list<TR::Instruction*>* jmpInstrs, bool firstJump)
{
    ...
    if (!isFallThrough && needsJump) {
        ...
        TR::Instruction* i = generateS390BranchInstruction(
            self(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, jmpLocation->getNode(), targetLabel, jmpLocation);
        traceMsg(self()->comp(), "split edge jmp instr at [%p]\n", i);
    }
    return cursor;
}

TR::Instruction* OMR::Z::CodeGenerator::splitBlockEntry(TR::Instruction* instr)
{
...
int32_t OMR::Z::CodeGenerator::computeRegisterSaveDescription(TR_BitVector* regs, bool populateInfo)
{
    uint32_t rsd = 0;
...
void OMR::Z::CodeGenerator::processIncomingParameterUsage(TR_BitVector** registerUsageInfo, int32_t blockNum)
{
    ...
    int32_t litPoolNumber
        = self()->getLinkage(bodySymbol->getLinkageConvention())->setupLiteralPoolRegister(self()->getFirstSnippet());
    if (litPoolNumber > 0)
        registerUsageInfo[blockNum]->set(litPoolNumber);
}

void OMR::Z::CodeGenerator::updateSnippetMapWithRSD(TR::Instruction* instr, int32_t rsd)
{
...
bool OMR::Z::CodeGenerator::isTargetSnippetOrOutOfLine(
...
////////////////////////////////////////////////////////////////////////////////
void OMR::Z::CodeGenerator::dumpTargetAddressSnippets(TR::FILE* outFile, bool isWarm)
{
    // *this    swipeable for debugging purposes
    ...
    if (self()->hasCCInfo() && self()->hasCCCompare()) {
        ...
        if (opcd == ccInst->getOpCodeValue() && tReg == ccTgtReg && sReg == ccSrcReg
            && performTransformation(self()->comp(),
                   ...
                   self()->getDebug()->getOpCodeName(&ccInst->getOpCode()), self()->getDebug()->getName(tReg),
...
TR::RegisterDependencyConditions* OMR::Z::CodeGenerator::addVMThreadPreCondition(
    TR::RegisterDependencyConditions* deps, TR::Register* reg)
{
    if (self()->needsVMThreadDependency()) {
        ...
        deps->addPreConditionIfNotAlreadyInserted(
            reg, (TR::RealRegister::RegNum)self()->getVMThreadRegister()->getAssociation());
    }
    return deps;
}

TR::RegisterDependencyConditions* OMR::Z::CodeGenerator::addVMThreadPostCondition(
    TR::RegisterDependencyConditions* deps, TR::Register* reg)
{
    if (self()->needsVMThreadDependency()) {
        // TODO This IF statement is for codegen level vm thread work, where we
...
TR::RegisterDependencyConditions* OMR::Z::CodeGenerator::addVMThreadDependencies(
    TR::RegisterDependencyConditions* deps, TR::Register* reg)
{
    ...
    TR_RegisterCandidate* rc, TR::Block** blocks, TR_BitVector& availableRegisters)
{
    ...
    while (loe.hasMoreElements()) {
        ...
        if (lastTreeTopNode->getNumChildren() > 0 && lastTreeTopNode->getFirstChild()->getOpCode().isFunctionCall()
            && lastTreeTopNode->getFirstChild()->getOpCode().isJumpWithMultipleTargets()) // ensure calls that have
                                                                                          // multiple targets remove all
                                                                                          // volatile registers from
                                                                                          // available registers
        {
...
int32_t OMR::Z::CodeGenerator::biasDecimalFloatFrac(TR::DataType dt, int32_t frac)
{
    switch (dt) {
#ifdef J9_PROJECT_SPECIFIC
    case TR::DecimalFloat:
...
#endif
    default:
        TR_ASSERT(0, "unexpected biasDecimalFloatFrac dt %d", dt);
    }
    return 0;
}
...
bool OMR::Z::CodeGenerator::nodeRequiresATemporary(TR::Node* node)
{
    ...
    if (node->getOpCode().isPackedRightShift()) {
        ...
        // These dead and implied bytes cannot be within a user variable as the actual full value must be materialized
        // therefore require a
...
bool OMR::Z::CodeGenerator::endAccumulatorSearchOnOperation(TR::Node* node)
{
    ...
    if (self()->comp()->getOption(TR_PrivatizeOverlaps)) {
        ...
        if (node->getRegister() == NULL && endHint) {
            ...
            // common)
            // cases where a node will initialize to a new storageReference (endHintOnOperation check) and one of the
            // conditions below is true
            // 1) a conversion between types that use storageRefs (BCD and Aggr) and types that don't (e.g. int,float)
...
void handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    if (!(opCodeToUse == TR::InstOpCode::LM || opCodeToUse == TR::InstOpCode::LMG || opCodeToUse == TR::InstOpCode::LMH
            || opCodeToUse == TR::InstOpCode::LAM || opCodeToUse == TR::InstOpCode::LMY
    ...
    for (uint32_t i = lowRegNum; i != highRegNum; i = ((i == (isVector ? numVRFs - 1 : 15)) ? 0 : i + 1)) {
        TR::RealRegister* reg
            = cg->machine()->getS390RealRegister(i + (isAR ? TR::RealRegister::AR0 : TR::RealRegister::GPR0));
    ...
    TR::Instruction* cursor, TR::DebugCounterBase* counter, int32_t delta, TR::RegisterDependencyConditions* cond)
{
    ...
    TR::Instruction* loadInst
        = generateRILInstruction(self(), TR::InstOpCode::LGRL, cursor->getNode(), addressReg, constant, cursor);
    TR::Instruction* adddInst = generateSIYInstruction(self(), TR::InstOpCode::AGSI, cursor->getNode(),
        generateS390MemoryReference(addressReg, 0, self()), delta, loadInst);
...
bool OMR::Z::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
...
void OMR::Z::CodeGenerator::setRAPassAR() { TR_ASSERT(0, "ASC Mode AR not supported"); }

void OMR::Z::CodeGenerator::resetRAPassAR() { TR_ASSERT(0, "ASC Mode AR not supported"); }
...
bool OMR::Z::CodeGenerator::canOptimizeEdit(uint8_t* parm, int32_t length)
{
    ...
    return self()->parseEditParm(
        parm, length, leadingSigs, trailingStr, fill, numDigits, numSigs, sigOff, lastDigit, digitsAfterSig);
}

bool OMR::Z::CodeGenerator::parseEditParm(uint8_t* parm, int32_t length, bool& leadingSigs, bool& trailingStr,
    uint8_t& fill, int32_t& numDigits, int32_t& numSigs, int32_t& sigOff, int32_t& lastDigit, int32_t& digitsAfterSig)
{
...
bool OMR::Z::CodeGenerator::replaceWithDestructiveFormIfPossible(TR::Instruction* inst)
{
    ...
    if (replacementOpCodeValue != TR::InstOpCode::BAD
        && (targetReg = inst->getRegisterOperand(1)) == inst->getRegisterOperand(2)
        && performTransformation(self()->comp(),
               "\nO^O S390 PEEPHOLE rewrite: change %s to %s because source and target operands are the same in [%p]\n",
               TR::InstOpCode::opCodeToNameMap[inst->getOpCodeValue()],
        ...
        case RR_FORMAT:
            generateRRInstruction(
                self(), replacementOpCodeValue, inst->getNode(), targetReg, inst->getRegisterOperand(3), inst);
            break;
        case RRE_FORMAT:
            ...
            break;
        case RI_FORMAT:
            generateRIInstruction(self(), replacementOpCodeValue, inst->getNode(), targetReg,
                toS390RIEInstruction(inst)->getSourceImmediate16(), inst);
            break;
        case RS_FORMAT: {
            ...
            if (oldMR) {
                ...
                generateRSInstruction(self(), replacementOpCodeValue, inst->getNode(), targetReg,
                    toS390RSInstruction(inst)->getSourceImmediate(), inst);
            }
            break;
        }
        default:
...
// from the source memRef
// Then shortly after, when a hint is used, the address child is evaluated to form the dest memRef. When the parent of
// the pdX in the address child
// (the pdadd here) has an already initialized child then it does a clobber evaluate.
// The clobber evaluate will update the storageRef on the pdX register to a new temp and then clobber the storageRef
// already on the pdX
// Normally this is ok as future references to the pdX node will use the new temp BUT in this case the pd2zd in the
// value child has already used the
...
bool OMR::Z::CodeGenerator::loadOrStoreAddressesMatch(TR::Node* node1, TR::Node* node2)
{
    ...
    if (node1->getSize() != node2->getSize()) {
        if (self()->comp()->getOption(TR_TraceVIP))
            traceMsg(self()->comp(), "\t\tloadOrStoreAddressesMatch = false (sizes differ) : node1 %s (%p) size = %d "
...
void OMR::Z::CodeGenerator::setBranchOnCountFlag(TR::Node* node, vcount_t visitCount)
{
    ...
    // compare, and in the
    // GLRegDeps under the compare. futureUseCount should've been initialized to be referenceCount - 1 (which will have
    // to be an iRegStore in order
    // for us to see the add node under GLRegDeps here). The compare node here will account for the other two uses, so
    // as long as there's no other
