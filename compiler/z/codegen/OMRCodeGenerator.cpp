    ...
    if (node->isUnneededIALoad()) {
        if (parent->getOpCodeValue() == TR::ifacmpne || parent->getOpCodeValue() == TR::ificmpeq || parent->getOpCodeValue() == TR::ificmpne || parent->getOpCodeValue() == TR::ifacmpeq) {
            if (!parent->isNopableInlineGuard() || !self()->getSupportsVirtualGuardNOPing()) {
                ...
                if (!parent->isHCRGuard() && !self()->comp()->performVirtualGuardNOPing() && self()->comp()->isVirtualGuardNOPingRequired(virtualGuard) && virtualGuard->canBeRemoved()) {
                    node->setUnneededIALoad(false);
                }
            }
...
bool OMR::Z::CodeGenerator::isStackBased(TR::MemoryReference* mr)
{
    ...
    {
        return (toRealRegister(mr->getBaseRegister()->getGPRofArGprPair()->getRealRegister())->getRegisterNumber() == self()->getStackPointerRealRegister()->getRegisterNumber());
    }
    return false;
}
...
{
    while (inst && (inst->getKind() == TR::Instruction::IsPseudo && !inst->isAsmGen() || inst->getKind() == TR::Instruction::IsNotExtended)) {
        inst = forward ? inst->getNext() : inst->getPrev();
    }
...
{
    while (inst && (inst->getKind() == TR::Instruction::IsPseudo && !inst->isAsmGen() || inst->getKind() == TR::Instruction::IsNotExtended) && !inst->isRet()) {
        inst = forward ? inst->getNext() : inst->getPrev();
    }
...
bool TR_S390Peephole::LAReduction()
{
    ...
    if (s390Instr->getKind() == TR::Instruction::IsRX) {
        ...
        if (mr && mr->isBucketBaseRegMemRef() && mr->getOffset() == 0 && laBaseReg && laTargetReg && laBaseReg == laTargetReg && laIndexReg == NULL && (symRef == NULL || symRef->getOffset() == 0) && (symRef == NULL || symRef->getSymbol() == NULL)) {
            // remove LA Rx,0(Rx)
            if (comp()->getOption(TR_TraceCG)) {
                printInfo("\n");
            }
...
bool TR_S390Peephole::unnecessaryNILHReduction()
{
    return false;
}
...
bool TR_S390Peephole::branchReduction()
{
    return false;
}
...
{
    _cg->setSpecializedEpilogues(comp()->getOption(TR_EnableSpecializedEpilogues) && comp()->getMethodHotness() > noOpt);
...
bool TR_S390Peephole::DeadStoreToSpillReduction()
{
    return false;
}

bool TR_S390Peephole::tryMoveImmediate()
{
    return false;
}
...
void TR_S390Peephole::perform()
{
    ...
    while (_cursor != NULL) {
        ...
        if (specializedEpilogues) {
            ...
            if (comp()->getOption(TR_EnableTailCallOpt) && !catchBlock && (_cursor->getOpCodeValue() == TR::InstOpCode::BRASL || _cursor->getOpCodeValue() == TR::InstOpCode::BRAS || _cursor->getOpCodeValue() == TR::InstOpCode::BASR))
                deferMarkingRegsUntilAfterTailCall = true;
        }

        if (!deferMarkingRegsUntilAfterTailCall && _cursor->getOpCodeValue() != TR::InstOpCode::FENCE && _cursor->getOpCodeValue() != TR::InstOpCode::ASSOCREGS && _cursor->getOpCodeValue() != TR::InstOpCode::DEPEND) {
        ...
        case TR::InstOpCode::SLL: {
            ...
            if (_cursor->getNext()->getOpCodeValue() == _cursor->getOpCodeValue()) {
                ...
                if ((na->getKind() == TR::Instruction::IsRS) && (nb->getKind() == TR::Instruction::IsRS)) {
                    ...
                    bool instrMatch = (cast_na->getFirstRegister() == cast_nb->getFirstRegister()) && (!cast_na->hasMask()) && (!cast_nb->hasMask()) && (cast_na->getMemoryReference() == NULL) && (cast_nb->getMemoryReference() == NULL);
...
void OMR::Z::CodeGenerator::addPICsListForInterfaceSnippet(TR_S390ConstantDataSnippet* ifcSnippet, TR::list<TR_OpaqueClassBlock*>* PICSlist)
{
...
bool OMR::Z::CodeGenerator::anyNonConstantSnippets()
{
    ...
    for (auto iterator = _snippetList.begin(); iterator != _snippetList.end(); ++iterator) {
        if ((*iterator)->getKind() != TR::Snippet::IsConstantData || (*iterator)->getKind() != TR::Snippet::IsWritableData || (*iterator)->getKind() != TR::Snippet::IsEyeCatcherData || (*iterator)->getKind() != TR::Snippet::IsTargetAddress || (*iterator)->getKind() != TR::Snippet::IsLookupSwitch) {
            return true;
        }
    }
    return false;
...
{
    return (!self()->comp()->getOptions()->getOption(TR_DisableUTF16BEEncoder) || (self()->getSupportsVectorRegisters() && !self()->comp()->getOptions()->getOption(TR_DisableSIMDUTF16BEEncoder)));
}
...
{
    TR::RegisterPair* regPair = NULL;
    TR_ASSERT((highRegister->getKind() == TR_GPR64 || highRegister->getKind() == TR_GPR) && lowRegister->getKind() == TR_AR, "expecting AR<->GPR pair\n");
 ...
 */
void OMR::Z::CodeGenerator::splitBaseRegisterPairsForRRMemoryInstructions(TR::Node* node, TR::RegisterPair* sourceReg, TR::RegisterPair* targetReg)
{
...
/////////////////////////////////////////////////////////////////////////////////
//  Rematerialization
bool OMR::Z::CodeGenerator::materializesLargeConstants()
{
    return false;
}
...
OMR::Z::CodeGenerator::getMaximumNumberOfAssignableGPRs()
{
    ...
    maxGPRs = 11 + (self()->isLiteralPoolOnDemandOn() ? 1 : 0) // => Litpool ptr is available
        + ((self()->getExtCodeBaseRegisterIsFree() && !self()->comp()->getOption(TR_DisableLongDispStackSlot)) ? 1 : 0) // => ExtCodeBase is free
...
bool OMR::Z::CodeGenerator::isMemcpyWithPadIfFoldable(TR::Node* node, TR_MemCpyPadTypes type)
{
    ...
    {
        return true;
    } else if (self()->inlineNDmemcpyWithPad(node) && equalNode->getOpCode().isLoadConst() && equalNode->get64bitIntegralValue() == 1) {
        ...
        if (callNode->getSymbolReference()) {
         ...
         if (symRef->getSymbol() && symRef->getSymbol()->isMethod())
            ...
            if (symRef->getSymbol()->isMethod()) {
                ...
                if ((m->isHelper() && !m->preservesAllRegisters()) || m->isVMInternalNative() || m->isJITInternalNative() || m->isJNI() || m->isSystemLinkageDispatch() || (m->getLinkageConvention() == TR_System))
...
bool OMR::Z::CodeGenerator::isTargetSnippetOrOutOfLine(TR::Instruction* instr, TR::Instruction** start, TR::Instruction** end)
{
    TR::S390LabelInstruction* labelInstr = (TR::S390LabelInstruction*)instr;
    TR::LabelSymbol* targetLabel = labelInstr->getLabelSymbol();
    TR_S390OutOfLineCodeSection* oiCursor = self()->findOutLinedInstructionsFromLabel(targetLabel);
...
void handleLoadWithRegRanges(TR::Instruction* inst, TR::CodeGenerator* cg)
{
    ...
    if (!(opCodeToUse == TR::InstOpCode::LM || opCodeToUse == TR::InstOpCode::LMG || opCodeToUse == TR::InstOpCode::LMH || opCodeToUse == TR::InstOpCode::LAM || opCodeToUse == TR::InstOpCode::LMY || opCodeToUse == TR::InstOpCode::VLM))
        return;
...
bool OMR::Z::CodeGenerator::suppressInliningOfRecognizedMethod(TR::RecognizedMethod method)
{
    ...
    if (!self()->comp()->compileRelocatableCode() && !self()->comp()->getOption(TR_DisableDFP) && TR::Compiler->target.cpu.getS390SupportsDFP()) {
        if (method == TR::java_math_BigDecimal_DFPIntConstructor || method == TR::java_math_BigDecimal_DFPLongConstructor || method == TR::java_math_BigDecimal_DFPLongExpConstructor || method == TR::java_math_BigDecimal_DFPAdd || method == TR::java_math_BigDecimal_DFPSubtract || method == TR::java_math_BigDecimal_DFPMultiply || method == TR::java_math_BigDecimal_DFPDivide || method == TR::java_math_BigDecimal_DFPScaledAdd || method == TR::java_math_BigDecimal_DFPScaledSubtract || method == TR::java_math_BigDecimal_DFPScaledMultiply || method == TR::java_math_BigDecimal_DFPScaledDivide || method == TR::java_math_BigDecimal_DFPRound || method == TR::java_math_BigDecimal_DFPSetScale || method == TR::java_math_BigDecimal_DFPCompareTo || method == TR::java_math_BigDecimal_DFPSignificance || method == TR::java_math_BigDecimal_DFPExponent || method == TR::java_math_BigDecimal_DFPBCDDigits || method == TR::java_math_BigDecimal_DFPUnscaledValue || method == TR::java_math_BigDecimal_DFPConvertPackedToDFP || method == TR::java_math_BigDecimal_DFPConvertDFPToPacked) {
            return true;
        }

        if (method == TR::com_ibm_dataaccess_DecimalData_DFPConvertPackedToDFP || method == TR::com_ibm_dataaccess_DecimalData_DFPConvertDFPToPacked) {
            return true;
        }
    }

    if (method == TR::java_lang_Integer_highestOneBit || method == TR::java_lang_Integer_numberOfLeadingZeros || method == TR::java_lang_Integer_numberOfTrailingZeros || method == TR::java_lang_Long_highestOneBit || method == TR::java_lang_Long_numberOfLeadingZeros || method == TR::java_lang_Long_numberOfTrailingZeros) {
        return true;
    }

    if (method == TR::java_lang_Long_reverseBytes || method == TR::java_lang_Integer_reverseBytes || method == TR::java_lang_Short_reverseBytes || method == TR::java_util_concurrent_atomic_AtomicBoolean_getAndSet || method == TR::java_util_concurrent_atomic_AtomicInteger_getAndAdd || method == TR::java_util_concurrent_atomic_AtomicInteger_getAndIncrement || method == TR::java_util_concurrent_atomic_AtomicInteger_getAndDecrement || method == TR::java_util_concurrent_atomic_AtomicInteger_getAndSet || method == TR::java_util_concurrent_atomic_AtomicInteger_addAndGet || method == TR::java_util_concurrent_atomic_AtomicInteger_decrementAndGet || method == TR::java_util_concurrent_atomic_AtomicInteger_incrementAndGet || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_incrementAndGet || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_decrementAndGet || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_addAndGet || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndIncrement || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndDecrement || method == TR::java_util_concurrent_atomic_AtomicIntegerFieldUpdater_getAndAdd) {
        return true;
    }

    // Transactional Memory
    if (self()->getSupportsTM()) {
        if (method == TR::java_util_concurrent_ConcurrentHashMap_tmEnabled || method == TR::java_util_concurrent_ConcurrentHashMap_tmPut || method == TR::java_util_concurrent_ConcurrentHashMap_tmRemove || method == TR::java_util_concurrent_ConcurrentLinkedQueue_tmOffer || method == TR::java_util_concurrent_ConcurrentLinkedQueue_tmPoll || method == TR::java_util_concurrent_ConcurrentLinkedQueue_tmEnabled) {
            return true;
        }
    }
#endif
...
bool OMR::Z::CodeGenerator::parseEditParm(uint8_t* parm,
    int32_t length,
    bool& leadingSigs,
    bool& trailingStr,
    uint8_t& fill,
...
bool OMR::Z::CodeGenerator::replaceWithDestructiveFormIfPossible(TR::Instruction* inst)
{
    ...
    if (replacementOpCodeValue != TR::InstOpCode::BAD && (targetReg = inst->getRegisterOperand(1)) == inst->getRegisterOperand(2) && performTransformation(self()->comp(), "\nO^O S390 PEEPHOLE rewrite: change %s to %s because source and target operands are the same in [%p]\n",
