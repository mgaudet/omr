...
#define TR_MAX_NUM_TRANSLATE_QUADS_BY_LOOP                                                                             \
    1 // note: for isInspectConvertingOp operations the length of the from/to strings is the # of 'all' replaces
...
namespace Z {
    ...
    public:
        ...
        {
            if (!(node->getOpCodeValue() == TR::lload || node->getOpCodeValue() == TR::lloadi
         ...
         * transactional lock elision.  On Z, we require 25-30 cycles between transactions, or else the latter
         * transaction will
         * be aborted (with significant penalty).  The 45 is an estimate based on CPI of 1.5-2, and average of 1
        ...
        bool processInstruction(TR::Instruction* instr, TR_BitVector** registerUsageInfo, int32_t& blockNum,
        ...
        TR::Instruction* splitEdge(TR::Instruction* cursor, bool isFallThrough, bool needsJump,
            TR::Instruction* newSplitLabel, TR::list<TR::Instruction*>* jmpInstrs, bool firstJump = false);
        ...
        void setCurrentlyClobberedRestrictedRegister(int32_t regNum) { _currentlyClobberedRestrictedRegister = regNum; }
        ...
        TR_BitVector* getKilledRestrictedRegisters() { return _killedRestrictedRegisters; }
        ...
        TR_BitVector* getCurrentlyRestrictedRegisters() { return _currentlyRestrictedRegisters; }
        ...
        typedef CS2::HashTable<TR_S390ConstantDataSnippetKey, TR_S390ConstantDataSnippet*, TR::Allocator,
            constantHashInfo>
        ...
        TR_S390ConstantInstructionSnippet* createConstantInstruction(
        ...
        bool supportsLongRegAllocation() { return TR::Compiler->target.isZOS() && TR::Compiler->target.is32Bit(); }
        ...
        TR::RegisterDependencyConditions* addVMThreadPreCondition(
            TR::RegisterDependencyConditions* deps, TR::Register* reg);
        TR::RegisterDependencyConditions* addVMThreadPostCondition(
            TR::RegisterDependencyConditions* deps, TR::Register* reg);
        TR::RegisterDependencyConditions* addVMThreadDependencies(
            TR::RegisterDependencyConditions* deps, TR::Register* reg);
        ...
        bool parseEditParm(uint8_t* parm, int32_t length, bool& leadingSigs, bool& trailingStr, uint8_t& fill,
            int32_t& numDigits, int32_t& numSigs, int32_t& sigOff, int32_t& lastDigit, int32_t& digitsAfterSig);
