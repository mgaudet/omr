...
OMR::Z::Instruction::Instruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n)
...
OMR::Z::Instruction::Instruction(
    TR::CodeGenerator* cg, TR::Instruction* precedingInstruction, TR::InstOpCode::Mnemonic op, TR::Node* n)
...
bool OMR::Z::Instruction::getUsedRegisters(TR::list<TR::Register*>& usedRegs)
{
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::BCR && _sourceRegSize == 1
        && _sourceReg[0]->getRealRegister() == machine->getS390RealRegister(TR::RealRegister::GPR0))
        return false;
...
TR::Instruction* OMR::Z::Instruction::getOutOfLineEXInstr()
{
    ...
    case TR::InstOpCode::EXRL: {
        TR_S390ConstantInstructionSnippet* cis
            = (TR_S390ConstantInstructionSnippet*)((TR::S390RILInstruction*)self())->getTargetSnippet();
...
bool OMR::Z::Instruction::containsRegister(TR::Register* reg)
{
    ...
    if (self()->isOutOfLineEX()) {
        if (self()->getOpCodeValue() == TR::InstOpCode::EX) {
            TR::MemoryReference* tempMR = ((TR::S390RXInstruction*)self())->getMemoryReference();
            TR_S390ConstantInstructionSnippet* cis
                = (TR_S390ConstantInstructionSnippet*)tempMR->getConstantDataSnippet();
            TR_ASSERT(cis != NULL, "Out of line EX instruction doesn't have a constantInstructionSnippet\n");
...
void OMR::Z::Instruction::attemptOpAdjustmentForLongDisplacement()
{
    ...
    if (self()->getOpCode().getInstructionFormat(self()->getOpCodeValue()) == RXY_FORMAT)
        ...
        TR_ASSERT(0, "only RX, RS and SI instructions can be safely mapped to long displacement: opCode: %s",
...
uint32_t OMR::Z::Instruction::useSourceMemoryReference(TR::MemoryReference* memRef)
{
    ...
    if (!self()->cg()->afterRA()) {
        ...
        if (self()->hasLongDisplacementSupport() && !memRef->hasTemporaryNegativeOffset()
...
TR::Register* OMR::Z::Instruction::assignRegisterNoDependencies(TR::Register* reg)
{
    ...
    if ((reg->getRegisterPair() && (reg->getRegisterPair()->getHighOrder()->getRealRegister() != NULL
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    for (i = 0; i < _targetRegSize; ++i) {
        ...
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            ...
            if (_targetReg[i]->getRegisterPair()
                ...
                availHPRMask
                    &= ~(TR::RealRegister::getBitMask(toRealRegister(_targetReg[i]->getRegisterPair()->getHighOrder())
                                                          ->getHighWordRegister()
                                                          ->getRegisterNumber()));
                availHPRMask
                    &= ~(TR::RealRegister::getBitMask(toRealRegister(_targetReg[i]->getRegisterPair()->getLowOrder())
                                                          ->getHighWordRegister()
                                                          ->getRegisterNumber()));
    ...
    if (_targetReg) {
        for (i = 0; i < _targetRegSize; ++i) {
            ...
            /*
              When we are mixing 32 and 64 bit registers, we have to ensure that 64 bit instructions use 64 bit
              registers.
              If this assertion is triggered, it is probably because an evaluator called allocateRegister() when it
              should
              have called allocate64bitRegister().
             */
            if ((_opcode.is64bit() || _opcode.is32to64bit()) && self()->cg()->use64BitRegsOn32Bit()
                && TR::Compiler->target.is32Bit()
                && !_targetReg[i]->getRealRegister()) // skip check if the target reg is already a real register.
            {
                TR_ASSERT(_targetReg[i]->getKind() != TR_GPR, "Using GPR for 64 bit instruction %s [%s] in 32 bit mode",
                    self()->cg()->getDebug()->getOpCodeName(&_opcode), self()->cg()->getDebug()->getName(self()));
            }

            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
                if (_opcode.is64bit() || _opcode.is32to64bit()
                    || (TR::Compiler->target.is64Bit() && (self()->getOpCodeValue() == TR::InstOpCode::LA
            ...
            if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)
                ...
                availHPRMask &= ~(TR::RealRegister::getBitMask(
                    toRealRegister(_targetReg[i])->getHighWordRegister()->getRegisterNumber()));
                self()->cg()->setAvailableHPRSpillMask(availHPRMask);
            }
...
void OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies)
{
    ...
    if ((self()->implicitlySetsGPR0() || self()->implicitlySetsGPR1() || self()->implicitlySetsGPR2())
        || ((_targetReg != NULL) && ((!self()->isStore() && (!self()->isCompare()) && (!self()->isTrap())
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::LM || self()->getOpCodeValue() == TR::InstOpCode::LMG
        || self()->getOpCodeValue() == TR::InstOpCode::LMH || self()->getOpCodeValue() == TR::InstOpCode::LAM
        || self()->getOpCodeValue() == TR::InstOpCode::LMY)
        loadOrStoreMultiple = 0; // load
    else if (self()->getOpCodeValue() == TR::InstOpCode::STM || self()->getOpCodeValue() == TR::InstOpCode::STMG
        || self()->getOpCodeValue() == TR::InstOpCode::STMH || self()->getOpCodeValue() == TR::InstOpCode::STAM
...
void OMR::Z::Instruction::addARDependencyCondition(TR::Register* virtAR, TR::Register* assignedGPR)
{
    ...
    if (self()->getDependencyConditions()) {
        if (self()->getDependencyConditions()->searchPreConditionRegister(assignedGPR->getAssignedRegister())) {
            ...
            TR::RegisterDependencyConditions* tmp = new (self()->cg()->trHeapMemory())
                TR::RegisterDependencyConditions(self()->getDependencyConditions(), 1, 0, self()->cg());
            self()->resetDependencyConditions(tmp);
            assignPreCondition = true;
        } else {
            // add another post condition to current list
            TR::RegisterDependencyConditions* tmp = new (self()->cg()->trHeapMemory())
                TR::RegisterDependencyConditions(self()->getDependencyConditions(), 0, 1, self()->cg());
            self()->resetDependencyConditions(tmp);
        }
    } else {
        // add new post condition
        TR::RegisterDependencyConditions* tmp
            = new (self()->cg()->trHeapMemory()) TR::RegisterDependencyConditions(0, 1, self()->cg());
        self()->resetDependencyConditions(tmp);
    }
    if (assignPreCondition) {
        ...
        newCondition = self()->getDependencyConditions()->addPreConditionIfNotAlreadyInserted(
            virtAR, REGNUM(TR::RealRegister::FirstAR - TR::RealRegister::FirstGPR
                        + toRealRegister(assignedGPR)->getRegisterNumber()));
    } else {
        self()->cg()->traceRegisterAssignment(
            "POST dependancy for instruction %p on virtual reg %s -> %s to be added\n", self(),
            self()->cg()->getDebug()->getName(virtAR), self()->cg()->getDebug()->getName(assignedGPR));
        newCondition = self()->getDependencyConditions()->addPostConditionIfNotAlreadyInserted(
            virtAR, REGNUM(TR::RealRegister::FirstAR - TR::RealRegister::FirstGPR
                        + toRealRegister(assignedGPR)->getRegisterNumber()));
    }
    ...
    if (newCondition) {
        ...
        self()->cg()->traceRegisterAssignment(
            "duplicate dependancy for instruction %p on virtual reg %s ->  %s not added\n", self(),
            self()->cg()->getDebug()->getName(virtAR), self()->cg()->getDebug()->getName(assignedGPR));
        virtAR->decTotalUseCount();
        virtAR->decFutureUseCount();
    }
...
bool OMR::Z::Instruction::isAsmGen() { return self()->getOpCodeValue() == TR::InstOpCode::ASM; }

bool OMR::Z::Instruction::isRet() { return self()->getOpCodeValue() == TR::InstOpCode::RET; }

bool OMR::Z::Instruction::isTailCall() { return self()->getOpCodeValue() == TR::InstOpCode::TAILCALL; }

int8_t OMR::Z::Instruction::lastOperandIndex(void) { return self()->numOperands() - 1; }

void OMR::Z::Instruction::setJITEntry()
{
    TR_ASSERT(self()->getOpCodeValue() == TR::InstOpCode::LABEL, "JIT entry point should be a branch instruction\n");
    _flags.set(JITEntry);
}
