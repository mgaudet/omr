...
OMR::Z::Instruction::useSourceMemoryReference(TR::MemoryReference* memRef)
{
    ...
    if (!self()->cg()->afterRA()) {
        ...
        if (self()->hasLongDisplacementSupport() && !memRef->hasTemporaryNegativeOffset() && (disp < 0 || disp >= MAXDISP) && (disp >= MINLONGDISP || disp <= MAXLONGDISP)) {
...
void OMR::Z::Instruction::assignOrderedRegisters(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (_targetReg) {
        for (i = 0; i < _targetRegSize; ++i) {
          ...
          */
            if ((_opcode.is64bit() || _opcode.is32to64bit()) && self()->cg()->use64BitRegsOn32Bit() && TR::Compiler->target.is32Bit() && !_targetReg[i]->getRealRegister()) // skip check if the target reg is already a real register.
            {
...
void OMR::Z::Instruction::setUseDefRegisters(bool updateDependencies)
{
    ...
    if (_sourceReg) {
        for (i = 0; i < _sourceRegSize; ++i) {
            ...
            if (regPair != NULL) {
                ...
                } else if (self()->getOpCodeValue() != TR::InstOpCode::LM && self()->getOpCodeValue() != TR::InstOpCode::LMG && self()->getOpCodeValue() != TR::InstOpCode::LMH && self()->getOpCodeValue() != TR::InstOpCode::LAM && self()->getOpCodeValue() != TR::InstOpCode::LMY) {
                    (*_useRegs)[indexSource++] = _sourceReg[i];
                } else {
                    (*_defRegs)[indexTarget++] = _sourceReg[i];
                }
    ...
    if (self()->getOpCodeValue() == TR::InstOpCode::LM || self()->getOpCodeValue() == TR::InstOpCode::LMG || self()->getOpCodeValue() == TR::InstOpCode::LMH || self()->getOpCodeValue() == TR::InstOpCode::LAM || self()->getOpCodeValue() == TR::InstOpCode::LMY)
        loadOrStoreMultiple = 0; //load
    else if (self()->getOpCodeValue() == TR::InstOpCode::STM || self()->getOpCodeValue() == TR::InstOpCode::STMG || self()->getOpCodeValue() == TR::InstOpCode::STMH || self()->getOpCodeValue() == TR::InstOpCode::STAM || self()->getOpCodeValue() == TR::InstOpCode::STMY)
...
void OMR::Z::Instruction::addARDependencyCondition(TR::Register* virtAR, TR::Register* assignedGPR)
{
    ...
    for (uint32_t x = 1; x <= self()->cg()->getS390Linkage()->getNumStackPointerRegisters(); x++) {
        if (REGINDEX((toRealRegister(assignedGPR->getRealRegister()))->getRegisterNumber()) == REGINDEX(self()->cg()->getS390Linkage()->getStackPointerRealRegister(x)->getRegisterNumber())) {
