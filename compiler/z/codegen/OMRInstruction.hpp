...
namespace Z {
    ...
    public:
        ...
        {
            TR_ASSERT(0, "Should not be called unless working with an RX, RS or RI instructions");
        }
    ...
    protected:
         ...
         * source registers, source mem, target mem
         *   Register operands fall into target or source depending how they are used. If a register operand is set it
         * will be in target category, if register
         *     is set and used it will also be in target category but not in source. The opcode property UsesTarget
         * indicates that all target registers are
         *     used for input. 390 Instructions have 0,1,2, or 3 register operands often refered in manual as R1,R2,R3.
         * These operands can be retreived using
         *     getRegisterOperand(i). Note that a compare or store instruction that does not set operand 1 will put R1
         * into the sourceReg section of operands.
         *     This is not what it was like before. getTargetRegister used to fetch R1 and getSourceRegister used to
         * fetch R2. These were not always meaning
         *     source and target so now these methods are disallowed. Register operands can be register pairs for
        ...
        template <typename T> class RegisterArray : public TR::Allocatable<RegisterArray<T>, TR::Allocator> {
        public:
