    ...
    int32_t blockNumber, bool backwards)
{
    ...
    for (int8_t curReg = firstReg; curReg != lastReg; curReg += dir) {
        ...
        if (!self()->restoreRegister(REGNUM(curReg), blockNumber)) {
            ...
            if ((curReg == firstSaved && !backwards) || (curReg == lastSaved && backwards)) // can't do anything without the ends
            {
                restoreEndReg = true;
            }
            stillCons = false;
...
void* OMR::Z::Linkage::saveArguments(void* cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol>* parameterList)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (ai >= 0 && loadOpCode == TR::InstOpCode::L && enableHighWordRA && self()->getS390RealRegister(REGNUM(ai))->isHighWordRegister())
        ...
        if (lri >= 0) {
            ...
            if (!self()->getIsLeafRoutine() && ((ai < 0 && paramCursor->isReferencedParameter()) || self()->hasToBeOnStack(paramCursor) || unconditionalSave)) {
                ...
                case TR::Aggregate: // Should only happen on zLinux
                    ...
                    if (secondStore && fullLong && TR::Compiler->target.is32Bit() && !self()->alreadySaved((TR::RealRegister::RegNum)(regNum + 1))) {
                        if (genBinary) {
...
OMR::Z::Linkage::copyArgRegister(TR::Node* callNode, TR::Node* child, TR::Register* argRegister)
{
    ...
    if (!self()->cg()->canClobberNodesRegister(child, 0)) {
        if (!(TR::Compiler->target.is64Bit()) && ((child->getDataType() == TR::Int64) || (child->getDataType() == TR::Int64))) {
...
void OMR::Z::Linkage::clearCachedStackRegisterForOutgoingArguments(bool justClearSlot)
{
    if (_cachedStackRegisterForOutgoingArguments != NULL) {
        if (!justClearSlot && self()->isFastLinkLinkageType())
            self()->cg()->stopUsingRegister(_cachedStackRegisterForOutgoingArguments); // faslink casued allocation of register
...
OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg, int32_t toreg)
{
    ...
    bool checkHPR = (self()->cg()->supportsHighWordFacility() && !self()->comp()->getOption(TR_DisableHighWordRA) && (self()->getS390RealRegister(REGNUM(fromreg)))->isLowWordRegister() && (self()->getS390RealRegister(REGNUM(toreg)))->isLowWordRegister());
    for (int32_t i = fromreg; i <= toreg; ++i) {
        if ((self()->getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod() || (checkHPR && (self()->getS390RealRegister(REGNUM(i)))->getHighWordRegister()->getHasBeenAssignedInMethod())) {
    ...
    TR::RealRegister::RegNum spRealReg)
{
    ...
    if (break64 || newNumUsedRegs == 1) {
        ...
        for (uint8_t i = 0; i < newNumUsedRegs; i++) {
            ...
            if (i != newNumUsedRegs && ((specializedEpilogues && self()->restoreRegister(REGNUM(curReg), blockNumber)) || !specializedEpilogues)) {
