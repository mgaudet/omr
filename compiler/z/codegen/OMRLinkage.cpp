...
bool OMR::Z::Linkage::restoreRegister(TR::RealRegister::RegNum reg, int32_t blockNumber)
{
    return self()->cg()->restoreRegister(reg, blockNumber);
}

int8_t OMR::Z::Linkage::getNumRegsToRestore(
    TR::RealRegister::RegNum firstUsedReg, TR::RealRegister::RegNum lastUsedReg, int32_t blockNumber)
{
...
uint8_t OMR::Z::Linkage::getRestoreBump(
    TR::RealRegister::RegNum firstSaved, TR::RealRegister::RegNum lastSaved, int32_t blockNumber, bool backwards)
{
...
void OMR::Z::Linkage::markPreservedRegsInDep(TR::RegisterDependencyConditions* deps, int32_t blockNum)
{
    ...
    void* cursor, bool genBinary, bool InPreProlog, int32_t frameOffset, List<TR::ParameterSymbol>* parameterList)
{
    ...
    for (paramCursor = paramIterator.getFirst(); paramCursor != NULL; paramCursor = paramIterator.getNext()) {
        ...
        if (lri >= 0) {
            ...
            // ensure we always store out to the stack for whenever we need all variables on the stack.
            ...
            if (!self()->getIsLeafRoutine() && ((ai < 0 && paramCursor->isReferencedParameter())
                ...
                case TR::Aggregate: // Should only happen on zLinux
                        ...
                        if (genBinary) {
                            ...
                            if (storeOpCode == TR::InstOpCode::STCM)
                                cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::STCM, firstNode,
                                    self()->getS390RealRegister(regNum), opcodeMask, mr, (TR::Instruction*)cursor);

                            else
                                cursor = generateRXInstruction(self()->cg(), storeOpCode, firstNode,
                                    self()->getS390RealRegister(regNum), mr, (TR::Instruction*)cursor);
                            ...
                            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                self()->removeOSCOnSavedArgument(
                                    (TR::Instruction*)cursor, self()->getS390RealRegister(regNum), offset);
                        }
                    }
                    ...
                    if (secondStore && fullLong && TR::Compiler->target.is32Bit()
                        ...
                        if (genBinary) {
                            cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::ST, (uint8_t*)cursor,
                                self()->getS390RealRegister(regNum), offset, self()->cg());
                        } else {
                            TR::MemoryReference* mr
                            ...
                            cursor = generateRXInstruction(self()->cg(), TR::InstOpCode::ST, firstNode,
                                self()->getS390RealRegister(REGNUM(regNum + 1)), mr, (TR::Instruction*)cursor);
                            ((TR::Instruction*)cursor)->setBinLocalFreeRegs(binLocalRegs);
                            if (!InPreProlog && !globalAllocatedRegisters.isSet(regNum))
                                self()->removeOSCOnSavedArgument(
                                    (TR::Instruction*)cursor, self()->getS390RealRegister(regNum), offset);
...
#endif
                    if (genBinary) {
                        cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::STE, (uint8_t*)cursor,
                            self()->getS390RealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
...
#endif
                    if (genBinary) {
                        cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD, (uint8_t*)cursor,
                            self()->getS390RealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                ...
                case TR::DecimalLongDouble:
                    if (genBinary) {
                        cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD, (uint8_t*)cursor,
                            self()->getS390RealRegister(regNum), offset, self()->cg());
                        cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::STD, (uint8_t*)cursor,
                            self()->getS390RealRegister(REGNUM(regNum + 2)), offset + 8, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                        ...
                        cursor = (void*)generateRXInstruction(self()->cg(), TR::InstOpCode::STD, firstNode,
                            self()->getS390RealRegister(regNum), mr, (TR::Instruction*)cursor);
                        cursor = (void*)generateRXInstruction(self()->cg(), TR::InstOpCode::STD, firstNode,
                            self()->getS390RealRegister(REGNUM(regNum + 2)), hiMR, (TR::Instruction*)cursor);
                        ...
                        if (!InPreProlog && !globalAllocatedRegisters.isSet(REGNUM(regNum + 2)))
                            self()->removeOSCOnSavedArgument(
                                (TR::Instruction*)cursor, self()->getS390RealRegister(REGNUM(regNum + 2)), offset + 8);
                    }
                    break;
#endif
                case TR::VectorInt8:
                ...
                case TR::VectorDouble:
                    if (genBinary) {
                        cursor = (void*)TR_S390CallSnippet::storeArgumentItem(TR::InstOpCode::VST, (uint8_t*)cursor,
                            self()->getS390RealRegister(regNum), offset, self()->cg());
                    } else {
                        TR::MemoryReference* mr
                            = generateS390MemoryReference(stackPtr, offset, self()->cg(), param_name);
                    ...
                    if (freeScratchable.isSet(ai)) {
                        if (dtype == TR::Float || dtype == TR::Double
                            ...
                            cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LDR, firstNode,
                                self()->getS390RealRegister(REGNUM(ai)), self()->getS390RealRegister(regNum),
                            ...
                            cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                                self()->cg()->allocateFPRegisterPair(
                                    self()->getS390RealRegister(REGNUM(ai_l)), self()->getS390RealRegister(REGNUM(ai))),
                                self()->cg()->allocateFPRegisterPair(self()->getS390RealRegister(REGNUM(regNum + 2)),
                            ...
                            if ((dtype == TR::Int64) && self()->cg()->use64BitRegsOn32Bit()) {
                                cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLG, firstNode,
                                    self()->getS390RealRegister(REGNUM(ai)), self()->getS390RealRegister(regNum), 32,
                                    (TR::Instruction*)cursor);
                                ...
                                if (enableHighWordRA && self()->getS390RealRegister(REGNUM(ai))->isHighWordRegister()) {
                                    cursor = generateExtendedHighWordInstruction(firstNode, self()->cg(),
                                        TR::InstOpCode::LHLR, self()->getS390RealRegister(REGNUM(ai)),
...
//
                    ...
                    if (freeScratchable.isSet(ai_l)) {
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                            self()->getS390RealRegister(REGNUM(ai_l)), self()->getS390RealRegister(REGNUM(regNum + 1)),
                            (TR::Instruction*)cursor);
                ...
                if (freeScratchable.isSet(ai)) {
                    ...
                    if (loadOpCode == TR::InstOpCode::ICM)
                        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM, firstNode,
                            self()->getS390RealRegister(REGNUM(ai)), opcodeMask, mr, (TR::Instruction*)cursor);
                    else
                        cursor = generateRXInstruction(self()->cg(), loadOpCode, firstNode,
                            self()->getS390RealRegister(REGNUM(ai)), mr, (TR::Instruction*)cursor);
    ...
    while (freeMore && numMoves > 0) {
        ...
        for (i1 = 0; i1 < busyIndex; i1++) {
            ...
            if (!(target < 0) && (freeScratchable.isSet(target) || (source == target && busyMoves[2][i1] == 6)
                ...
                case 0: // Reg 2 Reg
                    if (enableHighWordRA && self()->getS390RealRegister(REGNUM(target))->isHighWordRegister()) {
                        cursor = generateExtendedHighWordInstruction(firstNode, self()->cg(), TR::InstOpCode::LHLR,
                            self()->getS390RealRegister(REGNUM(target)), self()->getS390RealRegister(REGNUM(source)), 0,
                            (TR::Instruction*)cursor);
                    } else {
                        cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::getLoadRegOpCode(), firstNode,
                            self()->getS390RealRegister(REGNUM(target)), self()->getS390RealRegister(REGNUM(source)),
                            (TR::Instruction*)cursor);
                    }
                ...
                case 1: // Int or Addr  Reg from Stack Slot
                {
                    ...
                    if ((TR::InstOpCode::Mnemonic)busyMoves[3][i1] == TR::InstOpCode::ICM)
                        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::ICM, firstNode,
                            self()->getS390RealRegister(REGNUM(target)), busyMoves[4][i1], mr,
                            (TR::Instruction*)cursor);
                        ...
                        cursor = generateRXYInstruction(self()->cg(), TR::InstOpCode::LFH, firstNode,
                            self()->getS390RealRegister(REGNUM(target)), mr, (TR::Instruction*)cursor);

                    else
                        cursor = generateRXInstruction(self()->cg(), (TR::InstOpCode::Mnemonic)busyMoves[3][i1],
                            firstNode, self()->getS390RealRegister(REGNUM(target)), mr, (TR::Instruction*)cursor);
                ...
                case 6: // 64bit reg on 31bit
                    cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::SLLG, firstNode,
                        self()->getS390RealRegister(REGNUM(target)), self()->getS390RealRegister(REGNUM(source)), 32,
                        (TR::Instruction*)cursor);
                ...
                case 10: // LongDouble
                    cursor = generateRRInstruction(self()->cg(), TR::InstOpCode::LXR, firstNode,
                        self()->cg()->allocateFPRegisterPair(self()->getS390RealRegister(REGNUM(target + 2)),
                            self()->getS390RealRegister(REGNUM(target))),
                        self()->cg()->allocateFPRegisterPair(self()->getS390RealRegister(REGNUM(source + 2)),
    ...
    TR::ResolvedMethodSymbol* method, List<TR::ParameterSymbol>& parmList)
{
    ...
    int32_t paramNum = -1;
    while ((paramCursor != NULL) && (numIntArgs < self()->getNumIntegerArgumentRegisters()
...
TR::InstOpCode::Mnemonic OMR::Z::Linkage::getOpCodeForLinkage(TR::Node* child, bool isStore, bool isRegReg)
{
    ...
    case TR::Int64:
        ...
        break;
    case TR::Address:
        return (isStore) ? TR::InstOpCode::getStoreOpCode() : (isRegReg) ? TR::InstOpCode::getLoadRegOpCode()
    ...
    case TR::Int32:
        if (TR::Compiler->target.is32Bit())
            ...
            return (isStore) ? (isWiden) ? TR::InstOpCode::STG : TR::InstOpCode::ST : (isRegReg)
 ...
 */
void OMR::Z::Linkage::clearCachedStackRegisterForOutgoingArguments(bool justClearSlot)
{
    if (_cachedStackRegisterForOutgoingArguments != NULL) {
        if (!justClearSlot && self()->isFastLinkLinkageType())
            self()->cg()->stopUsingRegister(
                _cachedStackRegisterForOutgoingArguments); // faslink casued allocation of register
    ...
    TR::Register* argRegister)
{
    ...
    if (isNull) {
        ...
        TR::Instruction* br = generateS390BranchInstruction(
            self()->cg(), TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, child, nonNullLabel);
    ...
    int64_t killMask, TR::Register*& vftReg, bool PassReceiver)
{
    ...
    self()->clearCachedStackRegisterForOutgoingArguments(true); // outgoing arguments may be accessed via stack register
                                                                // realized lazily via
                                                                // getStackRegisterForOutgoingArgument()
    ...
    for (i = from; (rightToLeft && i >= to) || (!rightToLeft && i <= to); i += step) {
...
#endif
        {
            if (self()->isFloatParmDescriptors()
                ...
                uint32_t val
                    = TR_S390zOSSystemLinkage::getFloatParmDescriptorFlag(xplinkCallDescriptorFlags, numFloatArgs);
...
void OMR::Z::Linkage::lockAR(int32_t registerNo)
{
    ...
    TR::RealRegister* tempRegister
        = self()->cg()->machine()->getS390RealRegister(REGNUM(registerNo + TR::RealRegister::FirstAR));
...
TR::RealRegister::RegNum OMR::Z::Linkage::getFirstSavedRegister(int32_t fromreg, int32_t toreg)
{
    ...
    for (int32_t i = fromreg; i <= toreg; ++i) {
        if ((self()->getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()
            || (checkHPR
                   && (self()->getS390RealRegister(REGNUM(i)))->getHighWordRegister()->getHasBeenAssignedInMethod())) {
    ...
    TR::RealRegister* spReg, TR::MemoryReference* rsa, TR::RealRegister::RegNum spRealReg)
{
    ...
    if (frontBump > 0) {
        rsa = generateS390MemoryReference(
            spReg, (rsa->getOffset() + (frontBump * self()->cg()->machine()->getGPRSize())), self()->cg());
    }
    ...
    if (break64 || newNumUsedRegs == 1) {
        ...
        for (uint8_t i = 0; i < newNumUsedRegs; i++) {
            ...
            if (i != newNumUsedRegs && ((specializedEpilogues && self()->restoreRegister(REGNUM(curReg), blockNumber))
    ...
    // 64-bit non-optimized CASE AND 32-bit shorter LMG case
        ...
        cursor = generateRSInstruction(self()->cg(), TR::InstOpCode::getLoadMultipleOpCode(), nextNode,
            self()->getS390RealRegister(REGNUM(newFirstUsedReg)), self()->getS390RealRegister(REGNUM(newLastUsedReg)),
            rsa, cursor);
...
TR::RealRegister::RegNum OMR::Z::Linkage::getSpecialArgumentRegister(uint32_t specialIndex)
{
    if (self()->isSpecialArgumentRegisters()
        && ((specialIndex - TR_FirstSpecialLinkageIndex) < _numSpecialArgumentRegisters))
        return _specialArgRegisters[specialIndex - TR_FirstSpecialLinkageIndex];
    else
        return TR::RealRegister::NoReg;
}

void OMR::Z::Linkage::setSpecialArgumentRegister(uint32_t index, TR::RealRegister::RegNum r)
{
...
TR::RealRegister* OMR::Z::Linkage::getLitPoolRealRegister() { return self()->getS390RealRegister(_litPoolRegister); }
...
TR::RealRegister* OMR::Z::Linkage::getS390RealRegister(TR::RealRegister::RegNum rNum)
{
