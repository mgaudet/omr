    ...
    TR::Register* targetReg, TR::Register* sourceReg, TR::CodeGenerator* cg, flags32_t instFlags)
{
    ...
    case TR_AR:
        currentInstruction
            = generateRRInstruction(cg, TR::InstOpCode::CPYA, currentNode, targetReg, sourceReg, precedingInstruction);
        cg->traceRAInstruction(currentInstruction);
        break;
    case TR_VRF:
        currentInstruction
    ...
    // exchange access registers
        ...
        if (middleReg != NULL) {
            ...
            currentInstruction = generateRSInstruction(
                cg, TR::InstOpCode::STAM, currentNode, targetReg, targetReg, tempMR, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
            currentInstruction = generateRRInstruction(
                cg, TR::InstOpCode::CPYA, currentNode, targetReg, sourceReg, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
            TR::MemoryReference* tempMR2 = generateS390MemoryReference(*tempMR, 0, cg);
            currentInstruction = generateRSInstruction(
                cg, TR::InstOpCode::LAM, currentNode, sourceReg, sourceReg, tempMR2, currentInstruction);
            cg->traceRAInstruction(currentInstruction);
...
OMR::Z::Machine::Machine(TR::CodeGenerator* cg)
    : OMR::Machine(NUM_S390_GPR, NUM_S390_FPR, NUM_S390_VRF)
    , _cg(cg)
    ...
    , _lastLinkageFPR(-1)
    , _firstGlobalAccessRegisterNumber(-1)
    , _lastGlobalAccessRegisterNumber(-1)
...
bool OMR::Z::Machine::isLegalEvenOddRestrictedPair(
    TR::RealRegister* evenReg, TR::RealRegister* oddReg, uint64_t availRegMask)
{
    if (evenReg == NULL || oddReg == NULL) {
        return false;
    }
    if (toRealRegister(evenReg)->isHighWordRegister() || toRealRegister(oddReg)->isHighWordRegister()) {
        ...
        return self()->isLegalEvenRegister(evenReg, ALLOWBLOCKED, availRegMask, ALLOWLOCKED)
            && self()->isLegalOddRegister(oddReg, ALLOWBLOCKED, availRegMask, ALLOWLOCKED);
    } else
        return false;
}
    ...
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    // shouldn't
    if (regNum % 2 != 0 || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0
...
TR::RealRegister* OMR::Z::Machine::findBestLegalOddRegister(uint64_t availRegMask)
{
    ...
    for (int32_t i = first + 1; i <= last; i += 2) {
        ...
        } else if (foundFreePair == false && (_registerFile[i]->getState() == TR::RealRegister::Free
                                                 || _registerFile[i]->getState() == TR::RealRegister::Unlatched)
            && _registerFile[i]->getWeight() < bestWeightSoFar) {
            freeRegister = _registerFile[i];
    ...
    TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    if (regNum % 2 != 1 || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0
        || _registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Locked
            && allowLocked == false
        || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Blocked
               && allowBlocked == false)) {
...
TR::RealRegister* OMR::Z::Machine::findBestLegalSiblingFPRegister(bool isFirst, uint64_t availRegMask)
{
    ...
    for (int32_t i = 0; i < NUM_S390_FPR_PAIRS; i++) {
        ...
        } else if (foundFreePair == false && (lastSiblingFPReg->getState() == TR::RealRegister::Free
    ...
    TR::Instruction* currInst, TR::Register* toFreeReg, uint64_t availRegMask)
{
    ...
    if (bestRegister == NULL)
        ...
        TR::Instruction* cursor
            = self()->registerCopy(currInst, toFreeReg->getKind(), assignedRegister, bestRegister, _cg, 0);
    ...
    TR::Register* targetRegister, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (enableHighWordRA && assignedRegister != NULL) {
        ...
        if (targetRegister->is64BitReg()) {
            if ((TR::RealRegister::getBitMask(toRealRegister(assignedRegister)->getRegisterNumber()) & availRegMask)
                == 0) {
                ...
                TR::Instruction* cursor = self()->registerCopy(
                    appendInst, kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, _cg, 0);
                assignedRegister->setAssignedRegister(NULL);
                ...
                TR::RealRegister* assignedRegister64
                    = self()->findBestRegisterForShuffle(currInst, targetRegister, availRegMask);
                assignedRegister->unblock();
                TR::Instruction* cursor = generateExtendedHighWordInstruction(currInst->getNode(), _cg,
            ...
            } else if ((TR::RealRegister::getBitMask(toRealRegister(assignedRegister)->getRegisterNumber())
                           & availRegMask)
                == 0) {
                // Oh no.. targetRegister is assigned something it shouldn't be assigned to. Do some shuffling
                // find a new register to shuffle to
        ...
        && (TR::RealRegister::getBitMask(toRealRegister(assignedRegister)->getRegisterNumber()) & availRegMask) == 0) {
        ...
        assignedRegister->unblock();
        TR::Instruction* cursor = self()->registerCopy(
            currInst, kindOfRegister, toRealRegister(assignedRegister), newAssignedRegister, _cg, 0);
    ...
    if (doBookKeeping
        && (assignedRegister->getState() != TR::RealRegister::Locked || self()->supportLockedRegisterAssignment())) {
        targetRegister->decFutureUseCount();
        targetRegister->setIsLive();

        TR_ASSERT(targetRegister->getFutureUseCount() >= 0, "\nRegister assignment: register [%s] futureUseCount "
                                                            "should not be negative (for node [%s], ref count=%d) !\n",
            _cg->getDebug()->getName(targetRegister), _cg->getDebug()->getName(currInst->getNode()),
        ...
        if ((targetRegister->getFutureUseCount() == 0) || killOOLReg
            ...
            if (comp->getOption(TR_EnableTrueRegisterModel) && currInst->startOfLiveRange(targetRegister)
                ...
                traceMsg(comp, "Start of live range for a non global virtual yet its future count is not zero. Reg=%s "
                               "Instr=[%p]\n",
    ...
    TR::Register* regPair, TR::Instruction* currInst, bool doBookKeeping, uint64_t availRegMask)
{
    ...
    if (regPair->isArGprPair()) {
        if (_cg->getRAPassAR()) {
            ...
            TR::Register* targetReg
                = self()->assignBestRegisterSingle(regPair->getGPRofArGprPair(), currInst, doBookKeeping, availRegMask);
    ...
    TR::RegisterPair* assignedRegPair
        = new (_cg->trHeapMemory(), TR_MemoryBase::RegisterPair) TR::RegisterPair(lastReg, firstReg);
    ...
    if (freeRegisterHigh == NULL) {
        ...
        if (freeRegisterLow && TR_FPR != regPair->getKind()
            && self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED, availRegMask)
            && toRealRegister(toRealRegister(freeRegisterLow)->getSiblingRegister())->getState()
                == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        else if (freeRegisterLow && TR_FPR == regPair->getKind()
            && self()->isLegalSecondOfFPRegister(freeRegisterLow, DISALLOWBLOCKED, availRegMask)
            && toRealRegister(toRealRegister(freeRegisterLow)->getSiblingRegister())->getState()
                == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        // if nothing is using my coloured register use it
        if (colouredIsGood && firstCr && toRealRegister(firstCr)->getState() != TR::RealRegister::Blocked) {
            if (firstCr->getAssignedRegister() == NULL) {
                ...
                self()->coerceRegisterAssignment(
                    currInst, firstReg, toRealRegister(freeRegisterHigh)->getRegisterNumber(), PAIRREG);
    ...
    if (freeRegisterLow == NULL) {
        ...
        if (freeRegisterHigh && TR_FPR != regPair->getKind()
            && self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)
            && toRealRegister(toRealRegister(freeRegisterHigh)->getSiblingRegister())->getState()
                == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        else if (freeRegisterHigh && TR_FPR == regPair->getKind()
            && self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)
            && toRealRegister(toRealRegister(freeRegisterHigh)->getSiblingRegister())->getState()
                == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        // if nothing is using my coloured register use it
        if (colouredIsGood && lastCr && toRealRegister(lastCr)->getState() != TR::RealRegister::Blocked) {
            if (lastCr->getAssignedRegister() == NULL) {
                ...
                self()->coerceRegisterAssignment(
                    currInst, lastReg, toRealRegister(freeRegisterLow)->getRegisterNumber(), PAIRREG);
    ...
    if (((regPair->getKind() != TR_FPR) && (regPair->getKind() != TR_VRF)
        ...
        if (!self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask)) {
            ...
            freeRegisterHigh = self()->getS390RealRegister(
            ...
            freeRegisterLow = self()->getS390RealRegister(
    ...
    TR_RegisterKinds rk, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    bool enableHighWordRA
        = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF;
    ...
    TR::Register* virtualReg, uint64_t availRegMask, bool needsHighWord)
{
    ...
    bool enableHighWordRA
        = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && (rk == TR_GPR || rk == TR_GPR64);
    ...
    if (self()->cg()->enableRegisterPairAssociation() && preference == TR::RealRegister::LegalEvenOfPair
        ...
        if (self()->cg()->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)
            ...
            if (bestRegister != NULL && (bestRegister->getState() == TR::RealRegister::Free
        ...
        if (!enableHighWordRA) {
            ...
            if ((_registerFile[i]->getState() == TR::RealRegister::Free
                ...
                if (comp->getOption(TR_Randomize)) {
                    ...
                    if (performTransformation(comp, "O^O Random Codegen - Randomizing Weight for %s, Original "
    ...
    TR_RegisterKinds rk, uint64_t availRegMask, bool allowNullReturn, bool doNotSpillToSiblingHPR)
{
    ...
    if (enableHighWordRA && (virtReg->is64BitReg() || doNotSpillToSiblingHPR)) {
        uint32_t availHighWordRegMap
            = ~(TR::RealRegister::getBitMask(toRealRegister(best->getHighWordRegister())->getRegisterNumber()))
    ...
    TR::Instruction* currentInstruction, TR::RealRegister* targetReal, bool is64BitReg)
{
    ...
    if (virtReg) {
        if (enableHighWordRA && is64BitReg) {
            uint32_t availHighWordRegMap = ~(TR::RealRegister::getBitMask(
                                               toRealRegister(targetReal->getHighWordRegister())->getRegisterNumber()))
                & 0xffff0000;
    ...
    TR::Instruction* currentInstruction, TR::Register* virtReg, uint32_t availHighWordRegMap)
{
        ...
        if (freeHighWordReg) {
            ...
            if (alreadySpilledToHPR) {
                cursor = generateExtendedHighWordInstruction(
                    currentNode, _cg, TR::InstOpCode::LHHR, best, freeHighWordReg, 0, currentInstruction);
            } else {
                if (virtReg->containsCollectedReference()) {
                    if (debugHPR) {
                        printf("(collected) ");
                    ...
                    if (compressShift == 0) {
                        cursor = generateExtendedHighWordInstruction(
                            currentNode, _cg, TR::InstOpCode::LLHFR, best, freeHighWordReg, 0, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                        cursor = generateRILInstruction(
                            _cg, TR::InstOpCode::IIHF, currentNode, best, (uintptrj_t)0, cursor);
                        self()->cg()->traceRAInstruction(cursor);
                    } else {
                        cursor = generateRIEInstruction(_cg, TR::InstOpCode::RISBLG, currentNode, best, freeHighWordReg,
                            0, 31 + 0x80 - compressShift, 32 + compressShift, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                        cursor = generateRIEInstruction(_cg, TR::InstOpCode::RISBHG, currentNode, best, freeHighWordReg,
                            32 - compressShift, 31 + 0x80, 32 + compressShift, currentInstruction);
                        self()->cg()->traceRAInstruction(cursor);
                    }
                } else {
                    cursor = generateExtendedHighWordInstruction(
...

    {
        ...
        case TR_GPR:
            ...
            // if we selected the VM Thread Register to be freed, check to see if the value has already been spilled
            else if (comp->getOption(TR_DisableOOL) && _cg->needsVMThreadDependency()
        ...
        if (enableHighWordRA && alreadySpilledToHPRCollectible) {
            ...
            if (opCode == TR::InstOpCode::LAM)
                cursor
                    = generateRSInstruction(self()->cg(), opCode, currentNode, best, best, tempMR, currentInstruction);
    ...
    if (comp->getOption(TR_Enable390FreeVMThreadReg) && spilledRegister == self()->cg()->getVMThreadRegister()) {
        ...
        if (freeHighWordReg) {
            if (targetRegister->isHighWordRegister()) {
                cursor = generateExtendedHighWordInstruction(
                    currentNode, _cg, TR::InstOpCode::LHHR, freeHighWordReg, targetRegister, 0, currentInstruction);
            } else {
                if (spilledRegister->containsCollectedReference()) {
                    uint32_t compressShift = TR::Compiler->om.compressedReferenceShift();
                    // need to assert heapbase, offset = 0
                    if (compressShift == 0) {
                        cursor = generateExtendedHighWordInstruction(currentNode, _cg, TR::InstOpCode::LHLR,
                            freeHighWordReg, targetRegister, 0, currentInstruction);
                    } else {
                        cursor = generateRIEInstruction(_cg, TR::InstOpCode::RISBHG, currentNode, freeHighWordReg,
                            targetRegister, 0, 31 + 0x80, 32 - compressShift, currentInstruction);
                    }
                } else {
                    cursor = generateExtendedHighWordInstruction(
                        currentNode, _cg, TR::InstOpCode::LHLR, freeHighWordReg, targetRegister, 0, currentInstruction);
                }
            }
            if (debugObj) {
        ...
        if (needMVHI) {
            ...
            if (opCode == TR::InstOpCode::STAM)
                cursor = generateRSInstruction(
                    self()->cg(), opCode, currentNode, targetRegister, targetRegister, tempMR, currentInstruction);
    ...
    TR::Register* virtualRegister, TR::RealRegister::RegNum registerNumber, flags32_t instFlags)
{
    ...
    if (enableHighWordRA) {
        availHighWordRegMap = ~(
            TR::RealRegister::getBitMask(toRealRegister(targetRegister)->getHighWordRegister()->getRegisterNumber()));
    }
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            if (targetRegister->isHighWordRegister() && !virtualRegister->isPlaceholderReg()) {
                ...
                if (virtualRegister->getAssignedRegister() == NULL && currentHighWordReg) {
                    ...
                    cursor = generateExtendedHighWordInstruction(currentNode, _cg, TR::InstOpCode::LHHR,
                    ...
                    if (currentAssignedRegister == NULL) {
                        if (virtualRegister->getTotalUseCount() != virtualRegister->getFutureUseCount()
                            ...
                            TR::MemoryReference* tempMR = generateS390MemoryReference(
                                currentNode, virtualRegister->getBackingStorage()->getSymbolReference(), _cg);
                        ...
                        cursor = generateExtendedHighWordInstruction(currentNode, _cg, TR::InstOpCode::LLHFR,
                            currentAssignedRegister, targetRegister, 0, currentInstruction);
                        _cg->traceRAInstruction(cursor);
                        cursor = generateRILInstruction(
                            _cg, TR::InstOpCode::IIHF, currentNode, currentAssignedRegister, (uintptrj_t)0, cursor);
                        _cg->traceRAInstruction(cursor);
        ...
        if (enableHighWordRA && virtualRegister->is64BitReg()) {
            ...
            if (targetRegisterHW->getState() != TR::RealRegister::Free
                ...
                if (targetRegisterHW->getAssignedRegister()
                    ...
                    // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),_cg->comp(), "currentTargetVirtual is not
                    // LWOnly but HW is clobbered by another vreg?");
                    virtualRegister->block();
                    currentTargetVirtual->block();
            ...
            if (!self()->isAssignable(currentTargetVirtual, currentAssignedRegister) || enableHighWordRA) {
                ...
                if (spareReg == NULL) {
                    ...
                    if (enableHighWordRA && virtualRegister->is64BitReg()) {
                        ...
                        // targetRegister since it will not
                        // free up the full 64 bit register
                        spareReg = self()->freeBestRegister(currentInstruction, currentTargetVirtual,
                ...
                if (enableHighWordRA && virtualRegister->is64BitReg()) {
                    if (targetRegisterHW->getState() != TR::RealRegister::Free
                        ...
                        if (targetRegisterHW->getAssignedRegister()
                            && targetRegisterHW->getAssignedRegister() != targetRegister->getAssignedRegister()) {
                            // TR_ASSERTC( currentTargetVirtual->isLowWordOnly(),_cg->comp(), "currentTargetVirtual is
    ...
    if (targetRegister->getState() != TR::RealRegister::Locked) {
        ...
        if (self()->supportLockedRegisterAssignment()) {
            ...
            if (!_cg->getRAPassAR() && targetRegister->getAssignedRegister() != NULL
                ...
                if ((bestRegister = self()->findBestFreeRegister(
                         currentInstruction, toFreeRegister->getKind(), toFreeRegister, availRegMask))
                    == NULL) {
                    bestRegister = self()->freeBestRegister(
                        currentInstruction, toFreeRegister, toFreeRegister->getKind(), availRegMask);
                }
                self()->registerCopy(currentInstruction, toFreeRegister->getKind(), toRealRegister(targetRegister),
                    bestRegister, _cg, 0);
...
uint64_t OMR::Z::Machine::filterColouredRegisterConflicts(
    TR::Register* targetRegister, TR::Register* siblingRegister, TR::Instruction* currInst)
{
...
void OMR::Z::Machine::initialiseRegisterFile()
{
    ...
    _registerFile[TR::RealRegister::GPR0] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR0, TR::RealRegister::GPR0Mask, _cg);

    _registerFile[TR::RealRegister::GPR1] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR1, TR::RealRegister::GPR1Mask, _cg);

    _registerFile[TR::RealRegister::GPR2] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR2, TR::RealRegister::GPR2Mask, _cg);

    _registerFile[TR::RealRegister::GPR3] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR3, TR::RealRegister::GPR3Mask, _cg);

    _registerFile[TR::RealRegister::GPR4] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR4, TR::RealRegister::GPR4Mask, _cg);

    _registerFile[TR::RealRegister::GPR5] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR5, TR::RealRegister::GPR5Mask, _cg);

    _registerFile[TR::RealRegister::GPR6] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR6, TR::RealRegister::GPR6Mask, _cg);

    _registerFile[TR::RealRegister::GPR7] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR7, TR::RealRegister::GPR7Mask, _cg);

    _registerFile[TR::RealRegister::GPR8] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR8, TR::RealRegister::GPR8Mask, _cg);

    _registerFile[TR::RealRegister::GPR9] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR9, TR::RealRegister::GPR9Mask, _cg);

    _registerFile[TR::RealRegister::GPR10] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR10, TR::RealRegister::GPR10Mask, _cg);

    _registerFile[TR::RealRegister::GPR11] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR11, TR::RealRegister::GPR11Mask, _cg);

    _registerFile[TR::RealRegister::GPR12] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR12, TR::RealRegister::GPR12Mask, _cg);

    _registerFile[TR::RealRegister::GPR13] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR13, TR::RealRegister::GPR13Mask, _cg);

    _registerFile[TR::RealRegister::GPR14] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR14, TR::RealRegister::GPR14Mask, _cg);

    _registerFile[TR::RealRegister::GPR15] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::GPR15, TR::RealRegister::GPR15Mask, _cg);
    ...
    _registerFile[TR::RealRegister::FPR0] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR0, TR::RealRegister::FPR0Mask, _cg);

    _registerFile[TR::RealRegister::FPR1] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR1, TR::RealRegister::FPR1Mask, _cg);

    _registerFile[TR::RealRegister::FPR2] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR2, TR::RealRegister::FPR2Mask, _cg);

    _registerFile[TR::RealRegister::FPR3] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR3, TR::RealRegister::FPR3Mask, _cg);

    _registerFile[TR::RealRegister::FPR4] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR4, TR::RealRegister::FPR4Mask, _cg);

    _registerFile[TR::RealRegister::FPR5] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR5, TR::RealRegister::FPR5Mask, _cg);

    _registerFile[TR::RealRegister::FPR6] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR6, TR::RealRegister::FPR6Mask, _cg);

    _registerFile[TR::RealRegister::FPR7] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR7, TR::RealRegister::FPR7Mask, _cg);

    _registerFile[TR::RealRegister::FPR8] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR8, TR::RealRegister::FPR8Mask, _cg);

    _registerFile[TR::RealRegister::FPR9] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR9, TR::RealRegister::FPR9Mask, _cg);

    _registerFile[TR::RealRegister::FPR10] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR10, TR::RealRegister::FPR10Mask, _cg);

    _registerFile[TR::RealRegister::FPR11] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR11, TR::RealRegister::FPR11Mask, _cg);

    _registerFile[TR::RealRegister::FPR12] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR12, TR::RealRegister::FPR12Mask, _cg);

    _registerFile[TR::RealRegister::FPR13] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR13, TR::RealRegister::FPR13Mask, _cg);

    _registerFile[TR::RealRegister::FPR14] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR14, TR::RealRegister::FPR14Mask, _cg);

    _registerFile[TR::RealRegister::FPR15] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_FPR, 0, TR::RealRegister::Free, TR::RealRegister::FPR15, TR::RealRegister::FPR15Mask, _cg);

    // Initialize Access Regs
    _registerFile[TR::RealRegister::AR0] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR0, TR::RealRegister::AR0Mask, _cg);

    _registerFile[TR::RealRegister::AR1] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR1, TR::RealRegister::AR1Mask, _cg);

    _registerFile[TR::RealRegister::AR2] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR2, TR::RealRegister::AR2Mask, _cg);

    _registerFile[TR::RealRegister::AR3] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR3, TR::RealRegister::AR3Mask, _cg);

    _registerFile[TR::RealRegister::AR4] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR4, TR::RealRegister::AR4Mask, _cg);

    _registerFile[TR::RealRegister::AR5] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR5, TR::RealRegister::AR5Mask, _cg);

    _registerFile[TR::RealRegister::AR6] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR6, TR::RealRegister::AR6Mask, _cg);

    _registerFile[TR::RealRegister::AR7] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR7, TR::RealRegister::AR7Mask, _cg);

    _registerFile[TR::RealRegister::AR8] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR8, TR::RealRegister::AR8Mask, _cg);

    _registerFile[TR::RealRegister::AR9] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR9, TR::RealRegister::AR9Mask, _cg);

    _registerFile[TR::RealRegister::AR10] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR10, TR::RealRegister::AR10Mask, _cg);

    _registerFile[TR::RealRegister::AR11] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR11, TR::RealRegister::AR11Mask, _cg);

    _registerFile[TR::RealRegister::AR12] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR12, TR::RealRegister::AR12Mask, _cg);

    _registerFile[TR::RealRegister::AR13] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR13, TR::RealRegister::AR13Mask, _cg);

    _registerFile[TR::RealRegister::AR14] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR14, TR::RealRegister::AR14Mask, _cg);

    _registerFile[TR::RealRegister::AR15] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::AR15, TR::RealRegister::AR15Mask, _cg);

    // Initialize High Regs
    _registerFile[TR::RealRegister::HPR0] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR0, TR::RealRegister::HPR0Mask, _cg);

    _registerFile[TR::RealRegister::HPR1] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR1, TR::RealRegister::HPR1Mask, _cg);

    _registerFile[TR::RealRegister::HPR2] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR2, TR::RealRegister::HPR2Mask, _cg);

    _registerFile[TR::RealRegister::HPR3] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR3, TR::RealRegister::HPR3Mask, _cg);

    _registerFile[TR::RealRegister::HPR4] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR4, TR::RealRegister::HPR4Mask, _cg);

    _registerFile[TR::RealRegister::HPR5] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR5, TR::RealRegister::HPR5Mask, _cg);

    _registerFile[TR::RealRegister::HPR6] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR6, TR::RealRegister::HPR6Mask, _cg);

    _registerFile[TR::RealRegister::HPR7] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR7, TR::RealRegister::HPR7Mask, _cg);

    _registerFile[TR::RealRegister::HPR8] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR8, TR::RealRegister::HPR8Mask, _cg);

    _registerFile[TR::RealRegister::HPR9] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR9, TR::RealRegister::HPR9Mask, _cg);

    _registerFile[TR::RealRegister::HPR10] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR10, TR::RealRegister::HPR10Mask, _cg);

    _registerFile[TR::RealRegister::HPR11] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR11, TR::RealRegister::HPR11Mask, _cg);

    _registerFile[TR::RealRegister::HPR12] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR12, TR::RealRegister::HPR12Mask, _cg);

    _registerFile[TR::RealRegister::HPR13] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR13, TR::RealRegister::HPR13Mask, _cg);

    _registerFile[TR::RealRegister::HPR14] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR14, TR::RealRegister::HPR14Mask, _cg);

    _registerFile[TR::RealRegister::HPR15] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_GPR, 0, TR::RealRegister::Free, TR::RealRegister::HPR15, TR::RealRegister::HPR15Mask, _cg);
    ...
    _registerFile[TR::RealRegister::VRF16] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF16, TR::RealRegister::VRF16Mask, _cg);

    _registerFile[TR::RealRegister::VRF17] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF17, TR::RealRegister::VRF17Mask, _cg);

    _registerFile[TR::RealRegister::VRF18] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF18, TR::RealRegister::VRF18Mask, _cg);

    _registerFile[TR::RealRegister::VRF19] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF19, TR::RealRegister::VRF19Mask, _cg);

    _registerFile[TR::RealRegister::VRF20] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF20, TR::RealRegister::VRF20Mask, _cg);

    _registerFile[TR::RealRegister::VRF21] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF21, TR::RealRegister::VRF21Mask, _cg);

    _registerFile[TR::RealRegister::VRF22] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF22, TR::RealRegister::VRF22Mask, _cg);

    _registerFile[TR::RealRegister::VRF23] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF23, TR::RealRegister::VRF23Mask, _cg);

    _registerFile[TR::RealRegister::VRF24] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF24, TR::RealRegister::VRF24Mask, _cg);

    _registerFile[TR::RealRegister::VRF25] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF25, TR::RealRegister::VRF25Mask, _cg);

    _registerFile[TR::RealRegister::VRF26] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF26, TR::RealRegister::VRF26Mask, _cg);

    _registerFile[TR::RealRegister::VRF27] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF27, TR::RealRegister::VRF27Mask, _cg);

    _registerFile[TR::RealRegister::VRF28] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF28, TR::RealRegister::VRF28Mask, _cg);

    _registerFile[TR::RealRegister::VRF29] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF29, TR::RealRegister::VRF29Mask, _cg);

    _registerFile[TR::RealRegister::VRF30] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF30, TR::RealRegister::VRF30Mask, _cg);

    _registerFile[TR::RealRegister::VRF31] = new (_cg->trHeapMemory())
        TR::RealRegister(TR_VRF, 0, TR::RealRegister::Free, TR::RealRegister::VRF31, TR::RealRegister::VRF31Mask, _cg);
...
uint32_t* OMR::Z::Machine::initializeGlobalRegisterTable()
{
    ...
    if (!comp->getOption(TR_DisableRegisterPressureSimulation)) {
        ...
        if (dontInitializeGlobalRegisterTableFromLinkage) {
            ...
            //
            // p = addGlobalReg(TR::RealRegister::GPR0, p); // Local register assigner can't handle virtuals assigned to
            // GPR0 appearing in memrefs
            ...
            //
            p = self()->addGlobalReg(
                TR::RealRegister::GPR6, p); // NOTE: GPR6 must be avoided if on-demand literal pool opt isn't run
            static char* noGraFIX = feGetEnv("TR_NOGRAFIX");
            // Exclude GPR7 if we are not on Freeway+ hardware
            if (!noGraFIX && !comp->getOption(TR_DisableLongDispStackSlot) && _cg->getExtCodeBaseRegisterIsFree()) {
...
//
            ...
            if ((_cg->isLiteralPoolOnDemandOn() && !linkage->isZLinuxLinkageType())
                || (_cg->isLiteralPoolOnDemandOn() && !linkage->getPreserved(linkage->getLitPoolRegister())))
            ...
            if (linkage->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
            {
                for (i = TR::RealRegister::LastAssignableGPR; i >= TR::RealRegister::FirstGPR; --i) {
                    ...
                    if (linkage->getPreserved(reg)) {
                        // Dangling else above
                        if (reg != linkage->getStaticBaseRegister() && reg != linkage->getPrivateStaticBaseRegister()
                ...
                for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
                    ...
                    if (linkage->getPreserved(reg)) {
                        // Dangling else above
                        if (reg != linkage->getLitPoolRegister() && reg != linkage->getStaticBaseRegister()
            ...
            if (_cg->globalAccessRegistersSupported()) {
                ...
                for (i = self()->getFirstGlobalGPRRegisterNumber(); i <= self()->getLastGlobalGPRRegisterNumber();
                     i++) {
                    reg = (TR::RealRegister::RegNum)(_globalRegisterNumberToRealRegisterMap[i]
    ...
    //   _globalRegisterNumberToRealRegisterMap = new uint32_t[NUM_S390_GPR+NUM_S390_FPR]; // Make room for max num GPRs
    //   + FPRs
    ...
    //
    if (TR::Compiler->target.is64Bit() || !comp->getOption(TR_Enable390AccessRegs)) {
        for (int32_t i = self()->getFirstGlobalAccessRegisterNumber(); i <= self()->getLastGlobalAccessRegisterNumber();
             ++i) {
            _globalRegisterNumberToRealRegisterMap[i] = (uint32_t)(-1);
        }
    }
 ...
 * conventions
 * Each register is explicitly set to stay consistent with a majority of the global register number setup procedures
 * across z-supported
...
void OMR::Z::Machine::releaseGlobalRegister(int32_t globalRegisterTableIndex, TR::RealRegister::RegNum gReg)
{
...
TR_GlobalRegisterNumber OMR::Z::Machine::setFirstGlobalAccessRegisterNumber(TR_GlobalRegisterNumber reg)
{
    self()->setFirstGlobalRegisterNumber(TR_AR, reg);
    return _firstGlobalAccessRegisterNumber = reg;
}

TR_GlobalRegisterNumber OMR::Z::Machine::setLastGlobalAccessRegisterNumber(TR_GlobalRegisterNumber reg)
{
...
TR::Register* OMR::Z::Machine::getAccessRegisterFromGlobalRegisterNumber(TR_GlobalRegisterNumber reg)
{
    if (reg >= self()->getFirstGlobalAccessRegisterNumber() && reg <= self()->getLastGlobalAccessRegisterNumber()
...
{
    if (reg >= self()->getFirstGlobalGPRRegisterNumber() && reg <= self()->getFirstGlobalHPRRegisterNumber()
        && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
...
{
    if (reg >= self()->getFirstGlobalHPRRegisterNumber() && reg <= self()->getLastGlobalGPRRegisterNumber()
        && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
...
void OMR::Z::Machine::createRegisterAssociationDirective(TR::Instruction* cursor)
{
    ...
    if (_cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        ...
        associations = new (_cg->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
    ...
    TR::Instruction* cursor1 = new (_cg->trHeapMemory(), TR_MemoryBase::S390Instruction)
...
{
    if ((regNum == TR::RealRegister::ArGprPair) || (regNum == TR::RealRegister::ArOfArGprPair)
...
bool OMR::Z::Machine::isRestrictedReg(TR::RealRegister::RegNum reg)
{
    static const TR::RealRegister::RegNum regList[] = {
        TR::RealRegister::GPR9, TR::RealRegister::GPR10, TR::RealRegister::GPR11, TR::RealRegister::GPR12,
    };
    TR::Compilation* comp = TR::comp();
    ...
    if (numRestrictedRegs < 0 || numRestrictedRegs > regListSize) {
        ...
        if (!printed) {
            fprintf(stderr,
                "Invalid value for numRestrictedRegs or on-demand lit pool is disabled. Needs to range from 0 to %d\n",
                regListSize);
            printed = true;
        }
#endif
...
TR::RealRegister* OMR::Z::Machine::getRegisterFile(int32_t i) { return _registerFile[i]; }
    ...
    for (int32_t i = TR::RealRegister::FirstGPR; i < TR::RealRegister::NumRegisters; i++) {
        ...
        if (_registerFile[i]->getState() == TR::RealRegister::Free) {
            ...
            if (_registerFile[i]->getAssignedRegister() != NULL
                ...
                //  GPR15) to GPR12.
                //  When we subsequently handle _registerFile[15], we cannot blindly reset GPR_3545's assigned register
                //  to NULL,
    ...
    TR::list<TR::Register*>* spilledRegisterList)
{
    ...
    if (_cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
        for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR; i++) {
            if (self()->getS390RealRegister(i)->getState() == TR::RealRegister::Assigned
                && self()->getS390RealRegister(i)->getAssignedRegister()) {
                if (self()->getS390RealRegister(i)->getAssignedRegister()
                    != self()->getS390RealRegister(i)->getLowWordRegister()->getAssignedRegister())
                    c++;
    ...
    if (c) {
        deps = new (_cg->trHeapMemory(), TR_MemoryBase::RegisterDependencyConditions)
        ...
        if (_cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            for (i = TR::RealRegister::FirstHPR; i <= TR::RealRegister::LastHPR; i++) {
                if (self()->getS390RealRegister(i)->getState() == TR::RealRegister::Assigned
                    ...
                    if (self()->getS390RealRegister(i)->getAssignedRegister()
                        != self()->getS390RealRegister(i)->getLowWordRegister()->getAssignedRegister()) {
                        deps->addPostCondition(self()->getS390RealRegister(i)->getAssignedRegister(),
