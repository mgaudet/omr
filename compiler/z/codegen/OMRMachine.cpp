...
OMR::Z::Machine::genBitVectOfLiveGPRPairs()
{
    ...
    for (int32_t i = first; i <= last; i += 2) {
        if (_registerFile[i]->getState() != TR::RealRegister::Free || _registerFile[i + 1]->getState() != TR::RealRegister::Free) {
...
bool OMR::Z::Machine::isLegalEvenOddRestrictedPair(TR::RealRegister* evenReg, TR::RealRegister* oddReg, uint64_t availRegMask)
{
    if (evenReg == NULL || oddReg == NULL) {
        return false;
    }
...
bool OMR::Z::Machine::isLegalEvenRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    if (regNum % 2 != 0 || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() + 1]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
bool OMR::Z::Machine::isLegalFirstOfFPRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    if ((regNum % 4 != 0 && regNum % 4 != 1) || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() + 2]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
bool OMR::Z::Machine::isLegalSecondOfFPRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    // 2) its sibling FP Reg is NOT locked/blocked
    if ((regNum % 4 != 2 && regNum % 4 != 3) || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 2]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
OMR::Z::Machine::findBestLegalOddRegister(uint64_t availRegMask)
{
    ...
    for (int32_t i = first + 1; i <= last; i += 2) {
        ...
        if (_registerFile[i - 1]->getState() == TR::RealRegister::Locked || _registerFile[i]->getState() == TR::RealRegister::Locked || _registerFile[i - 1]->getState() == TR::RealRegister::Blocked || _registerFile[i]->getState() == TR::RealRegister::Blocked || (TR::RealRegister::getBitMask(i) & availRegMask) == 0 || (TR::RealRegister::getBitMask(i - 1) & availRegMask) == 0) {
            continue;
        }
        ...
        if ((_registerFile[i]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i - 1]->getState() == TR::RealRegister::Free || _registerFile[i - 1]->getState() == TR::RealRegister::Unlatched) && _registerFile[i]->getWeight() < bestWeightSoFar) {
...
bool OMR::Z::Machine::isLegalOddRegister(TR::RealRegister* reg, bool allowBlocked, uint64_t availRegMask, bool allowLocked)
{
    ...
    if (regNum % 2 != 1 || (TR::RealRegister::getBitMask(toRealRegister(reg)->getRegisterNumber()) & availRegMask) == 0 || _registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Locked && allowLocked == false || (_registerFile[toRealRegister(reg)->getRegisterNumber() - 1]->getState() == TR::RealRegister::Blocked && allowBlocked == false)) {
        return false;
    } else {
        return true;
    }
...
OMR::Z::Machine::findBestLegalEvenRegister(uint64_t availRegMask)
{
    ...
    for (int32_t i = first; i <= last; i += 2) {
        // Don't consider registers that can't be assigned.
        if (_registerFile[i]->getState() == TR::RealRegister::Locked || _registerFile[i + 1]->getState() == TR::RealRegister::Locked || _registerFile[i]->getState() == TR::RealRegister::Blocked || _registerFile[i + 1]->getState() == TR::RealRegister::Blocked || (TR::RealRegister::getBitMask(i) & availRegMask) == 0 || (TR::RealRegister::getBitMask(i + 1) & availRegMask) == 0) {
            continue;
        }
    ...
    uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && kindOfRegister != TR_FPR && kindOfRegister != TR_VRF;
    ...
    if (assignedRegister == NULL) {
        ...
        if ((comp->getOption(TR_EnableTrueRegisterModel)) && targetRegister->isLive() && (targetRegister->isValueLiveOnExit() || !targetRegister->isNotUsedInThisBB()))
    ...
    if (doBookKeeping && (assignedRegister->getState() != TR::RealRegister::Locked || self()->supportLockedRegisterAssignment())) {
        ...
        if ((targetRegister->getFutureUseCount() == 0) || killOOLReg || ((comp->getOption(TR_EnableTrueRegisterModel)) && currInst->startOfLiveRange(targetRegister))) {
#if DEBUG
            if (comp->getOption(TR_EnableTrueRegisterModel) && currInst->startOfLiveRange(targetRegister) && targetRegister->getFutureUseCount() > 0 && !currInst->getOpCode().isRegCopy()) {
                traceMsg(comp,
    ...
    uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && regPairKind != TR_FPR && regPairKind != TR_VRF;
    ...
    if (!comp->getOption(TR_EnableTrueRegisterModel) && freeRegisterLow == NULL && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount()) && freeRegisterHigh == NULL && (firstReg->getTotalUseCount() != firstReg->getFutureUseCount())) {
    ...
    // True register model will mark register with a pending restoreSpillState and only when we see a def of this
    // register will we store to spill.
    if (freeRegisterLow == NULL && (comp->getOption(TR_EnableTrueRegisterModel)) && lastReg->isLive() && (lastReg->isValueLiveOnExit() || !lastReg->isNotUsedInThisBB()))
        lastReg->setPendingSpillOnDef();
    if (freeRegisterHigh == NULL && (comp->getOption(TR_EnableTrueRegisterModel)) && firstReg->isLive() && (firstReg->isValueLiveOnExit() || !firstReg->isNotUsedInThisBB()))
        firstReg->setPendingSpillOnDef();

    // TotalUse & FutureUse are only equal upon the first assignment.  Hence, if they arn't
    // the same, and there is no assigned reg, we must have spilled the reg, so invert
    // the spill state to get the reg back
    if (freeRegisterLow == NULL && !comp->getOption(TR_EnableTrueRegisterModel) && (lastReg->getTotalUseCount() != lastReg->getFutureUseCount())) {
        if (freeRegisterHigh) {
    ...
    // the same, and there is no assigned reg, we must have spilled the reg, so invert
    // the spill state to get the reg back
    if (freeRegisterHigh == NULL && !comp->getOption(TR_EnableTrueRegisterModel) && (firstReg->getTotalUseCount() != firstReg->getFutureUseCount())) {
        if (freeRegisterLow) {
            freeRegisterLow->block();
        }
    ...
    if (freeRegisterHigh == NULL) {
        ...
        if (freeRegisterLow && TR_FPR != regPair->getKind() && self()->isLegalOddRegister(freeRegisterLow, DISALLOWBLOCKED, availRegMask) && toRealRegister(toRealRegister(freeRegisterLow)->getSiblingRegister())->getState() == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        else if (freeRegisterLow && TR_FPR == regPair->getKind() && self()->isLegalSecondOfFPRegister(freeRegisterLow, DISALLOWBLOCKED, availRegMask) && toRealRegister(toRealRegister(freeRegisterLow)->getSiblingRegister())->getState() == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        // if nothing is using my coloured register use it
    ...
    if (freeRegisterLow == NULL) {
        ...
        // Lets check to see if partner is assigned to a legal register and partner's match is free
        bool colouredIsGood = true;
        if (freeRegisterHigh && TR_FPR != regPair->getKind() && self()->isLegalEvenRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask) && toRealRegister(toRealRegister(freeRegisterHigh)->getSiblingRegister())->getState() == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        else if (freeRegisterHigh && TR_FPR == regPair->getKind() && self()->isLegalFirstOfFPRegister(freeRegisterHigh, DISALLOWBLOCKED, availRegMask) && toRealRegister(toRealRegister(freeRegisterHigh)->getSiblingRegister())->getState() == TR::RealRegister::Free)
            colouredIsGood = false; // Default algorithm should pick a better register
        // if nothing is using my coloured register use it
    ...
    if (doBookKeeping) {
        firstReg->setIsLive();
        if (((firstReg->decFutureUseCount() == 0) || ((comp->getOption(TR_EnableTrueRegisterModel)) && currInst->startOfLiveRange(firstReg)) || (self()->cg()->isOutOfLineHotPath() && firstReg->getStartOfRange() == currInst)) && (freeRegisterHigh->getState() != TR::RealRegister::Locked || self()->supportLockedRegisterAssignment())) {
        ...
        lastReg->setIsLive();
        if (((lastReg->decFutureUseCount() == 0) || ((comp->getOption(TR_EnableTrueRegisterModel)) && currInst->startOfLiveRange(lastReg)) || (self()->cg()->isOutOfLineHotPath() && lastReg->getStartOfRange() == currInst)) && (freeRegisterLow->getState() != TR::RealRegister::Locked || self()->supportLockedRegisterAssignment())) {
            lastReg->resetIsLive();
    ...
    TR_RegisterKinds rk, TR::Instruction* currInst, uint64_t availRegMask)
{
    ...
    bool enableHighWordRA = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && rk != TR_FPR && rk != TR_VRF;
    ...
    if (rk != TR_FPR && rk != TR_VRF) {
        ...
        for (int32_t i = first; i <= last; i += 2) {
            // Don't consider registers that can't be assigned.
            if ((_registerFile[i]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 1]->getState() == TR::RealRegister::Locked) || (TR::RealRegister::getBitMask(i) & availRegMask) == 0 || (TR::RealRegister::getBitMask(i + 1) & availRegMask) == 0) {
                continue;
            }
            ...
            if (enableHighWordRA && rk == TR_GPR64) {
                ...
                if ((_registerFile[i]->getHighWordRegister()->getState() == TR::RealRegister::Free || _registerFile[i]->getHighWordRegister()->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 1]->getHighWordRegister()->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getHighWordRegister()->getState() == TR::RealRegister::Unlatched))
                    highWordPairIsFree = true;
            }

            // See if this pair is available, and better than the prev
            if ((_registerFile[i]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 1]->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) && highWordPairIsFree && ((_registerFile[i]->getWeight() + _registerFile[i + 1]->getWeight()) < bestWeightSoFar)) {
    ...
    } else // if FP Reg Pair
    {
        ...
        for (int32_t k = 0; k < NUM_S390_FPR_PAIRS; k++) {
            ...
            // Don't consider registers that can't be assigned.
            if ((_registerFile[i]->getState() == TR::RealRegister::Locked) || (_registerFile[i + 2]->getState() == TR::RealRegister::Locked) || (TR::RealRegister::getBitMask(i) & availRegMask) == 0 || (TR::RealRegister::getBitMask(i + 2) & availRegMask) == 0) {
                continue;
            }
            ...
            // See if this pair is available, and better than the prev
            if ((_registerFile[i]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && (_registerFile[i + 2]->getState() == TR::RealRegister::Free || _registerFile[i + 2]->getState() == TR::RealRegister::Unlatched) && ((_registerFile[i]->getWeight() + _registerFile[i + 2]->getWeight()) < bestWeightSoFar)) {
                freeRegisterHigh = _registerFile[i];
    ...
    uint64_t availRegMask)
{
    ...
    if (bestVirtCandidateLow != NULL) {
        // True register model will mark register with a pending restoreSpillState and only when we see a def of this
        // register will we store to spill.
        if ((comp->getOption(TR_EnableTrueRegisterModel)) && (bestVirtCandidateLow->isValueLiveOnExit() || !bestVirtCandidateLow->isNotUsedInThisBB()))
    ...
    if (bestVirtCandidateHigh != NULL) {
        // True register model will mark register with a pending restoreSpillState and only when we see a def of this
        // register will we store to spill.
        if ((comp->getOption(TR_EnableTrueRegisterModel)) && (bestVirtCandidateHigh->isValueLiveOnExit() || !bestVirtCandidateHigh->isNotUsedInThisBB()))
    ...
    uint64_t availRegMask)
{
    ...
    for (int32_t i = first; i <= last; i += 2) {
        ...
        // Priority to assigned/unassigned pairs.
        if ((_registerFile[i]->getState() == TR::RealRegister::Free || _registerFile[i]->getState() == TR::RealRegister::Unlatched) && _registerFile[i + 1]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[i];
            bestCandidateLow = _registerFile[i + 1];
        } else if ((_registerFile[i + 1]->getState() == TR::RealRegister::Free || _registerFile[i + 1]->getState() == TR::RealRegister::Unlatched) && _registerFile[i]->getState() == TR::RealRegister::Assigned) {
            bestCandidateHigh = _registerFile[i];
            bestCandidateLow = _registerFile[i + 1];
        }
        // Both assigned
    ...
    bool needsHighWord)
{
    ...
    bool enableHighWordRA = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && (rk == TR_GPR || rk == TR_GPR64);
...
void OMR::Z::Machine::freeRealRegister(TR::Instruction* currentInstruction, TR::RealRegister* targetReal, bool is64BitReg)
{
    ...
    bool enableHighWordRA = _cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && targetReal->getKind() != TR_FPR && targetReal->getKind() != TR_VRF;
...
void OMR::Z::Machine::spillRegister(TR::Instruction* currentInstruction, TR::Register* virtReg, uint32_t availHighWordRegMap)
{
...

    {
        ...
        case TR_GPR:
            ...
            //if we selected the VM Thread Register to be freed, check to see if the value has already been spilled
            else if (comp->getOption(TR_DisableOOL) && _cg->needsVMThreadDependency() && virtReg == _cg->getVMThreadRegister()) {
...
bool OMR::Z::Machine::isAssignable(TR::Register* virtReg, TR::RealRegister* realReg)
{
    if (virtReg->isUsedInMemRef() && realReg == _registerFile[TR::RealRegister::GPR0]) {
        ...
        if (_cg->supportsHighWordFacility() && !TR::comp()->getOption(TR_DisableHighWordRA) && virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF) {
    ...
    flags32_t instFlags)
{
    ...
    if (targetRegister->getState() != TR::RealRegister::Locked) {
        ...
        if (self()->supportLockedRegisterAssignment()) {
            ...
            if (!_cg->getRAPassAR() && targetRegister->getAssignedRegister() != NULL && targetRegister->getAssignedRegister() != targetRegister) {
    ...
    TR::Instruction* currInst)
{
    ...
    if (currInst->defsAnyRegister(targetRegister)) {
        ...
        for (auto reg = conflictRegs.begin(); reg != conflictRegs.end(); ++reg) {
            ...
            if (cr && targetRegister != (*reg) && (*reg)->getAssignedRegister() != targetRegister && (siblingRegister == NULL || (*reg) != siblingRegister))
...
//
            ...
            if (linkage->isZLinuxLinkageType()) // ordering seems to make crashes on zos.
            {
                for (i = TR::RealRegister::LastAssignableGPR; i >= TR::RealRegister::FirstGPR; --i) {
                    ...
                    if (linkage->getPreserved(reg)) {
                        // Dangling else above
                        if (reg != linkage->getStaticBaseRegister() && reg != linkage->getPrivateStaticBaseRegister() && reg != linkage->getStackPointerRegister())
                ...
                for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastAssignableGPR; i++) {
                    ...
                    if (linkage->getPreserved(reg)) {
                        // Dangling else above
                        if (reg != linkage->getLitPoolRegister() && reg != linkage->getStaticBaseRegister() && reg != linkage->getPrivateStaticBaseRegister() && reg != linkage->getStackPointerRegister())
                            p = self()->addGlobalReg(reg, p);
                    }
                }
            }
    ...
    //
    if (
        TR::Compiler->target.is64Bit() || !comp->getOption(TR_Enable390AccessRegs)) {
...
{
    if (
        reg >= self()->getFirstGlobalAccessRegisterNumber() && reg <= self()->getLastGlobalAccessRegisterNumber() && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
...
{
    if (
        reg >= self()->getFirstGlobalGPRRegisterNumber() && reg <= self()->getFirstGlobalHPRRegisterNumber() && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
...
{
    if (
        reg >= self()->getFirstGlobalHPRRegisterNumber() && reg <= self()->getLastGlobalGPRRegisterNumber() && _globalRegisterNumberToRealRegisterMap[reg] >= 0) {
        return _registerFile[_globalRegisterNumberToRealRegisterMap[reg]];
    }
...
{
    if ((regNum == TR::RealRegister::ArGprPair) || (regNum == TR::RealRegister::ArOfArGprPair) || (regNum == TR::RealRegister::GprOfArGprPair)) {
