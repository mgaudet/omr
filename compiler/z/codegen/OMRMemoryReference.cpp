 ...
 * So a premature LAY or LA/AHI fixup is not done for these negative offsets a flag HasTemporaryNegativeOffset is set on
 * the memRef to prevent this fixup
 * However, if there is already a real negative offset then the fixup is required for this part of the negative offset
 * To deal with this first split off the real negative offset by calling enforceSSFormatLimits and then set the
 * HasTemporaryNegativeOffset flag and add in the new negative offset
 */
void OMR::Z::MemoryReference::addToTemporaryNegativeOffset(TR::Node* node, int32_t offset, TR::CodeGenerator* cg)
{
    ...
    if (offset < 0) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\taddToTemporaryNegativeOffset : %s (%p) new offset %d, existing mr offset %d (existing mr "
                           "hasTempNegOffset = %s)\n",
                node ? node->getOpCode().getName() : "NULL", node, offset, self()->getOffset(),
                self()->hasTemporaryNegativeOffset() ? "yes" : "no");
        if (self()->getOffset() < 0 && !self()->hasTemporaryNegativeOffset()) // if there is a 'real' negative offset
                                                                              // then deal with this first before adding
                                                                              // in the temporary negative offset
        {
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\t\texisting mr->offset %d < 0 so call enforceSSFormatLimits to clear this up before "
                               "setting HasTemporaryNegativeOffset\n",
                    self()->getOffset());
...
bool OMR::Z::MemoryReference::setForceFoldingIfAdvantageous(TR::CodeGenerator* cg, TR::Node* addressChild)
{
    ...
    if (!addressChild->getOpCode().isAdd()) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) is not an add type - most "
                           "likely cannot fold\n",
                addressChild->getOpCode().getName(), addressChild);
        }
        return false;
    ...
    if (!addressChild->getSecondChild()->getOpCode().isLoadConst()) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) second child is not a load "
                           "const - cannot fold\n",
                addressChild->getOpCode().getName(), addressChild);
        }
        return false;
    ...
    if (addressChild->getSecondChild()->getRegister() != NULL) {
        ...
        //    iconst (reg)
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) const child %s (%p) has "
    ...
    if (addressChild->getReferenceCount() != 1) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) has ref count %d, cannot fold, "
                           "need value in register.\n",
    ...
    if (haveToEvalConvIntoRegister) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. with ref "
                           "count > 1 and no register, must evaluate - setForceFolding()\n",
                addressChild->getOpCode().getName(), addressChild);
        }
    ...
    if (!allConvNodesAreUnneeded) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) first child has conv. which "
                           "are needed, folding after evaluation.\n",
                addressChild->getOpCode().getName(), addressChild);
        }
        cg->evaluate(addressChild->getFirstChild());
        self()->setForceFolding();
    ...
    if (!eventualNonConversionIsSuitableForFolding) {
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, addressChild %s (%p) eventual non conversion %s "
                           "(%p) is not suitable for folding.\n",
    ...
    if (!eventualNonConversion->getOpCode().isIndirect()) {
        ...
        //    iconst
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and "
                           "refCount==1 and is a aload+const so setForceFolding=true\n",
    ...
    } else // if (eventualNonConversion->getReferenceCount() == 1)
    {
        ...
        if (cg->traceBCDCodeGen()) {
            traceMsg(comp, " inside setForceFoldingIfAdvantageous, eventualNonConversion %s (%p) has no register and "
                           "refCount==1 and is an iaload+const so eval(%s - %p) and setForceFolding=true\n",
                eventualNonConversion->getOpCode().getName(), eventualNonConversion,
 ...
 * the reference count decrementing that evaluators do) will miss nodes that are part of the incremented nodes list and
 * that are under a node that
 * was assigned a register after memory reference population. This function will descend the tree and look for this
 * case, only removing nodes if a
 * node with a register was seen. It is important to only decend down nodes that are part of the incremented nodes list,
...
TR_S390ConstantDataSnippet* OMR::Z::MemoryReference::createLiteralPoolSnippet(TR::Node* node, TR::CodeGenerator* cg)
{
...
TR_S390TargetAddressSnippet* OMR::Z::MemoryReference::getTargetAddressSnippet()
{
    return self()->isTargetAddressSnippet() ? (TR_S390TargetAddressSnippet*)_targetSnippet : NULL;
}

TR_S390TargetAddressSnippet* OMR::Z::MemoryReference::setTargetAddressSnippet(TR_S390TargetAddressSnippet* s)
{
    self()->setTargetAddressSnippet();
    return (TR_S390TargetAddressSnippet*)(_targetSnippet = s);
}

TR_S390ConstantDataSnippet* OMR::Z::MemoryReference::getConstantDataSnippet()
{
    return self()->isConstantDataSnippet() ? (TR_S390ConstantDataSnippet*)_targetSnippet : NULL;
}

TR_S390ConstantDataSnippet* OMR::Z::MemoryReference::setConstantDataSnippet(TR_S390ConstantDataSnippet* s)
{
    self()->setConstantDataSnippet();
    return (TR_S390ConstantDataSnippet*)(_targetSnippet = s);
}

TR_S390LookupSwitchSnippet* OMR::Z::MemoryReference::getLookupSwitchSnippet()
{
    return self()->isLookupSwitchSnippet() ? (TR_S390LookupSwitchSnippet*)_targetSnippet : NULL;
}

TR_S390LookupSwitchSnippet* OMR::Z::MemoryReference::setLookupSwitchSnippet(TR_S390LookupSwitchSnippet* s)
{
    ...
    for (uint32_t i = 0; i < nodeArray.size(); i++) {
        ...
        if ((nodeArray[i]->getReferenceCount() == 1) && (nodeArray[i]->getRegister() != NULL)) {
            if (comp->getOption(TR_TraceCG)) {
                traceMsg(comp, " ArtificiallyInflateReferenceCountWhenNecessary: bug potential with node %p (reg %p "
                               "%s%s): ref count after %s is 1.\n",
            ...
            if (incrementedNodesList) {
                ...
                if (listNode == nodeArray[i]) {
                    ...
                    // somewhere else.
                    if (comp->getOption(TR_TraceCG)) {
                        traceMsg(comp, " ArtificiallyInflateReferenceCountWhenNecessary: skip inflating node %p (reg "
                                       "%p %s%s) because it exists in incrementedNodesList.\n",
                            nodeArray[i], nodeArray[i]->getRegister(),
                            nodeArray[i]->getRegister() ? (nodeArray[i]->getRegister()->isPlaceholderReg() ? "D_" : "")
                                                        : "?",
                            comp->getDebug()->getName(nodeArray[i]->getRegister()), functionString);
...
bool OMR::Z::MemoryReference::tryBaseIndexDispl(TR::CodeGenerator* cg, TR::Node* loadStore, TR::Node* topAdd)
{
    ...
    if (!loadStore->getOpCode().isLoadVar() && !loadStore->getOpCode().isStore())
        return false;
    if (topAdd->getRegister() == NULL && topAdd->getOpCode().hasSymbolReference()
...
bool OMR::Z::MemoryReference::alignmentBumpMayRequire4KFixup(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (_offset < MAXDISP && // skip if going to fixup naturally anyway
        ...
        int32_t maxAlignmentBump = self()->getFixedSizeForAlignment() - 1; // alignment is getFixedSizeForAlignment() -
                                                                           // (length or leftMostByte) and each of these
                                                                           // must be at least 1
        if (_offset + maxAlignmentBump >= MAXDISP) {
            if (cg->traceBCDCodeGen())
                traceMsg(cg->comp(), "\tz^z : node %s (%p) : _offset %d + maxAlignmentBump %d >= MAXDISP %d : (%d >= "
                                     "%d) -- force 4K fixup\n",
...
void OMR::Z::MemoryReference::assignRegisters(TR::Instruction* currentInstruction, TR::CodeGenerator* cg)
{
    ...
    if (_indexRegister && _indexRegister->isArGprPair() && _baseRegister == NULL) {
        self()->setBaseRegister(_indexRegister, cg);
        _indexRegister = NULL;
    }
    ...
    if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL) {
        ...
        if (_baseRegister != NULL && _baseRegister->getRealRegister() == NULL)
            assignedBaseRegister = (TR::RealRegister*)machine->assignBestRegisterSingle(_baseRegister,
                currentInstruction, NOBOOKKEEPING, ~(TR::RealRegister::getBitMask(TR::RealRegister::GPR0)));
    ...
    if (_indexRegister != NULL && _indexRegister->getRealRegister() == NULL) {
        ...
        assignedIndexRegister = (TR::RealRegister*)machine->assignBestRegisterSingle(
            _indexRegister, currentInstruction, BOOKKEEPING, ~(TR::RealRegister::getBitMask(TR::RealRegister::GPR0)));
 ...
 * 0         8     16   20       32  36          47
 */
uint8_t* OMR::Z::MemoryReference::generateBinaryEncoding2(uint8_t* cursor, TR::CodeGenerator* cg)
{
...
int32_t OMR::Z::MemoryReference::calcDisplacement(uint8_t* cursor, TR::Instruction* instr, TR::CodeGenerator* cg)
{
    ...
    if (self()->leftAlignMemRef()) {
        ...
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "instr %p : left aligning memRef with symRef #%d (%s, isTemp %s), _offset %d : bump = "
    ...
    if (self()->getSymbolReference()->isTempVariableSizeSymRef()) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "instr %p : bumping tempVariableSizeSymRef memRef with symRef #%d (%s), _offset %d : bump = "
                           "symSize - totalSize = %d - %d = ",
                instr, self()->getSymbolReference()->getReferenceNumber(),
    ...
    if (snippet != NULL) {
        {
            ...
            if (cursor != NULL) {
                cg->addRelocation(
                    new (cg->trHeapMemory()) TR_12BitLabelRelativeRelocation(cursor, snippet->getSnippetLabel()));
            }
        }
    } else if (symbol != NULL) {
        if (symbol->isRegisterMappedSymbol()) {
...
int32_t OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t* cursor, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    ...
    if (disp < 0 || disp >= MAXDISP) {
        ...
        if ((instr->hasLongDisplacementSupport()
            ...
            if (TR::Compiler->target.is64Bit()) {
                ...
                *(int32_t*)cursor = boi(0xa7090000 | (disp & 0xFFFF)); // LGHI Rscrtch, disp&0xFFFF
                ...
                if (!comp->getOption(TR_DisableLongDispStackSlot) && spillNeeded) {
                    *(int32_t*)cursor
                        = boi(0x50005000 | (offsetToLongDispSlot & 0xFFF)); // ST Rscrtch,offToLongDispSlot(,GPR5)
                    scratchReg->setRegisterField((uint32_t*)cursor);
                    cursor += 4;
                    nbytes += 4;
                }
            ...
            if (TR::Compiler->target.is64Bit()) {
                if (!comp->getOption(TR_DisableLongDispStackSlot) && spillNeeded) {
                    *(int32_t*)cursor
                        = (0xE3005000 | (offsetToLongDispSlot & 0xFFF)); // STG Rscrtch,offToLongDispSlot(,GPR5)
                    scratchReg->setRegisterField((uint32_t*)cursor);
                    cursor += 4;
                    nbytes += 4;
                    *(int16_t*)cursor = bos(0x0024);
        ...
        {
            TR_ASSERT(0, "OMR::Z::MemoryReference::generateBinaryEncoding -- 0x%x displacement not handled\n", disp);
        }

        // Now we need to figure out how to use the manufactured disp
        if (largeDisp) {
            if (instr->getKind() == TR::Instruction::IsRS || instr->getKind() == TR::Instruction::IsRSY
                || instr->getKind() == TR::Instruction::IsSI || instr->getKind() == TR::Instruction::IsSIY
                || instr->getKind() == TR::Instruction::IsSIL || instr->getKind() == TR::Instruction::IsSS1
                || instr->getKind() == TR::Instruction::IsSS2 || instr->getKind() == TR::Instruction::IsSS4
                || instr->getKind() == TR::Instruction::IsS) {
                TR_ASSERT(base != NULL, "OMR::Z::MemoryReference::generateBinaryEncoding --  Expected non-NULL base "
                                        "reg for RS/S/SI type inst.");
            ...
            {
                TR_ASSERT(instr->getKind() == TR::Instruction::IsRX || instr->getKind() == TR::Instruction::IsVRX
                        || instr->getKind() == TR::Instruction::IsRXY || instr->getKind() == TR::Instruction::IsRXYb
                        || instr->getKind() == TR::Instruction::IsRXE || instr->getKind() == TR::Instruction::IsRXF,
            ...
            {
                TR_ASSERT(instr->getKind() == TR::Instruction::IsRX || instr->getKind() == TR::Instruction::IsRXY
                        || instr->getKind() == TR::Instruction::IsRXYb || instr->getKind() == TR::Instruction::IsRXE
    ...
    if (instr->getKind() == TR::Instruction::IsRXY || instr->getKind() == TR::Instruction::IsRXYb
        || instr->getKind() == TR::Instruction::IsRSY || instr->getKind() == TR::Instruction::IsSIY) {
    ...
    if (!comp->getOption(TR_DisableLongDispStackSlot)) {
        ...
        if (largeDisp) {
            TR_ASSERT(instr->getOpCode().isBranchOp() == false, "OMR::Z::MemoryReference::generateBinaryEncoding -- We "
                                                                "don't handle long disp for branch insts %p using "
                                                                "ExtCodeBase stack slot.\n",
                instr);
        }

        //  No one else from inst selection phase using this slot
        if (!largeDisp && self()->getCheckForLongDispSlot() && disp == offsetToLongDispSlot
            && ((base == LongDispSlotBaseReg && index == NULL) || (index == LongDispSlotBaseReg && base == NULL))) {
            TR_ASSERT(0, "OMR::Z::MemoryReference::generateBinaryEncoding -- BAD use of ExtCodeBase SLOT %d on the "
                         "stack %x node is %x.\n",
...
bool OMR::Z::MemoryReference::doEvaluate(TR::Node* subTree, TR::CodeGenerator* cg)
{
    ...
    if (self()->forceFirstTimeFolding()) {
        if (cg->traceBCDCodeGen())
            traceMsg(comp, "\tforceFirstTimeFolding=true for subTree %s (%p) refCount %d, reg %s (reset firstTimeFlag "
                           "for next doEvaluate)\n",
    ...
    TR::CodeGenerator* cg, TR::Node* arrayCopyNode, TR::Node* srcNode)
{
    ...
    if (srcNode->getOpCodeValue() == TR::aiadd || srcNode->getOpCodeValue() == TR::aiuadd
        ...
        } else if ((firstChildOp == TR::aiuadd) || (firstChildOp == TR::aiadd) || (firstChildOp == TR::aluadd)
            || (firstChildOp == TR::aladd)) {
