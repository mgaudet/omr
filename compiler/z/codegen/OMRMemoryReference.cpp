...
OMR::Z::MemoryReference::generateBinaryEncoding(uint8_t* cursor, TR::CodeGenerator* cg, TR::Instruction* instr)
{
    ...
    */
    ...
    if (disp < 0 || disp >= MAXDISP) {
        ...
        if ((instr->hasLongDisplacementSupport() || instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) == RSY_FORMAT || instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) == RXY_FORMAT || instr->getOpCode().getInstructionFormat(instr->getOpCodeValue()) == SIY_FORMAT) && disp < MAXLONGDISP && disp > MINLONGDISP) {
        ...
        if (largeDisp) {
            if (instr->getKind() == TR::Instruction::IsRS || instr->getKind() == TR::Instruction::IsRSY || instr->getKind() == TR::Instruction::IsSI || instr->getKind() == TR::Instruction::IsSIY || instr->getKind() == TR::Instruction::IsSIL || instr->getKind() == TR::Instruction::IsSS1 || instr->getKind() == TR::Instruction::IsSS2 || instr->getKind() == TR::Instruction::IsSS4 || instr->getKind() == TR::Instruction::IsS) {
            ...
            {
                TR_ASSERT(instr->getKind() == TR::Instruction::IsRX || instr->getKind() == TR::Instruction::IsVRX || instr->getKind() == TR::Instruction::IsRXY || instr->getKind() == TR::Instruction::IsRXYb || instr->getKind() == TR::Instruction::IsRXE || instr->getKind() == TR::Instruction::IsRXF,
            ...
            {
                TR_ASSERT(instr->getKind() == TR::Instruction::IsRX || instr->getKind() == TR::Instruction::IsRXY || instr->getKind() == TR::Instruction::IsRXYb || instr->getKind() == TR::Instruction::IsRXE || instr->getKind() == TR::Instruction::IsRXF,
                    "OMR::Z::MemoryReference::generateBinaryEncoding -- Unexpected instruction type %p.", instr);
    ...
    if (instr->getKind() == TR::Instruction::IsRXY || instr->getKind() == TR::Instruction::IsRXYb || instr->getKind() == TR::Instruction::IsRSY || instr->getKind() == TR::Instruction::IsSIY) {
    ...
    if (!comp->getOption(TR_DisableLongDispStackSlot)) {
        ...
        if (!largeDisp && self()->getCheckForLongDispSlot() && disp == offsetToLongDispSlot && ((base == LongDispSlotBaseReg && index == NULL) || (index == LongDispSlotBaseReg && base == NULL))) {
...
static TR::SymbolReference* findBestSymRefForArrayCopy(TR::CodeGenerator* cg, TR::Node* arrayCopyNode, TR::Node* srcNode)
{
    ...
    if (srcNode->getOpCodeValue() == TR::aiadd || srcNode->getOpCodeValue() == TR::aiuadd || srcNode->getOpCodeValue() == TR::aladd || srcNode->getOpCodeValue() == TR::aluadd || srcNode->getOpCodeValue() == TR::asub) {
        ...
        } else if ((firstChildOp == TR::aiuadd) || (firstChildOp == TR::aiadd) || (firstChildOp == TR::aluadd) || (firstChildOp == TR::aladd)) {
