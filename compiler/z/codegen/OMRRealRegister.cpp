...
{
    if (reg < FirstGPR || (reg > LastAssignableGPR && reg < FirstFPR) || reg > LastAssignableFPR) {
        return true;
    } else {
        return false;
    }
...
// static method
bool OMR::Z::RealRegister::isRealReg(RegNum reg) { return !TR::RealRegister::isPseudoRealReg(reg); }
