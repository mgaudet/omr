 ...
 * the new registers in the list.
 * Calling useRegister again on the existing registers will cause the totalUseCount to be too high and the virtual
 * register will remain set to the real register
...
uint32_t TR_S390RegisterDependencyGroup::genBitVectOfAssignableGPRs(
    TR::Instruction* currentInstruction, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
{
    int32_t i;
    ...
    TR::Instruction* currentInstruction, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            ...
            if (_dependencies[i].getRegister(cg) && _dependencies[i].getRegister(cg)->getKind() != TR_FPR
                ...
                if (TR::RealRegister::isHPR(_dependencies[i].getRealRegister())) {
                    ...
                    // cg->getAvailableHPRSpillMask());
                } else if (_dependencies[i].getRegister(cg)->is64BitReg()) {
                    cg->maskAvailableHPRSpillMask(
                        ~(TR::RealRegister::getBitMask(_dependencies[i].getRealRegister()) << 16));
    ...
    if (numReqPairs > numAvailPairs) {
        ...
        cg->fe()->outOfMemory(
            comp, "checkDependencyGroup::Insufficient available pairs to satisfy all reg deps, abort compilation\n");
    }
    ...
    for (i = 0; i < (int32_t)(numberOfRegisters - 1); i++) {
        ...
        while (j < numberOfRegisters) {
            if (virtReg == _dependencies[j].getRegister(cg) && realReg != TR::RealRegister::SpilledReg
            ...
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)
                && _dependencies[j].getRegister(cg) && _dependencies[j].getRegister(cg)->getKind() != TR_FPR
                && _dependencies[j].getRegister(cg)->getKind() != TR_VRF
                && _dependencies[j].getRegister(cg)->is64BitReg() && virtReg->isPlaceholderReg()
                && realReg
                    == _dependencies[j].getRealRegister() + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR)) {
                traceMsg(comp, "HW RA: remove dup GPR/HPR deps i=%d\n", i);
                _dependencies[i].setRealRegister(TR::RealRegister::NoReg);
            }
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && virtReg
                && virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF && virtReg->is64BitReg()
