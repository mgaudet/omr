...
bool OMR::Z::RegisterDependencyConditions::refsRegister(TR::Register* r)
{
    ...
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister(_cg) == r && _preConditions->getRegisterDependency(i)->getRefsRegister()) {
            return true;
        }
    }
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister(_cg) == r && _postConditions->getRegisterDependency(j)->getRefsRegister()) {
            return true;
        }
    }
    return false;
...
bool OMR::Z::RegisterDependencyConditions::defsRegister(TR::Register* r)
{
    // *this    swipeable for debugging purposes
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister(_cg) == r && _preConditions->getRegisterDependency(i)->getDefsRegister()) {
            return true;
        }
    }
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister(_cg) == r && _postConditions->getRegisterDependency(j)->getDefsRegister()) {
            return true;
        }
    }
    return false;
...
bool OMR::Z::RegisterDependencyConditions::mayDefineRegister(TR::Register* r)
{
    // *this    swipeable for debugging purposes
    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister(_cg) == r && _postConditions->getRegisterDependency(j)->getRealRegister() == TR::RealRegister::MayDefine) {
            return true;
        }
    }
    return false;
...
bool OMR::Z::RegisterDependencyConditions::usesRegister(TR::Register* r)
{
    // *this    swipeable for debugging purposes
    for (int32_t i = 0; i < _numPreConditions; i++) {
        if (_preConditions->getRegisterDependency(i)->getRegister(_cg) == r && (_preConditions->getRegisterDependency(i)->getRefsRegister() || _preConditions->getRegisterDependency(i)->getDefsRegister())) {
            return true;
        }
    }

    for (int32_t j = 0; j < _numPostConditions; j++) {
        if (_postConditions->getRegisterDependency(j)->getRegister(_cg) == r && (_postConditions->getRegisterDependency(j)->getRefsRegister() || _postConditions->getRegisterDependency(j)->getDefsRegister())) {
            return true;
        }
    }
    return false;
    ...
    TR::CodeGenerator* cg)
{
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA)) {
            ...
            if (_dependencies[i].getRegister(cg) && _dependencies[i].getRegister(cg)->getKind() != TR_FPR && _dependencies[i].getRegister(cg)->getKind() != TR_VRF) {
                // on 64-bit conservatively assume rEP and rRA are going to clobber highword
                if (TR::Compiler->target.is64Bit() && !(currentInstruction->isLabel() && toS390LabelInstruction(currentInstruction)->getLabelSymbol()->isStartOfColdInstructionStream()) && (_dependencies[i].getRealRegister() == cg->getReturnAddressRegister() || _dependencies[i].getRealRegister() == cg->getEntryPointRegister())) {
    ...
    for (i = 0; i < (int32_t)(numberOfRegisters - 1); i++) {
        ...
        while (j < numberOfRegisters) {
            ...
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && _dependencies[j].getRegister(cg) && _dependencies[j].getRegister(cg)->getKind() != TR_FPR && _dependencies[j].getRegister(cg)->getKind() != TR_VRF && _dependencies[j].getRegister(cg)->is64BitReg() && virtReg->isPlaceholderReg() && realReg == _dependencies[j].getRealRegister() + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR)) {
                traceMsg(comp, "HW RA: remove dup GPR/HPR deps i=%d\n", i);
                _dependencies[i].setRealRegister(TR::RealRegister::NoReg);
            }
            if (cg->supportsHighWordFacility() && !comp->getOption(TR_DisableHighWordRA) && virtReg && virtReg->getKind() != TR_FPR && virtReg->getKind() != TR_VRF && virtReg->is64BitReg() && _dependencies[j].getRegister(cg)->isPlaceholderReg() && realReg + (TR::RealRegister::FirstHPR - TR::RealRegister::FirstGPR) == _dependencies[j].getRealRegister()) {
        ...
        for (i = 0; i < numberOfRegisters; i++) {
            ...
            if (dependentRegNum != TR::RealRegister::NoReg && dependentRegNum != TR::RealRegister::AssignAny && dependentRegNum != TR::RealRegister::SpilledReg && dependentRealReg->getState() == TR::RealRegister::Free) {
               ...
               fprintf(stdout, "COERCE1 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), dependentRegNum-1);
                ...
                if (comp->getOption(TR_EnableTrueRegisterModel) && _dependencies[i].getDefsRegister() && virtReg->isPendingSpillOnDef()) {
...
// Coerce the assignment
               ...
               fprintf(stdout, "COERCE2 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), dependentRegNum-1);
                    ...
                    if (assignedRegister != NULL && assignedRegister->getState() == TR::RealRegister::Free) {
                        ...
                        for (int32_t lcount = 0; lcount < numberOfRegisters; lcount++) {
                            ...
                            if (aRealNum != TR::RealRegister::NoReg && aRealNum != TR::RealRegister::AssignAny && aRealNum != TR::RealRegister::SpilledReg && assignedRegister == machine->getS390RealRegister(aRealNum)) {
#if 0
                        fprintf(stdout, "COERCE3 %i (%s, %d)\n", i, cg->getDebug()->getName(virtReg), aRealNum-1);
                                ...
                                if (comp->getOption(TR_EnableTrueRegisterModel) && _dependencies[lcount].getDefsRegister() && depReg->isPendingSpillOnDef()) {
    ...
    for (i = 0; i < numberOfRegisters; i++) {
        ...
        if (dependentRegister->getRegisterPair() == NULL && (dependentRegister->getAssignedRegister() || (comp->getOption(TR_EnableTrueRegisterModel) && dependentRegister->isLive())) && getRegisterDependency(i)->getRealRegister() != TR::RealRegister::SpilledReg) {
            ...
            if ((dependentRegister->getFutureUseCount() == 0) || (comp->getOption(TR_EnableTrueRegisterModel) && !currentInstruction->isLabel() && getRegisterDependency(i)->getDefsRegister() && !getRegisterDependency(i)->getRefsRegister() && !currentInstruction->defsRegister(dependentRegister))) {
