...
public:
    ...
    TR::Register* searchForRegister(TR::Register* vr, uint8_t flag, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
    {
        for (int32_t i = 0; i < numberOfRegisters; i++) {
            ...
            if (vr->isArGprPair() && _dependencies[i].getRegister(cg)->isArGprPair() && vr->getARofArGprPair() == _dependencies[i].getRegister(cg)->getARofArGprPair() && vr->getGPRofArGprPair() == _dependencies[i].getRegister(cg)->getGPRofArGprPair())
                return _dependencies[i].getRegister(cg);
        }
        return NULL;
    }
