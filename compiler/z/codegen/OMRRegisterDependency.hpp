...
public:
    ...
    TR::Register* searchForRegister(TR::Register* vr, uint8_t flag, uint32_t numberOfRegisters, TR::CodeGenerator* cg)
    {
        for (int32_t i = 0; i < numberOfRegisters; i++) {
            ...
            if (vr->isArGprPair() && _dependencies[i].getRegister(cg)->isArGprPair()
...
namespace Z {
    ...
    public:
        ...
        bool doesConditionExist(TR_S390RegisterDependencyGroup* regDepArr, TR::Register* vr,
            TR::RealRegister::RegNum rr, uint32_t flag, uint32_t numberOfRegisters, bool overwriteAssignAny = false);
