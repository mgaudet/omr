        ...
        if (cg->mayImmedInstructionCauseOverFlow(node)) {
            if (cg->canUseGoldenEagleImmediateInstruction(value)) {
                ...
                if (sourceRegister == targetRegister) {
                    ...
                    if (cg->getCurrentRegisterPressure() < CODEGEN_REGPRESSURE_THRESHOLD && !comp->getJittedMethodSymbol()->isNoTemps()) {
...
tryGenerateSIComparisons(TR::Node* node, TR::Node* constNode, TR::Node* otherNode, TR::CodeGenerator* cg, bool isOnlyTest)
{
    ...
    //
    if (!operand->isSingleRefUnevaluated() || (operand->getOpCode().hasSymbolReference() && operand->getSymbolReference()->getSymbol()->isRegisterSymbol()))
        return 0;
...
tryGenerateCLCForComparison(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (operand1->getSymbolReference()->getSymbol()->isRegisterSymbol() || operand2->getSymbolReference()->getSymbol()->isRegisterSymbol())
        return 0;
...
tryGenerateConversionRXComparison(TR::Node* node, TR::CodeGenerator* cg, bool* isForward)
{
    ...
    if (memNode->getOpCode().isConversion()) {
        ...
        if (memNode->getSize() == 2 && // only short memref
            ...
            if (regNode->isZeroExtension() && regNode->getFirstChild()->getSize() == 2 && regNode->isSingleRefUnevaluated() && performTransformation(comp,
    ...
    TR::InstOpCode::S390BranchCondition rBranchOpCond, TR::InstOpCode::S390BranchCondition& retBranchOpCond, TR::LabelSymbol* branchTarget = NULL)
{
    ...
    if (cg->profiledPointersRequireRelocation() && node->isProfiledGuard() && secondChild->getOpCodeValue() == TR::aconst && (secondChild->isMethodPointerConstant() || secondChild->isClassPointerConstant())) {
    ...
    if ((secondChild->getOpCode().isLoadConst() || firstChild->getOpCode().isLoadConst()) &&
        ...
        //
            ...
            if (nonConstNode->getRegister() != NULL) {
                ...
                if ((nonConstNode->getOpCodeValue() == TR::arraycmp || nonConstNode->getOpCodeValue() == TR::arraycmpWithPad) && (fBranchOpCond == TR::InstOpCode::COND_BERC || fBranchOpCond == TR::InstOpCode::COND_BNERC) && (constNode->getIntegerNodeValue<int64_t>() == 0)) {
    ...
    // FIXME: can't the binary commutative analyser handle this? that's where this should be done
    //
    else if (firstChild->getOpCodeValue() == TR::bu2i && firstChild->getRegister() == NULL && firstChild->getReferenceCount() == 1 && (firstChild->getFirstChild()->getOpCodeValue() == TR::buloadi || firstChild->getFirstChild()->getOpCodeValue() == TR::iRegLoad || (firstChild->getFirstChild()->getOpCode().hasSymbolReference() && firstChild->getFirstChild()->getSymbolReference()->getSymbol()->isRegisterSymbol()))
        && firstChild->getFirstChild()->getRegister() && secondChild->getOpCodeValue() == TR::bu2i && secondChild->getRegister() == NULL && secondChild->getReferenceCount() == 1 && (secondChild->getFirstChild()->getOpCodeValue() == TR::buloadi || secondChild->getFirstChild()->getOpCodeValue() == TR::iRegLoad || (secondChild->getFirstChild()->getOpCode().hasSymbolReference() && secondChild->getFirstChild()->getSymbolReference()->getSymbol()->isRegisterSymbol()))
...
OMR::Z::TreeEvaluator::extendCastEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else if (numberOfExtendBits == 64 && TR::Compiler->target.is32Bit() && cg->use64BitRegsOn32Bit() && (firstChild->getOpCode().isLoadReg() || (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference()->getSymbol()->isRegisterSymbol())))
...
bool relativeLongLoadHelper(TR::CodeGenerator* cg, TR::Node* node, TR::Register* tReg)
{
    ...
    if (symbol->isStatic() && !symRef->isUnresolved() && !cg->comp()->compileRelocatableCode() && !node->getOpCode().isIndirect() && !cg->getConditionalMovesEvaluationMode()) {
...
bool relativeLongStoreHelper(TR::CodeGenerator* cg, TR::Node* node, TR::Node* valueChild)
{
    ...
    if (!disableSTRL && symbol->isStatic() && !symRef->isUnresolved() && !cg->comp()->compileRelocatableCode() && !node->getOpCode().isIndirect() && !cg->getConditionalMovesEvaluationMode()) {
        uintptrj_t staticAddress = (uintptrj_t)symRef->getSymbol()->getStaticSymbol()->getStaticAddress();
        TR_ASSERT(!node->getType().isInt64() || TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(), "Helper cannot handle 64bit loads on 32-bit platforms.");
...
lstoreHelper64(TR::Node* node, TR::CodeGenerator* cg, bool isReversed)
{
    ...
    if (directToMemoryAddHelper(cg, node, valueChild, TR::InstOpCode::AGSI)) {
        ...
        else if (!node->getOpCode().isIndirect() && valueChild->getOpCodeValue() == TR::lload && valueChild->getReferenceCount() == 1 && valueChild->getRegister() == NULL && !valueChild->getSymbolReference()->getSymbol()->isRegisterSymbol() && generateS390MemoryReference(node, cg)->getIndexRegister() == NULL && !cg->getConditionalMovesEvaluationMode()) {
...
astoreHelper(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!relativeLongStoreHelper(cg, node, valueChild)) {
        ...
        else if (!node->getOpCode().isIndirect() && valueChild->getOpCodeValue() == TR::aload && valueChild->getReferenceCount() == 1 && valueChild->getRegister() == NULL && !valueChild->getSymbolReference()->getSymbol()->isRegisterSymbol() && generateS390MemoryReference(node, cg)->getIndexRegister() == NULL && !valueChild->getSymbolReference()->isLiteralPoolAddress() && !(cg->isAddressOfStaticSymRefWithLockedReg(valueChild->getSymbolReference())) && !(cg->isAddressOfPrivateStaticSymRefWithLockedReg(valueChild->getSymbolReference())) && node->getSymbol()->getSize() == node->getSize() && node->getSymbol()->getSize() == valueChild->getSymbol()->getSize() && !cg->getConditionalMovesEvaluationMode()) {
            int16_t length = 0;
...
OMR::Z::TreeEvaluator::aiaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!skipFinalLA) {
        ...
        if (aiaddMR->getOffset() == 0 && aiaddMR->getIndexRegister() == NULL && baseReg && cg->getBucketPlusIndexRegisters().ValueAt(baseReg->getIndex())) {
...
OMR::Z::TreeEvaluator::bstoreEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // Use MVI when we can
    else if (valueChild->getRegister() == NULL && (valueChild->getOpCodeValue() == TR::buconst || valueChild->getOpCodeValue() == TR::bconst)) {
        TR::MemoryReference* tempMR = generateS390MemoryReference(node, cg);
    ...
    //
    else if (valueChild->getReferenceCount() == 1 && valueChild->getRegister() == NULL && (valueChild->getOpCodeValue() == TR::buload || valueChild->getOpCodeValue() == TR::bload || valueChild->getOpCodeValue() == TR::buloadi || valueChild->getOpCodeValue() == TR::buload) && !valueChild->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
...
*/
   ...
   */
    ...
    cursor = generateRIEInstruction(cg, is64 ? TR::InstOpCode::CGIJ : TR::InstOpCode::CIJ, node, rInput, (int8_t)0x0, LabelDone, TR::InstOpCode::COND_BNE);
...
#define iComment(str) \
    ...
    iComment("Len perfect multiple of 16, goto main loop (rare)"); // go to main loop if no rResidue from result of NI 0xF (rare..)
    ...
    cursor = generateRIEInstruction(cg, TR::InstOpCode::CIJ, node, rLen, (int8_t)sizeOfVector, doneLabel, TR::InstOpCode::COND_BNH);
    iComment("len <= 16 ? we're done..")
        cursor
    ...
    cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadAddressOpCode(), node, rIdx, generateS390MemoryReference(rIdx, sizeOfVector, cg), cursor);
...
OMR::Z::TreeEvaluator::BBEndEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!nextTT || !nextTT->getNode()->getBlock()->isExtensionOfPreviousBlock()) {
        if (cg->enableRegisterAssociations() && comp->getAppendInstruction()->getOpCodeValue() != TR::InstOpCode::ASSOCREGS) {
...
OMR::Z::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isSimpleCopy && ((bool)disableAtomic || node->getArrayCopyElementType() == TR::Int8) && node->isForwardArrayCopy()) {
        if (byteLenReg == NULL) {
 ...
 */
void OMR::Z::TreeEvaluator::countDigitsHelper(TR::Node* node, TR::CodeGenerator* cg,
...
{
    if (node->getDataType() == TR::VectorDouble && (canUseNodeForFusedMultiply(node->getFirstChild()) || canUseNodeForFusedMultiply(node->getSecondChild())) && generateFusedMultiplyAddIfPossible(cg, node, TR::InstOpCode::VFMA)) {
...
{
    if (node->getDataType() == TR::VectorDouble && canUseNodeForFusedMultiply(node->getFirstChild()) && generateFusedMultiplyAddIfPossible(cg, node, TR::InstOpCode::VFMS)) {
        if (TR::comp()->getOption(TR_TraceCG))
...
OMR::Z::TreeEvaluator::vsplatsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool inRegister = !firstChild->isSingleRefUnevaluated() || (firstChild->getOpCode().hasSymbolReference() && firstChild->getSymbolReference()->getSymbol()->isRegisterSymbol());
...
OMR::Z::TreeEvaluator::vsetelemEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    bool inRegister = !valueNode->isSingleRefUnevaluated() || (valueNode->getOpCode().hasSymbolReference() && valueNode->getSymbolReference()->getSymbol()->isRegisterSymbol());
