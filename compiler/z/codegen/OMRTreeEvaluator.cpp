    ...
    TR::RegisterDependencyConditions* dependencies, TR::Register* literalPoolRegister)
{
    ...
    if (node->getOpCode().hasSymbolReference())
        sym = node->getSymbol();
    bool needRegPair
        = cg->evaluateNodeInRegPair(node) || !(TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit());
    bool load64bit = !needRegPair && (node->getType().isInt64() || node->force64BitLoad());
    ...
    TR::Register* targetRegister, TR::Instruction* cursor, TR::RegisterDependencyConditions* cond, TR::Register* base)
{
    ...
    if (node->isClassUnloadingConst()) {
        ...
        TR::Instruction* unloadableConstInstr
            = generateRILInstruction(cg, TR::InstOpCode::LARL, node, targetRegister, value);
            ...
            if (cg->canUseGoldenEagleImmediateInstruction(value)) {
                ...
                if (sourceRegister == targetRegister) {
                    ...
                    if (cg->getCurrentRegisterPressure() < CODEGEN_REGPRESSURE_THRESHOLD
                        ...
                        if (cond)
                            cond->addPostConditionIfNotAlreadyInserted(tempReg, TR::RealRegister::AssignAny);
                    ...
                    cursor
                        = generateRRInstruction(cg, TR::InstOpCode::ALR, node, targetRegister, sourceRegister, cursor);
                }
                return cursor;
            } else {
                memOp = TR::InstOpCode::AL;
    ...
    case TR::InstOpCode::AG:
        ...
        if (cg->mayImmedInstructionCauseOverFlow(node)) {
            if (cg->canUseGoldenEagleImmediateInstruction(value) && value > 0) {
                // LL: If Golden Eagle - can use Add Logical Long Immediate with max 32-bit value, cannot use negative
                // values because ALGFI does not sign extend its immediate value.
            ...
            if (value >= MIN_IMMEDIATE_VAL && value <= MAX_IMMEDIATE_VAL) {
                if (sourceRegister == targetRegister) {
                    ...
                    cursor
                        = generateRRInstruction(cg, TR::InstOpCode::ALGR, node, targetRegister, sourceRegister, cursor);
                }
                return cursor;
            } else {
                memOp = TR::InstOpCode::AGF;
    ...
    TR::InstOpCode::S390BranchCondition& retBranchOpCond, TR::LabelSymbol* branchTarget)
{
    ...
    if (signsAndDataAreValid) {
        ...
        if (knownSigns || cleanSigns) {
            if (cg->traceBCDCodeGen()) {
                traceMsg(comp, "\t+validateDecimalSignAndData=%s (firstReg (validData=%s validSign=%s) and secondReg "
                    ...
                    if (firstReg->getSize() < secondReg->getSize()) {
                        ...
                        if (firstReg->getLiveSymbolSize() >= secondReg->getSize()) {
                            ...
                            if (firstReg->getBytesToClear(firstReg->getSize(), secondReg->getSize()) == 0) {
                                if (cg->traceBCDCodeGen())
                                    traceMsg(comp, "\t+upper bytes (byte %d to %d) are clear so do gen CLC with "
                                                   "clcSize = secondReg->getSize() = %d\n",
            ...
            traceMsg(comp, "\t-firstReg (clean=%s known=0x%x) and secondReg (clean=%s known=0x%x) signs are not "
        ...
        traceMsg(comp, "\t-firstReg (validData=%s validSign=%s) and secondReg (validData=%s validSign=%s) signs/data "
    ...
    TR::Node* node, TR::Node* constNode, TR::Node* otherNode, TR::CodeGenerator* cg, bool isOnlyTest)
{
    ...
    //
    if (!operand->isSingleRefUnevaluated() || (operand->getOpCode().hasSymbolReference()
    ...
    if (operandSize == 1) // we can use CLI
    {
        ...
        if (isUnsignedCmp && 0 != (value >> 8)
            || // only the bottom byte
            !isUnsignedCmp
                && (operand == otherNode && 0 != (value >> 8)
                       || // have not skipped conversion
                       operand != otherNode
...
static TR::Instruction* tryGenerateCLCForComparison(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if ((operand1->getOpCode().isConversion()
        ...
        || (operand2->getOpCode().isConversion() && (operand2->getFirstChild()->getType().isBCD()
    ...
    // 120(3,GPR5), Shadow[<vft-symbol>] 0(GPR7)
    ...
    if (operand1IsVFTSymbol || operand2IsVFTSymbol) {
        if (TR::Compiler->target.is64Bit() && (cg->isCompressedClassPointerOfObjectHeader(operand1)
...
static TR::Instruction* tryGenerateConversionRXComparison(TR::Node* node, TR::CodeGenerator* cg, bool* isForward)
{
    ...
    //
    if (!memNode->isSingleRefUnevaluated() || memNode->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
        return 0;
    }
    ...
    TR::InstOpCode::S390BranchCondition& retBranchOpCond, TR::LabelSymbol* branchTarget = NULL)
{
    ...
    if ((secondChild->getOpCode().isLoadConst() || firstChild->getOpCode().isLoadConst()) &&
        ...
        //
            ...
            && !(nonConstNode->getOpCode().hasSymbolReference()
                     && nonConstNode->getSymbolReference()->getSymbol()->isRegisterSymbol())) {
            // case 1.3
            ...
            bool mustExtend = !useLTG && !nonConstNode->couldIgnoreExtend() && nonConstNode->force64BitLoad();
            ...
            case TR::Address: {
                ...
                if (testRegister->getKind() == TR_GPR64 || cg->use64BitRegsOn32Bit())
                    ...
                    compareOpCode = isUnsignedCmp ? TR::InstOpCode::getCmpLogicalOpCodeFromNode(nonConstNode)
    ...
    //
        ...
        && (firstChild->getFirstChild()->getOpCodeValue() == TR::buloadi
                 ...
                 || (firstChild->getFirstChild()->getOpCode().hasSymbolReference()
                        && firstChild->getFirstChild()->getSymbolReference()->getSymbol()->isRegisterSymbol()))
        ...
        && (secondChild->getFirstChild()->getOpCodeValue() == TR::buloadi
                 ...
                 || (secondChild->getFirstChild()->getOpCode().hasSymbolReference()
                        && secondChild->getFirstChild()->getSymbolReference()->getSymbol()->isRegisterSymbol()))
        && secondChild->getFirstChild()->getRegister()) {
        if (branchTarget != NULL) {
            returnInstruction = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CR, node,
                firstChild->getFirstChild()->getRegister(), secondChild->getFirstChild()->getRegister(),
    ...
    //
        ...
        case TR::Address: // FIXME: impossible to get here with TR::Address
            ...
            break;
        case TR::Int32:
            TR_ASSERT(dataType == TR::Int32 || comp->useCompressedPointers(), "First child, %p, and second child, %p, "
                                                                              "data types do not match. This is an "
                                                                              "issue when compression is disabled.",
    ...
    if (cg->hasCCInfo() && cg->ccInstruction()->getOpCode().setsCarryFlag()) {
        TR_ASSERT((returnCond == TR::InstOpCode::COND_BERC || returnCond == TR::InstOpCode::COND_BNERC),
    ...
    TR::InstOpCode::S390BranchCondition fBranchOpCond, TR::InstOpCode::S390BranchCondition rBranchOpCond)
{
    ...
    // Case: Pattern Matching for Byte Testing
        ...
        */
    ...
    } else if (constNode && (node->getOpCodeValue() == TR::iflcmpeq || node->getOpCodeValue() == TR::iflucmpeq
        ...
        cg->decReferenceCount(firstChild);
        cg->decReferenceCount(secondChild);
    } else if (constNode && (node->getOpCodeValue() == TR::ificmpeq || node->getOpCodeValue() == TR::ifiucmpeq
...
static const TR::InstOpCode::Mnemonic loadInstrs[2 /*Form*/][4 /*numberOfBits*/][2 /*isSigned*/]
    ...
    = {
        ...
        /*MemReg*/ { /* 8*/ {
                         { TR::InstOpCode::LLC, TR::InstOpCode::LLGC }, { TR::InstOpCode::LB, TR::InstOpCode::LGB } },
...
TR::Register* genericLoad(TR::Node* node, TR::CodeGenerator* cg, TR::MemoryReference* tempMR, TR::Register* srcRegister)
{
    ...
    if (node->getType().isAddress()
        || node->getType().isAggregate()) // o- and a-type and PLX-Fixed24/Fixed8 always unsigned
        nodeSigned = false;
 ...
 * executing
 * on GoldenEagle or better, this function would had been just couple of lines to pick the right instruction of the
 * loadInstr
 * array(!!!):
 *
 ...
 * Fallback path tries to pick the best instructions out of the reduced set. The best fallback alternative is ICM (or
 * IC) for load
 * from memory. Sign/zero extension, in the fall back case is done using two shifts. If ICM was used, a single shift
 * could be
 * used to sign/zero extend, by telling ICM to load into the top bits. Furthermore, in the fallback case, sign/zero
 * extension
    ...
    TR::Register* srcRegister, bool isSourceSigned, bool couldIgnoreExtend_OR_canClobberSrcReg)
{
    ...
    TR_S390ProcessorInfo::TR_S390ProcessorArchitectures ReqArch
        = reqArchForLoad[form][numberOfBytesLog2][isSourceSigned][numberOfExtendBits / 32 - 1];
    ...
    // check is for the mixed mode case where we have a 32 bit reg
    // on input and must allocate a new 64 bit register (otherwise later spills/fills may not preserve the top 32 bits
    // (i.e. LR/L will be used instead of LGR/LG)
    if (form == RegReg && (!canClobberSrcReg || (useRegPairs && numberOfBytes <= 4)
        ...
        && (srcRegister == NULL
                   || (numberOfExtendBits == 64 && ((!useRegPairs && srcRegister->getKind() == TR_GPR)
    ...
    if ((numberOfBits & (numberOfBits - 1)) == 0
        ...
        !useRegPairs && cg->getS390ProcessorInfo()->supportsArch(ReqArch)) // Are we running on awesome hardware and OS?
    {
        CASE(1);
...
void OMR::Z::TreeEvaluator::evaluateRegLoads(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    while (!queue.IsEmpty()) {
        ...
        if (visited[tmp->getNodePoolIndex()])
            continue;
        visited[tmp->getNodePoolIndex()] = 1;
        if (tmp->getOpCode().hasSymbolReference() && tmp->getSymbolReference()->getSymbol()->isRegisterSymbol()) {
            cg->evaluate(tmp);
        } else {
...
TR::Register* OMR::Z::TreeEvaluator::addressCastEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    } else // if (!dirToAddr && addrSize < otherSize) //i.e. a2(otherSize)
    {
        ...
        case 32:
            ...
            break;
        case 24:
            return extendCastEvaluator < false, 24, (otherSize > 32) ? 64 : 32 > (node, cg);
            break;
        case 16:
            return extendCastEvaluator < false, 16, (otherSize > 32) ? 64 : 32 > (node, cg);
            break;
        case 8:
            return extendCastEvaluator < false, 8, (otherSize > 32) ? 64 : 32 > (node, cg);
            break;
        default:
            TR_ASSERT(0, "Invalid Address Precision (%d)\n", addrSize);
            break;
...
namespace Z {
    ...
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<true, 16, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<false, 16, 32>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<false, 16, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
    ...
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<true, 32, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<false, 32, 32>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<false, 32, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<true, 64, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
    template TR::Register* OMR::Z::TreeEvaluator::extendCastEvaluator<false, 64, 64>(
        TR::Node* node, TR::CodeGenerator* cg);
...
bool relativeLongLoadHelper(TR::CodeGenerator* cg, TR::Node* node, TR::Register* tReg)
{
    ...
    if (symbol->isStatic() && !symRef->isUnresolved() && !cg->comp()->compileRelocatableCode()
        && !node->getOpCode().isIndirect() && !cg->getConditionalMovesEvaluationMode()) {
        ...
        if ((!disableFORCELRL || cg->canUseRelativeLongInstructions(staticAddress))
            ...
            // traceMsg(cg->comp(), "node->useSignExtensionMode = %d node->isUnsignedLoad =
...
bool relativeLongStoreHelper(TR::CodeGenerator* cg, TR::Node* node, TR::Node* valueChild)
{
    ...
    if (!disableSTRL && symbol->isStatic() && !symRef->isUnresolved() && !cg->comp()->compileRelocatableCode()
        && !node->getOpCode().isIndirect() && !cg->getConditionalMovesEvaluationMode()) {
        uintptrj_t staticAddress = (uintptrj_t)symRef->getSymbol()->getStaticSymbol()->getStaticAddress();
        TR_ASSERT(!node->getType().isInt64() || TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit(),
            "Helper cannot handle 64bit loads on 32-bit platforms.");
        ...
        if (!isNonZero) {
            ...
            if (addOrSubNode->getFirstChild()->getOpCode().isShift() && addOrSubNode->getFirstChild()->getRegister()) {
                ...
                generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::getCmpOpCode(), node, r, (signed char)0, TR::InstOpCode::COND_BERC, skipAdd);
            } else {
...
TR::Register* OMR::Z::TreeEvaluator::aiaddEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (aiaddMR->getOffset() >= MAXLONGDISP && aiaddMR->getOffset() <= TR::getMaxSigned<TR::Int32>()
        && TR::Compiler->target.is32Bit()) {
        skipFinalLA = true;
        TR::MemoryReference* tempMR
            = generateS390MemoryReference(aiaddMR->getBaseRegister(), aiaddMR->getIndexRegister(), 0, cg);
        tempMR->setSymbolReference(aiaddMR->getSymbolReference());
        generateRXInstruction(cg, TR::InstOpCode::LA, node, targetRegister, tempMR);
        generateS390ImmOp(
            cg, TR::InstOpCode::getAddOpCode(), node, targetRegister, targetRegister, (int32_t)aiaddMR->getOffset());
    } else {
    ...
    if (!skipFinalLA) {
        ...
        if (aiaddMR->getOffset() == 0 && aiaddMR->getIndexRegister() == NULL && baseReg
            && cg->getBucketPlusIndexRegisters().ValueAt(baseReg->getIndex())) {
            aiaddMR->setBucketBaseRegMemRef();
            if (cg->traceBCDCodeGen())
                traceMsg(comp, "\tfound baseReg %s (idx=%d) for node %s (%p) in BucketPlusIndexRegisters "
                               "setBucketBaseRegMemRef on LA inst %p memRef\n",
...
TR::Register* OMR::Z::TreeEvaluator::lloadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    else if (!cg->evaluateNodeInRegPair(node) && (TR::Compiler->target.is64Bit() || cg->use64BitRegsOn32Bit())) {
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::Register* inReg, TR::Register* outReg)
{
    ...
    generateRIInstruction(
        cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGHI : TR::InstOpCode::LHI, node, outReg, 0);
    generateRIInstruction(
...
#define iComment(str)                                                                                                  \
    ...
    */
    ...
    cursor = generateRIEInstruction(cg, is64 ? TR::InstOpCode::CGIJ : TR::InstOpCode::CIJ, node, rInput, (int8_t)0x0,
        LabelDone, TR::InstOpCode::COND_BNE);
...
static TR::RegisterPair* readKMCPair(TR::Node* node, TR::Node* baseArr, TR::Register* offset, TR::Register* length,
    TR::CodeGenerator* cg, TR::RegisterDependencyConditions* conditions)
{
    ...
    TR::MemoryReference* dataOffset
        = generateS390MemoryReference(arr, offset, TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg);
    generateRXInstruction(cg, TR::InstOpCode::LA, node, arr, dataOffset);

    return arrRegisterPair;
}

enum NumCryptArrays { ONE_ARRAY_SHA = 0, TWO_ARRAYS_CRYPT = 2, THREE_ARRAYS_CRYPT_CTR = 4 };

TR::Register* inlineZosKMC(
    TR::InstOpCode::Mnemonic opcode, TR::Node* node, TR::CodeGenerator* cg, NumCryptArrays numArrays)
{
    TR_ASSERT(node->getNumChildren() == 5 + numArrays,
        "Call to TR::com_ibm_jit_crypto_JITFullHardwareCrypt_z_km* expects 8 children.");
    ...
    iCursor = generateS390BranchInstruction(
        cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_MASK1, node, redoLabel, conditions, iCursor);
...
#define p256digit(x)                                                                                                   \
    ...
    TR::MemoryReference* partialOperands[10][8] = { // Add the following (2*S3 + 2*S2 + S1 + S4 + S5)
        ...
        /*S2*/ { NULL, NULL, NULL, p256digit(11), p256digit(12), p256digit(13), p256digit(14), p256digit(15) },
        /*S1*/ { p256digit(0), p256digit(1), p256digit(2), p256digit(3), p256digit(4), p256digit(5), p256digit(6),
            p256digit(7) },
        /*S4*/ { p256digit(8), p256digit(9), p256digit(10), NULL, NULL, NULL, p256digit(14), p256digit(15) },
        /*S5*/ { p256digit(9), p256digit(10), p256digit(11), p256digit(13), p256digit(14), p256digit(15), p256digit(13),
            p256digit(8) },
        ...
        /*S8*/ { p256digit(13), p256digit(14), p256digit(15), p256digit(8), p256digit(9), p256digit(10), NULL,
            p256digit(12) },
...
#define iComment(str)                                                                                                  \
    ...
    const int sizeOfVector = 16; // in Bytes. Should really make this dynamic (i.e read from some machine() query since
                                 // this can increase in future)
    ...
    cursor = generateRILInstruction(cg, TR::InstOpCode::NILF, node, rResidue, 0xF);
    iComment("Calc rResidue (can eq length if length < 16)");
    cursor = generateRSInstruction(cg, TR::InstOpCode::SRLG, node, rLoopCounter, rLen, 4);
    iComment("reduce len to multiple of 16 (loop counter)");
    cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZ, node, processNext16Bytes);
    iComment("Len perfect multiple of 16, goto main loop (rare)"); // go to main loop if no rResidue from result of NI
                                                                   // 0xF (rare..)
    ...
    cursor = generateVRSbInstruction(cg, TR::InstOpCode::VSTL, node, vBuf, rResidue,
    ...
    cursor = generateRIEInstruction(
        cg, TR::InstOpCode::CIJ, node, rLen, (int8_t)sizeOfVector, doneLabel, TR::InstOpCode::COND_BNH);
    iComment("len <= 16 ? we're done..") cursor = generateRRInstruction(cg, TR::InstOpCode::AR, node, rIdx, rResidue);
    iComment("Add # rResidue code-points to index");

    /* ===================== Step 4: Core loop - 16 byte at a time processing =====================*/

    cursor = generateS390LabelInstruction(cg, TR::InstOpCode::LABEL, node, processNext16Bytes);
    ...
    cursor = generateVRXInstruction(
        cg, TR::InstOpCode::VST, node, vBuf, generateS390MemoryReference(rDst, rIdx, 0, cg), 0, cursor);
    cursor = generateRXInstruction(cg, TR::InstOpCode::getLoadAddressOpCode(), node, rIdx,
        generateS390MemoryReference(rIdx, sizeOfVector, cg), cursor);
    iComment("Src/dst index += 16");
    cursor = generateS390BranchInstruction(cg, TR::InstOpCode::BRCT, node, rLoopCounter, processNext16Bytes);
    ...
    if ((isConstLength && (length == 0)) || (source1Node == source2Node)) {
        ...
        if (length <= (earlyCLCEnabled ? earlyCLCThreshold : TR_MAX_CLC_SIZE) && (isFoldedIf || !needResultReg)) {
            ...
            if (length > (earlyCLCEnabled ? earlyCLCThreshold : TR_MAX_CLC_SIZE)) {
                ...
                if (baseSource1Ref->getBaseRegister())
                    regDeps->addPostConditionIfNotAlreadyInserted(
                        baseSource1Ref->getBaseRegister()->getGPRofArGprPair(), TR::RealRegister::AssignAny);
                ...
                if (baseSource2Ref->getBaseRegister()
                    && (baseSource2Ref->getBaseRegister() != baseSource1Ref->getBaseRegister()))
                    regDeps->addPostConditionIfNotAlreadyInserted(
                        baseSource2Ref->getBaseRegister()->getGPRofArGprPair(), TR::RealRegister::AssignAny);
            }
        }
        ...
        if (earlyCLCEnabled && length > earlyCLCThreshold) {
            ...
            if (isFoldedIf) {
                if (ifxcmpBrCond == TR::InstOpCode::COND_MASK8)
                    setStartInternalControlFlow(generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
            ...
            if (needResultReg) {
                if (return102) {
                    ...
                    cursor = generateRIInstruction(cg,
                        TR::Compiler->target.is64Bit() ? TR::InstOpCode::LGHI : TR::InstOpCode::LHI, node, retValReg,
                        1);
                } else {
        ...
        if (maxLenIn256) {
            ...
            if (lengthCanBeZero) {
                ...
                if (isFoldedIf) {
                    if (isIfxcmpBrCondContainEqual)
                        ...
                        setStartInternalControlFlow(generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                                        TR::InstOpCode::COND_BLRC, node, endLabel),
                            isStartInternalControlFlowSet);
                } else
                    setStartInternalControlFlow(generateS390BranchInstruction(
                                                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node, endLabel),
                        isStartInternalControlFlowSet);
            }
            ...
            if (isWideChar) {
                ...
                if (lengthCanBeZero) {
                    ...
                    if (isFoldedIf) {
                        if (isIfxcmpBrCondContainEqual)
                            // generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, node,
                            // compareTarget);
                            setStartInternalControlFlow(generateS390BranchInstruction(cg, TR::InstOpCode::BRC,
                                                            TR::InstOpCode::COND_BLRC, node, compareTarget),
                                isStartInternalControlFlowSet);
                ...
                setStartInternalControlFlow(generateS390BranchInstruction(
                                                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node, bottomOfLoop),
                    isStartInternalControlFlowSet);
                ...
                generateRXInstruction(cg, TR::InstOpCode::LA, node, source1Reg->getGPRofArGprPair(),
                    generateS390MemoryReference(source1Reg->getGPRofArGprPair(), 256, cg));
                generateRXInstruction(cg, TR::InstOpCode::LA, node, source2Reg->getGPRofArGprPair(),
...
TR::Register* OMR::Z::TreeEvaluator::ixfrsEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isConstValue && isConstSign) {
        ...
        generateRSInstruction(cg, TR::InstOpCode::SRA, node, signReg, 31);
        generateRRInstruction(
            cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::XGR : TR::InstOpCode::XR, node, valueReg, signReg);
        generateRRInstruction(
            cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::SLGR : TR::InstOpCode::SLR, node, valueReg, signReg);
...
TR::Register* OMR::Z::TreeEvaluator::loadaddrEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (symRef->isUnresolved()) {
        ...
        LRLinst = (TR::S390RILInstruction*)generateRILInstruction(
            cg, TR::InstOpCode::getLoadRelativeLongOpCode(), node, targetRegister, 0xBABE, 0);
            ...
            if (comp->getOption(TR_EnableHCR) && node->getSymbol()->isMethod()
                ...
                if (sym && sym->isStartPC()) {
                    cursor = generateRILInstruction(cg, TR::InstOpCode::LARL, node, targetRegister,
                        node->getSymbolReference(), (uintptrj_t)sym->getStaticAddress(), NULL);
    ...
    if (!node->isArrayCmpLen()) {
        ...
        if (elemsExpr->getOpCode().isLoadConst()) {
            ...
            if (!node->isArrayCmpSign()) {
                resultReg = arraycmpHelper(node, cg,
                    false, // isWideChar
                    true, // isEqualCmp
...
TR::Register* OMR::Z::TreeEvaluator::arraytranslateAndTestEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (!node->isArrayTRT()) {
        ...
        cursor = generateS390BranchInstruction(
            cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node, topOfLoop); // repeat if CPU-defined limit hit
        cursor->setEndInternalControlFlow();
        cursor->setDependencyConditions(dependencies);
    ...
    } else // Use the new loopReducer - CISC Transfomer.
    {
        ...
        if (isAdditionalLen) {
            ...
            cursor
                = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, node, labelSkip);
            cursor->setStartInternalControlFlow();
        ...
        if (isUsingTRT) {
            ...
            cursor = generateS390CompareAndBranchInstruction(
                cg, TR::InstOpCode::CLR, node, indexReg, minReg, TR::InstOpCode::COND_BHRC, labelHelper);
            cursor->setStartInternalControlFlow();
            ...
            generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNZRC, node, labelFind); // Found test char!
            ...
            generateS390CompareAndBranchInstruction(
                cg, TR::InstOpCode::CLR, node, indexReg, minReg, TR::InstOpCode::COND_BNHRC, label256Top);
            ...
            if (isAdditionalLen) {
                instr = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNZRC, node, labelFind); // Find !!
                ...
                generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::CLR, node, indexReg, alenReg, TR::InstOpCode::COND_BLRC, labelEnd);

                instr = generateS390CompareAndBranchInstruction(
                    cg, TR::InstOpCode::CLR, node, lenReg, alenReg, TR::InstOpCode::COND_BNERC, boundCheckFailureLabel);
                cg->stopUsingRegister(lenReg);
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, labelEnd);
            } else {
                instr = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BZRC, node, boundCheckFailureLabel); // not find !!
                instr->setStartInternalControlFlow();
            }
            ...
            generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BORC, node,
            ...
            TR::Instruction* instr;
            if (isAdditionalLen) {
                generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BMRC, node,
                ...
                instr = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BPRC, node,
        ...
        TR::Instruction* cursor = generateS390LabelInstruction(
            cg, TR::InstOpCode::LABEL, node, TR::LabelSymbol::create(cg->trHeapMemory()));
        cursor->setDependencyConditions(dependencies);
        cursor->setEndInternalControlFlow();
...
TR::Register* OMR::Z::TreeEvaluator::arraytranslateEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (node->getTermCharNodeIsHint() && (opCode == TR::InstOpCode::TRTO)) {
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, node,
        ...
        if (node->getSourceCellIsTermChar()) {
            ...
            generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, node,
                afterLoop); // skip after loop if the source cell is not equal to stopChar
    ...
    if (constExpr->getOpCode().isLoadConst()) {
        ...
        case TR::Double: {
            ...
            if (bv == ((value & 0xFF00) >> 8) && bv == ((value & 0xFF0000) >> 16)
                ...
                if (bv == bv2 && (((value & 0xFF00) >> 8) && bv == ((value & 0xFF0000) >> 16)
                                     && bv == ((value & 0xFF000000) >> 24))) {
                    // most likely case is 0
                    mvcCopy = true;
                }
    ...
    if (mvcCopy) {
        ...
        if (varLength) {
            ...
            */
            ...
            TR::MemoryReference(baseReg, 0, cg));
...
TR::Register* OMR::Z::TreeEvaluator::arraycopyEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (isSimpleCopy) {
        ...
        if (byteLenNode->getOpCode().isLoadConst() && getIntegralValue(byteLenNode) <= 0) {
    ...
    if (evaluateChildren) {
        ...
        if (!cg->canClobberNodesRegister(byteSrcNode)) {
            ...
            generateRRInstruction(
                cg, TR::InstOpCode::getLoadRegOpCode(), node, copyByteSrcReg->getGPRofArGprPair(), byteSrcReg);
        ...
        if (!cg->canClobberNodesRegister(byteDstNode)) {
            ...
            generateRRInstruction(
                cg, TR::InstOpCode::getLoadRegOpCode(), node, copyByteDstReg->getGPRofArGprPair(), byteDstReg);
    ...
    if (!isSimpleCopy && !node->isNoArrayStoreCheckArrayCopy()) {
        ...
        if (byteLenReg == NULL) {
            ...
            TR::TreeEvaluator::genArrayCopyWithArrayStoreCHK(
                node, byteSrcObjReg, byteDstObjReg, byteSrcReg, byteDstReg, tempLenReg, cg);
#endif
    ...
    static char* disableAtomic
        = "ON"; // feGetEnv("TR_ArrayCopyDisableAtomic");   //always disable runtime check path for arraycopy

    // If it's a referenced arraycopy (i.e. !isSimpleCopy), we have to ensure the elements are copied atomically - do
    // not use MVC
    // Everything needs to be atomically copied... disabling this path for non 8 bit arrays
    if (isSimpleCopy && ((bool)disableAtomic || node->getArrayCopyElementType() == TR::Int8)
        ...
        if ((bool)disableAtomic) {
            ...
            MemCpyVarLenTypedMacroOp vtOp(node, byteDstNode, byteSrcNode, cg, node->getArrayCopyElementType(),
                byteLenReg, byteLenNode, node->isForwardArrayCopy());
            ...
            if (generateRuntimeMVCOverlap) {
                // use strideRegister to hold difference, since it doesn't get used before the real routine. stride =
                // dstAddr - srcAddr
                cursor = generateRRRInstruction(cg,
                    TR::Compiler->target.is64Bit() ? TR::InstOpCode::SGRK : TR::InstOpCode::SRK, byteSrcNode,
                    strideRegister, byteDstReg, byteSrcReg);

                // mvc if stride <= 0   (src >= dst)
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNHRC, byteSrcNode, mvcRoutine);
                // mvc if length <= stride (no overlap)
                cursor
                    = generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::getCmpRegOpCodeFromNode(byteLenNode),
                        byteSrcNode, byteLenReg, strideRegister, TR::InstOpCode::COND_BNHRC, mvcRoutine, false, false);
            }

            static char* unroll = feGetEnv("TR_ArrayCopyDontUnroll");
            MemCpyAtomicMacroOp vtOp(node, byteDstNode, byteSrcNode, cg, node->getArrayCopyElementType(), byteLenReg,
                byteLenNode, node->isForwardArrayCopy(), (bool)!unroll, isConst ? constantByteLength : -1);
            ...
            if (generateRuntimeMVCOverlap) {
                ...
                cursor = generateS390BranchInstruction(
                    cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, byteSrcNode, endOfCopy);
                ...
                if (byteLenReg == NULL) {
                    ...
                    MemCpyVarLenMacroOp op(node, byteDstNode, byteSrcNode, cg, byteLenReg, byteLenNode,
                        lengthMinusOneTmp, itersReg, raReg);
                    op.setUseEXForRemainder(true);
                ...
                for (int32_t i = 0; i < oldDep->getNumPostConditions(); i++) {
                    ...
                    if (oldDep->getPostConditions()->getRegisterDependency(i)
                        && oldDep->getPostConditions()->getRegisterDependency(i)->getRegister(cg))
    ...
    if (!isSimpleCopy) {
        TR::TreeEvaluator::genWrtbarForArrayCopy(
            node, cg->evaluate(byteSrcObjNode), cg->evaluate(byteDstObjNode), byteSrcNode->isNonNull(), cg);
    }
#endif
...
TR::Register* OMR::Z::TreeEvaluator::iRegLoadEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    if (globalReg == NULL) {
        if (node->getDataType() == TR::Aggregate && node->getSymbol()->getSize() == 8) {
            ...
            // register must be used here
            // so any instructions generated by local RA are the correct size to preserve the upper bits (e.g. use LGR
            // vs LR)
            globalReg = cg->allocate64bitRegister();
        } else {
            globalReg = cg->allocateRegister();
...
TR::Register* OMR::Z::TreeEvaluator::loadAutoOffsetEvaluator(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    TR::MemoryReference* mr
        = generateS390MemoryReference(cg->machine()->getS390RealRegister(TR::RealRegister::GPR0), 0, cg);
...
TR::InstOpCode::S390BranchCondition getButestBranchCondition(TR::ILOpCodes opCode, int32_t cmpValue)
{
    ...
    TR::InstOpCode::S390BranchCondition TMtoTMLLBranchMaskTable[6][4] = { /* Condition Code Value */
        ...
        /* TR::ifiucmpne */ { TR::InstOpCode::COND_MASK7, TR::InstOpCode::COND_MASK9, TR::InstOpCode::COND_MASK15,
            TR::InstOpCode::COND_MASK14 },
        /* TR::ifiucmpgt */ { TR::InstOpCode::COND_MASK7, TR::InstOpCode::COND_MASK1, TR::InstOpCode::COND_MASK1,
            TR::InstOpCode::COND_MASK0 },
        /* TR::ifiucmpge */ { TR::InstOpCode::COND_MASK15, TR::InstOpCode::COND_MASK7, TR::InstOpCode::COND_MASK1,
            TR::InstOpCode::COND_MASK1 },
        /* TR::ifiucmplt */ { TR::InstOpCode::COND_MASK0, TR::InstOpCode::COND_MASK8, TR::InstOpCode::COND_MASK14,
            TR::InstOpCode::COND_MASK14 },
        /* TR::ifiucmple */ { TR::InstOpCode::COND_MASK8, TR::InstOpCode::COND_MASK14, TR::InstOpCode::COND_MASK14,
...
void arraycmpWithPadHelper::generateVarCLCSetup()
{
    ...
    if (isEqual) {
        ...
        generateS390CompareAndBranchInstruction(cg,
            ...
            source1LenReg, TR::InstOpCode::COND_BHRC, doneAssignLabel);
...
void arraycmpWithPadHelper::generateVarCLCRemainder()
{
    ...
    if (TR::Compiler->target.is64Bit()) {
        ...
        generateS390CompareAndBranchInstruction(cg, TR::InstOpCode::CG, node, countReg, (int32_t)0,
            TR::InstOpCode::COND_BERC, isFoldedIf ? unequalLabel : cmpDoneLabel, false, false);
    } else {
...
void arraycmpWithPadHelper::generateConstCLCSpacePaddingLoop()
{
    ...
    if (remain) {
        ...
        // getBranchConditionForMask(getReverseBranchMask((getMaskForBranchCondition(finalBrCond) >> 4) & 0x0E));
 ...
 * TR::CodeGenerator::uncommonIfBifs()
 ...
 * For icall BIF, returnsConditionCodeOrBool flag should be set by including the BIF in idIsBIFReturningConditionCode()
 * during ILGen
 */
    ...
    TR::Node* node, TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op)
{
    ...
    case TR::InstOpCode::VFPSO:
        breakInst = generateVRRaInstruction(
            cg, op, node, returnReg, sourceReg1, 0 /* invert sign */, 0, 3 /* any other value is illegal */);
        break;
    default:
...
TR::Register* inlineStringHashCode(TR::Node* node, TR::CodeGenerator* cg, bool isCompressed)
{
    ...
    if (!isCompressed)
        generateRXYInstruction(
            cg, TR::InstOpCode::LLH, node, registerHash, generateS390MemoryReference(registerValue, registerIndex,
                                                             TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg));
    else
        generateRXYInstruction(
            cg, TR::InstOpCode::LLGC, node, registerHash, generateS390MemoryReference(registerValue, registerIndex,
                                                              TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg));
...
TR::Register* inlineStringHashCodeUnrolled(TR::Node* node, TR::CodeGenerator* cg)
{
    ...
    // registerHash += char at registerIndex
    generateRXInstruction(
        cg, TR::InstOpCode::AH, node, hashRegister, generateS390MemoryReference(valueRegister, indexRegister,
                                                        TR::Compiler->om.contiguousArrayHeaderSizeInBytes(), cg));
    ...
    TR::LabelSymbol* compareTarget, TR::Node* ificmpNode, bool needResultReg, bool return102)
{
    ...
    if (isFoldedIf) {
        ...
        if (isArrayCmp && node->isArrayCmpLen())
            ...
            generateRIInstruction(
                cg, TR::InstOpCode::getLoadHalfWordImmOpCode(), node, resultReg, 0); // Return zero to indicate equal
        generateS390BranchInstruction(cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BRC, node, afterLoop);
    }
...
int32_t getVectorElementSize(TR::Node* node)
{
    ...
    case TR::VectorDouble:
        return 8;
    default:
        TR_ASSERT(false, "Unknown vector node type %i for element size\n", node->getDataType());
        return 0;
    }
}
...
int32_t getVectorElementSizeMask(TR::Node* node)
{
    ...
    case TR::VectorDouble:
        return 3;
    default:
        TR_ASSERT(false, "Unknown vector node type %i for Element Size Control Mask\n", node->getDataType());
        return 0;
    }
}
...
{
    if (node->getDataType() == TR::VectorDouble && canUseNodeForFusedMultiply(node->getFirstChild())
