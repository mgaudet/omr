...
TR::Instruction* MemToMemVarLenMacroOp::generateLoop()
{
    ...
    if (useEXForRemainder()) {
        ...
        _startControlFlow
            = generateS390BranchInstruction(_cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BLRC, _rootNode, _doneLabel);
    }
    ...
    generateRXInstruction(_cg, TR::InstOpCode::LA, _srcNode, _srcReg->getGPRofArGprPair(),
        new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg->getGPRofArGprPair(), 256, _cg));
    if (_srcReg != _dstReg) {
        generateRXInstruction(_cg, TR::InstOpCode::LA, _dstNode, _dstReg->getGPRofArGprPair(),
            new (_cg->trHeapMemory()) TR::MemoryReference(_dstReg->getGPRofArGprPair(), 256, _cg));
    }
...
TR::Instruction* MemToMemConstLenMacroOp::generateLoop()
{
    ...
    if (largeCopies != 0 && largeCopies < _maxCopies) {
        ...
        if (largeCopies * 256 + _offset + srcMROffset >= 4096) {
            ...
            if (_srcReg == NULL)
                ...
                cursor = generateRXInstruction(_cg, TR::InstOpCode::LA, _srcNode, _srcReg->getGPRofArGprPair(),
                    new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg->getGPRofArGprPair(), _offset, _cg), cursor);
            _offset = 0;
        }
        ...
        if (_srcReg == NULL || _dstReg == NULL || _srcReg != _dstReg) {
            ...
            if (largeCopies * 256 + dstMROffset >= 4096) {
                ...
                if (_dstReg == NULL)
                    ...
                    cursor = generateRXInstruction(_cg, TR::InstOpCode::LA, _dstNode, _dstReg->getGPRofArGprPair(),
                        new (_cg->trHeapMemory()) TR::MemoryReference(_dstReg->getGPRofArGprPair(), 0, _cg), cursor);
            }
        }
    ...
    cursor = generateRXInstruction(_cg, TR::InstOpCode::LA, _srcNode, _srcReg->getGPRofArGprPair(),
        new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg->getGPRofArGprPair(), 256, _cg), cursor);
    if (_srcReg != _dstReg) {
        cursor = generateRXInstruction(_cg, TR::InstOpCode::LA, _dstNode, _dstReg->getGPRofArGprPair(),
            new (_cg->trHeapMemory()) TR::MemoryReference(_dstReg->getGPRofArGprPair(), 256, _cg), cursor);
    }
...
TR::Instruction* MemCmpConstLenMacroOp::generateInstruction(int32_t offset, int64_t length, TR::Instruction* cursor1)
{
    ...
    if (!inRemainder()) {
        cursor = generateS390BranchInstruction(
            _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, _rootNode, _falseLabel);
    }
...
TR::Instruction* MemToMemTypedVarLenMacroOp::generateLoop()
{
    ...
    startControlFlow = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::getCmpOpCodeFromNode(_lenNode),
        _dstNode, _lenReg, (int32_t)0, TR::InstOpCode::COND_BNHRC, doneLoop, false, false);
    ...
    if (_isForward) {
        ...
        if (_srcReg != _startReg)
            generateRXInstruction(_cg, TR::InstOpCode::LA, _srcNode, _srcReg->getGPRofArGprPair(),
                new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg->getGPRofArGprPair(), strideSize(), _cg));
        ...
        if (_srcReg != _startReg) {
            generateRXYInstruction(_cg, TR::InstOpCode::LAY, _srcNode, _srcReg->getGPRofArGprPair(),
                new (_cg->trHeapMemory()) TR::MemoryReference(_srcReg->getGPRofArGprPair(), -1 * strideSize(), _cg));
        }
    ...
    TR::InstOpCode::Mnemonic storeOp2)
{
    ...
    cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::getCmpOpCode(), _lenNode, _lenReg,
        strideSize1, TR::InstOpCode::COND_BLRC, skipRoutineLabel);
    ...
    cursor
        = generateS390BranchInstruction(_cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, _srcNode, doneCopyLabel);
...
TR::Instruction* MemCpyAtomicMacroOp::generateLoop()
{
    ...
    startControlFlow = cursor
        = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::getCmpOpCodeFromNode(_lenNode), _dstNode,
            _lenReg, (int32_t)0, TR::InstOpCode::COND_BNHRC, doneArrayCopyLabel, false, false);
    ...
    if (_destType == TR::NoType) {
        ...
        cursor
            = generateRRRInstruction(_cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::OGRK : TR::InstOpCode::ORK,
                _srcNode, _alignedReg, _srcReg, _startReg);

        cursor = generateRILInstruction(
            _cg, TR::InstOpCode::NILF, _srcNode, _alignedReg, 0x7); // 0x7 == 0x111, last 3 bits}

        // NILL/NILF will set condition code to 0 if result 0 (8 byte aligned) or to 1 if result not 0 (
        cursor = generateS390BranchInstruction(
            _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, _srcNode, fourByteLoop); // not aligned with 8 bytes
        // cursor = generateS390CompareAndBranchInstruction(_cg, TR::InstOpCode::C, _srcNode, _alignedReg, 0x0,
        // TR::InstOpCode::COND_BNERC, fourByteLoop); // alignment with 8 bytes
        ...
        cursor = generateS390BranchInstruction(
            _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, _srcNode, twoByteLoop); // not aligned with 4 bytes
        ...
        cursor = generateS390BranchInstruction(
            _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BNERC, _srcNode, oneByteLoop); // not aligned with 2 bytes
        ...
        if (_destType == TR::Int16) {
            cursor = generateRRRInstruction(_cg,
                TR::Compiler->target.is64Bit() ? TR::InstOpCode::NGRK : TR::InstOpCode::NRK, _srcNode, _alignedReg,
                _srcReg, _startReg);
            ...
            cursor = generateS390BranchInstruction(
                _cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_BERC, _srcNode, remainderLabel);
        ...
        cursor
            = generateRRRInstruction(_cg, TR::Compiler->target.is64Bit() ? TR::InstOpCode::AGRK : TR::InstOpCode::ARK,
                _srcNode, _alignedReg, _srcReg, _startReg);
