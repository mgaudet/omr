    ...
    TR::LabelSymbol* branchDestination, bool needsCC, bool targetIsFarAndCold)
{
    ...
    if (!cg->comp()->getOption(TR_DisableCompareAndBranchInstruction) && !needsCC
        ...
        if (cg->getIsInWarmCodeCache() && targetIsFarAndCold) {
            TR_S390WarmToColdTrampolineSnippet* trampolineSnippet = new (INSN_HEAP)
                TR_S390WarmToColdTrampolineSnippet(cg, node, TR::LabelSymbol::create(INSN_HEAP, cg), branchDestination);
    ...
    // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
        ...
        if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, first, second)
            && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 1 remove RR Compare[%s\t "
                                           "%s, %s]: reuse CC from ccInst [%p].",
                   debugObj->getOpCodeName(&opcTmp), debugObj->getName(first), debugObj->getName(second), ccInst))
            ...
            && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 2 remove RR Compare[%s\t "
                                           "%s, %s]: reuse CC from ccInst [%p].",
                   debugObj->getOpCodeName(&opcTmp), debugObj->getName(first), debugObj->getName(second), ccInst))
    ...
    TR::RealRegister::RegNum checkForThisRealRegister = TR::RealRegister::NoReg)
{
    for (int i = 0; i < conds1_addCursor; i++) {
        ...
        for (int j = 0; j < conds2_addCursor; j++) {
            ...
            if ((conds1_real == conds2_real) && (conds1->getRegisterDependency(i)->getRegister(cg)
    ...
    TR::Register* treg, TR::MemoryReference* mf, TR::Instruction* preced)
{
    ...
    TR_ASSERT(treg->getRealRegister() != NULL || // Not in RA
            op != TR::InstOpCode::L || n->couldIgnoreExtend() || !n->force64BitLoad(),
    ...
    if (op == TR::InstOpCode::CVB || op == TR::InstOpCode::EX) {
        ...
        if (BCDCHKNode && BCDCHKNode->getOpCodeValue() == TR::BCDCHK) {
            ...
            TR::Instruction* brcInstr = generateS390BranchInstruction(
                cg, TR::InstOpCode::BRC, TR::InstOpCode::COND_NOP, n, restoreSnippet->getSnippetLabel());
...
TR::Instruction* generateRXInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* treg, uint32_t constForMR, TR::Instruction* preced)
{
    TR::Instruction* instr;
    if (preced)
...
TR::Instruction* generateRILInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* treg, TR::SymbolReference* sr, uintptrj_t imm, TR::Instruction* preced)
{
    if (preced) {
...
TR::Instruction* generateRILInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* treg, uintptrj_t imm, TR::Instruction* preced)
{
...
{
    if (mf)
        preced = mf->separateIndexRegister(n, cg, false, preced); // enforce4KDisplacementLimit=false
...
TR::Instruction* generateRegRegInstruction(TR::CodeGenerator* cg, TR::InstOpCode::Mnemonic op, TR::Node* n,
    TR::Register* sreg, TR::Register* treg, TR::RegisterDependencyConditions* cond, TR::Instruction* preced)
{
    if (preced) {
...
TR::Instruction* generateLogicalImmediate(TR::CodeGenerator* cg, TR::Node* node, TR::InstOpCode::Mnemonic defaultOp,
    TR::InstOpCode::Mnemonic lhOp, TR::InstOpCode::Mnemonic llOp, TR::Register* reg, int32_t imm)
{
...
TR::Instruction* generateAndImmediate(TR::CodeGenerator* cg, TR::Node* node, TR::Register* reg, int32_t imm)
{
    return generateLogicalImmediate(cg, node, TR::InstOpCode::N, TR::InstOpCode::NILH, TR::InstOpCode::NILL, reg, imm);
}

TR::Instruction* generateOrImmediate(TR::CodeGenerator* cg, TR::Node* node, TR::Register* reg, int32_t imm)
{
    ...
    TR::RegisterDependencyConditions* cond, TR::SymbolReference* callSymRef, TR::Instruction* preced)
{
    ...
    if (s->getKind() == TR::Snippet::IsVirtualUnresolved) {
        ...
        generateS390LabelInstruction(
            cg, TR::InstOpCode::LABEL, callNode, TR::LabelSymbol::create(INSN_HEAP, cg), preDeps);
...
TR::Instruction* generateLoadLiteralPoolAddress(TR::CodeGenerator* cg, TR::Node* node, TR::Register* treg)
{
    ...
    TR::S390RILInstruction* LARLinst
        = (TR::S390RILInstruction*)generateRILInstruction(cg, TR::InstOpCode::LARL, node, treg, 0xBABE, 0);
    ...
    TR::RegisterDependencyConditions* deps)
{
    ...
    // dependencies to the EX inst
    // This is to make sure that RA assigns all these registers when we assign the EX instruction
    // Otherwise RA may insert register movement instructions attached to the out of line (lit pool) EX target
    // instruction.
...

    {
        ...
        cursor = generateRXInstruction(
            cg, TR::InstOpCode::EX, node, maskReg, tempMR, (preced != NULL ? preced : comp->getAppendInstruction()));
...
TR::Instruction* splitBaseRegisterIfNeeded(
    TR::MemoryReference* mf1, TR::MemoryReference* mf2, TR::CodeGenerator* cg, TR::Node* node, TR::Instruction* preced)
{
    ...
    if (mf1->getBaseRegister()->getGPRofArGprPair() == mf2->getBaseRegister()->getGPRofArGprPair()) {
        ...
        if (mf1->getBaseRegister()->getARofArGprPair() == mf2->getBaseRegister()->getARofArGprPair())
            return instr;
