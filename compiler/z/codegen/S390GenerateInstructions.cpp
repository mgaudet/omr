    ...
    bool targetIsFarAndCold)
{
    ...
    // otherwise we'll generate with the compare opcode pased and an TR::InstOpCode::BRC.
        ...
        if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, first, second) && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 1 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&opcTmp), debugObj->getName(first), debugObj->getName(second), ccInst))
            returnInstruction = generateS390BranchInstruction(cg, TR::InstOpCode::BRC, bc, node, branchDestination);
        else if (!needsCC && comp->getOption(TR_EnableEBBCCInfo) && cg->isActiveCompareCC(compareOpCode, second, first) && performTransformation(comp, "O^O generateS390CompareAndBranchInstruction case 2 remove RR Compare[%s\t %s, %s]: reuse CC from ccInst [%p].", debugObj->getOpCodeName(&opcTmp), debugObj->getName(first), debugObj->getName(second), ccInst))
    ...
    TR::RealRegister::RegNum checkForThisRealRegister = TR::RealRegister::NoReg)
{
    for (int i = 0; i < conds1_addCursor; i++) {
        ...
        for (int j = 0; j < conds2_addCursor; j++) {
            ...
            if ((conds1_real == conds2_real) && (conds1->getRegisterDependency(i)->getRegister(cg) != conds2->getRegisterDependency(j)->getRegister(cg))) {
                // Conflict found
    ...
    TR::Instruction* preced)
{
    ...
    if (op == TR::InstOpCode::CVB || op == TR::InstOpCode::EX) {
        ...
        if (BCDCHKNode && BCDCHKNode->getOpCodeValue() == TR::BCDCHK) {
            ...
            TR_S390RestoreGPR7Snippet* restoreSnippet = new (INSN_HEAP) TR_S390RestoreGPR7Snippet(cg, n, TR::LabelSymbol::create(INSN_HEAP, cg), label);
...
// we should always see EPReg defined in the post-deps of such calls.
    ...
    } else // address known
    {
        ...
        TR::S390RILInstruction* tempInst = (new (INSN_HEAP) TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, RegRA, imm, callSymRef, cg));
        ...
        if (cg->canUseRelativeLongInstructions(imm) || isHelper) {
            ...
            TR::S390RILInstruction* tempInst = new (INSN_HEAP) TR::S390RILInstruction(TR::InstOpCode::BRASL, callNode, RegRA, imm, cg);
            tempInst->setJITExit();
