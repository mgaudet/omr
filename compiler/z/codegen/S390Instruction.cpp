...
uint8_t* TR::S390LabelInstruction::generateBinaryEncoding()
{
    ...
    if (tryToUseLabelTargetNOPs) {
        ...
        if (tryToUseLabelTargetNOPs) {
            ...
            // aligned but programs in this object may only be 8 byte aligned)
            // instructionStart-cg()->getCodeStart() is the offset just for this one routine or nested routine so have
            // to add in the object size for any already
            ...
            if (traceLabelTargetNOPs) {
                ...
                traceMsg(comp,
                    ...
                    this, comp->getDebug()->getOpCodeName(&thisInst->getOpCode()), instructionStart,
    ...
    if (tryToUseLabelTargetNOPs) {
        ...
        if (offset > labelTargetNOPLimit) {
            ...
            if (traceLabelTargetNOPs)
                traceMsg(comp, "\tset useLabelTargetNOPs = true : offsetForLabelTargetNOPs=%p > labelTargetNOPLimit "
                               "%d, offset=%lld, cursor %p -> %p, labelTargetBytesInserted=%d\n",
    ...
    if (offset && (doUseLabelTargetNOPs || isNopCandidate())) {
        ...
        if (doUseLabelTargetNOPs) {
            if (traceLabelTargetNOPs)
                traceMsg(comp, "\tfinished NOP insertion : setBinaryLength to 0, addAccumError = estBinLen %d - "
...
bool TR::S390LabelInstruction::considerForLabelTargetNOPs(bool inEncodingPhase)
{
    ...
    if (traceLabelTargetNOPs)
        traceMsg(comp, "considerForLabelTargetNOPs check during %s phase : %p (%s), prev  %p (%s)\n",
            inEncodingPhase ? "encoding" : "estimating", this, comp->getDebug()->getOpCodeName(&thisInst->getOpCode()),
            prevInst, comp->getDebug()->getOpCodeName(&prevInst->getOpCode()));

    if (isSkipForLabelTargetNOPs()) {
        if (traceLabelTargetNOPs)
            traceMsg(comp, "\t%p (%s) marked with isSkipForLabelTargetNOPs = true : set doConsider=false\n", this,
                comp->getDebug()->getOpCodeName(&thisInst->getOpCode()));
        doConsider = false;
        ...
        // encoding phases
        // and therefore this routine would return false during estimation and true during encoding and the estimate
        // bump for NOPs would have been missed
        if (traceLabelTargetNOPs)
...
void TR::S390BranchInstruction::assignRegistersAndDependencies(TR_RegisterKinds kindToBeAssigned)
{
    ...
    if (getOpCode().isBranchOp() && getLabelSymbol()->isEndOfColdInstructionStream()) {
        ...
        if (cg()->getIsInOOLSection()) {
            ...
            TR_ASSERT(
                comp->getAppendInstruction() == this, "OOL section must have only one branch to the merge point\n");
...
uint8_t* TR::S390BranchOnIndexInstruction::generateBinaryEncoding()
{
    ...
    if (distance >= MIN_IMMEDIATE_VAL && distance <= MAX_IMMEDIATE_VAL) {
        ...
        *(int16_t*)(cursor + 2) |= bos(distance);
        ...
        TR_ASSERT((opCode == TR::InstOpCode::BRXLE) || (opCode == TR::InstOpCode::BRXH)
                || (opCode == TR::InstOpCode::BRXLG) || (opCode == TR::InstOpCode::BRXHG),
            "Only TR::InstOpCode::BRXLE/TR::InstOpCode::BRXH/TR::InstOpCode::BRXLG/TR::InstOpCode::BRXHG are handled "
            "here\n");
...
int32_t TR::S390RILInstruction::estimateBinaryLength(int32_t currentEstimate)
{
    ...
    if (TR::Compiler->target.is64Bit() && (getOpCode().getOpCodeValue() == TR::InstOpCode::LGRL
        ...
        //
        delta = 24;
    } else if (TR::Compiler->target.is64Bit() && (getOpCode().getOpCodeValue() == TR::InstOpCode::STRL
...
uint8_t* TR::S390RILInstruction::generateBinaryEncoding()
{
    ...
    if (getTargetSnippet() && getTargetSnippet()->getKind() == TR::Snippet::IsConstantData) {
        ...
        toRealRegister(getRegisterOperand(1))->setRegister1Field((uint32_t*)cursor);
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_32BitLabelRelativeRelocation(cursor, getTargetSnippet()->getSnippetLabel()));
        cursor += getOpCode().getInstructionLength();
        ...
        if (getTargetSnippet() != NULL) {
            cg()->addRelocation(new (cg()->trHeapMemory())
                    TR_32BitLabelRelativeRelocation(cursor, getTargetSnippet()->getSnippetLabel()));
...
uint8_t* TR::S390RIEInstruction::generateBinaryEncoding()
{
    ...
    if (getBranchDestinationLabel()) {
        ...
        if (getWarmToColdTrampolineSnippet()) {
            trampolineDistance = (cg()->getBinaryBufferStart()
                                     + getWarmToColdTrampolineSnippet()->getSnippetLabel()->getEstimatedCodeLocation())
                - (instructionStart + cg()->getAccumulatedInstructionLengthError());
        }
        ...
        } else if (getWarmToColdTrampolineSnippet() && trampolineDistance >= MIN_IMMEDIATE_VAL
            && trampolineDistance <= MAX_IMMEDIATE_VAL) {
            cg()->addRelocation(new (cg()->trHeapMemory())
                    TR_16BitLabelRelativeRelocation(cursor, getWarmToColdTrampolineSnippet()->getSnippetLabel()));
...
uint8_t* TR::S390RXInstruction::generateBinaryEncoding()
{
    ...
    if (getMemoryReference() != NULL) {
        ...
        TR_ASSERT((getConstForMRField() & 0xFFF00000) == 0,
            "TR::S390RXInstruction::_constForMRField greater than 0x000FFFFF for instruction 0x%x", this);
...
const char* getOpCodeName(TR::InstOpCode* opCode) { return TR::InstOpCode::opCodeToNameMap[opCode->getOpCodeValue()]; }
...
uint8_t* TR::S390VRIInstruction::generateBinaryEncoding()
{
    ...
    setMaskField(
        (uint32_t*)cursor, maskIn3, 3); // if the format doesn't have this field, the default value will always be zero.
...
uint8_t* TR::S390VRRInstruction::generateBinaryEncoding()
{
    ...
    if (getRegisterOperand(4) != NULL)
        toRealRegister(getRegisterOperand(4))
            ->setRegister4Field((uint32_t*)cursor); // Will cause assertion failure by now
    else
    ...
    if (getKindNOP() == FastLinkCallNOP) { // assumes 4 byte length
        ...
        if (getTargetSnippet() != NULL) {
            ...
            if (distance < 0x3FFF8 && distance > 0) {
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR_16BitLabelRelativeRelocation(cursor + 2, getTargetSnippet()->getSnippetLabel(), 8));
            } else {
                ...
                getCallDescInstr()->setCallDescValue(
                    ((TR_S390ConstantDataSnippet*)getTargetSnippet())->getDataAs8Bytes(), cg()->trMemory());
                cg()->addRelocation(new (cg()->trHeapMemory())
                        TR_16BitLabelRelativeRelocation(cursor + 2, getCallDescInstr()->getCallDescLabel(), 8));
            }
        }
        cursor += 4;
...
uint8_t* TR::S390MIIInstruction::generateBinaryEncoding()
{
    ...
    if (distance >= MIN_12_RELOCATION_VAL && distance <= MAX_12_RELOCATION_VAL && !bpp
        ...
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_12BitLabelRelativeRelocation(cursor, getLabelSymbol(), false));
...
uint8_t* TR::S390SMIInstruction::generateBinaryEncoding()
{
    ...
    if (distance >= MIN_RELOCATION_VAL && distance <= MAX_RELOCATION_VAL && !bpp) {
        ...
        *(cursor) |= ((uint8_t)getMask() << 4);
        ...
        cg()->addRelocation(
            new (cg()->trHeapMemory()) TR_16BitLabelRelativeRelocation(cursor, getLabelSymbol(), 4, true));
    ...
    if (performEmptyPatch) {
        ...
        for (TR::Instruction* nextI = getNext(); nextI != NULL; nextI = nextI->getNext()) {
            ...
            if (node && node->getOpCodeValue() == TR::BBEnd) {
                if (firstBBEnd == NULL)
                    firstBBEnd = node;
                else if (firstBBEnd != node && (node->getBlock()->getNextBlock() == NULL
...
TR::MemoryReference* getFirstReadWriteMemoryReference(TR::Instruction* i)
{
    ...
    default:
        TR_ASSERT(0,
            ...
            i->cg()->getDebug()->getOpCodeName(&i->getOpCode()), i->getKind());
        break;
    }
    return mr;
}
