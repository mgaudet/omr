...
public:
    ...
    {
        TR_ASSERT(getOpCodeValue() == TR::InstOpCode::LOCK || getOpCodeValue() == TR::InstOpCode::UNLOCK,
            "assertion failure");
    ...
    bool matchesTargetRegister(TR::Register* reg)
    {
        ...
        if (isTargetPair()) {
            ...
            if (reg->getRealRegister() && getRegisterOperand(1)->getRealRegister()
                ...
                && GPRmatchesAR(toRealRegister(reg), (TR::RealRegister*)getRegisterOperand(1)))
                return true;
...
public:
    S390RILInstruction(
        TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, uintptrj_t tp, TR::CodeGenerator* cg)
        : TR::Instruction(op, n, cg)
        , _targetSnippet(NULL)
        , _targetSymbol(NULL)
        , _flagsRIL(0)
    ...
    S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, uintptrj_t tp,
    ...
    S390RILInstruction(
        TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t mask, uintptrj_t targetPtr, TR::CodeGenerator* cg)
        : TR::Instruction(op, n, cg)
    ...
    S390RILInstruction(TR::InstOpCode::Mnemonic op, TR::Node* n, uint32_t mask, uintptrj_t targetPtr,
        TR::Instruction* precedingInstruction, TR::CodeGenerator* cg)
        : TR::Instruction(op, n, precedingInstruction, cg)
        , _targetPtr(targetPtr)
        , _targetSnippet(NULL)
...
public:
    ...
    S390RXInstruction(
        TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, uint32_t constMR, TR::CodeGenerator* cg)
        : S390RegInstruction(op, n, treg, cg)
        , _kind(IsRX)
    ...
    S390RXInstruction(TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, uint32_t constMR,
        TR::Instruction* precedingInstruction, TR::CodeGenerator* cg)
        : S390RegInstruction(op, n, treg, precedingInstruction, cg)
        , _kind(IsRX)
        , _constForMRField(constMR)
...
public:
    ...
    S390RXYInstruction(TR::InstOpCode::Mnemonic op, TR::Node* n, TR::Register* treg, uint32_t constMR,
        TR::Instruction* precedingInstruction, TR::CodeGenerator* cg)
...
protected:
    S390VInstruction(TR::CodeGenerator* cg = NULL, TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
    ...
    virtual void setMaskField(uint32_t* instruction, uint8_t mask, int nField)
    {
        TR_ASSERT(nField >= 0 && nField <= 3,
            "Field index out of range."); // field 0 to 3 corresponding to bit 20 to bit 35, 4 bits for each field
...
protected:
    S390VRIInstruction(TR::CodeGenerator* cg = NULL, TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
        TR::Node* n = NULL, TR::Register* targetReg = NULL, uint16_t constantImm = 0, /*  8, 12 or 16 bits */
...
protected:
    S390VStorageInstruction(TR::CodeGenerator* cg = NULL, TR::InstOpCode::Mnemonic op = TR::InstOpCode::BAD,
        TR::Node* n = NULL, TR::Register* targetReg = NULL, /* VRF or GPR */
        TR::Register* sourceReg = NULL, /* VRF or GPR */
        TR::MemoryReference* mr = NULL, uint8_t mask = 0) /* 4 bits  */
        : S390VInstruction(cg, op, n, targetReg),
          _displacement2(0),
          maskField(mask)
    {
