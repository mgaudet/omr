...
TR::SystemLinkage* TR_S390SystemLinkage::self() { return static_cast<TR::SystemLinkage*>(this); }
 ...
 * Non-Java use
 */
TR::SymbolReference* TR_S390SystemLinkage::createAutoMarkerSymbol(TR_S390AutoMarkers markerType)
{
    char* name = NULL;
    switch (markerType) {
 ...
 */
TR::Instruction* TR_S390SystemLinkage::getputFPRs(
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(
                cg(), opcode, node, getS390RealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), rsa, cursor);
            j++;
        }
    }
 ...
 * The operation depends on supplied opcode
 */
TR::Instruction* TR_S390SystemLinkage::getputARs(
    TR::InstOpCode::Mnemonic opcode, TR::Instruction* cursor, TR::Node* node, TR::RealRegister* spReg)
{
    int16_t ARSaveMask;
    int32_t offset, firstSaved, lastSaved, beginSaveOffset;
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (ARSaveMask & (1 << (i))) {
            ...
            rsa = generateS390MemoryReference(spReg, offset, cg());
            cursor
                = generateRSInstruction(cg(), opcode, node, getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)),
...
TR::Instruction* TR_S390SystemLinkage::getputHPRs(
    TR::InstOpCode::Mnemonic opcode, TR::Instruction* cursor, TR::Node* node, TR::RealRegister* spReg)
{
    int16_t HPRSaveMask;
    int32_t offset, firstSaved, lastSaved, beginSaveOffset;
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        ...
        if (previousWasOne && (i == lastSaved || !(HPRSaveMask & (1 << i)))) {
            ...
            cursor
                = generateRSInstruction(cg(), opcode, node, getS390RealRegister(REGNUM(k + TR::RealRegister::FirstGPR)),
 ...
 */
TR::Instruction* TR_S390SystemLinkage::addImmediateToRealRegister(TR::RealRegister* targetReg, int32_t value,
 ...
 */
uint32_t TR_S390zOSSystemLinkage::calculateInterfaceMappingFlags(TR::ResolvedMethodSymbol* method)
{
 ...
 */
uint32_t TR_S390zOSSystemLinkage::calculateReturnValueAdjustFlag(TR::DataType dataType, int32_t aggregateLength)
{
 ...
 * determined.
 */
bool TR_S390zOSSystemLinkage::updateFloatParmDescriptorFlags(uint32_t* parmDescriptorFields, TR::Symbol* funcSymbol,
    ...
    : TR::SystemLinkage(codeGen, TR_SystemLinux)
{
    ...
    setRegisterFlag(TR::RealRegister::GPR12, Preserved);
    setRegisterFlag(TR::RealRegister::GPR13, Preserved);
    setRegisterFlag(TR::RealRegister::GPR14, Preserved); // Setting the return address register as preserved so that way
                                                         // we do not always have to emit stores in prologues /
                                                         // epilogues
 ...
 */
void TR_S390SystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
 ...
 */
TR::Register* TR_S390SystemLinkage::callNativeFunction(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
    return 0;
 ...
 * Front-end customization of callNativeFunction
 */
void TR_S390zOSSystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
 ...
 */
TR::Register* TR_S390zOSSystemLinkage::callNativeFunction(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
 ...
 * Front-end customization of callNativeFunction
 */
void TR_S390zLinuxSystemLinkage::generateInstructionsForCall(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
    TR::CodeGenerator* codeGen = cg();
 ...
 */
TR::Register* TR_S390zLinuxSystemLinkage::callNativeFunction(TR::Node* callNode, TR::RegisterDependencyConditions* deps,
    intptrj_t targetAddress, TR::Register* methodAddressReg, TR::Register* javaLitOffsetReg,
    TR::LabelSymbol* returnFromJNICallLabel, TR_S390JNICallDataSnippet* jniCallDataSnippet, bool isJNIGCPoint)
{
    /********************************/
 ...
 */
void TR_S390SystemLinkage::mapStack(TR::ResolvedMethodSymbol* method, uint32_t stackIndex)
{
    int32_t i;
...
TR::Instruction* TR_S390SystemLinkage::saveGPRsInPrologue2(TR::Instruction* cursor)
{
    ...
    if (lastReg - firstReg == 0)
        ...
        cursor = generateRSInstruction(cg(), TR::InstOpCode::getStoreMultipleOpCode(), firstNode,
            getS390RealRegister(REGNUM(firstReg)), getS390RealRegister(REGNUM(lastReg)), retAddrMemRef, cursor);
    ...
    if (getPreserved(REGNUM(i)) && (getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getStoreOpCode(), firstNode, getS390RealRegister(REGNUM(i)), retAddrMemRef, cursor);
    }
...
TR::Instruction* TR_S390SystemLinkage::saveGPRsInPrologue(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    ...
    for (i = lastReg = firstReg = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR; ++i) {
        ...
        if (getPreserved(REGNUM(i))) {
            ...
            if (!findStartReg && (!(getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()
                ...
                if (lastReg - firstReg == 0)
                    ...
                    cursor = generateRSInstruction(cg(), TR::InstOpCode::getStoreMultipleOpCode(), firstNode,
                        getS390RealRegister(REGNUM(firstReg)), getS390RealRegister(REGNUM(lastReg)), retAddrMemRef,
                        cursor);
    ...
    int32_t stackFrameSize, TR::Instruction** callInstruction)
{
        ...
        || getHPRSaveMask()) // don't need to check gprmask or hpr mask as these are saved in caller stack.
    {
        if (comp()->getOption(TR_TraceCG))
            traceMsg(comp(),
                "LeafRoutine: stack pointer isUsedInMemRef = %d  or arsavemask = %d  or hprsavemask = %d is set to 1\n",
...
bool OMR::Z::Linkage::checkPreservedRegisterUsage(bool* regs, int32_t regsSize)
{
    ...
    for (i = TR::RealRegister::FirstGPR; i <= TR::RealRegister::LastGPR - 2;
         ++i) // special case for r14, the return value
    {
        // TR_ASSERT(regs[i] == (getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod(), "Mismatch between
        // register assigned information!!\n");
...
void TR_S390SystemLinkage::createPrologue(TR::Instruction* cursor)
{
    ...
    if (comp()->getOption(TR_TraceCG))
        traceMsg(comp(), "initial stackFrameSize: stack size %d = offset to first parm %d + arg size %d + localSize "
                         "%d, rounded up to 8\n",
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (FPRSaveMask & (1 << (i))) {
            ...
            cursor = generateRXInstruction(cg(), TR::InstOpCode::STD, firstNode,
                getS390RealRegister(REGNUM(i + TR::RealRegister::FirstFPR)), retAddrMemRef, cursor);
        }
    }
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (ARSaveMask & (1 << (i))) {
            ...
            cursor = generateRSInstruction(cg(), TR::InstOpCode::STAM, firstNode,
...
TR::Instruction* TR_S390SystemLinkage::restoreGPRsInEpilogue2(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
    int32_t offset = 0;
    ...
    if (getPreserved(REGNUM(i)) && (getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
        ...
        TR::MemoryReference* retAddrMemRef = generateS390MemoryReference(spReg, offset + stackFrameSize, cg());
        cursor = generateRXInstruction(
            cg(), TR::InstOpCode::getLoadOpCode(), nextNode, getS390RealRegister(REGNUM(i)), retAddrMemRef, cursor);
    }

    return cursor;
}

TR::Instruction* TR_S390SystemLinkage::restoreGPRsInEpilogue(TR::Instruction* cursor)
{
    int16_t GPRSaveMask = 0;
        ...
        if (getPreserved(REGNUM(i))) {
            ...
            if ((getS390RealRegister(REGNUM(i)))->getHasBeenAssignedInMethod()) {
                if ((!specializedEpilogues)
                    || restoreRegister(getS390RealRegister(REGNUM(i))->getRegisterNumber(), blockNumber)) {
                    if (comp()->getOption(TR_TraceCG))
                        traceMsg(comp(), "\tand Assigned. ");
                    findStartReg = false;
                }
...
void TR_S390SystemLinkage::createEpilogue(TR::Instruction* cursor)
{
    ...
    if (_notifiedOfalloca) {
        cursor = generateRRInstruction(cg(), TR::InstOpCode::getLoadRegOpCode(), nextNode, spReg,
    ...
    for (i = firstSaved; i <= lastSaved; ++i) {
        if (ARSaveMask & (1 << (i))) {
            cursor = generateRSInstruction(cg(), TR::InstOpCode::LAM, nextNode,
                getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)),
                getS390RealRegister(REGNUM(i + TR::RealRegister::FirstAR)), retAddrMemRef, cursor);
        }
    }
    ...
    cursor = generateS390RegInstruction(
        cg(), TR::InstOpCode::BCR, nextNode, getS390RealRegister(REGNUM(TR::RealRegister::GPR14)), cursor);
...
int32_t TR_S390zLinuxSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
...
int32_t TR_S390zOSSystemLinkage::getRegisterSaveOffset(TR::RealRegister::RegNum srcReg)
{
    int32_t offset;
...
bool TR_S390zOSSystemLinkage::isAggregateReturnedInIntRegisters(int32_t aggregateLenth)
{
 ...
 */
uint32_t TR_S390zOSSystemLinkage::calculateCallDescriptorFlags(TR::Node* callNode)
{
