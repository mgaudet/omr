...
public:
	...
	virtual void
	doFrequentObjectAllocationSampling(MM_EnvironmentBase *env)
	{
	}
	virtual bool
	checkForExcessiveGC(MM_EnvironmentBase *env, MM_Collector *collector)
	{
		return false;
	}
	...
	virtual bool
	globalCollector_isTimeForGlobalGCKickoff()
	{
		return false;
	}
	virtual void
	globalCollector_internalPostCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace)
	{
	}

	virtual void
	parallelGlobalGC_masterThreadGarbageCollect_beforeGC(MM_EnvironmentBase *env)
	{
	}
	virtual void
	parallelGlobalGC_masterThreadGarbageCollect_afterGC(MM_EnvironmentBase *env, bool compactThisCycle)
	{
	}
	virtual void
	...
	virtual void
	parallelGlobalGC_postMarkProcessing(MM_EnvironmentBase *env)
	{
	}
	virtual void
	parallelGlobalGC_setupBeforeGC(MM_EnvironmentBase *env)
	{
	}
	virtual void
	parallelGlobalGC_setMarkingScheme(MM_EnvironmentBase *env, void *markingScheme)
	{
		_markingScheme = (MM_MarkingScheme *)markingScheme;
	}
	virtual bool
	parallelGlobalGC_createAccessBarrier(MM_EnvironmentBase *env)
	{
		return true;
	}
	virtual void
	parallelGlobalGC_destroyAccessBarrier(MM_EnvironmentBase *env)
	{
	}
	virtual bool
	parallelGlobalGC_heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
								  void *lowAddress, void *highAddress)
	{
		return true;
	}
	virtual bool
	parallelGlobalGC_heapRemoveRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size,
									 void *lowAddress, void *highAddress, void *lowValidAddress, void *highValidAddress)
	{
		return true;
	}
	virtual bool
	parallelGlobalGC_createHeapWalker(MM_EnvironmentBase *env, MM_GlobalCollector *collector, MM_MarkMap *markMap)
	{
		return true;
	}
	virtual void
	parallelGlobalGC_destroyHeapWalker(MM_EnvironmentBase *env)
	{
	}
	virtual MM_HeapWalker *
	...
	virtual void
	parallelGlobalGC_masterThreadGarbageCollect_gcComplete(MM_EnvironmentBase *env, bool didCompact)
	{
	}
	virtual void
	parallelGlobalGC_collectorInitialized(MM_EnvironmentBase *env)
	{
	}
	virtual void
	parallelGlobalGC_reportObjectEvents(MM_EnvironmentBase *env)
	{
	}
	...
	virtual bool
	concurrentGC_forceConcurrentKickoff(MM_EnvironmentBase *env, uintptr_t gcCode, uintptr_t *languageKickoffReason)
	{
	...
	virtual uintptr_t concurrentGC_collectRoots(MM_EnvironmentStandard *env, uintptr_t concurrentStatus,
	...
	virtual void
	concurrentGC_kickoffCardCleaning(MM_EnvironmentStandard *env)
	{
	}
	virtual void
	concurrentGC_flushRegionReferenceLists(MM_EnvironmentBase *env)
	{
	}
	virtual void
	...
	virtual bool
	concurrentGC_startConcurrentScanning(MM_EnvironmentStandard *env, uintptr_t *bytesTraced, bool *collectedRoots)
	{
		*bytesTraced = 0;
	...
	virtual void
	concurrentGC_concurrentScanningStarted(MM_EnvironmentStandard *env, bool isConcurrentScanningComplete)
	{
	}
	virtual bool
	...
	virtual void
	concurrentGC_scanThread(MM_EnvironmentBase *env)
	{
	}
	...
	virtual uintptr_t markingScheme_scanObject(MM_EnvironmentBase *env, omrobjectptr_t objectPtr,
											   MarkingSchemeScanReason reason);
#if defined(OMR_GC_MODRON_CONCURRENT_MARK)
	virtual uintptr_t markingScheme_scanObjectWithSize(MM_EnvironmentBase *env, omrobjectptr_t objectPtr,
													   MarkingSchemeScanReason reason, uintptr_t sizeToDo);
#endif /* OMR_GC_MODRON_CONCURRENT_MARK */

	virtual bool
	collectorHeapRegionDescriptorInitialize(MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region)
	{
		return true;
	}
	virtual void
	collectorHeapRegionDescriptorTearDown(MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region)
	{
	}
	...
	virtual void
	workPacketOverflow_overflowItem(MM_EnvironmentBase *env, omrobjectptr_t objectPtr)
	{
	}
	...
	virtual bool scavenger_internalGarbageCollect_shouldPercolateGarbageCollect(MM_EnvironmentBase *envBase,
																				PercolateReason *reason,
																				uint32_t *gcCode);
	virtual GC_ObjectScanner *scavenger_getObjectScanner(MM_EnvironmentStandard *env, omrobjectptr_t objectPtr,
	...
	virtual void scavenger_fixupDestroyedSlot(MM_EnvironmentBase *env, MM_ForwardedHeader *forwardedHeader,
