...
public:
     ...
     */
    MMINLINE void setFlags(omrobjectptr_t objectPtr, uintptr_t bitsToClear, uintptr_t bitsToSet)
    {
    ...
    MMINLINE uintptr_t getConsumedSizeInSlotsWithHeader(omrobjectptr_t objectPtr)
    {
    ...
    MMINLINE uintptr_t getConsumedSizeInBytesWithHeaderForMove(omrobjectptr_t objectPtr)
    {
        return getConsumedSizeInBytesWithHeader(objectPtr);
    }

    MMINLINE uintptr_t getSizeInBytesWithHeader(omrobjectptr_t objectPtr) { return OMR_OBJECT_SIZE(objectPtr); }

    MMINLINE void setObjectSize(omrobjectptr_t objectPtr, uintptr_t size, bool preserveAgeAndFlags = true)
    {
    ...
    MMINLINE void preMove(OMR_VMThread* vmThread, omrobjectptr_t objectPtr) { /* do nothing */ }

    MMINLINE void postMove(OMR_VMThread* vmThread, omrobjectptr_t objectPtr) { /* do nothing */ }

#if defined(OMR_GC_MODRON_SCAVENGER)
    /**
     * Return true if the object holds references to heap objects not reachable from reference graph (eg, heap object
     * bound to associated class)
     ...
     */
    MMINLINE bool hasIndirectObjectReferents(void* vmThread, omrobjectptr_t objectPtr) { return false; }
     ...
     */
    MMINLINE uint32_t getRealAge(omrobjectptr_t objectPtr) { return OMR_OBJECT_AGE(objectPtr); }
     ...
     */
    MMINLINE bool atomicSetObjectCurrentlyReferencedAndRemembered(omrobjectptr_t objectPtr)
    {
        bool result = true;
     ...
     */
    MMINLINE bool hasBeenMoved(MM_ForwardedHeader* forwardedHeader) { return false; }
     ...
     */
    MMINLINE bool hasBeenHashed(MM_ForwardedHeader* forwardedHeader) { return false; }
     ...
     */
    MMINLINE bool isHasBeenHashedBitSet(MM_ForwardedHeader* forwardedHeader) { return false; }
     ...
     * @return TRUE if an object has been moved after being hashed, FALSE otherwise
     */
    MMINLINE bool hasBeenRecentlyMoved(MM_ForwardedHeader* forwardedHeader) { return false; }
     ...
     */
    MMINLINE bool isObjectJustHasBeenMoved(MM_ForwardedHeader* forwardedHeader) { return false; }
     ...
     * @return TRUE if an object's has been hashed bit set, FALSE otherwise
     */
    MMINLINE bool isObjectJustHasBeenMoved(omrobjectptr_t objectPtr) { return false; }
     ...
     * @return TRUE if an object's has been hashed bit set, FALSE otherwise
     */
    MMINLINE void calculateObjectDetailsForCopy(MM_ForwardedHeader* forwardedHeader, uintptr_t* objectCopySizeInBytes,
