...
TEST(OmrAlgoTest, avltest)
{
  ...
  if (avlTestFile) {
    if (verifyAVLTree(privateOmrPortLibrary, avlTestFile, &passCount,
                      &failCount)) {
      numSuitesNotRun++;
    }
  } else {
...
static void
showResult(OMRPortLibrary* portlib, uintptr_t passCount, uintptr_t failCount,
           int32_t numSuitesNotRun)
{
  ...
  EXPECT_TRUE(0 == numSuitesNotRun) << numSuitesNotRun << " SUITE"
                                    << (numSuitesNotRun != 1 ? "S" : "")
                                    << " NOT RUN!!!";
