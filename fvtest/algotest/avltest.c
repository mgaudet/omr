...
static uintptr_t get_node_string(
    OMRPortLibrary* portlib, J9AVLTree* tree, J9AVLTestNode* walk, char* buffer, uintptr_t bufSize);
    ...
    OMRPortLibrary* portlib, J9AVLTree* tree, J9AVLTestNode* walk, char* buffer, uintptr_t bufSize)
{
    ...
    len += get_node_string(
        portlib, tree, (J9AVLTestNode*)AVL_SRP_GETNODE(walk->leftChild), &buffer[len], bufSize - len);
    len += get_node_string(
        portlib, tree, (J9AVLTestNode*)AVL_SRP_GETNODE(walk->rightChild), &buffer[len], bufSize - len);
