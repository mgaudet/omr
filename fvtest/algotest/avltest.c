...
int32_t
verifyAVLTree(OMRPortLibrary* portLib, char* testListFile, uintptr_t* passCount,
              uintptr_t* failCount)
{
  J9AVLTree tree;
...
static uintptr_t
get_datum_string(OMRPortLibrary* portlib, J9AVLTestNode* walk, char* buffer,
                 uintptr_t bufSize)
{
  OMRPORT_ACCESS_FROM_OMRPORT(portlib);
...
static intptr_t
testInsertionComparator(J9AVLTree* tree, J9AVLTestNode* insertNode,
                        J9AVLTestNode* walkNode)
{
...
static void
avl_get_string(OMRPortLibrary* portlib, J9AVLTree* tree, char* buffer,
               uintptr_t bufSize)
{
  uintptr_t len = get_node_string(portlib, tree, (J9AVLTestNode*)tree->rootNode,
                                  buffer, bufSize);
  if (len >= bufSize) {
    buffer[bufSize - 1] = 0;
  } else {
...
static uintptr_t
get_node_string(OMRPortLibrary* portlib, J9AVLTree* tree, J9AVLTestNode* walk,
                char* buffer, uintptr_t bufSize)
{
  uintptr_t len;
