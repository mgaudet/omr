...
static void testCollisionResilientHashTable(OMRPortLibrary* portLib, char* id, uintptr_t* data, uintptr_t dataLength,
    uintptr_t* passCount, uintptr_t* failCount, BOOLEAN forceCollisions, uint32_t listToTreeThreshold);
...
int32_t verifyHashtable(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
    ...
    testHashtable(
        portLib, "Standard NoForce test1", data1, sizeof(data1) / sizeof(uintptr_t), passCount, failCount, FALSE);
    testHashtable(
        portLib, "Standard NoForce test2", data2, sizeof(data2) / sizeof(uintptr_t), passCount, failCount, FALSE);
    testHashtable(
        portLib, "Standard NoForce test3", data3, sizeof(data3) / sizeof(uintptr_t), passCount, failCount, FALSE);
    testHashtable(
        portLib, "Standard NoForce test4", data4, sizeof(data4) / sizeof(uintptr_t), passCount, failCount, FALSE);
    testHashtable(
        portLib, "Standard NoForce test5", data5, sizeof(data5) / sizeof(uintptr_t), passCount, failCount, FALSE);
    testHashtable(
        portLib, "Standard NoForce test6", data6, sizeof(data6) / sizeof(uintptr_t), passCount, failCount, FALSE);
    ...
    testSetStart = omrtime_hires_clock();
    testHashtable(
        portLib, "Standard Force test1", data1, sizeof(data1) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testHashtable(
        portLib, "Standard Force test2", data2, sizeof(data2) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testHashtable(
        portLib, "Standard Force test3", data3, sizeof(data3) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testHashtable(
        portLib, "Standard Force test4", data4, sizeof(data4) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testHashtable(
        portLib, "Standard Force test5", data5, sizeof(data5) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testHashtable(
        portLib, "Standard Force test6", data6, sizeof(data6) / sizeof(uintptr_t), passCount, failCount, TRUE);
    testSetEnd = omrtime_hires_clock();
    testDelta = omrtime_hires_delta(testSetStart, testSetEnd, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
    ...
    for (listToTreeThresholdIndex = 0; listToTreeThresholdIndex < sizeof(listToTreeThresholdValues) / sizeof(uint32_t);
        ...
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test1", data1,
            sizeof(data1) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test2", data2,
            sizeof(data2) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test3", data3,
            sizeof(data3) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test4", data4,
            sizeof(data4) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test5", data5,
            sizeof(data5) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient NoForce test6", data6,
            sizeof(data6) / sizeof(uintptr_t), passCount, failCount, FALSE, listToTreeThreshold);
        testSetEnd = omrtime_hires_clock();
        testDelta = omrtime_hires_delta(testSetStart, testSetEnd, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
    ...
    for (listToTreeThresholdIndex = 0; listToTreeThresholdIndex < sizeof(listToTreeThresholdValues) / sizeof(uint32_t);
        ...
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test1", data1,
            sizeof(data1) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test2", data2,
            sizeof(data2) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test3", data3,
            sizeof(data3) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test4", data4,
            sizeof(data4) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test5", data5,
            sizeof(data5) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testCollisionResilientHashTable(portLib, "CollisionResilient Force test6", data6,
            sizeof(data6) / sizeof(uintptr_t), passCount, failCount, TRUE, listToTreeThreshold);
        testSetEnd = omrtime_hires_clock();
        testDelta = omrtime_hires_delta(testSetStart, testSetEnd, OMRPORT_TIME_DELTA_IN_MICROSECONDS);
    ...
    for (i = 0; i < sizeof(RandomValues); i++) {
        ...
        for (listToTreeThresholdIndex = 0;
            ...
            testCollisionResilientHashTable(
                portLib, name, randData, randSize, passCount, failCount, FALSE, listToTreeThreshold);
            if (orgFail != *failCount) {
                printRandomData(portLib, randData, randSize);
            }
        }
        ...
        for (listToTreeThresholdIndex = 0;
            ...
            testCollisionResilientHashTable(
                portLib, name, randData, randSize, passCount, failCount, TRUE, listToTreeThreshold);
            if (orgFail != *failCount) {
                printRandomData(portLib, randData, randSize);
            }
        }
