...
/* Allocates memory */
static void *
customAlloc(J9PoolUserData *userData, uint32_t size, const char *callSite, uint32_t memoryCategory, uint32_t type,
			uint32_t *doInit)
{
...
createNewPools(OMRPortLibrary *portLib, uintptr_t *passCount, uintptr_t *failCount)
{
	...
	for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
		...
		resultData->poolResult = pool_new(inputData->structSize, inputData->numberElements, inputData->elementAlignment,
										  inputData->poolFlags, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM,
										  (omrmemAlloc_fptr_t)customAlloc, (omrmemFree_fptr_t)customFree, userData);
...
testNewElement(OMRPortLibrary *portLib, uintptr_t *passCount, uintptr_t *failCount)
{
	...
	for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
		...
		if (resultData->puddleAllocSize != puddleAllocSize) {
			...
			omrtty_printf(
				"Error: Pool initialized with minElements=%d but only %d elements were allocated in puddle, in pool "
				"test %d\n",
	...
	for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
		...
		do {
			...
			do {
				...
				if (!pool_includesElement(currentPool, element)) {
					omrtty_printf(
						"Error: pool_includesElement returned false before removing for walked element in pool test "
						"%d\n",
						poolCntr);
					numFailed++;
				}
