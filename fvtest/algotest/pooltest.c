...
static void* customAlloc(J9PoolUserData* userData, uint32_t size, const char* callSite, uint32_t memoryCategory,
...
static intptr_t createNewPools(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        resultData->poolResult = pool_new(inputData->structSize, inputData->numberElements, inputData->elementAlignment,
            inputData->poolFlags, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_VM, (omrmemAlloc_fptr_t)customAlloc,
        ...
        if (resultData->puddleAllocSize < ((expectedNumElems * ROUND_TO(expectedAlignment, inputData->structSize))
                                              + ROUND_TO(expectedAlignment, sizeof(J9PoolPuddle)))) {
            omrtty_printf("Error: Puddle size too small for pool test %d\n", poolCntr);
            (*failCount)++;
        } else if ((inputData->poolFlags & POOL_ROUND_TO_PAGE_SIZE) && (resultData->puddleAllocSize < OS_PAGE_SIZE)) {
            omrtty_printf(
                "Error: Allocation size did not respect POOL_ROUND_TO_PAGE_SIZE flag for pool test %d\n", poolCntr);
            (*failCount)++;
        } else {
            (*passCount)++;
        }
...
static void testNewElement(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        while (prevPuddleSize == 0) {
            ...
            prevPuddleSize
                = resultData->puddleAllocSize; /* puddleAllocSize will be updated when the next puddle is allocated */
        ...
        if (resultData->puddleAllocSize != puddleAllocSize) {
            omrtty_printf(
                "Error: New puddle allocated is not the same size as the previous puddle in pool test %d\n", poolCntr);
            (*failCount)++;
        } else if (elementsInPuddle < expectedNumElems) {
            omrtty_printf("Error: Pool initialized with minElements=%d but only %d elements were allocated in puddle, "
                          "in pool test %d\n",
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        do {
            ...
            if (!pool_includesElement(currentPool, element)) {
                omrtty_printf(
                    "Error: pool_includesElement returned false for walked element in pool test %d\n", poolCntr);
                numFailed++;
            }
            walkCount++;
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        do {
            ...
            do {
                expectedNumElements--;
                if (!pool_includesElement(currentPool, element)) {
                    omrtty_printf("Error: pool_includesElement returned false before removing for walked element in "
                                  "pool test %d\n",
                        poolCntr);
                    numFailed++;
                }
                ...
                if (pool_numElements(currentPool) != expectedNumElements) {
                    omrtty_printf(
                        "Error: pool_numElements returned the wrong value while removing in pool test %d\n", poolCntr);
                    numFailed++;
                }
                if (pool_includesElement(currentPool, element)) {
                    omrtty_printf(
                        "Error: pool_includesElement returned true while removing for walked element in pool test %d\n",
                        poolCntr);
                    numFailed++;
                }
...
static void testKill(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
    ...
    for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
        ...
        inputData->poolFlags
            &= ~POOL_NEVER_FREE_PUDDLES; /* Remove this flag so that the custom free functions don't complain */
...
static void testPuddleListSharing(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
    ...
    for (index = 1; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
        if (element != pool_startDo(pool[index], &state[index])) {
            omrtty_printf(
                "Error: Elements returned in different order when iterating over pools with shared puddle lists");
            (*failCount)++;
        } else {
            (*passCount)++;
        }
