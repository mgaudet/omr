...
static void *customAlloc(J9PoolUserData *userData, uint32_t size,
  ...
  if (type == POOL_ALLOC_TYPE_POOL && resultData->poolAllocSize == 0) {
    ...
    omrtty_printf(
        "Error: Alloc called with type!=POOL_ALLOC_TYPE_POOL but "
        "poolAllocSize==0\n");
...
static intptr_t createNewPools(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    if (resultData->puddleAllocSize <
      ...
      omrtty_printf(
          "Error: Allocation size did not respect POOL_ROUND_TO_PAGE_SIZE flag "
          "for pool test %d\n",
          poolCntr);
      (*failCount)++;
    } else {
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    while (prevPuddleSize == 0) {
      ...
      for (j = 0; j < inputData->structSize; j++) {
        if (walkBytes[j] != 0) {
          omrtty_printf(
              "Error: pool_newElement non-zero byte at offset %d in pool test "
              "%d\n",
              j, poolCntr);
          (*failCount)++;
        }
    ...
    if (resultData->puddleAllocSize != puddleAllocSize) {
      omrtty_printf(
          "Error: New puddle allocated is not the same size as the previous "
          "puddle in pool test %d\n",
          poolCntr);
      (*failCount)++;
    } else if (elementsInPuddle < expectedNumElems) {
      omrtty_printf(
          "Error: Pool initialized with minElements=%d but only %d elements "
          "were allocated in puddle, in pool test %d\n",
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    do {
      ...
      if (!pool_includesElement(currentPool, element)) {
        omrtty_printf(
            "Error: pool_includesElement returned false for walked element in "
            "pool test %d\n",
            poolCntr);
        numFailed++;
      }
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    do {
      ...
      do {
        ...
        if (!pool_includesElement(currentPool, element)) {
          omrtty_printf(
              "Error: pool_includesElement returned false before removing for "
              "walked element in pool test %d\n",
              poolCntr);
          numFailed++;
        }
        ...
        if (pool_numElements(currentPool) != expectedNumElements) {
          omrtty_printf(
              "Error: pool_numElements returned the wrong value while removing "
              "in pool test %d\n",
              poolCntr);
          numFailed++;
        }
        if (pool_includesElement(currentPool, element)) {
          omrtty_printf(
              "Error: pool_includesElement returned true while removing for "
              "walked element in pool test %d\n",
              poolCntr);
          numFailed++;
        }
    ...
    if (expectedNumElements != 0) {
      omrtty_printf(
          "Error: unexpected error - expectedNumElements is not zero for pool "
          "test %d\n",
          poolCntr);
      numFailed++;
    }
...
static void testPuddleListSharing(OMRPortLibrary *portLib, uintptr_t *passCount,
  ...
  for (index = 1; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    if (numElements != pool_numElements(pool[index])) {
      omrtty_printf(
          "Error: pool_numElements did not match expected value "
          "in shared puddle list test");
      (*failCount)++;
    } else {
      (*passCount)++;
  ...
  for (index = 1; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    if (element != pool_startDo(pool[index], &state[index])) {
      omrtty_printf(
          "Error: Elements returned in different order when iterating over "
          "pools with shared puddle lists");
      (*failCount)++;
    } else {
      (*passCount)++;
    }
