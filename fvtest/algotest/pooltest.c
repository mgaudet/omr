...
/* Table of parameters for pool_new, which will be used to test the various pool
 * API functions. */
static J9NewPoolInputData INPUTDATA[] = {
  ...
  { 16, 0, sizeof(uintptr_t), 0,
    0 }, /* page size pool - with 16-byte elements */
  ...
  { 64, 0, sizeof(uintptr_t), 0,
    0 },               /* zero minElements - should default to 1 */
  { 64, 10, 0, 0, 0 }, /* zero alignment - should default to MIN_GRANULARITY */
  { 32, 10, sizeof(uintptr_t), 0, POOL_NO_ZERO }, /* POOL_NO_ZERO flag */
  { 32, 10, sizeof(uintptr_t), 0,
    POOL_ALWAYS_KEEP_SORTED }, /* POOL_ALWAYS_KEEP_SORTED flag */
  { 32, 10, sizeof(uintptr_t), 0,
    POOL_ROUND_TO_PAGE_SIZE }, /* POOL_ROUND_TO_PAGE_SIZE flag */
  { 32, 10, sizeof(uintptr_t), 0,
    POOL_NEVER_FREE_PUDDLES } /* POOL_NEVER_FREE_PUDDLES flag */
};
...
static intptr_t createNewPools(OMRPortLibrary* portLib, uintptr_t* passCount,
                               uintptr_t* failCount);
static void testNewElement(OMRPortLibrary* portLib, uintptr_t* passCount,
                           uintptr_t* failCount);
static void testWalkFunctions(OMRPortLibrary* portLib, uintptr_t* passCount,
                              uintptr_t* failCount);
static void testRemoveElement(OMRPortLibrary* portLib, uintptr_t* passCount,
                              uintptr_t* failCount);
static void testKill(OMRPortLibrary* portLib, uintptr_t* passCount,
                     uintptr_t* failCount);
static void testClear(OMRPortLibrary* portLib, uintptr_t* passCount,
                      uintptr_t* failCount);
static void testPuddleListSharing(OMRPortLibrary* portLib, uintptr_t* passCount,
                                  uintptr_t* failCount);

static void* customAlloc(J9PoolUserData* userData, uint32_t size,
...
/* Allocates memory */
static void*
customAlloc(J9PoolUserData* userData, uint32_t size, const char* callSite,
            uint32_t memoryCategory, uint32_t type, uint32_t* doInit)
{
  ...
  if (type == POOL_ALLOC_TYPE_POOL && resultData->poolAllocSize == 0) {
    ...
    omrtty_printf("Error: Alloc called with type!=POOL_ALLOC_TYPE_POOL but "
                  "poolAllocSize==0\n");
...
customFree(J9PoolUserData* userData, void* address, uint32_t type)
{
  ...
  if (type == POOL_ALLOC_TYPE_PUDDLE &&
      userData->inputData->poolFlags & POOL_NEVER_FREE_PUDDLES) {
...
static void*
sharedPuddleListAlloc(OMRPortLibrary* portLib, uint32_t size,
                      const char* callSite, uint32_t memoryCategory,
                      uint32_t type, uint32_t* doInit)
{
...
static intptr_t
createNewPools(OMRPortLibrary* portLib, uintptr_t* passCount,
               uintptr_t* failCount)
{
  intptr_t poolCntr;
  OMRPORT_ACCESS_FROM_OMRPORT(portLib);
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    resultData->poolResult = pool_new(
      inputData->structSize, inputData->numberElements,
      inputData->elementAlignment, inputData->poolFlags, OMR_GET_CALLSITE(),
      OMRMEM_CATEGORY_VM, (omrmemAlloc_fptr_t)customAlloc,
      (omrmemFree_fptr_t)customFree, userData);
    ...
    if (resultData->puddleAllocSize <
        ((expectedNumElems *
          ROUND_TO(expectedAlignment, inputData->structSize)) +
      ...
      omrtty_printf("Error: Allocation size did not respect "
                    "POOL_ROUND_TO_PAGE_SIZE flag for pool test %d\n",
                    poolCntr);
      (*failCount)++;
    } else {
      (*passCount)++;
...
static void
testNewElement(OMRPortLibrary* portLib, uintptr_t* passCount,
               uintptr_t* failCount)
{
  uint32_t poolCntr, j;
  OMRPORT_ACCESS_FROM_OMRPORT(portLib);
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    /* Call pool_newElement until a new puddle is allocated... and then allocate
     * a couple of new elements into the new puddle */
    resultData->puddleAllocSize = 0;
    while (prevPuddleSize == 0) {

      /* Get a single element and check that it's the right size and that each
       * byte can be written to */
      prevPuddleSize = resultData->puddleAllocSize; /* puddleAllocSize will be
      ...
      if (((uintptr_t)element % expectedAlignment) != 0) {
        omrtty_printf(
          "Error: pool_newElement returned 0x%p, which is not aligned to %d "
          "for pool test %d\n",
      ...
      for (j = 0; j < inputData->structSize; j++) {
        if (walkBytes[j] != 0) {
          omrtty_printf("Error: pool_newElement non-zero byte at offset %d in "
                        "pool test %d\n",
                        j, poolCntr);
          (*failCount)++;
        }
      ...
      if (resultData->puddleAllocSize == 0) {
        elementsInPuddle++; /* Only increment while we're still in the first
                               puddle */
      }
    }
    if (resultData->puddleAllocSize != puddleAllocSize) {
      omrtty_printf("Error: New puddle allocated is not the same size as the "
                    "previous puddle in pool test %d\n",
                    poolCntr);
      (*failCount)++;
    } else if (elementsInPuddle < expectedNumElems) {
      omrtty_printf("Error: Pool initialized with minElements=%d but only %d "
                    "elements were allocated in puddle, in pool test %d\n",
...
static void
testWalkFunctions(OMRPortLibrary* portLib, uintptr_t* passCount,
                  uintptr_t* failCount)
{
  intptr_t poolCntr;
  OMRPORT_ACCESS_FROM_OMRPORT(portLib);
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    do {
      if (element[0] != FIRST_BYTE_MARKER) {
        omrtty_printf(
          "Error: First byte of element is corrupted in pool test %d\n",
          poolCntr);
        numFailed++;
      }
      if ((inputData->structSize > 1) &&
        ...
        omrtty_printf(
          "Error: Last byte of element is corrupted in pool test %d\n",
          poolCntr);
        numFailed++;
      }
      if (!pool_includesElement(currentPool, element)) {
        omrtty_printf("Error: pool_includesElement returned false for walked "
                      "element in pool test %d\n",
                      poolCntr);
        numFailed++;
      }
      walkCount++;
    ...
    if (walkCount != resultData->elementsAdded) {
      omrtty_printf(
        "Error: Walk did not cover all the pool elements in pool test %d\n",
        poolCntr);
      numFailed++;
    }
    if (pool_numElements(currentPool) != resultData->elementsAdded) {
      omrtty_printf(
        "Error: pool_numElements returned the wrong value in pool test %d\n",
        poolCntr);
      numFailed++;
    }
    if (numFailed != 0) {
...
static void
testRemoveElement(OMRPortLibrary* portLib, uintptr_t* passCount,
                  uintptr_t* failCount)
{
  intptr_t poolCntr;
  OMRPORT_ACCESS_FROM_OMRPORT(portLib);
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    do {
      ...
      do {
        expectedNumElements--;
        if (!pool_includesElement(currentPool, element)) {
          omrtty_printf("Error: pool_includesElement returned false before "
                        "removing for walked element in pool test %d\n",
                        poolCntr);
          numFailed++;
        }
        pool_removeElement(currentPool, element);
        if (pool_numElements(currentPool) != expectedNumElements) {
          omrtty_printf("Error: pool_numElements returned the wrong value "
                        "while removing in pool test %d\n",
                        poolCntr);
          numFailed++;
        }
        if (pool_includesElement(currentPool, element)) {
          omrtty_printf("Error: pool_includesElement returned true while "
                        "removing for walked element in pool test %d\n",
                        poolCntr);
          numFailed++;
        }
    ...
    if (expectedNumElements != 0) {
      omrtty_printf("Error: unexpected error - expectedNumElements is not zero "
                    "for pool test %d\n",
                    poolCntr);
      numFailed++;
    }
...
testKill(OMRPortLibrary* portLib, uintptr_t* passCount, uintptr_t* failCount)
{
  ...
  for (poolCntr = 0; poolCntr < NUM_POOLS; poolCntr++) {
    ...
    inputData->poolFlags &=
      ~POOL_NEVER_FREE_PUDDLES; /* Remove this flag so that the custom free
                                   functions don't complain */
...
static void
testPuddleListSharing(OMRPortLibrary* portLib, uintptr_t* passCount,
                      uintptr_t* failCount)
{
  uint32_t index;
  ...
  for (index = 0; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    ...
    if (numElements != (1 + index)) {
      omrtty_printf(
        "Error: pool_numElements (=%d) did not match expected value (=%d)"
  ...
  for (index = 1; index < NUM_POOLS_TO_SHARE_PUDDLE_LIST; index++) {
    if (element != pool_startDo(pool[index], &state[index])) {
      omrtty_printf("Error: Elements returned in different order when "
                    "iterating over pools with shared puddle lists");
      (*failCount)++;
    } else {
      (*passCount)++;
    }
  ...
  if (sharedPuddleList != NULL) {
    omrtty_printf(
      "Error: sharedPuddleList was not NULL after all pools were killed");
    (*failCount)++;
  } else {
    (*passCount)++;
  }
