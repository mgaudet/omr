...
bool FrontEnd::mapsAreIdentical(
    ...
    TR::Compilation* comp)
{
    if (nextMapCursor && nextMapCursor != stackAtlas->getParameterMap() && mapCursor != stackAtlas->getParameterMap() && mapCursor->getMapSizeInBytes() == nextMapCursor->getMapSizeInBytes() && mapCursor->getRegisterMap() == nextMapCursor->getRegisterMap() && !memcmp(mapCursor->getMapBits(), nextMapCursor->getMapBits(), mapCursor->getMapSizeInBytes()) &&
#ifdef TR_HOST_S390
        (mapCursor->getHighWordRegisterMap() == nextMapCursor->getHighWordRegisterMap()) &&
#endif
        (comp->getOption(TR_DisableShrinkWrapping) || (mapCursor->getRegisterSaveDescription() == nextMapCursor->getRegisterSaveDescription()))) {
        return true;
    } else {
        return false;
    }
