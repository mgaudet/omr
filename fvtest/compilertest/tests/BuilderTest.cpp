...
void BuilderTest::invokeControlFlowTests()
{
    ...
    for (uint32_t i = 0; i < sizeof(absDiffParms) / sizeof(absDiffParms[0]); ++i) {
        EXPECT_EQ(absDiff(absDiffParms[i][0], absDiffParms[i][1]),
    ...
    for (uint32_t i = 0; i < sizeof(maxIfThenParms) / sizeof(maxIfThenParms[0]); ++i) {
        EXPECT_EQ(maxIfThen(maxIfThenParms[i][0], maxIfThenParms[i][1]),
    ...
    for (uint32_t i = 0; i < sizeof(subIfFalseThenParms) / sizeof(subIfFalseThenParms[0]); ++i) {
        EXPECT_EQ(subIfFalseThen(subIfFalseThenParms[i][0], subIfFalseThenParms[i][1]),
...
void BuilderTest::invokeNestedControlFlowLoopTests()
{
    ...
    for (uint32_t i = 0; i < arrSize; ++i) {
        EXPECT_EQ(forLoopIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]),
            _forLoopUpIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]));
    }

    arrSize = sizeof(loopIfThenElseArr) / sizeof(loopIfThenElseArr[0]);
    for (uint32_t i = 0; i < arrSize; ++i) {
        EXPECT_EQ(whileDoIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]),
            _whileDoIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]));
    }

    arrSize = sizeof(loopIfThenElseArr) / sizeof(loopIfThenElseArr[0]);
    for (uint32_t i = 0; i < arrSize; ++i) {
        EXPECT_EQ(doWhileIfThenElse(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]),
            _doWhileIfThenElseMethod(loopIfThenElseArr[i][0], loopIfThenElseArr[i][1], loopIfThenElseArr[i][2],
                      loopIfThenElseArr[i][3]));
    }

    arrSize = sizeof(ifThenElseLoopArr) / sizeof(ifThenElseLoopArr[0]);
    for (int32_t i = 0; i < arrSize; i++) {
        EXPECT_EQ(ifThenElseLoop(ifThenElseLoopArr[i][0], ifThenElseLoopArr[i][1]),
...
bool RecursiveFibonnaciMethod::buildIL()
{
    ...
    recursiveCase->Store(
        "result", recursiveCase->Add(recursiveCase->Call("fib_recur", 1,
...
bool SubIfFalseThenMethod::buildIL()
{
    ...
    elsePath->Return(elsePath->Sub(elsePath->Load("valueA"), elsePath->Load("valueB")));
