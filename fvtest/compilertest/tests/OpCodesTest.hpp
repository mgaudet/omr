...
protected:
    ...
    template <typename T>
    static int32_t comparel(T a, T b) { return std::isnan(a) ? -1 : std::isnan(b) ? -1 : a > b ? 1 : a == b ? 0 : -1; }
    template <typename T>
    static int32_t compareg(T a, T b) { return std::isnan(a) ? 1 : std::isnan(b) ? 1 : a > b ? 1 : a == b ? 0 : -1; }
