...
int32_t GCConfigTest::attachChildEntry(ObjectEntry* parentEntry, ObjectEntry* childEntry)
{
    ...
    if ((uint32_t)parentEntry->numOfRef < slotCount) {
        ...
        gcTestEnv->log(LEVEL_VERBOSE, "\tadd child %s(%p[0x%llx]) to parent %s(%p[0x%llx]) slot %p[%llx].\n",
            childEntry->name, childEntry->objPtr, *(childEntry->objPtr), parentEntry->name, parentEntry->objPtr,
            *(parentEntry->objPtr), childSlot, (uintptr_t)*childSlot);
        parentEntry->numOfRef += 1;
    } else {
        gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: numOfFields %d defined for %s(%p[0x%llx]) is not enough "
                                    "to hold child reference for %s(%p[0x%llx]).\n",
            __FILE__, __LINE__, parentEntry->numOfRef, parentEntry->name, parentEntry->objPtr, *(parentEntry->objPtr),
    ...
    while (currentSlot < endSlot) {
        ...
        if (objEntry->objPtr == slotObject.readReferenceFromSlot()) {
            gcTestEnv->log(LEVEL_VERBOSE, "Remove object %s(%p[0x%llx]) from parent %s(%p[0x%llx]) slot %p.\n", name,
                objEntry->objPtr, *(objEntry->objPtr), parentEntry->name, parentEntry->objPtr, *(parentEntry->objPtr),
...
int32_t GCConfigTest::parseGarbagePolicy(pugi::xml_node node)
{
    ...
    if (!node.empty()) {
        ...
        if ((0 != strcmp(gp.frequency, "perObject")) && (0 != strcmp(gp.frequency, "perRootStruct"))) {
            rt = 1;
            gcTestEnv->log(LEVEL_ERROR, "%s:%d Invalid XML input: the valid value of attribute \"frequency\" is "
                                        "\"perObject\" or \"perRootStruct\".\n",
                __FILE__, __LINE__);
            goto done;
        }
