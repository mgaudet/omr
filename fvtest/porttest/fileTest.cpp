...
TEST_F(PortFileTest, WriteRead)
{
    ...
    OMRTEST_ASSERT_TRUE((strlen(readBufPtr) == strlen(expectedResult)), "omrfile_read_text() returned "
            << strlen(readBufPtr) << " expected " << strlen(expectedResult) << "\n");
...
TEST_F(PortFileTest, WriteAppendRead)
{
    ...
    OMRTEST_ASSERT_TRUE((!strcmp(expectedOutput, outputLine)), "Read back data \""
            << outputLine << "\" not matching expected data \"" << expectedOutput << "\"\n");
