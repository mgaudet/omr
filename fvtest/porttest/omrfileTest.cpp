...
TEST_F(PortFileTest2, file_test30)
{
    ...
    if (-1 == invalidFileFd) {
        ...
        if (lastErrorNumber == OMRPORT_ERROR_FILE_EXIST) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrerror_last_error_number(\"%s\") returned %i "
...
TEST_F(PortFileTest2, file_test31)
{
    ...
    if (myGid != fileGid) {
        outputComment(
            OMRPORTLIB, "unexpected group ID.  Possibly setgid bit set. Comparing to group of the current directory\n");
...
#define J9FILETEST_FILESIZE 1000 * 512
    ...
    if (buf.isRemote) {
        outputComment(
            OMRPORTLIB, "WARNING: Test omrfile_test34 cannot be run on a remote file system.\nSkipping test.\n");
...
TEST_F(PortFileTest2, file_test37)
{
    ...
    if (rc >= 0) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "Negative test failure: omrfile_fstat(-1, ..) on invalid file "
