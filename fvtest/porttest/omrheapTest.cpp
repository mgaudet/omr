    ...
    for (j = 0; j < (sizeof(heapSize) / sizeof(heapSize[0])); j++) {
        ...
        for (i = 0; i < 16; i++) {
            ...
            verifyHeapOutofRegionWrite(
                OMRPORTLIB, allocPtr, allocPtrAdjusted + heapSize[j], heapStartOffset + i, testName);
        }
    }
 ...
 * When we do a heap_free, we randomly pick an element from the pool and free the memory based on the values stored in
 * the identifier.
 * We keep a separate counter for number of heap_allocate and heap_free calls, and we terminate the loop when heap_free
 * count reaches a certain value e.g. 500,000.
 *
 * As an additional test for memory leak. We determine the largest amount of memory we can allocate before we enter the
 * loop of heap_alloc/heap_free.
 * After the loop is terminated, we free any remaining memory chunks in the pool and again allocate the largest possible
 * size.
...
TEST(PortHeapTest, heap_grow_test)
{
     ...
     *should be 128 slots in this test heap.
     * And firstFreeBlock should point to the first slot after header
     * which should have value 128 - sizeof(omrheap)/sizeof(uint64_t) - 2(head and tail of the slot) = 124 (on 64bit) or
     *125 (on 32bit)
     *
     *
     * |--------|-----|--------------------------------------------------------------------------------------------------------
...
TEST(PortHeapTest, heap_test_pmr_28277_999_760)
{
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |                  	|  |  |      |  |  |
     * |  |  |			     	|   |
     * |		|Block  | Slot	|  Size   |-2|Alloc0|-2| 2|FREE  | 2|-2|Alloc2|-2|-2|Alloc3|-2|8 |FREE              	|8
     * |-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	|
     * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  |8slots            	|  |
     * |2Slots|  |  |2Slots|  |  |		     		|   |
     * |	100	|   8   |   8   |   0     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |                  	|  |  |      |
     * |  |      |  |  |					|   |
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |      |  |
     * |      |  |  |			     	|   |
     * |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc |-2|-2|Alloc |-2|-3|Alloc4_1|-3|3 |FREE    |3
     * |-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	|
     ...
     * |--------|-------|-------|---------|--|------|--|--|------|--|--|------|--|--|------|--|--|--------|--|--|--------|--|--|------|--|--|------|--|--|--
     * ---------------|---|
     * 0        1       2       3         4  5  	7  8  9	 	11 12 13     15 16 17	  19 20 21	     24 25 26        29 30
     * 31     33 34 35   	37 38 39 				99	100
     *
     */
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |      |  |
     * |      |  |  |			     	|   |
     * |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc |-2|-2|Alloc
     * |-2|-3|Alloc4_1|-3|-3|Alloc4_2|-3|-2|Alloc |-2|-2|Alloc |-2|60|	FREE			|60	|
     * |		|       |	   	|  Visited|	 |2Slots|  |  |2Slots|  |  |2Slots|  |  |2Slots|  |  | 3Slots |  |  |3Slots  |  |
     * |2Slots|  |  |2Slots|  |  |		     		|   |
     * |	100	|   8   |   25  |   2     |	 |		|  |  |		 |  |  |	  |  |  |      |  |  |        |  |  |        |  |  |
     * |  |  |      |  |  |					|   |
     ...
     * |		|Free   | Alloc |  Alloc  |	 |		|  |  |		 |  |  |	  |  |  |	   |  |  |        |  |  |        |  |  |      |  |
     * |      |  |  |			     	|   |
     * |		|Block  | Slot	|  Size   |-2|Alloc	|-2| 2|FREE  | 2|-2|Alloc |-2|-2|Alloc |-2| 3|FREE	  |
    ...
    struct OMRPortLibrary* portLibrary, void* subAllocMem, uintptr_t allocSize, J9Heap* heapBase, const char* testName)
{
    ...
    if (0 == allocSize) {
        if (!(1 <= -blockSizeStart && -blockSizeStart <= 3)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "The allocated block seems to be overly large (3 or more free "
