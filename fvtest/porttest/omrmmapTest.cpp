...
#define J9MMAP_TEST3_BUFFER_SIZE 100
    ...
    if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_COPYONWRITE)) {
        outputComment(
            OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_COPYONWRITE not supported on this platform, bypassing test\n");
        goto exit;
    }
...
#define J9MMAP_TEST4_BUFFER_SIZE 100
    ...
    if (!(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_WRITE) || !(mmapCapabilities & OMRPORT_MMAP_CAPABILITY_MSYNC)) {
        outputComment(
            OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_WRITE or _MSYNC not supported on this platform, bypassing test\n");
        goto exit;
    }
...
TEST_F(PortMmapTest, mmap_test6)
{
    ...
    if (mmapCapabilities & OMRPORT_MMAP_CAPABILITY_PROTECT) {
        ...
        if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "Mmap is claims to be capable of PROTECT, however, "
                                                    "omrmmap_protect() returns OMRPORT_PAGE_PROTECT_NOT_SUPPORTED\n");
            goto exit;
        }
        ...
        rc = omrmmap_protect(mapAddr, (uintptr_t)fileLength, OMRPORT_PAGE_PROTECT_READ | OMRPORT_PAGE_PROTECT_WRITE);
        if ((rc & OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) == 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "Mmap is not capable of PROTECT, however, omrmmap_protect() does "
                                                    "not return OMRPORT_PAGE_PROTECT_NOT_SUPPORTED\n");
            goto exit;
        }
    }
...
TEST_F(PortMmapTest, mmap_test9)
{
    ...
    if (rc != 0) {
        if (rc == OMRPORT_PAGE_PROTECT_NOT_SUPPORTED) {
            outputComment(
                OMRPORTLIB, "OMRPORT_MMAP_CAPABILITY_PROTECT not supported on this platform, bypassing test\n");
        } else {
            lastErrorMessage = (char*)omrerror_last_error_message();
            lastErrorNumber = omrerror_last_error_number();
            outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (!omrsig_can_protect(signalHandlerFlags)) {
        outputComment(OMRPORTLIB, "Signal handling framework not available, can't test readonly functionality without "
                                  "crashing, bypassing test");
        goto exit;
    } else {
