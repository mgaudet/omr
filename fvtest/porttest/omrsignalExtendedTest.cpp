...
static int sigMaskThread(void* arg)
{
    ...
    outputComment(
        OMRPORTLIB, "\t%s\t:pthread_sigmask test has been done. send result back to main thread.\n", testName);
...
TEST(PortSignalExtendedTests, sig_ext_test1)
{
    ...
    if ((NULL != mainThread) && (NULL != maskThread) && (NULL != unmaskThread)) {
        ...
        outputComment(
            OMRPORTLIB, "\t%s\t:configure unmask thread to prepare for unmasked signal SIGSEGV test.\n", testName);
        ...
        if (!waitForEvent(testName, &unmaskThreadInfo, FINISHED, NULL, 0)) {
            ...
            outputComment(
                OMRPORTLIB, "\t%s\t:unmasked thread finished. checking unmask thread's signal status...\n", testName);
        ...
        if (!waitForEvent(testName, &maskThreadInfo, FINISHED, NULL, 0)) {
            ...
            outputComment(
                OMRPORTLIB, "\t%s\t:masked thread finished. checking mask thread's signal status...\n", testName);
