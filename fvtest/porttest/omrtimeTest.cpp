...
TEST(PortTimeTest, time_test3)
{
    ...
    if (1 == omrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE)) {
         ...
         * different one where we ask for the time,
         * there is no reason why the time values need to be monotonically increasing */
        outputComment(OMRPORTLIB, "Test is invalid since the host machine reports more than one CPU (time may differ "
                                  "across CPUs - makes test results useless - re-enable if we develop thread affinity "
                                  "support)\n");
    }
    outputComment(OMRPORTLIB, "\n");
...
TEST(PortTimeTest, time_nano_time_direction)
{
     ...
     */
    {
        ...
        if (QueryPerformanceCounter(&i)) {
            outputComment(OMRPORTLIB, "WARNING: Test is invalid since the host machine uses QueryPerformanceCounter() "
                                      "(time may differ across CPUs - makes test results useless - re-enable if we "
