...
/* reserve and commit */
        ...
        if (memPtr == NULL) {
            ...
            if (finalBlocks <= initialBlocks) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem reserve didn't increment category block as expected. "
                                                        "Final blocks=%zu, initial blocks=%zu, page size=%zu.\n",
                    finalBlocks, initialBlocks, pageSizes[i]);
            }

            if (finalBytes < (initialBytes + pageSizes[i])) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem reserve didn't increment category bytes as expected. "
                                                        "Initial bytes=%zu, final bytes=%zu, page size=%zu.\n",
...

        {
            ...
            if (finalBlocks != initialBlocks) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem free didn't decrement category block as expected. Final "
                                                        "blocks=%zu, initial blocks=%zu, page size=%zu.\n",
                    finalBlocks, initialBlocks, pageSizes[i]);
            }

            if (finalBytes != initialBytes) {
                outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem free didn't decrement category bytes as expected. "
                                                        "Initial bytes=%zu, final bytes=%zu, page size=%zu.\n",
    ...
    uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
    ...
    for (i = 0; numIterations != i; i++) {
        ...
        if (rc != 0) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_decommit_memory returned 0x%zx when trying to decommit "
                                                    "0x%zx bytes backed by 0x%zx-byte pages\n",
    ...
    struct OMRPortLibrary* portLibrary, uintptr_t physicalMemorySize, uintptr_t pageSize, BOOLEAN disclaim)
{
    ...
    deltaMillis = (omrtime_nano_time() - startTimeNanos) / (1000 * 1000);
    omrtty_printf("%s pageSize 0x%zx, byteAmount 0x%zx, millis: [force page, decommit pagedout region, write "
    ...
    uintptr_t byteAmount, BOOLEAN disclaim, uintptr_t numIterations)
{
    ...
    omrtty_printf("%s pageSize 0x%zx, byteAmount 0x%zx, millis for %u iterations: [reserve/write_to_all/decommit/free] "
                  "test: %lli\n",
        disclaim ? "disclaim" : "nodisclaim", pageSize, byteAmount, numIterations, deltaMillis);
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAdress)
{
    ...
    if (memoryIsAvailable(OMRPORTLIB, TRUE)) {
        ...
        outputComment(OMRPORTLIB, "***Did not find enough memory available on system, not running test "
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        for (j = 0; j < NUM_SEGMENTS; j++) {
            ...
            if (memPtr[j] == NULL) {
                if (strictPageSize) {
                    ...
                    if (OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES == lastErrorNumber) {
                        outputComment(OMRPORTLIB, "\tPortable error OMRPORT_ERROR_VMEM_INSUFFICENT_RESOURCES...\n");
                        outputComment(
                            OMRPORTLIB, "\t\tREBOOT THE MACHINE to free up resources AND TRY THE TEST AGAIN\n");
                    }
                }
            ...
            if (NULL != memPtr[j]) {
                ...
                if (finalBytes < (initialBytes + pageSizes[i])) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem reserve didn't increment category bytes as expected. "
                                                            "Final bytes=%zu, initial bytes=%zu, page size=%zu.\n",
                        finalBytes, initialBytes, pageSizes[i]);
                }
            }
        }
        ...
        for (j = 0; j < NUM_SEGMENTS; j++) {
            if (NULL != memPtr[j]) {
                ...
                if (finalBlocks >= initialBlocks) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS, "vmem free didn't decrement category blocks as expected. "
                                                            "Final blocks=%zu, initial blocks=%zu, page size=%zu.\n",
                        finalBlocks, initialBlocks, pageSizes[i]);
                }
        ...
        if (NULL == memPtr) {
            ...
            if (FALSE == isPageSizeSupportedBelowBar(pageSizes[i], pageFlags[i])) {
                if (FOUR_KB != vmemID.pageSize) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS, "\t didn't expect to allocate memory below 2GB using page "
                                                            "size 0x%zx and page flags 0x%x with strict address\n",
...
TEST(PortVmemTest, vmem_testReserveMemoryExStrictAddress_use2To32_zOSLargePageBelowBar)
{
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
         ...
         */
        if (memPtr != NULL) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "Did not expect to allocate memory using page size 0x%zx, page "
                                                    "type 0x%zx in < 2GB range with OMRPORT_VMEM_ZOS_USE2TO32G_AREA "
                                                    "and OMRPORT_VMEM_STRICT_ADDRESS .\n",
                pageSizes[i], pageFlags[i]);

            omrvmem_free_memory(memPtr, params.byteAmount, &vmemID);

        } else {
            outputComment(OMRPORTLIB, "\t Expected to fail to allocate large pages in < 2GB range with "
    ...
    for (i = 0; pageSizes[i] != 0; i++) {
        ...
        if (NULL == memPtr) {
            ...
            *((unsigned int*)memPtr) = (unsigned int)0x4e800020; /* blr instruction (equivalent to RET on x86)*/
            ...
            outputComment(
                OMRPORTLIB, "About to call dynamically created function, pageSize=0x%zx...\n", params.pageSize);
            function();
...
TEST(PortVmemTest, vmem_test_numa)
{
    ...
    if ((detailReturnCode != 0) || (0 == totalNumaNodeCount)) {
        ...
        if (nodeArray == NULL) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory returned NULL when trying to allocate "
        ...
        if ((detailReturnCode != rc) || (totalNumaNodeCount != originalNodeCount)) {
            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_numa_get_node_details returned different results on "
                                                    "second call (return %zd versus %zd, count %zu versus %zu)\n",
            ...
            if (memPtr != NULL) {
                ...
                for (nodeIndex = 0; nodeIndex < totalNumaNodeCount; nodeIndex++) {
                    if (policyForMemoryNode == nodeArray[nodeIndex].memoryPolicy) {
                        ...
                        if (rc != 0) {
                            outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_numa_set_affinity returned 0x%zx when "
                                                                    "trying to set affinity for 0x%zx bytes at 0x%p to "
                                                                    "node %zu\n",
                ...
                if (rc != 0) {
                    outputErrorMessage(PORTTEST_ERROR_ARGS, "omrvmem_free_memory returned 0x%zx when trying to free "
                                                            "0x%zx bytes backed by 0x%zx-byte pages\n",
    ...
    uintptr_t pageFlagsReturned, uintptr_t isSizeSupportedReturned)
{
    ...
    outputComment(OMRPORTLIB,
        ...
        pageFlagsExpected, isSizeSupportedExpected);
