...
#undef SI_DEBUG
    ...
    if (rc < 0) {
        if (OMRPORT_ERROR_NOT_SUPPORTED_ON_THIS_PLATFORM == rc) {
            ...
            outputErrorMessage(PORTTEST_ERROR_ARGS, "\tCrash passing in NULL buffer while running single-threaded. "
                                                    "This is a failure because no-one else should have been able to "
                                                    "modify it\n");
        } else {
    ...
    if (rc < 0) {
        ...
        envSize = rc;
#if defined(SI_DEBUG)
        omrtty_printf("Should have a buffer of size %x bytes, using one of size %x instead, which will result in a "
...
TEST(PortSysinfoTest, sysinfo_test_sysinfo_get_limit_FILE_DESCRIPTORS)
{
    ...
    if (OMRPORT_LIMIT_UNLIMITED == rc) { /* Not an error, just that it is not configured. */
        ...
        if (RLIM_INFINITY == curLimit) {
            outputComment(OMRPORTLIB, "omrsysinfo_get_limit(nofiles): soft limit=RLIM_INFINITY (unlimited).\n");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
    ...
    if (OMRPORT_LIMIT_UNLIMITED == rc) {
        ...
        if (RLIM_INFINITY == maxLimit) {
            outputComment(OMRPORTLIB, "omrsysinfo_get_limit(nofiles): hard limit = RLIM_INFINITY (unlimited).\n");
        } else {
            outputErrorMessage(PORTTEST_ERROR_ARGS,
...
TEST(PortSysinfoTest, sysinfo_test_get_groups)
{
    ...
    if (-1 != rc) {
        ...
        for (i = 0; i < rc; i++) {
            ...
            if (NULL == grent) {
                if (0 == error) {
                    outputComment(OMRPORTLIB, "\tthis group id is not found in group database (not an error as per "
                                              "getgrgid documentation)\n");
                } else {
                    outputErrorMessage(PORTTEST_ERROR_ARGS,
