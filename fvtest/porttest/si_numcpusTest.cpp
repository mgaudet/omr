...
TEST(PortSysinfoTest, sysinfo_numcpus_test1)
{
    ...
    if (0 == boundCPUs) {
        ...
        if (numberBoundCPUs < numberPhysicalCPUs) {
            outputComment(OMRPORTLIB, "The test was configured to run without CPU restrictions (e.g. taskset), but CPU "
...
TEST(PortSysinfoTest, sysinfo_numcpus_test3)
{
    ...
    if (numberOnlineCPUs > numberPhysicalCPUs) {
        outputErrorMessage(PORTTEST_ERROR_ARGS, "\tomrsysinfo_get_number_CPUs_by_type(OMRPORT_CPU_ONLINE) returned "
                                                "value greater than "
