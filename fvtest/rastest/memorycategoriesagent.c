...
omr_error_t
OMRAgent_OnLoad(OMR_TI const* ti, OMR_VM* vm, char const* options,
                                       ...
                                       OMRPortLibrary* portLibrary)
{
  ...
  if (OMR_ERROR_NONE == testRc) {
    omrtty_printf(
      "\nNegative path: get categories with an undersized buffer\n");
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, total_categories - 1, memoryCategories,
  ...
  if (OMR_ERROR_NONE == testRc) {
    rc = OMRTEST_PRINT_UNEXPECTED_RC(
      ti->GetMemoryCategories(vmThread, total_categories - 2, memoryCategories,
                              &written_count, total_categories_null),
      OMR_ERROR_OUT_OF_NATIVE_MEMORY);
    if (OMR_ERROR_OUT_OF_NATIVE_MEMORY != rc) {
      testRc = OMR_ERROR_INTERNAL;
...
static void
printMemoryCategory(OMRPortLibrary* portLib, OMR_TI_MemoryCategory* category,
                    int depth)
{
