...
static omr_error_t waitForTestChildThread(OMR_TI const *ti, OMR_VM *testVM, omrthread_t childThread,
					 ...
					 TestChildThreadData **childData)
{
	...
	if (OMR_ERROR_NONE == rc) {
		rc = OMRTEST_PRINT_UNEXPECTED_INT_RC(threadAPI->omrthread_attr_set_detachstate(&attr, J9THREAD_CREATE_JOINABLE),
											 J9THREAD_SUCCESS);
...
testTraceAPIs(OMR_TI const *ti, OMR_VM *vm, const char *threadName)
{
	...
	if (OMR_ERROR_NONE == rc) {
		rc = OMRTEST_PRINT_ERROR(ti->RegisterRecordSubscriber(testData1.vmThread, "sample1", subscribeFunc1, alarmFunc1,
															  (void *)&testData1, &testData1.subscriptionID));
	}

	if (OMR_ERROR_NONE == rc) {
		rc = OMRTEST_PRINT_ERROR(ti->RegisterRecordSubscriber(testData2.vmThread, "sample2", subscribeFunc2, alarmFunc2,
