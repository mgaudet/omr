...
static void
preforkSetup(OMR_VMThread** vmthread, struct OMR_Agent** agent,
             OMRTestVM* testVM, int pipedata[2])
{
  OMRPORT_ACCESS_FROM_OMRPORT(rasTestEnv->getPortLibrary());
  char* datDir = NULL;
 ...
 */
void
postForkChild(OMR_VMThread* vmthread, struct OMR_Agent* agent,
              OMRTestVM* testVM, int pipedata[2])
{
...
void
postForkParent(OMR_VMThread* vmthread, struct OMR_Agent* agent,
               OMRTestVM* testVM, int pipedata[2])
{
  /*  Fire some trace points! */
  Trc_OMR_Test_Init();
 ...
 */
static void
recordChildErrorStatus(OMRTraceSubscriberForkTestResult* result, omr_error_t rc,
