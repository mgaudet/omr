...
TEST(TraceLifecycleTest, rebindAndDeregisterSubscriberAfterShutdown)
{
  ...
  /* Register the subscriber */
  OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(
    vmthread, "rebindAndDeregisterSubscriberAfterShutdown", subscribeFunc, NULL,
    NULL, &subscriptionID));
...
static void
startChildThread(OMRTestVM* testVM, omrthread_t* childThread,
                 omrthread_entrypoint_t entryProc, ChildThreadData** childData)
{
...
static omr_error_t
waitForChildThread(OMRTestVM* testVM, omrthread_t childThread,
                   ChildThreadData* childData)
{
  OMRPORT_ACCESS_FROM_OMRPORT(testVM->portLibrary);
