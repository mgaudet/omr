...
TEST(TraceLogTest, stressTraceBufferManagement)
{
	...
	OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(vmthread, "fail", failOnSecondCall, failOnSecondCallAlarm,
	...
	for (size_t i = 0; i < NUM_CHILD_THREADS; i += 1) {
		...
		OMRTEST_ASSERT_ERROR_NONE(ti->RegisterRecordSubscriber(vmthread, "child", countTracepoints, NULL,
