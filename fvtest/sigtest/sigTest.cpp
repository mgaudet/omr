...
static omr_error_t test(TestAction testFunc)
{
    ...
    for (int i = 0; (i < NUM_TEST_CONDITIONS) && (OMR_ERROR_NONE == rc); i += 1) {
        ...
        if (OMR_ERROR_NONE != runTest(existingPrimary, existingSecondary,
            ...
            printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, %sexisting secondary, flags = %ld, "
                   "signum is %d, handler is 0x%p, %salt signal stack.\n",
#else /* (defined(S390) && defined(OMR_ENV_DATA64)) */
            printf("Test %d:%d failed at %s:%d. Conditions: %sexisting primary, %sexisting secondary, flags = %d, "
                   "signum is %d, handler is 0x%p, %salt signal stack.\n",
