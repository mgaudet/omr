...
/**
 * This function records process times after each call to the 'user time' CPU
 * burner.
 ...
 */
void runCPUBurnToTestUserTimes(omrthread_process_time_t* processTimeArray,
  ...
  for (i = 0; i < arrayLength; i++) {
    matrixSquare();
    retValOfGetProcessTimes =
        (omrthread_get_process_times(processTimeArray++) ||
         retValOfGetProcessTimes);
    EXPECT_TRUE(0 == retValOfGetProcessTimes);
  }
}
/**
 * This function records process times after each call to the 'system time' CPU
 * burner.
 * It also checks for valid return codes after each call.
 *
 * @param[in]  arrayLength		Length of the processTimeArray
 * @param[out] processTimeArray	Pointer to the array of process times
 */
void runCPUBurnToTestSystemTimes(omrthread_process_time_t* processTimeArray,
                                 uintptr_t arrayLength) {
  uintptr_t i = 0;
  intptr_t retValOfGetProcessTimes = 0;

  /*
   * If we increase the workload done (running the same function multiple
   * times),
