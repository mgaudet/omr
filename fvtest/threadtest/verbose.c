...
void port_printf(const char* str, ...) {
  va_list vargs;
...
#if defined(J9ZOS390)
        PRINTF(
            "%s unsupported: retVal %zd (%zx) : errno %zd (%zx) %s, errno2 %zd "
            "(%zx)\n",
            func, retVal, retVal, os_errno, os_errno, strerror((int)os_errno),
            os_errno2, os_errno2);
#else  /* !J9ZOS390 */
...
#if defined(J9ZOS390)
        PRINTF(
            "%s failed: retVal %zd (%zx) : errno %zd (%zx) %s, errno2 %zd "
            "(%zx)\n",
            func, retVal, retVal, os_errno, os_errno, strerror((int)os_errno),
            os_errno2, os_errno2);
#else  /* !J9ZOS390 */
        PRINTF("%s failed: retVal %zd (%zx) : errno %zd %s\n", func, retVal,
               retVal, os_errno, strerror((int)os_errno));
#endif /* !J9ZOS390 */
      } else {
