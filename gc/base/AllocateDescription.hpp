...
protected:
	...
	bool
		_collectorAllocateExpandOnFailure; /**< if the allocation fails then expand heap if possible to satify the allocation */
...
public:
	...
	getAllocateFlags()
	{
		return _allocateFlags;
	}

	MMINLINE void
	setObjectFlags(uint32_t objectFlags)
	{
		_objectFlags = objectFlags;
	}

	void
	setSpineBytes(uintptr_t sb)
	{
		_spineBytes = sb;
	}
	uintptr_t
	getNumArraylets()
	{
		return _numArraylets;
	}
	void
	setNumArraylets(uintptr_t na)
	{
		_numArraylets = na;
	}
	bool
	isChunkedArray()
	{
		return _chunkedArray;
	}
	void
	setChunkedArray(bool ca)
	{
		_chunkedArray = ca;
	}
	omrarrayptr_t
	...
	MMINLINE MM_MemorySubSpace::AllocationType
	getAllocationType()
	{
	...
	MMINLINE void
	setBytesRequested(uintptr_t bytesRequested)
	{
		_bytesRequested = bytesRequested;
	}
	MMINLINE void
	setMemorySpace(MM_MemorySpace *memorySpace)
	{
	...
	MMINLINE void
	setAllocationTaxSize(uintptr_t size)
	{
		_allocationTaxSize = size;
	}
	MMINLINE uintptr_t
	getAllocationTaxSize()
	{
		return _allocationTaxSize;
	}
	MMINLINE void
	payAllocationTax(MM_EnvironmentBase *env)
	{
		_memorySubSpace->payAllocationTax(env, this);
	}

	MMINLINE void
	setTLHAllocation(bool tlhAlloc)
	{
		_tlhAllocation = tlhAlloc;
	}
	MMINLINE bool
	isTLHAllocation()
	{
		return _tlhAllocation;
	}

	MMINLINE void
	setNurseryAllocation(bool nurseryAlloc)
	{
	...
	isNurseryAllocation()
	{
		return _nurseryAllocation;
	}

	MMINLINE void
	setLOAAllocation(bool loaAlloc)
	{
		_loaAllocation = loaAlloc;
	}
	MMINLINE bool
	isLOAAllocation()
	{
		return _loaAllocation;
	}

	MMINLINE void
	setThreadIsAtSafePoint(bool safe)
	{
		_threadAtSafePoint = safe;
	}
	MMINLINE bool
	isThreadAtSafePoint()
	{
		return _threadAtSafePoint;
	}
	...
	MMINLINE void
	setCollectorAllocateExpandOnFailure(bool expand)
	{
		_collectorAllocateExpandOnFailure = expand;
	}
	MMINLINE bool
	isCollectorAllocateExpandOnFailure()
	{
		return _collectorAllocateExpandOnFailure;
	}

	MMINLINE void
	setCollectorAllocateSatisfyAnywhere(bool anywhere)
	{
		_collectorAllocateSatisfyAnywhere = anywhere;
	}
	MMINLINE bool
	isCollectorAllocateSatisfyAnywhere()
	{
		return _collectorAllocateSatisfyAnywhere;
	}

	MMINLINE bool
	shouldCollectAndClimb()
	{
		return _collectAndClimb;
	}
	...
	isCompletedFromTlh()
	{
		return _completedFromTlh;
	}
	MMINLINE void
	completedFromTlh()
	{
		_completedFromTlh = true;
	}
	 ...
	 */
	MMINLINE void
	setAllocationSucceeded(bool succeeded)
	{
	 ...
	 */
	MMINLINE bool
	getAllocationSucceeded()
	{
		return _allocationSucceeded;
	}
	 ...
	 */
	MM_AllocateDescription(uintptr_t bytesRequested, uintptr_t allocateFlags, bool collectAndClimb,
						   bool threadAtSafePoint)
		: MM_BaseVirtual(), _bytesRequested(bytesRequested), _allocateFlags(allocateFlags), _allocationSucceeded(false),
		  _memorySpace(NULL), _memorySubSpace(NULL), _allocationTaxSize(0), _tlhAllocation(false),
		  _nurseryAllocation(false), _loaAllocation(false), _spineBytes(0), _numArraylets(0), _chunkedArray(false),
		  _spine(NULL), _threadAtSafePoint(threadAtSafePoint), _memoryPool(NULL),
		  _collectorAllocateExpandOnFailure(false), _collectorAllocateSatisfyAnywhere(false),
		  _allocationType(MM_MemorySubSpace::ALLOCATION_TYPE_INVALID), _collectAndClimb(collectAndClimb), _climb(false),
