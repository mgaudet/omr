...
public:
    ...
    MMINLINE omrobjectptr_t allocateAndInitializeObject(OMR_VMThread* omrVMThread)
    {
        ...
        Assert_MM_true(_allocateDescription.getContiguousBytes()
            == env->getExtensions()->objectModel.adjustSizeInBytes(_allocateDescription.getContiguousBytes()));
        ...
        if (NULL != heapBytes) {
            ...
            *headerPtr = (fomrobject_t)(_allocateDescription.getObjectFlags() << OMR_OBJECT_METADATA_FLAGS_SHIFT);
            ...
            if (NULL != objectPtr) {
                ...
                /* if concurrent mark is enabled thread might have to pay tax - must save/restore allocated object in
                 * case of GC */
