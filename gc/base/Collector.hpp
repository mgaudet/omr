...
protected:
	 ...
	 */
	virtual void
	processLargeAllocateStatsBeforeGC(MM_EnvironmentBase *env)
	{
	}
	 ...
	 */
	virtual void
	processLargeAllocateStatsAfterGC(MM_EnvironmentBase *env)
	{
	}
...
public:
	...
	virtual bool heapRemoveRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress,
								 void *highAddress, void *validLowAddress, void *validHighAddress) = 0;
	...
	virtual uint32_t
	getGCTimePercentage(MM_EnvironmentBase *env)
	{
		return 0;
	};
	 ...
	 */
	void
	setGlobalCollector(bool isGlobal)
	{
	...
	virtual void
	setGCThreadPriority(OMR_VMThread *vmThread, uintptr_t priority)
	{
	}
	...
	virtual void
	preMasterGCThreadInitialize(MM_EnvironmentBase *env)
	{
	}
	virtual void
	masterThreadGarbageCollect(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
							   bool initMarkMap = false, bool rebuildMarkBits = false)
	{
	}
	virtual bool
	isConcurrentWorkAvailable(MM_EnvironmentBase *env)
	{
		return false;
	}
	virtual void
	preConcurrentInitializeStatsAndReport(MM_EnvironmentBase *env, MM_ConcurrentGMPStats *stats)
	{
	}
	virtual uintptr_t
	masterThreadConcurrentCollect(MM_EnvironmentBase *env)
	{
		return 0;
	}
	virtual void
	postConcurrentUpdateStatsAndReport(MM_EnvironmentBase *env, MM_ConcurrentGMPStats *stats,
	...
	MM_Collector(MM_CollectorLanguageInterface *cli)
		: MM_BaseVirtual(), _exclusiveAccessCount(0), _cli(cli), _bytesRequested(0), _globalCollector(false),
		  _gcCompleted(false), _isRecursiveGC(false), _collectorExpandedSize(0), _cycleType(OMR_GC_CYCLE_TYPE_DEFAULT),
