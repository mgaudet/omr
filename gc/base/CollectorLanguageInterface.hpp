...
private:
    ...
    MMINLINE void generationalWriteBarrierStore(
        MM_EnvironmentStandard* env, omrobjectptr_t parentObject, omrobjectptr_t childObject);
...
public:
    ...
    virtual bool parallelGlobalGC_heapAddRange(
    ...
    virtual bool parallelGlobalGC_heapRemoveRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size,
        ...
        = 0;
    virtual bool parallelGlobalGC_createHeapWalker(
        MM_EnvironmentBase* env, MM_GlobalCollector* collector, MM_MarkMap* markMap)
        = 0;
    ...
    virtual uintptr_t markingScheme_scanObject(
     ...
     * hooktool is actually functionality better for this but is probably too heavy-weight for what we want for
     * performant code.
     */
     ...
     * 6- call originalForwardedHeader->restoreDestroyedOverlap(T), where T is the compressed pointer for P (see
     * GC_SlotObject)
     *
     * @param[in] env The environment for the calling thread.
     * @param[in] forwardedObject A forwarded header containing the original and destination addresses of the object.
    ...
    /**
     * This method must return a pointer to a signal handling function that will be used by the concurrent helper
     * thread,
     * along with an optional (NULLable) pointer to an opaque data structure that will be supplied to the signal
     * handling
     * function whenever it is called.
     *
     * @param[out] signalHandlerArg A pointer that will receive a pointer to an opaque data structure that will be
     * supplied
    ...
    /**
     * This method must return an integer-valued code to be reported in the verbose GC log at the end of a concurrent GC
     * cycle.
     *
     * @param[in] env The environment for the calling thread.
    ...
    /**
     * This method is called to scan the stack of a mutator thread. Implementation must call
     * MM_MarkingScheme::markObject()
