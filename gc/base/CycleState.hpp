...
public:
	...
	bool
		_shouldRunCopyForward; /**< True if this cycle is to run a copy-forward-based attempt at reclaiming memory, false if mark-compact is to be used */
...
public:
	MM_CycleState()
		: _gcCode(J9MMCONSTANT_IMPLICIT_GC_DEFAULT), _activeSubSpace(NULL), _collectionStatistics(NULL),
		  _workPackets(NULL), _markMap(NULL), _finalizationRequired(false), _dynamicClassUnloadingEnabled(false),
		  _collectionType(CT_GLOBAL_GARBAGE_COLLECTION), _markDelegateState(state_mark_idle), _currentIncrement(0),
		  _shouldRunCopyForward(false), _reasonForMarkCompactPGC(reason_not_exceptional), _externalCycleState(NULL),
		  _type(OMR_GC_CYCLE_TYPE_DEFAULT), _verboseContextID(0), _referenceObjectOptions(references_default)
	{
	}
};
