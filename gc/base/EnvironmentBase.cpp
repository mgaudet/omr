...
bool MM_EnvironmentBase::initialize(MM_GCExtensionsBase* extensions)
{
    ...
    if (extensions->isStandardGC()) {
         ...
         * footprint),
         * but if the threshold is bigger than maxHeap size, we would pass orignal threshold to indicate no
...
void MM_EnvironmentBase::acquireVMAccess() { _envLanguageInterface->acquireVMAccess(); }

void MM_EnvironmentBase::releaseVMAccess() { _envLanguageInterface->releaseVMAccess(); }
...
bool MM_EnvironmentBase::tryAcquireExclusiveVMAccessForGC(MM_Collector* collector)
{
    MM_GCExtensionsBase* extensions = getExtensions();
    uintptr_t collectorAccessCount = collector->getExclusiveAccessCount();
    ...
    while (_omrVMThread != extensions->gcExclusiveAccessThreadId) {
        ...
        if (_omrVMThread != extensions->gcExclusiveAccessThreadId) {
            ...
            /* there is a chance the GC will already have executed at this point or other threads will re-win and
             * re-execute.  loop until the
            ...
            /* May have been beaten to a GC, but perhaps not the one we wanted.  Check and if in fact the collection we
             * intended has been
    ...
    /* thread is the winner for requesting a GC (possibly through recursive calls).  proceed with acquiring exclusive
     * access. */
    Assert_MM_true(_omrVMThread == extensions->gcExclusiveAccessThreadId);
...
bool MM_EnvironmentBase::tryAcquireExclusiveForConcurrentKickoff(MM_ConcurrentGCStats* stats)
{
    MM_GCExtensionsBase* extensions = MM_GCExtensionsBase::getExtensions(_omrVM);
