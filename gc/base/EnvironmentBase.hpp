...
private:
	...
	bool
		_exclusiveAccessBeatenByOtherThread; /**< true if last exclusive access request had to wait for another GC thread */

protected:
	bool
		_allocationFailureReported; /**< verbose: used to report af-start/af-end once per allocation failure even more then one GC cycle need to resolve AF */
...
public:
	...
	bool
		_isInNoGCAllocationCall; /**< NOTE:  this is a "best-efforts" flag, only, for use by an assertion in the collector.  If this is true, the owning thread is attempting to perform a NoGC allocation.  If it is false, nothing can be implied about the owning thread's state. */
...
protected:
	...
	MMINLINE void
	setEnvironmentId(uintptr_t environmentId)
	{
...
public:
	 ...
	 */
	MMINLINE intptr_t
	setPriority(uintptr_t priority)
	{
	 ...
	 */
	MMINLINE bool
	setNumaAffinity(uintptr_t *numaNodes, uintptr_t arrayLength)
	{
	 ...
	 */
	uint64_t
	getMeanExclusiveAccessIdleTime()
	{
	 ...
	 */
	OMR_VMThread *
	getLastExclusiveAccessResponder()
	{
	 ...
	 */
	uintptr_t
	getExclusiveAccessHaltedThreads()
	{
	 ...
	 */
	bool
	exclusiveAccessBeatenByOtherThread()
	{
	...
	getWorkUnitToHandle()
	{
		return _workUnitToHandle;
	}
	MMINLINE void
	setWorkUnitToHandle(uintptr_t workUnitToHandle)
	{
	...
	MMINLINE void
	setThreadScanned(bool threadScanned)
	{
	...
	getCycleStateGCCode()
	{
		return _cycleState->_gcCode;
	}

	/**
	 * Return back pointer to thread's Allocation Context
	 */
	MMINLINE MM_AllocationContext *
	getAllocationContext() const
	{
	 ...
	 */
	MMINLINE void
	setAllocationContext(MM_AllocationContext *allocationContext)
	{
	 ...
	 */
	MMINLINE MM_AllocationContext *
	getCommonAllocationContext() const
	{
	...
	MMINLINE uint32_t
	getAllocationColor() const
	{
		return _allocationColor;
	}
	MMINLINE void
	setAllocationColor(uint32_t allocationColor)
	{
		_allocationColor = allocationColor;
	}

	MMINLINE MM_WorkStack *
	getWorkStack()
	{
		return &_workStack;
	}
#if defined(OMR_GC_SEGREGATED_HEAP)
	MMINLINE MM_HeapRegionQueue *
	getRegionWorkList() const
	{
		return _regionWorkList;
	}
	MMINLINE MM_HeapRegionQueue *
	getRegionLocalFree() const
	{
		return _regionLocalFree;
	}
	MMINLINE MM_HeapRegionQueue *
	getRegionLocalFull() const
	{
	...
	MM_EnvironmentBase(OMR_VMThread *omrVMThread)
		: MM_BaseVirtual(), _slaveID(0), _environmentId(0), _omrVM(omrVMThread->_vm), _omrVMThread(omrVMThread),
		  _portLibrary(omrVMThread->_vm->_runtime->_portLibrary), _workUnitIndex(0), _workUnitToHandle(0),
		  _threadScanned(false), _allocationContext(NULL), _commonAllocationContext(NULL), _exclusiveCount(0),
		  _exclusiveAccessTime(0), _meanExclusiveAccessIdleTime(0), _lastExclusiveAccessResponder(NULL),
		  _exclusiveAccessHaltedThreads(0), _exclusiveAccessBeatenByOtherThread(false),
...
#endif /* OMR_GC_SEGREGATED_HEAP */
		  ,
		  _envLanguageInterface(NULL), _objectAllocationInterface(NULL), _workStack(), _threadType(MUTATOR_THREAD),
		  _cycleState(NULL), _isInNoGCAllocationCall(false), _failAllocOnExcessiveGC(false), _currentTask(NULL),
		  _slaveThreadCpuTimeNanos(0), _freeEntrySizeClassStats(), _oolTraceAllocationBytes(0), _activeValidator(NULL),
	...
	MM_EnvironmentBase(OMR_VM *omrVM)
		: MM_BaseVirtual(), _slaveID(0), _environmentId(0), _omrVM(omrVM), _omrVMThread(NULL),
		  _portLibrary(omrVM->_runtime->_portLibrary), _workUnitIndex(0), _workUnitToHandle(0), _threadScanned(false),
		  _allocationContext(NULL), _commonAllocationContext(NULL), _allocationFailureReported(false)
#if defined(OMR_GC_SEGREGATED_HEAP)
		  ,
		  _regionWorkList(NULL), _regionLocalFree(NULL), _regionLocalFull(NULL)
#endif /* OMR_GC_SEGREGATED_HEAP */
