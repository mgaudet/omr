...
protected:
    bool
        _allocationFailureReported; /**< verbose: used to report af-start/af-end once per allocation failure even more
...
public:
    ...
    MM_SegregatedAllocationTracker*
        _allocationTracker; /**< tracks bytes allocated per thread and periodically flushes allocation data to
...
public:
     ...
     */
    bool inquireExclusiveVMAccessForGC() { return (_omrVMThread->exclusiveCount > 0); }
     ...
     * If the thread is beaten to acquiring exclusive access by another thread then the caller will not acquire
     * exclusive access.
     ...
     *
     * @note this call should be considered a safe-point as the thread may release VM access to allow the winning thread
