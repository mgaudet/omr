...
void* MM_Forge::allocate(uintptr_t bytesRequested, MM_AllocationCategory::Enum category, const char* callsite)
{
    ...
    memoryPointer = (MM_AlignedMemoryHeader*)_portLibrary->mem_allocate_memory(
        _portLibrary, bytesRequested + sizeof(MM_AlignedMemoryHeader), callsite, OMRMEM_CATEGORY_MM);
 ...
 * memory usage category type.
 * To locate memory usage statistics for a particular category, use the enumeration value as the array index (e.g.
 * stats[REFERENCES]).
 *
