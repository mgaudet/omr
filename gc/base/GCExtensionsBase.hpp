...
public:
    ...
    uintptr_t
        largeObjectAllocationProfilingThreshold; /**< object size threshold above which the object is large enough for
    ...
    MM_FreeEntrySizeClassStats
        freeEntrySizeClassStatsSimulated; /**< snapshot of free memory status used for simulated allocator for
                                             fragmentation estimation */
    uintptr_t
        freeMemoryProfileMaxSizeClasses; /**< maximum number of sizeClass maintained for heap free memory profile
    ...
    uintptr_t
        fvtest_forceSweepChunkArrayCommitFailureCounter; /**< Force failure at Sweep Chunk Array commit operation
                                                            counter */
    ...
    uintptr_t
        fvtest_forceReferenceChainWalkerMarkMapCommitFailureCounter; /**< Force failure at Reference Chain Walker Mark
                                                                        Map commit operation counter */
    ...
    double
        scvTenureStrategySurvivalThreshold; /**< The survival threshold (from 0.0 to 1.0) used for deciding to tenure
                                               particular ages. */
    ...
    double
        scavengerCollectorExpandRatio; /**< the ratio of _avgTenureBytes we use to expand when a collectorAllocate()
                                          fails */
...
#endif /* OMR_GC_MODRON_SCAVENGER || OMR_GC_VLHGC */
    double
        globalMaximumContraction; /**< maximum percentage of committed global heap which can contract in one GC cycle
                                     (set through -Xgc:globalMaximumContraction=) */
    double
        globalMinimumContraction; /**< minimum percentage of committed global heap which can contract in one GC cycle
    ...
    UDATA fvtest_forceConcurrentTLHMarkMapDecommitFailure; /**< Force failure at Concurrent TLH Mark Map decommit
                                                              operation */
    UDATA
        fvtest_forceConcurrentTLHMarkMapDecommitFailureCounter; /**< Force failure at Concurrent TLH Mark Map decommit
                                                                   operation  counter */
    ...
    uintptr_t managedAllocationContextCount; /**< The number of allocation contexts which will be instantiated and
                                                managed by the GlobalAllocationManagerRealtime (currently 2*cpu_count)
                                                */
    ...
    uintptr_t debug;
    uintptr_t
        allocationTrackerMaxTotalError; /**< The total maximum desired error for the free bytes approximation, the
    ...
    uintptr_t
        heapCeiling; /**< the highest point in memory where objects can be addressed (used for the -Xgc:lowMemHeap
                        option) */
    ...
    MM_NUMAManager _numaManager; /**< The object which abstracts the details of our NUMA support so that the
                                    GCExtensions and the callers don't need to duplicate the support to interpret our
                                    intention */
    ...
    uintptr_t
        tarokGlobalMarkIncrementTimeMillis; /**< The time period in millisecond a Global Mark increment is allowed to
                                               run (as set by the user, normally dynamic)*/
    ...
    MM_UserSpecifiedParameterUDATA
        tarokNurseryMaxAge; /**< The maximum age that a region will be before it is excluded from a partial garbage
                               collection */
    ...
    uintptr_t
        tarokKickoffHeadroomRegionCount; /**< Count of extra regions reserved for survivor set, in case of sudden
                                            changes of survivor rate. Used in calculation to predict GMP kickoff */
    ...
    bool
        tarokEnableDynamicCollectionSetSelection; /**< Enable dynamic selection of regions to include in the collection
                                                     set that reside outside of the nursery */
    uintptr_t tarokDynamicCollectionSetSelectionAbsoluteBudget; /**< Number of budgeted regions to dynamically select
                                                                   for PGC collection (outside of the required nursery
                                                                   set) */
    ...
    MM_InterRegionRememberedSet* interRegionRememberedSet; /**< The remembered set abstraction to be used to track
                                                              inter-region references found while processing this cycle
                                                              */
    ...
    bool tarokEnableLeafFirstCopying; /**< Enable copying of leaf children immediately after parent is copied in
                                         CopyForwardScheme */
    bool tarokUseDepthFirstCopyForward; /**< True if we should use CopyForwardSchemeDepthFirst instead of the
    ...
    bool
        tarokEnableExpensiveAssertions; /**< True if the collector should perform extra consistency verifications which
    ...
    uintptr_t _holdRandomThreadBeforeHandlingWorkUnitPeriod; /** < How often (in terms of number of times
                                                                ...
                                                                MM_ParallelTask::handleNextWorkUnit() */
    bool
        _forceRandomBackoutsAfterScan; /** < Whether we should force MM_Scavenger::completeScan() to randomly fail due
                                          to backout */
    ...
    uintptr_t
        darkMatterSampleRate; /**< the weight of darkMatterSample for standard gc, default:32, if the weight = 0,
...
private:
     ...
     * OMRPORT_VMEM_PAGE_FLAG_NOT_USED values
...
public:
    ...
    MM_GCExtensionsBase()
        ...
        , heapTailPadding(
              J9_GC_OBJECT_HEAP_TAIL_PADDING) /* Minimum amount of readable space past the end of the heap. See Jazz
                                                 31620. */
