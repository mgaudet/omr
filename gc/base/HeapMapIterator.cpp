...
omrobjectptr_t MM_HeapMapIterator::nextObject()
{
     ...
     * but in
     * fact avoids polluting the memory hierarchy and results in a net performance win.  Be aware when changing this
     * code that
    ...
    while (_heapSlotCurrent < _heapChunkTop) {
        ...
        if (_heapMapSlotValue != J9MODRON_HMI_SLOT_EMPTY) {
            ...
            if (_useLargeObjectOptimization) {
                sizeInHeapMapBits = _extensions->objectModel.getConsumedSizeInSlotsWithHeader(nextObject)
