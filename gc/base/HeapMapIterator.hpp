...
private:
	...
	bool
		_useLargeObjectOptimization; /**< Set to true if we want to read objects from the heap and determine their size in order to skip mark map bits which are inside the object.  If this is set to false, we will blindly return the addresses representing the set bits in the mark map */
...
public:
	...
	MM_HeapMapIterator(MM_GCExtensionsBase *extensions)
		: _heapSlotCurrent(NULL), _heapChunkTop(NULL), _heapMapSlotCurrent(NULL), _bitIndexHead(0),
		  _heapMapSlotValue(0), _extensions(extensions), _useLargeObjectOptimization(true)
	{
	}
};
