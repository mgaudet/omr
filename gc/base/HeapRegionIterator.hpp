...
private:
    MM_MemorySpace*
        _space; /**< The MemorySpace to which all regions returned by the iterator must belong (can be NULL if this
                   check is not needed) */
    ...
    uint32_t
        _includedRegionsMask; /**< A bitmap of MM_HeapRegionDescriptor::RegionProperties of the regions to include
                                 during iteration */
