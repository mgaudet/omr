...
protected:
    ...
    uintptr_t
        _totalHeapSize; /**< The size, in bytes, of all currently active regions on the heap (that is, both table
...
public:
     ...
     * include the "gap").
     * Calls to this method have the side-effect of allocating internal data structures to manage the regions backing
     * this contiguous heap
     * so a boolean return value is provided to inform the caller if these internal data structures were successfully
     * initialized.
     *
     ...
     * and spanning regions will be some multiple
     * of this number)
     */
...
protected:
     ...
     * [lowAddress, highAddress).  This is required since
     * the sub-class is allowed to implement descriptors as any sub-class of MM_HeapRegionDescriptor and the caller
     ...
     * required since the sub-class is
     * allowed to implement descriptors as any sub-class of MM_HeapRegionDescriptor and the caller cannot assume how its
     * storage is managed.
     *
     * @param env The environment
     ...
     * extent [lowHeapEdge, highHeapEdge).
     * This is required since the sub-class is allowed to implement the descriptor table as a contiguous array of any
     * sub-class of MM_HeapRegionDescriptor
     * and the caller cannot assume how its storage is managed.  The returned descriptor will later be passed to
     * freeRegionTable when it is no longer needed.
     *
     * @param env The environment
     * @param lowHeapEdge the lowest byte addressable in the contiguous heap (note that this might not be presently
     * committed)
     ...
     * tableBase.  This is required since the
     * sub-class is allowed to implement the descriptor table as a contiguous array of any sub-class of
     * MM_HeapRegionDescriptor and the caller cannot assume
