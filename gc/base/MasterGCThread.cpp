...
void MM_MasterGCThread::masterThreadEntryPoint()
{
    ...
    omrVMThread = cli->attachVMThread(
        _extensions->getOmrVM(), "Dedicated GC Master", MM_CollectorLanguageInterfaceImpl::ATTACH_GC_MASTER_THREAD);
    if (NULL == omrVMThread) {
        ...
        do {
            if (STATE_GC_REQUESTED == _masterThreadState) {
                /* Note that we might want to give up the _collectorControlMutex around the call to run the collect but
                 * it might allow
                 * for some bugs to become harder to track down (consider the case of an additional thread entering the
                 * monitor during
                 * the collect - this can't happen but allowing it to access the monitor would make the bug harder to
                 * find) and holding
                 * the monitor does expose the thread dependency to the OS so it can potentially give the master GC
                 * thread priority
                ...
                /* this thread effectively inherits exclusive access from the mutator thread -- set its state to
                 * indicate this */
            ...
            if (STATE_WAITING == _masterThreadState) {
                ...
                if (_collector->isConcurrentWorkAvailable(env)) {
                    ...
                    omrthread_monitor_exit(_collectorControlMutex);
                    /* note that we currently run the concurrent mark without VM access since it could deadlock us with
                     * another thread trying to acquire exclusive to invoke a GC */
                    ...
                    if (STATE_RUNNING_CONCURRENT == _masterThreadState) {
                        /* we are still in the concurrent state (might have changed if a GC was requested) so set it
                         * back to waiting */
                        _masterThreadState = STATE_WAITING;
                    }
                } else {
        ...
        omrthread_monitor_notify(_collectorControlMutex);
        cli->detachVMThread(
            _extensions->getOmrVM(), omrVMThread, MM_CollectorLanguageInterfaceImpl::ATTACH_GC_MASTER_THREAD);
        omrthread_exit(_collectorControlMutex);
    }
}
...
bool MM_MasterGCThread::garbageCollect(MM_EnvironmentBase* env, MM_AllocateDescription* allocDescription)
{
    ...
    if (NULL != _collector) {
        ...
        if (NULL == _masterGCThread) {
             ...
             * wait/notify mechanism here)
             * that another thread could come in under this mutex and stomp on the "parameters" while another thread is
             * waiting.
             */
