...
public:
	...
	virtual uintptr_t
	getActiveMemoryPoolCount()
	{
		return 1;
	}

	virtual void
	setParent(MM_MemoryPool *parent)
	{
		_parent = parent;
	}
	 ...
	 */
	void
	resetFreeEntryAllocateStats(MM_LargeObjectAllocateStats *largeObjectAllocateStats)
	{
	...
	virtual void
	mergeFreeEntryAllocateStats()
	{
	}
	virtual void
	mergeTlhAllocateStats()
	{
	}
	virtual void
	mergeLargeObjectAllocateStats()
	{
	}
	/**
	 ...
	 */
	virtual void
	averageLargeObjectAllocateStats(MM_EnvironmentBase *env, uintptr_t bytesAllocatedThisRound)
	{
	...
	{
		return _minimumFreeEntrySize;
	}
	MMINLINE uintptr_t
	setMinimumFreeEntrySize(uintptr_t minimumFreeEntrySize)
	{
	...
	MMINLINE virtual uintptr_t
	getApproximateFreeLOAMemorySize()
	{
		return 0;
	}

	MMINLINE virtual uintptr_t
	getActualFreeEntryCount()
	{
		return _freeEntryCount;
	}

	MMINLINE void
	setFreeMemorySize(uintptr_t freeMemorySize)
	{
		_freeMemorySize = freeMemorySize;
	}
	MMINLINE void
	setFreeEntryCount(uintptr_t entryCount)
	{
	...
	isMemoryPoolValid(MM_EnvironmentBase *env, bool postCollect)
	{
		return true;
	}
#endif
	...
	MMINLINE void
	setNext(MM_MemoryPool *memoryPool)
	{
		_next = memoryPool;
	}
	MMINLINE void
	setPrevious(MM_MemoryPool *memoryPool)
	{
	...
	MMINLINE void
	setLargestFreeEntry(uintptr_t largestFreeEntry)
	{
	...
	virtual void *collectorAllocate(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
	...
	virtual MM_HeapLinkedFreeHeader *rebuildFreeListInRegion(MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region,
	...
	virtual bool
	initializeSweepPool(MM_EnvironmentBase *env)
	{
		return true;
	}
	 ...
	 */
	MMINLINE void
	incrementDarkMatterBytes(uintptr_t bytes)
	{
	...
	MMINLINE void
	resetDarkMatterBytes()
	{
		_darkMatterBytes = 0;
	}

	MMINLINE void
	incrementDarkMatterSamples(uintptr_t samples)
	{
		_darkMatterSamples += samples;
	}

	MMINLINE virtual uintptr_t
	getDarkMatterSamples()
	{
	...
	MM_MemoryPool(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize)
		: MM_BaseVirtual(), _next(NULL), _previous(NULL), _children(NULL), _poolName("Unknown"), _parent(NULL),
		  _memorySubSpace(NULL), _minimumFreeEntrySize(minimumFreeEntrySize), _freeMemorySize(0), _freeEntryCount(0),
		  _largestFreeEntry(0), _approximateFreeMemorySize(0), _allocCount(0), _allocBytes(0), _lastFreeBytes(0),
		  _allocDiscardedBytes(0), _allocSearchCount(0), _extensions(env->getExtensions()),
		  _largeObjectAllocateStats(NULL), _darkMatterBytes(0), _darkMatterSamples(0)
	{
	...
	MM_MemoryPool(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize, const char *name)
		: MM_BaseVirtual(), _next(NULL), _previous(NULL), _children(NULL), _poolName(name), _parent(NULL),
		  _memorySubSpace(NULL), _minimumFreeEntrySize(minimumFreeEntrySize), _freeMemorySize(0), _freeEntryCount(0),
		  _largestFreeEntry(0), _approximateFreeMemorySize(0), _allocCount(0), _allocBytes(0), _lastFreeBytes(0),
		  _allocDiscardedBytes(0), _allocSearchCount(0), _extensions(env->getExtensions()),
		  _largeObjectAllocateStats(NULL), _darkMatterBytes(0), _darkMatterSamples(0)
