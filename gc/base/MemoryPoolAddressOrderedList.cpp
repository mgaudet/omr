...
MM_MemoryPoolAddressOrderedList::newInstance(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize, const char *name)
{
	...
	memoryPool = (MM_MemoryPoolAddressOrderedList *)env->getForge()->allocate(
		sizeof(MM_MemoryPoolAddressOrderedList), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (memoryPool) {
...
MM_MemoryPoolAddressOrderedList::initialize(MM_EnvironmentBase *env)
{
	...
	_largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
		...
		_extensions->heap->getMaximumMemorySize(), _extensions->tlhMaximumSize + _minimumFreeEntrySize,
...
void *
MM_MemoryPoolAddressOrderedList::collectorAllocate(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
												   bool lockingRequired)
{
...
MM_MemoryPoolAddressOrderedList::rebuildFreeListInRegion(MM_EnvironmentBase *env, MM_HeapRegionDescriptor *region,
...
/* TODO 108399 Determine whether this is still necessary (OMR_SCAVENGER_DEBUG is defined in Scqavenger.cpp and is not accessible here) */
	...
	if (createFreeEntry(env, rangeBase, rangeTop, previousFreeEntry, NULL)) {
		...
		TRIGGER_J9HOOK_MM_PRIVATE_REBUILD_FREE_LIST(env->getExtensions()->privateHookInterface, env->getOmrVMThread(),
													rangeBase, rangeTop);
	}
	unlock(env);
 ...
 */
void
MM_MemoryPoolAddressOrderedList::expandWithRange(MM_EnvironmentBase *env, uintptr_t expandSize, void *lowAddress,
 ...
 */
void *
MM_MemoryPoolAddressOrderedList::contractWithRange(MM_EnvironmentBase *env, uintptr_t contractSize, void *lowAddress,
												   void *highAddress)
{
...
MM_MemoryPoolAddressOrderedList::removeFreeEntriesWithinRange(MM_EnvironmentBase *env, void *lowAddress,
															  void *highAddress, uintptr_t minimumSize,
 ...
 */
void *
MM_MemoryPoolAddressOrderedList::findAddressAfterFreeSize(MM_EnvironmentBase *env, uintptr_t sizeRequired,
