 ...
 */
	void
	updateMemoryPoolStatistics(MM_EnvironmentBase *env, uintptr_t freeBytes, uintptr_t freeEntryCount,
	...
	MMINLINE bool
	internalRecycleHeapChunk(void *addrBase, void *addrTop, MM_HeapLinkedFreeHeader *next)
	{
	...
	MMINLINE bool
	appendToList(MM_EnvironmentBase *env, void *addrBase, void *addrTop, uintptr_t minimumSize,
	...
	MMINLINE bool
	insertToList(MM_EnvironmentBase *env, void *addrBase, void *addrTop, uintptr_t minimumSize,
				 MM_HeapLinkedFreeHeader *&freeListHead, MM_HeapLinkedFreeHeader *&freeListTail)
	{
		uintptr_t freeEntrySize = ((uint8_t *)addrTop - (uint8_t *)addrBase);
...
public:
	...
	MM_MemoryPoolAddressOrderedListBase(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize)
		: MM_MemoryPool(env, minimumFreeEntrySize), _referenceHeapFreeList(NULL), _sweepPoolState(NULL),
	...
	MM_MemoryPoolAddressOrderedListBase(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize, const char *name)
		: MM_MemoryPool(env, minimumFreeEntrySize, name), _referenceHeapFreeList(NULL), _sweepPoolState(NULL),
		  _sweepPoolManager(NULL){
			  //		_typeId = __FUNCTION__;
		  };
