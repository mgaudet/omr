 ...
 *
 */
void reportGlobalGCComplete(J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
...
void MM_MemoryPoolLargeObjects::postCollect(MM_EnvironmentBase* env)
{
    ...
    if ((_soaBytesAfterLastGC < minimumSOAFreeRequired) && (LOA_EMPTY != _currentLOABase)) {
        ...
        /* Recalculate minimum size required to account for memory transfer.  This is solving the following equation for
         * MinimumRequired:
         *
    ...
    MM_EnvironmentBase* env, uintptr_t expandSize, void* lowAddress, void* highAddress, bool canCoalesce)
{
    ...
    if (0 == _currentOldAreaSize) {
        ...
        if (debug) {
            omrtty_printf("LOA Initial Allocation: heapSize %zu, Initial LOA ratio is %.3f --> LOA base is %p initial "
                          "LOA size %zu\n",
        ...
        if (0 == _currentLOARatio) {
            ...
            if (debug) {
                omrtty_printf("LOA resized on heap expansion: heapSize %zu,  LOA ratio is %.3f --> LOA base is now %p "
                              "LOA size %zu\n",
                    oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
            }
        }
    ...
    MM_EnvironmentBase* env, uintptr_t contractSize, void* lowAddress, void* highAddress)
{
    ...
    if (0 == _currentLOARatio) {
        ...
        if (debug) {
            omrtty_printf("LOA resized on heap contraction: heapSize %zu,  LOA ratio is %.3f --> LOA base is now %p "
                          "LOA size %zu\n",
                oldAreaSize, _currentLOARatio, _currentLOABase, _loaSize);
        }
    }
