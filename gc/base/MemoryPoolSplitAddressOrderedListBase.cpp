...
MM_MemoryPoolSplitAddressOrderedListBase::printFreeListValidity(MM_EnvironmentBase *env)
{
	...
	for (uintptr_t i = 0; i < _heapFreeListCountExtended; ++i) {
		...
		omrtty_printf("  -- Free List %4zu (head: 0x%p, tail: 0x%p, expected size: %16zu, expected holes: %16zu): ", i,
					  _heapFreeLists[i]._freeList, previousFreeEntry, _heapFreeLists[i]._freeSize,
 ...
 */
void *
MM_MemoryPoolSplitAddressOrderedListBase::findAddressAfterFreeSize(MM_EnvironmentBase *env, uintptr_t sizeRequired,
