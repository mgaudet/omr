...
bool MM_MemoryPoolSplitAddressOrderedListBase::initialize(MM_EnvironmentBase* env)
{
    ...
    _currentThreadFreeList = (uintptr_t*)extensions->getForge()->allocate(
        sizeof(uintptr_t) * _heapFreeListCount, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    _heapFreeLists = (J9ModronHeapFreeList*)extensions->getForge()->allocate(
        sizeof(J9ModronHeapFreeList) * _heapFreeListCountExtended, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    _largeObjectAllocateStatsForFreeList = (MM_LargeObjectAllocateStats*)extensions->getForge()->allocate(
        sizeof(MM_LargeObjectAllocateStats) * _heapFreeListCountExtended, MM_AllocationCategory::FIXED,
        OMR_GET_CALLSITE());
     ...
     * Tenure SubSpace for Gencon will set _largeObjectCollectorAllocateStatsForFreeList to
     * _largeObjectCollectorAllocateStatsForFreeList (we append collector stats to mutator stats)
     * Tenure SubSpace for Flat will leave _largeObjectCollectorAllocateStatsForFreeList at NULL (no interest in
     * collector stats)
     * We do not even need _largeObjectCollectorAllocateStats here (as we do in plain MPAOL) - we'll just merge whatever
