...
public:
	...
	MMINLINE void
	setNext(MM_MemorySpace *next)
	{
		_next = next;
	}
	MMINLINE void
	setPrevious(MM_MemorySpace *previous)
	{
		_previous = previous;
	}
	...
	virtual void
	setDepth(MM_EnvironmentBase *env, uintptr_t depth)
	{
	}
	virtual uintptr_t *
	getBaseAddress()
	...
	virtual uintptr_t *
	setBaseAddress(uintptr_t *baseAddress)
	{
		return NULL;
	}
	virtual void
	cleanJVMTIReferences(MM_EnvironmentBase *env)
	{
		return;
	}
	virtual void
	resetFirstUnfinalized(MM_EnvironmentBase *env)
	{
		return;
	}
	...
	MMINLINE void
	setDefaultMemorySubSpace(MM_MemorySubSpace *memorySpace)
	{
		_defaultMemorySubSpace = memorySpace;
	}
	MMINLINE void
	setTenureMemorySubSpace(MM_MemorySubSpace *memorySpace)
	{
	...
	MMINLINE MM_PhysicalArena *
	getPhysicalArena()
	{
		return _physicalArena;
	}
	...
	static MM_MemorySpace *
	getMemorySpace(void *memorySpace)
	{
	...
	MM_MemorySpace(MM_Heap *heap, MM_PhysicalArena *physicalArena, MM_InitializationParameters *parameters,
		...
		: MM_BaseVirtual(), _next(NULL), _previous(NULL),
		  ...
		  _minimumSize(parameters->_minimumSpaceSize), _currentSize(0), _maximumSize(parameters->_maximumSpaceSize),
		  _heap(heap), _defaultMemorySubSpace(NULL), _tenureMemorySubSpace(NULL), _memorySubSpaceList(NULL),
		  _physicalArena(physicalArena), _name(name), _description(description), _uniqueID(0)
	{
