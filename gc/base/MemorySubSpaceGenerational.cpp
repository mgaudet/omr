											  ...
											  bool shouldCollectOnFailure)
{
	if (shouldCollectOnFailure) {
		...
		if (previousSubSpace == _memorySubSpaceNew) {
			...
			return _memorySubSpaceOld->allocateObject(env, allocDescription, baseSubSpace, this,
										   ...
										   bool shouldCollectOnFailure)
{
	if (shouldCollectOnFailure) {
		...
		if (previousSubSpace == _memorySubSpaceNew) {
			/* The allocate request is coming from new space - forward on to the old area */
			return _memorySubSpaceOld->allocateTLH(env, allocDescription, objectAllocationInterface, baseSubSpace, this,
												   false);
		}
...
MM_MemorySubSpaceGenerational::newInstance(MM_EnvironmentBase *env, MM_MemorySubSpace *memorySubSpaceNew,
										   ...
										   uintptr_t maximumSizeNew, uintptr_t minimumSizeOld, uintptr_t initialSizeOld,
										   uintptr_t maximumSizeOld, uintptr_t maximumSize)
{
	...
	memorySubSpace = (MM_MemorySubSpaceGenerational *)env->getForge()->allocate(
		sizeof(MM_MemorySubSpaceGenerational), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (memorySubSpace) {
		new (memorySubSpace) MM_MemorySubSpaceGenerational(
			env, memorySubSpaceNew, memorySubSpaceOld, usesGlobalCollector, minimumSize, minimumSizeNew, initialSizeNew,
			maximumSizeNew, minimumSizeOld, initialSizeOld, maximumSizeOld, maximumSize);
...
void
MM_MemorySubSpaceGenerational::checkResize(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
										   bool _systemGC)
{
