    ...
    MM_MemorySubSpace* baseSubSpace, MM_MemorySubSpace* previousSubSpace, bool shouldCollectOnFailure)
{
     ...
     * not complete (_allocateAtSafePointOnly is false),
     * so we most often proceed with allocate.
     * If we are notified that concurrent mark is complete ( _allocateAtSafePointOnly is true, and typically the stack
     * frame is not built yet,
     ...
     * we proceed with allocate (otherwise risk OOM),
