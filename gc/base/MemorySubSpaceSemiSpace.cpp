    ...
    MM_MemorySubSpace* previousSubSpace)
{
     ...
     * current subSpace will become the
     ...
     * collection is
     * successful, but not if we backout so we don't know which subspace will allocate until after the collect - our own
     * allocation
 ...
 ****************************************
 */
void MM_MemorySubSpaceSemiSpace::flip(MM_EnvironmentBase* env, uintptr_t step)
{
    if (1 == step) {
...
void MM_MemorySubSpaceSemiSpace::rebuildFreeListForEvacuate(MM_EnvironmentBase* env)
{
...
void MM_MemorySubSpaceSemiSpace::rebuildFreeListForBackout(MM_EnvironmentBase* env)
{
