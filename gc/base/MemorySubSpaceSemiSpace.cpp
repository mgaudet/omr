										   ...
										   bool shouldCollectOnFailure)
{
	...
	if (shouldCollectOnFailure) {
		addr =
			_memorySubSpaceAllocate->allocateObject(env, allocDescription, baseSubSpace, this, shouldCollectOnFailure);
	} else {
													...
													MM_MemorySubSpace *previousSubSpace)
{
	...
	if (ALLOCATION_TYPE_TLH != allocationType) {
		addr = _parent->allocationRequestFailed(env, allocateDescription, allocationType, objectAllocationInterface,
												this, this);
	}
...
void
MM_MemorySubSpaceSemiSpace::payAllocationTax(MM_EnvironmentBase *env, MM_MemorySubSpace *baseSubSpace,
										...
										uintptr_t minimumSize, uintptr_t initialSize, uintptr_t maximumSize)
{
	...
	memorySubSpace = (MM_MemorySubSpaceSemiSpace *)env->getForge()->allocate(
		sizeof(MM_MemorySubSpaceSemiSpace), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (memorySubSpace) {
		new (memorySubSpace)
			MM_MemorySubSpaceSemiSpace(env, collector, physicalSubArena, memorySubSpaceAllocate, memorySubSpaceSurvivor,
...
MM_MemorySubSpaceSemiSpace::initialize(MM_EnvironmentBase *env)
{
	...
	_largeObjectAllocateStats = MM_LargeObjectAllocateStats::newInstance(
		...
		_extensions->heap->getMaximumMemorySize(), _extensions->tlhMaximumSize + _extensions->minimumFreeEntrySize,
...
MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectTilt(MM_EnvironmentBase *env)
{
	...
	if (extensions->tiltedScavenge) {
		...
		_desiredSurvivorSpaceRatio =
			(_tiltedAverageBytesFlipped + _tiltedAverageBytesFlippedDelta) * survivorSizeAmplification / currentSize;
...
MM_MemorySubSpaceSemiSpace::checkSubSpaceMemoryPostCollectResize(MM_EnvironmentBase *env)
{
	...
	if (extensions->dynamicNewSpaceSizing) {
		...
		uint64_t scavengeTime =
			omrtime_hires_delta(extensions->scavengerStats._startTime, extensions->scavengerStats._endTime,
								OMRPORT_TIME_DELTA_IN_MILLISECONDS);
 ...
 */
void
MM_MemorySubSpaceSemiSpace::checkResize(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
										bool systemGC)
{
