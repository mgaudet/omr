 ...
 */
void
MM_MemorySubSpaceUniSpace::checkResize(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
									   bool _systemGC)
{
 ...
 */
bool
MM_MemorySubSpaceUniSpace::timeForHeapContract(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription,
											   bool systemGC)
{
	...
	if (allocSize > getApproximateActiveFreeMemorySize()) {
		...
		if (currentFree > maximumFree) {
			...
			if (currentHeapSize < targetHeapSize) {
				...
				Trc_MM_MemorySubSpaceUniSpace_calculateTargetContractSize_Event1(env->getLanguageVMThread(),
																				 contractionSize);
				...
				Trc_MM_MemorySubSpaceUniSpace_calculateTargetContractSize_Event2(env->getLanguageVMThread(),
																				 contractionSize, maxContract);
			}
		} else {
...
uintptr_t
MM_MemorySubSpaceUniSpace::calculateCollectorExpandSize(MM_EnvironmentBase *env, MM_Collector *requestCollector,
 ...
 */
MMINLINE uintptr_t
MM_MemorySubSpaceUniSpace::adjustExpansionWithinSoftMax(MM_EnvironmentBase *env, uintptr_t expandSize,
		...
		if ((minimumBytesRequired != 0) && ((activeMemorySize + minimumBytesRequired) > actualSoftMx)) {
			if (J9_EVENT_IS_HOOKED(env->getExtensions()->omrHookInterface, J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX)) {
				ALWAYS_TRIGGER_J9HOOK_MM_OMR_OOM_DUE_TO_SOFTMX(
					env->getExtensions()->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
					heap->getMaximumMemorySize(), heap->getActiveMemorySize(), env->getExtensions()->softMx,
