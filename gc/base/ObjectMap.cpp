...
MM_ObjectMap::newInstance(MM_EnvironmentBase *env)
{
	...
	objectMap = (MM_ObjectMap *)env->getForge()->allocate(sizeof(MM_ObjectMap), MM_AllocationCategory::FIXED,
														  OMR_GET_CALLSITE());
	if (objectMap) {
...
bool
MM_ObjectMap::heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress,
						   void *highAddress)
{
	bool result = true;
...
MM_ObjectMap::markValidObjectForRange(MM_EnvironmentBase *env, void *heapBase, void *heapTop)
{
	...
	GC_ObjectHeapIteratorAddressOrderedList objectHeapIterator(env->getExtensions(), (omrobjectptr_t)heapBase, heapTop,
															   false, false);
