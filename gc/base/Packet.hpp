...
private:
	...
	MMINLINE void
	setSublistIndex(uintptr_t sublistIndex)
	{
...
public:
	 ...
	 */
	MMINLINE void
	setOwner(MM_EnvironmentBase *owner)
	{
		_owner = owner;
	}
	 ...
	 */
	uintptr_t **
	getCurrentAddr(MM_EnvironmentBase *env)
	{
		return &_currentPtr;
	}
	 ...
	 */
	uintptr_t **
	getTopAddr(MM_EnvironmentBase *env)
	{
		return &_topPtr;
	}
	 ...
	 */
	void
	setFull(MM_EnvironmentBase *env)
	{
		_currentPtr = _topPtr;
	}

	MMINLINE bool
	isFull(MM_EnvironmentBase *env)
	{
	 ...
	 */
	MMINLINE void
	clear(MM_EnvironmentBase *env)
	{
		memset(_basePtr, 0, ((uint8_t *)_topPtr - (uint8_t *)_basePtr));
	}

	bool initialize(MM_EnvironmentBase *env, MM_Packet *next, MM_Packet *previous, uintptr_t *baseAddress,
					uintptr_t size);
	 ...
	 */
	MM_Packet()
		: MM_BaseNonVirtual(), _baseAddress(NULL), _basePtr(NULL), _topPtr(NULL), _currentPtr(NULL), _sublistIndex(0),
		  _owner(NULL), _next(NULL), _previous(NULL)
	{
...
protected:
	 ...
	 */
	MMINLINE void
	resetData(MM_EnvironmentBase *env)
	{
