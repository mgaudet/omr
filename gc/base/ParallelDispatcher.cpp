    ...
    MM_EnvironmentBase* env, omrsig_handler_fn handler, void* handler_arg, uintptr_t defaultOSStackSize)
{
    ...
    dispatcher = (MM_ParallelDispatcher*)env->getForge()->allocate(
        sizeof(MM_ParallelDispatcher), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (dispatcher) {
...
bool MM_ParallelDispatcher::initialize(MM_EnvironmentBase* env)
{
    ...
    _threadTable = (omrthread_t*)forge->allocate(
        _threadCountMaximum * sizeof(omrthread_t), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (!_threadTable) {
        goto error_no_memory;
    }
    ...
    _statusTable = (uintptr_t*)forge->allocate(
        _threadCountMaximum * sizeof(uintptr_t*), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (!_statusTable) {
        goto error_no_memory;
    }
    ...
    _taskTable = (MM_Task**)forge->allocate(
        _threadCountMaximum * sizeof(MM_Task*), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (!_taskTable) {
        goto error_no_memory;
    }
 ...
 */
void MM_ParallelDispatcher::recomputeActiveThreadCount(MM_EnvironmentBase* env)
{
...
void MM_ParallelDispatcher::prepareThreadsForTask(MM_EnvironmentBase* env, MM_Task* task)
{
    omrthread_monitor_enter(_slaveThreadMutex);
