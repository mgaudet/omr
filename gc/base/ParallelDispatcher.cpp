...
dispatcher_thread_proc2(OMRPortLibrary *portLib, void *info)
{
	...
	omrVMThread =
		cli->attachVMThread(omrVM, "GC Slave", MM_CollectorLanguageInterfaceImpl::ATTACH_GC_DISPATCHER_THREAD);
	if (NULL == omrVMThread) {
		goto startup_failed;
	}
...
MM_ParallelDispatcher::newInstance(MM_EnvironmentBase *env, omrsig_handler_fn handler, void *handler_arg,
								   uintptr_t defaultOSStackSize)
{
	MM_ParallelDispatcher *dispatcher;

	dispatcher = (MM_ParallelDispatcher *)env->getForge()->allocate(sizeof(MM_ParallelDispatcher),
...
MM_ParallelDispatcher::initialize(MM_EnvironmentBase *env)
{
	...
	_statusTable = (uintptr_t *)forge->allocate(_threadCountMaximum * sizeof(uintptr_t *), MM_AllocationCategory::FIXED,
												OMR_GET_CALLSITE());
	if (!_statusTable) {
		goto error_no_memory;
	}
	...
	_taskTable = (MM_Task **)forge->allocate(_threadCountMaximum * sizeof(MM_Task *), MM_AllocationCategory::FIXED,
											 OMR_GET_CALLSITE());
	if (!_taskTable) {
		goto error_no_memory;
	}
...
MM_ParallelDispatcher::startUpThreads()
{
	...
	while (slaveThreadCount < _threadCountMaximum) {
		...
		threadForkResult =
			createThreadWithCategory(&(_threadTable[slaveThreadCount]), _defaultOSStackSize, getThreadPriority(), 0,
									 dispatcher_thread_proc, (void *)&slaveInfo, J9THREAD_CATEGORY_SYSTEM_GC_THREAD);
