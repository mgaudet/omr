...
public:
	...
	static MM_ParallelDispatcher *newInstance(MM_EnvironmentBase *env, omrsig_handler_fn handler, void *handler_arg,
											  uintptr_t defaultOSStackSize);
	virtual void kill(MM_EnvironmentBase *env);

	virtual void reinitAfterFork(MM_EnvironmentBase *env, uintptr_t newThreadCount);

	MM_ParallelDispatcher(MM_EnvironmentBase *env, omrsig_handler_fn handler, void *handler_arg,
						  uintptr_t defaultOSStackSize)
		: MM_Dispatcher(env), _extensions(MM_GCExtensionsBase::getExtensions(env->getOmrVM())), _threadShutdownCount(0),
		  _threadTable(NULL), _statusTable(NULL), _taskTable(NULL), _slaveThreadMutex(NULL), _dispatcherMonitor(NULL),
		  _synchronizeMutex(NULL), _slaveThreadsReservedForGC(false), _inShutdown(false), _threadCountMaximum(1),
		  _threadCount(1), _activeThreadCount(1), _handler(handler), _handler_arg(handler_arg),
