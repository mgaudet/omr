...
MM_PhysicalArenaVirtualMemory::newInstance(MM_EnvironmentBase *env, MM_Heap *heap)
{
	...
	arena = (MM_PhysicalArenaVirtualMemory *)env->getForge()->allocate(
		sizeof(MM_PhysicalArenaVirtualMemory), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (arena) {
 ...
 */
bool
MM_PhysicalArenaVirtualMemory::attachSubArena(MM_EnvironmentBase *env, MM_PhysicalSubArena *subArena, uintptr_t size,
											  uintptr_t attachPolicy)
{
	...
	if (0 != currentSubArena->getNumaNode()) {
		...
		if (0 != j9NodeNumber) {
			if (!ext->memoryManager->setNumaAffinity(((MM_HeapVirtualMemory *)_heap)->getVmemHandle(), j9NodeNumber,
 ...
 */
bool
MM_PhysicalArenaVirtualMemory::canExpand(MM_EnvironmentBase *env, MM_PhysicalSubArenaVirtualMemory *subArena,
 ...
 */
uintptr_t
MM_PhysicalArenaVirtualMemory::maxExpansion(MM_EnvironmentBase *env, MM_PhysicalSubArenaVirtualMemory *subArena,
											void *expandAddress)
{
