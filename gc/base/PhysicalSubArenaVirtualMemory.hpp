...
public:
	MMINLINE MM_PhysicalSubArenaVirtualMemory *
	getNextSubArena()
	{
		return _highArena;
	}
	MMINLINE void
	setNextSubArena(MM_PhysicalSubArenaVirtualMemory *subArena)
	{
		_highArena = subArena;
	}
	MMINLINE MM_PhysicalSubArenaVirtualMemory *
	 ...
	 */
	MMINLINE uintptr_t
	calculateOffsetToHighAddress(void *address)
	{
		return (uintptr_t)(((uintptr_t)_highAddress - (uintptr_t)address));
	}

	MM_PhysicalSubArenaVirtualMemory(MM_Heap *heap)
		: MM_PhysicalSubArena(heap), _lowArena(NULL), _highArena(NULL), _lowAddress(NULL), _highAddress(NULL),
		  _hasVirtualLowAddress(false), _hasVirtualHighAddress(false), _virtualLowAddress(NULL),
		  _virtualHighAddress(NULL), _expandFromHighRange(false), _expandFromLowRange(false), _numaNode(0)
	{
