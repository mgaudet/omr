...
private:
	...
	MM_HeapRegionQueue *_perContextSmallFullRegions
		[OMR_SIZECLASSES_NUM_SMALL + 1]; /**< Per-context Regions that have been allocated into during this GC cycle. */
	MM_HeapRegionQueue *
...
public:
	...
	virtual void
	flushForShutdown(MM_EnvironmentBase *env)
	{
		/* ---- no implementation -----
	...
	void
	setMarkingScheme(MM_SegregatedMarkingScheme *markingScheme)
	{
...
protected:
	MM_AllocationContextSegregated(MM_EnvironmentBase *env, MM_GlobalAllocationManagerSegregated *gam,
								   MM_RegionPoolSegregated *regionPool)
		: MM_AllocationContext(), _regionPool(regionPool), _arrayletRegion(NULL), _markingScheme(NULL),
		  _mutexSmallAllocations(NULL), _mutexArrayletAllocations(NULL), _count(0),
		  _perContextArrayletFullRegions(NULL), _perContextLargeFullRegions(NULL)
	{
	...
	virtual bool trySweepAndAllocateRegionFromSmallSizeClass(MM_EnvironmentBase *env, uintptr_t sizeClass,
															 uintptr_t *sweepCount, uint64_t *sweepStartTime);
	...
	MMINLINE
	void
	flushHelper(MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region, uintptr_t cellSize)
	{
