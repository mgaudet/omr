...
MM_Configuration* MM_ConfigurationSegregated::newInstance(
  ...
  configuration = (MM_ConfigurationSegregated*)env->getForge()->allocate(
      sizeof(MM_ConfigurationSegregated), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
...
J9Pool* MM_ConfigurationSegregated::createEnvironmentPool(
  ...
  return pool_new(sizeof(MM_EnvironmentBase), numberElements, sizeof(uint64_t),
                  poolFlags, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_MM,
...
bool MM_ConfigurationSegregated::initializeEnvironment(
   ...
   * setting of the allocation
   * tracker in the env should be done by the base configuration
...
MM_ConfigurationSegregated::createObjectAllocationInterface(
    MM_EnvironmentBase* env) {
 ...
 */
bool MM_ConfigurationSegregated::aquireAllocationContext(
    MM_EnvironmentBase* env) {
...
/**
 * Initialize and return the appropriate allocation tracker for the given
 * Environment. Note
 * that this function is currently called by the ConfigurationRealtime since
 * Realtime GCs are
 * currently the only configurations with allocation trackers. When this
...
uintptr_t MM_ConfigurationSegregated::internalGetDefaultRegionSize(
    MM_EnvironmentBase* env) {
  return SEGREGATED_REGION_SIZE_BYTES;
}

uintptr_t MM_ConfigurationSegregated::internalGetDefaultArrayletLeafSize(
    MM_EnvironmentBase* env) {
  return SEGREGATED_ARRAYLET_LEAF_SIZE_BYTES;
}

uintptr_t MM_ConfigurationSegregated::internalGetWriteBarrierType(
    MM_EnvironmentBase* env) {
  return _configurationLanguageInterface->internalGetWriteBarrierType(env);
}

uintptr_t MM_ConfigurationSegregated::internalGetAllocationType(
    MM_EnvironmentBase* env) {
...
MM_GlobalCollector* MM_ConfigurationSegregated::createGlobalCollector(
    MM_EnvironmentBase* env) {
  /* OMRTODO should we be going through the configurationLanguageInterface to
   * create the segregated GC? */
