 ...
 * for the memory lost to
 * internal fragmentation. The amount of internal fragmentation depends on the
 * size class for which
...
MM_HeapRegionDescriptorSegregated::emptyRegionAllocated(MM_EnvironmentBase* env)
{
  ...
  if (isSmall()) {
     ...
     * arraylets, the allocation
     * context will call into the allocation tracker for the allocation of the
     * arraylet.
     */
    _memoryPoolACL.addBytesAllocated(
