...
public:
	MM_HeapRegionDescriptorSegregated(MM_EnvironmentBase *env, void *lowAddress, void *highAddress)
		: MM_HeapRegionDescriptor(env, lowAddress, highAddress), _sizeClass(0),
		  _memoryPoolACL(env, 0 /* minimumFreeEntrySize */), _prev(NULL), _next(NULL), _regionManager(NULL),
		  _segregatedSizeClasses(env->getOmrVM()->_sizeClasses), _nextArrayletIndex(0)
	{
	...
	MMINLINE MM_HeapRegionDescriptorSegregated *
	getNext()
	{
		return _next;
	}
	MMINLINE void
	setNext(MM_HeapRegionDescriptorSegregated *next)
	{
		_next = next;
	}
	MMINLINE MM_HeapRegionDescriptorSegregated *
	...
	MMINLINE void
	setPrev(MM_HeapRegionDescriptorSegregated *prev)
	{
		_prev = prev;
	}

	MMINLINE MM_MemoryPoolAggregatedCellList *
	getMemoryPoolACL()
	{
		return (MM_MemoryPoolAggregatedCellList *)getMemoryPool();
	}
	void
	setSizeClass(uintptr_t sizeClass)
	{
	...
	isReserved()
	{
		return getRegionType() == RESERVED;
	}
	bool
	isSmall()
	{
		return getRegionType() == SEGREGATED_SMALL;
	}
	bool
	isLarge()
	{
		return getRegionType() == SEGREGATED_LARGE;
	}
	...
	void
	setRangeHead(MM_HeapRegionDescriptorSegregated *head)
	{
		_headOfSpan = head;
	}
	uintptr_t
	getRangeCount()
	{
		return _regionsInSpan;
	}
	void
	setRangeCount(uintptr_t count)
	{
		_regionsInSpan = count;
	}
	...
	void setArraylet();
	MMINLINE uintptr_t
	whichArraylet(uintptr_t *arraylet, uintptr_t arrayletLeafLogSize)
	{
		return ((uintptr_t)arraylet - (uintptr_t)getLowAddress()) >> arrayletLeafLogSize;
	}
	MMINLINE uintptr_t *
	getArraylet(uintptr_t index, uintptr_t arrayletLeafLogSize)
	{
		return (uintptr_t *)(((uintptr_t)getLowAddress()) + (index << arrayletLeafLogSize));
	}
	MMINLINE uintptr_t *
	getArrayletParent(uintptr_t index)
	{
		return _arrayletBackPointers[index];
	}
	MMINLINE void
	setArrayletParent(uintptr_t index, uintptr_t *backPointer)
	{
		_arrayletBackPointers[index] = backPointer;
	}
	MMINLINE void
	clearArraylet(uintptr_t index)
	{
		_arrayletBackPointers[index] = NULL;
	}
	MMINLINE bool
	isArrayletUnused(uintptr_t index)
	{
		return _arrayletBackPointers[index] == NULL;
	}
	MMINLINE bool
	isArrayletUsed(uintptr_t index)
	{
		return _arrayletBackPointers[index] != NULL;
	}
	MMINLINE void
	setNextArrayletIndex(uintptr_t index)
	{
