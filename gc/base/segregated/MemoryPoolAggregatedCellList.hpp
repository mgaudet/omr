...
protected:
	MM_HeapLinkedFreeHeader
		*volatile _freeListHead; /**< Pointer to the first entry in the free list, NULL means there are no free entries in this region */
...
public:
	MM_MemoryPoolAggregatedCellList(MM_EnvironmentBase *env, uintptr_t minimumFreeEntrySize)
		: MM_MemoryPool(env, minimumFreeEntrySize), _freeListHead(NULL), _heapCurrent(NULL), _heapTop(NULL),
		  _region(NULL), _markCount(0), _freeCount(0), _preSweepFreeBytes(0)
	{
	...
	void *
	allocateObject(MM_EnvironmentBase *env, MM_AllocateDescription *allocDescription)
	{
		return NULL;
	}
	void
	expandWithRange(MM_EnvironmentBase *env, uintptr_t expandSize, void *lowAddress, void *highAddress,
					bool canCoalesce)
	{
	}
	void *
	...
	bool
	abandonHeapChunk(void *addrBase, void *addrTop)
	{
		return true;
	}
	...
	MMINLINE void
	setMarkCount(uintptr_t markCount)
	{
	...
	MMINLINE uintptr_t
	getPreSweepFreeBytes()
	{
		return _preSweepFreeBytes;
	}
	MMINLINE void
	setPreSweepFreeBytes(uintptr_t preSweepFreeBytes)
	{
		_preSweepFreeBytes = preSweepFreeBytes;
	}
	MMINLINE void
	addSweepFreeBytes(MM_EnvironmentBase *env, uintptr_t sweepFreeBytes)
	{
	...
	void
	setRegion(MM_HeapRegionDescriptorSegregated *region)
	{
		_region = region;
	}
};
