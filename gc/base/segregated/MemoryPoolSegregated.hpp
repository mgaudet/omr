...
public:
	...
	virtual void *allocateTLH(MM_EnvironmentBase *env, uintptr_t maxSizeInBytesRequired, void *&addrBase,
							  void *&addrTop);
	...
	virtual void insertRange(MM_EnvironmentBase *env, void *prevFreeListEntry, uintptr_t prevFreeListEntrySize,
							 void *expandRangeBase, void *expandRangeTop, void *nextFreeListEntry,
							 uintptr_t nextFreeListEntrySize);
	virtual void buildRange(MM_EnvironmentBase *env, void *expandRangeBase, void *expandRangeTop);
	virtual void expandWithRange(MM_EnvironmentBase *env, uintptr_t expandSize, void *lowAddr, void *highAddr,
								 bool canCoalesce);
	...
	MMINLINE uintptr_t
	verbose(MM_EnvironmentBase *env)
	{
		return _extensions->verbose;
	}
	MMINLINE uintptr_t
	debug(MM_EnvironmentBase *env)
	{
...
protected:
	...
	MM_MemoryPoolSegregated(MM_EnvironmentBase *env, MM_RegionPoolSegregated *regionPool,
		...
		: MM_MemoryPool(env, minimumFreeEntrySize), _regionPool(regionPool),
		  _globalAllocationManager(globalAllocationManager), _extensions(NULL), _bytesInUse(0)
	{
		_typeId = __FUNCTION__;
	}

private:
	uintptr_t *allocateContiguous(MM_EnvironmentBase *env, MM_AllocateDescription *allocDesc,
								  MM_AllocationContextSegregated *ac);
#if defined(OMR_GC_ARRAYLETS)
	void *allocateChunkedArray(MM_EnvironmentBase *env, MM_AllocateDescription *allocDesc,
							   MM_AllocationContextSegregated *ac);
#endif /* OMR_GC_ARRAYLETS */
};
