...
MM_OverflowSegregated::newInstance(MM_EnvironmentBase *env, MM_WorkPackets *workPackets)
{
	...
	MM_OverflowSegregated *overflow = (MM_OverflowSegregated *)env->getForge()->allocate(
		sizeof(MM_OverflowSegregated), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (NULL != overflow) {
	...
	if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
		...
		while ((region = (MM_HeapRegionDescriptorSegregated *)regionIterator.nextRegion()) != NULL) {
			GC_ObjectHeapIteratorSegregated objectIterator(
				_extensions, (omrobjectptr_t)region->getLowAddress(), (omrobjectptr_t)region->getHighAddress(),
			...
			while ((object = objectIterator.nextObject()) != NULL) {
				...
				if ((markMap->isBitSet(object))
					...
					_extensions->collectorLanguageInterface->markingScheme_scanObject(
						env, object, MM_CollectorLanguageInterface::SCAN_REASON_OVERFLOWED_OBJECT);
				}
			}
		}
