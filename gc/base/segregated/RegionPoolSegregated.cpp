...
MM_RegionPoolSegregated::newInstance(MM_EnvironmentBase *env, MM_HeapRegionManager *heapRegionManager)
{
	...
	regionPool = (MM_RegionPoolSegregated *)env->getForge()->allocate(sizeof(MM_RegionPoolSegregated),
...
MM_RegionPoolSegregated::initialize(MM_EnvironmentBase *env)
{
	...
	for (szClass = OMR_SIZECLASSES_MIN_SMALL; szClass <= OMR_SIZECLASSES_MAX_SMALL; szClass++) {
		for (int32_t i = 0; i < NUM_DEFRAG_BUCKETS; i++) {
			...
			_smallAvailableRegions[szClass][i] = (MM_LockingHeapRegionQueue *)env->getForge()->allocate(
				splitAvailableListsSize, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	...
	/* The available lists should track the free bytes in their regions (4th param = true) */
	_arrayletAvailableRegions =
		MM_RegionPoolSegregated::allocateHeapRegionQueue(env, MM_HeapRegionList::HRL_KIND_AVAILABLE, true, true, true);
...
void
MM_RegionPoolSegregated::countFreeRegions(uintptr_t *singleFree, uintptr_t *multiFree, uintptr_t *maxMultiFree,
...
void
MM_RegionPoolSegregated::enqueueAvailable(MM_HeapRegionDescriptorSegregated *region, uintptr_t sizeClass,
...
void
MM_RegionPoolSegregated::addFreeRegion(MM_EnvironmentBase *env, MM_HeapRegionDescriptorSegregated *region,
									   bool alreadyFree)
{
...
MM_RegionPoolSegregated::allocateFromRegionPool(MM_EnvironmentBase *env, uintptr_t numRegions, uintptr_t szClass,
												uintptr_t maxExcess)
{
