...
private:
    MM_HeapRegionManager* _heapRegionManager; /**< The Region Manager which contains all the descriptors used for
                                                 allocation (they contain the Memory Pools which manage their underlying
                                                 memory) */
    ...
    MM_FreeHeapRegionList*
        _coalesceFreeList; /**< Free regions that might be coalescable, so avoid allocating from them if at all
                              possible. */
     ...
     * most occupied.
    */
    MM_LockingHeapRegionQueue* _smallAvailableRegions[OMR_SIZECLASSES_NUM_SMALL + 1]
                                                     [NUM_DEFRAG_BUCKETS]; /**< Regions that are available to be given
                                                                              out to allocation contexts and aren't
                                                                              entirely free. */
