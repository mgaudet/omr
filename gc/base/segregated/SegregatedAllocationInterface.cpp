...
MM_SegregatedAllocationInterface::newInstance(MM_EnvironmentBase *env)
{
	...
	allocationInterface = (MM_SegregatedAllocationInterface *)env->getForge()->allocate(
		sizeof(MM_SegregatedAllocationInterface), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
												 ...
												 MM_MemorySpace *memorySpace, bool shouldCollectOnFailure)
{
	...
	if (shouldCollectOnFailure) {
		...
		if (NULL == cell) {
			cell = memorySpace->getDefaultMemorySubSpace()->allocateObject(env, allocateDescription, NULL, NULL,
 ...
 */
void
MM_SegregatedAllocationInterface::replenishCache(MM_EnvironmentBase *env, uintptr_t sizeInBytes, void *cacheMemory,
												 uintptr_t cacheSize)
{
	MM_GCExtensionsBase *extensions = env->getExtensions();
