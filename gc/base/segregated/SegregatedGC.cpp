...
MM_SegregatedGC::newInstance(MM_EnvironmentBase *env, MM_CollectorLanguageInterface *cli)
{
	...
	globalGC = (MM_SegregatedGC *)env->getForge()->allocate(sizeof(MM_SegregatedGC), MM_AllocationCategory::FIXED,
															OMR_GET_CALLSITE());
	if (NULL != globalGC) {
...
bool
MM_SegregatedGC::heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress,
							  void *highAddress)
{
 ...
 */
bool
MM_SegregatedGC::internalGarbageCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
...
void
MM_SegregatedGC::internalPreCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
