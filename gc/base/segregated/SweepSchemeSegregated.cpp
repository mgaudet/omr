...
MM_SweepSchemeSegregated* MM_SweepSchemeSegregated::newInstance(MM_EnvironmentBase* env, MM_MarkMap* markMap)
{
    ...
    instance = (MM_SweepSchemeSegregated*)env->getForge()->allocate(
        sizeof(MM_SweepSchemeSegregated), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (NULL != instance) {
...
void MM_SweepSchemeSegregated::unmarkRegion(MM_EnvironmentBase* env, MM_HeapRegionDescriptorSegregated* region)
{
     ...
     * Note we need to ensure that cell size is at least >16 bytes, otherwise 16 bytes past J9Object* would go off the
     * end of page, hence the if check below.
