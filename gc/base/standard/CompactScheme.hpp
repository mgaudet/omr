...
public:
	...
	MM_CompactMemoryPoolState()
		: _memoryPool(NULL), _freeListHead(NULL), _freeBytes(0), _freeHoles(0), _largestFreeEntry(0),
		  _previousFreeEntry(NULL), _previousFreeEntrySize(0)
	{
	}
...
protected:
	...
	omrobjectptr_t doCompact(MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace, omrobjectptr_t start,
							 omrobjectptr_t finish, omrobjectptr_t &deadObject, uintptr_t &objectCount,
     ...
     */
	MMINLINE intptr_t
	pageIndex(omrobjectptr_t objectPtr) const
	{
	 ...
	 */
	MMINLINE intptr_t
	pageOffset(omrobjectptr_t objectPtr) const
	{
		uintptr_t markIndex = (uintptr_t)objectPtr - _heapBase;
		return markIndex % sizeof_page;
	}
	 ...
	 */
	MMINLINE intptr_t
	compressedPageOffset(omrobjectptr_t objectPtr) const
	{
	 ...
	 */
	MMINLINE omrobjectptr_t
	nextPage(omrobjectptr_t objectPtr) const
	{
...
public:
	...
	MM_CompactScheme(MM_EnvironmentBase *env, MM_MarkingScheme *markingScheme)
		: MM_BaseVirtual(), _omrVM(env->getOmrVM()), _extensions(env->getExtensions()),
		  _dispatcher(_extensions->dispatcher), _markingScheme(markingScheme), _markMap(markingScheme->getMarkMap()),
