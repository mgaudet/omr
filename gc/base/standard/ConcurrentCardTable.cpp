...
MM_ConcurrentCardTable::calculateTLHMarkMapSize(MM_EnvironmentStandard *env, uintptr_t cardTableSize)
{
	...
	size =
		MM_Math::roundToCeiling(sizeof(uint32_t), MM_Math::roundToCeiling(BITS_IN_BYTE, cardTableSize) / BITS_IN_BYTE);
 ...
 */
bool
MM_ConcurrentCardTable::allocateCardTableEntriesForHeapRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
 ...
 */
bool
MM_ConcurrentCardTable::freeCardTableEntriesForHeapRange(MM_EnvironmentStandard *env, uintptr_t size, void *lowAddress,
 ...
 */
bool
MM_ConcurrentCardTable::allocateTLHMarkMapEntriesForHeapRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
															  uintptr_t size, void *lowAddress, void *highAddress)
{
	bool commited = true;
 ...
 */
bool
MM_ConcurrentCardTable::freeTLHMarkMapEntriesForHeapRange(MM_EnvironmentStandard *env, uintptr_t size, void *lowAddress,
														  void *highAddress, void *lowValidAddress,
														  void *highValidAddress)
{
 ...
 */
bool
MM_ConcurrentCardTable::heapAddRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace, uintptr_t size,
 ...
 */
bool
MM_ConcurrentCardTable::heapRemoveRange(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace, uintptr_t size,
	...
	if (NULL != _heapAlloc) {
		...
		if (result) {
			if (subspace->isConcurrentCollectable()) {
				...
				result = freeTLHMarkMapEntriesForHeapRange(env, size, lowAddress, highAddress, lowValidAddress,
														   highValidAddress);
				_cardTableReconfigured = true;
				if (!result) {
					/* put tracepoint here */
...
MM_ConcurrentCardTable::newInstance(MM_EnvironmentBase *env, MM_Heap *heap, MM_MarkingScheme *markingScheme,
...
/* We only allocate TLH mark bits if scavenger is NOT active.
...
#endif /* OMR_GC_MODRON_SCAVENGER */
		{
			...
			MM_MemoryManager *memoryManager = _extensions->memoryManager;
			if (!memoryManager->createVirtualMemoryForMetadata(env, &_tlhMarkMapMemoryHandle, sizeof(uintptr_t),
			...
			(*mmPrivateHooks)
				->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_CACHE_CLEARED, tlhCleared, (void *)this);
			(*mmPrivateHooks)
...
MM_ConcurrentCardTable::initializeCardCleaning(MM_EnvironmentStandard *env)
{
	...
	if (_extensions->cardCleaningPasses > 0) {
		MM_AtomicOperations::lockCompareExchangeU32((volatile uint32_t *)&_cardCleanPhase, (uint32_t)_cardCleanPhase,
 ...
 */
MMINLINE bool
MM_ConcurrentCardTable::getExclusiveCardTableAccess(MM_EnvironmentStandard *env, CardCleanPhase currentPhase,
 ...
 */
bool
MM_ConcurrentCardTable::cleanCards(MM_EnvironmentStandard *env, bool isMutator, uintptr_t sizeToDo, uintptr_t *sizeDone,
								   bool threadAtSafePoint)
{
	Card *nextDirtyCard;
 ...
 */
bool
MM_ConcurrentCardTable::cleanSingleCard(MM_EnvironmentStandard *env, Card *card, uintptr_t bytesToClean,
 ...
 */
void
MM_ConcurrentCardTable::processTLHMarkBits(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace, void *tlhBase,
...
MM_ConcurrentCardTable::determineCleaningRanges(MM_EnvironmentStandard *env)
{
	...
	while (!initDone) {
		...
		if (numRanges > _maxCleaningRanges) {
			...
			_cleaningRanges = (CleaningRange *)env->getForge()->allocate(sizeRequired, MM_AllocationCategory::FIXED,
																		 OMR_GET_CALLSITE());
