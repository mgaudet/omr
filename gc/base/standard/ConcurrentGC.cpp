 ...
 */
void
J9ConcurrentWriteBarrierStore(OMR_VMThread* vmThread,
 ...
 */
void
J9ConcurrentWriteBarrierBatchStore(OMR_VMThread* vmThread,
                                   omrobjectptr_t destinationObject)
{
  MM_EnvironmentStandard* env =
    MM_EnvironmentStandard::getEnvironment(vmThread);
...
void
MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentStandard* env,
...
void
MM_ConcurrentGC::reportConcurrentFinalCardCleaningStart(
  MM_EnvironmentStandard* env)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(
...
void
MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentStandard* env,
                                               uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentCompleteTracingStart(
  MM_EnvironmentStandard* env)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START(
    _extensions->privateHookInterface, env->getOmrVMThread(),
    omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_COMPLETE_TRACING_START,
    _stats->getConcurrentWorkStackOverflowCount());
}

void
MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(MM_EnvironmentStandard* env,
                                                    uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentRememberedSetScanStart(
  MM_EnvironmentStandard* env)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void
MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(
  MM_EnvironmentStandard* env, uint64_t duration)
{
  OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END(
    _extensions->privateHookInterface, env->getOmrVMThread(),
    omrtime_hires_clock(), J9HOOK_MM_PRIVATE_CONCURRENT_REMEMBERED_SET_SCAN_END,
 ...
 */
void
MM_ConcurrentGC::hookObjectRemovedFromRememberedSet(J9HookInterface** hook,
                                                    uintptr_t eventNum,
                                                    void* eventData,
                                                    void* userData)
{
 ...
 */
void
MM_ConcurrentGC::hookCardCleanPass2Start(J9HookInterface** hook,
 ...
 */
void
MM_ConcurrentGC::signalThreadsToDirtyCardsAsyncEventHandler(
...
MM_ConcurrentGC::initialize(MM_EnvironmentBase* env)
{
  ...
  if (_extensions->optimizeConcurrentWB) {
    _callback =
      env->getExtensions()
        ->collectorLanguageInterface->concurrentGC_createSafepointCallback(env);
    if (NULL == _callback) {
      goto error_no_memory;
    }
  ...
  _cardCleaningFactorPass1 = interpolateInRange(
    INITIAL_CARD_CLEANING_FACTOR_PASS1_1, INITIAL_CARD_CLEANING_FACTOR_PASS1_8,
 ...
 */
MMINLINE float
MM_ConcurrentGC::interpolateInRange(float val1, float val8, float val10,
                                    uintptr_t traceRate)
{
  float ret;
 ...
 */
bool
MM_ConcurrentGC::getInitRange(MM_EnvironmentStandard* env, void** from,
...
MM_ConcurrentGC::conHelperEntryPoint(OMR_VMThread* omrThread, uintptr_t slaveID)
{
  ...
  while (CONCURRENT_HELPER_SHUTDOWN != request) {
    ...
    Assert_MM_true(0 != (vmThread->privateFlags &
    J9_PRIVATE_FLAGS_CONCURRENT_MARK_ACTIVE));
    */
    Assert_GC_true_with_message(
      env, CONCURRENT_OFF != _stats->getExecutionMode(),
 ...
 */
uintptr_t
MM_ConcurrentGC::calculateInitSize(MM_EnvironmentStandard* env,
 ...
 */
bool
MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentStandard* env,
                                        uintptr_t freeSize)
{
 ...
 */
MMINLINE uintptr_t
MM_ConcurrentGC::potentialFreeSpace(MM_EnvironmentStandard* env,
                                    MM_AllocateDescription* allocDescription)
{
...
#else
  nurseryPromotion =
    scavengerStats->_avgTenureBytes == 0 ? 1 : scavengerStats->_avgTenureBytes;
  currentOldFree = oldSubspace->getApproximateActiveFreeMemorySize();
#endif
 ...
 */
void
MM_ConcurrentGC::periodicalTuning(MM_EnvironmentStandard* env,
                                  uintptr_t freeSize)
{
 ...
 */
void
MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentStandard* env,
 ...
 */
void
MM_ConcurrentGC::concurrentMark(MM_EnvironmentStandard* env,
                                MM_MemorySubSpace* subspace,
                                MM_AllocateDescription* allocDescription)
{
  ...
  while (!taxPaid) {
    ...
    switch (executionMode) {
      ...
      case CONCURRENT_EXHAUSTED:
         ...
         * can trigger
         * the final collection here provided we are at a safe point. If not
         * then GC will be
      ...
      default:
        ...
        Assert_GC_true_with_message(
          ...
          "MM_ConcurrentStats::_executionMode = %zu\n", executionMode);
        nextExecutionMode =
          _cli->concurrentGC_getNextTracingMode(executionMode);
        if (_stats->switchExecutionMode(executionMode, nextExecutionMode)) {
          ...
          if (collectedRoots) {
             ...
             * found. Another
             * CONCURRENT_ROOT_TRACING thread may have beat us to it but if
             * thread
 ...
 */
void
MM_ConcurrentGC::concurrentSweep(MM_EnvironmentStandard* env,
                                 MM_MemorySubSpace* subspace,
                                 MM_AllocateDescription* allocDescription)
{
 ...
 */
bool
MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentStandard* env,
                                           MM_MemorySubSpace* subSpace)
{
 ...
 */
MMINLINE bool
MM_ConcurrentGC::cleanCards(MM_EnvironmentStandard* env, bool isMutator,
                                 ...
                                 void* lowValidAddress, void* highValidAddress)
{
  ...
  Trc_MM_ConcurrentGC_heapRemoveRange_Entry(
    envStandard->getLanguageVMThread(), subspace, size, lowAddress, highAddress,
 ...
 */
void
MM_ConcurrentGC::objectRemovedFromRememberedSet(MM_EnvironmentStandard* env,
                                                omrobjectptr_t objectPtr)
{
  if (objectPtr >= _heapBase && objectPtr < _heapAlloc &&
      _markingScheme->isMarkedOutline(objectPtr)) {
