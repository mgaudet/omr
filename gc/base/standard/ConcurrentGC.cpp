...
MM_ConcurrentGC::reportConcurrentHalted(MM_EnvironmentStandard *env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_HALTED(
		_extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
		J9HOOK_MM_PRIVATE_CONCURRENT_HALTED, (uintptr_t)_stats->getExecutionModeAtGC(), _stats->getTraceSizeTarget(),
		...
		(uintptr_t)cardTable->isCardCleaningComplete(), _cli->concurrentGC_reportConcurrentScanningMode(env),
...
MM_ConcurrentGC::reportConcurrentFinalCardCleaningStart(MM_EnvironmentStandard *env)
{
	...
	TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_START(
		_extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
		J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_CARD_CLEANING_START, _stats->getConcurrentWorkStackOverflowCount());
}
...
MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentStandard *env)
{
	...
	if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START)) {
		...
		ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_START(
			...
			cardTable->getCardTableStats()->getConcurrentCleanedCards(), _stats->getCardCleaningThreshold(),
			_stats->getConcurrentWorkStackOverflowOcurred(), _stats->getConcurrentWorkStackOverflowCount(),
			_stats->getThreadsToScanCount(), _stats->getThreadsScannedCount(), _stats->getCardCleaningReason());
	}
}
...
MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentStandard *env, uint64_t duration)
{
	...
	if (J9_EVENT_IS_HOOKED(_extensions->privateHookInterface, J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END)) {
		...
		ALWAYS_TRIGGER_J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END(
			_extensions->privateHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
			J9HOOK_MM_PRIVATE_CONCURRENT_COLLECTION_END, duration, env->getExclusiveAccessTime(), &commonData);
	}
}
 ...
 */
void
MM_ConcurrentGC::hookObjectRemovedFromRememberedSet(J9HookInterface **hook, uintptr_t eventNum, void *eventData,
													void *userData)
{
...
MM_ConcurrentGC::initialize(MM_EnvironmentBase *env)
{
	...
	_cardCleaningFactorPass1 =
		interpolateInRange(INITIAL_CARD_CLEANING_FACTOR_PASS1_1, INITIAL_CARD_CLEANING_FACTOR_PASS1_8,
	...
	_cardCleaningThresholdFactor =
		interpolateInRange(CARD_CLEANING_THRESHOLD_FACTOR_1, CARD_CLEANING_THRESHOLD_FACTOR_8,
	...
	if (_extensions->debugConcurrentMark) {
		OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
		omrtty_printf(
			"Initial tuning statistics: Card Cleaning Factors Pass1=\"%.3f\" Pass2=\"%.3f\" (Maximum: Pass1=\"%.3f\" "
			"Pass2=\"%.3f\")\n",
	...
	if (_extensions->largeObjectArea) {
		if (_extensions->concurrentMetering == MM_GCExtensionsBase::METER_DYNAMIC) {
			...
			_meteringHistory = (MeteringHistory *)env->getForge()->allocate(historySize, MM_AllocationCategory::FIXED,
																			OMR_GET_CALLSITE());
	...
	(*mmPrivateHooks)
		->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START, hookCardCleanPass2Start,
						 (void *)this);
 ...
 */
bool
MM_ConcurrentGC::getInitRange(MM_EnvironmentStandard *env, void **from, void **to, InitType *type,
							  bool *concurrentCollectable)
{
	...
	while (i < _numInitRanges) {
		...
		if (localFrom < _initRanges[i].top) {
			...
			if (localFrom
				== (uint8_t *)MM_AtomicOperations::lockCompareExchange((volatile uintptr_t *)&(_initRanges[i].current),
...
{
	_cli->detachVMThread(_extensions->getOmrVM(), omrThread,
	...
	if (0 == _stats->getKickoffThreshold() || _retuneAfterHeapResize) {
		...
		if ((NULL == env->_cycleState) || env->_cycleState->_gcCode.isExplicitGC() || !_globalCollectionInProgress) {
			...
			if (_secondCardCleanPass) {
				...
				_bytesToTracePass1 = (uintptr_t)MM_Math::weightedAverage((float)_bytesToTracePass1, (float)totalTraced,
																		 LIVE_PART_HISTORY_WEIGHT);
				_bytesToCleanPass1 = (uintptr_t)MM_Math::weightedAverage((float)_bytesToCleanPass1, (float)totalCleaned,
	...
	if (env->_cycleState->_gcCode.isExplicitGC() || (CONCURRENT_TRACE_ONLY > _stats->getExecutionModeAtGC())) {
		...
		*/
		...
		case CONCURRENT_TRACE_ONLY:
			_cardCleaningFactorPass1 = MM_Math::weightedAverage(_cardCleaningFactorPass1, _maxCardCleaningFactorPass1,
																CARD_CLEANING_HISTORY_WEIGHT);
			_cardCleaningFactorPass2 = MM_Math::weightedAverage(_cardCleaningFactorPass2, _maxCardCleaningFactorPass2,
																CARD_CLEANING_HISTORY_WEIGHT);
			_bytesTracedInPass1Factor = MM_Math::weightedAverage(_bytesTracedInPass1Factor, ALL_BYTES_TRACED_IN_PASS_1,
		...
		case CONCURRENT_FINAL_COLLECTION:
			...
			_cardCleaningFactorPass1 = MM_Math::weightedAverage(_cardCleaningFactorPass1, newCardCleaningFactorPass1,
																CARD_CLEANING_HISTORY_WEIGHT);
			_bytesTracedInPass1Factor = MM_Math::weightedAverage(_bytesTracedInPass1Factor, newBytesTracedInPass1Factor,
																 BYTES_TRACED_IN_PASS_1_HISTORY_WEIGHT);
			...
			break;
		default:
			Assert_GC_true_with_message(env, (CONCURRENT_ROOT_TRACING <= executionModeAtGC)
		...
		if (_extensions->debugConcurrentMark) {
			...
			omrtty_printf(
				"Update tuning statistics: Total Traced=\"%zu\" (Pass 2 KO=\"%zu\")  Total Cleaned=\"%zu\" (Pass 2 "
				"KO=\"%zu\")\n",
				totalTraced, _totalTracedAtPass2KO, totalCleaned, _totalCleanedAtPass2KO);
			omrtty_printf(
				"                          Tenure Live object Factor=\"%.3f\" Tenure non-leaf object factor=\"%.3f\" "
				"\n",
	...
	if (_lastFreeSize == LAST_FREE_SIZE_NEEDS_INITIALIZING) {
		...
		if (_conHelpersStarted > 0) {
			...
			_alloc2ConHelperTraceRate =
				MM_Math::weightedAverage(_alloc2ConHelperTraceRate, newConHelperRate, CONCURRENT_HELPER_HISTORY_WEIGHT);
 ...
 */
void
MM_ConcurrentGC::concurrentMark(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
								MM_AllocateDescription *allocDescription)
{
	...
	while (!taxPaid) {
		...
		case CONCURRENT_ROOT_TRACING:
			...
			Assert_GC_true_with_message(
				env, (CONCURRENT_ROOT_TRACING < nextExecutionMode) || (CONCURRENT_TRACE_ONLY == nextExecutionMode),
		...
		default:
			...
			Assert_GC_true_with_message(env, (CONCURRENT_ROOT_TRACING < executionMode)
 ...
 */
bool
MM_ConcurrentGC::forceKickoff(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
 ...
 */
void
MM_ConcurrentGC::concurrentSweep(MM_EnvironmentStandard *env, MM_MemorySubSpace *subspace,
								 MM_AllocateDescription *allocDescription)
{
 ...
 */
uintptr_t
MM_ConcurrentGC::doConcurrentTrace(MM_EnvironmentStandard *env, MM_AllocateDescription *allocDescription,
 ...
 */
MMINLINE bool
MM_ConcurrentGC::cleanCards(MM_EnvironmentStandard *env, bool isMutator, uintptr_t sizeToDo, uintptr_t *sizeDone,
 ...
 */
void
MM_ConcurrentGC::internalPreCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
									MM_AllocateDescription *allocDescription, uint32_t gcCode)
{
	MM_EnvironmentStandard *envStandard = MM_EnvironmentStandard::getEnvironment(env);
	OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
	...
	if (_extensions->isRememberedSetInOverflowState()
		...
		if (CONCURRENT_FINAL_COLLECTION > executionModeAtGC) {
			...
			if (!_markingScheme->getWorkPackets()->tracingExhausted()) {
				 ...
				 * final card cleaning.
				 */
				MM_ConcurrentCompleteTracingTask completeTracingTask(envStandard, _dispatcher, this,
																	 envStandard->_cycleState);
		...
		if (_extensions->scavengerEnabled) {
			...
			if (!(_extensions->rememberedSet.isEmpty())) {
				MM_ConcurrentScanRememberedSetTask scanRememberedSetTask(envStandard, _dispatcher, this,
																		 envStandard->_cycleState);
 ...
 */
bool
MM_ConcurrentGC::internalGarbageCollect(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
										MM_AllocateDescription *allocDescription)
{
	MM_EnvironmentStandard *envStandard = MM_EnvironmentStandard::getEnvironment(env);
	...
	masterThreadGarbageCollect(envStandard, static_cast<MM_AllocateDescription *>(allocDescription), _initializeMarkMap,
							   false);
 ...
 */
bool
MM_ConcurrentGC::heapAddRange(MM_EnvironmentBase *env, MM_MemorySubSpace *subspace, uintptr_t size, void *lowAddress,
							  void *highAddress)
{
	MM_EnvironmentStandard *envStandard = MM_EnvironmentStandard::getEnvironment(env);
								 ...
								 void *highAddress, void *lowValidAddress, void *highValidAddress)
{
	...
	Trc_MM_ConcurrentGC_heapRemoveRange_Entry(envStandard->getLanguageVMThread(), subspace, size, lowAddress,
	...
	bool result = MM_ParallelGlobalGC::heapRemoveRange(envStandard, subspace, size, lowAddress, highAddress,
													   lowValidAddress, highValidAddress);

	/* ...and then contract the card table */
	result = result
			 && ((MM_ConcurrentCardTable *)_cardTable)
					->heapRemoveRange(envStandard, subspace, size, lowAddress, highAddress, lowValidAddress,
									  highValidAddress);
...
MM_ConcurrentGC::completeTracing(MM_EnvironmentStandard *env)
{
	...
	while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
		bytesTraced += _markingScheme->scanObjectWithSize(env, objectPtr,
