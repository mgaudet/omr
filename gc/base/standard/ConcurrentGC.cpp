...
int J9THREAD_PROC con_helper_thread_proc(void* info)
{
    ...
    omrsig_handler_fn signalHandler
        = conHelperThreadInfo->collector->_cli->concurrentGC_getProtectedSignalHandler(&signalHandlerArg);
...
uintptr_t con_helper_thread_proc2(OMRPortLibrary* portLib, void* info)
{
    ...
    OMR_VMThread* omrThread = collector->_cli->attachVMThread(
        omrVM, "Concurrent Mark Helper", MM_CollectorLanguageInterfaceImpl::ATTACH_GC_HELPER_THREAD);
...
void MM_ConcurrentGC::reportConcurrentAborted(MM_EnvironmentStandard* env, CollectionAbortReason reason)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentFinalCardCleaningStart(MM_EnvironmentStandard* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentFinalCardCleaningEnd(MM_EnvironmentStandard* env, uint64_t duration)
{
    MM_ConcurrentCardTable* cardTable = (MM_ConcurrentCardTable*)_cardTable;
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentCollectionStart(MM_EnvironmentStandard* env)
{
    MM_ConcurrentCardTable* cardTable = (MM_ConcurrentCardTable*)_cardTable;
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentCollectionEnd(MM_EnvironmentStandard* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentBackgroundThreadActivated(MM_EnvironmentStandard* env)
{
...
void MM_ConcurrentGC::reportConcurrentBackgroundThreadFinished(MM_EnvironmentStandard* env, uintptr_t traceTotal)
{
...
void MM_ConcurrentGC::reportConcurrentCompleteTracingStart(MM_EnvironmentStandard* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentCompleteTracingEnd(MM_EnvironmentStandard* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentRememberedSetScanStart(MM_EnvironmentStandard* env)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
...
void MM_ConcurrentGC::reportConcurrentRememberedSetScanEnd(MM_EnvironmentStandard* env, uint64_t duration)
{
    OMRPORT_ACCESS_FROM_ENVIRONMENT(env);
 ...
 */
void MM_ConcurrentGC::hookObjectRemovedFromRememberedSet(
 ...
 */
void MM_ConcurrentGC::hookCardCleanPass2Start(
    J9HookInterface** hook, uintptr_t eventNum, void* eventData, void* userData)
{
 ...
 *
 */
void MM_ConcurrentGC::signalThreadsToDirtyCardsAsyncEventHandler(OMR_VMThread* omrVMThread, void* userData)
{
 ...
 */
MM_ConcurrentGC* MM_ConcurrentGC::newInstance(MM_EnvironmentBase* env, MM_CollectorLanguageInterface* cli)
{
    MM_ConcurrentGC* concurrentGC = (MM_ConcurrentGC*)env->getForge()->allocate(
        sizeof(MM_ConcurrentGC), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
bool MM_ConcurrentGC::initialize(MM_EnvironmentBase* env)
{
    ...
    if (_conHelperThreads > 0) {
        ...
        _conHelpersTable = (omrthread_t*)env->getForge()->allocate(
            _conHelperThreads * sizeof(omrthread_t), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
        if (!_conHelpersTable) {
            goto error_no_memory;
        }
    ...
    _allocToTraceRateMinFactor = (float)1 / interpolateInRange(MIN_ALLOC_2_TRACE_RATE_1, MIN_ALLOC_2_TRACE_RATE_8,
    ...
    (*mmPrivateHooks)
        ->J9HookRegister(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_CARD_CLEANING_PASS_2_START, hookCardCleanPass2Start, (void*)this);

#if defined(OMR_GC_MODRON_SCAVENGER)
    /* attach to the hooks for objects removed from the remembered set */
    (*mmPrivateHooks)
        ->J9HookRegister(mmPrivateHooks, J9HOOK_MM_PRIVATE_OBJECT_REMOVED_FROM_REMEMBERED_SET,
    ...
    while (!initDone) {
        ...
        if (_numInitRanges > _numPhysicalInitRanges) {
            ...
            _initRanges = (InitWorkItem*)env->getForge()->allocate(
                sizeof(InitWorkItem) * _numInitRanges, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
            if (NULL == _initRanges) {
                initDone = true;
 ...
 */
void MM_ConcurrentGC::resetInitRangesForConcurrentKO(MM_EnvironmentStandard* env)
{
    for (uint32_t i = 0; i < _numInitRanges; i++) {
        _initRanges[i].current = _initRanges[i].base;
    }
...
bool MM_ConcurrentGC::getInitRange(
    MM_EnvironmentStandard* env, void** from, void** to, InitType* type, bool* concurrentCollectable)
{
    uint32_t i;
...
MMINLINE MM_ConcurrentGC::ConHelperRequest MM_ConcurrentGC::switchConHelperRequest(
...
{
    _cli->detachVMThread(
        _extensions->getOmrVM(), omrThread, MM_CollectorLanguageInterfaceImpl::ATTACH_GC_HELPER_THREAD);
    omrthread_monitor_enter(_conHelpersActivationMonitor);
 ...
 */
uintptr_t MM_ConcurrentGC::calculateInitSize(MM_EnvironmentStandard* env, uintptr_t allocationSize)
{
 ...
 * @return the allocation tax
 */
uintptr_t MM_ConcurrentGC::calculateTraceSize(MM_EnvironmentStandard* env, MM_AllocateDescription* allocDescription)
{
    float thisTraceRate;
 ...
 */
bool MM_ConcurrentGC::periodicalTuningNeeded(MM_EnvironmentStandard* env, uintptr_t freeSize)
{
 ...
 * @param  freeSize the current amount of free space in the old area
 */
void MM_ConcurrentGC::periodicalTuning(MM_EnvironmentStandard* env, uintptr_t freeSize)
{
 ...
 *
 */
void MM_ConcurrentGC::kickoffCardCleaning(MM_EnvironmentStandard* env, ConcurrentCardCleaningReason reason)
{
...
void MM_ConcurrentGC::concurrentMark(
    MM_EnvironmentStandard* env, MM_MemorySubSpace* subspace, MM_AllocateDescription* allocDescription)
{
    ...
    while (!taxPaid) {
        ...
        case CONCURRENT_ROOT_TRACING:
            ...
            Assert_GC_true_with_message(env,
                ...
                "_cli->concurrentGC_getNextTracingMode(CONCURRENT_ROOT_TRACING) = %zu\n", nextExecutionMode);
        ...
        default:
            ...
            if (_stats->switchExecutionMode(executionMode, nextExecutionMode)) {
                Assert_GC_true_with_message2(env,
                    (CONCURRENT_ROOT_TRACING < nextExecutionMode) && (CONCURRENT_TRACE_ONLY >= nextExecutionMode),
                    "MM_ConcurrentStats::_executionMode = %zu; "
                    "_cli->concurrentGC_getNextTracingMode(MM_ConcurrentStats::_executionMode) = %zu\n",
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription, uint32_t gcCode)
{
    if (_extensions->concurrentKickoffEnabled
        && _cli->concurrentGC_forceConcurrentKickoff(env, gcCode, &_languageKickoffReason)) {
 ...
 */
bool MM_ConcurrentGC::timeToKickoffConcurrent(MM_EnvironmentStandard* env, MM_AllocateDescription* allocDescription)
{
 ...
 * Finish all concurrent sweep activities.
 */
void MM_ConcurrentGC::completeConcurrentSweepForKickoff(MM_EnvironmentStandard* env)
{
    /* If concurrent sweep is not enabled, do nothing */
    if (!_extensions->concurrentSweep) {
        return;
 ...
 */
uintptr_t MM_ConcurrentGC::doConcurrentInitialization(MM_EnvironmentStandard* env, uintptr_t initToDo)
{
 ...
 */
uintptr_t MM_ConcurrentGC::doConcurrentTrace(MM_EnvironmentStandard* env, MM_AllocateDescription* allocDescription,
                ...
                if (!((MM_ConcurrentCardTable*)_cardTable)->isCardCleaningComplete()) {
                    ...
                    if (!env->isThreadScanned()) {
                        ...
                        if (_markingScheme->getWorkPackets()->tracingExhausted()) {
                             ...
                             * be suspended and
                             * hanging onto packets and so preventing us from getting to CONCURENT_EXHAUSTED. This will
                             * delay
                             * the final collection; maybe until an AF which we don't want. We need to tell the
                             * concurrent helpers
                             * to release any packets they have so we can get them processed. This is easily achieved by
                             * suspending them.
                             * Anyway, now is also a good time to tell the concurrent helpers to park themselves for the
 ...
 */
bool MM_ConcurrentGC::concurrentFinalCollection(MM_EnvironmentStandard* env, MM_MemorySubSpace* subSpace)
{
 ...
 */
uintptr_t MM_ConcurrentGC::localMark(MM_EnvironmentStandard* env, uintptr_t sizeToTrace)
{
        ...
        if ((uintptr_t)objectPtr & PACKET_ARRAY_SPLIT_TAG) {
            ...
            */
            ...
            sizeTraced += _markingScheme->scanObjectWithSize(
                env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_PACKET, (sizeToTrace - sizeTraced));
        }
...
MMINLINE bool MM_ConcurrentGC::cleanCards(
    MM_EnvironmentStandard* env, bool isMutator, uintptr_t sizeToDo, uintptr_t* sizeDone, bool threadAtSafePoint)
{
 ...
 *
 */
void MM_ConcurrentGC::clearConcurrentWorkStackOverflow(MM_EnvironmentStandard* env)
{
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subSpace, MM_AllocateDescription* allocDescription)
{
    ...
    masterThreadGarbageCollect(
        envStandard, static_cast<MM_AllocateDescription*>(allocDescription), _initializeMarkMap, false);
     ...
     * collection, so we may miss to restore it.
     * It's too late to do it in postCollect(), as the AF will make us re-try allocate just after collection is done,
     * but postCollect() is invoked after the re-try.
     */
    ...
    MM_EnvironmentBase* env, MM_MemorySubSpace* subspace, uintptr_t size, void* lowAddress, void* highAddress)
{
    ...
    if (result) {
        ...
        result = ((MM_ConcurrentCardTable*)_cardTable)
                     ->heapAddRange(envStandard, subspace, size, lowAddress, highAddress, clearCards);
        if (!result) {
    ...
    void* lowAddress, void* highAddress, void* lowValidAddress, void* highValidAddress)
{
    ...
    Trc_MM_ConcurrentGC_heapRemoveRange_Entry(
        envStandard->getLanguageVMThread(), subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
    ...
    bool result = MM_ParallelGlobalGC::heapRemoveRange(
        envStandard, subspace, size, lowAddress, highAddress, lowValidAddress, highValidAddress);
...
void MM_ConcurrentGC::completeTracing(MM_EnvironmentStandard* env)
{
    ...
    while (NULL != (objectPtr = (omrobjectptr_t)env->_workStack.popNoWait(env))) {
        bytesTraced += _markingScheme->scanObjectWithSize(
            env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_PACKET, SCAN_MAX);
    }
            ...
            while ((slotPtr = (omrobjectptr_t*)rememberedSetSlotIterator.nextSlot()) != NULL) {
                ...
                if ((objectPtr >= _heapBase) && (objectPtr < _heapAlloc) && _markingScheme->isMarkedOutline(objectPtr)
                    ...
                    if (_extensions->dirtCardDuringRSScan) {
                        ...
                        bytesTraced += _markingScheme->scanObjectWithSize(
                            env, objectPtr, MM_CollectorLanguageInterface::SCAN_REASON_REMEMBERED_SET_SCAN, SCAN_MAX);
 ...
 */
void MM_ConcurrentGC::objectRemovedFromRememberedSet(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    if (objectPtr >= _heapBase && objectPtr < _heapAlloc && _markingScheme->isMarkedOutline(objectPtr)) {
        _cardTable->dirtyCard(env, objectPtr);
    }
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryBeforeGC(MM_EnvironmentStandard* env)
{
 ...
 * @see MM_ParallelGlobalGC::collectorShutdown()
 */
void MM_ConcurrentGC::updateMeteringHistoryAfterGC(MM_EnvironmentStandard* env)
{
    /* If LOA disabled or a system GC then nothing to do */
    if (!_extensions->largeObjectArea || env->_cycleState->_gcCode.isExplicitGC()) {
        return;
