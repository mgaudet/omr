...
void J9ConcurrentWriteBarrierStore(OMR_VMThread *vmThread, omrobjectptr_t destinationObject,
...
private:
	...
	uintptr_t doConcurrentTrace(MM_EnvironmentStandard *env, MM_AllocateDescription *allocDescription,
	...
	static void hookObjectRemovedFromRememberedSet(J9HookInterface **hook, uintptr_t eventNum, void *eventData,
												   void *userData);
...
public:
	...
	virtual void
	scanThread(MM_EnvironmentBase *env)
	{
	...
	*/
	MMINLINE bool
	isExclusiveAccessRequestWaitingSparseSample(MM_EnvironmentStandard *env, uintptr_t value)
	{
	 ...
	 */
	MMINLINE MM_ConcurrentCardTable *
	getCardTable()
	{
		return _cardTable;
	}
	...
	MM_ConcurrentGC(MM_EnvironmentBase *env, MM_CollectorLanguageInterface *cli)
		: MM_ParallelGlobalGC(env, cli), _cardTable(NULL), _heapBase(NULL), _heapAlloc(NULL), _rebuildInitWork(false),
...
#endif /* OMR_GC_LARGE_OBJECT_AREA */
		  ,
		  _conHelpersTable(NULL), _conHelperThreads((uint32_t)_extensions->concurrentBackground), _conHelpersStarted(0),
		  _conHelpersShutdownCount(0), _conHelpersActivationMonitor(NULL), _conHelpersRequest(CONCURRENT_HELPER_WAIT),
		  _globalCollectionInProgress(false), _initializeMarkMap(false), _initWorkMonitor(NULL),
		  _initWorkCompleteMonitor(NULL), _concurrentTuningMonitor(NULL), _initRanges(NULL), _numInitRanges(0),
		  _numPhysicalInitRanges(0), _nextInitRange(0), _initializers(0), _initSetupDone(false), _bytesToCleanPass1(0),
		  _bytesToCleanPass2(0), _bytesToTracePass1(0), _bytesToTracePass2(0),
		  _tenureLiveObjectFactor(INITIAL_OLD_AREA_LIVE_PART_FACTOR),
		  _tenureNonLeafObjectFactor(INITIAL_OLD_AREA_NON_LEAF_FACTOR), _tuningUpdateInterval(_minTraceSize),
		  _lastFreeSize(LAST_FREE_SIZE_NEEDS_INITIALIZING), _lastAverageAlloc2TraceRate(0),
		  _maxAverageAlloc2TraceRate(0), _lastTotalTraced(0), _lastConHelperTraceSizeCount(0),
		  _alloc2ConHelperTraceRate(0), _forcedKickoff(false), _languageKickoffReason(NO_LANGUAGE_KICKOFF_REASON),
		  _concurrentCycleState(), _callback(NULL), _stats(cli->concurrentGC_getConcurrentStats())
	{
