...
private:
	...
	volatile bool *const
		_forceExit; /**< Shared state concurrently updated by an external thread to force the receiver to cause all threads to yield (by setting the destination of the pointer to true) */
protected:
public:
...
public:
	...
	virtual void
	setup(MM_EnvironmentBase *env)
	{
		MM_ParallelScavengeTask::setup(env);
	}
	virtual void run(MM_EnvironmentBase *env);
	virtual void
	cleanup(MM_EnvironmentBase *env)
	{
		MM_ParallelScavengeTask::cleanup(env);
	}
	virtual bool
	shouldYieldFromTask(MM_EnvironmentBase *env)
	{
		return false;
	}

	MM_ConcurrentScavengeTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher, MM_Scavenger *scavenger,
							  ConcurrentAction action, uintptr_t bytesToScan, volatile bool *forceExit,
							  MM_CycleState *cycleState)
		: MM_ParallelScavengeTask(env, dispatcher, scavenger, cycleState), _bytesToScan(bytesToScan), _bytesScanned(0),
		  _forceExit(forceExit), _action(action)
	{
