...
class MM_ConcurrentScavengeTask : public MM_ParallelScavengeTask {
  /* Data Members */
private:
  UDATA const _bytesToScan; /**< The number of bytes that this must scan before
                               it will stop trying to do more work */
  UDATA volatile _bytesScanned; /**< The number of bytes scanned by this */
  volatile bool *const
      _forceExit; /**< Shared state concurrently updated by an external thread
                     to force the receiver to cause all threads to yield (by
                     setting the destination of the pointer to true) */
protected:
public:
...
protected:
public:
  virtual UDATA getVMStateID() { return J9VMSTATE_GC_CONCURRENT_SCAVENGER; }
  ...
  MM_ConcurrentScavengeTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher,
                            ...
                            uintptr_t bytesToScan, volatile bool *forceExit,
                            MM_CycleState *cycleState)
      : MM_ParallelScavengeTask(env, dispatcher, scavenger, cycleState),
        _bytesToScan(bytesToScan), _bytesScanned(0), _forceExit(forceExit),
        _action(action) {
