...
private:
    ...
    MM_ParallelSweepChunk*
        _currentInitChunk; /**< Last chunk initialized for concurrent sweep in the given memory pool (used for _next
                              pointer) */
...
void MM_ConcurrentSweepScheme::postConnectChunk(MM_EnvironmentBase* env, MM_ParallelSweepChunk* chunk)
{
    ...
    if (NULL != sweepState->_connectPreviousFreeEntry) {
         ...
         * with the last valid connection
         * being at the end of the current chunk.  The result is the 2nd chunk can be unwalkable.
         * The solution is to fix the heap by splitting the overlapping memory in the two chunks, and install a dead
         * object header (or slots)
         * at the beginning of the next chunk.  This ensures that a walk starting at the beginning of the next chunk
         * will be safe.
         */
        void* endFreeEntry
 ...
 * Connected))
 * which reduces to,
 *   (Estimated Free Memory) = ((Previous GC Free memory found) / (Previous GC Total Memory)) * (Total Memory to be
 * Connected)
 *
 */
        ...
        while (poolNum < poolCount && NULL != (memoryPool = mssPoolIterator.nextPool())) {
            ...
            if (env->_currentTask->synchronizeGCThreadsAndReleaseMaster(env, UNIQUE_ID)) {
                ...
                if (task->_foundLiveObject) {
                     ...
                     * swept chunk.
                     * If sweepState->_currentSweepChunkReverse is now pointing to NULL, then all the chunks have been
                     * swept;
    ...
    MM_EnvironmentBase* envModron, MM_ParallelSweepChunk* startingChunk, bool isFirstChunkInSubpool)
{
    ...
    do {
         ...
         * projection into the chunk.
         * This is a well defined starting point.  A walk is required to properly abandon any memory overlap as this
         * chunk may be the chunk
            ...
            if (NULL != currentChunk->leadingFreeCandidate) {
                ...
                if (((void*)(((UDATA)currentChunk->leadingFreeCandidate) + currentChunk->leadingFreeCandidateSize)
                    ...
                    if (isFirstChunkInSubpool && (currentChunk == startingChunk)) {
                         ...
                         * per comment in
                         * workThreadFindMinimumSizeFreeEntry.)  This must be a chunk before the chunk with the live
                         * object in it.  If there was
                         * no live object then this would have to be the first chunk in the subpool which would have
                         * been handle in the case above.
                     ...
                     * live object at the end of
                     * the chunk.  The live object can not overlap between chunks, otherwise there would have been a
                     * projection, which is handled
                     * above.  The next chunk will be walkable from the beginning of the chunk.  No walk needs to be
                 ...
                 * end of the chunk.  If the
                 * live object overlaps onto the next chunk there would have been a projection, which is handled above.
    ...
    MM_EnvironmentBase* envModron, MM_ParallelSweepChunk* currentChunk, void* walkStart)
{
    ...
    while (NULL != (currentObject = objectIterator.nextObjectNoAdvance())) {
         ...
         * abandon both halves.
         * In order to get here, the currentChunk must not have a projection, so we know that the currentObject would be
         * dead, and
