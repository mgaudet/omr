...
private:
	...
	UDATA
		_freeMemoryConnected; /**< Running count of the total free memory connected into the memory pool  since last GC */
...
public:
	 ...
	 */
	void
	kill(MM_EnvironmentBase *env, J9Pool *pool, omrthread_monitor_t mutex)
	{
		tearDown(env);
	 ...
	 */
	static void
	create(MM_EnvironmentBase *env, void *memPtr, MM_MemoryPool *memoryPool)
	{
	...
	MM_ConcurrentSweepPoolState(MM_MemoryPool *memoryPool)
		: MM_SweepPoolState(memoryPool), _sweepCount(0), _currentSweepChunk(NULL), _currentSweepChunkReverse(NULL),
		  _sweepChunkIteratorLock(), _connectCurrentChunk(NULL), _connectNextFreeEntry(NULL),
		  _connectNextFreeEntrySize(0), _currentInitChunk(NULL), _freeMemoryConnected(0),
		  _previousFreeRatio(PREVIOUS_FREE_RATIO_INITIAL),
		  _previousFreeHistoryWeight(PREVIOUS_FREE_HISTORY_WEIGHT_STARTUP), _heapSizeToConnect(0),
		  _heapSizeConnected(0), _finalFlushed(false)
	{
	}
...
public:
	...
	MM_ConcurrentSweepCompleteSweepTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher,
...
public:
	...
	MM_ConcurrentSweepFindMinimumSizeFreeTask(MM_EnvironmentBase *env, MM_Dispatcher *dispatcher,
		...
		: MM_ParallelSweepTask(env, dispatcher, concurrentSweepScheme), _memorySubSpace(memorySubSpace),
		  _minimumFreeSize(minimumFreeSize), _foundMinimumSizeFreeEntry(false), _foundLiveObject(false)
	{
	}
};
...
MM_ConcurrentSweepScheme::newInstance(MM_EnvironmentBase *env, MM_GlobalCollector *collector)
{
	...
	sweepScheme = (MM_ConcurrentSweepScheme *)env->getForge()->allocate(
		sizeof(MM_ConcurrentSweepScheme), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (sweepScheme) {
	...
	if (NULL != sweepState->_connectPreviousFreeEntry) {
		...
		if (endFreeEntry == chunk->chunkTop) {
			GC_ObjectHeapIteratorAddressOrderedList objectIterator(
				_extensions, (omrobjectptr_t)sweepState->_connectPreviousFreeEntry, (omrobjectptr_t)chunk->chunkTop,
				true);
 ...
 */
MMINLINE void
MM_ConcurrentSweepScheme::calculateApproximateFree(MM_EnvironmentBase *env, MM_MemoryPool *memoryPool,
												   MM_ConcurrentSweepPoolState *sweepState)
{
 ...
 */
void
MM_ConcurrentSweepScheme::payAllocationTax(MM_EnvironmentBase *env, MM_MemorySubSpace *subSpace,
 ...
 */
bool
MM_ConcurrentSweepScheme::sweepForMinimumSize(MM_EnvironmentBase *env, MM_MemorySubSpace *baseMemorySubSpace,
											  MM_AllocateDescription *allocateDescription)
{
	...
	while (NULL != (memorySubSpace = mssChildIterator.nextSubSpace())) {
		if (isConcurrentSweepCandidate(memorySubSpace)) {
			...
			MM_ConcurrentSweepFindMinimumSizeFreeTask findMinimumSizeFreeEntryTask(env, _dispatcher, this,
																				   memorySubSpace, minimumFreeSize);
 ...
 */
void
MM_ConcurrentSweepScheme::abandonOverlappedChunks(MM_EnvironmentBase *envModron, MM_ParallelSweepChunk *startingChunk,
...
MM_ConcurrentSweepScheme::walkChunkForOverlappingDeadSpace(MM_EnvironmentBase *envModron,
														   MM_ParallelSweepChunk *currentChunk, void *walkStart)
{
	CONSS_DEBUG_OMRPORT_ACCESS_FROM_OMRPORT(envModron->getPortLibrary());
