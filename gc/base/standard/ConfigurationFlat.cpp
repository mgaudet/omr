...
MM_Configuration* MM_ConfigurationFlat::newInstance(
    MM_EnvironmentBase* env, MM_ConfigurationLanguageInterface* configurationLanguageInterface)
{
    ...
    configuration = (MM_ConfigurationFlat*)env->getForge()->allocate(
        sizeof(MM_ConfigurationFlat), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    MM_EnvironmentBase* env, MM_Heap* heap, MM_InitializationParameters* parameters)
{
    ...
    if (NULL == (memorySubSpaceFlat = MM_MemorySubSpaceFlat::newInstance(env, physicalSubArena, memorySubSpaceGeneric,
                     true, parameters->_minimumSpaceSize, parameters->_initialOldSpaceSize,
