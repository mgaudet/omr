    ...
    MM_EnvironmentBase* env, MM_ConfigurationLanguageInterface* configurationLanguageInterface)
{
    ...
    configuration = (MM_ConfigurationGenerational*)env->getForge()->allocate(
        sizeof(MM_ConfigurationGenerational), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    MM_Scavenger* scavenger, MM_InitializationParameters* parameters, UDATA numaNode)
{
    ...
    /* allocate space */
    if (NULL == (memoryPoolAllocate
                    = MM_MemoryPoolAddressOrderedList::newInstance(env, minimumFreeEntrySize, "Allocate/Survivor1"))) {
        return NULL;
    }
    if (NULL == (memorySubSpaceGenericAllocate = MM_MemorySubSpaceGeneric::newInstance(env, memoryPoolAllocate, NULL,
    ...
    if (NULL == (memorySubSpaceGenericSurvivor = MM_MemorySubSpaceGeneric::newInstance(env, memoryPoolSurvivor, NULL,
                     false, parameters->_minimumNewSpaceSize / 2, parameters->_initialNewSpaceSize / 2,
                     parameters->_maximumNewSpaceSize, MEMORY_TYPE_NEW, 0))) {
    ...
    MM_EnvironmentBase* envBase, MM_Heap* heap, MM_InitializationParameters* parameters)
{
    ...
    if (NULL == (memorySubSpaceGenericOld = MM_MemorySubSpaceGeneric::newInstance(env, memoryPoolOld, NULL, false,
    ...
    if (NULL == (memorySubSpaceOld = MM_MemorySubSpaceFlat::newInstance(env, physicalSubArenaFlat,
    ...
    MM_EnvironmentBase* env, UDATA heapBytesRequested, MM_HeapRegionManager* regionManager)
{
    ...
    if (extensions->enableSplitHeap) {
         ...
         * require that we
         * set and "inverted" flag in the extensions and check that in the PSA attach code when determining attachment
         * policy.
...
bool MM_ConfigurationGenerational::initializeEnvironment(MM_EnvironmentBase* env)
{
