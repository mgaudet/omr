...
MM_Configuration*
MM_ConfigurationGenerational::newInstance(
  ...
  MM_ConfigurationLanguageInterface* configurationLanguageInterface)
{
  ...
  configuration = (MM_ConfigurationGenerational*)env->getForge()->allocate(
    sizeof(MM_ConfigurationGenerational), MM_AllocationCategory::FIXED,
    OMR_GET_CALLSITE());
  if (NULL != configuration) {
    new (configuration)
      MM_ConfigurationGenerational(env, configurationLanguageInterface);
  ...
  MM_InitializationParameters* parameters, UDATA numaNode)
{
  ...
  if (NULL ==
      (memorySubSpaceGenericAllocate = MM_MemorySubSpaceGeneric::newInstance(
         ...
         parameters->_initialNewSpaceSize / 2, parameters->_maximumNewSpaceSize,
         MEMORY_TYPE_NEW, 0))) {
  ...
  MM_InitializationParameters* parameters)
{
  ...
  if (NULL == (scavenger =
                 MM_Scavenger::newInstance(env, ext->collectorLanguageInterface,
                                           ext->heapRegionManager))) {
  ...
  MM_HeapRegionManager* regionManager)
{
  ...
  if (extensions->enableSplitHeap) {
     ...
     * the other order but that would require that we
     * set and "inverted" flag in the extensions and check that in the PSA
     * attach code when determining attachment policy.
