...
MM_CopyScanCacheList::initialize(MM_EnvironmentBase *env, volatile uintptr_t *cachedEntryCount)
{
	...
	_sublists = (struct CopyScanCacheSublist *)extensions->getForge()->allocate(
		sizeof(struct CopyScanCacheSublist) * _sublistCount, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (NULL == _sublists) {
		result = false;
	} else {
		memset(_sublists, 0, sizeof(struct CopyScanCacheSublist) * _sublistCount);
		for (uintptr_t i = 0; i < _sublistCount; i++) {
			if (!_sublists[i]._cacheLock.initialize(env, &extensions->lnrlOptions,
...
bool
MM_CopyScanCacheList::resizeCacheEntries(MM_EnvironmentBase *env, uintptr_t allocateCacheEntryCount,
...
MM_CopyScanCacheList::appendCacheEntriesInHeap(MM_EnvironmentStandard *env, MM_MemorySubSpace *memorySubSpace,
