...
MM_HeapRegionManagerStandard::MM_HeapRegionManagerStandard(
  MM_EnvironmentBase* env, uintptr_t regionSize, uintptr_t tableDescriptorSize,
  ...
  MM_RegionDescriptorDestructor regionDescriptorDestructor)
{
  MM_HeapRegionManagerStandard* regionManager =
    (MM_HeapRegionManagerStandard*)env->getForge()->allocate(
      sizeof(MM_HeapRegionManagerStandard), MM_AllocationCategory::FIXED,
      OMR_GET_CALLSITE());
  if (regionManager) {
  ...
  MM_GCExtensionsBase* extensions, MM_HeapMemorySnapshot* snapshot, bool gcEnd)
{
  ...
  if (extensions->scavengerEnabled) {
    ...
    snapshot->_totalNurserySurvivorSize =
      heap->getActiveSurvivorMemorySize(MEMORY_TYPE_NEW);
    snapshot->_freeNurserySurvivorSize =
      heap->getApproximateActiveFreeSurvivorMemorySize(MEMORY_TYPE_NEW);
  }
