...
protected:
    ...
    GC_IndexableObjectScanner(
        MM_EnvironmentStandard* env, omrobjectptr_t arrayPtr, fomrobject_t* basePtr, fomrobject_t* limitPtr, fomrobject_t* scanPtr, fomrobject_t* endPtr, uintptr_t flags)
        : GC_ObjectScanner(
              env, arrayPtr, scanPtr, ((endPtr - scanPtr) < _bitsPerScanMap) ? (((uintptr_t)1 << (endPtr - scanPtr)) - 1) : UDATA_MAX, setNoMoreSlots(flags | GC_ObjectScanner::indexableObject, (endPtr - scanPtr) <= _bitsPerScanMap))
        , _endPtr(endPtr)
        , _basePtr(basePtr)
        , _limitPtr(limitPtr)
    {
    ...
    GC_IndexableObjectScanner(MM_EnvironmentStandard* env, GC_IndexableObjectScanner* objectScanner, uintptr_t splitAmount)
        : GC_ObjectScanner(
              env, objectScanner->_parentObjectPtr, objectScanner->_endPtr, splitAmount < (uintptr_t)_bitsPerScanMap ? (((uintptr_t)1 << splitAmount) - 1) : UDATA_MAX, setNoMoreSlots(objectScanner->_flags, (intptr_t)splitAmount <= _bitsPerScanMap))
