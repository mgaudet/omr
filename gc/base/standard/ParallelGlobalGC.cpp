 ...
 * Initialization
 */
MM_ParallelGlobalGC* MM_ParallelGlobalGC::newInstance(MM_EnvironmentBase* env, MM_CollectorLanguageInterface* cli)
{
    MM_ParallelGlobalGC* globalGC;

    globalGC = (MM_ParallelGlobalGC*)env->getForge()->allocate(
        sizeof(MM_ParallelGlobalGC), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    if (globalGC) {
...
bool MM_ParallelGlobalGC::initialize(MM_EnvironmentBase* env)
{
    ...
    (*mmPrivateHooks)
        ->J9HookRegister(
            mmPrivateHooks, J9HOOK_MM_PRIVATE_ALLOCATION_FAILURE_CYCLE_START, globalGCHookAFCycleStart, NULL);
    (*mmPrivateHooks)
...
uintptr_t MM_ParallelGlobalGC::getVMStateID() { return J9VMSTATE_GC_COLLECTOR_GLOBALGC; }
...
void MM_ParallelGlobalGC::masterThreadReportObjectEvents(MM_EnvironmentBase* env)
{
