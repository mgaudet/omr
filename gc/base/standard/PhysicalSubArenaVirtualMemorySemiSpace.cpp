...
MM_PhysicalSubArenaVirtualMemorySemiSpace::newInstance(MM_EnvironmentBase *env, MM_Heap *heap)
{
	...
	subArena = (MM_PhysicalSubArenaVirtualMemorySemiSpace *)env->getForge()->allocate(
		sizeof(MM_PhysicalSubArenaVirtualMemorySemiSpace), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
	if (subArena) {
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate(MM_EnvironmentBase *env)
{
	...
	if (_parent->attachSubArena(env, this, _subSpace->getInitialSize(), attachPolicy)) {
		...
		if (NULL == (_highSemiSpaceRegion = getHeapRegionManager()->createAuxiliaryRegionDescriptor(
		...
		if ((_highSemiSpaceRegion->getLowAddress() != semiSpaceMiddle)
			...
			omrtty_printf(
				"!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad address range (%p,%p) for "
				"highSemiSpaceRegion, expected to be (%p, %p)\n",
				_highSemiSpaceRegion->getLowAddress(), _highSemiSpaceRegion->getHighAddress(), semiSpaceMiddle,
				_highAddress);
			Assert_MM_inflateInvalidRange();
		}
		if (NULL == (_lowSemiSpaceRegion = getHeapRegionManager()->createAuxiliaryRegionDescriptor(
		...
		if ((_lowSemiSpaceRegion->getLowAddress() != _lowAddress)
			...
			omrtty_printf(
				"!!! Fatal Error in MM_PhysicalSubArenaVirtualMemorySemiSpace::inflate - bad address range (%p,%p) for "
				"lowSemiSpaceRegion, expected to be (%p, %p)\n",
		...
		bool result =
			subSpaceAllocate->expanded(env, this, _lowSemiSpaceRegion->getSize(), _lowSemiSpaceRegion->getLowAddress(),
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expand(MM_EnvironmentBase *env, uintptr_t expandSize)
{
	...
	assume0(expandSize == MM_Math::roundToCeiling(MM_GCExtensions::getExtensions(env)->heapAlignment,
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::contract(MM_EnvironmentBase *env, uintptr_t contractSize)
{
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		if (allocateLeadingFreeSize > 0) {
			subSpaceAllocate->removeExistingMemory(env, this, allocateLeadingFreeSize, allocateLeadingFreeBase,
		...
		if (allocateSegmentBase > allocateLeadingFreeTop) {
			...
			MM_HeapWalker *heapWalker =
				env->getExtensions()->collectorLanguageInterface->parallelGlobalGC_getHeapWalker();
			heapWalker->allObjectSlotsDo(env, psavmssMoveFunction, (void *)&psavmssMoveData,
										 J9_MU_WALK_ALL | J9_MU_WALK_NEW_AND_REMEMBERED_ONLY, false, true);
		}
 ...
 */
void
MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(MM_EnvironmentBase *env, uintptr_t allocateSpaceSize,
												uintptr_t survivorSpaceSize)
{
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _lowSemiSpaceRegion, _lowSemiSpaceRegion->getLowAddress(),
													  expandTop);
	...
	subSpaceSurvivor->removeExistingMemory(env, this, ((uintptr_t)expandTop) - ((uintptr_t)expandBase), expandBase,
										   expandTop);
	subSpaceAllocate->addExistingMemory(env, this, ((uintptr_t)expandTop) - ((uintptr_t)expandBase), expandBase,
										expandTop, true);

	/* Set the new tilt ratio of the receiver */
	((MM_MemorySubSpaceSemiSpace *)_subSpace)
		->setSurvivorSpaceSizeRatio(survivorSpaceSize
									/ ((_highSemiSpaceRegion->getSize() + _lowSemiSpaceRegion->getSize()) / 100));
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::tilt(MM_EnvironmentBase *env, uintptr_t survivorSpaceSizeRequested)
{
	...
	minimumBytesFree =
		(uintptr_t)((_lowSemiSpaceRegion->getSize() + _highSemiSpaceRegion->getSize()) * survivorSpaceMinimumSizeRatio);
...
MM_PhysicalSubArenaVirtualMemorySemiSpace::expandNoCheck(MM_EnvironmentBase *env, uintptr_t expandSize)
{
	...
	if (subSpaceAllocate == _lowSemiSpaceRegion->getSubSpace()) {
		...
		getHeapRegionManager()->resizeAuxillaryRegion(env, _highSemiSpaceRegion, _lowSemiSpaceRegion->getHighAddress(),
													  _highSemiSpaceRegion->getHighAddress());
