...
uintptr_t MM_Scavenger::getVMStateID() { return J9VMSTATE_GC_COLLECTOR_SCAVENGER; }
 ...
 */
MM_Scavenger* MM_Scavenger::newInstance(
    MM_EnvironmentStandard* env, MM_CollectorLanguageInterface* cli, MM_HeapRegionManager* regionManager)
{
...
void MM_Scavenger::reportGCEnd(MM_EnvironmentStandard* env)
{
    ...
    Trc_MM_LocalGCEnd(env->getLanguageVMThread(), _extensions->scavengerStats._rememberedSetOverflow,
        _extensions->scavengerStats._causedRememberedSetOverflow, _extensions->scavengerStats._scanCacheOverflow,
        _extensions->scavengerStats._failedFlipCount, _extensions->scavengerStats._failedFlipBytes,
        _extensions->scavengerStats._failedTenureCount, _extensions->scavengerStats._failedTenureBytes,
        _extensions->scavengerStats._flipCount, _extensions->scavengerStats._flipBytes,
    ...
    Trc_OMRMM_LocalGCEnd(env->getOmrVMThread(), _extensions->scavengerStats._rememberedSetOverflow,
        _extensions->scavengerStats._causedRememberedSetOverflow, _extensions->scavengerStats._scanCacheOverflow,
        _extensions->scavengerStats._failedFlipCount, _extensions->scavengerStats._failedFlipBytes,
        _extensions->scavengerStats._failedTenureCount, _extensions->scavengerStats._failedTenureBytes,
        _extensions->scavengerStats._flipCount, _extensions->scavengerStats._flipBytes,
    ...
    TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_END(_extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
        ...
        _extensions->scavengerStats._gcCount, _extensions->scavengerStats._rememberedSetOverflow,
        _extensions->scavengerStats._causedRememberedSetOverflow, _extensions->scavengerStats._scanCacheOverflow,
        _extensions->scavengerStats._failedFlipCount, _extensions->scavengerStats._failedFlipBytes,
        _extensions->scavengerStats._failedTenureCount, _extensions->scavengerStats._failedTenureBytes,
        _extensions->scavengerStats._backout, _extensions->scavengerStats._flipCount,
        _extensions->scavengerStats._flipBytes, _extensions->scavengerStats._tenureAggregateCount,
        _extensions->scavengerStats._tenureAggregateBytes, _extensions->tiltedScavenge ? 1 : 0,
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_NEW),
        _extensions->heap->getActiveMemorySize(MEMORY_TYPE_NEW),
        _extensions->heap->getApproximateActiveFreeMemorySize(MEMORY_TYPE_OLD),
        ...
        _extensions->scavengerStats._tenureAge, _extensions->heap->getMemorySize());
}
 ...
 */
void MM_Scavenger::clearGCStats(MM_EnvironmentStandard* env) { _extensions->scavengerStats.clear(); }
...
MMINLINE uintptr_t MM_Scavenger::calculateOptimumCopyScanCacheSize(MM_EnvironmentStandard* env)
{
    ...
    uintptr_t scaleSize
        = (uintptr_t)(_extensions->copyScanRatio.getScalingFactor(env) * _extensions->scavengerScanCacheMaximumSize);
...
// it's valid for this atomic write to fail. it means another thread raced us with either fixing the slot (with same
// value, though),
...
omrobjectptr_t MM_Scavenger::copy(MM_EnvironmentStandard* env, MM_ForwardedHeader* forwardedHeader)
{
    ...
    _extensions->objectModel.calculateObjectDetailsForCopy(
        forwardedHeader, &objectCopySizeInBytes, &objectReserveSizeInBytes, &hotFieldsDescriptor);
...
/* now correct for the hot field alignment */
    ...
    if (0 != hotFieldsAlignment) {
         ...
         * pointer
         * and now we need to revert to the amount needed for the object allocation and its array alignment so the rest
         * of
...
/* Succeeded in forwarding the object - copy and adjust the age value */
         ...
         * we might have created a TLH remainder from previous cache just before reserving this space. This space
         * eventaully can create another remainder.
         * At that point, ideally (to recycle as much memory as possibly) we could enqueue this remainder, but as a
         * simple solution we will now abandon
         * the current remainder (we assert across the code, there is at most one at a give point of time).
         * If we see large amount of discards even with low discard threshold, we may reconsider enqueueing discarded
         * TLHs.
         */
        if (copyCache->flags & OMR_SCAVENGER_CACHE_TYPE_TENURESPACE) {
    ...
    uintptr_t startIndex, omrobjectptr_t* rememberedSetSlot)
{
    ...
    if (endIndex < maxIndex) {
        ...
        if (NULL != splitCache) {
            ...
            void* arrayTop
                = (void*)((uintptr_t)arrayPtr + _extensions->indexableObjectModel.getSizeInBytesWithHeader(arrayPtr));
        ...
        if (NULL != *copyCache) {
            if (NULL != hotFieldStats) {
                hotFieldStats->setHotnessOfField(
                    slotObject->readAddressFromSlot(), objectScanner->getHotFieldsDescriptor());
                hotFieldStats->updateStats(
                    isParentInNewSpace, isSlotObjectInNewSpace, slotObject->readReferenceFromSlot());
            }
            slotsCopied += 1;
        }
        slotsScanned += 1;
 ...
 */
MMINLINE void MM_Scavenger::incrementalScavengeObjectSlots(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr,
    MM_CopyScanCacheStandard* scanCache, MM_CopyScanCacheStandard** nextScanCache)
{
    ...
    if (!scanCache->_hasPartiallyScannedObject) {
        if (!scanCache->isSplitArray()) {
            ...
            objectScanner = _cli->scavenger_getObjectScanner(
                env, objectPtr, (void*)scanCache->getObjectScanner(), GC_ObjectScanner::scanHeap);
            if ((NULL == objectScanner) || objectScanner->isLeafObject()) {
                /* Object scanner will be NULL if object not scannable by cli (eg, empty pointer array, primitive array)
                 */
                if (NULL != objectScanner) {
    ...
    *nextScanCache = NULL;
    ...
    if (scanCache->_shouldBeRemembered) {
        if (NULL != scanCache->_arraySplitRememberedSlot) {
            Assert_MM_true(!isObjectInNewSpace(objectPtr));
            Assert_MM_true(_extensions->objectModel.isRemembered(objectPtr));
            Assert_MM_true(objectPtr == (omrobjectptr_t)((uintptr_t) * (scanCache->_arraySplitRememberedSlot)
...
MM_CopyScanCacheStandard* MM_Scavenger::getNextScanCache(MM_EnvironmentStandard* env)
{
    ...
    if (NULL != (cache = getDeferredCopyCache(env))) {
         ...
         * fitting and resuing remainder.
         * we want to delay scanning them as much as possible (up to the size of the original cache size being chopped
         * up),
...
bool MM_Scavenger::completeScan(MM_EnvironmentStandard* env)
{
    ...
    while ((env->_scanCache = getNextScanCache(env)) != NULL) {
        ...
        omrtty_printf("{SCAV: Completing scan (%p) %p-%p-%p}\n", env->_scanCache, env->_scanCache->cacheBase,
            env->_scanCache->cacheAlloc, env->_scanCache->cacheTop);
#endif /* OMR_SCAVENGER_TRACE */
     ...
     * raised the flag,
     * we ensure consistent behavior (return of backOutRaised flag) of all threads within this scan cycle, so that all
     * threads proceed
...
void MM_Scavenger::workThreadGarbageCollect(MM_EnvironmentStandard* env)
{
     ...
     * point,
     * otherwise it is hang situation when one group of threads might wait for more work in complete scan and another
     * group of threads
 ...
 ********************************************************************/
bool MM_Scavenger::walkObjectSlotsForRSO(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    ...
    GC_ObjectScanner* objectScanner
        = _cli->scavenger_getObjectScanner(env, objectPtr, &objectScannerState, GC_ObjectScanner::scanRoots);
                ...
                if (_extensions->objectModel.isRemembered(objectPtr)) {
                    ...
                    if (shouldBeRemembered) {
                        ...
                        TRIGGER_J9HOOK_MM_PRIVATE_OBJECT_REMOVED_FROM_REMEMBERED_SET(
                            _extensions->privateHookInterface, env->getOmrVMThread(), objectPtr);
                    }
                }
            }
        }
 ...
 */
MMINLINE bool MM_Scavenger::isWorkAvailableInCache(MM_CopyScanCacheStandard* cache)
{
...
MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::getDeferredCopyCache(MM_EnvironmentStandard* env)
{
    return env->_deferredCopyCache;
}

MMINLINE MM_CopyScanCacheStandard* MM_Scavenger::getSurvivorCopyCache(MM_EnvironmentStandard* env)
{
    MM_CopyScanCacheStandard* cache = NULL;
 ...
 */
bool MM_Scavenger::scavengeCompletedSuccessfully(MM_EnvironmentStandard* env) { return !backOutFlagRaised(); }
...
void MM_Scavenger::setBackOutFlag(MM_EnvironmentBase* env, bool value)
{
    ...
    if (!(_backOutFlag && value)) {
        ...
        _backOutFlag = value;
        /* Might be an overkill, but ensure that other CPUs see correct _backOutDoneIndex, by the time they see
         * _backOutFlag is set */
...
void MM_Scavenger::backOutObjectScan(MM_EnvironmentStandard* env, omrobjectptr_t objectPtr)
{
    ...
    GC_ObjectScannerState objectScannerState;
    GC_ObjectScanner* objectScanner
        = _cli->scavenger_getObjectScanner(env, objectPtr, (void*)&objectScannerState, GC_ObjectScanner::scanRoots);
    if (NULL != objectScanner) {
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
        OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
...
void MM_Scavenger::backoutFixupAndReverseForwardPointersInSurvivor(MM_EnvironmentStandard* env)
{
    ...
    while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
        if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
             ...
             * destroyed slot fixed up (but
             * the above loop must complete before we have the information with which to fixup the destroyed slot).
             * Fixing up a slot in dark
             * matter could crash, though, since the slot could point to contracted memory or could point to corrupted
             * data updated in a previous
             * backout.  The simple work-around for this problem is to check if the slot points at a readable part of
             * the heap (specifically,
             * tenure or survivor - the only locations which would require us to fix up the slot) and only read and
             * fixup the slot in those cases.
             * This means that we could still corrupt the slot but we will never crash during fixup and nobody else
             * should be trusting slots found
             * in dead objects.
             */
            GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(_extensions, rootRegion, false);
            omrobjectptr_t objectPtr = NULL;
...
/* Flip the memory space allocate profile */
...
#endif
            {
                _activeSubSpace->flip(env, 1);
                /* Let know MemorySpace about new default MemorySubSpace */
                _activeSubSpace->getMemorySpace()->setDefaultMemorySubSpace(
                    _activeSubSpace->getDefaultMemorySubSpace());
            }
 ...
 */
bool MM_Scavenger::percolateGarbageCollect(MM_EnvironmentBase* envModron, MM_MemorySubSpace* subSpace,
...
bool MM_Scavenger::scavengeInit(MM_EnvironmentBase* env, int64_t timeThreshold)
{
    GC_OMRVMThreadListIterator threadIterator(_extensions->getOmrVM());
    OMR_VMThread* walkThread = NULL;
...
bool MM_Scavenger::scavengeRoots(MM_EnvironmentBase* env)
{
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_ROOTS, U_64_MAX, NULL, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);

    return false;
}

bool MM_Scavenger::scavengeScan(MM_EnvironmentBase* env, int64_t timeThreshold)
{
    Assert_MM_true(concurrent_state_scan == _concurrentState);

    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN, U_64_MAX, NULL, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);
...
void MM_Scavenger::mutatorFinalReleaseCopyCaches(MM_EnvironmentBase* envBase, MM_EnvironmentBase* threadEnvironmentBase)
{
    MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
    MM_EnvironmentStandard* threadEnvironment = MM_EnvironmentStandard::getEnvironment(threadEnvironmentBase);

    if (MUTATOR_THREAD == threadEnvironment->getThreadType()) {
        /* in case of scavenge complete phase, master thread will act on behalf (use its own environment) of mutator
         * threads
...
bool MM_Scavenger::scavengeComplete(MM_EnvironmentBase* envBase)
{
    ...
    MM_ConcurrentScavengeTask scavengeTask(
        env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_COMPLETE, U_64_MAX, NULL, env->_cycleState);
    _dispatcher->run(env, &scavengeTask);

    return false;
}

bool MM_Scavenger::scavengeIncremental(MM_EnvironmentBase* env, int64_t scavengeIncrementEndTime)
{
    bool result = false;
    bool timeout = false;
...
void MM_Scavenger::workThreadScan(MM_EnvironmentStandard* env)
{
    ...
    // we probably have to clear all things for master since it'll be doing final release/clear on behalf of mutator
    // threads
...
uintptr_t MM_Scavenger::scavengeConcurrent(
    MM_EnvironmentBase* env, UDATA totalBytesToScavenge, volatile bool* forceExit)
{
    Assert_MM_true(concurrent_state_scan == _concurrentState);
    ...
    /* we can't assert the work queue is empty. some mutator threads could have just flushed their copy caches, after
     * the task terminated */
    _concurrentState = concurrent_state_complete;
...
uintptr_t MM_Scavenger::masterThreadConcurrentCollect(MM_EnvironmentBase* env)
{
     ...
     * _forceConcurrentTermination since the
     ...
     * setting the
     * flag to true if we want to interrupt it so that the master thread returns to the control mutex in order to
     * receive a
     * new GC request.
     */
