...
MM_Scavenger::reportGCEnd(MM_EnvironmentStandard* env)
{
  ...
  TRIGGER_J9HOOK_MM_OMR_LOCAL_GC_END(
    _extensions->omrHookInterface, env->getOmrVMThread(), omrtime_hires_clock(),
    J9HOOK_MM_OMR_LOCAL_GC_END, env->_cycleState->_activeSubSpace,
...
/* Succeeded in forwarding the object - copy and adjust the age value */
     ...
     * the current remainder (we assert across the code, there is at most one at
 ...
 * a range of objects. This may trigger
...
MM_Scavenger::updateCopyScanCounts(MM_EnvironmentBase* env,
                                   uint64_t slotsScanned, uint64_t slotsCopied)
{
 ...
 * as soon as there is a copy cache that is preferred to the current scan cache.
 * This is returned
 * in nextScanCache.
 *
...
MM_Scavenger::getNextScanCache(MM_EnvironmentStandard* env)
{
  ...
  if (NULL != (cache = getDeferredCopyCache(env))) {
     ...
     * chopped up due to large objects not fitting and resuing remainder.
     * we want to delay scanning them as much as possible (up to the size of the
...
/**
 * Scans all the objects to scan in the env->_scanCache, remembering objects as
 * required,
...
MM_Scavenger::completeScanCache(MM_EnvironmentStandard* env)
{
  ...
  if (scanCache->isSplitArray()) {
    ...
    bool shouldBeRemembered =
      scavengeObjectSlots(env, scanCache, objectPtr, GC_ObjectScanner::scanHeap,
                          scanCache->_arraySplitRememberedSlot);
  ...
  while (isWorkAvailableInCache(scanCache)) {
    ...
    while ((objectPtr = heapChunkIterator.nextObjectNoAdvance()) != NULL) {
      ...
      if (scanCache->_hasPartiallyScannedObject) {
         ...
         * if deferred cache is occupied, then queue current scan cache on scan
         * list
         */
        scanCache->flags &= ~OMR_SCAVENGER_CACHE_TYPE_SCAN;
...
MM_Scavenger::completeScan(MM_EnvironmentStandard* env)
{
  ...
  while ((env->_scanCache = getNextScanCache(env)) != NULL) {
    ...
    omrtty_printf("{SCAV: Completing scan (%p) %p-%p-%p}\n", env->_scanCache,
                  env->_scanCache->cacheBase, env->_scanCache->cacheAlloc,
   ...
   * we ensure consistent behavior (return of backOutRaised flag) of all threads
   * within this scan cycle, so that all threads proceed
 ...
 ********************************************************************/
bool
MM_Scavenger::walkObjectSlotsForRSO(MM_EnvironmentStandard* env,
                                    omrobjectptr_t objectPtr)
{
    ...
    while ((region = regionIterator.nextRegion()) != NULL) {
      ...
      while ((objectPtr = objectIterator.nextObject()) != NULL) {
        if (_extensions->objectModel.isRemembered(objectPtr)) {
          ...
          /* The remembered state of a class object also depends on the class
           * statics */
          if (_extensions->objectModel.hasIndirectObjectReferents(
                env->getLanguageVMThread(), objectPtr)) {
            shouldBeRemembered |=
              _cli->scavenger_hasIndirectReferentsInNewSpace(env, objectPtr);
          }
#if !defined(OMR_GC_CONCURRENT_SCAVENGER)
          ...
          /* Re-remember the object if necessary. This will add it to the list
           * if possible. */
          if (shouldBeRemembered) {
            addToRememberedSetFragment(env, objectPtr);
          } else {
    ...
    if (J9MODRON_HANDLE_NEXT_WORK_UNIT(env)) {
      ...
      while ((slotPtr = (omrobjectptr_t*)remSetSlotIterator.nextSlot()) !=
        ...
        if (NULL == objectPtr) {
          ...
          /* Inform interested parties that an object has been removed from the
           * remembered set */
          TRIGGER_J9HOOK_MM_PRIVATE_OBJECT_REMOVED_FROM_REMEMBERED_SET(
            _extensions->privateHookInterface, env->getOmrVMThread(),
            objectPtr);
...
MMINLINE void
MM_Scavenger::reinitCache(MM_CopyScanCacheStandard* cache, void* base,
                          void* top)
{
...
#if defined(OMR_SCAVENGER_TRACE_BACKOUT)
      OMRPORT_ACCESS_FROM_OMRPORT(env->getPortLibrary());
      omrtty_printf("{SCAV: Back out object in %p; slot[%p] %p->", objectPtr,
                    slotObject->readAddressFromSlot(),
                    slotObject->readReferenceFromSlot());
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
  ...
  MM_EnvironmentStandard* env)
{
    ...
    if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
      ...
      while ((objectPtr = evacuateHeapIterator.nextObjectNoAdvance()) != NULL) {
        ...
        if (header.isForwardedPointer()) {
           ...
           * This keeps tenure space walkable once the reverse forwarded objects
           * are abandoned.
           */
          ...
          omrtty_printf("{SCAV: Back out forward pointer %p@%p -> %p[%p]}\n",
                        objectPtr, fwdObjectPtr, freeHeader->getNext(),
                        freeHeader->getSize());
#endif /* OMR_SCAVENGER_TRACE_BACKOUT */
        }
      }
  ...
  while (NULL != (rootRegion = evacuateRegionIterator1.nextRegion())) {
    if (isObjectInEvacuateMemory((omrobjectptr_t)rootRegion->getLowAddress())) {
       ...
       * the above loop must complete before we have the information with which
       * to fixup the destroyed slot).  Fixing up a slot in dark
       ...
       * This means that we could still corrupt the slot but we will never crash
       * during fixup and nobody else should be trusting slots found
       * in dead objects.
       */
      GC_ObjectHeapIteratorAddressOrderedList evacuateHeapIterator(
        _extensions, rootRegion, false);
...
/* Flip the memory space allocate profile */
      ...
      if (_extensions->scvTenureStrategyAdaptive) {
        ...
        uintptr_t newSpaceConsumedSize =
          newSpaceTotalSize - _activeSubSpace->getActualActiveFreeMemorySize();
 ...
 */
bool
MM_Scavenger::heapAddRange(MM_EnvironmentBase* env, MM_MemorySubSpace* subspace,
 ...
 */
bool
MM_Scavenger::percolateGarbageCollect(MM_EnvironmentBase* envModron,
                                      MM_MemorySubSpace* subSpace,
                                      MM_AllocateDescription* allocDescription,
                                      ...
                                      uint32_t gcCode)
{
  ...
  /* Percolate the collect to parent MSS */
  bool result =
    subSpace->percolateGarbageCollect(envModron, allocDescription, gcCode);
...
MM_Scavenger::reportGCIncrementStart(MM_EnvironmentStandard* env)
{
  ...
  TRIGGER_J9HOOK_MM_PRIVATE_GC_INCREMENT_START(
    _extensions->privateHookInterface, env->getOmrVMThread(), stats->_startTime,
    J9HOOK_MM_PRIVATE_GC_INCREMENT_START, stats);
}
...
MM_Scavenger::calculateTenureMaskUsingLookback(double minimumSurvivalRate)
{
  ...
  for (uintptr_t age = 0; age <= OBJECT_HEADER_AGE_MAX + 1; ++age) {
     ...
     * now).
     * Also skip the last row in the history (there's no previous to compare it
     * to)
     */
     ...
     * back one collection.
     * If the generation would occupy at least a quarter of survivor space, look
     * back two collections.
     * If the generation would occupy at least an eighth of survivor space, look
     * back three collections.
     * . . .
     */
...
MM_Scavenger::scavengeRoots(MM_EnvironmentBase* env)
{
  ...
  MM_ConcurrentScavengeTask scavengeTask(
    env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_ROOTS, U_64_MAX,
...
MM_Scavenger::scavengeScan(MM_EnvironmentBase* env, int64_t timeThreshold)
{
  ...
  MM_ConcurrentScavengeTask scavengeTask(
    env, _dispatcher, this, MM_ConcurrentScavengeTask::SCAVENGE_SCAN, U_64_MAX,
    NULL, env->_cycleState);
  _dispatcher->run(env, &scavengeTask);
...
void
MM_Scavenger::mutatorFinalReleaseCopyCaches(
  MM_EnvironmentBase* envBase, MM_EnvironmentBase* threadEnvironmentBase)
{
  MM_EnvironmentStandard* env = MM_EnvironmentStandard::getEnvironment(envBase);
  MM_EnvironmentStandard* threadEnvironment =
  ...
  if (MUTATOR_THREAD == threadEnvironment->getThreadType()) {
    /* in case of scavenge complete phase, master thread will act on behalf (use
     * its own environment) of mutator threads
    ...
    if (threadEnvironment->_tenureCopyScanCache) {
      ...
      addCacheEntryToScanListAndNotify(env,
                                       threadEnvironment->_tenureCopyScanCache);
...
MM_Scavenger::workThreadScan(MM_EnvironmentStandard* env)
{
  ...
  // we probably have to clear all things for master since it'll be doing final
  // release/clear on behalf of mutator threads
...
uintptr_t
MM_Scavenger::scavengeConcurrent(MM_EnvironmentBase* env,
