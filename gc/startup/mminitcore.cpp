...
intptr_t initializeMutatorModel(OMR_VMThread* omrVMThread) {
  ...
  if (NULL != omrVMThread->_gcOmrVMThreadExtensions) {
    if (extensions->isStandardGC()) {
       ...
       * low and high addresses of the heap for which card
       * dirtying is required (the JIT uses this as a range check to determine
       * if it needs to dirty a card when writing into an
       * object).  Setting these for Tarok is just a work-around until a more
       * generic solution is implemented
       */
