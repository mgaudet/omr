...
omrobjectptr_t static inline allocHelper(OMR_VMThread *omrVMThread,
                                         size_t sizeInBytes, uintptr_t flags,
  ...
  uintptr_t sizeAdjusted =
      extensions->objectModel.adjustSizeInBytes(sizeInBytes);
  bool threadAtSafePoint =
      J9_ARE_ALL_BITS_SET(flags, OMR_GC_THREAD_AT_SAFEPOINT);
  MM_AllocateDescription allocdescription(sizeAdjusted, allocatedFlags,
                                          collectOnFailure, threadAtSafePoint);

  /* OMRTODO: Under what conditions could this assert fail? */
  assert(omrVMThread->memorySpace == env->getMemorySpace());

  omrobjectptr_t heapBytes =
      (omrobjectptr_t)env->_objectAllocationInterface->allocateObject(
  ...
  if (collectOnFailure) {
    ...
    /* OMRTODO: We are releasing exclusive before writing any of the object
     * header, meaning
     * it may not be walkable in the event of another GC or heap traversal. Is
     * this
...
omrobjectptr_t OMR_GC_Allocate(OMR_VMThread *omrVMThread, size_t sizeInBytes,
                               uintptr_t flags) {
