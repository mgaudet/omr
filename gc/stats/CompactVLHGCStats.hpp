...
public:
	...
	MMINLINE void
	addToMoveStallTime(uint64_t startTime, uint64_t endTime)
	{
		_moveStallTime += (endTime - startTime);
	}

	MMINLINE void
	addToRebuildStallTime(uint64_t startTime, uint64_t endTime)
	{
		_rebuildStallTime += (endTime - startTime);
	}

	void
	merge(MM_CompactVLHGCStats *statsToMerge)
	{
