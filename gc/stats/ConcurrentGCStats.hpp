...
class MM_ConcurrentGCStats : public MM_Base {
 ...
 public:
  ...
  MMINLINE bool switchExecutionMode(uintptr_t oldMode, uintptr_t newMode) {
    if (oldMode == MM_AtomicOperations::lockCompareExchange(&_executionMode,
                                                            oldMode, newMode)) {
#if defined(CONCURRENT_EXECUTION_MODE_DEBUG)
      fprintf(stderr,
              "+++ MM_ConcurrentGCStats::switchExecutionMode: %d -> %d\n",
...
#if defined(CONCURRENT_EXECUTION_MODE_DEBUG)
      fprintf(stderr,
              "--- MM_ConcurrentGCStats::switchExecutionMode: %d -> %d [%d]\n",
