    ...
    uintptr_t maxSizeClasses, uintptr_t veryLargeObjectThreshold, uintptr_t factorVeryLargeEntryPool, bool simulation)
{
    ...
    if (0 != _maxSizeClasses) {
        _count = (uintptr_t*)env->getForge()->allocate(
            sizeof(uintptr_t) * _maxSizeClasses, MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
    ...
    for (; (0 == unsatisfiedAlloc) && (currentFreeMemory > initialFreeMemory / 100)
        ...
        for (uintptr_t i = 0; (i < maxFrequentAllocateSizes) && (0 == unsatisfiedAlloc); i++) {
            ...
            if (objectPercent >= ignoreObjectPercentThreshold) {
                 ...
                 * other large object allocations)
                 * in this stride we will simulate allocation of 30 / 10 = 3% for the large object and  30/(30+10) x 60
                 * / 10 = 4.5% (tlhAllocBytesPerObjectPerObjectStride) for TLHs.
                 * TLHs allocations will be further divided into several strides (allocTLHStridesInteger), where each
                 * TLH allocation
    ...
    uintptr_t bytesAllocatedThisRound)
{
     ...
     * 1/10)
     * if bytesAllocatedThisRound = 9 x _averageBytesAllocated, the historic average will will carry 1/2 weight (new
     * weight is 1/2)
     * as bytesAllocatedThisRound / _averageBytesAllocated ratio approaches infinity, the historic average weight will
     * approach 0 (new weight approaches 1)
     * if _averageBytesAllocated = 9 x bytesAllocatedThisRound, the historic average weight will carry 99/100 weigth
     * (new weight is 1/100)
     * as bytesAllocatedThisRound / _averageBytesAllocated approaches 0, the historic average weight will approach 1
