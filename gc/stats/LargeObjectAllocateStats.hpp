...
private:
	...
	float *
		_fractionFrequentAllocation; /**< fraction Array for cumulating the fraction of frequentAllocation during estimating fragmentation */
...
public:
	...
	uintptr_t
	getCount(uintptr_t sizeClassIndex)
	{
		return _count[sizeClassIndex];
	}
	uintptr_t
	getVeryLargeEntrySizeClass()
	{
		return _veryLargeEntrySizeClass;
	}
	...
	FrequentAllocation *
	getFrequentAllocationHead(uintptr_t sizeClassIndex)
	{
	...
	bool initialize(MM_EnvironmentBase *env, uintptr_t maxAllocateSizes, uintptr_t maxSizeClasses,
					uintptr_t veryLargeObjectThreshold, uintptr_t factorVeryLargeEntryPool = 1,
	 ...
	 */
	void
	mergeLocked(MM_FreeEntrySizeClassStats *stats)
	{
		_lock.acquire();
		merge(stats);
		_lock.release();
	...
	MM_FreeEntrySizeClassStats()
		: _count(NULL), _frequentAllocationHead(NULL), _maxSizeClasses(0), _frequentAllocation(NULL),
		  _veryLargeEntryPool(NULL), _fractionFrequentAllocation(NULL), _maxFrequentAllocateSizes(0),
		  _maxVeryLargeEntrySizes(0), _veryLargeEntrySizeClass(0), _frequentAllocateSizeCounters(0)
	{
	}
};
...
private:
	 ...
	 */
	uintptr_t simulateAllocateObjects(MM_EnvironmentBase *env, uintptr_t totalAllocBytes, uintptr_t objectSize,
	 ...
	 * @param[out] currentFreeMemory free memory after allocate
	 */
	uintptr_t simulateAllocateTLHs(MM_EnvironmentBase *env, uintptr_t totalAllocBytes, uintptr_t *currentFreeMemory,
								   uintptr_t strides);
...
public:
	...
	static MM_LargeObjectAllocateStats *newInstance(MM_EnvironmentBase *env, uint16_t maxAllocateSizes,
													...
													float sizeClassRatio, uintptr_t maxHeapSize,
													uintptr_t tlhMaximumSize, uintptr_t tlhMinimumSize,
	...
	getMaxAllocateSizes()
	{
		return _maxAllocateSizes;
	}
	void
	setMaxAllocateSizes(uint16_t maxAllocateSizes)
	{
	...
	getSpaceSavingSizes()
	{
		return _spaceSavingSizes;
	}
	OMRSpaceSaving *
	getSpaceSavingSizeClasses()
	{
		return _spaceSavingSizeClasses;
	}
	OMRSpaceSaving *
	getSpaceSavingSizesAveragePercent()
	{
		return _spaceSavingSizesAveragePercent;
	}
	OMRSpaceSaving *
	getSpaceSavingSizeClassesAveragePercent()
	{
	...
	uintptr_t
	getTlhAllocSizeClassCount(uintptr_t sizeClassIndex)
	{
		return _tlhAllocSizeClassStats._count[sizeClassIndex];
	}
#endif
	uintptr_t
	getSizeClassSizes(uintptr_t sizeClassIndex)
	{
	...
	uint64_t
	getTimeEstimateFragmentation()
	{
		return _timeEstimateFragmentation;
	}
	uint64_t
	getCPUTimeEstimateFragmentation()
	{
	...
	getTimeMergeAverage()
	{
		return _timeMergeAverage;
	}
	void
	setTimeMergeAverage(uint64_t time)
	{
		_timeMergeAverage = time;
	}
	void
	addTimeMergeAverage(uint64_t time)
	{
		_timeMergeAverage += time;
	}
	uintptr_t
	getRemainingFreeMemoryAfterEstimate()
	{
		return _remainingFreeMemoryAfterEstimate;
	}
	void
	resetRemainingFreeMemoryAfterEstimate()
	{
		_remainingFreeMemoryAfterEstimate = 0;
	}
	uintptr_t
	getFreeMemoryBeforeEstimate()
	{
		return _freeMemoryBeforeEstimate;
	}
	uintptr_t
	getMaxHeapSize()
	{
...
#endif
		  _spaceSavingSizes(NULL), _spaceSavingSizeClasses(NULL), _spaceSavingSizesAveragePercent(NULL),
		  _spaceSavingSizeClassesAveragePercent(NULL), _spaceSavingTemp(NULL), _maxAllocateSizes(0),
		  _largeObjectThreshold(0), _veryLargeEntrySizeClass(0), _sizeClassRatio(0.0), _sizeClassRatioLog(0.0),
		  _averageBytesAllocated(0), _timeEstimateFragmentation(0), _cpuTimeEstimateFragmentation(0),
		  _timeMergeAverage(0), _remainingFreeMemoryAfterEstimate(0), _freeMemoryBeforeEstimate(0), _maxHeapSize(0),
		  _TLHSizeClassIndex(0), _TLHFrequentAllocationSize(0)
	{
	}
};
