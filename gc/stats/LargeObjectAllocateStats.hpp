...
private:
    ...
    FrequentAllocation**
        _frequentAllocationHead; /**< for each size class, there is a list of ascending (per size, not per count)
                                    frequent allocation (exact) sizes) */
    ...
    FrequentAllocation* _frequentAllocation; /**< an array of FrequentAllocation structs of size
                                                _maxFrequentAllocateSizes (current size _frequentAllocateSizeCounters)
                                                */
...
private:
     ...
     * will miss to count those that fit within TLH
     * This method extrapolate allocate stats to account even for those that fit within TLH. For given allocation size
     * and total bytes allocated out of line,
     * the method returns extrapolated total amount of bytes (including in-line and out-of-line) for that size.
     * The extrapolation logic relies on distribution of TLH sizes that have been created, and calculates probability
