...
public:
	...
	MM_ScavengerCopyScanRatio()
		: _accumulatingSamples(0), _accumulatedSamples(SCAVENGER_COUNTER_DEFAULT_ACCUMULATOR), _majorUpdateThreadEnv(0),
		  _scalingUpdateCount(0), _overflowCount(0), _resetTimestamp(0), _threadCount(0), _historyFoldingFactor(1),
	...
	getScalingFactor(MM_EnvironmentBase *env, UpdateHistory *historyRecord)
	{
		...
		return getScalingFactor(env, threads, historyRecord->waits, historyRecord->copied, historyRecord->scanned,
	 ...
	 */
	MMINLINE uintptr_t
	getScalingUpdateCount()
	{
		return _scalingUpdateCount;
	}
...
private:
	 ...
	 */
	MMINLINE double
	getScalingFactor(MM_EnvironmentBase *env, uint64_t threadCount, uint64_t waits, uint64_t copied, uint64_t scanned,
					 uint64_t updates)
	{
