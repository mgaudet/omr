...
public:
	 ...
	 */
	void
	mergeStats(MM_ScavengerHotFieldStats *statsToMerge)
	{
		for (uintptr_t i = 0; i < SizeScavengerHotness; i++) {
			for (uintptr_t j = 0; j < SizeScavengerObjectConnectorType; j++) {
	 ...
	 */
	void
	setHotnessOfField(fomrobject_t *slotPtr, uintptr_t hotFieldsDescriptor)
	{
	 ...
	 */
	void
	updateStats(bool isParentInNewSpace, bool isChildInNewSpace, omrobjectptr_t childPtr)
	{
		if (NULL == _objectPtr) {
