...
MM_ScavengerStats::MM_ScavengerStats()
	: _gcCount(UDATA_MAX), _rememberedSetOverflow(0), _causedRememberedSetOverflow(0), _scanCacheOverflow(0),
	  _scanCacheAllocationFromHeap(0), _scanCacheAllocationDurationDuringSavenger(0), _backout(0),
	  _failedTenureCount(0), _failedTenureBytes(0), _failedTenureLargest(0), _failedFlipCount(0), _failedFlipBytes(0),
	  _tenureAge(0), _startTime(0), _endTime(0)
#if defined(J9MODRON_TGC_PARALLEL_STATISTICS)
	  ,
	  _releaseScanListCount(0), _acquireFreeListCount(0), _releaseFreeListCount(0), _acquireScanListCount(0),
	  _acquireListLockCount(0), _aliasToCopyCacheCount(0), _arraySplitCount(0), _arraySplitAmount(0),
	  _workStallCount(0), _completeStallCount(0), _syncStallCount(0), _workStallTime(0), _completeStallTime(0),
	  _syncStallTime(0)
#endif /* J9MODRON_TGC_PARALLEL_STATISTICS */
	  ,
	  _avgInitialFree(0), _avgTenureBytes(0), _tiltRatio(0), _nextScavengeWillPercolate(false)
#if defined(OMR_GC_LARGE_OBJECT_AREA)
	  ,
	  _avgTenureLOABytes(0), _avgTenureSOABytes(0)
#endif /* OMR_GC_LARGE_OBJECT_AREA */
	  ,
	  _flipDiscardBytes(0), _tenureDiscardBytes(0), _survivorTLHRemainderCount(0), _tenureTLHRemainderCount(0),
	  _semiSpaceAllocBytesAcumulation(0), _tenureSpaceAllocBytesAcumulation(0), _semiSpaceAllocationCountLarge(0),
	  _semiSpaceAllocationCountSmall(0), _tenureSpaceAllocationCountLarge(0), _tenureSpaceAllocationCountSmall(0),
	  _tenureExpandedBytes(0), _tenureExpandedCount(0), _tenureExpandedTime(0), _leafObjectCount(0),
	  _copy_cachesize_sum(0), _slotsCopied(0), _slotsScanned(0)
