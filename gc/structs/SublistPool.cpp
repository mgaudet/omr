 ...
 * available
 * in the current sublist memory, allocate a new sublist puddle (until the
 * maximum sublist size is reached).
 *
...
MM_SublistPool::compact(MM_EnvironmentBase* env)
{
  ...
  while (currentPuddle) {
    ...
    if (!destinationPuddle) {
       ...
       * the lesser
       * number of allocated elements, and copy it into the other puddle.  It
       * may be this is
       * only a partial copy
       */
