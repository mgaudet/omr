...
MM_VerboseHandlerOutput::newInstance(MM_EnvironmentBase *env, MM_VerboseManager *manager)
{
	...
	MM_VerboseHandlerOutput *verboseHandlerOutput = (MM_VerboseHandlerOutput *)extensions->getForge()->allocate(
		sizeof(MM_VerboseHandlerOutput), MM_AllocationCategory::FIXED, OMR_GET_CALLSITE());
...
MM_VerboseHandlerOutput::MM_VerboseHandlerOutput(MM_GCExtensionsBase *extensions)
	: _extensions(extensions), _omrVM(NULL), _mmPrivateHooks(NULL), _mmOmrHooks(NULL), _manager(NULL){};
...
MM_VerboseHandlerOutput::enableVerbose()
{
	...
	(*_mmPrivateHooks)
		->J9HookRegister(_mmPrivateHooks, J9HOOK_MM_PRIVATE_HEAP_RESIZE, verboseHandlerHeapResize, (void *)this);
...
MM_VerboseHandlerOutput::getTagTemplate(char *buf, uintptr_t bufsize, uintptr_t id, const char *type,
										uintptr_t contextId, uint64_t wallTimeMs)
{
	OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
	uintptr_t bufPos = 0;
...
MM_VerboseHandlerOutput::handleCycleStart(J9HookInterface **hook, uintptr_t eventNum, void *eventData)
{
	...
	if (hasCycleStartInnerStanzas()) {
		writer->formatAndOutput(env, 0, "<cycle-start %s intervalms=\"%llu.%03llu\">", tagTemplate, deltaTime / 1000,
								deltaTime % 1000);
...
void
MM_VerboseHandlerOutput::handleCycleStartInnerStanzas(J9HookInterface **hook, uintptr_t eventNum, void *eventData,
...
void
MM_VerboseHandlerOutput::handleCycleEndInnerStanzas(J9HookInterface **hook, uintptr_t eventNum, void *eventData,
													uintptr_t indentDepth)
{
	/* do nothing */
}
...
MM_VerboseHandlerOutput::handleExclusiveStart(J9HookInterface **hook, uintptr_t eventNum, void *eventData)
{
	...
	writer->formatAndOutput(env, 0, "<exclusive-start %s intervalms=\"%llu.%03.3llu\">", tagTemplate, deltaTime / 1000,
							deltaTime % 1000);
...
MM_VerboseHandlerOutput::handleExclusiveEnd(J9HookInterface **hook, uintptr_t eventNum, void *eventData)
{
	...
	writer->formatAndOutput(env, 0, "<exclusive-end %s durationms=\"%llu.%03llu\" />", tagTemplate, deltaTime / 1000,
							deltaTime % 1000);
	writer->formatAndOutput(env, 0, "");
	writer->flush(env);
...
void
MM_VerboseHandlerOutput::handleAllocationFailureStartInnerStanzas(J9HookInterface **hook, uintptr_t eventNum,
																  void *eventData, uintptr_t indentDepth)
{
}
...
MM_VerboseHandlerOutput::handleAllocationFailureStart(J9HookInterface **hook, uintptr_t eventNum, void *eventData)
{
	...
	if (gc_policy_gencon == _extensions->configurationOptions._gcPolicy) {
		...
		writer->formatAndOutput(
			env, 0, "<af-start id=\"%zu\" threadId=\"%p\" totalBytesRequested=\"%zu\" %s intervalms=\"%llu.%03llu\" %s",
...
void
MM_VerboseHandlerOutput::handleAcquiredExclusiveToSatisfyAllocation(J9HookInterface **hook, uintptr_t eventNum,
																	void *eventData)
{
...
void
MM_VerboseHandlerOutput::outputMemoryInfoInnerStanza(MM_EnvironmentBase *env, uintptr_t indent,
...
MM_VerboseHandlerOutput::outputCollectorHeapResizeInfo(MM_EnvironmentBase *env, uintptr_t indent,
													   ...
													   uintptr_t resizeCount, uintptr_t subSpaceType, uintptr_t reason,
...
void
MM_VerboseHandlerOutput::outputStringConstantInfo(MM_EnvironmentBase *env, uintptr_t ident, uintptr_t candidates,
												  uintptr_t cleared)
{
	MM_VerboseWriterChain *writer = _manager->getWriterChain();

	if (0 != candidates) {
		writer->formatAndOutput(env, ident, "<stringconstants candidates=\"%zu\" cleared=\"%zu\"  />", candidates,
								cleared);
	}
}
