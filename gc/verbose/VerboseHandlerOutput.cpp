    ...
    uint64_t wallTimeMs)
{
    OMRPORT_ACCESS_FROM_OMRVM(_omrVM);
    uintptr_t bufPos = 0;
    bufPos += omrstr_printf(buf, bufsize, "id=\"%zu\" type=\"%s\" contextid=\"%zu\" durationms=\"%llu.%03.3llu\" "
...
void MM_VerboseHandlerOutput::handleExclusiveStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    writer->formatAndOutput(env, 1, "<response-info timems=\"%llu.%03.3llu\" idlems=\"%llu.%03.3llu\" threads=\"%zu\" "
                                    "lastid=\"%p\" lastname=\"%s\" />",
...
void MM_VerboseHandlerOutput::handleAllocationFailureStart(J9HookInterface** hook, uintptr_t eventNum, void* eventData)
{
    ...
    if (gc_policy_gencon == _extensions->configurationOptions._gcPolicy) {
        writer->formatAndOutput(env, 0, "<af-start id=\"%zu\" threadId=\"%p\" totalBytesRequested=\"%zu\" %s "
    ...
    uint64_t timeInMicroSeconds)
{
    ...
    writer->formatAndOutput(env, indent, "<heap-resize id=\"%zu\" type=\"%s\" space=\"%s\" amount=\"%zu\" "
