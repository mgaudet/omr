...
MM_VerboseWriter::initialize(MM_EnvironmentBase *env)
{
	...
	_header = (char *)ext->getForge()->allocate(sizeof(char) * headerLength, MM_AllocationCategory::DIAGNOSTIC,
												OMR_GET_CALLSITE());
	if (NULL == _header) {
		return false;
	}
	...
	_footer = (char *)ext->getForge()->allocate(sizeof(char) * footerLength, MM_AllocationCategory::DIAGNOSTIC,
												OMR_GET_CALLSITE());
	if (NULL == _footer) {
		ext->getForge()->free(_header);
		return false;
