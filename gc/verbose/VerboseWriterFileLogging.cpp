...
MM_VerboseWriterFileLogging::MM_VerboseWriterFileLogging(MM_EnvironmentBase *env, MM_VerboseManager *manager,
														 WriterType type)
	: MM_VerboseWriter(type), _filename(NULL), _mode(single_file), _currentFile(0), _currentCycle(0), _tokens(NULL),
	  _manager(manager)
{
	/* No implementation */
}
 ...
 */
bool
MM_VerboseWriterFileLogging::initialize(MM_EnvironmentBase *env, const char *filename, uintptr_t numFiles,
										uintptr_t numCycles)
{
...
MM_VerboseWriterFileLogging::initializeFilename(MM_EnvironmentBase *env, const char *filename)
{
	...
	if (_mode == rotating_files) {
		...
		_filename =
			(char *)extensions->getForge()->allocate(nameLen, MM_AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
		if (NULL == _filename) {
			return false;
		}
		...
		*write = '\0';
		...
		_filename = (char *)extensions->getForge()->allocate(strlen(filename) + 1, MM_AllocationCategory::DIAGNOSTIC,
															 OMR_GET_CALLSITE());
		if (NULL == _filename) {
			return false;
		}
...
MM_VerboseWriterFileLogging::expandFilename(MM_EnvironmentBase *env, uintptr_t currentFile)
{
	...
	char *filenameToOpen =
		(char *)extensions->getForge()->allocate(len, MM_AllocationCategory::DIAGNOSTIC, OMR_GET_CALLSITE());
 ...
 */
bool
MM_VerboseWriterFileLogging::reconfigure(MM_EnvironmentBase *env, const char *filename, uintptr_t numFiles,
										 uintptr_t numCycles)
{
	closeFile(env);
