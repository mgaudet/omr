...
typedef struct OMR_TI {
     ...
     * initialized.
     * 								return OMR_ERROR_RETRY on the first call or if the interval between two calls is too
     * small.
     * 								return OMR_ERROR_INTERNAL if the calculated CPU load is
     * invalid.
     * 								return OMR_ERROR_ILLEGAL_ARGUMENT if systemCpuLoad is
     * NULL.
     ...
     * 							  	return OMR_ERROR_INTERNAL if the calculated CPU load is
     * invalid.
     * 							  	return OMR_ERROR_ILLEGAL_ARGUMENT if processCpuLoad is
     * NULL.
     * 							  	return OMR_THREAD_NOT_ATTACHED if vmThread is NULL.
     */
     ...
     * address
      ...
      * OMR_ERROR_OUT_OF_MEMORY - Memory category data was truncated because category_buffer/max_categories was not
     * large enough
