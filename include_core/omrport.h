...
typedef struct J9PortVmemParams {
     ...
     *  		- If set, scan memory in a quick way, using memory information in file /proc/self/maps. (still use linear
     * search if failed)
     */
    uintptr_t options;
 ...
 * stdlib.h for a list.
 * Use J9STR_CODE_PLATFORM_OMR_INTERNAL to when processing the output of these calls.  Otherwise, use
...
typedef struct OMRPortLibrary {
    ...
    uint32_t (*sig_set_async_signal_handler)(
