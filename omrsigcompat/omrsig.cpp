...
#if defined(POSIX_SIGNAL)
int
omrsig_primary_sigaction(int signum, const struct sigaction* act,
                         struct sigaction* oldact)
{
...
int
omrsig_signalOS_internal(int signum, const struct sigaction* act,
                         struct sigaction* oldact)
{
  int rc = 0;
#if defined(J9ZOS390)
...
static int
omrsig_sigaction_internal(int signum, const struct sigaction* act,
                          struct sigaction* oldact, bool primary)
{
  int rc = 0;
 ...
 */
      ...
      if (handlerIsFunction(&sigData[signum].primaryAction)) {
        ...
        newRegisteringHandler.sa_flags |=
          sigData[signum].secondaryAction.sa_flags & SECONDARY_FLAGS_WHITELIST;
#endif /* defined(POSIX_SIGNAL) */
      } else {
...
int
__sigactionset(size_t newct, const __sigactionset_t newsets[], size_t* oldct,
               __sigactionset_t oldsets[], int options)
{
