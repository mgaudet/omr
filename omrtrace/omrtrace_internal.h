 ...
 * initialize
 * it's trace setup and to take trace points. These functions are usually
 * accessed via
 ...
 * (generally the
 * currently running thread). The trace point is in the module described by
 * modInfo and
 * has it's trace id masked with it's activation information. (This is generated
 * as part
 ...
 * const char *spec, ...);
 * and pass in the var args arguments (since the use of var args prevents simply
 * calling
 * the original UtModuleInterface.Trace function). This allows trace
 * implementors to
 * map from a per thread env parameter that is convenient for their application
 * to a
 ...
 * rastrace code
 *  outside of main.c
 *  All functions on the server interface (and only functions on the server
 * interface) start
 *  with trc **/
 ...
 * outside main.c
 *  All functions on the module interface (and only functions on the module
 * interface) start
 *  with j9 **/
