...
struct OMR_TraceGlobal {
	...
	omrthread_monitor_t
		bufferPoolLock; /* Lock for buffer pool. Do not allow tracepoints while locking, holding, or releasing this monitor. */
	J9Pool *threadPool; /* Pool for allocating all UtThreadData */
	omrthread_monitor_t
		threadPoolLock; /* Lock for thread pool. Do not allow tracepoints while locking, holding, or releasing this monitor. */
};
...
omr_error_t initialiseComponentData(UtComponentData **componentDataPtr, UtModuleInfo *moduleInfo,
...
omr_error_t setTracePointsToParsed(const char *componentName, UtComponentList *componentList, int32_t all,
								   int32_t first, int32_t last, unsigned char value, int level, const char *groupName,
								   BOOLEAN suppressMessages, int32_t setActive);
