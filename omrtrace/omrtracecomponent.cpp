...
omr_error_t initialiseComponentData(
...
omr_error_t processComponentDefferedConfig(UtComponentData* componentData, UtComponentList* componentList)
{
    ...
    if (componentList->deferredConfigInfoHead != NULL) {
        ...
        UT_DBGOUT(
            2, ("<UT> processComponentDefferedConfig: component %s - apply global deferred config info complete\n",
                   componentData->componentName));
    }
    ...
    const char* componentName, unsigned char value, int32_t setActive, BOOLEAN suppressMessages)
{
    ...
    while (*temp != '}' && *temp != '\0') {
        ...
        if (*temp == '-') {
            ...
            ret = 0;
            if (!isdigit(*temp)) {
                reportCommandLineError(suppressMessages, "Expecting tracepoint range specified as "
