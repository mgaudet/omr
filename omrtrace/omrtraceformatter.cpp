...
uint32_t
omr_trc_getBufferIteratorThreadName(UtTracePointIterator* iter, char* buffer,
                                    uint32_t buffLen)
{
...
static uint32_t
readConsumeAndSPrintfParameter(OMRPortLibrary* portLib, char* rawParameterData,
                               uint32_t rawParameterDataLength,
                               uint32_t* offsetInParameterData,
                               char* destBuffer, uint32_t destBufferLength,
                               uint32_t* offsetInDestBuffer, const char* format,
                               int32_t dataType, uint32_t nWidthAndPrecisions,
                               int32_t isBigEndian)
{
  uintptr_t temp = 0;
  uint64_t u64data = 0;
...
static uint32_t
formatTracePointParameters(UtTracePointIterator* iter, char* destBuffer,
                           uint32_t destBufferLength, const char* format,
...
static const char*
parseTracePoint(OMRPortLibrary* portLib, UtTraceRecord* record, uint32_t offset,
                int tpLength, uint64_t* timeStampMostSignificantBytes,
                UtTracePointIterator* iter, char* buffer, uint32_t bufferLength)
{
  uint32_t traceId;
  ...
  modName = &tempPtr[offset +
                     TRACEPOINT_RAW_DATA_MODULE_NAME_DATA_OFFSET]; /* the
                                                                      modName
                                                                      was
                                                                      written
                                                                      straight
  ...
  offsetOfParameters = (uint32_t)omrstr_printf(
    buffer, bufferLength, "%02u:%02u:%02u:%09u GMT %.*s.%u - ", hours, minutes,
...
const char*
omr_trc_formatNextTracePoint(UtTracePointIterator* iter, char* buffer,
  ...
  if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
    if (iter->isCircularBuffer) {
      ...
      returnString = parseTracePoint(
        iter->portLib, (UtTraceRecord*)iter->tempBuffForWrappedTP, 0, tpLength,
