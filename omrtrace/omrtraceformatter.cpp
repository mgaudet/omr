...
omr_trc_getTracePointIteratorForNextBuffer(UtTraceFileIterator *fileIterator, UtTracePointIterator **bufferIteratorPtr)
{
	...
	UT_DBGOUT_CHECKED(4, ("<UT> firstEntry: %d, offset of record: %ld buffer size: %d endianness %s\n", iterator->start,
						  offsetof(OMR_TraceBuffer, record), fileIterator->header->bufferSize,
						  (iterator->isBigEndian) ? "bigEndian" : "littleEndian"));
	UT_DBGOUT_CHECKED(2, ("<UT> omr_trc_getTracePointIteratorForNextBuffer: Thread %s returning iterator %p\n",
...
readConsumeAndSPrintfParameter(OMRPortLibrary *portLib, char *rawParameterData, uint32_t rawParameterDataLength,
							   uint32_t *offsetInParameterData, char *destBuffer, uint32_t destBufferLength,
							   uint32_t *offsetInDestBuffer, const char *format, int32_t dataType,
							   uint32_t nWidthAndPrecisions, int32_t isBigEndian)
{
	uintptr_t temp = 0;
	uint64_t u64data = 0;
	uint32_t u32data = 0;
	...
	if (dataType == UT_TRACE_FORMATTER_64BIT_DATA) {
		...
		if (nWidthAndPrecisions == 2) {
			...
			if (temp > destBufferLength) {
				...
				UT_DBGOUT_CHECKED(1, ("<UT> readConsumeAndSPrintfParameter destination buffer exhausted: %d [%s]\n",
									  dataType, format));
				return 0;
			}
		...
		*offsetInParameterData += 4;
		...
		temp =
			omrstr_printf(destBuffer + (*offsetInDestBuffer), destBufferLength - (*offsetInDestBuffer), format, u8data);
		*offsetInParameterData += 1;
		...
		if (nWidthAndPrecisions == 1) {
			*offsetInParameterData -= 4;
			i16data =
				(int16_t)getU_16FromBuffer((UtTraceRecord *)rawParameterData, *offsetInParameterData, isBigEndian);
						   ...
						   char *rawParameterData, uint32_t rawParameterDataLength)
{
	...
	if (destBuffer == NULL || destBufferLength == 0) {
		UT_DBGOUT_CHECKED(
			1, ("<UT> formatTracePointParameters called with destination buffer %p and destination buffer length %u\n",
		...
		if (format[offsetInFormat] == '%') {
			if (format[offsetInFormat + 1] == '%') {
				...
				case 'f':
					...
					break;
				default:
					UT_DBGOUT_CHECKED(1, ("<UT> formatTracePointParameters unknown trace format type: %c [%s]\n", type,
										  formatBuffer));
					break;
				}
				offsetInDestBuffer--;
			...
			if (offsetInDestBuffer > destBufferLength) {
				...
				UT_DBGOUT_CHECKED(1, ("<UT> formatTracePointParameters truncated output due to buffer exhaustion at "
									  "format type: %c [%s]\n",
									  type, formatBuffer));
...
parseTracePoint(OMRPortLibrary *portLib, UtTraceRecord *record, uint32_t offset, int tpLength,
				uint64_t *timeStampMostSignificantBytes, UtTracePointIterator *iter, char *buffer,
				uint32_t bufferLength)
{
	...
	if (offsetOfParameters > bufferLength) {
		UT_DBGOUT_CHECKED(1, ("<UT> parseTracePoint called with buffer length %d - too small for tracepoint details\n",
							  bufferLength));
		return NULL;
	}
...
omr_trc_formatNextTracePoint(UtTracePointIterator *iter, char *buffer, uint32_t bufferLength)
{
	...
	if ((tpLength > offset) || ((offset - tpLength + 1) < iter->start)) {
		if (iter->isCircularBuffer) {
			...
			UT_DBGOUT_CHECKED(4, ("<UT> getNextTracePointForIterator: remainder found at end of buffer: %d tplength = "
			...
			UT_DBGOUT_CHECKED(1, ("<UT> getNextTracePointForIterator: recombined a tracepoint into %s\n",
