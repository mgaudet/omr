 ...
 ******************************************************************************/
static void
copyToBuffer(OMR_TraceThread* thr, int bufferType, const char* var, char** p,
             int length, int* entryLength, OMR_TraceBuffer** trcBuf)
{
  int bufLeft =
 ...
 ******************************************************************************/
static void
tracePrint(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
           va_list var)
{
  int id;
  char* format;
  ...
  if (OMR_TRACEGLOBAL(indentPrint)) {
    ...
    omrtty_err_printf(
      "%02d:%02d:%02d.%03d%c" UT_POINTER_SPEC "%16s.%-6d %c %c ", hh, mm, ss,
      millis, threadSwitch, thr->id, qualifiedModuleName, id, excpt, entryexit);
    omrtty_err_vprintf(format + 2, var);
  }
  omrtty_err_printf("\n");
  freeTraceLock(thr);
 ...
 ******************************************************************************/
static void
traceAssertion(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
               va_list var)
{
  int id;
  const char* format;
 ...
 ******************************************************************************/
static void
traceV(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
       const char* spec, va_list var, int bufferType)
{
  OMR_TraceBuffer* trcBuf;
  int lastSequence;
  int entryLength;
 ...
 ******************************************************************************/
static void
trace(OMR_TraceThread* thr, UtModuleInfo* modinfo, uint32_t traceId,
...
static void
logTracePoint(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
 ...
 ******************************************************************************/
void
doTracePoint(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
             const char* spec, va_list varArgs)
{
 ...
 ******************************************************************************/
void
internalTrace(OMR_TraceThread* thr, UtModuleInfo* modInfo, uint32_t traceId,
              const char* spec, ...)
{
  va_list var;
