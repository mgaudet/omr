...
#endif                                      /* OMR_ALLOW_OUTPUT_OPTION */
  { UT_BUFFERS_KEYWORD, TRUE, setBuffers }, /* Not all buffers functions are
                                               exposed - but are controlled in
                                               the set function*/
 ...
 ******************************************************************************/
static omr_error_t
addTraceCmd(OMR_TraceThread* thr, const char* cmd, const char* value,
            BOOLEAN atRuntime)
{
 ...
 *************************************************************************/
omr_error_t
setSuspendResumeCount(OMR_TraceThread* thr, const char* value, int32_t resume,
                      BOOLEAN atRuntime)
{
  omr_error_t rc = OMR_ERROR_NONE;
  const char* p;
...
static omr_error_t
processSuspendCountOption(OMR_TraceThread* thr, const char* value,
                          BOOLEAN atRuntime)
{
    ...
    for (size_t j = 0; j < NUMBER_OF_UTE_OPTIONS; j++) {
      if (optNameLen == strlen(UTE_OPTIONS[j].name) &&
        ...
        if (atRuntime && !UTE_OPTIONS[j].runtimeModifiable) {
          reportCommandLineError(atRuntime, "Option \"%s\" cannot be set at "
                                            "run-time. Set it on the command "
                                            "line at start-up.",
                                 optName);
 ...
 ******************************************************************************/
static int
selectComponent(const char* cmd, int32_t* first, char traceType,
                int32_t setActive, BOOLEAN atRuntime)
{
  int length = 0;
  ...
  if (*cmd == '\0') {
    if (*first) {
       ...
       */
      rc =
        setTracePointsTo((const char*)UT_ALL, OMR_TRACEGLOBAL(componentList),
                         TRUE, 0, 0, traceType, -1, NULL, atRuntime, setActive);
      if (OMR_ERROR_NONE != rc) {
 ...
 *************************************************************************/
int
decimalString2Int(const char* decString, int32_t signedAllowed, omr_error_t* rc,
                  BOOLEAN atRuntime)
{
  ...
  if (OMR_ERROR_NONE == *rc) {
    ...
    if ((*p != ',') && (*p != '}') && (*p != '\0') && (*p != ' ')) {
      reportCommandLineError(
        atRuntime, "Invalid character(s) encountered in decimal number \"%s\".",
        decString);
      *rc = OMR_ERROR_INTERNAL;
    }
  }
