...
int32_t omrvmem_startup(struct OMRPortLibrary* portLibrary)
{
    ...
    if (num_psizes == -1) {
         ...
         * it may, some day.
         * We will use the earliest values in the set since they are the ones we should understand (if the OS adds new
         * sizes at the beginning of the list
    ...
    struct OMRPortLibrary* portLibrary, void* address, uintptr_t byteAmount, struct J9PortVmemIdentifier* identifier)
{
    ...
    if (rangeIsValid(identifier, address, byteAmount)) {
        if (0 == (identifier->mode & OMRPORT_VMEM_MEMORY_MODE_EXECUTE)) {
             ...
             * which may not return page aligned address and any attempt to align it to page boundary would waste one
             * page of memory.
             */
    ...
    struct OMRPortLibrary* portLibrary, struct J9PortVmemIdentifier* identifier, struct J9PortVmemParams* params)
{
     ...
     * memory), and
     * a 64-bit process can request at least 1 TB.  We will consider 1 TB to be an acceptable restriction, and not
 ...
 * detectAndRecordPageSize() (below) will determine if the page size we get is acceptable */
        ...
        if (0 == shmctl(addressKey, IPC_RMID, NULL)) {
            if (NULL != memoryPointer) {
                ...
                if (FALSE == detectAndRecordPageSize(portLibrary, identifier, addressKey, memoryPointer, category,
                                 pageSize, byteAmount, vmemOptions, mode)) {
    ...
    void* endAddress, uintptr_t alignmentInBytes, uintptr_t vmemOptions, uintptr_t mode)
{
     ...
     * explicitly use the mmap
     * allocator for default-sized pages (large pages still must use shmat).
     * The only reasons to use shmat, other than unification of the code paths, are to support large pages or NUMA.
     * NUMA is only supported on
     ...
     * This is
     * not supported for memory returned by default_pageSize_reserve_memory(), as it uses the mmap() API for historical
     * reasons.
     *
     ...
     * memory as
     * mmap (up to one segment is lost - 256M).  However, the NUMA API is not supported prior to AIX 6.1 so these
     * requirements
     * never appear together.  Thus, if NUMA is supported on the system, use shmem.  Otherwise, use mmap (as we will not
     * be trying
     * to use the NUMA API).
