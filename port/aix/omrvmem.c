...
int32_t
omrvmem_free_memory(struct OMRPortLibrary* portLibrary, void* userAddress,
                    uintptr_t byteAmount,
                    struct J9PortVmemIdentifier* identifier)
{
  int32_t result = 0;
...
reserveLargePages(struct OMRPortLibrary* portLibrary,
                  struct J9PortVmemIdentifier* identifier,
                  OMRMemCategory* category, uintptr_t byteAmount,
                  void* startAddress, void* endAddress, uintptr_t pageSize,
...
detectAndRecordPageSize(struct OMRPortLibrary* portLibrary,
                        struct J9PortVmemIdentifier* identifier,
                        key_t addressKey, void* memoryPointer,
                        OMRMemCategory* category, uintptr_t pageSize,
                        uintptr_t byteAmount, uintptr_t vmemOptions,
                        uintptr_t mode)
{
 ...
 */
static void*
getNextPossibleAddress(uintptr_t alignment, void* currentAddress,
                       intptr_t direction)
{
...
getMemoryInRangeForLargePages(struct OMRPortLibrary* portLibrary,
                              struct J9PortVmemIdentifier* identifier,
                              key_t addressKey, OMRMemCategory* category,
                              uintptr_t byteAmount, void* startAddress,
                              void* endAddress, uintptr_t alignmentInBytes,
                              uintptr_t vmemOptions)
{
  ...
  uintptr_t mode)
{
  ...
  allocAnywhere:
    memoryPointer =
      default_pageSize_reserve_memory(portLibrary, NULL, byteAmount, identifier,
                                      mode, PPG_vmem_pageSize[0], category);
  }
 ...
 */
static BOOLEAN
rangeIsValid(struct J9PortVmemIdentifier* identifier, void* address,
             uintptr_t byteAmount)
{
                                ...
                                uint64_t* memorySize)
{
  ...
  switch (queryType) {
    ...
    case OMRPORT_VMEM_PROCESS_VIRTUAL:
       ...
       * are
       * not marked MA_SHARED does not account for shared code pages when in
       * fact
       * command-line AIX utilities (such as svmon) do show that pages are
       * shared.
       */
