 ...
 */
void
j9nls_set_locale(struct OMRPortLibrary* portLibrary, const char* lang,
 ...
 */
static void
writeSyslog(struct OMRPortLibrary* portLibrary, uintptr_t flags,
            const char* format, va_list args)
{
 ...
 */
void
j9nls_printf(struct OMRPortLibrary* portLibrary, uintptr_t flags,
             uint32_t module_name, uint32_t message_num, ...)
{
  va_list args;
#if defined(NLS_DEBUG_TRACE)
 ...
 */
void
j9nls_vprintf(struct OMRPortLibrary* portLibrary, uintptr_t flags,
 ...
 */
void
j9nls_set_catalog(struct OMRPortLibrary* portLibrary, const char** paths,
...
static char*
build_catalog_name(struct OMRPortLibrary* portLibrary, int32_t usePath,
                   int32_t useDepth)
{
  uintptr_t len = 1;
  char* catalog = NULL;
...
static const char*
parse_catalog(struct OMRPortLibrary* portLibrary, uintptr_t flags,
              uint32_t module_name, uint32_t message_num,
              const char* default_string)
{
#define MSG_NONE 0
...
static const char*
nlsh_lookup(struct OMRPortLibrary* portLibrary, uint32_t module_name,
            uint32_t message_num)
{
...
nls_allocateHashEntry(struct OMRPortLibrary* portLibrary, uint32_t module_name,
                      uint32_t message_num, const char* message,
                      uint32_t sizeOfMessage)
{
...
char*
read_from_catalog(struct OMRPortLibrary* portLibrary, intptr_t fd, char* buf,
                  intptr_t bufsize)
{
  char temp[BUF_SIZE];
