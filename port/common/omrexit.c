...
void OMRNORETURN omrexit_shutdown_and_exit(struct OMRPortLibrary *portLibrary,
   ...
   * normally,
   * but we don't want to omit the cuda shutdown step because it resets the
   * devices
   * (if any were discovered) avoiding resource leaks that may lead to trouble
   * for
   * future processes. If CUDA support was never requested, this is effectively
   * a nop.
 ...
 * that were created by @ref omrexit_startup
