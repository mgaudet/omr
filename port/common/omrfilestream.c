 ...
 * be suitable for the
 * function fdopen. fdopen does not accept 'b', and does not truncate a file
 * when using 'w' or 'w+'.
 *
 ...
 * the filestream operations may be created here.  All resources created here
 ...
 * buffered.
 * Opening a filestream with EsOpenRead is not supported, and will return an
 * error code.
 *
 * @param[in] portLibrary The port library
 ...
 * referenced by the
 * filestream. It is not an error if not all bytes requested are written to the
 * file.
 *
 * @param[in] portLibrary The port library
...
omrfilestream_write(struct OMRPortLibrary* portLibrary,
                    OMRFileStream* fileStream, const void* buf, intptr_t nbytes)
{
  intptr_t rc = 0;
 ...
 * buffer is NULL, a buffer
 * is automatically allocated, and deallocated when the stream is closed.  If a
 * buffer is passed in,
 * it is the caller's responsibility to free this buffer after closing the
 * stream.
 ...
 * are ignored.
 * OMRPORT_FILESTREAM_LINE_BUFFERING has limited support, and is not guaranteed
 * that it will flush the buffer at
 ...
 * of the buffer
 * automatically allocated.  It must be greater than 0, unless mode is
 * OMRPORT_FILESTREAM_NO_BUFFERING.
 *
 * @return 0 on success, negative portable error code on failure.
 *
 ...
 */
int32_t
omrfilestream_setbuffer(OMRPortLibrary* portLibrary, OMRFileStream* fileStream,
