                      ...
                      va_list args)
{
  ...
  if (sizeof(outputBuffer) > (numberWritten + 1)) {
    ...
    portLibrary->filestream_write_text(portLibrary, fileStream, outputBuffer,
                                       numberWritten, J9STR_CODE_PLATFORM_RAW);
    Trc_PRT_filestream_vprintf_Exit();
    return;
  }
  ...
  numberWritten =
    portLibrary->str_vprintf(portLibrary, NULL, (U_32)(-1), format, copyOfArgs);
  numberWritten += 1;
  ...
  portLibrary->filestream_write_text(portLibrary, fileStream, allocatedBuffer,
                                     numberWritten, J9STR_CODE_PLATFORM_RAW);
 ...
 * and convert it into
 * the specified encoding. This makes it suitable for printing human readable
 * text to the console
 * or a file.
 *
                         ...
                         intptr_t nbytes, int32_t toEncoding)
{
   ...
   * the buffer.  At
   * this point, we want to return the number of characters consumed from the
   * input buffer, not the
 ...
 */
static intptr_t
write_all(struct OMRPortLibrary* portLibrary, OMRFileStream* fileStream,
          const char* buf, intptr_t nbytes)
{
  const char* from = buf;
