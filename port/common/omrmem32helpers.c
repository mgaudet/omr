 ...
 * amount of classes than usual.
 * For testing purposes, this value is mirrored in port library test. If we tune
 * this value, we should also adjust it in omrmemTest.cpp
 */
...
updatePPGHeapSizeInfo(struct OMRPortLibrary* portLibrary,
                      uintptr_t totalSizeDelta, BOOLEAN isIncrement)
{
  if (isIncrement) {
...
static void*
allocateLargeRegion(struct OMRPortLibrary* portLibrary, uintptr_t regionSize,
                    const char* callSite, uintptr_t vmemAllocOptions)
{
  ...
  if (NULL != allocPtr) {
     ...
     * omrmemtag.c. To prevent double-accounting,
                           ...
                           uintptr_t byteAmount)
{
   ...
   * to satisfy the requested byteAmount.
   * Then check the subCommitHeapWrapper whether it can grow or not.
   * If it can grow, meaning not all of the initially reserved memory is
   * committed,
   * then commit more memory from the reserved memory, grow the heap and try to
   * allocate byteAmount.
   * If the memory is allocated successfully after growing the heap, then return
   * the memory address of allocated memory,
 ...
 * suballocat byteAmount within that memory.
 * If the overhead of omrheap precludes using suballocation, omrheap will not be
 * used and the memory will be used directly instead.
 */
static void*
allocateRegion(struct OMRPortLibrary* portLibrary, uintptr_t regionSize,
               ...
               uintptr_t vmemAllocOptions)
{
   ...
   */
  alloc32Ptr =
    allocateVmemRegion32(portLibrary, roundedRegionSize, &heapWrapper, callSite,
                         OMRMEM_CATEGORY_PORT_LIBRARY_UNUSED_ALLOCATE32_REGIONS,
                         VMEM_MODE_COMMIT, vmemAllocOptions);
  if (NULL == alloc32Ptr) {
  ...
  if (NULL != subAllocPtr) {
     ...
     * suballocate byteAmount in the omrheap
     * cannot be satisfied due to omrheap management overhead and roudning
     * effect, we simply return the vmem allocated
 ...
 */
void*
allocate_memory32(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount,
                  const char* callSite)
{
  void* returnPtr = NULL;
 ...
 * search.
 * If the underlying vmem allocation is successful, allocate a J9HeapWrapper and
 * initialize its fields.
 */
static void*
allocateVmemRegion32(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount,
                     J9HeapWrapper** heapWrapper, const char* callSite,
                     uint32_t memoryCategory, uintptr_t vmemMode,
                     uintptr_t vmemAllocOptions)
{
  void* pointer = NULL;

/* If the port library does not support
 * OMR_PORT_CAPABILITY_CAN_RESERVE_SPECIFIC_ADDRESS capability
 * on this platform, allocateVmemRegion32 should return NULL, as omrvmem won't
 * be able to
 * satisfy the request for a specific address (it might get lucky, but we don't
...
findMatchingHeap(struct OMRPortLibrary* portLibrary, void* memoryPointer,
                 J9HeapWrapper*** heapWrapperLocation)
{
  J9HeapWrapper* currentHeapWrapper = NULL;
