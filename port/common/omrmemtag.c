...
static void*
wrapBlockAndSetTags(struct OMRPortLibrary* portLibrary, void* memoryPointer,
 ...
 */
void*
omrmem_allocate_memory(struct OMRPortLibrary* portLibrary, uintptr_t byteAmount,
                         ...
                         const char* callSite, uint32_t category)
{
  ...
  if (memoryPointer == NULL) {
    pointer = omrmem_allocate_memory(
      portLibrary, byteAmount, NULL == callSite ? OMR_GET_CALLSITE() : callSite,
      category);
