 ...
 * basis of the name (invalid
 *                              characters being converted to '_') of the file mapping object on Windows
 *                              so that it can be shared between processes.  If a named object is not required, this
 * parameter can be
 *                              specified as NULL
...
* not
 *                                                                     return until file updated.  Note that to achieve
* this on Windows requires the
 *                                                                     file to be opened with the
 ...
 *
* @return                                          0 on success, -1 on failure.  Errors will be reported using the usual
* port library mechanism
 */
 ...
 * marked
 * OMRPORT_PAGE_PROTECT_READ and omrmmap_protect is called with OMRPORT_PAGE_PROTECT_WRITE, it will no longer be
 * readable.
 *
 * @param[in] portLibrary the Port Library.
