 ...
 * \arg OMRPORT_SIG_SIGNAL_HANDLER -- when category = OMRPORT_SIG_SIGNAL, returns the address of the handler. Always
 * defined.
 * \arg OMRPORT_SIG_SIGNAL_ADDRESS -- when category = OMRPORT_SIG_SIGNAL, returns the address where the fault occurred,
 * if available
 * \arg OMRPORT_SIG_CONTROL_PC -- when category = OMRPORT_SIG_CONTROL, returns the PC value when the fault occurred, if
 * available
 * \arg OMRPORT_SIG_MODULE_NAME -- when category = OMRPORT_SIG_MODULE, returns the name of the module where the fault
 ...
 * \arg OMRPORT_SIG_VALUE_FLOAT_64 -- value is a (double*) and points to a floating point value. No assumptions should
 * be made about the validity of the floating point number.
 *
 ...
 *      OMRPORT_SIG_EXCEPTION_CONTINUE_SEARCH, indicating that the next exception handler in the chain should be
 *invoked,
 *      OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION, indicating that execution should be resumed using the information
 *stored in the info structure
 *      OMRPORT_SIG_EXCEPTION_RETURN, indicating that execution should continue as if the protected function had
 *returned 0.
 *
 * @param[in] portLibrary The port library
 ...
 *	Only the first handler may return OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION. Behaviour is undefined if a handler
 *returns
 ...
 * (omrsig_protect).
 * Asynchronous signals, such as SIGQUIT, are not suitable for structured exception handling, since they don't represent
 * an error condition, but instead
 * indicate that some external event has occurred. They are not associated with a specific thread or with a  section of
 * code.
 *
 ...
 * asynchronous signal
 * have completed. This has two consequences. Firstly, omrsig_set_async_signal_handler is a blocking operation. Do not
 * call it
 * while you hold a synchronization resource which might be required by a handler. Secondly, you may safely assume that
 * your
 * signal handler will never be invoked again once you have removed it. There is no possibility that the handler is
 * currently running,
 * or that it is just about to run. Once a handler is removed (by using flags=0) any resources it relies on may safely
 * be freed.
 *
 * @param[in] portLibrary The port library
 ...
 * @return 0 on success
 */
uint32_t omrsig_set_async_signal_handler(
 ...
 *	OMRPORT_SIG_FLAG_MAY_RETURN - indicates that the handler might return OMRPORT_SIG_EXCEPTION_RETURN, so a jmpbuf
 must be maintained in case that happens
 *	OMRPORT_SIG_FLAG_MAY_CONTINUE_EXECUTION - indicates that the handler might return
 OMRPORT_SIG_EXCEPTION_CONTINUE_EXECUTION

 *	Synchronous Signals: If the signal option OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS has been set we
 cannot protect against any synchronous signals.
 *		OMRPORT_SIG_FLAG_SIGSEGV - catch SIGSEGV
 *		OMRPORT_SIG_FLAG_SIGBUS - catch SIGBUS
 *		OMRPORT_SIG_FLAG_SIGILL - catch SIGILL
 *		OMRPORT_SIG_FLAG_SIGFPE - catch SIGFPE
