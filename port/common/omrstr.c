 ...
 */
uintptr_t
omrstr_printf(struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen,
              const char* format, ...)
{
  uintptr_t rc;
  va_list args;
...
#endif
int32_t
omrstr_convert(struct OMRPortLibrary* portLibrary, int32_t fromCode,
               int32_t toCode, uint8_t* inBuffer, uintptr_t inBufferSize,
               uint8_t* outBuffer, uintptr_t outBufferSize)
{
    ...
    case J9STR_CODE_MUTF8: {
      switch (toCode) {
        ...
        case J9STR_CODE_WIDE: {
           ...
           * to insufficient space in the output buffer.
 ...
 */
uintptr_t
omrstr_vprintf(struct OMRPortLibrary* portLibrary, char* buf, uintptr_t bufLen,
...
static void
readValues(struct OMRPortLibrary* portLibrary, J9FormatData* result,
           va_list args)
{
  uint8_t index;
...
static uintptr_t
writeSpec(J9FormatData* data, J9FormatSpecifier* spec, char* result,
          uintptr_t length)
{
 ...
 */
static uintptr_t
writeIntToBuffer(char* buf, uintptr_t bufLen, uint64_t width,
                 uint64_t precision, uint64_t value, uint8_t tag, int isSigned,
                 const char* digits)
{
  uint32_t index = 0;
  uint32_t length = 0;
  ...
  int32_t actualPrecision =
    0; /* precision is specified unsigned, but code may decrement temp values
          below zero */
  char signChar = 0;
...
static uintptr_t
writeStringToBuffer(char* buf, uintptr_t bufLen, uint64_t width,
                    uint64_t precision, const char* value, uint8_t tag)
{
 ...
 */
static uintptr_t
writeFormattedString(struct OMRPortLibrary* portLibrary, J9FormatData* data,
                     char* result, uintptr_t length)
{
...
static uintptr_t
writeDoubleToBuffer(char* buf, uintptr_t bufLen, uint64_t width,
                    uint64_t precision, double value, uint8_t type, uint8_t tag)
{
...
static uintptr_t
writeUnicodeStringToBuffer(char* buf, uintptr_t bufLen, uint64_t width,
                           uint64_t precision, const uint16_t* value,
                           uint8_t tag)
{
                 ...
                 const char* format, ...)
{
  ...
  tokenBufLen =
    portLibrary->str_vprintf(portLibrary, NULL, (unsigned int)-1, format, args);
  va_end(args);
 ...
 */
uintptr_t
omrstr_subst_tokens(struct OMRPortLibrary* portLibrary, char* buf,
 ...
 */
uint32_t
omrstr_ftime(struct OMRPortLibrary* portLibrary, char* buf, uint32_t bufLen,
             const char* format, int64_t timeMillisUTC)
{
  if (buf && bufLen > 0) {
 ...
 */
static uint32_t
omrstr_subst_time(struct OMRPortLibrary* portLibrary, char* buf,
                  uint32_t bufLen, const char* format, int64_t timeMillis)
{
...
/* set up buffers and convertors */
  ...
  while ((platformRemaining > 0) &&
    ...
    int32_t wideBufferPartialSize = convertPlatformToWide(
      portLibrary, encodingState, codePage, &platformCursor, &platformRemaining,
...
*/
static int32_t
convertWideToMutf8(const uint8_t** inBuffer, uintptr_t* inBufferSize,
                   uint8_t* outBuffer, uintptr_t outBufferSize)
{
 ...
 */
static BOOLEAN
checkAndCopyUtfBytes(const uint8_t* utf8Bytes, uintptr_t numBytes,
                     uint8_t* mutf8Bytes)
{
  uintptr_t cursor;
...
*/
static int32_t
convertUtf8ToMutf8(struct OMRPortLibrary* portLibrary, const uint8_t** inBuffer,
                   uintptr_t* inBufferSize, uint8_t* outBuffer,
                   uintptr_t outBufferSize)
{
...
*/
static int32_t
convertMutf8ToWide(const uint8_t** inBuffer, uintptr_t* inBufferSize,
                   uint8_t* outBuffer, uintptr_t outBufferSize)
{
  uintptr_t mutf8Remaining =
                      ...
                      uint8_t* outBuffer, uintptr_t outBufferSize)
{
  ...
  if ((outBufferSize > 0) && (0 == mbChars)) {
    resultSize = OMRPORT_ERROR_STRING_BUFFER_TOO_SMALL; /* should not happen:
                                                           caller should have
                                                           allocated a
                                                           sufficiently large
                                                           buffer */
...
convertWideToPlatform(struct OMRPortLibrary* portLibrary,
                      charconvState_t encodingState, const uint8_t** inBuffer,
                      uintptr_t* inBufferSize, uint8_t* outBuffer,
                      uintptr_t outBufferSize)
{
  int32_t resultSize = -1;
  ...
  resultSize = WideCharToMultiByte(
    OS_ENCODING_CODE_PAGE, OS_ENCODING_MB_FLAGS, wideCursor, (int)wideRemaining,
