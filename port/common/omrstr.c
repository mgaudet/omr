 ...
 * that
 * the implementation will not do any timezone correction and assumes that this value is in terms of the required
 * timezone.
 *
...
/* set up buffers and convertors */
    ...
    requiredBufferSize = WIDE_CHAR_SIZE * MultiByteToWideChar(codePage, OS_ENCODING_MB_FLAGS, inBuffer,
                                              (int)inBufferSize, NULL, 0); /* get required buffer size */
    ...
    uint32_t codePage, const uint8_t** inBuffer, uintptr_t* inBufferSize, uint8_t* outBuffer, uintptr_t outBufferSize)
{
    ...
    int32_t mbChars = (int32_t)MultiByteToWideChar(
        codePage, OS_ENCODING_MB_FLAGS, *inBuffer, (int)*inBufferSize, (LPWSTR)outBuffer, (int)outBufferSize);
