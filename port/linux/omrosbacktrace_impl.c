 ...
 */
static uintptr_t
handler(struct OMRPortLibrary* portLibrary, uint32_t gpType, void* gpInfo,
        void* userData)
{
 ...
 * causes a segfault then we check the output
 * array to see if we got any frames as we may be able to provide a partial
 * backtrace. We also record that the stack walk
...
backtrace_sigprotect(struct OMRPortLibrary* portLibrary,
                     J9PlatformThread* threadInfo, void** address_array,
                     int capacity)
{
  uintptr_t ret;
  ...
  if (omrthread_self()) {
    if (portLibrary->sig_protect(portLibrary, protectedBacktrace, &args,
                                 handler, NULL, OMRPORT_SIG_FLAG_SIGALLSYNC |
