...
static uintptr_t getSharedAndPrivateDataSegments(struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize,
...
static void copyDataIntoELFCore(struct OMRPortLibrary *portLibrary, intptr_t fd_core, uintptr_t addrSize,
								Elf64_Off phoff);
static intptr_t getContentsFromProcFileSystem(struct OMRPortLibrary *portLibrary, const char *fileName, char *buffer,
											  size_t buflen);
...
static intptr_t deriveCoreFileName(struct OMRPortLibrary *portLibrary, char *corePatterFormat, BOOLEAN coreUsesPID,
								   char *baseDir, char *derivedCoreFileName, uintptr_t derivedCoreFileNameLen,
								   pid_t pid, int signalNumber);

#ifdef DUMP_DBG
static void printMemoryMap(struct OMRPortLibrary *portLibrary, uintptr_t addrSize);
static void printProgramHeader(struct OMRPortLibrary *portLibrary, uintptr_t addrSize, void *pelfPgmHdr,
							   uintptr_t count);
static void printProgramHeaders(struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize, Elf64_Off phoff,
								Elf64_Half phnum);
#endif
...
renameDump(struct OMRPortLibrary *portLibrary, char *filename, pid_t pid, int signalNumber)
{
	...
	if (0 != waitCoreRC) {
		...
		portLibrary->str_printf(portLibrary, filename, EsMaxPath,
								"The core file created by child process with pid = %i was not found. Expected to find "
								"core file with name \"%s\"",
 ...
 */
static intptr_t
deriveCoreFileName(struct OMRPortLibrary *portLibrary, char *corePatterFormat, BOOLEAN coreUsesPID, char *baseDir,
...
static void
printProgramHeaders(struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize, Elf64_Off phoff,
					Elf64_Half phnum)
{
	uintptr_t count = 0;
	int offset = 0;
 ...
 */
static uintptr_t
getSharedAndPrivateDataSegments(struct OMRPortLibrary *portLibrary, intptr_t fd, uintptr_t addrSize, uint8_t **ppStart,
								uint8_t **ppEnd)
{
	char ch;
...
copyDataIntoELFCore(struct OMRPortLibrary *portLibrary, intptr_t fd_core, uintptr_t addrSize, Elf64_Off phoff)
{
	...
	if (32 == addrSize) {
		...
		if (0 == p32elfPgmHdr->p_filesz) {
			...
			if (dataOffset != -1) {
				dataSize = portLibrary->file_write(portLibrary, fd_core, (const void *)(uintptr_t)p32elfPgmHdr->p_vaddr,
												   p32elfPgmHdr->p_memsz);
				if (dataSize > 0) {
		...
		if (0 == p64elfPgmHdr->p_filesz) {
			...
			dataOffset = portLibrary->file_seek(portLibrary, fd_core, 0, EsSeekEnd);
			if (dataOffset != -1) {
				dataSize = portLibrary->file_write(portLibrary, fd_core, (const void *)(uintptr_t)p64elfPgmHdr->p_vaddr,
												   p64elfPgmHdr->p_memsz);
				if (dataSize > 0) {
