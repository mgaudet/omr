...
uintptr_t renameDump(struct OMRPortLibrary* portLibrary, char* filename, pid_t pid, int signalNumber)
{
    ...
    if (0 != waitCoreRC) {
        ...
        portLibrary->str_printf(portLibrary, filename, EsMaxPath, "The core file created by child process with pid = "
...
/* at this point we know the file in derivedCorePath exists */
    ...
    if (filename[0] != '\0') {
        ...
        if ((0 != renameRC) && (errno == EXDEV)) { /* failed with 'cross device rename error' */
            ...
            portLibrary->tty_printf(portLibrary, "Warning: unable to move dump to \"%s\" across file systems (check "
 ...
 * specified, we use a wildcard to
 * 		  obtain a filename match. The child process PID provides reasonable confidence that we will find the
 * correct dump. Otherwise return failure.
 *  Note: From the man pages: "A single % at the end of the template is dropped from the core filename, as is the
 * combination of a % followed by any character other than those listed above."
 *
 ...
 * separator) of the base directory to use if corePatternFormat does not
...
#define PR_GET_NAME 16
    ...
    *outCursor = '\0';
    ...
    if (corePatternSpecifiesPercentT && !corePatternSpecifiesPercentP) {
        ...
        portLibrary->str_printf(
            portLibrary, derivedCoreFileName, PATH_MAX, "%s", "\"%t\" specifier is not supported without \"%p\".");
        return -1;
    }
