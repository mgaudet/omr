...
getContentsFromProcFileSystem(struct OMRPortLibrary* portLibrary,
                              const char* fileName, char* buffer, size_t buflen)
{
...
uintptr_t
renameDump(struct OMRPortLibrary* portLibrary, char* filename, pid_t pid,
           int signalNumber)
{
 ...
 */
static intptr_t
deriveCoreFileName(struct OMRPortLibrary* portLibrary, char* corePatterFormat,
                   BOOLEAN coreUsesPID, char* baseDir,
                   char* derivedCoreFileName, uintptr_t derivedCoreFileNameLen,
...
waitCore(char* path)
{
  ...
  FILE* fd;
  starttime = time(NULL);
  char* wildcardPos = strchr(path,
                             '*'); /* position of '*' wildcard, if any, in the
                                      expected core dump filename */
...
#ifdef DUMP_DBG
static void
printProgramHeader(struct OMRPortLibrary* portLibrary, uintptr_t addrSize,
                   void* pelfPgmHdr, uintptr_t count)
{
...
static void
printProgramHeaders(struct OMRPortLibrary* portLibrary, intptr_t fd,
                    uintptr_t addrSize, Elf64_Off phoff, Elf64_Half phnum)
{
  uintptr_t count = 0;
  int offset = 0;
  int bytesread = 0;
 ...
 */
static uintptr_t
readElfHeader(struct OMRPortLibrary* portLibrary, intptr_t fd,
              Elf64_Ehdr* pElfHeader)
{
 ...
 */
static uintptr_t
findProgramHeader(struct OMRPortLibrary* portLibrary, intptr_t fd,
                  uintptr_t addrSize, Elf64_Off phoff, Elf64_Half phnum,
                  uint8_t* start, uint8_t* end)
{
  uintptr_t count;
 ...
 */
static void
copyDataIntoELFCore(struct OMRPortLibrary* portLibrary, intptr_t fd_core,
                    uintptr_t addrSize, Elf64_Off phoff)
{
  intptr_t dataOffset;
  intptr_t dataSize;
