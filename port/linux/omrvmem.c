...
static intptr_t initializeNumaGlobals(struct OMRPortLibrary* portLibrary)
{
    ...
    if (0 == mempolicyReturnCode) {
         ...
         * default or process default
         * policies then the mask will be all 0's which (if used later) would indicate no nodes are available for
        ...
        if ((numaNode > 0) && (numaNode <= PPG_numa_max_node_bits) && (numaNode <= maxnode)) {
            ...
            if (bit == (PPG_numa_available_node_mask.mask[wordIndex] & bit)) {
                ...
                if (0 != do_mbind(address, byteAmount, MPOL_PREFERRED, nodeMask.mask, PPG_numa_max_node_bits, 0)) {
                     ...
                     * port_numa_interleave_memory.
                     * This assert is useful to determine any problems calling mbind and improve the performance by
                     * succeeding mbind call.
                     *
                     */
                    Assert_PRT_true(FALSE);
                } else {
