 ...
 */
uint64_t
omrtime_hires_delta(struct OMRPortLibrary* portLibrary, uint64_t startTime,
                    uint64_t endTime, uint64_t requiredResolution)
{
  uint64_t ticks;
 ...
 */
static BOOLEAN
getLinuxKernelVersion(uint32_t* kernelVersion, uint32_t* majorRevision,
