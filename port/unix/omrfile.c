...
static void
setPortableError(OMRPortLibrary* portLibrary, const char* funcName,
                 int32_t portlibErrno, int systemErrno)
{
...
#if defined(LINUX) || defined(OSX) || defined(AIXPPC) && !defined(J9OS_I5)
static void
updateJ9FileStat(struct OMRPortLibrary* portLibrary, J9FileStat* j9statBuf,
                 struct stat* statBuf, PlatformStatfs* statfsBuf)
#else  /* defined(LINUX) || defined(OSX) || defined(AIXPPC) &&                 \
          !defined(J9OS_I5) */
static void
updateJ9FileStat(struct OMRPortLibrary* portLibrary, J9FileStat* j9statBuf,
...
intptr_t
omrfile_open(struct OMRPortLibrary* portLibrary, const char* path,
 ...
 */
intptr_t
omrfile_write(struct OMRPortLibrary* portLibrary, intptr_t inFD,
 ...
 */
intptr_t
omrfile_read(struct OMRPortLibrary* portLibrary, intptr_t inFD, void* buf,
             intptr_t nbytes)
{
  int fd = (int)inFD;
  intptr_t result;
 ...
 */
int64_t
omrfile_seek(struct OMRPortLibrary* portLibrary, intptr_t inFD, int64_t offset,
             int32_t whence)
{
  int fd = (int)inFD;
...
int32_t
omrfile_chmod(struct OMRPortLibrary* portLibrary, const char* path,
              int32_t mode)
{
  int32_t result;
...
intptr_t
omrfile_chown(struct OMRPortLibrary* portLibrary, const char* path,
...
uintptr_t
omrfile_findfirst(struct OMRPortLibrary* portLibrary, const char* path,
                  char* resultbuf)
{
#if defined(AIXPPC)
  DIR64* dirp = NULL;
 ...
 */
int32_t
omrfile_findnext(struct OMRPortLibrary* portLibrary, uintptr_t findhandle,
                 char* resultbuf)
{
#if defined(AIXPPC)
  struct dirent64* entry;
...
int32_t
omrfile_move(struct OMRPortLibrary* portLibrary, const char* pathExist,
             const char* pathNew)
{
 ...
 */
void
omrfile_vprintf(struct OMRPortLibrary* portLibrary, intptr_t fd,
 ...
 */
void
omrfile_printf(struct OMRPortLibrary* portLibrary, intptr_t fd,
               const char* format, ...)
{
  va_list args;
 ...
 */
int32_t
omrfile_set_length(struct OMRPortLibrary* portLibrary, intptr_t inFD,
                   int64_t newLength)
{
  int fd = (int)inFD;
  int32_t rc;
 ...
 */
int32_t
omrfile_lock_bytes(struct OMRPortLibrary* portLibrary, intptr_t fd,
 ...
 */
int32_t
omrfile_unlock_bytes(struct OMRPortLibrary* portLibrary, intptr_t fd,
...
int32_t
omrfile_fstat(struct OMRPortLibrary* portLibrary, intptr_t fd,
...
int32_t
omrfile_stat(struct OMRPortLibrary* portLibrary, const char* path,
