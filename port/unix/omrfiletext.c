...
intptr_t
omrfile_write_text(struct OMRPortLibrary* portLibrary, intptr_t fd,
...
static intptr_t
file_write_using_wctomb(struct OMRPortLibrary* portLibrary, intptr_t fd,
                        const char* buf, intptr_t nbytes)
{
  intptr_t result;
  intptr_t newLength = 0;
 ...
 */
static int
growBuffer(struct OMRPortLibrary* portLibrary, char* stackBuf, char** bufStart,
           char** cursor, size_t* bytesLeft, uintptr_t* bufLen)
{
...
static intptr_t
file_write_using_iconv(struct OMRPortLibrary* portLibrary, intptr_t fd,
                       const char* buf, intptr_t nbytes)
{
  intptr_t result;
  char stackBuf[512];
...
char*
omrfile_read_text(struct OMRPortLibrary* portLibrary, intptr_t fd, char* buf,
                  intptr_t nbytes)
{
#if (defined(J9ZOS390))
...
int32_t
omrfile_get_text_encoding(struct OMRPortLibrary* portLibrary, char* charsetName,
                          uintptr_t nbytes)
{
  char *codepage, *c_ptr;
