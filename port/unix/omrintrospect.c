...
static int barrier_init_r(barrier_r* barrier, int value)
{
    ...
    do {
        ...
        compareAndSwapUDATA((uintptr_t*)&barrier->initial_value, old_value, value, &barrier->spinlock) != old_value);
    do {
...
static int barrier_enter_r(barrier_r* barrier, uintptr_t deadline)
{
    ...
    do {
        ...
        compareAndSwapUDATA((uintptr_t*)&barrier->in_count, old_value, old_value - 1, &barrier->spinlock) != old_value);
...
static int sem_destroy_r(sem_t_r* sem)
{
    ...
    do {
        old_value = sem->sem_value;
    } while (compareAndSwapUDATA((uintptr_t*)&sem->sem_value, old_value, old_value - sem->initial_value, &sem->spinlock)
        != old_value);
