...
barrier_init_r(barrier_r* barrier, int value)
{
  ...
  } while (compareAndSwapUDATA((uintptr_t*)&barrier->in_count, old_value, value,
  ...
  if (old_value < 0 || (old_value == 0 && barrier->initial_value != 0)) {
    ...
    do {
      old_value = barrier->initial_value;
                                 ...
                                 new_value, &barrier->spinlock) != old_value);
...
#if defined(J9OS_I5)
void
get_thread_Info(struct PlatformWalkData* data, void* context_arg,
                unsigned long tid)
{
 ...
 */
static int
setup_native_thread(J9ThreadWalkState* state, thread_context* sigContext,
                    int heapAllocate)
{
  struct PlatformWalkData* data =
    (struct PlatformWalkData*)state->platform_data;
  ...
  if (heapAllocate || sigContext) {
    ...
    if (sigContext) {
      ...
      memcpy(state->current_thread->context,
             ((J9UnixSignalInfo*)sigContext)->platformSignalInfo.context, size);
