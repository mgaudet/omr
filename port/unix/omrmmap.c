...
omrmmap_map_file(struct OMRPortLibrary* portLibrary, intptr_t file,
                 ...
                 uint32_t flags, uint32_t categoryCode)
{
  int mmapProt = 0;
  int mmapFlags = 0;
  void* pointer = NULL;
 ...
 */
intptr_t
omrmmap_msync(struct OMRPortLibrary* portLibrary, void* start, uintptr_t length,
              uint32_t flags)
{
  int msyncFlags = 0;
  int rc = 0;
...
intptr_t
omrmmap_protect(struct OMRPortLibrary* portLibrary, void* address,
 ...
 */
void
omrmmap_dont_need(struct OMRPortLibrary* portLibrary, const void* startAddress,
                  size_t length)
{
  size_t pageSize =
  ...
  if (pageSize > 0 && length >= pageSize) {
    ...
    uintptr_t const* roundedStart =
      (uintptr_t const*)ROUND_UP_TO_POWEROF2((uintptr_t)startAddress, pageSize);
