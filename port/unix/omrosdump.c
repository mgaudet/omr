...
uintptr_t omrdump_create(struct OMRPortLibrary* portLibrary, char* filename, char* dumpType, void* userData)
{
    ...
    if ((sys_parmRC == 0) && (myvar.v.v_fullcore.value == 1)) {
        ...
        portLibrary->tty_err_printf(portLibrary, "Note: \"Enable full CORE dump\" in smit is set to FALSE and as a "
        ...
        if (0 == portLibrary->sl_lookup_name(portLibrary, handle, "proc_setattr", (uintptr_t*)&setattr, "IPLj")) {
            ...
            if (rc != 0) {
                if (ENOSYS == errno) {
                     ...
                     * subsequent reboot,
                     * in which case, behave the same as if we're running on an AIX version which does not support
                     * proc_setattr() */
                } else {
