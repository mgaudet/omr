    ...
    omrsig_handler_fn handler, void* handler_arg, uint32_t flags, uintptr_t* result)
{
     ...
     * We will need to aquire the masterHandlerMonitor if flagsWithoutMasterHandlers is not 0 in order to install the
     * handlers (and to check if another thread had "just" taken care of it).
     * If we do have masterHandlers installed already then OMRPORT_SIG_OPTIONS_REDUCED_SIGNALS_SYNCHRONOUS has not been
     * set.
     */
    ...
    if (0 != flagsWithoutMasterHandlers) {
        ...
        /* we have not installed handlers for all the signals, aquire the masterHandlerMonitor, check again and install
         * handlers as required all done in registerMasterHandlers */
...
#define MSR_TS_MASK 0x6U
    ...
    if ((NULL != platformContext->uc_link)
        && J9_ARE_ANY_BITS_SET(platformContext->uc_link->uc_mcontext.regs->msr, MSR_TS_MASK))
#endif
    {
        /*
    ...
    if ((OMRPORT_SIG_EXCEPTION_COOPERATIVE_SHUTDOWN == result)
        || (J9_ARE_ALL_BITS_SET(signalOptionsGlobal, OMRPORT_SIG_OPTIONS_COOPERATIVE_SHUTDOWN))) {
    ...
    OMRPortLibrary* portLibrary, uint32_t portLibrarySignalNo, unix_sigaction handler)
{
     ...
     * "__sighandler_t sa_handler". Both are members of struct sigaction.
...
/* mask out subtypes */
    ...
    portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT | OMRPORT_SIG_FLAG_SIGABRT
        | OMRPORT_SIG_FLAG_SIGTERM | OMRPORT_SIG_FLAG_SIGXFSZ | OMRPORT_SIG_FLAG_SIGRECONFIG;
#else
    portLibSignal &= OMRPORT_SIG_FLAG_SIGALLSYNC | OMRPORT_SIG_FLAG_SIGQUIT | OMRPORT_SIG_FLAG_SIGABRT
...
#if defined(OMR_PORT_ASYNC_HANDLER)
    if (J9THREAD_SUCCESS != createThreadWithCategory(&asynchSignalReporterThread, 256 * 1024, J9THREAD_PRIORITY_MAX, 0,
