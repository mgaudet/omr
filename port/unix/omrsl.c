  ...
  if (!openExec && decorate) {
    ...
    if (NULL != p) {
      ...
      pathLength = portLibrary->str_printf(
        ...
        "%.*slib%s" PLATFORM_DLL_EXTENSION, (uintptr_t)p + 1 - (uintptr_t)name,
        name, p + 1);
    } else {
      pathLength = portLibrary->str_printf(
        portLibrary, mangledName, (MAX_STRING_LENGTH + 1),
...
static void
convertWithIConv(struct OMRPortLibrary* portLibrary, char* error, char* errBuf,
                 uintptr_t bufLen)
{
  iconv_t converter;
...
static void
convertWithMBTOWC(struct OMRPortLibrary* portLibrary, const char* error,
                  char* errBuf, uintptr_t bufLen)
{
  char* out = errBuf;
