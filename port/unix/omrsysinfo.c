...
intptr_t
omrsysinfo_get_env(struct OMRPortLibrary* portLibrary, const char* envVar,
                   char* infoString, uintptr_t bufSize)
{
...
#if defined(LINUX) || defined(AIXPPC) || defined(J9ZOS390)
static intptr_t
readSymbolicLink(struct OMRPortLibrary* portLibrary, char* linkFilename,
                 char** result)
{
 ...
 */
static intptr_t
searchSystemPath(struct OMRPortLibrary* portLibrary, char* filename,
                 char** result)
{
  char* pathCurrent;
  char* pathNext;
...
intptr_t
omrsysinfo_get_username(struct OMRPortLibrary* portLibrary, char* buffer,
                        uintptr_t length)
{
...
intptr_t
omrsysinfo_get_groupname(struct OMRPortLibrary* portLibrary, char* buffer,
                         uintptr_t length)
{
  char* remoteCopy = NULL;
...
uint32_t
omrsysinfo_get_limit(struct OMRPortLibrary* portLibrary, uint32_t resourceID,
                     uint64_t* limit)
{
...
uint32_t
omrsysinfo_set_limit(struct OMRPortLibrary* portLibrary, uint32_t resourceID,
                     uint64_t limit)
{
  uint32_t hardLimitRequested =
    OMRPORT_LIMIT_HARD == (resourceID & OMRPORT_LIMIT_HARD);
...
static uintptr_t
copyEnvToBufferSignalHandler(struct OMRPortLibrary* portLib, uint32_t gpType,
                             void* gpInfo, void* unUsed)
{
...
static intptr_t
convertWithIConv(struct OMRPortLibrary* portLibrary, iconv_t* converter,
                 char* inputBuffer, char* outputBuffer, uintptr_t bufLen)
{
 ...
 * terminator, or -1 if the buffer wasn't big enough */
static intptr_t
convertWithMBTOWC(struct OMRPortLibrary* portLibrary, char* inputBuffer,
                          ...
                          struct J9ProcessorInfos* procInfo)
{
  ...
  rc =
    host_processor_info(host, PROCESSOR_CPU_LOAD_INFO, &processorCount,
                        (processor_info_array_t*)&cpuLoadInfo, &msgTypeNumber);
  if (KERN_SUCCESS == rc) {
    rc =
      host_processor_info(host, PROCESSOR_BASIC_INFO, &processorCount,
...
intptr_t
omrsysinfo_get_cwd(struct OMRPortLibrary* portLibrary, char* buf,
                   uintptr_t bufLen)
{
  intptr_t rc = -1;
...
intptr_t
omrsysinfo_get_tmp(struct OMRPortLibrary* portLibrary, char* buf,
