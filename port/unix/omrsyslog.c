...
get_conv_txt_num_bytes(struct OMRPortLibrary *portLibrary, const char *message)
{
	...
	const int32_t convRes = portLibrary->str_convert(portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW, message,
...
syslog_write(struct OMRPortLibrary *portLibrary, const char *buf, int priority)
{
	...
	if (NULL_TERM_SZ < lclMsgLen) {
		lclMsg =
			portLibrary->mem_allocate_memory(portLibrary, lclMsgLen, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
		...
		if (NULL != lclMsg) {
			 ...
			 */
			const int32_t convRes = portLibrary->str_convert(portLibrary, J9STR_CODE_MUTF8, J9STR_CODE_PLATFORM_RAW,
