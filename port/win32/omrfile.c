 ...
 * 					unicode path - the caller is responsible for freeing this memory by calling @ref
    ...
    OMRPortLibrary* portLibrary, const char* utf8Path, wchar_t* unicodeBuffer, uintptr_t unicodeBufferSize)
{
    ...
    if (pathLen * sizeof(wchar_t) <= MAX_PATH) {
         ...
         * state
         * that it should not be used in multi-threaded applications, but the docs are fairly clear that this is because
         * the conversion from a
         * relative path to a fully qualified path depends on the current working directory (CWD), which is a process
         ...
         * "." and "..",
         * need to be removed. This includes the occurrences that don't simply prepend the path, in which case the CWD
         * is not relevant and
...
int32_t omrfile_sync(struct OMRPortLibrary* portLibrary, intptr_t fd)
{
     ...
     * all buffered data from the pipe.
     * 2) The function fails if hFile is a handle to the console output. That is because the console output is not
     * buffered. The function returns FALSE, and GetLastError returns ERROR_INVALID_HANDLE.
     *
     * This behaviour caused JAZZ 44899 defect: Test_SimpleTenantLimitXmt would freeze while waiting for process to
     * terminate while child
     ...
     * would still get receive 'X' and 'Y'
...
void omrfile_vprintf(struct OMRPortLibrary* portLibrary, intptr_t fd, const char* format, va_list args)
{
    ...
    allocatedBuffer = portLibrary->mem_allocate_memory(
        portLibrary, numberWritten, OMR_GET_CALLSITE(), OMRMEM_CATEGORY_PORT_LIBRARY);
...
intptr_t omrfile_write(struct OMRPortLibrary* portLibrary, intptr_t fd, void* buf, intptr_t nbytes)
{
    DWORD nCharsWritten;
