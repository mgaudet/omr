 ...
 */
int32_t
omrfile_lock_bytes_helper(struct OMRPortLibrary* portLibrary, intptr_t fd,
                          ...
                          BOOLEAN async)
{
  ...
  Trc_PRT_file_lock_bytes_win32_callingLockFileEx(
    dwFlags, overlapped.Offset, overlapped.OffsetHigh, nNumberOfBytesToLockLow,
 ...
 */
int32_t
omrfile_unlock_bytes_helper(struct OMRPortLibrary* portLibrary, intptr_t fd,
                            uint64_t offset, uint64_t length, BOOLEAN async)
{
  HANDLE hFile = (HANDLE)fd;
  DWORD nNumberOfBytesToLockLow = (DWORD)(length & 0xFFFFFFFF);
  DWORD nNumberOfBytesToLockHigh = (DWORD)(length >> 32);
