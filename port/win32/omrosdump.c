 ...
 */
uintptr_t
omrdump_create(struct OMRPortLibrary* portLibrary, char* filename,
...
#define MAX_MINIDUMP_ATTEMPTS 10
  ...
  for (i = 0; i < MAX_MINIDUMP_ATTEMPTS; i++) {
     ...
     */
    retVal =
      args->dmp_function(GetCurrentProcess(), GetCurrentProcessId(), hFile,
...
static HANDLE
openFileFromEnvVar(struct OMRPortLibrary* portLibrary, char* envVarName,
                   char* fileNameBuff, uint32_t fileNameBuffSize)
{
  HANDLE hFile;
  intptr_t retCode;
  uint32_t i;
  char
    pidStore[25]; /* This is roughly the 3 * sizeof(int)+1 which is more than
                     enough space*/
...
static HANDLE
openFileInCWD(struct OMRPortLibrary* portLibrary, char* fileNameBuff,
              uint32_t fileNameBuffSize)
{
  HANDLE hFile;
  uint32_t length;
