...
static HANDLE openFileFromEnvVar(struct OMRPortLibrary *portLibrary, char *envVarName, char *fileNameBuff,
...
omrdump_create(struct OMRPortLibrary *portLibrary, char *filename, char *dumpType, void *userData)
{
	...
	if (*filename == '\0') {
		...
		/* use name provided */
		hFile =
			(HANDLE)portLibrary->file_open(portLibrary, filename, EsOpenWrite | EsOpenCreate | EsOpenRead,
