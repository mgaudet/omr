 ...
 */
wchar_t *
port_convertFromUTF8(OMRPortLibrary *portLibrary, const char *string, wchar_t *unicodeBuffer,
					 uintptr_t unicodeBufferSize)
{
	...
	if (length < unicodeBufferSize) {
		...
		unicodeString = portLibrary->mem_allocate_memory(portLibrary, (length + 1) * 2, OMR_GET_CALLSITE(),
...
{
	if (0 == WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, unicodeString, -1, utf8Buffer, (int)size,
								 NULL, NULL)) {
