...
copyEnvToBuffer(struct OMRPortLibrary* portLibrary, void* args)
{
  ...
  while ((0 == envVars[0] && 0 == envVars[1]) == FALSE) {
    ...
    storageRequired +=
      WideCharToMultiByte(OS_ENCODING_CODE_PAGE, OS_ENCODING_WC_FLAGS, envVars,
                          -1, cursor, 0, NULL, NULL);
 ...
 */
static int32_t
AddCounter(OMRPortLibrary* portLibrary, PDH_HQUERY statsHandle,
           struct J9ProcessorInfos* procInfo, const char* cntrName,
