...
omrsyslog_write_utf8(struct OMRPortLibrary *portLibrary, uintptr_t flags, const char *message)
{
	if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
		switch (flags) {
		case J9NLS_ERROR:
			return ReportEvent(PPG_syslog_handle, EVENTLOG_ERROR_TYPE, 0, MSG_ERROR, NULL, 1, 0, (LPCTSTR *)&message,
							   NULL);
		case J9NLS_WARNING:
...
omrsyslog_write_unicode(struct OMRPortLibrary *portLibrary, uintptr_t flags, const wchar_t *message)
{
	if (NULL != portLibrary->portGlobals && NULL != PPG_syslog_handle) {
		switch (flags) {
		case J9NLS_ERROR:
			return ReportEventW(PPG_syslog_handle, EVENTLOG_ERROR_TYPE, 0, MSG_ERROR, NULL, 1, 0, (LPCWSTR *)&message,
								NULL);
		case J9NLS_WARNING:
