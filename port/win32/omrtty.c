...
/* Note that this could be done on the stack if we believe that it might overflow (40 k) so dynamic allocation is safer
 * for now */
            ...
            if (PeekConsoleInput(
                ...
                for (x = 0; x < (int)available; x++) {
                    if ((KEY_EVENT == inputRecordPointer[x].EventType)
                        ...
                        bytesCanBeRead
                            = TRUE; /* report true if we find a return char since we know that it is safe to read at
                                       least that one byte */
                        break;
                    }
                }
