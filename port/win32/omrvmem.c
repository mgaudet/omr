    ...
    struct OMRPortLibrary* portLibrary, J9MemoryNodeDetail* numaNodes, uintptr_t* nodeCount)
{
    ...
    if (NULL != kernel32Module) {
         ...
         * to change our definition of "NUMA enabled" so that all systems
         * have NUMA properties but some have only 1 node.  This way, the special case is removed and the GC can decide
         * how to handle that environment.
         */
