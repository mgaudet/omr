...
typedef BOOL(__stdcall *SYMGETLINEFROMADDR64)(HANDLE hProcess, DWORD64 dwAddr, PDWORD pdwDisplacement,
...
typedef struct OMRPortPlatformGlobals {
	...
	uintptr_t vmem_pageFlags
		[OMRPORT_VMEM_PAGESIZE_COUNT]; /** <0 terminated array of flags describing type of the supported page sizes */
	char *si_osType;
	char *si_osTypeOnHeap;
	char *si_osVersion;
	uint64_t time_hiresClockFrequency;
	void *
		tty_consoleEventBuffer; /* windows.h cannot be included here and it has a complex definition so use void * in place of INPUT_RECORD* */
	...
	GetThreadIdealProcessorExType
		GetThreadIdealProcessorExProc; /**< Cached pointer to GetThreadIdealProcessorEx function (or NULL if not supported on host OS level) */
	SetThreadIdealProcessorExType
		SetThreadIdealProcessorExProc; /**< Cached pointer to SetThreadIdealProcessorEx function (or NULL if not supported on host OS level) */
	GetNumaNodeProcessorMaskExType
		GetNumaNodeProcessorMaskExProc; /**< Cached pointer to GetNumaNodeProcessorMaskEx function (or NULL if not supported on host OS level) */
	J9AVLTree
		bindingTree;	 /**< The tree of all tracked NUMA memory extents for all reserved memory extents - Should ideally be moved into VMIdentifier and tracked on a per-extent basis */
