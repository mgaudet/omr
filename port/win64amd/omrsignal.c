    ...
    do {
        ...
        if (!runtimeFunction) {
            ...
            RtlVirtualUnwind(UNW_FLAG_NHANDLER, /* suppresses the invocation of any unwind or exception handlers
                ...
                imageBase,
                context->Rip, runtimeFunction, context, &handlerData, &establisherFrame, &nvContext);
        }
    ...
    struct OMRPortLibrary* portLibrary, struct J9Win32SignalInfo* info, int32_t index, const char** name, void** value)
{
    *name = "";
     ...
     * OMRPORT_SIG_WINDOWS_DEFER_TRY_EXCEPT_HANDLER
    ...
    if (0 == omrthread_attach_ex(NULL, J9THREAD_ATTR_DEFAULT)) {
        ...
        while (cursor) {
            ...
            if (cursor->flags & flags) {
                ...
                /* Returning TRUE stops control from being passed to the next handler routine. The OS default handler
                 * may be the next handler routine.
        ...
        /* TODO: possible timing hole. The thread library could be unloaded by the time we reach this line. We can't use
         * omrthread_exit(), as that kills the async reporting thread */
