...
#endif
    typedef void CSRSI_calltype(CSRSIRequest __REQUEST, /* Input  - request type           */
...
#define csrsi_byaddr(Request, Flen, Fptr, Rcptr)                                                    \
    ...
    struct CSRSI_CSRT
    ? ? < unsigned char CSRSI_csrt_filler1
    ? ? (48 ? ?);
...
typedef struct ? ? < uint32_t si22v1format : 8; /* A 1-byte value. When the
...
typedef struct ? ? < uint32_t si22v1altcpucapability; /* A 32-bit unsigned binary
                                  ...
                                  alternate capability.
                                                                 @L1A*/
struct ? ? < unsigned char _si22v1altmpcpucapaf ? ? (2 ? ?); /* Each individual
