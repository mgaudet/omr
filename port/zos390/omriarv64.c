...
omrallocate_1M_fixed_pages(int *numMBSegments, int *userExtendedPrivateAreaMemoryType, const char *ttkn)
{
	...
	case ZOS64_VMEM_ABOVE_BAR_GENERAL:
		__asm(
			" IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,PAGEFRAMESIZE=1MEG,"
...
omrallocate_1M_pageable_pages_above_bar(int *numMBSegments, int *userExtendedPrivateAreaMemoryType, const char *ttkn)
{
	...
	case ZOS64_VMEM_2_TO_32G:
		__asm(
			" IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,USE2GTO32G=YES,"
			"PAGEFRAMESIZE=PAGEABLE1MEG,TYPE=PAGEABLE,SEGMENTS=(%2),"
			"ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
			"r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
		break;
	case ZOS64_VMEM_2_TO_64G:
		__asm(
			" IARV64 REQUEST=GETSTOR,COND=YES,SADMP=NO,CONTROL=UNAUTH,USE2GTO64G=YES,"
			"PAGEFRAMESIZE=PAGEABLE1MEG,TYPE=PAGEABLE,SEGMENTS=(%2),"
			"ORIGIN=(%1),TTOKEN=(%4),RETCODE=%0,MF=(E,(%3))" ::"m"(iarv64_rc),
			"r"(&origin), "r"(&segments), "r"(&wgetstor), "r"(ttkn));
		break;
 ...
 */
void *
omrallocate_4K_pages_in_userExtendedPrivateArea(int *numMBSegments, int *userExtendedPrivateAreaMemoryType,
												const char *ttkn)
{
	long segments;
	long origin;
