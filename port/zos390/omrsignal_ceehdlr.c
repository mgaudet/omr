 ...
 *	@param[in]	token 	An J9ZOSLEConditionHandlerRecord as specified when this handler was registered by
 *@ref omrsig_protect_ceehdlr
 *	@param[in]	leResult	A 4-byte integer that contains instructions about responses the user-written
 *condition
 * 							handler wants Language Environment to make when processing the
 *condition.
 * 							The result_code is passed by reference.
 * 							Valid responses are shown in Table 38 of the Language Environment
 *Programming Guide
 ...
 *32)
 *						 or the requested fix-up actions for a fix-up and resume response (result_code
 *value of 60).
 *						 When a result_code of 60, denoting fix-up and resume, is set by the condition
 *handler,
 *						 new_condition must be set to a condition token that indicates what fix-up action
 *is requested.
 *						 Many conditions, including mathematical routines, use the condition tokens
 *in
 *						 Table 39 to resume with corrective action (either resume with new input
 *value
 *						 or resume with new output value).
 *						 For some conditions, there may be other condition tokens that can be provided by
 *the condition
 ...
 *http://publibz.boulder.ibm.com/epubs/pdf/ceea91a0.pdf,
