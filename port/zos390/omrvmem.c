...
void *omrallocate_4K_pages_in_userExtendedPrivateArea(int numMBSegments, int userExtendedPrivateAreaMemoryType,
													  const char *ttkn);

/* omrvmem_support_above_bar.s */
#pragma linkage(omrallocate_1M_pageable_pages_above_bar, OS_NOSTACK)
void *omrallocate_1M_pageable_pages_above_bar(int numMBSegments, int userExtendedPrivateAreaMemoryType,
											  const char *ttkn);
						  ...
						  uintptr_t byteAmount, OMRMemCategory *category)
{
	...
	if (NULL != ptr) {
		...
		update_vmemIdentifier(identifier, ptr, ptr, byteAmount, mode, ONE_M, OMRPORT_VMEM_PAGE_FLAG_PAGEABLE, allocator,
							  category);
	} else {
...
reservePagesAboveBar(struct OMRPortLibrary *portLibrary, J9PortVmemIdentifier *identifier, uintptr_t mode,
					 uintptr_t byteAmount, uintptr_t pageSize, uintptr_t pageFlags, uintptr_t options,
					 OMRMemCategory *category)
{
...
#define MAX_2TO64G_MEMORY_IN_MB ((UDATA)62 * 1024) /* unit is MB i.e. 62*1024 MB = 62 GB */
	...
	Trc_PRT_vmem_reservePagesAboveBar_Entry(byteAmount, pageSize, pageFlags, options,
											userExtendedPrivateAreaMemoryType);

	LP_DEBUG_PRINTF5(
		"\t reservePagesAboveBar byteAmount=0x%zx, pageSize=x%zx, pageFlags=0x%zx, useStrictPageSize=0x%x, "
	...
	if ((FOUR_K == pageSize) && (ZOS64_VMEM_ABOVE_BAR_GENERAL != userExtendedPrivateAreaMemoryType)) {
		...
		if (0 != PPG_vmem_pageSize[index]) {
			...
			LP_DEBUG_PRINTF4(
				"\t reservePagesAboveBar invalid params: byteAmount = 0x%x, pageSize = 0x%x, pageFlags = 0x%x, "
				"userExtendedPrivateAreaMemoryType = 0x%x\n",
...
_end:
	if (NULL != ptr) {
		omrmem_categories_increment_counters(category, byteAmount);
		update_vmemIdentifier(identifier, ptr, ptr, byteAmount, mode, actualPageSize, actualPageFlags, allocator,
							  category);
	} else {
		update_vmemIdentifier(identifier, 0, 0, 0, 0, 0, 0, 0, NULL);
	}
...
#else  /* if 0 */
		{
			...
			if (0 != rc) {
				...
				if (EINVAL == errno) {
					...
					Trc_PRT_vmem_default_reserve_using_4K_pages_above_bar_iarv64_failed(mymopl.__mopl_iarv64_rc,
						  ...
						  struct J9PortVmemParams *callerParams)
{
	...
	Trc_PRT_vmem_omrvmem_reserve_memory_Entry_replacement_v1(params.startAddress, params.byteAmount, params.pageSize,
															 params.pageFlags);
			 ...
			 struct J9PortVmemParams *params, OMRMemCategory *category)
{
	...
	if (FALSE == isLargePageSizeSupported(portLibrary, params->pageSize, params->pageFlags)) {
		...
		if ((ONE_M != params->pageSize) || (0 == (OMRPORT_VMEM_PAGE_FLAG_PAGEABLE & params->pageFlags))) {
			update_vmemIdentifier(identifier, NULL, NULL, 0, 0, 0, 0, 0, NULL);
			Trc_PRT_vmem_omrvmem_reserve_memory_unsupported_page_size_executable_largepages(params->pageSize,
																							params->pageFlags);
		} else {
			baseAddress =
		...
		if (TRUE == use2To32GArea) {
			...
			if ((NULL != baseAddress) && (TRUE == isStrictAndOutOfRange(params, baseAddress))) {
				 ...
				 */
				Trc_PRT_vmem_reservePages_UnableToAllocateWithinSpecifiedRange(params->byteAmount, params->startAddress,
																			   params->endAddress);
				omrvmem_free_memory(portLibrary, baseAddress, params->byteAmount, identifier);
				baseAddress = NULL;
			}
			   ...
			   struct J9PortVmemParams *params, OMRMemCategory *category)
{
	...
	if (!use2To32GArea) {
		...
		if (((uintptr_t)params->endAddress <= FOUR_GIG_LIMIT) || (TRUE == useExecutablePages)) {
			...
			baseAddress = default_pageSize_reserve_low_memory(portLibrary, params->byteAmount, identifier, params->mode,
															  category);
			...
			if ((NULL == baseAddress) && (FALSE == useStrictAddress) && (FALSE == useExecutablePages)) {
				baseAddress = default_pageSize_reserve_memory(portLibrary, params->byteAmount, identifier, params->mode,
															  category);
			}
		} else {
