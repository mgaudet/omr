                    ...
                    struct J9PortVmemIdentifier* identifier)
{
  ...
  if (OMRPORT_VMEM_RESERVE_USED_J9MEM_ALLOCATE_MEMORY ==
    ...
    Trc_PRT_vmem_omrvmem_free_memory_using_free_memory_below_bar(address,
                                                                 byteAmount);
    rc =
      omrfree_memory_below_bar(address, identifier->size, OMRPORT_VMEM_SUBPOOL);
    omrmem_categories_decrement_counters(identifier->category,
                                         identifier->size);
#if defined(OMR_ENV_DATA64)
  } else if (
...
reserve1MPageableBelowBar(struct OMRPortLibrary* portLibrary,
                          J9PortVmemIdentifier* identifier, uintptr_t mode,
...
reservePagesAboveBar(struct OMRPortLibrary* portLibrary,
                     J9PortVmemIdentifier* identifier, uintptr_t mode,
                     uintptr_t byteAmount, uintptr_t pageSize,
                     uintptr_t pageFlags, uintptr_t options,
                     OMRMemCategory* category)
{
 ...
 */
static BOOLEAN
isLargePageSizeSupported(struct OMRPortLibrary* portLibrary, uintptr_t pageSize,
                         uintptr_t pageFlags)
{
