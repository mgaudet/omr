...
static intptr_t monitor_alloc_and_init(omrthread_monitor_t *handle, uintptr_t flags, intptr_t policy,
...
static intptr_t monitor_wait_three_tier(omrthread_t self, omrthread_monitor_t monitor, int64_t millis, intptr_t nanos,
...
static intptr_t monitor_wait_original(omrthread_t self, omrthread_monitor_t monitor, int64_t millis, intptr_t nanos,
									  uintptr_t interruptible);
	...
	if (0 != value) {
		...
		if (0 == strcmp(J9THREAD_LIB_CONTROL_GET_MEM_CATEGORIES, key)) {
			...
			if (fillInEmptyMemCategorySlot(categorySet, &lib->threadLibraryCategory, 0, &filledSlotIndex)) {
				if (fillInEmptyMemCategorySlot(categorySet, &lib->nativeStackCategory, filledSlotIndex + 1,
											   &filledSlotIndex)) {
#if defined(OMR_THR_FORK_SUPPORT)
					if (fillInEmptyMemCategorySlot(categorySet, &lib->mutexCategory, filledSlotIndex + 1,
												   &filledSlotIndex)) {
						if (fillInEmptyMemCategorySlot(categorySet, &lib->condvarCategory, filledSlotIndex + 1,
													   &filledSlotIndex)) {
							rc = 0;
						}
					}
...
omrthread_sleep_interruptable(int64_t millis, intptr_t nanos)
{
	...
	J9OSCOND_WAIT_IF_TIMEDOUT(self->condition, self->mutex, boundedMillis, nanos) { break; }
	else
	{
		if (self->flags & J9THREAD_FLAG_INTERRUPTED) {
...
omrthread_sleep(int64_t millis)
{
	...
	J9OSCOND_WAIT_IF_TIMEDOUT(self->condition, self->mutex, boundedMillis, 0) { break; }
	J9OSCOND_WAIT_TIMED_LOOP();
...
monitor_init(omrthread_monitor_t monitor, uintptr_t flags, omrthread_library_t lib, const char *name)
{
	 ...
	 */
	if ((J9THREAD_MONITOR_OBJECT != (flags & J9THREAD_MONITOR_OBJECT)) || (0 != lib->secondarySpinForObjectMonitors)) {
...
static intptr_t
monitor_alloc_and_init(omrthread_monitor_t *handle, uintptr_t flags, intptr_t policy, intptr_t policyData,
					   const char *name)
{
	omrthread_monitor_t monitor;
	omrthread_t self = MACRO_SELF();
 ...
 */
static intptr_t
monitor_wait_original(omrthread_t self, omrthread_monitor_t monitor, int64_t millis, intptr_t nanos,
					  uintptr_t interruptible)
{
	omrthread_t *queue;
	intptr_t count = -1;
...
#if defined(OMR_THR_THREE_TIER_LOCKING)
static intptr_t
monitor_wait_three_tier(omrthread_t self, omrthread_monitor_t monitor, int64_t millis, intptr_t nanos,
						uintptr_t interruptible)
{
	omrthread_t *queue;
	intptr_t count = -1;
