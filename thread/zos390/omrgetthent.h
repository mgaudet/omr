...
void BPX4GTH(unsigned int *InputArea_length, unsigned char **InputArea, unsigned int *OuputArea_length,
			 unsigned char **OuputArea, unsigned int *Return_value, unsigned int *Return_code,
...
struct pgthh {
	char id
		[2]; /* rd = root directory (fist), cd = current directory (second), fd = file directory, vd = vnode directory */
	char type;
	char open;
	int inode;
	int devno;
