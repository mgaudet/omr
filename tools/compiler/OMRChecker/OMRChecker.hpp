 ...
 * extensible.
 *       If you find an extensible class, you've found an extensible class that is in a hierarchy like so: (parent ->
 * child)
 *
...
public:
    ...
    void printRelations()
    {
        ...
        for (std::map<CXXRecordDecl *, CXXRecordDecl *>::iterator I = MostDerivedType.begin(),
...
private:
    ...
    bool inSameClassString(CXXRecordDecl* queryType, CXXRecordDecl* derivedType, int level = 0)
    {
        ...
        for (CXXRecordDecl::base_class_iterator BI = derivedType->bases_begin(), BE = derivedType->bases_end();
             BI != BE; ++BI) {
            CXXRecordDecl* base_class = BI->getType()->getAsCXXRecordDecl();
            if (base_class) {
...
public:
    ...
    bool VisitCXXMemberCallExpr(CXXMemberCallExpr* call)
    {
        ...
        if (receiver->isImplicitCXXThis()) {
            ...
            if (thisExpr) {
                CXXRecordDecl* thisConcrete = ClassChecker->getAssociatedConcreteType(
    ...
    while (true) {
        ...
        if (ImplicitCastExpr* ICE = dyn_cast<ImplicitCastExpr>(E)) {
            if (ICE->getCastKind() == CK_NoOp || ICE->getCastKind() == CK_LValueToRValue
                || ICE->getCastKind() == CK_DerivedToBase || ICE->getCastKind() == CK_UncheckedDerivedToBase) {
                E = ICE->getSubExpr();
                continue;
            }
        }
