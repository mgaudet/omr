...
JavaBlobGenerator::genBinaryBlob(OMRPortLibrary* portLibrary,
  ...
  if (DDR_RC_OK == rc) {
    ...
    if ((unsigned long)amountWritten != _buildInfo.header.structDataSize) {
      ERRMSG(
          "Written: %d fields, %d constants, %d structs.\nCounted: %d fields, "
          "%d constants, %d structures.\n",
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(EnumUDT* e,
                                     bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(UnionUDT* u,
                                     bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
  /* Do not add anonymous inner types as their own type. */
  ...
  if ((DDR_RC_OK == rc) && !addFieldsOnly) {
    for (vector<UDT*>::iterator v = u->_subUDTs.begin(); v != u->_subUDTs.end();
         ++v) {
      rc = (*v)->buildBlob(this, false, "");
      if (DDR_RC_OK != rc) {
        break;
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(ClassUDT* cu,
                                     bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
  /* Do not add anonymous inner types as their own type. */
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(NamespaceUDT* ns,
                                     bool addFieldsOnly,
                                     string prefix) {
  DDR_RC rc = DDR_RC_OK;
...
DDR_RC
JavaBlobGenerator::addBlobField(Field* f,
                                uint32_t* fieldCount,
                                uint32_t* constCount) {
...
DDR_RC
JavaBlobGenerator::addBlobConst(string name,
                                long long value,
                                uint32_t* constCount) {
  DDR_RC rc = DDR_RC_OK;
  uint32_t offset = UINT32_MAX;
...
DDR_RC
JavaBlobGenerator::addBlobStruct(string name,
                                 string superName,
                                 uint32_t constCount,
                                 uint32_t fieldCount,
                                 uint32_t size) {
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(Type* type,
                                     bool addFieldsOnly,
                                     string prefix) {
  /* No op: base types are not added to the blob at this time. */
  return DDR_RC_OK;
...
DDR_RC
JavaBlobGenerator::dispatchBuildBlob(TypedefUDT* type,
                                     bool addFieldsOnly,
                                     string prefix) {
  /* No op: base types are not added to the blob at this time. */
  return DDR_RC_OK;
