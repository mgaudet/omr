...
int main(int argc, char *argv[]) {
  ...
  if (sizeof(blobHdrV1) != rb) {
    ERRMSG("read blob header returned %lu, expected %lu\n", (unsigned long)rb,
           (unsigned long)sizeof(blobHdrV1));
    return -1;
  }
  ...
  size_t blobLength = sizeof(blobHdrV1) + blobHdrV1.structDataSize +
  ...
  /* Read strings */
  {
    ...
    const uint8_t *const stringDataStart =
        blobBuffer + sizeof(blobHdrV1) + blobHdrV1.structDataSize;
    ...
    while (currentString < stringDataEnd) {
       ...
       */
      printf("%5u: %8lx [%u] %.*s\n", stringNum,
             (long unsigned int)(currentString - stringDataStart),
