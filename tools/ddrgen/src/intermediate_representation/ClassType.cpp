...
ClassType::ClassType(SymbolType symbolType, size_t size,
...
bool ClassType::isAnonymousType() { return _name.empty(); }

bool ClassType::equal(Type const &type, set<Type const *> *checked) const {
  bool ret = false;
  if (checked->find(this) != checked->end()) {
    ...
    if (NULL != classType) {
      bool fieldsEqual =
          _fieldMembers.size() == classType->_fieldMembers.size();
      if (fieldsEqual) {
      ...
      bool enumMembersEqual =
          _enumMembers.size() == classType->_enumMembers.size() && fieldsEqual;
      if (enumMembersEqual) {
      ...
      ret = (NamespaceUDT::equal(type, checked)) && (fieldsEqual) &&
            (enumMembersEqual);
    }
  }
  return ret;
...
void ClassType::replaceType(Type *typeToReplace, Type *replaceWith) {
