...
bool NamespaceUDT::equal(Type const &type, set<Type const *> *checked) const {
  bool ret = false;
...
void NamespaceUDT::replaceType(Type *typeToReplace, Type *replaceWith) {
...
DDR_RC
NamespaceUDT::buildBlob(BlobGenerator *blobGenerator, bool addFieldsOnly,
                        string prefix) {
  return blobGenerator->dispatchBuildBlob(this, addFieldsOnly, prefix);
}

DDR_RC
NamespaceUDT::printToSuperset(SupersetGenerator *supersetGenerator,
