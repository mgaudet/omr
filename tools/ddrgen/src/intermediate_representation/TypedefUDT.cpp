...
bool TypedefUDT::equal(Type const &type, set<Type const *> *checked) const {
  bool ret = false;
...
void TypedefUDT::replaceType(Type *typeToReplace, Type *replaceWith) {
...
DDR_RC
TypedefUDT::buildBlob(BlobGenerator *blobGenerator, bool addFieldsOnly,
                      string prefix) {
  return blobGenerator->dispatchBuildBlob(this, addFieldsOnly, prefix);
}

DDR_RC
TypedefUDT::printToSuperset(SupersetGenerator *supersetGenerator,
