...
string MacroTool::getFileName(string s) {
...
MacroTool::addMacrosToIR(Symbol_IR *ir) {
  ...
  for (vector<Type *>::iterator it = ir->_types.begin(); it != ir->_types.end();
       it += 1) {
  ...
  for (vector<MacroInfo>::iterator it = macroList.begin();
       it != macroList.end(); it += 1) {
    ...
    if (macroInfo->getNumMacros() > 0) {
      ...
      if (NULL == ns) {
        ...
        for (set<pair<string, string> >::iterator it =
                 macroInfo->getMacroStart();
             it != macroInfo->getMacroEnd(); ++it) {
          /* Check if the macro already exists before adding it. */
          bool alreadyExists = false;
          for (vector<Macro>::iterator it2 = ns->_macros.begin();
               it2 != ns->_macros.end(); ++it2) {
