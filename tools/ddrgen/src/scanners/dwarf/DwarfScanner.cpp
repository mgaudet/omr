...
DwarfScanner::getType(Dwarf_Die die, Dwarf_Half tag, Type** const newType, bool isSubUDT, int* typeNum)
{
     ...
     * Set typeNum to 1 for full types which have been found before as a stub--do not add to types list but add
     * children.
                ...
                for (vector<UDT*>::iterator it = ct->_subUDTs.begin(); it != ct->_subUDTs.end();) {
                    ...
                    if ((NULL != enumUDT) && (*it)->isAnonymousType()) {
                        ...
                        if (!usedAsField) {
                            ...
                            ct->_enumMembers.insert(
                                ct->_enumMembers.end(), eu->_enumMembers.begin(), eu->_enumMembers.end());
