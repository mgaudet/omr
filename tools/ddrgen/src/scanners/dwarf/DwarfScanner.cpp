  ...
  if (DDR_RC_OK != rc) {
    ...
    if (DDR_RC_OK ==
      ...
      if (dieName.empty() && !isSubUDT && (DW_TAG_enumeration_type == tag)) {
        ...
        dieName =
          fileName.substr(lastSlash + 1, lastDot - lastSlash - 1) + "Constants";
...
DwarfScanner::traverse_cu_in_debug_section(Symbol_IR* const ir)
{
  ...
  while (DDR_RC_OK == rc) {
    int ret =
      dwarf_next_cu_header(_debug, &cuHeaderLength, &versionStamp,
                           &abbrevOffset, &addressSize, &nextCUheader, &error);
    if (DW_DLV_ERROR == ret) {
                       ...
                       const char* filepath)
{
  ...
  if (DDR_RC_OK == rc) {
    ...
    if (DW_DLV_OK != res) {
      if (DW_DLV_ERROR == res) {
        ERRMSG(
          "Failed to Initialize libDwarf! DW_DLV_ERROR: res: %s\nExiting...\n",
          dwarf_errmsg(error));
        ...
        ERRMSG(
          "Failed to Initialize libDwarf! Unknown ERROR: res: %s\nExiting...\n",
          dwarf_errmsg(error));
      }
      if (NULL != error) {
        dwarf_dealloc(_debug, error, DW_DLA_ERROR);
                            ...
                            unsigned int* lineNumber, string* fileName)
{
  ...
  if (hasAttr) {
     ...
     */
    key->fileName =
      (char*)malloc(sizeof(char) * (strlen(_fileNamesTable[declFile - 1]) + 1));
