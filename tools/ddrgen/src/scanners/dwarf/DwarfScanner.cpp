  ...
  for (int i = 0; i < _fileNameCount; i += 1) {
    ...
    if (0 == strncmp(_fileNamesTable[i], "../", 3)) {
      ...
      fileNamesTableConcat[i] =
          (char*)malloc(sizeof(char) * (strlen(compDir) + strlen(path) + 1));
 ...
 */
DDR_RC
DwarfScanner::addType(Dwarf_Die die,
                      Dwarf_Half tag,
                      bool ignoreFilter,
                      bool isSubUDT,
                      Type** type) {
 ...
 */
DDR_RC
DwarfScanner::getType(Dwarf_Die die,
                      Dwarf_Half tag,
                      Type** const newType,
                      bool isSubUDT,
                      int* typeNum) {
 ...
 */
DDR_RC
DwarfScanner::createType(Dwarf_Die die,
                         Dwarf_Half tag,
                         string dieName,
...
DwarfScanner::scanFile(OMRPortLibrary* portLibrary,
  ...
  if (DDR_RC_OK == rc) {
    ...
    if (DW_DLV_OK != res) {
      if (DW_DLV_ERROR == res) {
        ERRMSG(
            "Failed to Initialize libDwarf! DW_DLV_ERROR: res: "
            "%s\nExiting...\n",
            dwarf_errmsg(error));
      } else if (DW_DLV_NO_ENTRY == res) {
        ERRMSG(
            "Failed to Initialize libDwarf! DW_DLV_NO_ENTRY: res: "
            "%s\nExiting...\n",
            dwarf_errmsg(error));
      } else {
        ERRMSG(
            "Failed to Initialize libDwarf! Unknown ERROR: res: "
            "%s\nExiting...\n",
            dwarf_errmsg(error));
      }
      if (NULL != error) {
...
DwarfScanner::~DwarfScanner() {
  for (unordered_map<TypeKey, Type*, KeyHasher>::iterator it = _typeMap.begin();
       it != _typeMap.end(); it++) {
...
DDR_RC
DwarfScanner::createTypeKey(Dwarf_Die die,
                            string typeName,
                            TypeKey* key,
                            unsigned int* lineNumber,
                            string* fileName) {
