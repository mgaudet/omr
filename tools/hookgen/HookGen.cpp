 ...
 */
void
HookGen::writeEventToPublicHeader(const char* name, const char* description,
 ...
 */
void
HookGen::writeEventToPrivateHeader(const char* name, const char* condition,
                                   ...
                                   pugi::xml_node event)
{
  ...
  for (pugi::xml_node data = event.child("data"); data;
    ...
    if (data.attribute("return").as_bool()) {
      fprintf(
        _privateFile, "\t\t(arg_%s) = eventData.%s; /* return argument */ \\\n",
