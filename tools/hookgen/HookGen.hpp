...
private:
  ...
  const char*
    _publicFileName; /**< value (or substring) from XML parsing so no need to
                        free */
  FILE*
    _publicFile; /**< FILE handle for public header file.  close in tearDown if
                    non-NULL */
  const char*
    _privateFileName; /**< value (or substring) from XML parsing so no need to
                         free */
