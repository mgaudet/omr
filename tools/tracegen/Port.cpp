...
char* Port::omrfile_realpath(const char* utf8Path)
{
     ...
     * conversion from a
     * relative path to a fully qualified path depends on the current working directory (CWD), which is a process global
     * that may change.
     *
     * However, we can't simply limit the use of GetFullPathNameW to non-absolute paths, because all occurrences of "."
     * and "..",
     * need to be removed. This includes the occurrences that don't simply prepend the path, in which case the CWD is
     * not relevant and
     * where we assume the call to GetFullPathNameW is thread-safe.
     *
     */
