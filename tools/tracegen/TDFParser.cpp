...
TDFParser::parse()
{
  ...
  if (NULL == tdf->header.datfilename) {
    ...
    strncpy(tdf->header.datfilename, DEFAULT_DAT_FILE,
            strlen(DEFAULT_DAT_FILE));
  }
...
RCType
TDFParser::processTracePointDetail(const char* line, J9TDFTracepoint* tp,
...
TDFParser::getTemplate(const char* line, const char* key)
{
   ...
   * Note that the template string should include the quotes as it will be
   * included in
   * #defines later on!
   */
  while (0 != *pos) {
 ...
 */
RCType
TDFParser::processTemplate(const char* formatTemplate, char* str,
...
void
TDFParser::flagTdfSyntaxError(const char* message, unsigned int parmCount,
