...
TraceHeaderWriter::tpTemplate(FILE *fd, unsigned int overhead, unsigned int test, const char *name, const char *module,
							  unsigned int id, unsigned int envParam, const char *parameters, unsigned int parmCount,
							  unsigned int auxiliary)
{
	...
	if (auxiliary) {
		...
		if (0 <= fprintf(fd, TP_TEMPLATE, overhead, testMacro, name, envParam ? "thr" : "",
						 ...
						 testNop, name, envParam ? "thr" : "", envParam ? parmString : parmStringNoLeadingComma, module,
						 id)) {
			rc = RC_OK;
		} else {
			rc = RC_FAILED;
							...
							unsigned int id, unsigned int envParam, const char *conditionStr, unsigned int parmCount)
{
	...
	if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name, parmStringNoLeadingComma, module, id, module,
					 id, conditionStr, module, module, envParam ? "thr" : "(void *)NULL", module, id, module, id,
					 conditionStr, module, id, name, conditionStr, testnop, name, parmStringNoLeadingComma, module,
					 id)) {
		rc = RC_OK;
	} else {
		rc = RC_FAILED;
