...
const char* TP_ASSERT_TEMPLATE
    = "#if UT_TRACE_OVERHEAD >= %u\n"
      ...
      "				%s_UtModuleInfo.intf->Trace(%s, &%s_UtModuleInfo, (UT_SPECIAL_ASSERTION | (%uu "
      "<< 8) | %s_UtActive[%u]), \"\\377\\4\\377\", __FILE__, __LINE__, UT_STR((%s))); \\\n"
      "				Trace_Unreachable(); \\\n"
      "			} else { \\\n"
    ...
    const char* module, unsigned int id, unsigned int envParam, const char* conditionStr, unsigned int parmCount)
{
    ...
    if (0 <= fprintf(fd, TP_ASSERT_TEMPLATE, overhead, testmacro, name, parmStringNoLeadingComma, module, id, module,
                 ...
                 conditionStr, module, id, name, conditionStr, testnop, name, parmStringNoLeadingComma, module, id)) {
        rc = RC_OK;
    } else {
        rc = RC_FAILED;
        goto failed;
