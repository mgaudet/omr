...
static const uint32_t primesTable[] = {
    17, 37, 73, 149, 293, 587, 811, 1171, 1693, 2347, 3511, 4691, 9391, 18787, 37573, 75149, 150299, 300007, 600071,
    1200077, 2200103,
};
 ...
 * it
 * 							must be easily searchable, i.e. there should not be more than a few false
 * hits
 * 							when searching for the name. If necessary to ensure uniqueness or
 * find-ability,
 * 							create a list of the unique names or name conventions in this
 * file.
 ...
 * Note also that lists may not be transformed into trees if there was not enough memory to allocate the nodes.  In
 * other words lists of length greater
...
void hashTableRehash(J9HashTable* table)
{
    ...
    if (J9HASH_TABLE_COLLISION_RESILIENT == (table->flags & J9HASH_TABLE_COLLISION_RESILIENT)) {
         ...
         * vice-versa) and
         * it's impossible to predict the required pool capacities.  One might expect this is a just a special case of
         * collisionResilientHashTableGrow,
         * however the problem is much harder as you cannot simply rebuild the original hash table on failure as the
         * entries' hashcodes may
