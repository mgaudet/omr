 ...
 * not
 * necessary for the element to be currently allocated (ie: it could be a free
 * slot).
 *
 ...
 */
J9Pool*
pool_new(uintptr_t structSizeArg, uintptr_t numberElementsArg,
         uintptr_t elementAlignmentArg, uintptr_t poolFlags,
         const char* poolCreatorCallsite, uint32_t memoryCategory,
         omrmemAlloc_fptr_t memAlloc, omrmemFree_fptr_t memFree, void* userData)
{
  uint32_t doInit;
   ...
   * +--------------+-------------+--------------------+
   * +-------------+------------+-------------+------------+-----+
   * | J9PoolPuddle | puddle bits | alignment overhead |   | 1st element |
   * puddle SRP | 2nd element | puddle SRP | ... |
   * +--------------+-------------+--------------------+
   * +-------------+------------+-------------+------------+-----+
   *
 ...
 */
void
pool_do(J9Pool* pool, void (*doFunction)(void* anElement, void* userData),
        void* userData)
{
  void* anElement;
  pool_state aState;
 ...
 *be
 *  specified as the start point and also provides the option to only walk that
 *puddle.
 ...
 */
void*
poolPuddle_startDo(J9Pool* pool, J9PoolPuddle* currentPuddle, pool_state* state,
